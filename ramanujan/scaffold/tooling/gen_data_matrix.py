...
from ramanujan.layout import BOUNDARY_ROOT, DATA_ROOT, INTAKE_ROOT, REPO_ROOT, resolve_ramanujan_path


ROOT = REPO_ROOT
MANIFEST = BOUNDARY_ROOT / "RAMANUJAN_OFFLINE_MANIFEST.json"
MATRIX = INTAKE_ROOT / "RAMANUJAN_DATA_SOURCE_MATRIX.json"
QUEUE = INTAKE_ROOT / "RAMANUJAN_ACQUISITION_QUEUE.json"
GENERATION_RECEIPT = DATA_ROOT / "corpora" / "GENERATION_RECEIPT.json"
CONTAMINATION_RECEIPT = DATA_ROOT / "corpora" / "CONTAMINATION_RECEIPT.json"
...
def _verified_current_snapshot(receipt: dict, *, root: Path = ROOT) -> dict[str, str]:
    """Bind live JSONL bytes only when reproducible content still matches."""
    locator = receipt.get("local_offline_location")
    expected_content = receipt.get("content_digest")
    if not isinstance(locator, str) or not locator or not isinstance(expected_content, str):
        raise RuntimeError("present local source lacks location or reproducible content digest")
    path = resolve_ramanujan_path(locator, repo_root=root)
    if not path.is_file():
        raise RuntimeError(f"present local source is missing: {path}")
    ...
    for number, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        try:
            ...
            raise RuntimeError(f"local source is invalid JSONL: {path}:{number}") from exc
        content = item.get("content_hash") if isinstance(item, dict) else None
        if not isinstance(content, str) or not content:
            raise RuntimeError(f"local source lacks content_hash: {path}:{number}")
        ordered_hashes.append(content)
    digest = hashlib.sha256()
    for content in ordered_hashes:
        digest.update(content.encode("utf-8"))
        digest.update(b"\n")
    if digest.hexdigest() != expected_content:
        raise RuntimeError(f"local source content drifted from generation receipt: {path}")
    return {
        "path": str(path),
...
def build() -> tuple[dict, dict]:
    ...
    for s in man["sources"]:
        ...
        if receipt and receipt.get("n_items", 0) > 0:
            current_snapshot = _verified_current_snapshot(receipt)
            ...
            # Timestamped provenance may alter the file SHA after a clean
            # re-derivation; bind the current bytes only after stable content
            # hashes match the generation receipt above.
            bindings["hash"] = current_snapshot["sha256"]
        ...
        if n_items is not None:
            ...
            row["content_digest"] = receipt.get("content_digest")
        rows.append(row)
