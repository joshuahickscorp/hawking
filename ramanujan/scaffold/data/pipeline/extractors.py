...
def extract_d4(
    ...
    *,
    ...
    # Process pools need semaphores that some sandboxes refuse.  Candidate
    # order, rather than subprocess completion order, is the membership
    # authority: a faster late Lean process must not alter a frozen corpus.
    # We can still run the checks concurrently, but consume their results in
    # the original candidate order and only then stop at ``limit`` successes.
    if workers == 1 or len(candidates) <= 1:
        ...
        with ThreadPoolExecutor(max_workers=workers) as ex:
            futures = [ex.submit(_run_lean_check, candidate) for candidate in candidates]
