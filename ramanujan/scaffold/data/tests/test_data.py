...
import ramanujan.data.extractors as extractors
...
from ramanujan.gen_data_matrix import _verified_current_snapshot
...
class TestExtractors(unittest.TestCase):
    ...
    def test_d4_parallel_selection_uses_candidate_order_not_completion_order(self):
        def successful(candidate):
            import time

            # Force later candidates to complete before the first one.  The
            # expected result remains the serial candidate-order selection.
            tag = candidate[-1]
            if tag.endswith("replace_tactic"):
                time.sleep(0.02)
            return {
                "import": candidate[0],
                "signature": candidate[1],
                "broken_proof": candidate[2],
                "fix_proof": candidate[3],
                "tag": tag,
                "error": "synthetic Lean error",
                "lean_returncode": 1,
            }

        with patch.object(extractors, "_run_lean_check", side_effect=successful):
            serial = extractors.extract_d4([], limit=4, workers=1)
        with patch.object(extractors, "_run_lean_check", side_effect=successful):
            parallel = extractors.extract_d4([], limit=4, workers=3)

        self.assertEqual(
            [item["content_hash"] for item in parallel],
            [item["content_hash"] for item in serial],
        )
...
class TestDataMatrixSnapshotBinding(unittest.TestCase):
    def test_current_bytes_refresh_only_under_same_content_digest(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            source = root / "source.jsonl"
            source.write_text(
                json.dumps({"content_hash": "a" * 64, "provenance": {"at": "later"}}) + "\n",
                encoding="utf-8",
            )
            content = hashlib.sha256((("a" * 64) + "\n").encode()).hexdigest()
            receipt = {"local_offline_location": "source.jsonl", "content_digest": content}
            snapshot = _verified_current_snapshot(receipt, root=root)
            self.assertEqual(snapshot["sha256"], hashlib.sha256(source.read_bytes()).hexdigest())
            self.assertEqual(snapshot["content_digest"], content)

            source.write_text(json.dumps({"content_hash": "b" * 64}) + "\n", encoding="utf-8")
            with self.assertRaisesRegex(RuntimeError, "content drifted"):
                _verified_current_snapshot(receipt, root=root)
