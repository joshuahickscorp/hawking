...
LADDER_ORDER = ["R0_parent", "A1_1p0", "R2_subhalf_best", "R1_c1_corrected", "A2_0p85",
    ...
    for L in range(start, n_layers):
        ...
        for key, h in hs.items():
            ...
            for p in range(h.shape[0]):
                idx, wts = Q.route_topk(lg[p], g.top_k, g.norm_topk_prob)
        ...
        fit_ids = order[:max(1, int(fit_experts))]
        for e in fit_ids:
            if _cache_get(cache, (L, e)) is None:
                _cache_put(cache, (L, e), _read_expert(rd, L, e))
        fit_mats = {"gate_up": [], "down": []}
        for e in fit_ids:
            ex = _cache_get(cache, (L, e))
            fit_mats["gate_up"] += [ex["gate"], ex["up"]]
            fit_mats["down"].append(ex["down"])
        ...
        for v in packed_vars:
            for grp in ("gate_up", "down"):
                for cold in (False, True):
                    ...
                    if spec is None or spec.get("family") != "shared_grammar":
                        continue
        ...
        del hs, outs, need, books, fit_mats
