...
"""Live truth for the Hawking Gravity FRONTIER campaign - its `hawking status` equivalent.

This reads state, it never mutates it and never acquires-and-holds the controller lease.
Liveness is decided by the FLOCK, not by a PID field or a JSON file: a controller is RUNNING
only if a live process provably holds the singleton lease (com.hawking.gravity_frontier). A stale
PID, a dead process, or a committed/historical checkpoint.json can therefore never report RUNNING.

Reported truth:
  * lease liveness (flock held by a live pid) + the holder pid and its aliveness;
  * heartbeat freshness (age < 3 * interval);
  * real progress, RECOUNTED from the durable per-trial checkpoints (not trusted from the cursor);
  * derived state: RUNNING / DRAINING / PAUSED / COMPLETE / FAILED / NOT_STARTED;
  * the GEOMETRY FRONTIER: the lowest functional divergence seen so far and its winning family, plus
    the per-tensor-class frontier and (when present) the sealed FRONTIER_SELECTION.json winners;
  * PID, lease, program hash, current trial, completed/total, last checkpoint, heartbeat, ETA.
"""
...
from gravity_frontier_controller import (  # noqa: E402
    ...
    LEASE_LABEL, FrontierConfig, GravityFrontierController,
)

STATUS_SCHEMA = "hawking.gravity_frontier.status.v1"
...
def _lease_flock_held(lease_path: Path) -> tuple[bool, int | None, str | None]:
    """Return (held, holder_pid, acquired_at). `held` is True only when a LIVE process holds the
    exclusive flock (we probe non-blocking and immediately release if we win)."""
...
def snapshot(config: FrontierConfig) -> dict[str, Any]:
    controller = GravityFrontierController(config)
    program_ok = False
    ...
    state_counts = controller._collect_state() if program_ok else {
        ...
        "eta_seconds": None, "best_by_functional_divergence": None, "frontier_by_tensor_class": {}}
    ...
    return {
        ...
        "geometry_frontier": {
            "best_by_functional_divergence": state_counts["best_by_functional_divergence"],
...
def render_human(snap: dict[str, Any]) -> str:
    ...
    lease = snap["lease"]
    hb = snap["heartbeat"]
    gf = snap["geometry_frontier"]
    lines = [
        "GRAVITY FRONTIER :: GPT-OSS-120B geometry search",
        f"  state            {snap['state']}",
        ...
        f"  current_trial    {snap['current_row']}",
        ...
        f"  avg_trial        {snap['avg_row_seconds']}s   ETA {_fmt_eta(snap['eta_seconds'])}",
    ]
    best = gf.get("best_by_functional_divergence")
    if best:
        lines.append(f"  best_geometry    {best['family']} ({best['row_id']}) "
                     f"func_div={best['functional_divergence']} "
                     f"weight_err={best.get('weight_rel_error')} bpw={best.get('whole_artifact_bpw')}")
    for tc, entry in (gf.get("frontier_by_tensor_class") or {}).items():
        lines.append(f"  frontier[{tc}]  {entry['family']} func_div={entry['functional_divergence']}")
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Gravity Frontier live status (read-only truth).")
