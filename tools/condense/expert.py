...
"""expert.py - the MoE expert lane, two subcommands (was expert_sensitivity.py + expert_cache_policy.py).

  expert.py sensitivity <moe_dir> [--label L] [--bits 1,2] | expert.py sensitivity --synthetic [--label L]
  expert.py cache --sim <n_experts> <active_k> [--active-gb G] [--skew S] [--cache-frac F] [--ssd-gbps R] ...

sensitivity: per-expert bit-allocation probe (does MoE expert sensitivity vary enough to justify
  per-expert bits). The real MoE path is gated; --synthetic runs a self-test with fabricated routing.
cache: hot-expert cache-hit / blended-tok-s simulation across cache sizes, so the eventual OOC pager
  cache size is chosen from a measured sweep, not a guess. Pure projection; no serve-win claim.
"""
...
# ==================== cache (was expert_cache_policy.py) ====================
...
if __name__ == "__main__":
    ...
    if _sub == "sensitivity":
        sys.argv = ["expert_sensitivity.py"] + sys.argv[2:]
        main()
    elif _sub == "cache":
        sys.argv = ["expert_cache_policy.py"] + sys.argv[2:]
        _main_cache()
    else:
        print(__doc__)
