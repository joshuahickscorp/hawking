...
def test_imports_terminal_skips_running(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_imports_terminal_skips_running'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
...
def test_seal_tamper_flagged(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_seal_tamper_flagged'
...
def test_wrong_plan_refused(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_wrong_plan_refused'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
...
def test_missing_status_does_not_break_seal(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_missing_status_does_not_break_seal'
    doc = seal_document({'campaign': 'retired', 'n': 1})
    verify_document_seal(doc)
    bad = dict(doc)
    bad['n'] = 2
...
def test_queue_plan_mismatch_refused(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_queue_plan_mismatch_refused'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
