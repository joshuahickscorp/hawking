...
FAMILY = 'succession'
RETIRED_MODULES = ['succ_engine', 'succ_harvest', 'succ_retire']


def test_harvest_selftest(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_harvest_selftest'
...
def test_retire_selftest(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_retire_selftest'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
...
def test_classification_never_conflates_deferral_with_collapse(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_classification_never_conflates_deferral_with_collapse'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
...
def test_eta_observations_are_per_segment_not_global(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_eta_observations_are_per_segment_not_global'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
...
def test_engine_skips_retired_experiments(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_engine_skips_retired_experiments'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
...
def test_retirement_receipts_preserve_evidence_and_reopening(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_retirement_receipts_preserve_evidence_and_reopening'
