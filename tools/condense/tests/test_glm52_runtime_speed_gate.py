...
FAMILY = 'glm52'
RETIRED_MODULES = ['glm52_runtime_speed_gate']


def test_milestone_ladder_has_no_artificial_ceiling(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_milestone_ladder_has_no_artificial_ceiling'
...
def test_receipt_binds_exact_index_and_reconciles_tps(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_receipt_binds_exact_index_and_reconciles_tps'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
...
def test_receipt_refuses_wrong_index_or_unreconciled_rate(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_receipt_refuses_wrong_index_or_unreconciled_rate'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
...
def test_sustained_command_requests_curve_and_progress(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_sustained_command_requests_curve_and_progress'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
