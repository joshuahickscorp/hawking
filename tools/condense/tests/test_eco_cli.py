...
FAMILY = 'eco'
RETIRED_MODULES = ['eco_activation', 'eco_cli', 'eco_import']


def test_selftest_all_green(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_selftest_all_green'
...
def test_pipeline_valid(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_pipeline_valid'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
...
def test_passport_selftest(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_passport_selftest'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
...
def test_admission_no_prior(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_admission_no_prior'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
...
def test_import_and_plan_and_materialize_against_mini_campaign(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_import_and_plan_and_materialize_against_mini_campaign'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
...
def test_activation_gate_refuses_running(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_activation_gate_refuses_running'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
