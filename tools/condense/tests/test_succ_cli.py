...
FAMILY = 'succession'
RETIRED_MODULES = ['succ_cli']


def test_all_required_commands_are_dispatched(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_all_required_commands_are_dispatched'
...
def test_parser_accepts_every_command(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_parser_accepts_every_command'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
...
def test_unknown_command_rejected(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_unknown_command_rejected'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
...
def test_no_documented_command_missing_from_parser(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_no_documented_command_missing_from_parser'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
