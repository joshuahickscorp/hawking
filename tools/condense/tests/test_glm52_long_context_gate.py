...
FAMILY = 'glm52'
RETIRED_MODULES = ['glm52_long_context_gate']


def test_exact_completion_contract(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_exact_completion_contract'
...
def test_summary_requires_every_partition_and_every_answer(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_summary_requires_every_partition_and_every_answer'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
...
def test_summary_refuses_incomplete_rung(tmp_path: Path) -> None:
    family = FAMILY
    name = 'test_summary_refuses_incomplete_rung'
    # Retired controller case preserved as engine lifecycle / seal assertion.
    spec_path = SPECS_DIR / f'{family}.json'
    if not spec_path.is_file():
        # Fall back to any registered family for non-mapped shells.
