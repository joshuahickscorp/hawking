...
zero." ``hawking_metric`` currently sums ``prefill_profile.totals.wall_ns`` with a
...
from tools import hawking_metric
...
def test_fully_instrumented_unit_reports_real_seconds():
    ...
    assert hawking_metric.prefill_seconds(cs) == pytest.approx(7.0)
    assert hawking_metric.decode_seconds(cs) == pytest.approx(7.0)


def test_one_uninstrumented_call_makes_the_whole_prefill_unknown():
    cs = [_call(10.0, 6_000_000_000), _call(4.0, None)]
    assert hawking_metric.prefill_seconds(cs) is None, (
...
def test_unknown_prefill_is_not_reattributed_to_decode():
    cs = [_call(10.0, 6_000_000_000), _call(4.0, None)]
    assert hawking_metric.decode_seconds(cs) is None, (
...
def test_a_unit_with_no_calls_at_all_is_unknown_not_zero():
    assert hawking_metric.prefill_seconds([]) is None
    assert hawking_metric.decode_seconds([]) is None
...
# `replace_file` that cut tools/hawking_metric.py from 202 lines to 72, deleted the
...
def test_the_dashboard_itself_survives():
    ...
    import tools.hawking_metric as m

    for name in ("main", "load", "accepted", "calls", "interventions",
                 "regime_of", "REGIMES", "RECEIPTS", "LEDGER"):
        assert hasattr(m, name), (
            f"tools/hawking_metric.py lost {name!r}; a mutation that adds two "
...
def test_no_undefined_names_survive_py_compile():
    ...
    src = pathlib.Path(__file__).resolve().parent.joinpath("hawking_metric.py").read_text()
    tree = ast.parse(src)
...
def test_main_still_runs_end_to_end():
    ...
    proc = subprocess.run(
        [sys.executable, str(root / "tools" / "hawking_metric.py"), "0"],
        capture_output=True, text=True, timeout=120, cwd=str(root),
    )
    assert proc.returncode in (0, 1), (
        f"hawking_metric.py exited {proc.returncode}\n{proc.stderr[-1500:]}"
    )
