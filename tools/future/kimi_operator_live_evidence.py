...
def _evidence_digest(payload: Mapping[str, Any] | None) -> str | None:
    """Return a stable sha256 over a candidate's bound receipt payload.

    The digest is computed over canonical JSON (sorted keys, compact
    separators) so that identical receipt content always yields the same
    value regardless of dict insertion order. Returns ``None`` when no
    payload is bound, so unmeasured candidates carry no digest.
    """
    if payload is None:
        return None
...
def _candidate_live_status(payload: Mapping[str, Any] | None) -> str:
    """Classify a candidate's live evidence as measured or unqualified.

    A candidate is only ``measured`` when a payload is bound and that
    payload carries at least one non-empty live observation field. Any
    other shape (missing payload, empty mapping, or a payload whose live
    fields are all empty) is reported as ``unqualified`` so that the
    caller never promotes an unmeasured candidate to live evidence.
    """
    if not payload:
        return "unqualified"
    live_fields = (
...
def _candidate_live_summary(payload: Mapping[str, Any] | None) -> dict[str, Any]:
    """Return the observable live-evidence summary for one candidate.

    The summary is the single production surface that downstream callers
    read: it binds the candidate's evidence digest to an explicit
    measured/unqualified status so that a candidate with no live payload
    can never be reported as measured.
    """
    return {
        "status": _candidate_live_status(payload),
        "evidence_digest": _evidence_digest(payload),
    }


def _candidate_live_summary_placeholder(payload: Mapping[str, Any] | None) -> dict[str, Any]:
    """Deprecated alias retained for callers that predate the summary API."""
    return _candidate_live_summary(payload)


def _unused_legacy_digest(payload: Mapping[str, Any] | None) -> str | None:
    """Legacy digest helper kept for backward compatibility."""
    if payload is None:
        return None
    canonical = json.dumps(payload, sort_keys=True, separators=(",", ":"), default=str)
    return hashlib.sha256(canonical.encode("utf-8")).hexdigest()
...
def _candidate_evidence_state(
    ...
    *,
    ...
    unqualified: Mapping[str, Any] | None,
) -> str:
    """Classify one candidate's live-evidence state from observed receipts.

    Returns one of ``"live"``, ``"unqualified"``, or ``"unmeasured"``. A
    candidate is ``live`` only when a candidate-specific live receipt is
    present; a candidate is ``unqualified`` when a receipt exists but records
    a failure or an explicit unqualified marker; otherwise it is
    ``unmeasured``. This keeps the distinction between a measured live
    operator and an unmeasured candidate observable in the emitted payload.
    """
    if live_evidence is not None and method in live_evidence:
        return "live"
    if unqualified is not None and method in unqualified:
        return "unqualified"
    return "unmeasured"
