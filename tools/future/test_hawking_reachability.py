...
def _graph_from(registry_src: str):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        (fake / "hawking" / "tool_registry.py").write_text(registry_src)
        mod = REPO / "tools" / "future" / "hawking_reachability.py"
        out = subprocess.run(
            [sys.executable, "-c",
             "import sys,json,pathlib; sys.path.insert(0,%r); "
             "from hawking_reachability import build; "
             "print(json.dumps(build(pathlib.Path(%r))))"
             % (str(mod.parent), str(fake))],
            capture_output=True, text=True, check=False)
        if out.returncode:
            raise AssertionError(
                "scratch reachability graph failed:\n" + (out.stderr or out.stdout)
            )
        import json
...
def _drop_odyssey_ledger(src: str) -> str:
    """Remove the ledger source and its current consolidated public op."""
    src = _drop_toolspec(src, "odyssey.ledger")
    rewritten, count = re.subn(r'\s*"ledger": "odyssey\.ledger",', "", src)
    if count != 1:
        raise AssertionError("odyssey.read ledger mapping changed -- test is stale")
    return rewritten


def main() -> int:
    src = (REPO / "hawking" / "tool_registry.py").read_text()
    fails = []
    ...
    g2 = _graph_from(_drop_odyssey_ledger(src))
