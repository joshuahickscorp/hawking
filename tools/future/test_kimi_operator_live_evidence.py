from tools.future.kimi_operator_live_evidence import _evidence_digest, build


def test_evidence_digest_is_stable_and_order_independent():
    a = {"status": "OPH_DEVICE_BUSY", "candidate_method": "OPH"}
    b = {"candidate_method": "OPH", "status": "OPH_DEVICE_BUSY"}
    assert _evidence_digest(a) == _evidence_digest(b)
    assert _evidence_digest(a) is not None
    assert len(_evidence_digest(a)) == 64
    assert _evidence_digest(None) is None
    assert _evidence_digest(a) != _evidence_digest({"status": "OPH_DEVICE_BUSY"})
...
def test_candidate_physical_parity_is_bound_to_candidate_without_qualifying_it_alone():
    ...
    assert result["qualified_candidates"] == []
def test_candidate_evidence_state_classifies_live_unqualified_and_unmeasured():
    from tools.future.kimi_operator_live_evidence import _candidate_evidence_state

    assert _candidate_evidence_state(
        "OPE", live_evidence={"OPE": {"ok": True}}, unqualified=None
    ) == "live"
    assert _candidate_evidence_state(
        "OPH", live_evidence={}, unqualified={"OPH": {"reason": "device_busy"}}
    ) == "unqualified"
    assert _candidate_evidence_state(
        "OPG", live_evidence={}, unqualified={}
    ) == "unmeasured"
    assert _candidate_evidence_state(
        "OPA", live_evidence=None, unqualified=None
    ) == "unmeasured"