"""The Hawking scientific reachability graph (G025).

Sixteen autonomy rounds surfaced sixteen defects and most were one family: a
tool Hawking could not reach, a required argument it could not construct from any
...
REGISTRY = REPO / "hawking" / "tool_registry.py"
ENGINE = REPO / "hawking" / "engine.py"
...
def build(repo: Path = REPO) -> Dict[str, Any]:
    registry_path = repo / "hawking" / "tool_registry.py"
    engine_path = repo / "hawking" / "engine.py"
    ...
    # audit. The live registry is source truth -- it is the same object Hawking
    ...
    if repo != REPO.resolve():
        # The verifier calls build() in a fresh subprocess after placing a
        # complete scratch ``hawking`` package first on sys.path.  Evicting
        # only that package family keeps its relative imports coherent instead
        # of executing tool_registry.py as an orphan module.
        for module_name in tuple(sys.modules):
            if module_name == "hawking" or module_name.startswith("hawking."):
                del sys.modules[module_name]
    if str(repo) not in sys.path:
        sys.path.insert(0, str(repo))
    importlib.invalidate_caches()
    mod = importlib.import_module("hawking.tool_registry")
    live = mod.default_tool_registry(str(repo), repo_root=str(repo))
    public_specs = {item["name"]: item for item in live.discover()}
    raw_specs = getattr(live, "_tools", None)
    if not isinstance(raw_specs, dict):
        raise RuntimeError("Hawking registry does not expose its registered tool table")
    # Discover() deliberately hides absorbed members. This audit must still
    # follow their real handler and per-op schema; otherwise one public
    # Hawking command would turn its constituent capabilities into invisible
    # unknowns. The output labels every member with the single public command
    # that now owns it.
    consolidated_ops = {
        target: {"public_name": public_name, "operation": operation}
        for public_name, group in getattr(mod, "CONSOLIDATION", {}).items()
        for operation, target in group.get("ops", {}).items()
    }
    specs = {
        name: {
            ...
            **consolidated_ops.get(name, {}),
        }
        for name, spec in raw_specs.items()
    }

    # Handler return keys come from the AST. Prefer the actual registered
    # callable's name, then retain the literal-registration fallback for
    ...
    for name, d in specs.items():
        ...
        hname = d.get("handler") or ast_handlers.get(name)
        ...
        tools[name] = {
            ...
            "public_name": d.get("public_name") or d.get("alias_of") or name,
    ...
    caller_roots = [repo / "hawking", repo / "workspace" / "campaign", repo / "tools" / "future"]
    corpus = []
    ...
    for n, t in sorted(tools.items()):
        if t["public_name"] != n or n in agentic:
            continue
        if n not in blob:
        ...
        for arg in t["required_args"]:
            ...
            if not srcs and deep:
                ...
                findings["REQUIRED_ARGUMENT_WITH_NO_SOURCE"].append(
                    {"tool": t["name"], "arg": arg,
                     "note": "no reachable tool emits a key of this name at any depth; Hawking "
    ...
    return {
        "source_of_truth": "hawking.tool_registry.default_tool_registry(...), including absorbed members mapped to their public Hawking command",
        "n_tools": len(tools),
        "n_public_tools": len(public_specs),
        ...
        "n_registered_not_in_agentic_catalog": len([t for t in public_specs if t not in agentic]),
...
if __name__ == "__main__":
    g = build()
    out = REPO / "receipts" / "future" / "G025_HAWKING_REACHABILITY_GRAPH.json"
