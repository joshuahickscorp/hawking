...
def test_discover_heldout_gates_excludes_accepted_and_preserves_order():
    from tools.future.kimi_operator_gate import discover_heldout_gates

    candidates = ["g_alpha", "g_beta", "g_gamma", "g_delta"]
    accepted = ["g_beta", "g_delta"]

    result = discover_heldout_gates(candidates, accepted)

    assert [entry["gate"] for entry in result] == ["g_alpha", "g_gamma"]
    assert all(entry["status"] == "heldout" for entry in result)
    assert all(entry["reason"] == "not_in_accepted_set" for entry in result)


def test_discover_heldout_gates_empty_when_all_accepted():
    from tools.future.kimi_operator_gate import discover_heldout_gates

    assert discover_heldout_gates(["g_a", "g_b"], ["g_a", "g_b"]) == []