#!/usr/bin/env python3
"""Representation-native Nova fan-out on a real KIMI MoE activation corpus.

Each arm learns the deployed factors ``U``, ``V`` and expert cores ``C_e``
against teacher pairs ``(x, W_e x)``.  The dense parent is used only as a
teacher during fitting and held-out scoring; the direct consumer is
``U(C_e(V^T x))``.  Optimizer state is training state, not hidden deployed
representation bytes, and is not counted in complete EBPW.
"""
...
from tools.future.kimi_shared_tucker_fanout import (  # noqa: E402
    _direct,
    _fit_tucker,
)

SCHEMA = "hawking.future.kimi_representation_native_nova_fanout.v1"
EXPERT_COUNT = 8
TENSOR_SUFFIX = "up_proj.weight"
RANK_PAIRS = (
    (8, 8), (16, 16), (32, 16), (16, 32), (32, 32),
    (48, 32), (48, 48), (64, 32), (64, 48), (64, 64),
)
SAMPLE_ROWS = 128
TRAIN_STEPS = 300
LEARNING_RATE = 0.02
...
def _split_corpus(path: Path, expert_count: int, width: int):
    with np.load(path, allow_pickle=False) as corpus:
        ...
        for expert in range(expert_count):
            ...
            if arr.ndim != 2 or arr.shape[1] != width or arr.shape[0] < 2:
                raise RuntimeError(f"invalid corpus rows for {key}: {arr.shape}")
            # Interleaved split prevents the held-out set from being only a
            # late prompt suffix while preserving deterministic provenance.
            tr = arr[::2].copy()
            ho = arr[1::2].copy()
            if ho.shape[0] == 0:
                ho = tr[-1:].copy()
            ...
            train.append(tr)
            heldout.append(ho)
            counts[str(expert)] = {"train": int(tr.shape[0]), "heldout": int(ho.shape[0])}
    return train, heldout, counts


def _one(weights, train, heldout, names: list[str], row_rank: int, col_rank: int,
         sample_rows: int, steps: int, learning_rate: float):
    import torch
    ...
    optimizer = torch.optim.Adam([u, v, *cores], lr=learning_rate)
    ...
    with torch.enable_grad():
        for step in range(steps):
            ...
            for weight, core, xs in zip(weights, cores, train):
    ...
    cosine: list[float] = []
    with torch.no_grad():
        for weight, core, xs in zip(weights, cores, heldout):
            for row in xs:
        ...
        def direct_batch():
            return [_direct(u, v, core, torch.from_numpy(xs[0]))
                    for core, xs in zip(cores, heldout)]
    ...
    candidate = candidate_from_parts(
        family_id=f"KIMI_representation_native_nova_u{row_rank}_v{col_rank}",
        ...
        parts={
            ...
            "residuals": [],
            "runtime_auxiliaries": [{"name": "direct_nova_decoder_support",
    ...
    return {
        ...
        "status": "REPRESENTATION_NATIVE_NOVA_HELDOUT_REFERENCE_PASS",
        "tensor_names": names,
        "training": {"steps": steps, "learning_rate": learning_rate,
                      "initial_final_normalized_loss": losses,
                      "optimizer_state_deployed": False},
        "representation": {
            "family": "learned_shared_output_basis_expert_core_shared_input_basis",
        ...
        "timing": {
            "factor_initialization_ms": fit_ms,
            "training_ms": fit_ms,
...
def main() -> int:
    ...
    entry = require_ok("representation-native Nova organ fanout", expected_gb=2.0).as_dict()
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        futures = [pool.submit(_one, weights, train, heldout, names, row_rank, col_rank,
                               args.sample_rows, args.steps, args.learning_rate)
    ...
    out = {
        "schema": SCHEMA,
        "status": "REPRESENTATION_NATIVE_NOVA_FANOUT_COMPLETE",
        ...
        "fit": {"rank_pairs": RANK_PAIRS, "sample_rows_per_tensor": args.sample_rows,
        ...
        "claim_boundary": (
            ...
            "targets only; deployed factors are consumed without dense-parent materialization and "
            "held-out real routed inputs are measured. This is not full-model capability, end-to-end "
            "TPS, a production kernel, NX, or promotion evidence."
        ),
        "next_discriminator": "If learned factors improve held-out output materially, add organ capability and native kernel checks; otherwise the factorization is closed and the next move is a richer architecture or teacher-driven topology change.",
