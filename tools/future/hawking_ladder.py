#!/usr/bin/env python3
"""Drive the resident DOWN a BPW ladder through Hawking (G012, G013 / S005).
...
CONSTITUTION = (W / "hawking" / "CONSTITUTION.txt").read_text().rstrip() + "\n\n"
LOG = W / "receipts/future/O003_HAWKING_LADDER.jsonl"
...
def ask(prompt: str, timeout: int = 1800) -> str:
    ...
    r = subprocess.run(
        [sys.executable, "-u", "-m", "hawking", "1", "--task-file", str(pf),
         "--model", str(PROFILE)],
