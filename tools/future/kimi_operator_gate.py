...
def discover_heldout_gates(candidate_gates, accepted_gates):
    """Return candidate gates not already accepted, in deterministic order.

    A held-out gate is any candidate gate identifier that is absent from the
    accepted set. Ordering follows the candidate sequence so results are
    reproducible across runs. Each entry records an explicit skip reason when
    the gate is already accepted.
    """
    accepted = set(accepted_gates)
    discovered = []
    for gate in candidate_gates:
        if gate in accepted:
            continue
        discovered.append({"gate": gate, "status": "heldout", "reason": "not_in_accepted_set"})
    return discovered