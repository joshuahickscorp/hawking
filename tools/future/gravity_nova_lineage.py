...
def validate(
    ...
    *,
    ...
    declared_axes = lineage.get("measurement_axes")
    if declared_axes is not None:
        if not isinstance(declared_axes, (list, tuple, set)):
            errors.append("measurement_axes must be a sequence of axis names")
        else:
            declared = {str(axis) for axis in declared_axes}
            unknown = sorted(declared - set(MEASUREMENT_AXES))
            if unknown:
                errors.append(f"measurement_axes contains unknown axes: {unknown!r}")
            if len(declared) != len(declared_axes):
                errors.append("measurement_axes must not repeat an axis name")
            for axis in MEASUREMENT_AXES:
                if axis not in declared:
                    missing.append(f"measurement_axes.{axis}")
...
def lineage_chain_depth(records):
    """Return the maximum lineage chain depth across the given records.

    Each record is a mapping with an optional ``parent`` key referencing the
    ``id`` of another record. Records whose parent is absent or unknown are
    roots with depth 1. Cycles are treated as terminating at the repeated node
    so the function always returns a finite, deterministic integer.
    """
    by_id = {}
    ...
    def depth(rid, seen):
        ...
        if parent is None or parent not in by_id:
            return 1
        return 1 + depth(parent, seen | {rid})

    if not by_id:
        return 0
    return max(depth(rid, frozenset()) for rid in by_id)


def resolve_lineage_root(lineage, node_id):
    """Walk parent links from node_id to the lineage root.

    Returns (root_id, depth) where depth is the number of parent hops
    traversed. Raises KeyError if node_id is unknown and ValueError if a
    cycle is detected.
    """
    ...
    while True:
        if current in seen:
            raise ValueError("cycle detected at %r" % (current,))
        seen.add(current)
