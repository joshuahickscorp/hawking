...
def train(args: argparse.Namespace) -> int:
    ...
    # Pin MLX default device per --device. This must happen BEFORE any mx.array
    # is constructed so the head loads onto the right device.
    if args.device == "cpu":
        mx.set_default_device(mx.cpu)
        print("[train] device=CPU (no GPU contention — slower but parallel-safe)", file=sys.stderr)
    else:
        mx.set_default_device(mx.gpu)
        print("[train] device=GPU (Metal)", file=sys.stderr)
...
def main() -> int:
    ...
    p.add_argument("--device", default="gpu", choices=["gpu", "cpu"],
                   help="gpu: M-series Metal (default, fast). cpu: M-series CPU (~10-30x "
                        "slower but doesn't compete with concurrent GPU workloads like "
                        "an active capture). Useful for continuous-training while capture "
                        "runs.")
    # Schedule
