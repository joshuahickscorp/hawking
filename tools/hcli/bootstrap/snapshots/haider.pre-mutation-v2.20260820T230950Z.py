...
def apply_scoped_edit(
    ...
    mission = (
        ...
        if isinstance(mission_text, str) and mission_text.strip()
            ...
            "Inspect the repository and make one small safe reversible "
            "improvement grounded in the evidence."
        )
    )

    explicit_mission = bool(
        isinstance(mission_text, str)
        and mission_text.strip()
    )

    if explicit_mission:
        system_prompt = (
            "You are HAIDER's mutation phase.\n"
            "You MUST advance the explicit mission supplied below.\n"
            "Do not substitute generic cleanup, formatting, gitignore changes, "
            "repository metadata changes, or another merely-safe edit.\n"
            "Use the observed/deterministically supplied file evidence.\n"
            "Choose ONE coherent mutation to ONE existing repository file that "
            "materially advances the mission.\n"
            "The mutation may be substantial when required; there is no 1-5 line "
            "artificial ceiling.\n"
            "Never mutate .git/**.\n"
            "If the evidence is insufficient to advance THIS mission, emit no "
            "unrelated edit.\n"
            "Respond ONLY as JSON: "
            '{"path":"relative/path","old_text":"exact unique substring",'
            '"new_text":"replacement"}\n'
            "old_text must occur exactly once. No prose or markdown."
        )
    else:
        system_prompt = (
            "You are HAIDER. Based on inspected repository evidence, make one "
            "minimal safe reversible edit to one repository file.\n"
            "Never mutate .git/**.\n"
            "Respond ONLY as JSON: "
            '{"path":"relative/path","old_text":"exact unique substring",'
            '"new_text":"replacement"}\n'
    ...
    user_prompt = (
        ...
        "Return the single highest-leverage valid edit that advances the mission."
    )
