#!/usr/bin/env python3
"""Durable Hawking turns over the already-resident Genesis worker sessions.

This is the execution bridge between the persisted logical-worker registry and
Hawking's model/tool plane. It does not load a second model, run a protected GPU
benchmark, or mutate lineage.  A worker may inspect, edit, compile, and test in
its owned worktree, then use Hawking's ``submit_candidate`` tool to hand a concrete
...
from hawking.worker.special_unit import (  # noqa: E402
    DEFAULT_TOOL_NAMES,
    TOOL_MAX_NEW_TOKENS,
...
from hawking.lineage.bus import ResearchBus  # noqa: E402
from hawking.lineage.continuity import (  # noqa: E402
    WorkerCheckpointStore,
    compile_worker_context,
)
from hawking.lineage.lifecycle import (  # noqa: E402
...
from hawking.receipts import seal  # noqa: E402
from hawking.genesis_contract import contract_provenance  # noqa: E402


GPU_LOCK = Path("/tmp/hawking-gpu-lane.lock")
CANONICAL_SESSION_ROOT = REPO / "workspace" / "ops" / "genesis-worker-sessions"
LEGACY_SESSION_ROOT = REPO / "workspace" / "ops" / "genesis-agentos-sessions"
DEFAULT_STOPFILE = REPO / "workspace" / "ops" / "GENESIS_STOP"
SCHEMA = "hawking.genesis.worker_tick.v1"
...
HAWKING_TOOL_NAMES = tuple(name for name in DEFAULT_TOOL_NAMES if name != "bash")


class HawkingTurn(Protocol):
...
def default_session_root() -> Path:
    """Use the canonical store for new installs and preserve an existing run.

    Existing session state is user-owned durable evidence. It remains readable
    at its prior location until an explicit state migration lands, rather than
    making a nomenclature change discard its continuity.
    """
    if CANONICAL_SESSION_ROOT.exists() or not LEGACY_SESSION_ROOT.exists():
        return CANONICAL_SESSION_ROOT
...
def _gpu_lane_busy(lock_path: Path = GPU_LOCK) -> bool:
    ...
    when the next decode starts. Hawking must be allowed to reach that decode;
...
def _claim_worker(
    ...
    *,
    ...
    if previous_sha is None:
        raise RuntimeError("Hawking worker registry is not bootstrapped")
    changed = False
    for worker in workers:
        durable = worker["durable_task_state"]
        claim = durable.get("hawking_claim") or durable.get("agentos_claim")
        if worker.get("state") == "RUNNING" and isinstance(claim, Mapping):
            if not _process_alive(claim.get("pid")):
                worker["state"] = "READY"
                durable["hawking_recovered_claim"] = {
                    "at": _utc(),
                    "previous_claim": dict(claim),
                }
                durable.pop("hawking_claim", None)
    ...
    worker["durable_task_state"]["hawking_claim"] = {
        "pid": os.getpid(),
        "claimed_at": _utc(),
    ...
    if claimed_sha is None:
        raise RuntimeError("worker registry disappeared after Hawking claim")
...
def _finish_worker(
    ...
    *,
    ...
    if observed_sha != claimed_sha:
        raise RuntimeError("worker registry changed during Hawking turn; refusing blind overwrite")
    ...
    durable["last_hawking_turn_receipt_sha256"] = turn_digest
    durable["last_hawking_turn"] = {
        "at": _utc(),
        "ok": bool(turn.get("ok")),
    ...
    if turn.get("outcome") == "TURN_COMPLETE" and turn.get("ok") is True:
        durable["NEXT_ACTION"] = (
            "Resume from actual Hawking worker turn "
            f"{turn_digest[:16]}: inspect its recorded tool result and execute the next "
            "smallest falsifiable implementation or test step."
        )
    else:
        durable["NEXT_ACTION"] = (
            "Repair or falsify the failed Hawking worker turn "
            f"{turn_digest[:16]} before broadening the search."
        )
    session_id = turn.get("hawking_session_id") or turn.get("hcli_session_id")
    if isinstance(session_id, str) and session_id:
        durable["hawking_session_id"] = session_id
        durable.pop("hcli_session_id", None)
    durable.pop("hawking_claim", None)
    durable.pop("agentos_claim", None)
    worker["state"] = "READY"
    WorkerCheckpointStore(checkpoint_root).save(worker, reason="Hawking worker turn persisted")
...
def _default_unit_factory(
    *,
    ...
    def make(worker: Mapping[str, Any], worktree: Path) -> SpecialUnit:
        durable = worker["durable_task_state"]
        session_id = str(
            durable.get("hawking_session_id")
            or durable.get("hcli_session_id")
            or f"genesis-{worker['worker_id']}"
        )
...
def _worker_prompt(
    *,
    ...
    return (
        ...
        "Take one concrete implementation step in your OWNED WORKTREE. Use Hawking tools to "
...
def run_once(
    *,
    ...
    # that transient as a failed Hawking turn; the daemon will retry it safely.
    ...
    try:
        ...
        if current is None or not current.valid:
            raise RuntimeError("CURRENT Genesis unavailable for Hawking worker context")
        ...
        checkpoint = WorkerCheckpointStore(Path(checkpoint_root)).save(
            worker, reason="Hawking worker turn before model action"
        )
        ...
        act = unit.act(
            ...
            known_tools=HAWKING_TOOL_NAMES,
            max_new_tokens=max_new_tokens,
        ...
        raw_turn = {
            ...
            "act": act.to_dict(),
            "hawking_session_id": getattr(getattr(unit, "session", None), "session_id", None),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="genesis-worker")
