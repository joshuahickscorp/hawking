...
STATE="$PWD/.hawking/resident/state.json"
PY=/Users/scammermike/.venvs/hawking-aider/bin/python3
[ -x "$PY" ] || PY=python3

log() { echo "--- $(date -u +%FT%TZ) $*" >> .hawking/school/run.log; }

# hawkingd, restarted whenever it is not there.
(
    while true; do
        if ! pgrep -f "hawking.hawkingd --supervise" > /dev/null 2>&1; then
            log "hawkingd absent, starting"
            "$PY" -m hawking.hawkingd --supervise "$STATE" >> .hawking/school/hawkingd.log 2>&1
...
while true; do
    python3 tools/hawking_school.py --start-level 2 >> .hawking/school/run.log 2>&1
