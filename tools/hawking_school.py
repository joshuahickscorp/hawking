#!/usr/bin/env python3
"""Drive Hawking through a competence ladder, unattended.

Claude teaches by writing the goal and reading the receipt. Hawking does the work.
...
Run:  python3 tools/hawking_school.py [--once] [--start-level N]
"""
...
STATE = REPO / ".hawking" / "mission" / "state.json"
RESIDENT = REPO / ".hawking" / "resident" / "state.json"
LOG = REPO / ".hawking" / "school" / "log.jsonl"
GOALS = REPO / ".hawking" / "school" / "goals"
...
LADDER_FILE = REPO / ".hawking" / "school" / "ladder.json"
...
LADDER = [
    (2, "In hawking/tool_registry.py, make a whole-file fs.read result also report total_lines, the way the windowed read already does."),
    (3, "In hawking/resources.py, add a one-line docstring to pid_is_alive saying it reaps a zombie before testing liveness."),
    (4, "Write hawking/tests/test_pid_is_alive_contract.py asserting pid_is_alive returns False for a pid of 0, -1 and None."),
    (5, "In hawking/context_budget.py, report per_request_ctx and usable_input_tokens in the ContextBudget repr so a budget refusal names its own numbers."),
    (6, "In hawking/engine.py, record prefill_tokens_stepped and prefix_reused_tokens as a single reuse_fraction field on each model_call entry."),
    (7, "Write hawking/tests/test_prefix_reuse_fraction.py asserting reuse_fraction is prefix_reused_tokens divided by prompt_tokens and is None when either is absent."),
    (8, "Read .hawking/school/log.jsonl and name in one sentence the single change to Hawking that would most raise effective_prompt_tps."),
]
...
def run(*argv: str, timeout: int = 300) -> subprocess.CompletedProcess:
    return subprocess.run(
        [sys.executable, "-m", "hawking.resident", *argv],
...
def newest_receipt_mtime() -> float:
    files = sorted((REPO / ".hawking" / "receipts").glob("*.json"),
...
def _last_activity() -> float:
    ...
    newest = 0.0
    for path in (REPO / ".hawking" / "receipts").glob("*.json"):
        newest = max(newest, path.stat().st_mtime)
    for path in (STATE, REPO / ".hawking" / "mission" / "events.jsonl"):
        if path.exists():
            newest = max(newest, path.stat().st_mtime)
    return newest
...
def main() -> int:
    ...
    while True:
        ...
        if index >= len(rungs):
            ...
            # point the rungs start being about Hawking rather than the harness.
