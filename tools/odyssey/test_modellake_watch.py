...
def test_scheduling_authority_admits_only_exact_pinned_star_queue_entries(tmp_path):
    source = json.loads(
        (mw.ODYSSEY / "MODELLAKE_SCHEDULING_AUTHORITY.json").read_text()
    )
    star = source["acquisition"]["star_family_queue"]["items"][1]
    star["revision"] = "a" * 40
    star["resolved_sha"] = "a" * 40
    star["expected_selected_bytes"] = 3_750_000_000
    star["expected_file_count"] = 20
    authority_path = tmp_path / "authority.json"
    authority_path.write_text(json.dumps(source))

    authority = mw.load_scheduling_authority(authority_path)
    jobs = mw.scheduled_authority_jobs(authority)
    assert jobs[0]["tag"] == "deepseek-ai--DeepSeek-V4.1-Flash@dba1be0a40aa"
    assert jobs[1]["tag"] == "nvidia--NVIDIA-Nemotron-3.5-Lightning-30B-A3B-BF16@aaaaaaaaaaaa"
    assert jobs[1]["authority_star_candidate"] is True
    assert jobs[1]["expected"] == 3_750_000_000


def test_unpinned_star_queue_entries_remain_non_executable(tmp_path):
    source = json.loads((mw.ODYSSEY / "MODELLAKE_SCHEDULING_AUTHORITY.json").read_text())
    source["acquisition"]["star_family_queue"]["items"][1]["revision"] = "RESOLVE_AND_PIN_BEFORE_DOWNLOAD"
    path = tmp_path / "authority.json"
    path.write_text(json.dumps(source))
    authority = mw.load_scheduling_authority(path)
    jobs = mw.scheduled_authority_jobs(authority)
    assert len(jobs) == 9


def test_star_queue_preserves_unfinished_v41_reservation(monkeypatch):
    authority = mw.load_scheduling_authority()
    assert authority is not None
    target = authority["acquisition"]["target"]["slug"]
    assert mw.authority_reserved_target_bytes(authority, target) == 0
    assert mw.authority_reserved_target_bytes(authority, "some-star-candidate") == 510313353565
