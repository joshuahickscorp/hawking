...
VECTOR_REDUCTION_POLICY = "numpy_vectorized_f32"
LTR_F32_REDUCTION_POLICY = "ltr_f32"
REDUCTION_POLICIES = frozenset((VECTOR_REDUCTION_POLICY, LTR_F32_REDUCTION_POLICY))
...
def _require_reduction_policy(reduction_policy: str) -> str:
    if reduction_policy not in REDUCTION_POLICIES:
        raise ValueError(
            "PLE reduction policy must be one of "
            f"{sorted(REDUCTION_POLICIES)!r}, got {reduction_policy!r}"
        )
    return reduction_policy


def _ltr_f32_sum(values: np.ndarray, *, axis: int = -1) -> np.ndarray:
    """Force the same left-to-right F32 sum order as the native seam control.

    This is a diagnostic numerical-policy owner, not a claim about the
    reference framework's internal reduction tree.  It lets a source-bound
    formula fixture distinguish associativity from PLE-state layout or source
    payload identity before changing native runtime math.
    """
    values = np.asarray(values, dtype=np.float32)
    if values.shape[axis] == 0:
        raise ValueError("left-to-right F32 reduction has an empty axis")
    return np.take(np.cumsum(values, dtype=np.float32, axis=axis), -1, axis=axis)
...
def _matmul_f32(
    ...
    *,
    ...
    """Evaluate [tokens, columns] x [rows, columns].T under one policy."""
    reduction_policy = _require_reduction_policy(reduction_policy)
    inputs = np.asarray(inputs, dtype=np.float32)
    weights = np.asarray(weights, dtype=np.float32)
    if inputs.ndim != 2 or weights.ndim != 2 or inputs.shape[1] != weights.shape[1]:
        raise ValueError("PLE F32 matmul geometry is inconsistent")
    if reduction_policy == VECTOR_REDUCTION_POLICY:
        return (inputs @ weights.T).astype(np.float32)
    output = np.empty((inputs.shape[0], weights.shape[0]), dtype=np.float32)
    for token_index, input_row in enumerate(inputs):
        for row_index, weight_row in enumerate(weights):
            output[token_index, row_index] = _ltr_f32_sum(weight_row * input_row)
    return output
...
def grouped_rms_norm(
    ...
    *,
    group_size: int,
    eps: float,
    reduction_policy: str = VECTOR_REDUCTION_POLICY,
) -> np.ndarray:
    ...
    squares = (groups * groups).astype(np.float32)
    if reduction_policy == VECTOR_REDUCTION_POLICY:
        mean_square = np.mean(squares, axis=-1, keepdims=True)
    else:
        mean_square = _ltr_f32_sum(squares, axis=-1)[..., None] / np.float32(group_size)
    normalized = groups * np.reciprocal(np.sqrt(mean_square + np.float32(eps)))
...
def evaluate_ple_sequence(
    ...
    *,
    ...
    reduction_policy = _require_reduction_policy(reduction_policy)
    key = _matmul_f32(embeddings, support.key_proj, reduction_policy=reduction_policy)
    value = _matmul_f32(embeddings, support.value_proj, reduction_policy=reduction_policy)
    key_normed = grouped_rms_norm(
        key,
        support.norm_key,
    ...
    gate_products = (key_normed * query_normed).astype(np.float32)
    if reduction_policy == VECTOR_REDUCTION_POLICY:
        gate_dot = np.sum(gate_products, axis=-1, keepdims=True)
    else:
        gate_dot = _ltr_f32_sum(gate_products, axis=-1)[..., None]
    gate = gate_dot / np.float32(math.sqrt(hidden_size))
...
def evaluate_ple_zero_state_context_batch(
    hidden_states: np.ndarray,
    ...
    *,
    ...
    """Evaluate independent one-token PLE contexts with a zero convolution history.

    This is a source-formula primitive for cheap context-response
    discriminators.  Every batch row is an independent one-token call, so no
    row may borrow convolution state from another.  Its result must therefore
    equal calling :func:`evaluate_ple_sequence` once per row with
    ``initial_conv_state=None``.  It does not create a repeated-token or
    source-trajectory claim.
    """
    hidden_states = np.asarray(hidden_states, dtype=np.float32)
    ...
    if (
        ...
        or hidden_states.shape[1] != hc_hidden_size
        or embedding.size != support.key_proj.shape[1]
        or support.key_proj.shape[0] != hc_hidden_size
        or support.value_proj.shape != (hidden_size, embedding.size)
        or support.conv.shape[0] != hc_hidden_size
        or support.conv.shape[1] < 1
    ):
        raise ValueError("independent PLE context-batch geometry is inconsistent")
    # These are address-only for the batch: compute them once, then expose the
    # full source nonlinearity with respect to every supplied context state.
    key = embedding @ support.key_proj.T
    value = embedding @ support.value_proj.T
    ...
    gate = np.sum(key_normed[None, :, :] * query_normed, axis=-1, keepdims=True) / np.float32(
        math.sqrt(hidden_size)
    )
    gate = (np.sqrt(np.maximum(np.abs(gate), np.float32(1e-6))) * np.sign(gate)).astype(
        np.float32
    )
    gated_value = (_sigmoid(gate) * value.reshape(1, 1, -1)).astype(np.float32)
    gated_value_flat = gated_value.reshape(-1, hc_hidden_size)
    gated_value_normed = grouped_rms_norm(
        gated_value_flat, support.norm_conv, group_size=hidden_size, eps=eps
    )
    # With causal left padding and zero history, only the final (current-token)
    # convolution tap can contribute.  ``ngram_size`` is still validated to
    # bind this fast batch form to the source sequence semantics.
    if int(ngram_size) < 2:
        raise ValueError("PLE n-gram size must be at least two")
    convolved = gated_value_normed * support.conv[:, -1][None, :]
    outputs = (gated_value_flat + _silu(convolved)).astype(np.float32)
    return outputs, {
        "key": key,
        "value": value,
        "gate": gate.reshape(-1, hc_count),
...
def main() -> int:
    ...
    parser.add_argument(
        "--reduction-policy",
        choices=sorted(REDUCTION_POLICIES),
        default=VECTOR_REDUCTION_POLICY,
        help=(
            "F32 reduction policy. ltr_f32 is a bounded native-comparison diagnostic, "
            "not a claim about the framework's internal reduction tree."
        ),
    )
    parser.add_argument(
        "--output-gated-value-normed",
        type=Path,
        help="Optional F32 capture of the newly written PLE convolution-state tail.",
    )
    ...
    output_gated_value_normed_path = (
        args.output_gated_value_normed.expanduser()
        if args.output_gated_value_normed is not None
        else None
    )
    ...
    gated_value_normed_summary = (
        _write_state(output_gated_value_normed_path, intermediates["gated_value_normed"])
        if output_gated_value_normed_path is not None
        else None
    )
    ...
    document: dict[str, Any] = {
        ...
        "outputs": {
            ...
            "gated_value_normed": gated_value_normed_summary,
        ...
        "claim_boundary": (
            ...
            "direct F32 PLE injection and persistent PLE convolution state. The optional ltr_f32 "
            "mode isolates native reduction ordering; it is not source-framework state parity. It does not establish "
