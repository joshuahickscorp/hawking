...
def test_residual_product_quantizer_is_deterministic_and_accounts_all_stages() -> None:
    train = np.array(
        [[0.0, 1.0, 2.0, 3.0], [0.2, 1.2, 2.2, 3.2], [4.0, 5.0, 6.0, 7.0]],
        dtype=np.float32,
    )
    first = fit_residual_product_quantizer(
        train, subdimension=2, cardinality=2, stages=2, iterations=3, seed=17
    )
    second = fit_residual_product_quantizer(
        train, subdimension=2, cardinality=2, stages=2, iterations=3, seed=17
    )
    assert first.stages == 2
    assert first.code_bits_per_subspace == 2
    assert first.subspaces_per_row == 2
    assert len(first.codebooks) == 4
    for a, b in zip(first.codebooks, second.codebooks, strict=True):
        assert np.array_equal(a, b)
    np.testing.assert_array_equal(
        encode_decode_residual_product_quantizer(train, first),
        encode_decode_residual_product_quantizer(train, second),
    )
