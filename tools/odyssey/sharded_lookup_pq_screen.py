...
class ResidualProductQuantizer:
    """A deterministic additive PQ family with independently BF16-rounded stages.

    A stage encodes the residual left by prior stages.  It is a distinct
    representation hypothesis from a single codebook plus copied source
    values: every stage's codebooks and packed codes belong to the declared
    representation and can be accounted or directly decoded without a dense
    source table.
    """

    subdimension: int
    cardinality: int
    stage_codebooks: tuple[tuple[np.ndarray, ...], ...]

    @property
    def stages(self) -> int:
        return len(self.stage_codebooks)

    @property
    def code_bits_per_subspace(self) -> int:
        return self.stages * _require_power_of_two(self.cardinality, "residual PQ card")

    @property
    def subspaces_per_row(self) -> int:
        return len(self.stage_codebooks[0]) if self.stage_codebooks else 0

    @property
    def codebooks(self) -> tuple[np.ndarray, ...]:
        """Flattened physical codebook payloads for generic byte accounting."""
        return tuple(book for stage in self.stage_codebooks for book in stage)
...
def fit_residual_product_quantizer(
    train: np.ndarray,
    *,
    subdimension: int,
    cardinality: int,
    ...
    """Fit a greedy additive PQ without serializing or retaining source rows.

    This is deliberately kept beside the base PQ fit so architecture adapters
    do not recreate subtly divergent residual-quantization implementations.
    Codebooks are fit to the residual generated by the already BF16-rounded
    preceding stage, which is the only representation that the decoder sees.
    """
    train = np.asarray(train, dtype=np.float32)
    stages = int(stages)
    if stages < 2:
        raise ValueError("residual PQ needs at least two additive stages")
    residual = train.copy()
    fitted_stages: list[tuple[np.ndarray, ...]] = []
    for stage in range(stages):
        fitted = fit_product_quantizer(
            ...
            # Do not reuse the same initialization sequence for every stage.
            seed=seed + 1_000_003 * stage,
        )
        reconstructed = encode_decode_product_quantizer(residual, fitted)
        fitted_stages.append(fitted.codebooks)
        residual = residual - reconstructed
    return ResidualProductQuantizer(
        subdimension=int(subdimension),
        cardinality=int(cardinality),
        stage_codebooks=tuple(fitted_stages),
    )


def encode_decode_residual_product_quantizer(
    values: np.ndarray, quantizer: ResidualProductQuantizer
) -> np.ndarray:
    """Greedily decode an additive PQ using only its staged codebooks."""
    values = np.asarray(values, dtype=np.float32)
    if values.ndim != 2 or not quantizer.stage_codebooks:
        raise ValueError("residual PQ needs a non-empty 2D value matrix and staged codebooks")
    result = np.zeros_like(values)
    residual = values.copy()
    for stage in quantizer.stage_codebooks:
        if len(stage) != values.shape[1] // quantizer.subdimension:
            raise ValueError("residual PQ stage does not match the input row geometry")
        reconstructed = _encode_decode(residual, list(stage), quantizer.subdimension)
