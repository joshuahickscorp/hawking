...
class Runner:
    ...
    def write_status(self, stage: str, state: str, note: str = "") -> None:
        ...
        payload = {
            ...
            "free_disk_gb": free_disk_gb(self.root),
        }
    ...
    def _finalize(self, status: str, measures: MeasurementRecorder) -> None:
        ...
        receipt = self.receipts.emit(
            ...
            measures=[r for r in rows if r.get("kind") == "metric"],
            status=status,
            meta=self.spec.meta,
        )
        st = {
            "state": status,
            "current_stage": self._stage_results[-1]["id"] if self._stage_results else "",
            "uptime_seconds": int(time.time() - self._start),
        }
        md = self.reports.render_md(
            title=self.spec.title or self.spec.id,
            experiment_id=self.spec.id,
            status=st,
            stages=self._stage_results,
            measures=rows,
            receipt_path=str(self.receipt_path.relative_to(self.root))
            if self.receipt_path.is_relative_to(self.root)
            else str(self.receipt_path),
        )
