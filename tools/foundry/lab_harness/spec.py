...
class Stage:
    ...
    cwd: str | None = None
    on_fail: str = "abort"  # abort | continue | skip_rest
    ...
    def from_dict(cls, d: dict[str, Any]) -> "Stage":
        ...
        return cls(
            id=str(d["id"]),
            argv=[str(x) for x in d.get("argv", [])],
            shell=d.get("shell"),
            env={str(k): str(v) for k, v in (d.get("env") or {}).items()},
            cwd=d.get("cwd"),
            on_fail=str(d.get("on_fail", "abort")),
            measure_keys=[str(x) for x in d.get("measure_keys", [])],
            timeout_s=d.get("timeout_s"),
            skip_if_done=bool(d.get("skip_if_done", True)),
        )
...
class ExperimentSpec:
    ...
    def from_dict(cls, d: dict[str, Any]) -> "ExperimentSpec":
        ...
        return cls(
            ...
            artifact_dir=str(d.get("artifact_dir", "artifacts/runs/lab")),
            status_name=str(d.get("status_name", "status.json")),
            log_name=str(d.get("log_name", "chain.log")),
            pid_name=str(d.get("pid_name", "pid")),
            measures_name=str(d.get("measures_name", "measures.jsonl")),
            receipt_name=str(d.get("receipt_name", "receipt.json")),
            report_name=str(d.get("report_name", "report.md")),
            pause_flag=str(d.get("pause_flag", "artifacts/runs/PAUSE")),
            resume_flag=str(d.get("resume_flag", "artifacts/runs/RESUME")),
            pause_poll_s=float(d.get("pause_poll_s", 10.0)),
            matrix=list(d.get("matrix") or []),
            fixtures=list(d.get("fixtures") or []),
            receipt_schema=str(d.get("receipt_schema", "hawking.lab.receipt.v1")),
