...
use strand_quant::format::{
    read_strand_v2, read_strand_v2_header, write_strand_v2, write_strand_v2_rht, PackedTensor,
    PackedTensorV2,
};
use strand_quant::selfdesc::{
    append_sdsc_with_tensor_luts, build_sdsc_for_archive_with_tensor_luts, decode_q12_with_sdsc_at,
    read_sdsc_bytes, tensor_lut_wire_bytes, TensorLutInput,
};
...
fn learned_vector_luts_survive_hawking_wire_and_reconstruct_exact_q12() {
    ...
    let lut_b: Vec<i32> = (0..cfg.lut_len())
        .map(|i| (31 - i as i32) * 173 + (i % 2) as i32 * 401)
        .collect();
    ...
    let base = write_strand_v2_rht(&packed, [0x5Au8; 32], true, false, &[true, false])
        .expect("write col-RHT STR2");
    let path = std::env::temp_dir().join(format!(
        "strand-sdsc-vector-integration-{}-{}.strand",
        std::process::id(),
    ...
    assert_eq!(
        tensor_lut_wire_bytes(&written.tensor_luts[0]),
        52 + lut_a.len() * 4
    );
    ...
    let sdsc = read_sdsc_bytes(&wire, true)
        .expect("parse SDSC")
        .expect("SDSC present");
    ...
    let digest_off = corrupt
        .windows(digest.len())
        .position(|w| w == digest)
        .expect("record digest in wire");
...
fn vector_decoder_matches_native_for_d2_d3_d4_d8_ragged_tail_biting() {
    for d in [2u32, 3, 4, 8] {
        ...
        let path = std::env::temp_dir().join(format!(
            "strand-sdsc-vector-d{d}-{}-{}.strand",
            std::process::id(),
            COUNTER.fetch_add(1, Ordering::Relaxed)
        ));
        std::fs::write(&path, &base).unwrap();
        append_sdsc_with_tensor_luts(
            &path,
            &[TensorLutInput {
                tensor_index: 0,
                entries: &lut,
...
fn interrupted_tensor_lut_append_is_prefix_recoverable_and_idempotent() {
    ...
    let input = [TensorLutInput {
        tensor_index: 0,
        entries: &lut,
    }];

    let full_path = std::env::temp_dir().join(format!(
        "strand-sdsc-recovery-full-{}-{}.strand",
        std::process::id(),
        COUNTER.fetch_add(1, Ordering::Relaxed)
    ));
    ...
    assert_eq!(
        append_sdsc_with_tensor_luts(&full_path, &input).unwrap(),
        expected_sdsc
    );
    ...
    for cut in [1usize, tail.len() / 2, tail.len() - 1] {
        let path = std::env::temp_dir().join(format!(
            "strand-sdsc-recovery-cut-{cut}-{}-{}.strand",
            std::process::id(),
            COUNTER.fetch_add(1, Ordering::Relaxed)
        ));
        ...
        assert_eq!(
            append_sdsc_with_tensor_luts(&path, &input).unwrap(),
            expected_sdsc,
            "cut={cut}"
        );
    ...
    let foreign_path = std::env::temp_dir().join(format!(
        "strand-sdsc-recovery-foreign-{}-{}.strand",
        std::process::id(),
        COUNTER.fetch_add(1, Ordering::Relaxed)
    ));
