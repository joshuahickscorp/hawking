...
fn frozen_lut_entries_within_clamp() {
    ...
    for l in FROZEN_MIN_L..=FROZEN_MAX_L {
        ...
        assert_eq!(
            qs, cs,
            "codebook L={l} is not a permutation of the quantile LUT"
        );
    }
...
fn reconstruct_q_total_corners() {
    for s in [i32::MIN, i32::MIN + 1, -1, 0, 1, i32::MAX - 1, i32::MAX] {
        for q in [-Q_CLAMP, -Q_CLAMP + 1, -1, 0, 1, Q_CLAMP - 1, Q_CLAMP] {
            ...
            assert!(
                prod128.unsigned_abs() < 1u128 << 63,
                "i64 product overflow at ({s},{q})"
            );
...
fn eff_scale_q_total() {
    ...
    for code in 0u16..=255 {
        ...
        for &s in &scales {
            ...
            assert_eq!(
                eff_scale_q(s, code) as i128,
                oracle,
                "impl != oracle at ({s},{code})"
            );
            checked += 1;
        }
    }
    ...
    eprintln!(
        "eff_scale_q sweep: 256 codes x {} scales = {checked} pairs",
        scales.len()
    );
}
...
fn eff_min_q_total_on_encoder_domain() {
    ...
    for code in 0u16..=255 {
        ...
        for &b in &bases {
            ...
            assert_eq!(
                eff_min_q(b, code) as i128,
                oracle,
                "impl != oracle at ({b},{code})"
            );
    ...
    eprintln!(
        "eff_min_q sweep: 256 codes x {} bases = {checked} pairs",
        bases.len()
    );
}
...
fn eff_min_q_i32_min_wrap_is_out_of_domain() {
    assert_eq!(
        eff_min_q(i32::MIN, 0x3F),
        i32::MIN,
        "wrap behaviour changed — update the ledger"
    );
...
fn word_reader_bit_basis_16_byte_buffers_full_grid() {
    ...
    eprintln!(
        "word reader basis grid: {} buffers x 64 offsets x 8 k = {checked} drains",
        bufs.len()
    );
}
