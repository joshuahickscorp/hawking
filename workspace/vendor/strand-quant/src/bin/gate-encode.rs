...
fn run_identity() -> bool {
    ...
    let mut check =
        |label: String, w: &[f32], cfg: &TrellisConfig, opts: &EncodeOpts, lut: &[i32]| {
...

    {
        ...
        for (oname, opts) in &OPT_COMBOS[..2] {
            check(
                format!("scalar k=2 L=12 outlier-shaped n=4096 opts={oname}"),
                &w,
                &cfg,
                opts,
                lut,
    ...
    for l in [6u32, 8] {
        ...
        for &n in &[2048usize, 1000] {
            for (oname, opts) in &OPT_COMBOS[..2] {
                ...
                let w = normal_vec(n, seed);
                check(
                    format!("vec d=2 k=2 L={l} n={n} opts={oname}"),
                    &w,
                    &cfg,
                    opts,
                    &lut,
...
fn bench_one(
    ...
    println!(
        "    {label:<22} {mw_s:8.3} Mw/s   ({:.2}s, sink={sink})",
        dt
    );
    mw_s
}
...
fn run_bench() {
    stamp();
    println!(
        "== ENCODE THROUGHPUT (CPU explicit-LUT path; GPU dispatch bypassed by construction) =="
    );
    println!("   7B-shaped slices: a 7B gate/up tensor is 18944x3584 = 67.9M w; the per-tensor");
    println!(
        "   Viterbi work is block-local (256 w), so a fixed-N slice measures the same kernel."
    );
    let opts = EncodeOpts::default();
    ...
    for (name, k, l, n, iters) in configs {
        ...
        let r_f32 = bench_one("new + f32 metric", &w, &cfg, lut, iters, |w, c, lu| {
            encode_tensor_with_lut_metric(w, c, &opts, lu, true)
                .bits
                .len()
        });
        println!(
