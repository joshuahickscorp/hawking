...
use strand_quant::decode::{
    decode_lean_with_lut, decode_tensor_fixed_with_lut, decode_tensor_fixed_with_lut_vec,
};
use strand_quant::encode::{
    encode_tensor_with_lut, vector_lut_from_scalar, EncodeOpts, EncodedTensor,
};
...
fn main() {
    ...
    let n: usize = std::env::var("VT_N")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(262_144);
    let iters: usize = std::env::var("VT_ITERS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(40);

    let lwide: u32 = std::env::var("VT_LWIDE")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(8);
    ...
    writeln!(
        report,
        "GATE B1/C1 — higher-dimensional FROZEN vector trellis: does it close the sub-2-bit gap?"
    )
    .unwrap();
    ...
    writeln!(
        report,
        "================ PER-CONFIG (rel-RMS is the gate) ================"
    )
    .unwrap();
    ...
    for dist in [Dist::Gaussian, Dist::HeavyTailed] {
        ...
        writeln!(
            report,
            "    0.75 bpw : d8={:?}   (sub-1-bit; no scalar analog)",
            find(dl, 8, 6)
        )
        .unwrap();
        writeln!(report).unwrap();
