...
{
    model_root_from_tensor_roots(
        tensors
            .into_iter()
            .map(|(name, enc, cfg, lut)| (name, tensor_root(enc, cfg, lut))),
    )
}
...
pub fn block_bit_offset(enc: &EncodedTensor, cfg: &TrellisConfig, block_index: usize) -> usize {
    ...
    enc.blocks[..block_index]
        .iter()
        .map(|b| cfg.num_steps(b.n as usize) * k)
        .sum()
}

pub fn decode_block_q12(
    enc: &EncodedTensor,
    cfg: &TrellisConfig,
    lut: &[i32],
    block_index: usize,
...
pub fn make_test_vectors(
    ...
    chosen
        .into_iter()
        .map(|bi| ProvenanceVector {
            block_index: bi as u64,
            block_hash: block_hash(bi as u64, &decode_block_q12(enc, cfg, lut, bi)),
        })
        .collect()
}
...
mod tests {
    ...
    fn kat_canonical_serialization_pinned() {
        ...
        assert_eq!(
            hex(&k1),
            "2234d6fcd138505967fb0e109d23cc8452e9498895621e3b30bdb943fd72296f"
        );
        let k2 = block_hash(1, &[1, -2, 1000]);
        assert_eq!(
            hex(&k2),
            "3990f085d2669dd68b30e9adcfe5a9e69ddcd7ccb2c5d703326aa5c1df7f30c8"
        );
        let k3 = tensor_root_from_hashes(&[]);
        assert_eq!(
            hex(&k3),
            "c6f7a00b2d83315a5315962a057a525886a321b5b2c76200fb9694d6db476a53"
        );
        let k4 = tensor_root_from_hashes(&[k1, k2]);
        assert_eq!(
            hex(&k4),
            "1492f395a50b7c64691f10646c707c6966e09ae4f017fb023ba37356dfb19edf"
        );
        let k5 = model_root_from_tensor_roots(std::iter::empty());
        assert_eq!(
            hex(&k5),
            "b482c517817d47c0d95c6808b6ef48b1d86e14fa7c02b780969b467ea23026a7"
        );
        let k6 = model_root_from_tensor_roots([("alpha", k3), ("beta", k4)]);
        assert_eq!(
            hex(&k6),
            "d390075de4f8486e6d3d6e975ec5bea8dc5e5858694817e4d635ec8545508666"
        );
    }
        ...
        for cfg in scalar_cfgs {
            ...
            for &n in &lens {
                for (vi, opts) in opt_variants().iter().enumerate() {
                    ...
                    assert_eq!(
                        cat, reference,
                        "per-block decode diverged: L={} k={} n={} variant={}",
                        cfg.l_bits, cfg.k_bits, n, vi
                    );
                }
            }
        }
    ...
    fn kat_descriptor_and_outlier_digests_pinned() {
        ...
        assert_eq!(
            hex(&od),
            "d890a1d65968ba070379f9fb0111a64517f1f9f0c9f9cce55449225ca7bb769a"
        );
        let d_no = descriptor_digest(
        ...
        assert_eq!(
            hex(&d_no),
            "4bf793e1276d4ac5b6a7ff569be975c6dc8a5f60e95d175420f4520a3cc98f69"
        );
        let d_with = descriptor_digest(
            "model.layers.0.q_proj",
            &[4, 256],
            0x1234_5678_9ABC_DEF1,
            7,
    ...
    fn hashes_and_roots_are_deterministic() {
        ...
        let model = [
            ...
            ("model.layers.0.down_proj", &enc_b, &cfg, lut),
        ];
        let m1 = model_root(model);
        let m2 = model_root(model);
        assert_eq!(m1, m2, "model root is not deterministic");
        let m3 = model_root_from_tensor_roots([
            ("model.layers.0.q_proj", tensor_root(&enc_a, &cfg, lut)),
            ("model.layers.0.down_proj", tensor_root(&enc_b, &cfg, lut)),
        ]);
    ...
    fn payload_bit_flip_flips_exactly_one_leaf_and_all_roots() {
        ...
        assert_ne!(
            ...
            tensor_root(&tampered, &cfg, lut),
            "tensor root must change"
        );
