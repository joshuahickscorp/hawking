...
|-----|----------|----------------|--------------|---------------------------|
| **Sequence sharding** 1.40×@2w / 1.79×@4w | `FULLSEQ_CAPTURE_PARALLELISM_FINDINGS.json` (`PASS_SHARDING_IMPLEMENTED_AND_MEASURED`; bit-exact merge) | **CROSS_FAMILY_TOPOLOGY** | Multi-process sequence partition + offline merge whenever **host serial orchestration** is the bottleneck and residuals do not cross workers | Layer sharding without residual handoff; stream/link-bound capture (GLM official path Amdahl ~1.05× — see `GLM_TEACHER_FORCED_PARALLELISM_FINDINGS.json`) |
