# DELEGATION — O001 AGGRESSIVE GRAVITY (q2-g32-attn-mlp; gate profile: MLX/Metal)
...
AGGRESSIVE candgen spec `q2-g32-attn-mlp`: attn+mlp (+embed/lm_head); protect SSM/conv/norm full. candidate_class from grammar. Complete bpw (payload+scales+biases+metadata). On fail: localize, do not globally retreat.
...
Reuse tools/odyssey_patient_runner.py `--gravity q2-g32-attn-mlp`. One candidate, do not sweep.
...
Write receipts/odyssey-i/O001_GRAVITY_q2-g32-attn-mlp.json (schema odyssey.patient.gravity.v1): spec, stored/active bpw, complete_bpw,
...
## ACCEPTANCE
- receipts/odyssey-i/O001_GRAVITY_q2-g32-attn-mlp.json exists with stored_bpw < 16, complete_bpw, nominal_bits, candidate_class,
...
VERIFY receipts/odyssey-i/O001_GRAVITY_q2-g32-attn-mlp.json by running the unfenced command below; must pass, exit 0.
python3 tools/odyssey_patient_runner.py --oxx O001 --weights /Users/scammermike/.cache/huggingface/hub/models--tiiuae--Falcon-H1-7B-Instruct/snapshots/41e72f27effbab80cd45b6e884688452253a3686 --runtime mlx --out receipts/odyssey-i/O001_GRAVITY_q2-g32-attn-mlp.json --packet workspace/campaign/odyssey/patients/O001/ODYSSEY_PATIENT_O001.json --skip-route --gravity q2-g32-attn-mlp
