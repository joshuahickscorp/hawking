# DELEGATION — O003 FRONTIER NOVELTY / kernel

Patient O003 (moe; KimiVLForConditionalGeneration).
...
LARGE remaining target delta (primary=0.8111, pressure=2.5 bpw,
stored_bpw=3.0637, active_bpw=3.3111). Best conventional
anchor: spec=q3-g32-experts class=CONVENTIONAL_ANCHOR
complete_bpw=3.9412.
Aggressive failures: q2-g32-experts, q2-g64-experts. Localization: [{"failed": true, "delta_hits": -2, "threshold": -1, "ranked_organs": [{"organ": "attn", "sensitivity_delta": -1, "treatment": "round8"}, {"organ": "embed", "sensitivity_delta": -1, "treatment": "round8"}, {"organ": "expert", "sensitivity_delta": -1, "treatment": "round8"}, {"organ": "shared_expert", "sensitivity_delta": -1, "treatment": "round8"}, {"organ": "lm_head", "sensitivity_delta": 0, "tre.
Native / NX so far: active-expert-gather (accounting only; not a Rust runtime).
...
## QUESTION
What native kernel / NX primitive must exist for a proposed repr to become a Hawking win (not a foreign-runtime SPECIMEN), and what relative kernel gain is the cheapest honest target?

## SEARCH SPACE (nonconventional only)
bible §54: 1.15x / 1.25x / 1.5x where headroom is obvious; do not invent a 5x. Wrapper TPS is not a Hawking win. Gather must move only active experts. Record kernel implications of the proposed codec (ALU, gather, prefetch, residency).

Live negatives that already constrain this lane:
NS-large-expert-cache (no cross-layer reuse on a lockstep pass); R-affine-grouped-q2-if-native-kernel (Doctor-valid q2 without a native kernel is not a pass)
...
## BUILD
Write `receipts/odyssey-i/O003_NOVELTY_kernel.json` (schema hawking.odyssey.novelty_lane_report.v1) with one or more
...
## ACCEPTANCE
- `receipts/odyssey-i/O003_NOVELTY_kernel.json` exists, schema hawking.odyssey.novelty_lane_report.v1, each proposal has
...
python3 -c "import json,pathlib,sys; p=pathlib.Path('receipts/odyssey-i/O003_NOVELTY_kernel.json'); d=json.loads(p.read_text()); ps=d.get('proposals') if isinstance(d.get('proposals'), list) else [d]; req=('mechanism','complete_byte_accounting','cheapest_falsifier','execution_path','kernel_implications','applicability_class','doctor_risk'); missing=[k for pr in ps if isinstance(pr, dict) for k in req if k not in pr]; sys.exit(0 if p.is_file() and ps and not missing else 1)"
