...
K=4;grp=[min(l*K//64,K-1) for l in range(64)]
print(f"grouped K={K} train {X.shape[0]} held {Xh.shape[0]}; q3 held {q3m:.4f}",flush=True)
class Grp(torch.nn.Module):
 def __init__(s,m,K,nL=64):
  super().__init__();s.K=K;s.grp=grp
  ...
  s.gt=torch.tensor(grp)
 def forward(s,x,z):
  out=torch.zeros(x.shape[0],5120,device=x.device);gz=s.gt.to(z.device)[z]
  for k in range(s.K):
   mi=(gz==k)
   if mi.sum()==0: continue
   xk=x[mi];zk=z[mi];inter=torch.nn.functional.silu(s.g[k](xk))*s.u[k](xk);inter=inter*s.gamma(zk)+s.beta(zk);out[mi]=s.d[k](inter)
  return out
G=Grp(6144,K).to(dev);opt=torch.optim.AdamW(G.parameters(),lr=5e-4,weight_decay=1e-4);t=time.time();best=9;bst=None
for it in range(1800):
 ...
 if (it+1)%300==0:
  with torch.no_grad(): e=rel(G(Xh,Zh),Yh)
  if e<best: best=e;bst={k:v.cpu().clone() for k,v in G.state_dict().items()}
  print(f"  it{it+1} held {e:.4f} best {best:.4f} [{time.time()-t:.0f}s]",flush=True)
st={f"g.{k}.weight":bst[f"g.{k}.weight"] for k in range(K)};st.update({f"u.{k}.weight":bst[f"u.{k}.weight"] for k in range(K)});st.update({f"d.{k}.weight":bst[f"d.{k}.weight"] for k in range(K)})
st["gamma.weight"]=bst["gamma.weight"];st["beta.weight"]=bst["beta.weight"]
torch.save({'state':st,'m':6144,'K':K,'held':best,'q3':q3m,'input':'post_attn_norm'},"workspace/campaign/phaseB/ckpt/grouped_k4_stable.pt")
print(f"\nGROUPED K=4: held {best:.4f} vs q3 {q3m:.4f} | saved grouped_k4_stable.pt",flush=True)
