...
---
# LANE: auto-dsv4f-level-host-exclusive-not
...
Source lane: `dsv-mla-20260816-003211` (status SHIPPED)

    token-level host exclusive ~670 ms (not this lane). MLA GPU now ~128 ms; largest remaining isolated kernels are wo_a ~0.67–1.08 ms and wo_b ~0.64–1.01 ms per layer (~29–44 ms/token), then wq_a/wkv on 1024/512-thread grids. Still ~9× the 15 ms GPU-body budget. Occupancy + convert+einsum, not bandwidth.
