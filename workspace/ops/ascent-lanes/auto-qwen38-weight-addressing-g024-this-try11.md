...
ASSUMPTION:
Each token's GEMV dispatch independently addresses and transfers the full weight tensor from DRAM; total MLP DRAM traffic = N_tokens × |W_mlp|. The 21.3 ms is N sequential full-weight reads, not a per-read bandwidth deficit.

MECHANISM:
Replace N GEMV dispatches with ONE tiled GEMM dispatch (M=out_dim, N=batch, K=in_dim). The Metal kernel loads each W tile into registers/shared once, then multiplies against all N input columns before storing. DRAM read of W drops from N×|W| to 1×|W|; the N× FLOP increase is absorbed by the existing compute headroom (GEMV is memory-bound, not compute-bound, at N=1).

DISCRIMINATOR:
Implement the single-dispatch GEMM for the MLP layer at N=4 (matching the refuted 4×GEMV control). Instrument: (a) Metal GPU counter for actual DRAM bytes read for W, (b) wall-clock of the one dispatch vs 4× single-GEMV baseline. If bytes ≈ |W| (not 4×|W|) AND wall-clock < 4× single-GEMV time → mechanism confirmed.

REJECT_IF:
Single GEMM dispatch wall-clock ≥ 4× single-GEMV time despite DRAM bytes ≈ |W|. That would mean the 21.3 ms is compute-bound (ALU/LSU issue) rather than DRAM-addressing-bound, and the "weight_addressing" label is misattributing a compute stall to memory traffic. In that case the lever is FLOP reduction (precision, sparsity, shared-structure), not dispatch topology.

---
# LANE: auto-qwen38-weight-addressing-g024-this-try11
