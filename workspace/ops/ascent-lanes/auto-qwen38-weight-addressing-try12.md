...
ASSUMPTION: Execution genome dispatches one GEMV per token; each dispatch independently addresses the full weight tensor from DRAM, so weight_addressing cost = N_dispatches × |W|/BW. The 21293103 is per-dispatch DRAM read of W; N tokens → N reads.

MECHANISM: Replace N independent GEMV dispatches with ONE batched-GEMV kernel: single dispatch, W streamed from DRAM exactly once, inner loop over N resident input vectors (N×d, fits registers/shared) per W-row. Weight DRAM traffic: N×|W| → 1×|W|. Kernel structure: `for row in W: for i in 0..N: acc[i] += W[row]·x[i]` (W on outer loop, N on inner).

DISCRIMINATOR: Compile batched GEMV with N=2 (two input vectors, one W body). Measure weight_addressing. Compare to N=1 baseline 21293103.

REJECT_IF: weight_addressing(N=2) > 1.1×21293103. If the "single dispatch" still re-reads W per column (inner loop over W, outer over N), the access pattern is structurally identical to the refuted N-independent-dispatches case and the mechanism collapses to 4x.

---
# LANE: auto-qwen38-weight-addressing-try12
