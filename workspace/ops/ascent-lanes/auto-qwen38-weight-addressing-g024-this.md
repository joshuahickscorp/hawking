...
ASSUMPTION: Each token's GEMV is a separate Metal dispatch; the GPU re-fetches W from DRAM per dispatch, so weight_addressing = N_tokens × W_bytes / BW. The 82.9% addr/GEMV ratio means compute is free; the cost is N redundant DRAM reads of the same weight body.

MECHANISM: Replace N separate GEMV dispatches with ONE batched GEMV dispatch: kernel receives W(M×K) and X(K×N); each workgroup loads its W-tile into registers once, then loops i=0..N-1 computing y_i = W_tile @ x_i. Weight DRAM traffic = W_bytes (read once), independent of N. This is the "single dispatch, N columns" path still open.

DISCRIMINATOR: Instrument the existing mlp GEMV kernel to accept X as (K×4) instead of (K×1); add a per-workgroup column loop after the W-tile load. Run N=4 on current main. Read the existing addr_probe counter. If batched-N=4 weight_addressing ≤ 1.2× single-token weight_addressing (≈12.7 ms, not ≈51 ms), the GPU retains W in L1/L2 across the column loop and the mechanism is live.

REJECT_IF: Batched-N=4 weight_addressing > 1.5× single-token (i.e. > ~19 ms) — the Metal GPU flushes the W-tile between column iterations within one dispatch, so "one dispatch" does not actually read W once. Kill the mechanism; next lever is persistent-kernel weight residency in L2 or a layout change that makes W-tile reuse L1-local without a column loop.

---
# LANE: auto-qwen38-weight-addressing-g024-this
