...
ASSUMPTION: Each token's GEMV is a separate Metal dispatch that independently fetches W from DRAM; N tokens cost N×|W| bytes because the dispatch boundary forces a full weight re-read per token.

MECHANISM: Replace N sequential GEMV dispatches with ONE GEMM dispatch: W[d_out×d_in] @ X[d_in×N]. W is fetched once; the N input vectors (N×d_in floats, already in shared memory) are the only per-column traffic. DRAM: |W| + N·d_in ≈ |W|, vs N·|W| for the 4-dispatch control.

DISCRIMINATOR: On the existing lm_head W, build one Metal kernel computing W@[x1..x4] (N=4, identical inputs to the 4-dispatch control already measured at 4×). Measure wall time of the single packed dispatch.

REJECT_IF: Packed N=4 wall time > 1.2× single-GEMV time — kernel is compute-bound on MAC, not DRAM-bound on W-read; batching columns adds FLOPs without saving bytes, and the mechanism is dead.

---
# LANE: auto-qwen38-qwen3-complete-still-bpw-try12
