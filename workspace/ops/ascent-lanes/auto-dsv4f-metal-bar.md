...
- Bottleneck: metal.bar 2 ns/token
- Already refuted: fusing tiny kernels into GEMV, cross-token cache reuse, N sessions sharing one weight body

The key insight here: weight addressing is 60.44% of token time, moving 13.618 GB at 97.6% of the DRAM bandwidth ceiling. This means the weight loading is ALREADY AT BANDWIDTH LIMIT. You can't make DRAM faster. The 13.618 GB of weight data must be read from DRAM every token.

The "metal.bar 2 ns/token" is the synchronization barrier cost - the GPU/metal fence that waits for the weight load to complete before the next operation can proceed.
...
- 13.618 GB of weights must be read from DRAM per token
- We're at 97.6% of the 411.51 GB/s ceiling
- So the weight read time is essentially 13.618 / 411.51 * 1e9 ≈ 33,093 ns... wait, let me recalculate. 13.618 GB / 411.51 GB/s = 0.03309 s = 33,093,000 ns. But the weight addressing is 21,293,103 ns. Hmm, that doesn't quite match. Let me re-read.

Actually wait - 13.618 GB at 97.6% of 411.51 GB/s. So effective bandwidth is 0.976 * 411.51 = 401.63 GB/s. Time = 13.618 / 401.63 = 0.03391 s = 33,910,000 ns. But they say weight_addressing is 21,293,103 ns. That's about 62.8% of the full bandwidth time. Hmm, maybe the 13.618 GB isn't all in the weight_addressing phase, or maybe I'm misreading the units.

Actually, let me re-read: "moves 13.618 GB at 97.6% of the 411.51 GB/s unique-once decode ceiling"

So the weight addressing phase moves 13.618 GB and achieves 97.6% of peak bandwidth. The time for this is 13.618 GB / (0.976 * 411.51 GB/s) = 13.618 / 401.63 s = 0.03391 s ≈ 33.9 ms. But they say weight_addressing is 21,293,103 ns = 21.29 ms. 

Hmm, maybe the 13.618 GB is the total weight size and the 21.29 ms is the actual measured time, and 97.6% is the achieved bandwidth relative to ceiling. Let me check: 13.618

---
# LANE: auto-dsv4f-metal-bar
...
Source lane: `dsv-y-1` (status SHIPPED)

    metal.bar 2 ns/token
