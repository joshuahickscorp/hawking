...
ASSUMPTION: GEMV kernel is one-dispatch-per-column; W address stream is per-dispatch, so weight_addressing = N × |W|_read. The 19.95 ms mlp cost is 4 independent W reads, not 1.

MECHANISM: Single dispatch, batched input [x1..xN]. Kernel loads W into registers (or shared) ONCE, then computes N dot-products from that resident copy. Weight_addressing becomes O(1) in N; the N columns are register-resident, not DRAM-resident.

DISCRIMINATOR: Implement batched single-dispatch GEMV for mlp, N=4. Measure weight_addressing. Falsification check: dump Metal compiled code or use `MTLDevice` memory-access trace — confirm W is fetched in 1 address stream, not 4. If the kernel's inner loop re-indexes W per column (compiler hoists the load outside the column loop → 1 read; compiler keeps it inside → 4 reads), the trace distinguishes them in one run.

REJECT_IF: weight_addressing(N=4) ≥ 3.5 × weight_addressing(N=1) (W still re-read per column internally), OR register pressure spills W to local memory making the single dispatch slower than 4 separate dispatches, OR output mismatch vs reference > 1e-4 (capability regression).

---
# LANE: auto-qwen38-weight-addressing-mlp-try12
...
Source lane: `auto-qwen38-weight-addressing-this-run-try12-20260817-030101` (status SHIPPED)

    weight_addressing 19952248 ns (mlp 12934375)
