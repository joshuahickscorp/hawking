...
ASSUMPTION: 21.3 ms is per-element Q4-unpack ALU + per-row address-transaction overhead (152k row-reads × ~135 ns setup), not raw DRAM byte-transfer. Evidence: 312 MB at 411 GB/s roof = 0.76 ms; measured 21.3 ms ⇒ 20.5 ms is non-bandwidth overhead that scales with output-row count, not with bytes.

MECHANISM: Single-dispatch GEMM W@[x₁..x_N] (one kernel, one W read). Each thread-block loads a 64×64 W tile, unpacks Q4 once, multiplies against all N columns. Unpack ALU and address-transaction cost amortize by ×N; W bytes hit DRAM exactly once. N=1 collapses to current GEMV (no regression risk).

DISCRIMINATOR: N=2 GEMM on lm_head, single dispatch, same Q4 body, same addr_probe instrumentation. One run. Compare addr_ns to 21 346 665.

REJECT_IF: addr_ns(N=2) > 0.9 × 21 346 665 (i.e. > 19.2 M ns). If 2× column reuse does not cut the cost ≥10 %, the 21.3 ms is raw DRAM byte-transfer (effective BW ≈ 14.6 GB/s) and no GEMM tiling removes it; pivot to a ≤3-bit codec or attention-family representation.

---
# LANE: auto-qwen38-weight-addressing-this-run-try12
