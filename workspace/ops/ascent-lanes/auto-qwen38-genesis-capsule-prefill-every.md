...
ASSUMPTION: Prefill runs 1558 sequential per-token GEMV dispatches per linear layer (W fetched 1558×/layer from DRAM); 79 s ≈ 1558 × per-token decode wall. The "single GEMM, W-read-once, N-columns" path is not wired.

MECHANISM: One Metal GEMM dispatch per linear layer: W[hidden,hidden] @ X[hidden,1558] → Y[hidden,1558]. W fetched once; 1558 columns computed in a single kernel launch. Replaces the 1558-iteration GEMV loop. This is the OPEN mechanism, not the refuted N-independent-dispatches control.

DISCRIMINATOR: Instrument one prefill pass; count GPU dispatches. If count ≈ 1558×L×G (L=layers, G=GEMMs/layer) → assumption confirmed, mechanism applies. If count ≈ L×G → assumption false; cost is in GEMM kernel geometry, not dispatch topology.

REJECT_IF: Dispatch count is already L×G AND a single-layer 1558-column

---
# LANE: auto-qwen38-genesis-capsule-prefill-every
...
Source lane: `auto-qwen38-live-act-through-genesis-20260817-110826` (status PARTIAL)

    genesis capsule prefill every propose; act prefill_wall 79.00–79.76 s (1558 prompt tokens; 95% of complete wall; DIRTY_ENGINEERING)
