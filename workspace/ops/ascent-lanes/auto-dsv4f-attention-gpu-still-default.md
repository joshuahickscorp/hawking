...
---
# LANE: auto-dsv4f-attention-gpu-still-default
...
Source lane: `dsv-integrate-occupancy-20260816-012855` (status PARTIAL)

    attention GPU still ~127236000 ns on the default path (WO-A / WQ-A / WKV / WO-B still one-thread-per-row). Host exclusive remains the token-level majority (~1.0 s clean body).
