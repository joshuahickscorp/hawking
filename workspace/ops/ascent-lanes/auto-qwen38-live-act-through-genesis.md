...
ASSUMPTION: Execution genome issues one Metal dispatch per token; each GEMV independently fetches full W from DRAM, so N tokens cost N×|W| bytes. No cross-dispatch weight residency exists in the memory controller.

MECHANISM: Pack N token vectors into one [d_model×N] matrix X; issue a SINGLE Metal GEMM dispatch (W@X) that streams W from DRAM exactly once and writes N output columns in-kernel. Tiling reuses W across all N columns within the one dispatch—no cross-dispatch state, no protocol change.

DISCRIMINATOR: On current main, build X=[d_model×4] (4 random vectors), issue one Metal GEMM for lm_head_W@X, measure wall with the protected timer already used for the 4×GEMV baseline. Single GEMM must land ≤1.2× the 1×GEMV time.

REJECT_IF: Single [W×4] GEMM wall ≥ 3× 1×GEMV time—meaning the GEMM kernel's internal tiling still re-reads W per column-group (or Metal decomposes the wide GEMM into per-column GEMVs internally). Cost is then in the kernel's tile strategy, not dispatch count; mechanism dead, pivot to tile-geometry edit.

---
# LANE: auto-qwen38-live-act-through-genesis
...
Source lane: `genesis-agentos-hcli-backfill-20260817-093419` (status SHIPPED)

    live act() through genesis_resident.propose still fail-closes because the resident protocol injects the system capsule and rejects a second system role; say()/plain prompts are the legal live path. 100-TPS remains the protected DeltaNet kernel lane.
