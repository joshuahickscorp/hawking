...
ASSUMPTION: 48 separate GPU kernel launches (one per layer's 16-wide gated_rmsnorm) each pay full command-encode + GPU-scheduler-dispatch + memory-fence latency; the 1.33 ms is 48× launch_overhead, not 48× FLOPs. The work per kernel is sub-wave (16 threads) so the GPU sits idle between dispatches.

MECHANISM: Replace 48 individual gated_rmsnorm dispatches with ONE batched kernel: a single command-buffer entry whose kernel takes a device-side array of 48 (x_ptr, w_ptr, out_ptr) tuples and processes all 768 threads in one wave. One launch, one fence, one retire. Does NOT touch the following GEMV (avoids the refused prologue-fusion regression). The 48 GEMVs remain separate; only the 48 trivial rmsnorms collapse to 1 dispatch.

DISCRIMINATOR: In the current checkout, replace the 48-iteration dispatch loop with a single `dispatchBatchedGatedRMSNorm(48, tuples_ptr)` call. Run one protected forward pass. Read the GPU timestamp for that single dispatch from the command-buffer completion event. Compare to 1330791 ns.

REJECT_IF: (a) batched dispatch GPU time ≥ 1330791 ns (cost is per-element memory latency, not launch overhead — batching is inert); OR (b) any of the 48 layer outputs differ from the 48-kernel reference by >0 ulp (correctness break); OR (c) total forward-pass wall increases >50 µs (the single dispatch serializes with a dependency that 48 parallel dispatches previously overlapped).

---
# LANE: auto-qwen38-gated-rmsnorm-still-gpu
...
Source lane: `auto-qwen38-deltanet-20260817-092519` (status SHIPPED)

    gated_rmsnorm_48 still 1330791 ns GPU (48×16-wide); weight_addressing remains ~21e6 ns existential
