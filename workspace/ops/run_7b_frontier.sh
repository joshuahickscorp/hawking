...
#
#      ./workspace/ops/run_7b_frontier.sh            # launch / resume + arm backstop
#      ./workspace/ops/run_7b_frontier.sh status     # results so far (run anytime)
#      ./workspace/ops/run_7b_frontier.sh kill       # stop everything
#
...
PY=python3.12
RUN=7b_frontier
REPORTS=workspace/campaign/records/reports/cron
LOCK=$REPORTS/$RUN.lock
JSONL=$REPORTS/$RUN.jsonl
HEALTH=$REPORTS/${RUN}_health.log
HEALTH_PID=$REPORTS/${RUN}_health.pid
CAFFEINATE_PID=$REPORTS/${RUN}_caffeinate.pid
KEEPALIVE_PID=$REPORTS/${RUN}_keepalive.pid
CONDUCTOR_PID=$REPORTS/${RUN}_conductor.pid
VERIFIER_PID=$REPORTS/${RUN}_verifier.pid
SEED=$REPORTS/7b_ladder.jsonl          # the first ladder's results to carry over
mkdir -p "$REPORTS"
export LADDER_MODEL="$HOME/Downloads/hawking/scratch/qwen-7b"
export LADDER_LABEL=7B LADDER_SET=frontier LADDER_OUT=$REPORTS/$RUN
export LADDER_LOCK=$LOCK LADDER_HEALTH=$HEALTH LADDER_HEALTH_PID=$HEALTH_PID
export LADDER_RUN_LOG=$REPORTS/${RUN}_run.log
export LADDER_SIGMA=$REPORTS/${RUN}_sigma.safetensors
...
PT=/tmp/ppl24k.txt
[ -f "$PT" ] || cat README.md workspace/docs/plans/*.md 2>/dev/null | head -c 24000 > "$PT"
export PPL_TEXT=$PT
...
run = os.environ["RUN"]
rp = os.environ["RP"]
root = pathlib.Path("workspace/campaign/records/reports/cron")
...
env = os.environ.copy()
subprocess.Popen(
    ["python3.12", "tools/condense/frontier_conductor.py", "--outbase", "workspace/campaign/records/reports/cron/7b_frontier"],
    stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
    env=env, start_new_session=True,
)
PY
...
case "$cmd" in
  status)
    ...
    [ -f "$REPORTS/7b_frontier_verified.md" ] && { echo "--- VERIFIED (independent repro) ---"; tail -n +3 "$REPORTS/7b_frontier_verified.md"; }
...
PY
  ...
  launch|*)
    ...
    rp=$(cat "$REPORTS/$RUN.pid" 2>/dev/null)
    ...
    echo "   Check anytime:  ./workspace/ops/run_7b_frontier.sh status" ;;
esac
