"""PROTECTED SOVEREIGN VERIFIER — G007

DeltaNet recurrent state as a checkpointable history primitive.
...
RECEIPT = Path("receipts/sovereign/G007_deltanet_state.json")
...
def test_state_round_trips_with_functional_equivalence():
    doc = _load()
    assert doc.get("state_bytes"), "state size unmeasured"
    assert doc.get("save_ns") and doc.get("restore_ns"), "save/restore unmeasured"
    assert doc.get("output_equivalent") is True, (
        "restored state did not reproduce equivalent output; serialisable is not restorable"
    )


def test_prefill_was_actually_avoided():
    doc = _load()
    avoided = doc.get("avoided_prefill_tokens")
    assert isinstance(avoided, (int, float)) and avoided > 0, (
        "no prefill was avoided; this is serialisation, not a context primitive"
    )
    assert doc.get("full_attention_kv_handled") is True, (
        "the 16 full-attention layers' KV was not accounted for in the restore"
    )
