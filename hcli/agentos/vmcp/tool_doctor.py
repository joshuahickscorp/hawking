...
dangerous argv are refused before exec. subprocess never uses shell=True.
    ...
    from hcli.agentos.vmcp.tool_doctor import profile, report
"""
...
from hcli.agentos.vmcp.receipt import (
    argv_list,
    basename_of,
    content_digest,
    dangerous_command,
...
E3_CLASSES: tuple[tuple[str, str, str], ...] = (
    ("file classifier", "CONNECTED", "hcli.agentos.vmcp.file_eye.observe"),
    ...
    ("image handling", "CONNECTED", "PNG/JPEG/GIF magic in hcli.agentos.vmcp.file_eye (no PIL)"),
    ...
    ("PTY capture", "PARKED", "openpty EPERM in this sandbox; hcli.agentos.vmcp.pty_eye"),
    ("process inspection", "CONNECTED", "hcli.agentos.vmcp.tool_doctor.profile"),
    ...
    for name, disposition, hosted_by in E3_CLASSES:
        ...
        if name == "PTY capture":
            if probed.get("ok"):
                ...
                row["hosted_by"] = "hcli.agentos.vmcp.pty_eye.capture"
            else:
