...
>
> **Correction 2 — the §3 baseline anchor is UNRECONCILED (~39 vs ~31).** The envelope below is drawn off a **~39 tps clean anchor** (the line just below and the "now (clean)" table row). The most recent clean measurement pins **~31** (A1 paired 30.94 → 31.55, `plans/overnight_build_queue_2026_05_31.md`; A4 untraced 31.0 median, `reports/a4_per_kernel_decode_profile_2026_05_31.md` + `reports/a4_clean_walltime.json`). This ~8-tps gap is **OPEN, not resolved** — every absolute number and every "% of the way to ~50" in §3 inherits the ambiguity until an attended session reconciles the two anchors under the §1 thermal protocol.
...
Anchored at ~39 tps clean (Q4_K_M, ~50% of peak, ~1.9 GB/token, theoretical-100% ≈ 78 tps). ⚠ *provisional — see §3.0 status correction; anchor unreconciled (~39 here vs ~31 most-recent-clean).* Deltas interact; this is reasoned composition, not multiplication.
