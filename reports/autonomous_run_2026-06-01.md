...
**Moved:** verified MOVE-1's QTIP oracle green + the stale-test caveat resolved, and produced the gate-sorted build-list. **Blocked:** the run's queue was never populated, so no builds were authorized; QTIP's decisive verdict is Colab-only; the clean-room absolute numbers need the app closed. **Needs your call:** authorize/paste the real queue (#1), then the goalpost + §3/§6 re-projection (#2/#3), and decide the 2 uncommitted Colab JSONs.
