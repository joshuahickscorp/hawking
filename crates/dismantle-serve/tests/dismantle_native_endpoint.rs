//! Track 8.2 gate — native `/v1/dismantle/generate` request mapping + stats
...
//!   cargo test -p dismantle-serve --test dismantle_native_endpoint

use dismantle_serve::http::{
    dismantle_generate_stats_json, map_dismantle_generate_req, DismantleGenerateReq,
};

fn parse(json: serde_json::Value) -> DismantleGenerateReq {
    serde_json::from_value(json).expect("parse DismantleGenerateReq")
}

#[test]
fn absent_temperature_maps_to_greedy() {
    let r = parse(serde_json::json!({"prompt": "hi", "max_tokens": 8}));
    let g = map_dismantle_generate_req(&r);
...
fn stats_json_has_genstats_field_names_and_dec_tps() {
    ...
    let j = dismantle_generate_stats_json(14, 64, 2000.0, true, "q4k-predec-f16s");
...
fn stats_dec_tps_zero_safe_when_no_decode() {
    let j = dismantle_generate_stats_json(5, 0, 0.0, false, "f16");
