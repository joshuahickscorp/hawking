...
pub trait ExactTarget {
    ...
    fn forward_tokens_verify(&mut self, tokens: &[u32], positions: &[usize]) -> Result<(Vec<u32>, Vec<Vec<f32>>)>;
...
impl Default for Verifier {
    fn default() -> Self {
        Self { max_batch: 8, want_residuals: false }
    }
}
...
impl Verifier {
    ...
    pub fn verify_line<T: ExactTarget>(&self, target: &mut T, bonus: u32, bonus_pos: usize, draft: &[u32]) -> Result<VerifyOutcome> {
        // Degenerate: empty draft → one plain greedy bonus step (still lossless).
        if draft.is_empty() {
            let corr = target.forward_token_greedy(bonus, bonus_pos)?;
            return Ok(VerifyOutcome { accepted: Vec::new(), correction: Some(corr), next_seq_len: bonus_pos + 1, residuals: Vec::new() });
        }
        ...
        let VerifyResult { accepted_count, first_divergent_token } = verify_draft_ids_until_mismatch(draft, |i| Ok(preds[i]))?;

        let accepted = draft[..accepted_count].to_vec();
        let next_seq_len = if first_divergent_token.is_some() { bonus_pos + accepted_count + 1 } else { bonus_pos + k };
        Ok(VerifyOutcome { accepted, correction: first_divergent_token, next_seq_len, residuals: if self.want_residuals { residuals } else { Vec::new() } })
    }
}
