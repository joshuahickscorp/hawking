...
/// primary unless `HAWKING_FORCE_CPU_OP` forces it onto the CPU
/// primitive. DEFAULT-UNSET ⇒ pure Metal ⇒ golden hash unchanged.
#[cfg(target_os = "macos")]
#[path = "backend_router.rs"]
pub mod router;
...
pub trait ComputeBackend:
    Backend + BackendGemv + BackendNorm + BackendElementwise + BackendRope + BackendAttention + BackendKvCache + BackendQuant + BackendEmbed + BackendSample + BackendMoe
{
}

impl<T> ComputeBackend for T where
    T: Backend + BackendGemv + BackendNorm + BackendElementwise + BackendRope + BackendAttention + BackendKvCache + BackendQuant + BackendEmbed + BackendSample + BackendMoe
{
}
