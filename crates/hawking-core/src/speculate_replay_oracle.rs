...
impl ReplayReport {
    ...
    pub fn best(&self) -> Option<&KReport> {
        self.per_k.iter().reduce(|a, b| if b.tau > a.tau || (b.tau == a.tau && b.k < a.k) { b } else { a })
    }
...
pub fn replay_k(corpus: &[u32], k: usize, warm_start_tokens: usize) -> KReport {
    ...
    let hit_rate = if cycles_with_proposal > 0 { cycles_hit as f32 / cycles_with_proposal as f32 } else { 0.0 };
    let proposal_coverage = cycles_with_proposal as f32 / f;
    let draft_accept_frac = if drafts_proposed > 0 { drafts_accepted as f32 / drafts_proposed as f32 } else { 0.0 };
...
pub fn replay_grid(corpus: &[u32], ks: &[usize], warm_start_tokens: usize) -> ReplayReport {
    ...
    ReplayReport { scored_tokens: corpus.len() - warm, warm_start_tokens: warm, per_k }
}
...
mod tests {
    ...
    fn repetitive_corpus_yields_positive_acceptance() {
        ...
        assert!(best.drafts_accepted > 0, "repetitive corpus must accept drafts (got {})", best.drafts_accepted);
        assert!(best.tau > 1.05, "repetitive corpus must beat plain decode (tau {})", best.tau);
        assert!(best.mean_accepted_len > 0.0 && best.proposal_coverage > 0.5, "draft should propose on most cycles and accept (mal {}, cov {})", best.mean_accepted_len, best.proposal_coverage);
        // Per-token accounting must close: positions advanced == corpus scored.
        for r in &rep.per_k {
            assert_eq!(r.tokens_emitted as usize, rep.scored_tokens, "k={} retired {} != scored {}", r.k, r.tokens_emitted, rep.scored_tokens);
            assert_eq!(r.drafts_accepted, r.accept_hist.iter().enumerate().map(|(na, c)| na as u64 * c).sum::<u64>(), "accept_hist must reconstruct drafts_accepted (k={})", r.k);
        }
    }
    ...
    fn non_repetitive_corpus_yields_near_zero_acceptance() {
        ...
        let rep = replay_grid(&corpus, &[4, 7], 0);
        for r in &rep.per_k {
            assert_eq!(r.drafts_accepted, 0, "an all-unique stream cannot accept any draft (k={}, got {})", r.k, r.drafts_accepted);
            assert!((r.tau - 1.0).abs() < 1e-3, "no acceptance must give tau~=1 (k={}, tau {})", r.k, r.tau);
            assert!(r.mean_accepted_len < 1e-6, "mean accepted-run length must be ~0 (k={}, {})", r.k, r.mean_accepted_len);
    ...
    fn warm_start_split_is_honored_and_helps() {
        ...
        let r = &rep.per_k[0];
        assert!(r.drafts_accepted > 0 && r.tau > 1.2, "warm-started repetitive suffix should draft well (acc {}, tau {})", r.drafts_accepted, r.tau);
    }
    ...
    fn governor_shuts_off_on_unpredictable_corpus() {
        ...
        let r = &rep.per_k[0];
        assert!(r.governor_propose_frac < 0.2, "governor must mostly disable on an unpredictable stream (frac {})", r.governor_propose_frac);
        // ...whereas on a predictable stream it stays mostly enabled.
        let good = repeated(&[1, 2, 3, 4], 500);
        let gr = replay_grid(&good, &[4], 0);
        assert!(gr.per_k[0].governor_propose_frac > 0.8, "governor must stay enabled on a predictable stream (frac {})", gr.per_k[0].governor_propose_frac);
    }
