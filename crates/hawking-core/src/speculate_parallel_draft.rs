...
mod tests {
    ...
    fn parallel_draft_scaffold_is_disabled() {
        ...
        assert!(!ParallelDraftProposer::is_enabled(), "ParallelDraftProposer must be disabled when HAWKING_EH_PARALLEL_DRAFT is not set");
    }
    ...
    fn parallel_draft_shape_and_budget_respected() {
        ...
        match proposal {
            Proposal::TokenLine(v) => {
                assert!(v.len() <= MAX_DRAFT_LEN, "propose must never emit more than MAX_DRAFT_LEN={} tokens, got {}", MAX_DRAFT_LEN, v.len());
            }
            other => panic!("expected TokenLine, got len={}", other.draft_len()),
        }
    }
