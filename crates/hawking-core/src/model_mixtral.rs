...
impl MixtralConfig {
    pub fn from_gguf(g: &GgufFile) -> Result<Self> {
        ...
        let n_layers = get_u32("llama.block_count").ok_or_else(|| Error::Model("missing llama.block_count".into()))? as usize;
        let hidden = get_u32("llama.embedding_length").ok_or_else(|| Error::Model("missing llama.embedding_length".into()))? as usize;
        let n_heads = get_u32("llama.attention.head_count").ok_or_else(|| Error::Model("missing llama.attention.head_count".into()))? as usize;
        let n_kv_heads = get_u32("llama.attention.head_count_kv").unwrap_or(n_heads as u32) as usize;
        let intermediate = get_u32("llama.feed_forward_length").ok_or_else(|| Error::Model("missing llama.feed_forward_length".into()))? as usize;
        let n_experts = get_u32("llama.expert_count").ok_or_else(|| Error::Model("missing llama.expert_count".into()))? as usize;
        let top_k = get_u32("llama.expert_used_count").unwrap_or(2) as usize;
        let vocab_size = get_u32("llama.vocab_size").map(|v| v as usize).or_else(|| token_embd_vocab_size_opt(g)).ok_or_else(|| Error::Model("missing llama vocab size".into()))?;

        if hidden % n_heads != 0 {
            return Err(Error::Model(format!("Mixtral hidden/head mismatch: hidden={hidden} n_heads={n_heads}")));
        }
...
impl MixtralEngine {
    ...
    pub fn synthetic_forward_shape_for_test(config: &MixtralConfig, token: u32) -> Vec<f32> {
        (0..config.vocab_size).map(|i| ((i as u32 ^ token) as f32) / config.vocab_size as f32).collect()
    }

    fn dequant_f32_expected(g: &GgufFile, name: &str, dtype: GgmlType, rows: usize, cols: usize) -> Result<Vec<f32>> {
    ...
    fn tensor_ref_expected(g: &GgufFile, name: &str, dtype: GgmlType, rows: usize, cols: usize) -> Result<TensorRef> {
        Self::tensor_ref_expected_any(g, name, &[dtype], rows, cols)
    }

    fn tensor_ref_expected_any(g: &GgufFile, name: &str, dtypes: &[GgmlType], rows: usize, cols: usize) -> Result<TensorRef> {
        ...
        let expected = rows.checked_mul(cols).ok_or_else(|| Error::Model(format!("tensor `{name}` shape overflow")))?;
        if !dtypes.contains(&info.dtype) || n_elems != expected {
            return Err(Error::Model(format!("tensor `{name}` expected one of {:?} {rows}x{cols} ({expected} elems), got {:?} dims {:?}", dtypes, info.dtype, info.dims)));
        }
        ...
        if chunk_offset + byte_size > WEIGHT_CHUNK_STRIDE + WEIGHT_CHUNK_OVERLAP {
            return Err(Error::Model(format!("tensor `{name}` crosses Mixtral weight chunk coverage: chunk_offset={chunk_offset} byte_size={byte_size}")));
        }
        Ok(TensorRef { offset, byte_size, dtype: info.dtype, n_elems, rows, cols, chunk_index: offset / WEIGHT_CHUNK_STRIDE, chunk_offset })
    }
    ...
    fn pin_weight_chunks(ctx: &MetalContext, gguf: &GgufFile) -> Result<Vec<PinnedBuffer>> {
        ...
        while start < gguf.mmap.len() {
            ...
            if buf.length() < requested as u64 {
                return Err(Error::Metal(format!("Mixtral weight chunk no-copy failed at bytes [{start}, {end}) requested={requested} actual={}", buf.length())));
            }
            chunks.push(buf);
...
impl Engine for MixtralEngine {
    fn load(weights: &Path, config: EngineConfig) -> Result<Self> {
        ...
        for li in 0..cfg.n_layers {
            ...
            let attn_q = Self::tensor_ref_k_quant(&gguf, &lp("attn_q.weight"), cfg.hidden, cfg.hidden)?;
            let attn_output = Self::tensor_ref_k_quant(&gguf, &lp("attn_output.weight"), cfg.hidden, cfg.hidden)?;
            ...
            for eid in 0..cfg.n_experts {
                ffn_gate.push(Self::tensor_ref_k_quant(&gguf, &lp(&format!("ffn_gate.{eid}.weight")), cfg.intermediate, cfg.hidden)?);
                ffn_up.push(Self::tensor_ref_k_quant(&gguf, &lp(&format!("ffn_up.{eid}.weight")), cfg.intermediate, cfg.hidden)?);
                ffn_down.push(Self::tensor_ref_k_quant(&gguf, &lp(&format!("ffn_down.{eid}.weight")), cfg.hidden, cfg.intermediate)?);
            }

            let mut pinned = MixtralLayerPinned::default();
            #[cfg(target_os = "macos")]
            if let Some(ctx) = metal_ctx.as_ref() {
                let upload_f32 = |w: &[f32]| ctx.new_buffer_with_bytes(bytemuck::cast_slice::<f32, u8>(w));
                pinned.attn_norm = Some(upload_f32(&attn_norm));
                pinned.ffn_norm = Some(upload_f32(&ffn_norm));
                pinned.attn_k = Some(upload_f32(&Self::dequant_f32_expected(&gguf, &lp("attn_k.weight"), GgmlType::Q8_0, kv_hidden, cfg.hidden)?));
                pinned.attn_v = Some(upload_f32(&Self::dequant_f32_expected(&gguf, &lp("attn_v.weight"), GgmlType::Q8_0, kv_hidden, cfg.hidden)?));
                pinned.gate_inp = Some(upload_f32(&Self::dequant_f32_expected(&gguf, &lp("ffn_gate_inp.weight"), GgmlType::F16, cfg.n_experts, cfg.hidden)?));
            }
            #[cfg(not(target_os = "macos"))]
            {
                let _ = kv_hidden;
            ...
            layers.push(MixtralLayer { attn_norm, ffn_norm, attn_q, attn_output, ffn_gate, ffn_up, ffn_down, pinned });
        }

        #[cfg(target_os = "macos")]
        let weights_mmap_buf = if let Some(ctx) = metal_ctx.as_ref() { Some(Self::pin_weight_chunks(ctx, &gguf)?) } else { None };
        #[cfg(not(target_os = "macos"))]
        let weights_mmap_buf: Option<Vec<PinnedBuffer>> = None;

        #[cfg(target_os = "macos")]
        let (embed_buf, final_norm_buf, lm_head_buf, decode_arena) = if let Some(ctx) = metal_ctx.as_ref() {
        ...
        #[cfg(not(target_os = "macos"))]
        let (embed_buf, final_norm_buf, lm_head_buf, decode_arena): (Option<PinnedBuffer>, Option<PinnedBuffer>, Option<PinnedBuffer>, Option<MixtralDecodeArena>) = (None, None, None, None);
    ...
    fn generate(&mut self, req: GenerateRequest, sink: &mut dyn FnMut(StreamEvent)) -> Result<GenStats> {
        ...
        let mut last_id = *prompt_ids.last().ok_or_else(|| Error::Model("empty prompt after tokenization".into()))?;
...
impl MixtralEngine {
    ...
    fn read_f32_buffer(buf: &PinnedBuffer, out: &mut [f32]) -> Result<()> {
        ...
        if buf.length() < bytes as u64 {
            return Err(Error::Kernel(format!("read_f32_buffer: buffer too small: got {} expected {bytes}", buf.length())));
        }
    ...
    fn write_f32_buffer(buf: &PinnedBuffer, xs: &[f32]) -> Result<()> {
        ...
        if buf.length() < bytes as u64 {
            return Err(Error::Kernel(format!("write_f32_buffer: buffer too small: got {} expected {bytes}", buf.length())));
        }
    ...
    fn l2_norm(xs: &[f32]) -> f32 {
        xs.iter().map(|v| (*v as f64) * (*v as f64)).sum::<f64>().sqrt() as f32
    }
    ...
    fn debug_buffer(label: &str, buf: &PinnedBuffer, len: usize, first: usize) -> Result<()> {
        ...
        eprintln!("[mixtral-debug] {label} l2={:.6} first{}={:?}", Self::l2_norm(&xs), first, Self::debug_first(&xs, first));
        Ok(())
    }
    ...
    fn debug_top_logits(&self, logits: &[f32]) {
        ...
        let decoded: Vec<(usize, String)> = idx.iter().take(3).map(|&i| (i, self.tokenizer.decode_one(i as u32).unwrap_or_default())).collect();
        eprintln!("[mixtral-debug] lm_head top10={top:?} top3_decoded={decoded:?}");
    }

    #[cfg(target_os = "macos")]
    fn encode_k_quant_tcb(&self, tcb: &mut crate::metal::TokenCommandBuffer<'_>, t: &TensorRef, x_buf: &PinnedBuffer, out_buf: &PinnedBuffer) -> Result<()> {
        let chunks = self.weights_mmap_buf.as_ref().ok_or_else(|| Error::Model("Mixtral quant path missing weight chunks".into()))?;
        let chunk = chunks.get(t.chunk_index).ok_or_else(|| Error::Model(format!("Mixtral tensor chunk {} missing for offset {}", t.chunk_index, t.offset)))?;
        match t.dtype {
            GgmlType::Q3_K => crate::kernels::gemv_q3_k_pinned_tcb(tcb, chunk, t.chunk_offset, t.byte_size, t.rows, t.cols, x_buf, out_buf),
            GgmlType::Q4_K => crate::kernels::gemv_q4_k_m_v2_pinned_tcb(tcb, chunk, t.chunk_offset, t.byte_size, t.rows, t.cols, x_buf, out_buf),
            other => Err(Error::Model(format!("Mixtral GPU path unsupported quant dtype {:?}", other))),
        }
    }
    ...
    fn forward_token_tcb(&mut self, token: u32, pos: usize) -> Result<Vec<f32>> {
        ...
        let ctx = self.metal_ctx.as_ref().ok_or_else(|| Self::missing_buf("metal_ctx"))?;
        let arena = self.decode_arena.as_ref().ok_or_else(|| Self::missing_buf("decode_arena"))?;
        let embed_buf = self.embed_buf.as_ref().ok_or_else(|| Self::missing_buf("embed_buf"))?;
        let final_norm_buf = self.final_norm_buf.as_ref().ok_or_else(|| Self::missing_buf("final_norm_buf"))?;
        let lm_head_buf = self.lm_head_buf.as_ref().ok_or_else(|| Self::missing_buf("lm_head_buf"))?;
        let debug = std::env::var("HAWKING_MIXTRAL_DEBUG").is_ok();
        if debug {
            eprintln!("[mixtral-debug] token={token} pos={pos} seq_len={} rope_theta={} hidden={} heads={} kv_heads={}", self.kv.seq_len, cfg.rope_theta, h, n_heads, n_kv_heads);
        }

        for li in 0..cfg.n_layers {
            {
                let layer = &self.layers[li];
                let attn_norm = layer.pinned.attn_norm.as_ref().ok_or_else(|| Self::missing_buf("layer.attn_norm"))?;
                let attn_k = layer.pinned.attn_k.as_ref().ok_or_else(|| Self::missing_buf("layer.attn_k"))?;
                let attn_v = layer.pinned.attn_v.as_ref().ok_or_else(|| Self::missing_buf("layer.attn_v"))?;
                let debug_layer = debug && li < 3;

                let mut tcb = crate::metal::TokenCommandBuffer::new(ctx);
                if li == 0 {
                    crate::kernels::embed_lookup_metal_f32_tcb(&mut tcb, embed_buf, token, h, &arena.x_buf)?;
                }
                crate::kernels::rmsnorm_metal_buf_tcb(&mut tcb, &arena.x_buf, attn_norm, cfg.rms_norm_eps, h, &arena.x_norm_buf)?;
                self.encode_k_quant_tcb(&mut tcb, &layer.attn_q, &arena.x_norm_buf, &arena.q_buf)?;
                crate::kernels::gemv_f32_attn_pinned_buf_tcb(&mut tcb, attn_k, kv_hidden, h, &arena.x_norm_buf, &arena.k_buf)?;
                crate::kernels::gemv_f32_attn_pinned_buf_tcb(&mut tcb, attn_v, kv_hidden, h, &arena.x_norm_buf, &arena.v_buf)?;
                if debug_layer {
                    tcb.commit_and_wait()?;
                    if li == 0 {
                    ...
                    crate::kernels::rope_q_f32_inplace_tcb(&mut rope_tcb, &arena.q_buf, n_heads, head_dim, 0, head_dim, pos as u32, cfg.rope_theta)?;
                    crate::kernels::rope_q_f32_inplace_tcb(&mut rope_tcb, &arena.k_buf, n_kv_heads, head_dim, 0, head_dim, pos as u32, cfg.rope_theta)?;
                    rope_tcb.commit_and_wait()?;
                    Self::debug_buffer(&format!("layer{li}.q_post_rope_head0"), &arena.q_buf, head_dim, 8)?;
                } else {
                    crate::kernels::rope_q_f32_inplace_tcb(&mut tcb, &arena.q_buf, n_heads, head_dim, 0, head_dim, pos as u32, cfg.rope_theta)?;
                    crate::kernels::rope_q_f32_inplace_tcb(&mut tcb, &arena.k_buf, n_kv_heads, head_dim, 0, head_dim, pos as u32, cfg.rope_theta)?;
                    tcb.commit_and_wait()?;
                }
            }
            ...
            {
                let layer = &self.layers[li];
                let ffn_norm = layer.pinned.ffn_norm.as_ref().ok_or_else(|| Self::missing_buf("layer.ffn_norm"))?;
                let gate_inp = layer.pinned.gate_inp.as_ref().ok_or_else(|| Self::missing_buf("layer.gate_inp"))?;
                let mut tcb = crate::metal::TokenCommandBuffer::new(ctx);
                self.encode_k_quant_tcb(&mut tcb, &layer.attn_output, &arena.attn_out_buf, &arena.proj_out_buf)?;
                crate::kernels::add_inplace_metal_tcb(&mut tcb, &arena.x_buf, &arena.proj_out_buf, h)?;
                crate::kernels::rmsnorm_metal_buf_tcb(&mut tcb, &arena.x_buf, ffn_norm, cfg.rms_norm_eps, h, &arena.x_norm_buf)?;
                crate::kernels::gemv_f32_attn_pinned_buf_tcb(&mut tcb, gate_inp, cfg.n_experts, h, &arena.x_norm_buf, &arena.gate_logits_buf)?;
                tcb.commit_and_wait()?;
            }
            ...
            if debug && li < 3 {
                eprintln!("[mixtral-debug] layer{li}.gate raw={raw_gate_logits:?} routes={routes:?}");
            }

            {
                ...
                for (route_i, (eid, _weight)) in routes.iter().enumerate() {
                    self.encode_k_quant_tcb(&mut tcb, &layer.ffn_gate[*eid], &arena.x_norm_buf, &arena.expert_gate_bufs[route_i])?;
                    self.encode_k_quant_tcb(&mut tcb, &layer.ffn_up[*eid], &arena.x_norm_buf, &arena.expert_up_bufs[route_i])?;
                    crate::kernels::silu_mul_tcb(&mut tcb, &arena.expert_gate_bufs[route_i], &arena.expert_up_bufs[route_i], &arena.expert_act_bufs[route_i], cfg.intermediate)?;
                    self.encode_k_quant_tcb(&mut tcb, &layer.ffn_down[*eid], &arena.expert_act_bufs[route_i], &arena.expert_out_bufs[route_i])?;
                }
                tcb.commit_and_wait()?;
            }
        ...
        let mut tcb = crate::metal::TokenCommandBuffer::new(ctx);
        crate::kernels::rmsnorm_metal_buf_tcb(&mut tcb, &arena.x_buf, final_norm_buf, cfg.rms_norm_eps, h, &arena.x_norm_buf)?;
        crate::kernels::gemv_f16_metal_buf_tcb(&mut tcb, lm_head_buf, cfg.vocab_size, h, &arena.x_norm_buf, &arena.logits_buf)?;
        tcb.commit_and_wait()?;
        if debug {
