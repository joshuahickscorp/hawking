...
pub mod inner {
    ...
    impl LayerMetalBuffers {
        ...
        pub fn upload(ctx: &MetalContext, w: &MegakernelLayerWeightsF16) -> (Self, MkLayerArgs) {
            ...
            (Self { qw, kw, vw, ow, gw, uw, dw, attn_norm, ffn_norm, qb, kb, vb }, args)
        }
        ...
        pub fn mark_used(&self, enc: &metal::ComputeCommandEncoderRef) {
            let mut refs: Vec<&metal::ResourceRef> = vec![&self.qw, &self.kw, &self.vw, &self.ow, &self.gw, &self.uw, &self.dw, &self.attn_norm, &self.ffn_norm];
    ...
    pub fn megakernel_2layer_dispatch(
        ...
        if x_in.len() != MK_HIDDEN {
            return Err(Error::Metal(format!("megakernel_2layer_dispatch: x_in len {} != MK_HIDDEN {}", x_in.len(), MK_HIDDEN)));
        }
        if (max_seq as usize) < 1 {
            return Err(Error::Metal("megakernel_2layer_dispatch: max_seq must be ≥ 1".into()));
        }

        // Residual buffers.
        let x_in_bytes = unsafe { std::slice::from_raw_parts(x_in.as_ptr() as *const u8, std::mem::size_of_val(x_in)) };
        ...
        let l0_arg_bytes: [u8; std::mem::size_of::<MkLayerArgs>()] = unsafe { std::mem::transmute(l0_args) };
        let l1_arg_bytes: [u8; std::mem::size_of::<MkLayerArgs>()] = unsafe { std::mem::transmute(l1_args) };
        let l0_arg_buf = ctx.new_buffer_with_bytes(&l0_arg_bytes);
        let l1_arg_buf = ctx.new_buffer_with_bytes(&l1_arg_bytes);

        // Scalar argbuf.
        let scalar_args = MkArgs { pos, seq_len, max_seq, probe_stage, n_layers: 2 };
        let scalar_bytes: [u8; std::mem::size_of::<MkArgs>()] = unsafe { std::mem::transmute(scalar_args) };
        let scalar_buf = ctx.new_buffer_with_bytes(&scalar_bytes);

        let shmem_bytes = (MK_SHMEM_HALFS * std::mem::size_of::<f16>()) as u64;

        ctx.dispatch_threads("qwen3b_megakernel_2layer", (MK_TG_SIZE, 1, 1), (MK_TG_SIZE, 1, 1), |enc| {
    ...
    pub fn megakernel_nlayer_dispatch(ctx: &MetalContext, layers: &[MegakernelLayerWeightsF16], x_in: &[f16], pos: u32, seq_len: u32, max_seq: u32) -> Result<Vec<f16>> {
        if x_in.len() != MK_HIDDEN {
            return Err(Error::Metal(format!("megakernel_nlayer_dispatch: x_in len {} != MK_HIDDEN {}", x_in.len(), MK_HIDDEN)));
        }
        if layers.is_empty() {
            return Err(Error::Metal("megakernel_nlayer_dispatch: layers must be non-empty".into()));
        }
        if (max_seq as usize) < 1 || seq_len > max_seq {
            return Err(Error::Metal(format!("megakernel_nlayer_dispatch: need 1 ≤ seq_len ({seq_len}) ≤ max_seq ({max_seq})")));
        }
        let n = layers.len();
        ...
        // wire layout.
        let layer_args_bytes = unsafe { std::slice::from_raw_parts(layer_args.as_ptr() as *const u8, std::mem::size_of_val(layer_args.as_slice())) };
        let layer_args_buf = ctx.new_buffer_with_bytes(layer_args_bytes);

        // Scalar argbuf.
        let scalar_args = MkArgs { pos, seq_len, max_seq, probe_stage: 0, n_layers: n as u32 };
        let scalar_bytes: [u8; std::mem::size_of::<MkArgs>()] = unsafe { std::mem::transmute(scalar_args) };
        let scalar_buf = ctx.new_buffer_with_bytes(&scalar_bytes);

        let shmem_bytes = (MK_SHMEM_HALFS * std::mem::size_of::<f16>()) as u64;

        ctx.dispatch_threads("qwen3b_megakernel_nlayer", (MK_TG_SIZE, 1, 1), (MK_TG_SIZE, 1, 1), |enc| {
            enc.set_buffer(0, Some(&scalar_buf), 0);
            enc.set_buffer(1, Some(&x_in_buf), 0);
            enc.set_buffer(2, Some(&x_out_buf), 0);
            enc.set_buffer(3, Some(&k_cache_buf), 0);
    ...
    impl MegakernelRunner {
        pub fn new(ctx: &MetalContext, layers: &[MegakernelLayerWeightsF16], max_seq: u32) -> Result<Self> {
            if layers.is_empty() {
                return Err(Error::Metal("MegakernelRunner::new: layers must be non-empty".into()));
            }
            let n = layers.len();
        ...
        pub fn step(&self, ctx: &MetalContext, x_in: &[f16], pos: u32, seq_len: u32) -> Result<Vec<f16>> {
            if x_in.len() != MK_HIDDEN {
                return Err(Error::Metal(format!("MegakernelRunner::step: x_in len {} != {}", x_in.len(), MK_HIDDEN)));
            }
            unsafe {
                let dst = self.x_in_buf.contents() as *mut f16;
                std::ptr::copy_nonoverlapping(x_in.as_ptr(), dst, MK_HIDDEN);
            }
            let scalar_args = MkArgs { pos, seq_len, max_seq: self.max_seq, probe_stage: 0, n_layers: self.n_layers };
            let scalar_bytes: [u8; std::mem::size_of::<MkArgs>()] = unsafe { std::mem::transmute(scalar_args) };
            let scalar_buf = ctx.new_buffer_with_bytes(&scalar_bytes);
            let shmem_bytes = (MK_SHMEM_HALFS * std::mem::size_of::<f16>()) as u64;
            ctx.dispatch_threads("qwen3b_megakernel_nlayer", (MK_TG_SIZE, 1, 1), (MK_TG_SIZE, 1, 1), |enc| {
...
pub use inner::{
    megakernel_2layer_dispatch, megakernel_nlayer_dispatch, LayerMetalBuffers, MegakernelRunner, MkArgs, MkLayerArgs, MK_PROBE_ATTN_OUT, MK_PROBE_FFN_DOWN, MK_PROBE_O_PROJ, MK_PROBE_Q_ROT,
