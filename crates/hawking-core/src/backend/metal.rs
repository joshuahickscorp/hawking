    ...
    fn read_u32(&mut self, buf: &Self::Buffer, index: usize) -> Result<u32> {
        ...
        if ptr.is_null() {
            return Err(Error::Metal(
                "MetalRecorder::read_u32: buffer contents() returned null".into(),
            ));
        }
...
impl BackendGemv for MetalBackend {
    fn gemv(
        ...
        match spec.weight {
            ...
            WeightKind::Q4K | WeightKind::Q4kFast => {
                ...
                if spec.w4a8 {
                    ...
                    kernels::gemm_q4_k_a8_v3_8r_pinned_tcb(
                        tcb, weight, w_off, w_bytes, rows, cols, x_int8, x_scales, out,
                    )
                } else {
                    kernels::gemv_q4_k_m_v3_8r_pinned_tcb(
                        tcb, weight, w_off, w_bytes, rows, cols, x, out,
                    )
                }
            }
            // ── Q6_K ─────────────────────────────────────────────────
            ...
            WeightKind::F32 => {
                kernels::gemv_f32_attn_pinned_buf_tcb(tcb, weight, rows, cols, x, out)
            }
        }
    }
}
...
impl BackendRope for MetalBackend {
    ...
    fn rope(
        ...
        match layout {
            ...
            RopeLayout::Partial { offset, rope_dim } => {
                kernels::rope_slice_f32_inplace_tcb(tcb, buf, offset, rope_dim, pos, base)
            }
        }
    }
}
...
impl BackendAttention for MetalBackend {
    fn attention(
        ...
        match kind {
            ...
            AttentionKind::Mha => kernels::mha_decode_f32_tcb(
                &mut rec.tcb,
                q,
                k_cache,
                /* k_off_bytes */ 0,
