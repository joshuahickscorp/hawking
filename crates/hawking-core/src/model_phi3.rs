...
use crate::kernels::{add_inplace, embed_lookup, rope_inplace_longrope, silu_mul};
...
impl Phi3Config {
    pub fn from_gguf(g: &GgufFile) -> Result<Self> {
        ...
        // sliding-window check below keep `get_u32`.
        let r = ArchReader::new(g, "phi3");
        ...
        let max_seq_len = r.opt_usize("context_length", 4096);

        if let Some(win) = get_u32("phi3.attention.sliding_window") {
            if (win as usize) < max_seq_len {
                eprintln!(
                    "hawking: warning — GGUF declares phi3 sliding_window={win} but the \
                     engine runs full causal attention; output may drift beyond {win} tokens"
                );
            }
        }

        Ok(Self { n_layers, hidden, n_heads, n_kv_heads, head_dim, intermediate, vocab_size, rope_theta, rms_norm_eps, max_seq_len })
    }
}
...
impl Phi3 {
    ...
    fn sub_rows(t: &TensorRef, cols: usize, row_start: usize, n_rows: usize) -> Result<TensorRef> {
        ...
        if total_rows * cols != t.n_elems {
            return Err(Error::Model(format!("phi3 fused split: n_elems={} not divisible by cols={cols}", t.n_elems)));
        }
        if t.byte_size % total_rows != 0 {
            return Err(Error::Model(format!("phi3 fused split: byte_size={} not divisible by rows={total_rows}", t.byte_size)));
        }
        let bytes_per_row = t.byte_size / total_rows;
        if row_start + n_rows > total_rows {
            return Err(Error::Model(format!("phi3 fused split out of range: {row_start}+{n_rows} > {total_rows}")));
        }
        Ok(TensorRef { offset: t.offset + row_start * bytes_per_row, byte_size: n_rows * bytes_per_row, dtype: t.dtype, n_elems: n_rows * cols })
    }

    fn rmsnorm_dispatch(&self, x: &[f32], weight: &[f32], eps: f32, out: &mut [f32]) -> Result<()> {
        super::rmsnorm_dispatch(self.metal_ctx.as_ref(), x, weight, eps, out)
    }

    fn matmul_q4_dispatch(&self, t: &TensorRef, rows: usize, cols: usize, x: &[f32], out: &mut [f32], scratch: &mut Vec<f32>) -> Result<()> {
        super::matmul_q4_dispatch(&self.gguf, self.metal_ctx.as_ref(), t, rows, cols, x, out, scratch)
    }

    fn gemv_f16_dispatch(&self, w_f16: &[f16], rows: usize, cols: usize, x: &[f32], out: &mut [f32]) -> Result<()> {
        super::gemv_f16_dispatch(self.metal_ctx.as_ref(), w_f16, rows, cols, x, out)
    }
    ...
    pub(crate) fn forward_token(&mut self, token: u32, pos: usize) -> Result<Vec<f32>> {
        ...
        for li in 0..n_layers {
            ...
            for h_i in 0..n_heads {
                ...
                rope_inplace_longrope(&mut q_full[off..off + head_dim], pos as u32, rope_theta, &self.rope_ext_factors, mscale);
            }
            for h_i in 0..n_kv_heads {
                let off = h_i * head_dim;
                rope_inplace_longrope(&mut k_token[off..off + head_dim], pos as u32, rope_theta, &self.rope_ext_factors, mscale);
            }
...
impl Engine for Phi3 {
    fn load(weights: &Path, config: EngineConfig) -> Result<Self> {
        ...
        let orig_ctx = gguf.metadata.get("phi3.rope.scaling.original_context_length").and_then(|v| v.as_u32()).map(|v| v as usize).unwrap_or(max_seq);
        ...
        let rope_ext_factors = if use_long { long_f.or(short_f) } else { short_f.or(long_f) }.filter(|f| f.len() == half).unwrap_or_else(|| vec![1.0f32; half]);
        ...
        let rope_mscale = gguf.metadata.get("phi3.rope.scaling.attn_factor").and_then(|v| v.as_f32()).unwrap_or_else(|| {
    ...
    fn generate(&mut self, req: GenerateRequest, sink: &mut dyn FnMut(StreamEvent)) -> Result<GenStats> {
        ...
        let mut stats = GenStats { prompt_tokens: prompt_len, profile_id: self.kernel_profile.as_ref().map(|p| p.profile_id.clone()), ..Default::default() };
...
mod tests {
    ...
    fn sub_rows_splits_on_block_boundaries() {
        ...
        let fused = TensorRef { offset: 1000, byte_size: rows * bytes_per_row, dtype: GgmlType::Q4_K, n_elems: rows * cols };
    ...
    fn sub_rows_rejects_out_of_range() {
        let fused = TensorRef { offset: 0, byte_size: 12 * 144, dtype: GgmlType::Q4_K, n_elems: 12 * 256 };
