...
impl PhysicalTraceIdentity {
    pub fn new(interval_id: String, run_nonce: String, phase: String, role: String, batch: Option<usize>, iteration: usize) -> Result<Self> {
        ...
        if !is_sha256(&interval_id) || !is_sha256(&run_nonce) {
            return Err(Error::Metal("physical trace interval id and run nonce must be lowercase SHA-256".into()));
        }
        if !safe(&phase) || !safe(&role) {
            return Err(Error::Metal("physical trace phase and role contain unsafe label bytes".into()));
        }
        Ok(Self { interval_id, run_nonce, phase, role, batch, iteration })
    }
    ...
    fn base_label(&self) -> String {
        format!(
            ...
            self.phase,
            self.role,
            self.batch.map(|value| value.to_string()).unwrap_or_else(|| "none".to_owned()),
            self.iteration,
        )
    }
}
...
fn physical_command_label(kind: &str) -> Option<(PhysicalCommandIdentity, String)> {
    ...
    let label = format!("{}|kind={kind}|command_index={command_index}", state.identity.base_label(),);
    Some((PhysicalCommandIdentity { state, command_index }, label))
}

fn physical_encoder_label(command: &PhysicalCommandIdentity, kind: &str, kernel: &str) -> String {
    let encoder_index = command.state.next_encoder.fetch_add(1, AtomicOrdering::Relaxed);
    let safe_kernel: String = kernel.chars().map(|ch| if ch.is_ascii_alphanumeric() || matches!(ch, '_' | '-' | '.') { ch } else { '_' }).collect();
    format!("{}|kind={kind}|command_index={}|encoder_index={encoder_index}|kernel={safe_kernel}", command.state.identity.base_label(), command.command_index,)
}
...
mod physical_signpost {
    ...
    impl Interval {
        pub fn begin(name: String, interval_id: &str) -> std::result::Result<Self, String> {
            ...
            let mut id = u64::from_str_radix(&interval_id[..16], 16).map_err(|_| "invalid signpost interval id".to_owned())?;
...
impl PhysicalTraceGuard {
    pub fn begin(identity: PhysicalTraceIdentity) -> Result<Self> {
        let state = Arc::new(PhysicalTraceState { identity, next_command: AtomicU64::new(0), next_encoder: AtomicU64::new(0) });
        let mut active = active_physical_trace().lock().map_err(|_| Error::Metal("physical trace mutex poisoned".into()))?;
        if active.is_some() {
            return Err(Error::Metal("a physical trace interval is already active".into()));
        }
        #[cfg(target_os = "macos")]
        let signpost = physical_signpost::Interval::begin(state.identity.base_label(), &state.identity.interval_id).map_err(Error::Metal)?;
...
impl Drop for PhysicalTraceGuard {
    fn drop(&mut self) {
        if let Ok(mut active) = active_physical_trace().lock() {
            if active.as_ref().is_some_and(|current| Arc::ptr_eq(current, &self.state)) {
                *active = None;
            }
        }
...
pub fn all_shader_sources() -> String {
    ...
    let mut srcs = vec![SHADER_COMMON, SHADER_QUANT, SHADER_MOE, SHADER_ATTN, SHADER_SAMPLE, SHADER_MATMUL, SHADER_MHA, SHADER_MEGAKERNEL, SHADER_RWKV7];
...
mod imp {
    ...
    use metal::{Buffer, CommandBufferRef, CommandQueue, ComputeCommandEncoder, ComputePipelineState, Device, Library, MTLDispatchType, MTLResourceOptions, MTLSize};
    ...
    unsafe fn install_residency_set(device: &metal::DeviceRef, queue: &metal::CommandQueueRef, allocations: &[&metal::BufferRef]) -> Result<()> {
        ...
        let set: *mut Object = msg_send![device, newResidencySetWithDescriptor: desc error: &mut err];
    ...
    impl ProdCbTracer {
        ...
        fn try_new(device: &Device) -> Option<Self> {
            ...
            Some(Self { sample_buf, next_pair: AtomicUsize::new(0), capacity_pairs: Self::CAPACITY_PAIRS, pending: Mutex::new(Vec::with_capacity(Self::CAPACITY_PAIRS)) })
        }
        ...
        fn record_pending(&self, kernel_name: &'static str, cpu_us: u64, pair_index: usize, layer_hint: Option<u32>) {
            self.pending.lock().push(ProdCbPending { kernel_name, cpu_us, pair_index, layer_hint });
        }
        ...
        fn drain(&self) -> Vec<super::DispatchSample> {
            ...
            if pair_count == 0 {
                return pending
                    .into_iter()
                    .map(|p| super::DispatchSample { kernel_name: p.kernel_name, wall_us: p.cpu_us, layer_hint: p.layer_hint, gpu_us: None, gpu_start_ns: None, gpu_end_ns: None })
                    .collect();
            }
            ...
            let timestamps = unsafe {
                let ns_range = ::metal::NSRange { location: 0, length: (pair_count * 2) as u64 };
                let nsdata: *mut metal::objc::runtime::Object = msg_send![&*self.sample_buf, resolveCounterRange: ns_range];
                if nsdata.is_null() {
                    Vec::new()
                } else {
            ...
            pending
                ...
                .map(|p| {
                    ...
                    let i1 = i0 + 1;
                    let valid = i1 < timestamps.len() && timestamps[i0] != ERR && timestamps[i1] != ERR && timestamps[i1] >= timestamps[i0];
                    let (gpu_us, gpu_start_ns, gpu_end_ns) = if valid { (Some((timestamps[i1] - timestamps[i0]) / 1000), Some(timestamps[i0]), Some(timestamps[i1])) } else { (None, None, None) };
                    super::DispatchSample { kernel_name: p.kernel_name, wall_us: p.cpu_us, layer_hint: p.layer_hint, gpu_us, gpu_start_ns, gpu_end_ns }
                })
                .collect()
        }
    }
    ...
    impl DispatchTrace {
        ...
        fn record(&self, kernel_name: &'static str, wall_us: u64, layer_hint: Option<u32>) {
            self.samples.lock().push(super::DispatchSample { kernel_name, wall_us, layer_hint, gpu_us: None, gpu_start_ns: None, gpu_end_ns: None });
        }
    ...
    impl MetalContextStats {
        fn new() -> Self {
            Self { buffers_created: AtomicUsize::new(0), bytes_allocated: AtomicUsize::new(0), commits: AtomicUsize::new(0) }
        }

        /// Drain counters, returning (buffers_created, bytes_allocated, commits).
        pub fn drain(&self) -> (usize, usize, usize) {
            (self.buffers_created.swap(0, Ordering::Relaxed), self.bytes_allocated.swap(0, Ordering::Relaxed), self.commits.swap(0, Ordering::Relaxed))
        }
    }
    ...
    impl MetalContext {
        ...
        pub fn new_with_trace(trace_dispatch: bool) -> Result<Self> {
            let device = Device::system_default().ok_or_else(|| Error::Metal("no Metal-capable GPU".into()))?;
            let queue = device.new_command_queue();
            let opts = metal::CompileOptions::new();
            let src = super::all_shader_sources();
            let library = device.new_library_with_source(&src, &opts).map_err(|e| Error::Metal(format!("shader compile: {e}")))?;
            ...
            Ok(Self {
                inner: Arc::new(Inner { device, queue, library, pipelines: Mutex::new(HashMap::new()) }),
        ...
        pub fn pipeline(&self, fn_name: &str) -> Result<ComputePipelineState> {
            ...
            let f = self.inner.library.get_function(fn_name, None).map_err(|e| Error::Metal(format!("kernel `{fn_name}` not found: {e}")))?;
            let p = self.inner.device.new_compute_pipeline_state_with_function(&f).map_err(|e| Error::Metal(format!("pipeline `{fn_name}`: {e}")))?;
        ...
        pub fn new_buffer(&self, len: usize) -> Buffer {
            ...
            self.inner.device.new_buffer(len as u64, MTLResourceOptions::StorageModeShared)
        }
        ...
        pub fn new_buffer_with_bytes(&self, bytes: &[u8]) -> Buffer {
            ...
            self.inner.device.new_buffer_with_data(bytes.as_ptr() as *const _, bytes.len() as u64, MTLResourceOptions::StorageModeShared)
        }
        ...
        pub fn new_buffer_checked(&self, len: usize) -> Result<Buffer> {
            ...
            if len as u64 > ceiling {
                return Err(Error::Metal(format!("MTLBuffer allocation of {len} B exceeds device working-set ceiling {ceiling} B")));
            }
            if self.trace_dispatch {
                self.stats.buffers_created.fetch_add(1, Ordering::Relaxed);
                self.stats.bytes_allocated.fetch_add(len, Ordering::Relaxed);
            }
            Ok(self.inner.device.new_buffer(len as u64, MTLResourceOptions::StorageModeShared))
        }
        ...
        pub fn new_buffer_with_bytes_checked(&self, bytes: &[u8]) -> Result<Buffer> {
            ...
            if bytes.len() as u64 > ceiling {
                return Err(Error::Metal(format!("MTLBuffer allocation of {} B exceeds device working-set ceiling {ceiling} B", bytes.len())));
            }
            if self.trace_dispatch {
                self.stats.buffers_created.fetch_add(1, Ordering::Relaxed);
                self.stats.bytes_allocated.fetch_add(bytes.len(), Ordering::Relaxed);
            }
            Ok(self.inner.device.new_buffer_with_data(bytes.as_ptr() as *const _, bytes.len() as u64, MTLResourceOptions::StorageModeShared))
        }
        ...
        pub unsafe fn new_buffer_no_copy(&self, bytes: &[u8]) -> Buffer {
            self.inner.device.new_buffer_with_bytes_no_copy(bytes.as_ptr() as *const _, bytes.len() as u64, MTLResourceOptions::StorageModeShared, None)
        }
        ...
        pub fn dispatch_threads(&self, fn_name: &str, grid: (u32, u32, u32), tg: (u32, u32, u32), encode: impl FnOnce(&metal::ComputeCommandEncoderRef)) -> Result<()> {
            ...
            encode(enc);
            enc.dispatch_threads(MTLSize::new(grid.0 as u64, grid.1 as u64, grid.2 as u64), MTLSize::new(tg.0 as u64, tg.1 as u64, tg.2 as u64));
            enc.end_encoding();
            cmd.commit();
            if trace_enabled {
            ...
            if let Some(t0) = t0 {
                ...
                self.trace.record(static_kernel_name(fn_name), wall_us, super::current_layer());
            }
            Ok(())
        }

        pub fn dispatch_batch(&self, encode: impl FnOnce(&mut CommandBatch<'_>) -> Result<()>) -> Result<()> {
            let trace_enabled = self.trace_dispatch;
            let t0 = if trace_enabled { Some(Instant::now()) } else { None };
            ...
            let mut batch = CommandBatch { ctx: self, cmd, physical_trace: physical_trace.map(|(identity, _)| identity) };
            encode(&mut batch)?;
    ...
    impl CommandBatch<'_> {
        pub fn dispatch_threads(&mut self, fn_name: &str, grid: (u32, u32, u32), tg: (u32, u32, u32), encode: impl FnOnce(&metal::ComputeCommandEncoderRef)) -> Result<()> {
    ...
    impl<'ctx> TokenCommandBuffer<'ctx> {
        pub fn new(ctx: &'ctx MetalContext) -> Self {
            ...
            let prod_cb_tracer = if mode == TcbTraceMode::ProdCbGpu { ProdCbTracer::try_new(&ctx.inner.device) } else { None };
            Self { ctx, cmd: Some(cmd), mode, tcb_samples: Vec::new(), prod_cb_tracer, physical_trace: physical_trace.map(|(identity, _)| identity), concurrent_encoder: None, dispatch_count: 0 }
        }
        ...
        pub fn begin_concurrent_group(&mut self) -> Result<()> {
            if self.concurrent_encoder.is_some() {
                return Err(Error::Metal("begin_concurrent_group called while a group is already active".into()));
            }
            if !matches!(self.mode, TcbTraceMode::Off | TcbTraceMode::CpuEncode) {
                return Ok(());
            }
            let cmd = self.cmd.as_ref().ok_or_else(|| Error::Metal("TokenCommandBuffer already committed".into()))?;
            let enc = cmd.compute_command_encoder_with_dispatch_type(MTLDispatchType::Concurrent);
            if let Some(command) = self.physical_trace.as_ref() {
                enc.set_label(&physical_encoder_label(command, "compute_encoder", "concurrent_group"));
            } else {
        ...
        pub fn dispatch_threads(&mut self, fn_name: &str, grid: (u32, u32, u32), tg: (u32, u32, u32), encode: impl FnOnce(&metal::ComputeCommandEncoderRef)) -> Result<()> {
            ...
            if self.concurrent_encoder.is_some() {
                let t0 = if self.mode == TcbTraceMode::CpuEncode { Some(Instant::now()) } else { None };
                let pipe = self.ctx.pipeline(fn_name)?;
                let enc = self.concurrent_encoder.as_ref().expect("checked is_some above");
                enc.set_compute_pipeline_state(&pipe);
                encode(enc);
                enc.dispatch_threads(MTLSize::new(grid.0 as u64, grid.1 as u64, grid.2 as u64), MTLSize::new(tg.0 as u64, tg.1 as u64, tg.2 as u64));
                if let Some(t0) = t0 {
        ...
        fn dispatch_threads_prod_cb(&mut self, fn_name: &str, grid: (u32, u32, u32), tg: (u32, u32, u32), encode: impl FnOnce(&metal::ComputeCommandEncoderRef)) -> Result<()> {
            let t0_cpu = Instant::now();
            let cmd = self.cmd.as_ref().ok_or_else(|| Error::Metal("TokenCommandBuffer already committed".into()))?;
            let tracer = self.prod_cb_tracer.as_ref().expect("ProdCbGpu path requires a tracer; constructor ensures this");
            let pipe = self.ctx.pipeline(fn_name)?;
            ...
            if let Some(p) = pair_index {
                ...
                self.tcb_samples.push(super::DispatchSample { kernel_name: kn, wall_us: cpu_us, layer_hint: super::current_layer(), gpu_us: None, gpu_start_ns: None, gpu_end_ns: None });
            }
            Ok(())
        }
        ...
        fn dispatch_threads_split_cb(&mut self, fn_name: &str, grid: (u32, u32, u32), tg: (u32, u32, u32), encode: impl FnOnce(&metal::ComputeCommandEncoderRef)) -> Result<()> {
            let t0_cpu = Instant::now();
        ...
        pub fn copy_buffer_bytes(&mut self, src: &metal::Buffer, src_offset: u64, dst: &metal::Buffer, dst_offset: u64, size: u64) -> Result<()> {
            if size == 0 {
                return Ok(());
            }
            if self.mode == TcbTraceMode::SplitCbGpu {
...
mod imp {
    ...
    impl<'ctx> TokenCommandBuffer<'ctx> {
        ...
        pub fn dispatch_threads(&mut self, _fn_name: &str, _grid: (u32, u32, u32), _tg: (u32, u32, u32), _encode: impl FnOnce(())) -> Result<()> {
            Err(Error::Metal("metal unavailable on this platform".into()))
        }
