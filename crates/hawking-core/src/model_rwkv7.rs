...
impl RwkvConfig {
    fn from_gguf(g: &GgufFile) -> Result<Self> {
        ...
        let req = |k: &str| -> Result<usize> { get_u32(k).map(|v| v as usize).ok_or_else(|| Error::Model(format!("missing {k}"))) };
        ...
        let head_count = if head_count_meta == 0 { n_embd / head_size } else { head_count_meta };
        if head_count * head_size != n_embd {
            return Err(Error::Model(format!("rwkv7: head_count({head_count}) * head_size({head_size}) != n_embd({n_embd})")));
        }
        ...
        let ln_eps = g.metadata.get("rwkv7.attention.layer_norm_epsilon").and_then(|v| v.as_f32()).unwrap_or(1e-5);
        ...
        Ok(Self { n_embd, n_layer, n_ff, head_size, head_count, vocab_size, max_seq_len, ln_eps, decay_lora, iclr_lora, value_res_lora, gate_lora, token_shift_count })
    }
}
...
impl RwkvState {
    fn new(cfg: &RwkvConfig) -> Self {
        ...
        Self {
            wkv: (0..cfg.n_layer).map(|_| vec![0.0f32; s_per_layer]).collect(),
    ...
    pub fn from_int8_bytes(bytes: &[u8]) -> std::result::Result<Self, StateDecodeError> {
        ...
        for _ in 0..n_layer {
            ...
            let scale = f32::from_le_bytes([bytes[off], bytes[off + 1], bytes[off + 2], bytes[off + 3]]);
            off += 4;
        ...
        let read_planes = |off: &mut usize, per: usize| -> std::result::Result<Vec<Vec<f32>>, StateDecodeError> {
            ...
            for _ in 0..n_layer {
                ...
                let plane: Vec<f32> = bytes[*off..*off + nbytes].chunks_exact(4).map(|c| f32::from_le_bytes([c[0], c[1], c[2], c[3]])).collect();
                *off += nbytes;
                planes.push(plane);
            }
            Ok(planes)
...
pub fn quantize_plane_int8(plane: &[f32]) -> (Vec<i8>, f32) {
    ...
    let q = plane.iter().map(|&x| (x / scale).round().clamp(-127.0, 127.0) as i8).collect();
    (q, scale)
}
...
pub fn int8_plane_roundtrip_error(plane: &[f32]) -> f32 {
    ...
    plane.iter().zip(&deq).fold(0.0f32, |m, (a, b)| m.max((a - b).abs()))
}
...
mod state_serde_tests {
    ...
    fn sample() -> RwkvState {
        RwkvState { wkv: vec![vec![0.5, -1.0, 2.0, 3.5], vec![4.0, 5.0, 6.0, 7.0]], att_shift: vec![vec![0.1, 0.2], vec![0.3, 0.4]], ffn_shift: vec![vec![-0.1, -0.2], vec![-0.3, -0.4]], fresh: false }
    }
    ...
    fn state_share_group_is_copy_only_and_independent() {
        ...
        assert_ne!(g.member("a").unwrap().to_bytes(), g.member("b").unwrap().to_bytes());
    ...
    fn int8_state_rejects_wrong_magic() {
        ...
        assert!(matches!(RwkvState::from_int8_bytes(&s.to_bytes()), Err(StateDecodeError::BadMagic)));
    }
...
impl RwkvMultiState {
    ...
    pub fn new(cfg: &RwkvConfig, b: usize) -> Self {
        Self { slots: (0..b).map(|_| RwkvState::new(cfg)).collect() }
    }
...
impl RwkvSeven {
    ...
    pub fn forward_token(&mut self, token: u32) -> Result<Vec<f32>> {
        ...
        let RwkvSeven { config, layers, embed, tok_norm_w, tok_norm_b, output_norm_w, output_norm_b, output, state, .. } = self;
        Self::forward_token_core(config, layers, embed, tok_norm_w, tok_norm_b, output_norm_w, output_norm_b, output, state, token)
    }
    ...
    fn forward_token_core(
        ...
        for li in 0..n_layer {
            ...
            {
                let layer = &layers[li];
                layernorm(&ffn_inp, &layer.attn_norm2_w, &layer.attn_norm2_b, eps, &mut ffn_in);
            }
    ...
    pub fn forward_tokens_multiseq_cpu(&mut self, tokens: &[u32], multi: &mut RwkvMultiState) -> Result<Vec<Vec<f32>>> {
        if tokens.len() != multi.batch() {
            return Err(Error::Model(format!("rwkv7 multiseq: tokens={} != states={}", tokens.len(), multi.batch())));
        }
        let RwkvSeven { config, layers, embed, tok_norm_w, tok_norm_b, output_norm_w, output_norm_b, output, .. } = self;
        let mut out = Vec::with_capacity(tokens.len());
        for (b, &tok) in tokens.iter().enumerate() {
            out.push(Self::forward_token_core(config, layers, embed, tok_norm_w, tok_norm_b, output_norm_w, output_norm_b, output, &mut multi.slots[b], tok)?);
        }
        Ok(out)
    }
    ...
    pub fn forward_tokens_multiseq_greedy_cpu(&mut self, tokens: &[u32], multi: &mut RwkvMultiState) -> Result<Vec<u32>> {
    ...
    fn time_mix_with_state(config: &RwkvConfig, layers: &[RwkvLayer], state: &mut RwkvState, li: usize, att_in: &[f32], v_first: &mut Vec<f32>) -> Result<Vec<f32>> {
        let cfg = config;
        let n = cfg.n_embd;
    ...
    fn channel_mix_with_state(config: &RwkvConfig, layers: &[RwkvLayer], state: &RwkvState, li: usize, ffn_in: &[f32]) -> Result<Vec<f32>> {
        let cfg = config;
        let n = cfg.n_embd;
        let layer = &layers[li];
...
impl Engine for RwkvSeven {
    fn load(weights: &Path, _config: EngineConfig) -> Result<Self> {
        ...
        for li in 0..cfg.n_layer {
            ...
            layers.push(RwkvLayer {
                ...
                time_mix_receptance: Self::dequant_layer_matrix(&gguf, &p("time_mix_receptance.weight"))?,
                ...
                channel_mix_value: Self::dequant_layer_matrix(&gguf, &p("channel_mix_value.weight"))?,
            });
        }
        ...
        let (metal_ctx, gpu) = {
            ...
            if force_cpu {
                ...
                match MetalContext::new_with_trace(_config.trace_dispatch) {
                    Ok(ctx) => {
                        let built = gpu::RwkvGpu::build(&ctx, &cfg, &gguf, &embed, &tok_norm_w, &tok_norm_b, &output_norm_w, &output_norm_b, &layers);
                        match built {
    ...
    fn generate(&mut self, req: GenerateRequest, sink: &mut dyn FnMut(StreamEvent)) -> Result<GenStats> {
        ...
        let mut stats = GenStats { prompt_tokens: prompt_len, ..Default::default() };
        ...
        let json_vocab_index = if req.json_mode {
            self.tokenizer.as_ref().map(|tok| {
                ...
                crate::json_constrain::JsonVocabIndex::build(vs, |id| tok.decode_one(id).unwrap_or_default())
            })
        } else {
            None
        };
        ...
        for _ in 0..req.max_new_tokens {
            ...
            let text = self.tokenizer.as_ref().and_then(|t| t.decode_one(next_id).ok()).unwrap_or_default();
    ...
    fn load_checkpoint(&mut self, bytes: &[u8]) -> Result<()> {
        self.state = RwkvState::from_bytes(bytes).map_err(|e| Error::Model(format!("rwkv state checkpoint decode: {e}")))?;
        Ok(())
    }
    ...
    fn encode_prompt_for_batch(&self, prompt: &str) -> Result<Vec<u32>> {
        let tokenizer = self.tokenizer.as_ref().ok_or_else(|| Error::Model("rwkv7: World tokenizer not available from GGUF; serve path needs a tokenizer".into()))?;
    ...
    fn forward_multiseq_batched(&mut self, tokens: &[u32], positions: &[usize], regions: &[usize]) -> Result<Vec<Vec<f32>>> {
        #[cfg(target_os = "macos")]
        {
            ...
            if self.ensure_gpu_batch(SERVE_BATCH)? {
                ...
                return regions.iter().map(|&r| all.get(r).cloned().ok_or_else(|| Error::Model(format!("rwkv7 multiseq: region {r} >= SERVE_BATCH {SERVE_BATCH}")))).collect();
            }
        }
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            if self.ensure_gpu_batch(SERVE_BATCH)? {
                ...
                for &tok in prompt_ids {
                    last_logits =
                        Self::forward_token_core(&self.config, &self.layers, &self.embed, &self.tok_norm_w, &self.tok_norm_b, &self.output_norm_w, &self.output_norm_b, &self.output, &mut tmp, tok)?;
                }
                ...
                return Ok(last_logits.iter().copied().enumerate().max_by(|a, b| a.1.partial_cmp(&b.1).unwrap_or(std::cmp::Ordering::Less)).map(|(i, _)| i as u32).unwrap_or(0));
            }
        }
...
impl RwkvSeven {
    ...
    fn copy_cpu_state_to_gpu_slot(&self, cpu: &RwkvState, slot: usize) {
        ...
        unsafe {
            let wkv_base = (arena.wkv_state.contents() as *mut f32).add(slot * n_layer * s_per_layer);
            for (l, wkv_l) in cpu.wkv.iter().enumerate() {
                std::ptr::copy_nonoverlapping(wkv_l.as_ptr(), wkv_base.add(l * s_per_layer), s_per_layer);
            }
...
impl RwkvSeven {
    ...
    pub fn forward_token_gpu(&mut self, token: u32) -> Result<Vec<f32>> {
        ...
        let RwkvSeven { config, metal_ctx, gpu, embed, .. } = self;
    ...
    pub fn ensure_gpu_batch(&mut self, batch: usize) -> Result<bool> {
        ...
        let rebuilt = gpu::RwkvGpu::build_with_batch(ctx, &self.config, &self.gguf, &self.embed, &self.tok_norm_w, &self.tok_norm_b, &self.output_norm_w, &self.output_norm_b, &self.layers, need)?;
...
pub mod gpu {
    ...
    use crate::kernels::{gemm_q4_k_m_batched_v3w_predec_xoff_pinned_tcb, gemv_q4_k_v4_predec_xoff_pinned_tcb, gemv_q6_k_pinned_off_tcb, gemv_q6_k_pinned_tcb, predecode_q4_k_scale_table};
    use crate::metal::rwkv_decode_arena::{
        rwkv7_add_into_flat_tcb, rwkv7_add_into_layernorm_tcb, rwkv7_add_into_tcb, rwkv7_channel_mix_shift_multiseq_tcb, rwkv7_channel_mix_shift_tcb, rwkv7_copy_tcb, rwkv7_decay_act_multiseq_tcb,
        rwkv7_gemv_f32_off_tcb, rwkv7_gemv_f32_xoff_yoff_tcb, rwkv7_kk_kmix_multiseq_tcb, rwkv7_kk_kmix_tcb, rwkv7_layernorm_multiseq_tcb, rwkv7_layernorm_tcb, rwkv7_lora_grouped_gemv_tcb,
        rwkv7_lora_mid_act_tcb, rwkv7_relu_sq_inplace_tcb, rwkv7_shift_writeback_multiseq_tcb, rwkv7_sigmoid_bias_multiseq_tcb, rwkv7_sigmoid_inplace_tcb, rwkv7_tanh_inplace_tcb,
        rwkv7_token_shift_lerp_multiseq_tcb, rwkv7_token_shift_lerp_tcb, rwkv7_value_residual_mix_multiseq_tcb, rwkv7_value_residual_mix_tcb, rwkv7_wa_prep_tcb, rwkv7_wkv_decode_multiseq_tcb,
    ...
    pub enum ProjWeight {
        ...
        Q4k { q4: PinnedBuffer, scales: PinnedBuffer, rows: usize, cols: usize },
        /// Q6_K bytes (decoded inline by `gemm_q6_k_fused_v2`).
        Q6k { q6: PinnedBuffer, rows: usize, cols: usize },
        /// f32-dequantized fallback (F32/F16/BF16 tensors, or any non-Q4_K/Q6_K).
        F32 { w: PinnedBuffer, rows: usize, cols: usize },
        ...
        #[cfg(feature = "tq")]
        Tq { gpu: TqGpuReady, res: Option<TqGpuReady>, rows: usize, cols: usize },
    }
    ...
    impl ProjWeight {
        ...
        fn build(ctx: &MetalContext, g: &GgufFile, name: &str, rows: usize, cols: usize) -> Self {
            let info = g.tensor(name).unwrap_or_else(|| panic!("rwkv7 gpu: missing tensor {name}"));
            ...
            match info.dtype {
                GgmlType::Q4_K if aligned => {
                    ...
                    ProjWeight::Q4k { q4: ctx.new_buffer_with_bytes(bytes), scales: ctx.new_buffer_with_bytes(bytemuck::cast_slice::<f32, u8>(&scales)), rows, cols }
                }
                GgmlType::Q6_K if aligned => ProjWeight::Q6k { q6: ctx.new_buffer_with_bytes(bytes), rows, cols },
                _ => {
                    // F32/F16/BF16 (or unaligned): dequantize to f32 and serve via
                    // the bit-identical f32 GEMV (preserves the exact-parity gate
                    // on all-F32 GGUFs).
                    let w = crate::model::weights::dequant_f32(g, name).unwrap_or_else(|e| panic!("rwkv7 gpu: dequant {name}: {e}"));
                    ProjWeight::F32 { w: ctx.new_buffer_with_bytes(bytemuck::cast_slice::<f32, u8>(&w)), rows, cols }
                }
            }
        ...
        fn gemv(&self, tcb: &mut TokenCommandBuffer<'_>, x: &PinnedBuffer, x_off: usize, out: &PinnedBuffer) -> Result<()> {
            match self {
                ProjWeight::Q4k { q4, scales, rows, cols } => gemv_q4_k_v4_predec_xoff_pinned_tcb(tcb, q4, 0, q4.length() as usize, scales, 0, *rows, *cols, x, x_off, out),
                ...
                ProjWeight::F32 { w, rows, cols } => rwkv7_gemv_f32_off_tcb(tcb, w, *rows, *cols, x, x_off, out),
                #[cfg(feature = "tq")]
        ...
        fn gemv_batched(&self, tcb: &mut TokenCommandBuffer<'_>, x: &PinnedBuffer, x_off: usize, out: &PinnedBuffer, batch: usize) -> Result<()> {
            let f = std::mem::size_of::<f32>();
            match self {
                ProjWeight::Q4k { q4, scales, rows, cols } => {
                    ...
                    gemm_q4_k_m_batched_v3w_predec_xoff_pinned_tcb(tcb, q4, 0, q4.length() as usize, scales, 0, *rows, *cols, batch, x, x_off, out)
                }
                ProjWeight::Q6k { q6, rows, cols } => {
                    for bi in 0..batch {
                        gemv_q6_k_pinned_off_tcb(tcb, q6, 0, q6.length() as usize, *rows, *cols, x, x_off + bi * *cols * f, out, bi * *rows * f)?;
                    }
                    Ok(())
                }
                ProjWeight::F32 { w, rows, cols } => {
                    for bi in 0..batch {
                        rwkv7_gemv_f32_xoff_yoff_tcb(tcb, w, 0, *rows, *cols, x, x_off + bi * *cols * f, out, bi * *rows * f)?;
                    }
                    Ok(())
                }
                #[cfg(feature = "tq")]
                ProjWeight::Tq { gpu, res, cols, rows } => {
                    for bi in 0..batch {
                        ...
                        crate::kernels::strand_bitslice_gemv_tcb(tcb, gpu, x, x_off_b, out, out_off_b)?;
                        if let Some(res) = res {
                            crate::kernels::strand_bitslice_gemv_tcb_accum(tcb, res, x, x_off_b, out, out_off_b)?;
                        }
                    }
                    Ok(())
                }
        ...
        pub fn try_replace_with_tq(
            ...
            let Some(st) = tq_store.get(name) else {
                ...
                if res_store.is_some_and(|m| m.contains_key(name)) {
                    return Err(crate::Error::Model(format!("TQ residual override for {name} has no matching base TQ tensor")));
                }
                return Ok(self);
            };
            let (r, c) = match &self {
            ...
            if st.out_features != r || st.in_features != c {
                return Err(crate::Error::Model(format!("TQ override shape mismatch for {name}: artifact ({},{}) vs GGUF ({r},{c})", st.out_features, st.in_features)));
            }
            if c % 256 != 0 {
                return Err(crate::Error::Model(format!("TQ override alignment error for {name}: cols={c} is not a multiple of 256")));
            }
            ...
            let res = match res_store.and_then(|m| m.get(name)) {
                Some(rst) => {
                    if rst.out_features != r || rst.in_features != c {
                        return Err(crate::Error::Model(format!("TQ residual shape mismatch for {name}: artifact ({},{}) vs GGUF ({r},{c})", rst.out_features, rst.in_features)));
                    }
            ...
            Ok(ProjWeight::Tq { gpu, res, rows: r, cols: c })
        }
    }
    ...
    impl RwkvGpu {
        ...
        pub fn build(
            ...
            Self::build_with_batch(ctx, cfg, g, embed, tok_norm_w, tok_norm_b, output_norm_w, output_norm_b, layers, 1)
        }
        ...
        pub fn build_with_batch(
            ...
            #[cfg(feature = "tq")]
            let tq_store_opt: Option<std::collections::HashMap<String, crate::tq::StrandTensor>> = {
                if std::env::var("HAWKING_RWKV7_TQ").is_ok() {
                    let tq_path = std::env::var("HAWKING_RWKV7_TQ_PATH").map(std::path::PathBuf::from).unwrap_or_else(|_| std::path::PathBuf::new());
                    if tq_path.exists() {
                        Some(load_tq_artifact(&tq_path)?)
                    } else if std::env::var("HAWKING_RWKV7_TQ_STRICT").is_ok() {
                        return Err(crate::Error::Model(format!("HAWKING_RWKV7_TQ_STRICT: TQ artifact not found at {}", tq_path.display())));
                    } else {
                        None
                    }
                } else {
            ...
            #[cfg(feature = "tq")]
            let tq_res_store_opt: Option<std::collections::HashMap<String, crate::tq::StrandTensor>> = {
                if tq_store_opt.is_some() && crate::env_on("HAWKING_TQ_RESIDUAL") {
                    let res_path = std::env::var("HAWKING_RWKV7_TQ_RES_PATH").map(std::path::PathBuf::from).unwrap_or_else(|_| {
                        // Default: sibling `<base>.res.tq` next to the base artifact.
                        std::env::var("HAWKING_RWKV7_TQ_PATH").map(|p| std::path::PathBuf::from(format!("{p}.res.tq"))).unwrap_or_else(|_| std::path::PathBuf::new())
                    });
                    if res_path.exists() {
                        Some(load_tq_artifact(&res_path)?)
                    } else if std::env::var("HAWKING_RWKV7_TQ_STRICT").is_ok() {
                        return Err(crate::Error::Model(format!("HAWKING_RWKV7_TQ_STRICT: residual TQ artifact not found at {}", res_path.display())));
                    } else {
                        None
                    }
                } else {
            ...
            let stack_w1 = |l: &RwkvLayer| -> PinnedBuffer {
                let mut v = Vec::with_capacity((cfg.decay_lora + cfg.iclr_lora + cfg.value_res_lora) * n);
            ...
            let gpu_layers: Vec<RwkvGpuLayer> = layers
                ...
                .map(|(li, l)| -> Result<RwkvGpuLayer> {
                    ...
                    let proj = |suf: &str, rows: usize, cols: usize| ProjWeight::build(ctx, g, &p(suf), rows, cols);
                    ...
                    Ok(RwkvGpuLayer {
                        ...
                        receptance: tq_swap(proj("time_mix_receptance.weight", n, n), "time_mix_receptance.weight")?,
                        key: tq_swap(proj("time_mix_key.weight", n, n), "time_mix_key.weight")?,
                        value: tq_swap(proj("time_mix_value.weight", n, n), "time_mix_value.weight")?,
                        output: tq_swap(proj("time_mix_output.weight", n, n), "time_mix_output.weight")?,
                        w0: up(&l.time_mix_w0),
                        a0: up(&l.time_mix_a0),
                        v0: up(&l.time_mix_v0),
                        ...
                        channel_mix_key: tq_swap(proj("channel_mix_key.weight", n_ff, n), "channel_mix_key.weight")?,
                        channel_mix_value: tq_swap(proj("channel_mix_value.weight", n, n_ff), "channel_mix_value.weight")?,
            ...
            let lm_head = {
                ...
                #[cfg(feature = "tq")]
                let w = if let Some(ref store) = tq_store_opt { w.try_replace_with_tq("output.weight", store, tq_res_store_opt.as_ref(), ctx)? } else { w };
                w
            };
            ...
            let has_gate = layers.first().map(|l| l.time_mix_g1.is_some() && l.time_mix_g2.is_some()).unwrap_or(false);
            ...
            for &(rank, slot) in &groups {
                down.push(LoraGroup { row_start: lo_off as u32, w_off: w_off as u32, x_off: (slot * n) as u32, out_off: lo_off as u32, cols: n as u32 });
                w_off += rank * n;
                lo_off += rank;
            }
            ...
            for (gi, &(rank, _)) in groups.iter().enumerate() {
                up_tbl.push(LoraGroup { row_start: (gi * n) as u32, w_off: w2_off as u32, x_off: lo_in as u32, out_off: (gi * n) as u32, cols: rank as u32 });
                w2_off += n * rank;
                lo_in += rank;
            }
    ...
    #[cfg(feature = "tq")]
    pub fn load_tq_artifact(path: &std::path::Path) -> crate::Result<std::collections::HashMap<String, crate::tq::StrandTensor>> {
        let bytes = std::fs::read(path).map_err(crate::Error::Io)?;
        let tensors = crate::tq::read_strand(&bytes).map_err(|e| crate::Error::Model(format!("TQ artifact parse: {}", e)))?;
    ...
    pub fn forward_token_gpu(ctx: &MetalContext, g: &mut RwkvGpu, cfg: &RwkvConfig, embed: &[f32], token: u32) -> Result<Vec<f32>> {
        let n = cfg.n_embd;
        let eps = cfg.ln_eps;
        let a = &g.arena;
        ...
        rwkv7_layernorm_tcb(&mut tcb, &a.x, 0, &g.tok_norm_w, &g.tok_norm_b, &a.x_norm, 0, n, eps)?;
        ...
        for li in 0..cfg.n_layer {
            ...
            // ── time-mix ──
            // att_in = layernorm(x, attn_norm)
            rwkv7_layernorm_tcb(&mut tcb, &a.x, 0, &layer.attn_norm_w, &layer.attn_norm_b, &a.att_in, 0, n, eps)?;
            // token-shift lerp using stored att_shift[li] (x_prev) → xs slot-major.
            let n_slots = if layer.has_gate { 6 } else { 5 };
            rwkv7_token_shift_lerp_tcb(&mut tcb, &a.att_in, &a.att_shift, shift_off, &layer.lerp_fused, &a.xs, n, n_slots, g.fresh)?;
            ...
            rwkv7_lora_grouped_gemv_tcb(&mut tcb, &layer.lora_w1_stacked, &a.xs, &a.lora_lo, &g.lora_down_table, g.lora_groups, g.lora_down_rows)?;
            ...
            rwkv7_lora_grouped_gemv_tcb(&mut tcb, &layer.lora_w2_stacked, &a.lora_lo, &a.lora_up, &g.lora_up_table, g.lora_groups, g.lora_up_rows)?;
            ...
            if !have_v_first {
                ...
                rwkv7_value_residual_mix_tcb(&mut tcb, &a.v, &a.v_first, &a.lora_up, up_vmix_off, &layer.v0, n)?;
            }
            ...
            rwkv7_kk_kmix_tcb(&mut tcb, &a.k, &layer.k_k, &layer.k_a, &a.lora_up, up_a_off, &a.a_op, &a.b_op, cfg.head_size, cfg.head_count)?;
            ...
            rwkv7_add_into_layernorm_tcb(&mut tcb, &a.cur, &a.x, &a.ffn_inp, &layer.attn_norm2_w, &layer.attn_norm2_b, &a.ffn_in, n, eps)?;
            // xk = ffn_in + (x_prev - ffn_in) * lerp_k  (per-layer ffn_shift[li]).
            rwkv7_channel_mix_shift_tcb(&mut tcb, &a.ffn_in, &a.ffn_shift, shift_off, &layer.channel_mix_lerp_k, &a.xk_ffn, n, g.fresh)?;
            // k = relu(Wk @ xk)^2 — Q4_K projection
            layer.channel_mix_key.gemv(&mut tcb, &a.xk_ffn, 0, &a.ffn_k)?;
            rwkv7_relu_sq_inplace_tcb(&mut tcb, &a.ffn_k, cfg.n_ff)?;
            // cmix = Wv @ k — Q4_K projection
            layer.channel_mix_value.gemv(&mut tcb, &a.ffn_k, 0, &a.cmix)?;
        ...
        // final norm + LM head.
        rwkv7_layernorm_tcb(&mut tcb, &a.x, 0, &g.output_norm_w, &g.output_norm_b, &a.x_norm, 0, n, eps)?;
    ...
    fn read_rows_batch(buf: &PinnedBuffer, batch: usize, vocab: usize) -> Vec<Vec<f32>> {
        let ptr = buf.contents() as *const f32;
        (0..batch).map(|b| unsafe { std::slice::from_raw_parts(ptr.add(b * vocab), vocab) }.to_vec()).collect()
    }
    ...
    pub fn forward_token_gpu_multiseq(ctx: &MetalContext, g: &mut RwkvGpu, cfg: &RwkvConfig, embed: &[f32], tokens: &[u32]) -> Result<Vec<Vec<f32>>> {
        let n = cfg.n_embd;
        let eps = cfg.ln_eps;
        let b = g.arena.batch;
        if tokens.len() != b {
            return Err(Error::Model(format!("rwkv7 gpu multiseq: tokens={} != arena batch={}", tokens.len(), b)));
        }
        let f32b = std::mem::size_of::<f32>();
        ...
        rwkv7_layernorm_multiseq_tcb(&mut tcb, &a.x, 0, &g.tok_norm_w, &g.tok_norm_b, &a.x_norm, 0, n, b, eps)?;
        ...
        for li in 0..cfg.n_layer {
            ...
            rwkv7_layernorm_multiseq_tcb(&mut tcb, &a.x, 0, &layer.attn_norm_w, &layer.attn_norm_b, &a.att_in, 0, n, b, eps)?;
            // token-shift lerp from the stream-major att_shift window → xs (slot,B,n).
            let n_slots = if layer.has_gate { 6 } else { 5 };
            rwkv7_token_shift_lerp_multiseq_tcb(&mut tcb, &a.att_in, &a.att_shift, shift_base, shift_stream_stride, &layer.lerp_fused, &a.xs, n, n_slots, b, g.fresh)?;
            ...
            for bi in 0..b {
                rwkv7_gemv_f32_xoff_yoff_tcb(&mut tcb, &layer.lora_w1_stacked, lw1_w, cfg.decay_lora, n, &a.xs, xs_slot(1) + bi * n * f32b, &a.w_lo, bi * cfg.decay_lora * f32b)?;
            }
            rwkv7_tanh_inplace_tcb(&mut tcb, &a.w_lo, b * cfg.decay_lora)?;
            for bi in 0..b {
                rwkv7_gemv_f32_xoff_yoff_tcb(&mut tcb, &layer.lora_w2_stacked, lw2_w, n, cfg.decay_lora, &a.w_lo, bi * cfg.decay_lora * f32b, &a.w_raw, bi * n * f32b)?;
            }
            ...
            if !have_v_first {
                ...
                for bi in 0..b {
                    rwkv7_gemv_f32_xoff_yoff_tcb(&mut tcb, &layer.lora_w1_stacked, lw1_v, cfg.value_res_lora, n, &a.xs, xs_slot(3) + bi * n * f32b, &a.v_lo, bi * cfg.value_res_lora * f32b)?;
                }
                for bi in 0..b {
                    rwkv7_gemv_f32_xoff_yoff_tcb(&mut tcb, &layer.lora_w2_stacked, lw2_v, n, cfg.value_res_lora, &a.v_lo, bi * cfg.value_res_lora * f32b, &a.v_mix, bi * n * f32b)?;
                }
                rwkv7_value_residual_mix_multiseq_tcb(&mut tcb, &a.v, &a.v_first, &a.v_mix, &layer.v0, n, b)?;
            }

            // gate g = G2 @ sigmoid(G1 @ xg)   (slot 5; only if gated)
            if layer.has_gate {
                for bi in 0..b {
                    rwkv7_gemv_f32_xoff_yoff_tcb(&mut tcb, &layer.lora_w1_stacked, lw1_g, cfg.gate_lora, n, &a.xs, xs_slot(5) + bi * n * f32b, &a.g_lo, bi * cfg.gate_lora * f32b)?;
                }
                rwkv7_sigmoid_inplace_tcb(&mut tcb, &a.g_lo, b * cfg.gate_lora)?;
                for bi in 0..b {
                    rwkv7_gemv_f32_xoff_yoff_tcb(&mut tcb, &layer.lora_w2_stacked, lw2_g, n, cfg.gate_lora, &a.g_lo, bi * cfg.gate_lora * f32b, &a.gate, bi * n * f32b)?;
                }
            }

            // a = sigmoid(a0 + A2 @ (A1 @ xa))   (slot 4)
            for bi in 0..b {
                rwkv7_gemv_f32_xoff_yoff_tcb(&mut tcb, &layer.lora_w1_stacked, lw1_a, cfg.iclr_lora, n, &a.xs, xs_slot(4) + bi * n * f32b, &a.a_lo, bi * cfg.iclr_lora * f32b)?;
            }
            for bi in 0..b {
                rwkv7_gemv_f32_xoff_yoff_tcb(&mut tcb, &layer.lora_w2_stacked, lw2_a, n, cfg.iclr_lora, &a.a_lo, bi * cfg.iclr_lora * f32b, &a.a, bi * n * f32b)?;
            }
            rwkv7_sigmoid_bias_multiseq_tcb(&mut tcb, &a.a, &layer.a0, n, b)?;

            // kk = l2norm_per_head(k*k_k); k += (a-1)*(k*k_a); a_op=-kk; b_op=kk*a.
            rwkv7_kk_kmix_multiseq_tcb(&mut tcb, &a.k, &layer.k_k, &layer.k_a, &a.a, &a.a_op, &a.b_op, cfg.head_size, cfg.head_count, b)?;
            ...
            rwkv7_shift_writeback_multiseq_tcb(&mut tcb, &a.att_in, &a.att_shift, shift_base, shift_stream_stride, n, b)?;
            ...
            rwkv7_layernorm_multiseq_tcb(&mut tcb, &a.ffn_inp, 0, &layer.attn_norm2_w, &layer.attn_norm2_b, &a.ffn_in, 0, n, b, eps)?;
            // xk = ffn_in + (x_prev - ffn_in) * lerp_k   (stream-major ffn_shift)
            rwkv7_channel_mix_shift_multiseq_tcb(&mut tcb, &a.ffn_in, &a.ffn_shift, shift_base, shift_stream_stride, &layer.channel_mix_lerp_k, &a.xk_ffn, n, b, g.fresh)?;
            // k = relu(Wk @ xk)^2 — batched Q4_K projection (B, n_ff)
            layer.channel_mix_key.gemv_batched(&mut tcb, &a.xk_ffn, 0, &a.ffn_k, b)?;
            rwkv7_relu_sq_inplace_tcb(&mut tcb, &a.ffn_k, b * cfg.n_ff)?;
            // cmix = Wv @ k — batched Q4_K projection (B, n)
            layer.channel_mix_value.gemv_batched(&mut tcb, &a.ffn_k, 0, &a.cmix, b)?;
            // store ffn_in as next token-shift (scatter into stream-major plane).
            rwkv7_shift_writeback_multiseq_tcb(&mut tcb, &a.ffn_in, &a.ffn_shift, shift_base, shift_stream_stride, n, b)?;
        ...
        rwkv7_layernorm_multiseq_tcb(&mut tcb, &a.x, 0, &g.output_norm_w, &g.output_norm_b, &a.x_norm, 0, n, b, eps)?;
