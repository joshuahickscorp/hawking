...
//! makes spec slower than plain greedy decode (see `docs/RESEARCH.md`:
...
impl SpecGovernor {
    ...
    pub fn with_thresholds(window: usize, enable_above: f32, disable_below: f32, max_consecutive_rejections: usize, cooldown_steps: usize) -> Self {
    ...
    pub fn step(&mut self, accepted: bool) -> bool {
        ...
        match self.state {
            GovState::Enabled => {
                ...
                let rate_bail = self.accepted.len() >= self.window && self.accept_rate() <= self.disable_below;
                if streak_bail || rate_bail {
                    self.state = GovState::Cooldown { remaining: self.cooldown_steps };
                }
            }
    ...
    pub fn tick_disabled(&mut self) -> bool {
        if let GovState::Cooldown { remaining } = self.state {
            self.state = GovState::Cooldown { remaining: remaining.saturating_sub(1) };
        }
        self.is_enabled()
    }
...
mod tests {
    ...
    fn full_enable_disable_reenable_cycle() {
        ...
        feed(&mut gov, &[true, true, true, true]);
        assert!(gov.is_enabled(), "sustained recovery re-enables spec (rate {} state {:?})", gov.accept_rate(), gov.state());
    }
    ...
    fn dead_band_does_not_flap() {
        ...
        feed(&mut gov, &[true, false, true, false, true, false, true, false, true, false]);
        ...
        feed(&mut gov, &[true, false, true, false, true, false, true, false, true, false]);
        assert!(!gov.is_enabled(), "dead-band rate must not re-enable from Cooldown (rate {})", gov.accept_rate());
    }
