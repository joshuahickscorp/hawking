...
//! (EAGLE-3 is NO-GO, τ=0.877 net-negative — `docs/RESEARCH.md`). The
...
impl UserNgramDraft {
    ...
    pub fn note_token(&mut self, token: u32) {
        if let Some(cur) = self.cur {
            ...
            *self.unigram.entry(cur).or_default().entry(token).or_insert(0) += 1;
            if let Some(prev) = self.prev {
                // bigram: (prev, cur) → token
                *self.bigram.entry((prev, cur)).or_default().entry(token).or_insert(0) += 1;
            }
            self.transitions += 1;
        }
        self.prev = self.cur;
    ...
    pub fn propose(&self, ctx: &[u32], k: usize) -> Vec<u32> {
        ...
        let prev0 = if ctx.len() >= 2 { Some(ctx[ctx.len() - 2]) } else { None };
...
fn argmax_count(m: &HashMap<u32, u32>) -> Option<u32> {
    m.iter().max_by(|a, b| a.1.cmp(b.1).then(b.0.cmp(a.0))).map(|(&id, _)| id)
}
...
/// head — see docs/RESEARCH.md). Newtype so adapter-only state can be added
