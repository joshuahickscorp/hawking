...
//! SCAFFOLD, NEVER ENABLED without τ≥2.5 oracle GO (docs/RESEARCH.md).
//!
//! Kill-ledger entry (docs/RESEARCH.md EAGLE-3 / EAGLE-5 head):
    ...
    fn propose(&mut self, ctx: &Ctx<'_>, budget: Budget, _tel: &Telemetry) -> Proposal {
        ...
        match ctx.hidden.as_ref() {
            Some(tap) => {
                ...
                let drafts = self.head.propose_rollout_chained(tap.start_token, tap.residual, tap.intermediate, k);
...
mod tests {
    ...
    fn propose_with_hidden_tap_uses_rollout() {
        ...
        let tap = HiddenTap { residual, intermediate, start_token: 5 };
        let ctx = Ctx { tokens: &tokens, pos: tokens.len(), hidden: Some(tap) };
        let tel = Telemetry::default();
