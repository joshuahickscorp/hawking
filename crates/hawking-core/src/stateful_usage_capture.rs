...
//!
//! Entirely gated behind `HAWKING_QWEN_USAGE_CAPTURE=1`. With the flag unset
...
/// file. Overridable via `HAWKING_USAGE_CAPTURE_TOPK`.
#[cfg(target_os = "macos")]
...
impl UsageState {
    fn new() -> Self {
        UsageState { argmax_freq: HashMap::new(), total_emitted: 0, ctx_ledger: HashMap::new(), total_draft_accepted: 0, total_draft_rejected: 0, total_draft_cycles: 0 }
    }
}
...
/// `true` when `HAWKING_QWEN_USAGE_CAPTURE=1`. Cheap; called per emitted
/// token, so it short-circuits the whole instrument when unset.
#[cfg(target_os = "macos")]
pub fn enabled() -> bool {
    crate::env_on("HAWKING_QWEN_USAGE_CAPTURE")
}
...
pub fn flush() {
    ...
    let out_path = std::env::var("HAWKING_USAGE_CAPTURE_OUT").unwrap_or_else(|_| "reports/bench/usage_capture.json".to_string());
    let topk = std::env::var("HAWKING_USAGE_CAPTURE_TOPK").ok().and_then(|v| v.parse().ok()).unwrap_or(DEFAULT_TOPK);
    let top_ctx = std::env::var("HAWKING_USAGE_CAPTURE_TOP_CTX").ok().and_then(|v| v.parse().ok()).unwrap_or(DEFAULT_TOP_CTX);
    ...
    let pooled_tau = if total_cycles > 0 { (total_acc as f64 + total_cycles as f64) / total_cycles as f64 } else { 1.0 };
    ...
    if let Some(st) = st_ref {
        ...
        for ((prev, cur), t) in rows.into_iter().take(top_ctx) {
            ...
            let hit_rate = if total > 0 { t.accepted as f64 / total as f64 } else { 0.0 };
...
mod tests {
    ...
    /// flip `HAWKING_QWEN_USAGE_CAPTURE`, which is process-global.
    static ENV_LOCK: Mutex<()> = Mutex::new(());

    #[test]
    fn off_is_a_noop() {
        let _g = ENV_LOCK.lock().unwrap();
        std::env::remove_var("HAWKING_QWEN_USAGE_CAPTURE");
        reset_for_test();
        assert!(!enabled());
        ...
        assert!(guard.is_none(), "capture state must stay None when the flag is off");
    }

    #[test]
    fn on_records_argmax_and_draft() {
        let _g = ENV_LOCK.lock().unwrap();
        std::env::set_var("HAWKING_QWEN_USAGE_CAPTURE", "1");
        reset_for_test();
        assert!(enabled());
