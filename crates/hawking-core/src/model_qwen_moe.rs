...
impl Engine for QwenMoE {
    fn load(_weights: &Path, _config: EngineConfig) -> Result<Self> {
        Err(Error::Unimplemented("qwen-moe: lands in Phase 3 (DeepSeek-V2-Lite ships first)"))
    }

    fn generate(&mut self, _req: GenerateRequest, _sink: &mut dyn FnMut(StreamEvent)) -> Result<GenStats> {
    ...
    fn forward_tokens_for_test(&mut self, _tokens: &[u32], _positions: &[usize]) -> Result<Vec<Vec<f32>>> {
