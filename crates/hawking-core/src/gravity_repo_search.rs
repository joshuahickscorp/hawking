...
struct FileStamp {
    ...
    is_directory: bool,
    /// Content identity for files. Metadata remains a cheap invalidation hint,
    /// never the durable identity of a source artifact.
...
impl RepoSearchService {
    ...
    /// Content-derived identity for the last complete resident generation.
    pub fn index_revision(&self) -> String {
        self.index
            .read()
            .map(|index| index.index_revision().to_owned())
            .unwrap_or_else(|_| "gravity-index-v1:unavailable".to_owned())
    }
...
impl RepoSearchIndex {
    ...
    pub fn build(root: impl AsRef<Path>, limits: RepoSearchLimits) -> Result<Self> {
        ...
        index.revision = content_index_revision(
            root,
            &index.file_stamps,
            index.skipped_large,
            index.truncated_at_build,
        );
        Ok(index)
    }

    /// Content-derived identity for this immutable index generation.
...
fn stamp_matches_metadata(stamp: &FileStamp) -> bool {
    ...
    if metadata.file_type().is_dir() != stamp.is_directory
    ...
    let Some(expected_hash) = stamp.content_hash.as_deref() else {
        return false;
    };
    fs::read(&stamp.path)
        .ok()
        .is_some_and(|bytes| sha256_hex(&bytes) == expected_hash)
}

fn content_index_revision(
    root: &Path,
    file_stamps: &[FileStamp],
    skipped_large: usize,
    truncated: bool,
) -> String {
    let mut digest = Sha256::new();
    digest.update(b"hawking.gravity.index.v1\0");
    for stamp in file_stamps {
        let relative = stamp.path.strip_prefix(root).unwrap_or(&stamp.path);
    ...
    digest.update(skipped_large.to_string().as_bytes());
    digest.update(b"\0");
    if truncated {
        digest.update(b"truncated");
    } else {
        digest.update(b"complete");
    }
    format!("gravity-index-v1:{:x}", digest.finalize())
}
...
mod tests {
    ...
    fn filesystem_search_rebuilds_when_source_metadata_changes() {
        ...
        let initial_revision = service.index_revision();
        write(&root.path().join("a.py"), b"after_token\n");
        let result = service
            .search_files(root.path(), "after_token", "*.py", 10, None)
            .unwrap();
        assert_eq!(result.matches.len(), 1);
        assert_ne!(initial_revision, service.index_revision());
        assert_eq!(service.generation(), 2);
        assert_eq!(service.path_generation(), 1);
    }
