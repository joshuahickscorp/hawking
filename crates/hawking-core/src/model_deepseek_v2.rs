...
impl DeepSeekConfig {
    fn from_gguf(g: &GgufFile) -> Result<Self> {
        ...
        let n_layers = get_u32("deepseek2.block_count").or_else(|| get_u32("llama.block_count")).ok_or_else(|| Error::Model("missing block_count".into()))? as usize;
        let hidden = get_u32("deepseek2.embedding_length").ok_or_else(|| Error::Model("missing embedding_length".into()))? as usize;
        let n_heads = get_u32("deepseek2.attention.head_count").ok_or_else(|| Error::Model("missing head_count".into()))? as usize;
        let n_kv_heads = get_u32("deepseek2.attention.head_count_kv").unwrap_or(n_heads as u32) as usize;
        let vocab_size = get_u32("deepseek2.vocab_size").or_else(|| get_u32("llama.vocab_size")).ok_or_else(|| Error::Model("missing vocab_size".into()))? as usize;
        ...
        Ok(Self {
            ...
            qk_nope_head_dim: get_u32("deepseek2.attention.qk_nope_head_dim").unwrap_or(128) as usize,
            qk_rope_head_dim: get_u32("deepseek2.attention.qk_rope_head_dim").unwrap_or(64) as usize,
            v_head_dim: get_u32("deepseek2.attention.v_head_dim").unwrap_or(128) as usize,
            ffn_intermediate: get_u32("deepseek2.feed_forward_length").or_else(|| get_u32("llama.feed_forward_length")).unwrap_or(10944) as usize,
            moe_intermediate: get_u32("deepseek2.expert_feed_forward_length").unwrap_or(1408) as usize,
            ...
            first_k_dense_layers: get_u32("deepseek2.leading_dense_block_count").unwrap_or(1) as usize,
            vocab_size,
...
impl FfnMoeSetup {
    ...
    fn routed_down_kernel_with_schedule(&self, q4k_schedule: &str, routed_down_schedule: &str) -> &'static str {
    ...
    fn shared_down_kernel_with_schedule(&self, q4k_schedule: &str, shared_down_schedule: &str) -> &'static str {
...
impl DeepSeekV2 {
    ...
    fn dequant(g: &GgufFile, name: &str) -> Result<Vec<f32>> {
        let info = g.tensor(name).ok_or_else(|| Error::Model(format!("missing tensor `{name}`")))?;
    ...
    fn fused_expert_refs(g: &GgufFile, name: &str, n_experts: usize) -> Result<Vec<TensorRef>> {
        ...
        if total_elems % n_experts != 0 {
            return Err(Error::Model(format!("tensor {name}: {total_elems} elems not divisible by {n_experts} experts")));
        }
...
impl Engine for DeepSeekV2 {
    fn load(weights: &Path, config: EngineConfig) -> Result<Self> {
            ...
            if pruned_vocab.is_some() {
                return Err(Error::Model(
                    "vocab_prune_path set but model has tied embeddings (no output.weight); \
                     pruning the tied embed table would break input embed lookup"
                        .into(),
                ));
            }
            None
        ...
        for li in 0..cfg.n_layers {
            ...
            let mode = if li < cfg.first_k_dense_layers {
                LayerMode::Dense { gate_w: Self::dequant(&gguf, &lp("ffn_gate.weight"))?, up_w: Self::dequant(&gguf, &lp("ffn_up.weight"))?, down_w: Self::dequant(&gguf, &lp("ffn_down.weight"))? }
            } else {
                ...
                let gate_exps = Self::fused_expert_refs(&gguf, &lp("ffn_gate_exps.weight"), cfg.n_routed_experts)?;
                let up_exps = Self::fused_expert_refs(&gguf, &lp("ffn_up_exps.weight"), cfg.n_routed_experts)?;
                let down_exps = Self::fused_expert_refs(&gguf, &lp("ffn_down_exps.weight"), cfg.n_routed_experts)?;

                let routed: Vec<Expert> = gate_exps.into_iter().zip(up_exps.into_iter()).zip(down_exps.into_iter()).map(|((g, u), d)| Expert { gate_w: g, up_w: u, down_w: d }).collect();
                ...
                let (shared_fused, shared) = if cfg.n_shared_experts > 0 {
                    ...
                    (Some(fused.clone()), vec![Expert { gate_w: fused.gate_w.clone(), up_w: fused.up_w.clone(), down_w: fused.down_w.clone() }])
                } else {
                    (None, Vec::new())
                };
                LayerMode::MoE { gate_logits_w, routed_fused, routed, shared_fused, shared }
            };

            layers.push(Layer { attn_norm, ffn_norm, q_proj, q_a_proj, q_a_norm, q_b_proj, kv_a_proj_with_mqa, kv_a_norm, kv_b_proj, o_proj, mode, pinned: LayerPinned::default() });
        }

        let max_seq = config.max_seq_len.min(cfg.max_seq_len);
        let kv = KvCache::new(cfg.n_layers, max_seq, cfg.n_kv_heads, cfg.qk_nope_head_dim + cfg.qk_rope_head_dim);
        ...
        let cpu_only = config.force_cpu || crate::env_on("HAWKING_FORCE_CPU");
        let mla_metal = {
            #[cfg(target_os = "macos")]
            {
                !cpu_only && config.kernel_profile.as_ref().map(|p| p.selected.mla_schedule.as_str() == "metal-mla").unwrap_or(true)
            }
        ...
        let (mla_c_kv, mla_k_pe) = if mla_metal {
            let c_kv = (0..cfg.n_layers).map(|_| vec![0.0f32; max_seq * cfg.kv_lora_rank]).collect();
            let k_pe = (0..cfg.n_layers).map(|_| vec![0.0f32; max_seq * cfg.qk_rope_head_dim]).collect();
            (c_kv, k_pe)
        } else {
            (Vec::new(), Vec::new())
        };
        ...
        let speculate_mode = if config.speculate && config.speculate_mode == SpeculateMode::Off { SpeculateMode::ExactShared } else { config.speculate_mode };
        ...
        let eagle5_head: Option<crate::speculate::eagle5::Eagle5Head> = if speculate_mode == SpeculateMode::Eagle5 {
            let h = cfg.hidden;
            let v = cfg.vocab_size;
            let head = match config.eagle5_head_path.as_deref() {
                Some(p) => match crate::speculate::eagle5::Eagle5Head::load_from_safetensors(p, h, v) {
                    Ok(loaded) => {
                        eprintln!("[eagle5] loaded trained head from {} (hidden={h}, vocab={v})", p.display());
                        loaded
                    }
                    Err(e) => {
                        eprintln!(
        ...
        let weights_mmap_buf = {
            #[cfg(target_os = "macos")]
            {
                metal_ctx.as_ref().map(|ctx| unsafe { ctx.new_buffer_no_copy(&gguf.mmap) })
            }
            #[cfg(not(target_os = "macos"))]
            {
                let _ = &metal_ctx;
        ...
        if config.max_routed_expert_ram_mb.is_none() {
                ...
                if let LayerMode::MoE { routed_fused, shared_fused, .. } = &layer.mode {
                    for tref in [&routed_fused.gate_w, &routed_fused.up_w, &routed_fused.down_w] {
                        ...
                        if off.saturating_add(len) <= total_len {
                            unsafe {
                                let _ = posix_madvise(base.add(off) as *mut _, len, POSIX_MADV_WILLNEED);
                            }
            ...
            if config.trace_dispatch && hinted_mib > 0 {
                eprintln!("[hawking] madvise WILLNEED hinted {hinted_mib} MiB of MoE expert weights");
            }
        }
        ...
        if let Some(ctx) = metal_ctx.as_ref() {
            for layer in layers.iter_mut() {
                let upload = |w: &[f32]| ctx.new_buffer_with_bytes(bytemuck::cast_slice::<f32, u8>(w));
                ...
                let upload_f16 = |w: &[f32]| {
                    let f16_vec: Vec<half::f16> = w.iter().map(|&v| half::f16::from_f32(v)).collect();
        ...
        #[cfg(target_os = "macos")]
        let decode_arena = {
            let wants_arena = config.kernel_profile.as_ref().map(|p| p.selected.gpu_buffer_reuse == "decode-arena").unwrap_or(false);
            if wants_arena {
                metal_ctx.as_ref().map(|ctx| {
                    DecodeArena::new(
                        ctx,
        ...
        let (mla_c_kv_gpu, mla_k_pe_gpu) = {
            if metal_ctx.is_some() && mla_metal {
                ...
                let c_kv: Vec<PinnedBuffer> = (0..cfg.n_layers).map(|_| ctx.new_buffer(max_seq * cfg.kv_lora_rank * std::mem::size_of::<f32>())).collect();
                let k_pe: Vec<PinnedBuffer> = (0..cfg.n_layers).map(|_| ctx.new_buffer(max_seq * cfg.qk_rope_head_dim * std::mem::size_of::<f32>())).collect();
                (c_kv, k_pe)
            } else {
                (vec![], vec![])
            }
    ...
    fn generate(&mut self, req: GenerateRequest, sink: &mut dyn FnMut(StreamEvent)) -> Result<GenStats> {
        ...
        if self.speculate_mode == SpeculateMode::Eagle5 {
            ...
            if req.sampling.temperature > 0.0 {
                return Err(Error::Model("--speculate eagle5 currently requires temperature=0".into()));
            }
            if req.sampling.repetition_penalty != 1.0 {
                return Err(Error::Model("--speculate eagle5 currently requires repetition_penalty=1.0".into()));
            }
            if self.eagle5_head.is_none() {
                return Err(Error::Model(
                    "--speculate eagle5 requested but no Eagle5Head was constructed; \
                     this is a load-time bug — check EngineConfig::eagle5_head_path"
                        .into(),
                ));
            }
        }
        if self.speculate_mode == SpeculateMode::ExactShared {
            if req.sampling.temperature > 0.0 {
                return Err(Error::Model("--speculate exact-shared currently requires temperature=0".into()));
            }
            if req.sampling.repetition_penalty != 1.0 {
                return Err(Error::Model("--speculate exact-shared currently requires repetition_penalty=1.0".into()));
            }
            if !matches!(self.verify_window, 4 | 8 | 16) {
                return Err(Error::Model(format!("--verify-window must be 4, 8, or 16 for exact-shared; got {}", self.verify_window)));
            }
        }
        ...
        let mut stats = GenStats {
            ...
            device_id: self.kernel_profile.as_ref().map(|p| p.device_name.clone()).or_else(|| self.metal_ctx.as_ref().map(|ctx| ctx.device_name())),
            ..Default::default()
        };
        ...
        if self.speculate_mode == crate::SpeculateMode::ExactShared {
            ...
            let spec_log = std::env::var("HAWKING_SPEC_LOG").is_ok();
            ...
            'spec_loop: while produced < req.max_new_tokens {
                ...
                let verify_result = crate::speculate::shared::verify_draft_ids_until_mismatch(&draft_ids, |k| {
            ...
            // `crates/hawking-core/src/speculate_eagle5.rs` for the
            ...
            'eagle5_loop: while produced < req.max_new_tokens {
                ...
                // batch[k] at pos+k.
                let verify_t0 = Instant::now();
                let batch_tokens: Vec<u32> = std::iter::once(last_id).chain(draft_ids.iter().copied()).collect();
                let batch_positions: Vec<usize> = (0..=draft_actual_k).map(|ki| pos + ki).collect();
                ...
                let bonus_id = correction_id.unwrap_or_else(|| crate::kernels::argmax_f32(&logits_batch[draft_actual_k]));
                let verify_ms = verify_t0.elapsed().as_secs_f64() * 1000.0;
    ...
    fn forward_token_shared_only_for_test(&mut self, token: u32, pos: usize) -> Result<Vec<f32>> {
        ...
        let mut result: Vec<Vec<u64>> = (0..n_layers)
            .map(|li| {
                if li < first_dense {
                    ...
                    layer_stats.experts.iter().map(|e| e.active_count()).collect()
                }
            })
            .collect();
...
impl DeepSeekV2 {
    ...
    fn layer_has_tcb_attention(layer: &Layer) -> bool {
        let q_lora_ready = layer.pinned.q_a_proj.is_some() && layer.pinned.q_b_proj.is_some() && layer.pinned.q_a_norm.is_some();
    ...
    fn use_x_norm_f16(&self) -> bool {
        self.kernel_profile.as_ref().map(|p| p.selected.x_norm_dtype == "f16").unwrap_or(false)
    }
    ...
    fn shared_only_gpu_argmax_available(&self) -> bool {
        self.greedy_gpu_argmax_available()
            ...
            && self.layers.iter().all(|layer| match &layer.mode {
                LayerMode::Dense { .. } => layer.pinned.dense_gate_w.is_some() && layer.pinned.dense_up_w.is_some() && layer.pinned.dense_down_w.is_some(),
                LayerMode::MoE { shared_fused, .. } => {
                    shared_fused.as_ref().map(|s| s.gate_w.dtype == GgmlType::Q4_K && s.up_w.dtype == GgmlType::Q4_K && matches!(s.down_w.dtype, GgmlType::Q6_K | GgmlType::Q4_K)).unwrap_or(false)
                }
            })
    }
    ...
    fn encode_shared_only_ffn_tcb(
        ...
        if shared.gate_w.dtype != GgmlType::Q4_K || shared.up_w.dtype != GgmlType::Q4_K || !matches!(shared.down_w.dtype, GgmlType::Q6_K | GgmlType::Q4_K) {
            return Ok(false);
        }
    ...
    fn forward_token_shared_only_gpu_argmax(&mut self, token: u32, pos: usize) -> Result<u32> {
        ...
        if !self.mla_kv_gpu_synced && self.kv.seq_len > 0 {
            for li in 0..n_layers {
                MetalContext::write_buffer_bytes(&self.mla_c_kv_gpu[li], bytemuck::cast_slice(&self.mla_c_kv[li][..self.kv.seq_len * kv_lora_rank]));
                MetalContext::write_buffer_bytes(&self.mla_k_pe_gpu[li], bytemuck::cast_slice(&self.mla_k_pe[li][..self.kv.seq_len * qk_rope_head_dim]));
            }
        ...
        let q4k_schedule = self.kernel_profile.as_ref().map(|p| p.selected.gemm_q4_k_schedule.as_str()).unwrap_or("scalar");
        let shared_down_schedule = self.kernel_profile.as_ref().map(|p| p.selected.shared_down_schedule.as_str()).unwrap_or("basic");
        let use_simdmat = self.kernel_profile.as_ref().map(|p| p.selected.lm_head_schedule.contains("simdgroup-matrix")).unwrap_or(false);
        let mut tcb = crate::metal::TokenCommandBuffer::new(ctx);

        for li in 0..n_layers {
            crate::metal::set_current_layer(Some(li as u32));
            let kv_b_proj_buf = self.layers[li].pinned.kv_b_proj.as_ref().ok_or_else(|| Error::Model(format!("shared-only: l{li} kv_b_proj not pinned")))?;
            let o_proj_buf = self.layers[li].pinned.o_proj.as_ref().ok_or_else(|| Error::Model(format!("shared-only: l{li} o_proj not pinned")))?;
            ...
            crate::kernels::rmsnorm_metal_buf_tcb(&mut tcb, &arena.x_buf, ffn_norm_buf, eps, h, &arena.x_norm_buf)?;
            if !self.encode_shared_only_ffn_tcb(&mut tcb, li, arena, model_buf, q4k_schedule, shared_down_schedule)? {
                return Err(Error::Model(format!("shared-only GPU path could not encode layer {li}")));
            }
        }
        ...
        if x_norm_f16 {
            crate::kernels::rmsnorm_f32_to_f16_tcb(&mut tcb, &arena.x_buf, final_norm_buf, eps, h, &arena.x_norm_f16_buf)?;
            crate::kernels::gemv_f16_f16in_tcb(&mut tcb, lm_head_buf, self.config.vocab_size, h, &arena.x_norm_f16_buf, logits_buf)?;
        } else if use_simdmat {
            crate::kernels::rmsnorm_metal_buf_tcb(&mut tcb, &arena.x_buf, final_norm_buf, eps, h, &arena.x_norm_buf)?;
            crate::kernels::gemv_f16_simdmat_tcb(&mut tcb, lm_head_buf, self.config.vocab_size, h, &arena.x_norm_buf, logits_buf)?;
        } else {
            crate::kernels::rmsnorm_metal_buf_tcb(&mut tcb, &arena.x_buf, final_norm_buf, eps, h, &arena.x_norm_buf)?;
            crate::kernels::gemv_f16_metal_buf_tcb(&mut tcb, lm_head_buf, self.config.vocab_size, h, &arena.x_norm_buf, logits_buf)?;
        }
        crate::kernels::sample_argmax_f32_tcb(&mut tcb, logits_buf, tok_buf, self.config.vocab_size)?;
        tcb.commit_and_wait()?;
    ...
    fn gemv_f32_attn_dispatch(&self, w: &[f32], pinned: Option<&PinnedBuffer>, rows: usize, cols: usize, x: &[f32], out: &mut [f32]) -> Result<()> {
        #[cfg(target_os = "macos")]
        if let Some(ctx) = &self.metal_ctx {
    ...
    fn gemv_f32_attn_pair_dispatch(
        &self,
        w_a: &[f32],
        pinned_a: Option<&PinnedBuffer>,
        rows_a: usize,
        w_b: &[f32],
        pinned_b: Option<&PinnedBuffer>,
        rows_b: usize,
        cols: usize,
        x: &[f32],
        out_a: &mut [f32],
        ...
        if let Some(ctx) = &self.metal_ctx {
            ...
            if let (Some(buf_a), Some(buf_b)) = (buf_a, buf_b) {
                return crate::kernels::dispatch_gemv_f32_attn_pinned_pair_batched(ctx, buf_a, rows_a, buf_b, rows_b, cols, x, out_a, out_b);
            }
        }
    ...
    fn gemv_f32_moe_dispatch(&self, w: &[f32], rows: usize, cols: usize, x: &[f32], out: &mut [f32]) -> Result<()> {
        #[cfg(target_os = "macos")]
        if let Some(ctx) = &self.metal_ctx {
    ...
    fn moe_expert_pair_matmul_dispatch(&self, t_gate: &TensorRef, t_up: &TensorRef, rows: usize, cols: usize, x: &[f32], a: &mut [f32], scratch: &mut Vec<f32>) -> Result<()> {
        #[cfg(target_os = "macos")]
        if let Some(ctx) = &self.metal_ctx {
            if t_gate.dtype == GgmlType::Q4_K && t_up.dtype == GgmlType::Q4_K {
                let use_simd = self.kernel_profile.as_ref().map(|p| p.selected.gemm_q4_k_schedule == "simdgroup").unwrap_or(false);
                if use_simd {
                    ...
                    return crate::kernels::dispatch_gemv_q4_k_m_simd_pair_silu_batched(ctx, bytes_gate, bytes_up, rows, cols, x, a);
                }
            }
        }
    ...
    fn moe_expert_matmul_dispatch(&self, t: &TensorRef, rows: usize, cols: usize, x: &[f32], out: &mut [f32], scratch: &mut Vec<f32>) -> Result<()> {
        #[cfg(target_os = "macos")]
        if let Some(ctx) = &self.metal_ctx {
            if t.dtype == GgmlType::Q4_K {
                ...
                if schedule == "v2" {
                    if let Some(model_buf) = &self.weights_mmap_buf {
                        return crate::kernels::gemv_q4_k_m_v2_pinned(ctx, model_buf, t.offset, t.byte_size, rows, cols, x, out);
                    }
                    return crate::kernels::gemv_q4_k_m_v2(ctx, bytes, rows, cols, x, out);
                }
                if schedule == "simdmat" {
                    if let Some(model_buf) = &self.weights_mmap_buf {
                        return crate::kernels::gemv_q4_k_m_simdmat_pinned(ctx, model_buf, t.offset, t.byte_size, rows, cols, x, out);
                    }
                }
                if schedule == "v3_8r" {
                    if let Some(model_buf) = &self.weights_mmap_buf {
                        return crate::kernels::gemv_q4_k_m_v3_8r_pinned(ctx, model_buf, t.offset, t.byte_size, rows, cols, x, out);
                    }
                }
                if schedule == "v3_dual" {
                    if let Some(model_buf) = &self.weights_mmap_buf {
                        return crate::kernels::gemv_q4_k_m_v3_dual_pinned(ctx, model_buf, t.offset, t.byte_size, rows, cols, x, out);
                    }
                }
                if schedule == "v3_llama" || schedule == "llama_port" {
                    if let Some(model_buf) = &self.weights_mmap_buf {
                        return crate::kernels::gemv_q4_k_m_llama_port_pinned(ctx, model_buf, t.offset, t.byte_size, rows, cols, x, out);
                    }
                }
                if schedule == "simdgroup" {
                    return crate::kernels::dispatch_gemv_q4_k_m_simd_batched(ctx, bytes, rows, cols, x, out);
                }
    ...
    fn moe_block_batched_dispatch(&self, routed_fused: &MoEFusedTensors, routed: &[Expert], shared_fused: Option<&MoEFusedTensors>, routes: &[(usize, f32)], x: &[f32]) -> Result<Option<Vec<f32>>> {
        #[cfg(target_os = "macos")]
        {
            ...
            if routed_fused.gate_w.dtype != GgmlType::Q4_K || routed_fused.up_w.dtype != GgmlType::Q4_K || routed_fused.down_w.dtype != GgmlType::Q8_0 {
                return Ok(None);
            }
            if let Some(shared) = shared_fused {
                if shared.gate_w.dtype != GgmlType::Q4_K || shared.up_w.dtype != GgmlType::Q4_K || shared.down_w.dtype != GgmlType::Q6_K {
                    return Ok(None);
                }
            }
            ...
            for &(eid, weight) in routes {
                if eid >= routed.len() {
                    return Err(Error::Model(format!("route selected expert {eid}, but only {} experts are loaded", routed.len())));
                }
                if eid > u32::MAX as usize {
                    return Err(Error::Model(format!("route selected expert {eid}, but Metal route ids are u32")));
                }
            ...
            let (shared_gate_offset, shared_up_offset, shared_down_offset, shared_mid) = if let Some(shared) = shared_fused {
                (Some(shared.gate_w.offset), Some(shared.up_w.offset), Some(shared.down_w.offset), self.config.n_shared_experts * self.config.moe_intermediate)
            } else {
                (None, None, None, 0)
            };
    ...
    fn forward_tokens(&mut self, tokens: &[u32], positions: &[usize]) -> Result<Vec<Vec<f32>>> {
        if tokens.len() != positions.len() {
            return Err(Error::Model(format!("forward_tokens shape: tokens={} positions={}", tokens.len(), positions.len())));
        }
    ...
    fn forward_tokens_batched(&mut self, tokens: &[u32], positions: &[usize]) -> Result<Vec<Vec<f32>>> {
        if tokens.len() != positions.len() {
            return Err(Error::Model(format!("forward_tokens_batched: tokens={} positions={}", tokens.len(), positions.len())));
        }
        if tokens.is_empty() {
            return Ok(vec![]);
        }
        ...
        #[cfg(target_os = "macos")]
        {
            let tcb_base = self.metal_ctx.is_some() && self.decode_arena.is_some() && self.layers.iter().all(|l| l.pinned.attn_norm.is_some() && l.pinned.ffn_norm.is_some());
    ...
    #[cfg(target_os = "macos")]
    fn forward_tokens_batched_tcb(&mut self, tokens: &[u32], positions: &[usize]) -> Result<Vec<Vec<f32>>> {
        let k = tokens.len();
        let h = self.config.hidden;
        let n_layers = self.config.n_layers;
        let eps = self.config.rms_norm_eps;
        ...
        if !self.mla_kv_gpu_synced && self.kv.seq_len > 0 {
            ...
            for li in 0..n_layers {
                crate::metal::MetalContext::write_buffer_bytes(&self.mla_c_kv_gpu[li], bytemuck::cast_slice(&self.mla_c_kv[li][..sl * kv_lora_rank]));
                crate::metal::MetalContext::write_buffer_bytes(&self.mla_k_pe_gpu[li], bytemuck::cast_slice(&self.mla_k_pe[li][..sl * qk_rope_head_dim]));
            }
        }
        ...
        let routed_down_schedule = self.kernel_profile.as_ref().map(|p| p.selected.routed_down_schedule.as_str()).unwrap_or("basic");
...

        {
            ...
            for (ki, &token) in tokens.iter().enumerate() {
                ...
                for li in 0..n_layers {
                    ...
                    let kv_b_proj_buf = self.layers[li].pinned.kv_b_proj.as_ref().ok_or_else(|| crate::Error::Model(format!("batched5A: l{li} kv_b_proj not pinned")))?;
                    let o_proj_buf = self.layers[li].pinned.o_proj.as_ref().ok_or_else(|| crate::Error::Model(format!("batched5A: l{li} o_proj not pinned")))?;
                    let ffn_norm_buf = self.layers[li].pinned.ffn_norm.as_ref().unwrap();
                    let head_dim_q = self.config.qk_nope_head_dim + self.config.qk_rope_head_dim;
                    let scale = 1.0f32 / (head_dim_q as f32).sqrt();
                    ...
                    self.encode_attention_phase1_into_tcb(&mut global_tcb, li, pos, Some(token), seq_slot)?;
                    ...
                    // Default OFF; flip via HAWKING_FUSED_ADD_RMSNORM=1.
                    // Fuses x += attn_out and rmsnorm(x → x_norm) into one
                    // dispatch, eliminating one DRAM pass over x.
                    if std::env::var_os("HAWKING_FUSED_ADD_RMSNORM").is_some() {
                        crate::kernels::add_rmsnorm_fused_tcb(&mut global_tcb, &arena.x_buf, &arena.out, ffn_norm_buf, &arena.x_norm_buf, eps, h)?;
                    } else {
                        // Residual: x_buf += attn_out (arena.out).
                        crate::kernels::add_inplace_metal_tcb(&mut global_tcb, &arena.x_buf, &arena.out, h)?;
                        // FFN norm: x_buf → x_norm_buf.
                        crate::kernels::rmsnorm_metal_buf_tcb(&mut global_tcb, &arena.x_buf, ffn_norm_buf, eps, h, &arena.x_norm_buf)?;
                    }

                    // FFN: MoE or dense.
                    if let Some(ref setup) = moe_setup {
                        let gate_buf = self.layers[li].pinned.gate_logits_w.as_ref().unwrap();
                        crate::kernels::gemv_f32_moe_pinned_buf_tcb(&mut global_tcb, gate_buf, self.config.n_routed_experts, self.config.hidden, &arena.x_norm_buf, &arena.moe_logits_buf)?;
                        ...
                        if !dense_ok {
                            ...
                            return Err(Error::Model(format!("forward_tokens_batched_tcb: l{li} dense weights not pinned")));
                        }
                    }
                }

                // Final residual accumulation + final norm for token ki.
                crate::kernels::add_inplace_metal_tcb(&mut global_tcb, &arena.x_buf, &arena.ffn_out_buf, h)?;
                crate::kernels::rmsnorm_metal_buf_tcb(&mut global_tcb, &arena.x_buf, final_norm_buf, eps, h, &arena.x_norm_buf)?;
                ...
                global_tcb.copy_buffer_bytes(&arena.x_norm_buf, 0, &arena.batch_x_norm_buf[ki], 0, sz)?;
            }
    ...
    fn mla_kv_append(mla_c_kv: &mut Vec<Vec<f32>>, mla_k_pe: &mut Vec<Vec<f32>>, li: usize, seq_slot: usize, kv_lora_rank: usize, qk_rope_head_dim: usize, c_kv: &[f32], k_pe: &[f32]) -> Result<()> {
        if c_kv.len() != kv_lora_rank {
            return Err(Error::Model(format!("mla_kv_append c_kv len: got {} expected {}", c_kv.len(), kv_lora_rank)));
        }
        if k_pe.len() != qk_rope_head_dim {
            return Err(Error::Model(format!("mla_kv_append k_pe len: got {} expected {}", k_pe.len(), qk_rope_head_dim)));
        }
    ...
    fn forward_token_greedy(&mut self, token: u32, pos: usize) -> Result<Option<u32>> {
        ...
        #[cfg(target_os = "macos")]
        {
            if wedge_e_ok {
                ...
                let result = {
                    ...
                    let use_simdmat = self.kernel_profile.as_ref().map(|p| p.selected.lm_head_schedule.contains("simdgroup-matrix")).unwrap_or(false);
                    if use_simdmat {
                        crate::kernels::gemv_f16_simdmat_tcb(&mut tcb, lm_head_buf, vocab, cols, &arena.x_norm_buf, logits_buf)?;
                    } else {
                        crate::kernels::gemv_f16_metal_buf_tcb(&mut tcb, lm_head_buf, vocab, cols, &arena.x_norm_buf, logits_buf)?;
                    }
        ...
        let x_norm = x_norm.ok_or_else(|| Error::Model("forward_token_greedy: missing CPU final norm for fallback argmax".into()))?;
        self.gemv_f16_argmax_dispatch(self.config.vocab_size, self.config.hidden, &x_norm).map(|opt| opt.map(|t| self.id_to_original(t)))
    }

    fn forward_token_argmax(&mut self, token: u32, pos: usize, use_profiled_greedy: bool) -> Result<u32> {
    ...
    fn forward_token_final_norm_maybe_read(&mut self, token: u32, pos: usize, read_back: bool) -> Result<(Option<Vec<f32>>, Option<u32>)> {
        let h = self.config.hidden;
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            if wedge_c_active && !self.mla_c_kv_gpu.is_empty() {
                ...
                let use_gpu_shared_draft = self.speculate_mode == crate::SpeculateMode::ExactShared && self.shared_only_gpu_argmax_available();
                let use_single_tcb = self.speculate_mode != crate::SpeculateMode::ExactShared || use_gpu_shared_draft;
                ...
                let lm_head_foldable = use_single_tcb && !read_back && self.lm_head_buf.is_some() && self.logits_buf.is_some() && self.token_buf.is_some();
                ...
                let t0 = std::time::Instant::now();
                {
                    ...
                    let mut global_tcb = if use_single_tcb { Some(crate::metal::TokenCommandBuffer::new(ctx)) } else { None };
                    ...
                    for li in 0..n_layers {
                        ...
                        let kv_b_proj_buf = self.layers[li].pinned.kv_b_proj.as_ref().ok_or_else(|| crate::Error::Model(format!("merged: l{li} kv_b_proj not pinned")))?;
                        let o_proj_buf = self.layers[li].pinned.o_proj.as_ref().ok_or_else(|| crate::Error::Model(format!("merged: l{li} o_proj not pinned")))?;
                        let ffn_norm_buf = self.layers[li].pinned.ffn_norm.as_ref().unwrap();
                        let head_dim_q = self.config.qk_nope_head_dim + self.config.qk_rope_head_dim;
                        let scale = 1.0f32 / (head_dim_q as f32).sqrt();
                        ...
                        let encode_layer = |tcb: &mut crate::metal::TokenCommandBuffer<'_>| -> Result<bool> {
                            ...
                            crate::kernels::rmsnorm_metal_buf_tcb(tcb, &arena.x_buf, ffn_norm_buf, eps, h, &arena.x_norm_buf)?;
                            if let Some(ref setup) = moe_setup {
                                let gate_buf = self.layers[li].pinned.gate_logits_w.as_ref().unwrap();
                                crate::kernels::gemv_f32_moe_pinned_buf_tcb(tcb, gate_buf, self.config.n_routed_experts, self.config.hidden, &arena.x_norm_buf, &arena.moe_logits_buf)?;
                                crate::kernels::moe_topk_gate_tcb(
                                    tcb,
                                    &arena.moe_logits_buf,
                                    &arena.moe_route_ids_buf,
                                ...
                                if self.expert_cache.is_some() {
                                    ...
                                    let dst_off = (moe_li * self.config.top_k_routed * std::mem::size_of::<u32>()) as u64;
                                    let sz = (self.config.top_k_routed * std::mem::size_of::<u32>()) as u64;
                                    tcb.copy_buffer_bytes(&arena.moe_route_ids_buf, 0, &arena.route_history_buf, dst_off, sz)?;
                                }
                                crate::kernels::encode_moe_block_batched_indexed_tcb_with_scratch(
                                    tcb,
                                    model_buf,
                        ...
                        if use_single_tcb {
                            ...
                            unsafe {
                                ...
                                self.mla_c_kv[li][off_c..off_c + kv_lora_rank].copy_from_slice(std::slice::from_raw_parts(ptr_c, kv_lora_rank));
                                self.mla_k_pe[li][off_pe..off_pe + qk_rope_head_dim].copy_from_slice(std::slice::from_raw_parts(ptr_pe, qk_rope_head_dim));
                            }
                        }
                    ...
                    if let Some(ref mut tcb) = global_tcb {
                        ...
                        let x_norm_f16 = self.use_x_norm_f16();
                        if x_norm_f16 {
                            crate::kernels::rmsnorm_f32_to_f16_tcb(tcb, &arena.x_buf, final_norm_buf, eps, h, &arena.x_norm_f16_buf)?;
                        } else {
                            crate::kernels::rmsnorm_metal_buf_tcb(tcb, &arena.x_buf, final_norm_buf, eps, h, &arena.x_norm_buf)?;
                        }
                        if lm_head_foldable {
                            let lm_head_buf = self.lm_head_buf.as_ref().unwrap();
                            let logits_buf = self.logits_buf.as_ref().unwrap();
                            ...
                            if x_norm_f16 {
                                ...
                                crate::kernels::gemv_f16_f16in_tcb(tcb, lm_head_buf, vocab, h, &arena.x_norm_f16_buf, logits_buf)?;
                            } else {
                                let use_simdmat = self.kernel_profile.as_ref().map(|p| p.selected.lm_head_schedule.contains("simdgroup-matrix")).unwrap_or(false);
                                if use_simdmat {
                                    crate::kernels::gemv_f16_simdmat_tcb(tcb, lm_head_buf, vocab, h, &arena.x_norm_buf, logits_buf)?;
                                } else {
                                    crate::kernels::gemv_f16_metal_buf_tcb(tcb, lm_head_buf, vocab, h, &arena.x_norm_buf, logits_buf)?;
                                }
                            }
                            crate::kernels::sample_argmax_f32_tcb(tcb, logits_buf, tok_buf, vocab)?;
                            lm_head_folded = true;
                        }
                    }
                    ...
                    if let Some(cache) = self.expert_cache.as_ref() {
                        ...
                        if top_k > 0 && n_moe_li > 0 {
                            ...
                            let history = unsafe { std::slice::from_raw_parts(ptr, n_moe_li * top_k) };
                            let first_dense = self.config.first_k_dense_layers;
                            for moe_li in 0..n_moe_li {
                                for slot in 0..top_k {
                                    let expert_id = history[moe_li * top_k + slot];
                                    cache.note_access(first_dense + moe_li, expert_id, pos as u64);
                                }
                            }
                        }
                    }
                ...
                if decode_timing {
                    eprintln!("[timing/merged] total={:.1}ms tps_ceil={:.0}", total_us as f64 / 1000.0, 1_000_000.0 / total_us as f64);
                }
        ...
        // (HAWKING_FORCE_CPU=1, via Edit A). This is the per-layer driver
        ...
        if self.metal_ctx.is_none() {
            ...
            for li in 0..self.config.n_layers {
                ...
                let mut x_norm = vec![0.0f32; h];
                self.rmsnorm_dispatch(&x, &self.layers[li].attn_norm, self.config.rms_norm_eps, &mut x_norm)?;
                let attn_out = self.attention(li, pos, &x_norm)?;
                add_inplace(&mut x, &attn_out);

                self.rmsnorm_dispatch(&x.clone(), &self.layers[li].ffn_norm, self.config.rms_norm_eps, &mut x_norm)?;
                let ffn_out = self.ffn(li, &x_norm)?;
        ...
        let _ = pos;
        let _ = read_back;
        Err(Error::Model("forward_token: Wedge C preconditions not met; legacy fallback retired".into()))
    }
    ...
    #[cfg(target_os = "macos")]
    fn encode_attention_phase2_tcb(&self, tcb: &mut crate::metal::TokenCommandBuffer<'_>, li: usize, pos: usize) -> Result<()> {
        let q_lora_path = self.layers[li].pinned.q_a_proj.is_some() && self.layers[li].pinned.q_b_proj.is_some() && self.layers[li].pinned.q_a_norm.is_some();
        if !q_lora_path {
            return Ok(());
        }
        let arena = self.decode_arena.as_ref().unwrap();
        ...
        let q_b_proj_buf = self.layers[li].pinned.q_b_proj.as_ref().ok_or_else(|| Error::Model(format!("phase2_tcb: l{li} q_b_proj not pinned")))?;
        let q_out_rows = n_heads * head_dim_q;
        // q_b_proj pinned as f16; cols=1536 rows=3072 both % 8 == 0
        crate::kernels::gemv_f16_simdmat_tcb(tcb, q_b_proj_buf, q_out_rows, q_lora, &arena.q_lora_normed_buf, &arena.q)?;
        crate::kernels::rope_q_f32_inplace_tcb(tcb, &arena.q, n_heads, head_dim_q, qk_nope_head_dim, qk_rope_head_dim, pos as u32, rope_theta)
    }
    ...
    #[cfg(target_os = "macos")]
    fn encode_attention_phase1_into_tcb(&self, tcb: &mut crate::metal::TokenCommandBuffer<'_>, li: usize, pos: usize, pre_phase_token: Option<u32>, seq_slot: usize) -> Result<()> {
        ...
        let rmsnorm_attn_schedule = self.kernel_profile.as_ref().map(|p| p.selected.rmsnorm_attn_schedule.as_str()).unwrap_or("basic");
        let use_v2t_rmsnorm_attn = rmsnorm_attn_schedule == "v2t";

        let arena = self.decode_arena.as_ref().unwrap();
        let kv_a_proj_buf = self.layers[li].pinned.kv_a_proj_with_mqa.as_ref().ok_or_else(|| Error::Model(format!("p1_into_tcb: l{li} kv_a_proj not pinned")))?;
        let attn_norm_buf = self.layers[li].pinned.attn_norm.as_ref().ok_or_else(|| Error::Model(format!("p1_into_tcb: l{li} attn_norm not pinned")))?;
        let kv_a_norm_buf = self.layers[li].pinned.kv_a_norm.as_ref().ok_or_else(|| Error::Model(format!("p1_into_tcb: l{li} kv_a_norm not pinned")))?;
        ...
        let dispatch_rmsnorm_attn = |tcb: &mut crate::metal::TokenCommandBuffer<'_>,
         ...
         -> Result<()> {
            if use_v2t_rmsnorm_attn && rows % 8 == 0 && cols % 32 == 0 {
                crate::kernels::rmsnorm_gemv_f16w_attn_pinned_v2t_tcb(tcb, w, x, attn_norm_buf, eps, out, rows, cols)
            } else {
                crate::kernels::rmsnorm_gemv_f16w_attn_pinned_tcb(tcb, w, x, attn_norm_buf, eps, out, rows, cols)
            }
        };
        ...
        if q_lora_path {
            let q_a_proj_buf = self.layers[li].pinned.q_a_proj.as_ref().ok_or_else(|| Error::Model(format!("p1_into_tcb: l{li} q_a_proj not pinned")))?;
            let q_a_norm_buf = self.layers[li].pinned.q_a_norm.as_ref().ok_or_else(|| Error::Model(format!("p1_into_tcb: l{li} q_a_norm not pinned")))?;
            dispatch_rmsnorm_attn(tcb, q_a_proj_buf, &arena.x_buf, &arena.q_lora_buf, q_lora, h)?;
            crate::kernels::rmsnorm_metal_buf_tcb(tcb, &arena.q_lora_buf, q_a_norm_buf, eps, q_lora, &arena.q_lora_normed_buf)?;
        } else {
            let q_proj_buf = self.layers[li].pinned.q_proj.as_ref().ok_or_else(|| Error::Model(format!("p1_into_tcb: l{li} q_proj not pinned")))?;
            // v2.1.0-T2.13: q_proj pinned as f16; use f16w rmsnorm kernel.
            dispatch_rmsnorm_attn(tcb, q_proj_buf, &arena.x_buf, &arena.q, n_heads * head_dim_q, h)?;
        }
        dispatch_rmsnorm_attn(tcb, kv_a_proj_buf, &arena.x_buf, &arena.kv_a_out_buf, kv_a_dim, h)?;
        crate::kernels::rmsnorm_metal_buf_tcb(tcb, &arena.kv_a_out_buf, kv_a_norm_buf, eps, kv_lora_rank, &arena.c_kv_normed_buf)?;
        crate::kernels::rope_slice_f32_inplace_tcb(tcb, &arena.kv_a_out_buf, kv_lora_rank, qk_rope_head_dim, pos as u32, rope_theta)?;
        if !q_lora_path {
            crate::kernels::rope_q_f32_inplace_tcb(tcb, &arena.q, n_heads, head_dim_q, qk_nope_head_dim, qk_rope_head_dim, pos as u32, rope_theta)?;
        }
        crate::kernels::kv_append_f32_tcb(tcb, &arena.c_kv_normed_buf, &arena.kv_a_out_buf, &self.mla_c_kv_gpu[li], &self.mla_k_pe_gpu[li], seq_slot, kv_lora_rank, qk_rope_head_dim)
    }
    ...
    #[cfg(target_os = "macos")]
    fn encode_dense_ffn_tcb(&self, tcb: &mut crate::metal::TokenCommandBuffer<'_>, li: usize, arena: &DecodeArena) -> Result<bool> {
        ...
        crate::kernels::gemv_f32_attn_pinned_buf_tcb(tcb, gate_buf, mid, h, &arena.x_norm_buf, &arena.dense_gate_out_buf)?;
        crate::kernels::gemv_f32_attn_pinned_buf_tcb(tcb, up_buf, mid, h, &arena.x_norm_buf, &arena.dense_up_out_buf)?;
        crate::kernels::silu_mul_tcb(tcb, &arena.dense_gate_out_buf, &arena.dense_up_out_buf, &arena.dense_act_buf, mid)?;
        crate::kernels::gemv_f32_attn_pinned_buf_tcb(tcb, down_buf, h, mid, &arena.dense_act_buf, &arena.ffn_out_buf)?;
        Ok(true)
    }
            ...
            match &layer.mode {
                LayerMode::MoE { routed_fused, shared_fused, .. } => {
                    if routed_fused.gate_w.dtype != GgmlType::Q4_K
                        ...
                        || !matches!(routed_fused.down_w.dtype, GgmlType::Q8_0 | GgmlType::Q5_0 | GgmlType::Q4_K)
                    {
                        return Ok(None);
                    }
                    let (sg, su, sd, sdty, smid) = if let Some(sf) = shared_fused {
                        if sf.gate_w.dtype != GgmlType::Q4_K || sf.up_w.dtype != GgmlType::Q4_K || !matches!(sf.down_w.dtype, GgmlType::Q6_K | GgmlType::Q4_K) {
                            return Ok(None);
                        }
                        let smid = self.config.n_shared_experts * self.config.moe_intermediate;
                        (Some(sf.gate_w.offset), Some(sf.up_w.offset), Some(sf.down_w.offset), Some(sf.down_w.dtype), smid)
                    } else {
    ...
    fn attention(&mut self, li: usize, pos: usize, x: &[f32]) -> Result<Vec<f32>> {
        ...
        if let (Some(qa), Some(qan), Some(qb)) = (&layer.q_a_proj, &layer.q_a_norm, &layer.q_b_proj) {
            ...
            self.gemv_f32_attn_pair_dispatch(qa, layer.pinned.q_a_proj.as_ref(), q_lora, &layer.kv_a_proj_with_mqa, layer.pinned.kv_a_proj_with_mqa.as_ref(), kv_a_dim, h, x, &mut t, &mut kv_a)?;

            let mut tn = vec![0.0f32; q_lora];
            self.rmsnorm_dispatch(&t, qan, cfg.rms_norm_eps, &mut tn)?;
            self.gemv_f32_attn_dispatch(qb, layer.pinned.q_b_proj.as_ref(), n_heads * head_dim_q, q_lora, &tn, &mut q_full)?;
        ...
        if !self.mla_c_kv.is_empty() {
            ...
            let seq_slot = self.kv.seq_len;
            let n_layers = self.config.n_layers;
            Self::mla_kv_append(&mut self.mla_c_kv, &mut self.mla_k_pe, li, seq_slot, cfg.kv_lora_rank, cfg.qk_rope_head_dim, &c_kv, &k_pe)?;
            if li + 1 == n_layers {
                self.kv.seq_len += 1;
            }
            ...
            if let Some(ctx) = self.metal_ctx.as_ref() {
                let kv_b_buf = layer.pinned.kv_b_proj.as_ref().ok_or_else(|| Error::Model(format!("layer {li}: kv_b_proj not pinned for MLA Metal path")))?;

                let layer_cb = self.kernel_profile.as_ref().map(|p| p.selected.command_buffering == "layer-cb").unwrap_or(false);
                ...
                if layer_cb {
                    if let (Some(o_proj_buf), Some(arena)) = (layer.pinned.o_proj.as_ref(), self.decode_arena.as_ref()) {
                        arena.write_q(&q_full);
                        MetalContext::write_buffer_bytes(&arena.c_kv, bytemuck::cast_slice(&self.mla_c_kv[li][..seq_len * cfg.kv_lora_rank]));
                        MetalContext::write_buffer_bytes(&arena.k_pe, bytemuck::cast_slice(&self.mla_k_pe[li][..seq_len * cfg.qk_rope_head_dim]));
                ...
                let attn_schedule = self.kernel_profile.as_ref().map(|p| p.selected.attn_block_schedule.as_str()).unwrap_or("mla");
                if attn_schedule == "flash" {
                    ...
                    let mut out = vec![0.0f32; h];
                    self.gemv_f32_attn_dispatch(&layer.o_proj, layer.pinned.o_proj.as_ref(), h, n_heads * cfg.v_head_dim, &attn_out, &mut out)?;
                    return Ok(out);
                }
            ...
            return Err(Error::Model("mla_decode: Metal context unavailable on this platform".into()));
        }
        ...
        self.gemv_f32_attn_dispatch(&layer.kv_b_proj, layer.pinned.kv_b_proj.as_ref(), n_heads * kv_b_out_per_head, cfg.kv_lora_rank, &c_kv, &mut kv_b_out)?;
        ...
        for h_i in 0..n_heads {
            ...
            let kv_h = if cfg.n_kv_heads == n_heads { h_i } else { h_i * cfg.n_kv_heads / n_heads };
        ...
        crate::attn::mha_decode_step(&q_full, &self.kv.keys[li][..seq_len * stride], &self.kv.values[li][..seq_len * stride], n_heads, cfg.n_kv_heads, head_dim_q, seq_len, &mut attn_out)?;
        ...
        let mut out = vec![0.0f32; h];
        self.gemv_f32_attn_dispatch(&layer.o_proj, layer.pinned.o_proj.as_ref(), h, n_heads * cfg.v_head_dim, &concat, &mut out)?;
        Ok(out)
    }
    ...
    fn ffn(&self, li: usize, x: &[f32]) -> Result<Vec<f32>> {
        ...
        match &layer.mode {
            LayerMode::Dense { gate_w, up_w, down_w } => {
            ...
            LayerMode::MoE { gate_logits_w, routed_fused, routed, shared_fused, shared } => {
                ...
                self.gemv_f32_moe_dispatch(gate_logits_w, cfg.n_routed_experts, cfg.hidden, x, &mut logits)?;
                let routes = topk_gate(&mut logits, cfg.top_k_routed, true);

                // Batched indexed one-command-buffer path (current default).
                if let Some(batched) = self.moe_block_batched_dispatch(routed_fused, routed, shared_fused.as_ref(), &routes, x)? {
                    out = batched;
                    return Ok(out);
                }
                ...
                for &(eid, weight) in &routes {
                    let e = &routed[eid];
                    self.moe_expert_pair_matmul_dispatch(&e.gate_w, &e.up_w, mid, cfg.hidden, x, &mut a_buf, &mut w_buf)?;
                    self.moe_expert_matmul_dispatch(&e.down_w, cfg.hidden, mid, &a_buf, &mut tmp, &mut w_buf)?;
                ...
                if let Some(s) = shared.first() {
                    ...
                    self.moe_expert_pair_matmul_dispatch(&s.gate_w, &s.up_w, smid, cfg.hidden, x, &mut sa, &mut w_buf)?;

                    self.moe_expert_matmul_dispatch(&s.down_w, cfg.hidden, smid, &sa, &mut tmp, &mut w_buf)?;
                    for i in 0..cfg.hidden {
                        out[i] += tmp[i];
                    }
                }
    ...
    fn ffn_shared_only(&self, li: usize, x: &[f32]) -> Result<Vec<f32>> {
        ...
        match &layer.mode {
            ...
            LayerMode::MoE { gate_logits_w, routed_fused: _, routed: _, shared_fused, shared } => {
