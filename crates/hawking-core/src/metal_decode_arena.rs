    ...
    impl DecodeArena {
        pub fn new(
            ...
            Self {
                ...
                route_history_buf: ctx.new_buffer(n_moe_layers.max(1) * top_k_sz * std::mem::size_of::<u32>()),
                batch_x_norm_buf: (0..max_batch_size.max(1)).map(|_| ctx.new_buffer(hidden * std::mem::size_of::<f32>())).collect(),
