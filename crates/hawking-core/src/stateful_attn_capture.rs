...
//!
//! Entirely gated behind `HAWKING_QWEN_ATTN_CAPTURE=1`. With the flag
...
/// `true` when `HAWKING_QWEN_ATTN_CAPTURE=1`. Cheap; called per layer per
/// prefill token, so it short-circuits the whole instrument when unset.
#[cfg(target_os = "macos")]
pub fn enabled() -> bool {
    crate::env_on("HAWKING_QWEN_ATTN_CAPTURE")
}
...
pub fn record_layer(layer: usize, n_layers: usize, head_weights: &[&[f32]]) {
    ...
    let st = guard.get_or_insert_with(|| CaptureState {
        ...
        min_ctx_len: std::env::var("HAWKING_ATTN_CAPTURE_MIN_CTX").ok().and_then(|v| v.parse().ok()).unwrap_or(512),
    });
    ...
    let sink_mass: f64 = avg[..SINK_SPAN.min(ctx_len)].iter().map(|&w| w as f64).sum();
    ...
    for ri in 0..RECENT_SPANS.len() {
        ...
        let overlap = if overlap_start < SINK_SPAN.min(ctx_len) { avg[overlap_start..SINK_SPAN.min(ctx_len)].iter().map(|&w| w as f64).sum::<f64>() } else { 0.0 };
...
pub fn flush() {
    ...
    let out_path = std::env::var("HAWKING_ATTN_CAPTURE_OUT").unwrap_or_else(|_| "reports/bench/attn_capture.json".to_string());
    ...
    s.push_str(&format!("  \"sink_span\": {}, \"recent_spans\": [{}],\n", SINK_SPAN, RECENT_SPANS.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(", ")));
    s.push_str(&format!("  \"mass_thresholds\": [{}],\n", MASS_THRESHOLDS.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(", ")));
    s.push_str("  \"layers\": [\n");
    let mut first = true;
    for (li, a) in layers.iter().enumerate() {
        ...
        let mean_min_pos: Vec<String> = a.sum_min_pos.iter().map(|&x| format!("{:.2}", x as f64 / n)).collect();
        let mean_min_frac: Vec<String> = a.sum_min_frac.iter().map(|&x| format!("{:.5}", x / n)).collect();
        let mean_recent: Vec<String> = a.sum_recent_mass.iter().map(|&x| format!("{:.5}", x / n)).collect();
        let mean_sink_recent: Vec<String> = a.sum_sink_plus_recent.iter().map(|&x| format!("{:.5}", x / n)).collect();
        s.push_str(&format!(
