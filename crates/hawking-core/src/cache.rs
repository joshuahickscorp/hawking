...
#[path = "cache_prefill_disk.rs"]
pub mod prefill_disk;
#[path = "cache_prefix_lint.rs"]
pub mod prefix_lint;
#[path = "cache_sstate_disk.rs"]
pub mod sstate_disk;
...
impl KvCache {
    pub fn new(n_layers: usize, max_seq: usize, n_kv_heads: usize, head_dim: usize) -> Self {
        ...
        Self { max_seq, n_layers, n_kv_heads, head_dim, keys: (0..n_layers).map(|_| vec![0.0f32; per_layer]).collect(), values: (0..n_layers).map(|_| vec![0.0f32; per_layer]).collect(), seq_len: 0 }
    }
    ...
    pub fn append(&mut self, k_per_layer: &[Vec<f32>], v_per_layer: &[Vec<f32>]) -> Result<()> {
        if self.seq_len >= self.max_seq {
            return Err(crate::Error::Model(format!("kv cache full at {}", self.max_seq)));
        }
