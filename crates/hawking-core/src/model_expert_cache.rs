...
impl ExpertAccessStats {
    pub fn new() -> Self {
        Self { active_count: AtomicU64::new(0), last_active_pos: AtomicU64::new(0) }
    }
...
impl ExpertCache {
    pub fn new(n_layers: usize, n_experts: usize, eviction_threshold_tokens: u64) -> Self {
        Self {
            stats: (0..n_layers).map(|_| LayerExpertStats { experts: (0..n_experts).map(|_| ExpertAccessStats::new()).collect(), window_size: eviction_threshold_tokens }).collect(),
...
mod tests {
    ...
    fn evict_without_model_base_is_noop() {
        ...
        let ranges = [ExpertRange { layer: 0, expert_id: 0, byte_offset: 0, byte_size: 4096 }, ExpertRange { layer: 0, expert_id: 1, byte_offset: 4096, byte_size: 4096 }];
