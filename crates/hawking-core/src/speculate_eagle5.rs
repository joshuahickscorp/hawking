...
impl Eagle5Head {
    ...
    pub fn mock(seed: u64, hidden: usize, vocab: usize) -> Self {
        ...
        Self { inner: Inner::Mock { embed, out_w }, vocab, hidden, last_token: None, lm_head_pruned: None }
    }
    ...
    pub fn set_vocab_prune(&mut self, remap: &[u32]) {
        ...
        if let Inner::Trained { lm_head_f16, .. } = &self.inner {
            ...
            eprintln!("[eagle5] built vocab-pruned propose LM head: {} -> {} cols", v, remap.len());
        }
    }
    ...
    pub fn load_from_safetensors(path: &Path, expected_hidden: usize, expected_vocab: usize) -> Result<Self> {
        let st = SafeTensors::open(path)?;
        let meta = st.metadata();
        let parse_usize = |k: &str| -> Result<Option<usize>> { meta.get(k).map(|v| v.parse::<usize>().map_err(|e| Error::Model(format!("eagle5: '{k}' parse failed: {e}")))).transpose() };
        let parse_f32 = |k: &str| -> Result<Option<f32>> { meta.get(k).map(|v| v.parse::<f32>().map_err(|e| Error::Model(format!("eagle5: '{k}' parse failed: {e}")))).transpose() };
        let in_proj_shape = st.shape("in_proj.weight")?;
        if in_proj_shape.len() != 2 || in_proj_shape[1] != 3 * in_proj_shape[0] {
            return Err(Error::Model(format!("eagle5: in_proj.weight shape is {in_proj_shape:?}; expected [hidden, 3*hidden]")));
        }
        let inferred_hidden = in_proj_shape[0];
        let emb_shape = st.shape("_token_embd")?;
        if emb_shape.len() != 2 {
            return Err(Error::Model(format!("eagle5: _token_embd shape is {emb_shape:?}; expected [hidden, vocab]")));
        }
        ...
        if gate_shape.len() != 2 || gate_shape[1] != inferred_hidden {
            return Err(Error::Model(format!("eagle5: block.mlp.gate.weight shape is {gate_shape:?}; expected [ff_dim, hidden]")));
        }
        ...
        let inferred_n_heads = if inferred_hidden % 16 == 0 { 16 } else { [12usize, 8, 4, 2, 1].into_iter().find(|h| inferred_hidden % h == 0).unwrap_or(1) };
        ...
        if hidden_dim != expected_hidden {
            return Err(Error::Model(format!("eagle5: head hidden_dim={hidden_dim} but verifier expects {expected_hidden}")));
        }
        if vocab_size != expected_vocab {
            return Err(Error::Model(format!("eagle5: head vocab_size={vocab_size} but verifier expects {expected_vocab}")));
        }
        if num_blocks == 0 {
            return Err(Error::Model("eagle5: num_blocks must be ≥1".to_string()));
        }
        if n_heads == 0 || hidden_dim % n_heads != 0 {
            return Err(Error::Model(format!("eagle5: invalid n_heads={n_heads} for hidden={hidden_dim}")));
        }
        if inferred_ff_dim != ff_dim {
            return Err(Error::Model(format!("eagle5: inferred ff_dim={inferred_ff_dim} but metadata implies {ff_dim}")));
        }

        let config = TrainedConfig { hidden_dim, vocab_size, n_heads, ff_mult, num_blocks, ff_dim };
        ...
        let load_block = |prefix: &str| -> Result<TrainedBlock> {
            Ok(TrainedBlock {
                ...
                q_proj: st.read_f32(&format!("{prefix}q_proj.weight"), &[hidden_dim, hidden_dim])?,
                k_proj: st.read_f32(&format!("{prefix}k_proj.weight"), &[hidden_dim, hidden_dim])?,
                v_proj: st.read_f32(&format!("{prefix}v_proj.weight"), &[hidden_dim, hidden_dim])?,
                out_proj: st.read_f32(&format!("{prefix}out_proj.weight"), &[hidden_dim, hidden_dim])?,
                mlp_norm: st.read_f32(&format!("{prefix}mlp_norm"), &[hidden_dim])?,
                mlp_gate: st.read_f32(&format!("{prefix}mlp.gate.weight"), &[ff_dim, hidden_dim])?,
                mlp_up: st.read_f32(&format!("{prefix}mlp.up.weight"), &[ff_dim, hidden_dim])?,
                mlp_down: st.read_f32(&format!("{prefix}mlp.down.weight"), &[hidden_dim, ff_dim])?,
            })
        };
        ...
        Ok(Self {
            inner: Inner::Trained { config, in_proj, blocks, residual_gate, output_norm, token_embd_f16, lm_head_f16 },
            vocab: vocab_size,
            hidden: hidden_dim,
            last_token: None,
            lm_head_pruned: None,
    ...
    pub fn propose_with_capture(&mut self, prev_token: u32, residual_in: &[f32], intermediate: &[f32], k: usize) -> Vec<u32> {
        if k == 0 || self.vocab == 0 {
            return Vec::new();
        }
        let mut out = Vec::with_capacity(k);
    ...
    pub fn propose_rollout(&self, start_token: u32, residual_in: &[f32], intermediate: &[f32], k: usize) -> Vec<u32> {
        if k == 0 || self.vocab == 0 {
            return Vec::new();
        }
        let mut out = Vec::with_capacity(k);
    ...
    pub fn propose_rollout_chained(&self, start_token: u32, residual_in: &[f32], intermediate: &[f32], k: usize) -> Vec<u32> {
        if k == 0 || self.vocab == 0 {
            return Vec::new();
        }
        match &self.inner {
            Inner::Mock { .. } => self.propose_rollout(start_token, residual_in, intermediate, k),
            Inner::Trained { config, in_proj, blocks, residual_gate, output_norm, token_embd_f16, lm_head_f16 } => {
                ...
                for _ in 0..k {
                    let draft_hidden = compute_draft_hidden(config, in_proj, blocks, *residual_gate, output_norm, token_embd_f16, cur, &res, inter);
    ...
    fn argmax_step_full(&self, prev: u32, residual_in: &[f32], intermediate: &[f32]) -> u32 {
        match &self.inner {
            ...
            Inner::Trained { .. } => {
                let logits = self.forward_logits(prev, residual_in, intermediate).expect("Trained variant must return Some(logits)");
                crate::kernels::argmax_f32(&logits) as u32
            }
        }
    }
    ...
    pub fn forward_logits(&self, prev_token: u32, residual_in: &[f32], intermediate: &[f32]) -> Option<Vec<f32>> {
        match &self.inner {
            Inner::Mock { .. } => None,
            Inner::Trained { config, in_proj, blocks, residual_gate, output_norm, token_embd_f16, lm_head_f16 } => {
                Some(crate::speculate::eagle5_forward::forward_single_step(config, in_proj, blocks, *residual_gate, output_norm, token_embd_f16, lm_head_f16, prev_token, residual_in, intermediate))
            }
        }
    }
    ...
    pub fn lens_argmax(&self, residual_in: &[f32]) -> Option<u32> {
        match &self.inner {
            Inner::Mock { .. } => None,
            Inner::Trained { config, output_norm, lm_head_f16, .. } => {
                let h = config.hidden_dim;
                let v = config.vocab_size;
    ...
    fn argmax_step(&self, prev: u32) -> u32 {
        ...
        match &self.inner {
            ...
            Inner::Trained { token_embd_f16, lm_head_f16, .. } => {
...
mod tests {
    ...
    fn trained_loader_rejects_nonexistent_file() {
        let result = Eagle5Head::load_from_safetensors(Path::new("/nonexistent/path.safetensors"), 128, 5000);
    ...
    fn trained_loader_reads_synthetic_head() {
        ...
        let push_f32 = |name: &str, shape: Vec<usize>, entries: &mut Vec<_>, bytes: &mut Vec<u8>| {
        ...
        let push_f16 = |name: &str, shape: Vec<usize>, entries: &mut Vec<_>, bytes: &mut Vec<u8>| {
            let n = shape.iter().product::<usize>();
            let start = bytes.len();
            for i in 0..n {
        ...
        push_f32("in_proj.weight", vec![hidden, 3 * hidden], &mut entries, &mut tensor_bytes);
        push_f32("block.attn_norm", vec![hidden], &mut entries, &mut tensor_bytes);
        for k in ["q_proj", "k_proj", "v_proj", "out_proj"] {
            push_f32(&format!("block.{k}.weight"), vec![hidden, hidden], &mut entries, &mut tensor_bytes);
        }
        push_f32("block.mlp_norm", vec![hidden], &mut entries, &mut tensor_bytes);
        push_f32("block.mlp.gate.weight", vec![ff_dim, hidden], &mut entries, &mut tensor_bytes);
        push_f32("block.mlp.up.weight", vec![ff_dim, hidden], &mut entries, &mut tensor_bytes);
        push_f32("block.mlp.down.weight", vec![hidden, ff_dim], &mut entries, &mut tensor_bytes);
        push_f32("residual_gate", vec![1], &mut entries, &mut tensor_bytes);
        push_f32("_output_norm", vec![hidden], &mut entries, &mut tensor_bytes);
        push_f16("_token_embd", vec![hidden, vocab], &mut entries, &mut tensor_bytes);
        push_f16("_lm_head", vec![hidden, vocab], &mut entries, &mut tensor_bytes);
        ...
        header.push_str(&format!("\"__metadata__\":{{\"hidden_dim\":\"{hidden}\",\"vocab_size\":\"{vocab}\",\"n_heads\":\"{n_heads}\",\"ff_mult\":\"{ff_mult}\",\"num_blocks\":\"{num_blocks}\"}}"));
        for (name, dtype, shape, start, end) in &entries {
            let shape_str = shape.iter().map(|d| d.to_string()).collect::<Vec<_>>().join(",");
            header.push_str(&format!(",\"{name}\":{{\"dtype\":\"{dtype}\",\"shape\":[{shape_str}],\"data_offsets\":[{start},{end}]}}"));
        }
        header.push('}');
        ...
        for (idx, (name, dtype, shape, start, end)) in entries.iter().enumerate() {
            ...
            let shape_str = shape.iter().map(|d| d.to_string()).collect::<Vec<_>>().join(",");
            legacy_header.push_str(&format!("\"{name}\":{{\"dtype\":\"{dtype}\",\"shape\":[{shape_str}],\"data_offsets\":[{start},{end}]}}"));
        }
        legacy_header.push('}');
        let legacy_path = dir.join("synthetic_legacy_no_metadata.safetensors");
        let mut f = std::fs::File::create(&legacy_path).unwrap();
        f.write_all(&(legacy_header.len() as u64).to_le_bytes()).unwrap();
