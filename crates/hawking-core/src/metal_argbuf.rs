...
mod imp {
    ...
    impl KernelArgBuffer {
        ...
        pub fn new(ctx: &MetalContext, layout: &[ArgLayout]) -> Result<Self> {
            ...
            let buf = ctx.device().new_buffer(size as u64, MTLResourceOptions::StorageModeShared);
            Ok(Self { buf, offsets })
        }
...
mod imp {
    ...
    impl KernelArgBuffer {
        pub fn new(_ctx: &MetalContext, _layout: &[ArgLayout]) -> Result<Self> {
            Err(crate::Error::Metal("KernelArgBuffer: Metal unavailable on this platform".into()))
        }
