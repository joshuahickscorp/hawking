...
#[path = "model_arch_config.rs"]
pub mod arch_config;
#[path = "model_deepseek_v2.rs"]
pub mod deepseek_v2;
#[path = "model_expert_cache.rs"]
pub mod expert_cache;
#[path = "model_gemma2.rs"]
pub mod gemma2;
#[path = "model_llama.rs"]
pub mod llama;
#[path = "model_mamba2.rs"]
pub mod mamba2;
#[path = "model_mixtral.rs"]
pub mod mixtral;
#[path = "model_olmoe.rs"]
pub mod olmoe;
#[path = "model_phi3.rs"]
pub mod phi3;
#[path = "model_qwen_dense.rs"]
pub mod qwen_dense;
#[path = "model_qwen_moe.rs"]
pub mod qwen_moe;
#[path = "model_rwkv7.rs"]
pub mod rwkv7;
...
use self::weights::TensorRef;
use crate::gguf::{GgmlType, GgufFile};
use crate::kernels::{gemv_f16, gemv_f32, rmsnorm};
use crate::metal::MetalContext;
use crate::{Engine, EngineConfig, Error, Result};
use half::f16;
use std::path::Path;

fn dequant_ref_into(gguf: &GgufFile, tensor: &TensorRef, scratch: &mut Vec<f32>) -> Result<()> {
    if scratch.len() != tensor.n_elems {
        scratch.resize(tensor.n_elems, 0.0);
    }
    crate::quant::dequant_into(tensor.dtype, &gguf.mmap[tensor.offset..tensor.offset + tensor.byte_size], scratch)
}

fn rmsnorm_dispatch(metal: Option<&MetalContext>, x: &[f32], weight: &[f32], eps: f32, out: &mut [f32]) -> Result<()> {
    #[cfg(target_os = "macos")]
    if let Some(ctx) = metal {
...
fn matmul_q4_dispatch(gguf: &GgufFile, metal: Option<&MetalContext>, tensor: &TensorRef, rows: usize, cols: usize, x: &[f32], out: &mut [f32], scratch: &mut Vec<f32>) -> Result<()> {
    #[cfg(target_os = "macos")]
    if let Some(ctx) = metal {
        if tensor.dtype == GgmlType::Q4_K {
            return crate::kernels::gemv_q4_k_m(ctx, &gguf.mmap[tensor.offset..tensor.offset + tensor.byte_size], rows, cols, x, out);
        }
    }
    #[cfg(not(target_os = "macos"))]
    let _ = metal;
    dequant_ref_into(gguf, tensor, scratch)?;
    gemv_f32(scratch, rows, cols, x, out);
    Ok(())
}

fn gemv_f16_dispatch(metal: Option<&MetalContext>, weights: &[f16], rows: usize, cols: usize, x: &[f32], out: &mut [f32]) -> Result<()> {
    #[cfg(target_os = "macos")]
    if let Some(ctx) = metal {
        return crate::kernels::gemv_f16_metal(ctx, bytemuck::cast_slice::<f16, u8>(weights), rows, cols, x, out);
    }
    #[cfg(not(target_os = "macos"))]
    let _ = metal;
    gemv_f16(weights, rows, cols, x, out);
    Ok(())
}
...
pub fn load_engine(weights: &Path, mut config: EngineConfig) -> Result<Box<dyn Engine>> {
    ...
    if let Some(limit_mb) = config.memory_limit_mb {
        ...
        if effective_limit_mb > 0 {
            let file_mb = std::fs::metadata(weights).map(|m| m.len() / (1024 * 1024)).unwrap_or(0) as usize;
    ...
    match arch.as_str() {
        ...
        other => Err(Error::Model(format!("unknown architecture {other:?}; supports llama (dense + mixtral) + deepseek2 + qwen2 + qwen-moe + gemma2 + phi3 + rwkv7 + mamba2 + olmoe"))),
    }
}
...
fn tier_map_overrides_for_names<'a>(tm: &crate::sidecar::SidecarTierMap, names: impl IntoIterator<Item = &'a str>) -> usize {
    names.into_iter().filter(|name| matches!(tm.dtype_for(name), Ok(Some(_)))).count()
}
...
/// materialization is `docs/RESEARCH.md` #16 (Type-1 dead) /
...
fn honor_sidecar_tier_map(weights: &Path, gguf: &crate::gguf::GgufFile) -> usize {
    ...
    for (name, info) in &gguf.tensors {
        match tier_map.dtype_for(name) {
            Ok(Some(target)) => {
                eprintln!("[tier-map] {name}: gguf={:?} -> sidecar override {:?} (read-only; requant out of scope)", info.dtype, target);
            }
            Ok(None) => {}
            Err(e) => {
    ...
    eprintln!("[tier-map] sidecar {:?}: {honored} of {} tier-map entries matched live GGUF tensors", sidecar_path, tier_map.entries.len());
    honored
}
...
mod tier_map_hook_tests {
    ...
    fn tm(pairs: &[(&str, &str)]) -> SidecarTierMap {
        SidecarTierMap { entries: pairs.iter().map(|(t, d)| SidecarTierEntry { tensor: (*t).to_string(), dtype: (*d).to_string() }).collect() }
    }
