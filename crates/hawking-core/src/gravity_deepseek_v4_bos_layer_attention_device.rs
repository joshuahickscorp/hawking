//! Parameterized BOS/position-0 attention device graph for any DeepSeek-V4 base layer.
//!
//! Generalizes the layer-1 BOS successor into a layer-N primitive: accept a
//! device-resident predecessor HC child (BF16[4,4096]), stage authenticated
//! `layers.{N}.*` attention controls, and execute the same ordered window-KV
//! sparse graph used at BOS for ratio-0 layers.
//!
//! Honesty for compressed layers (ratio-4 / ratio-128):
//! - At `start_pos=0` with `seqlen=1`, the source algorithm yields
//!   `end_pos // ratio == 0` compressed slots, so indexer / compress topk is
//!   empty and sparse attention is pure sliding-window over the single written
//!   KV row. That is the exact source BOS specialization, not a substitute for
//!   the full compressed causal graph at later positions.
//! - Full compressed-slot attention, growing multi-token KV for ratio>0,
//!   Engine, HCLI, serve, and TPS claims are out of scope.
...
// grammar, deliberately declared locally so this BOS layer successor has no runtime
...
pub const DSV4F_BOS_LAYER_TOKEN_ID: u32 = 0;
pub const DSV4F_BOS_LAYER_POSITION: usize = 0;
pub const DSV4F_BOS_LAYER_HC_STATE_BF16_BYTES: usize = HC_FLAT_WIDTH * size_of::<u16>();
pub const DSV4F_BOS_LAYER_KV_SLOT0_BF16_BYTES: usize = HEAD_DIM * size_of::<u16>();
/// One ordered command buffer: mHC pre/norm, Q/KV, attention, output, mHC
/// post. Topology accounting for this bounded primitive only.
pub const DSV4F_BOS_LAYER_ATTENTION_DISPATCHES: usize = 22;
...
/// Immutable source-native identity of one static BOS layer control.  The raw bytes
/// are uploaded directly to Metal and are never retained by this public
/// surface.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DeepSeekV4BosLayerAttentionSourceTensorBinding {
    pub name: String,
    pub dtype: String,
    pub shape: Vec<u64>,
    pub bytes: usize,
...
/// be an authenticated `layers.{N}.*` source tensor.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DeepSeekV4BosLayerAttentionSourceBindings {
    ...
    pub controls: Vec<DeepSeekV4BosLayerAttentionSourceTensorBinding>,
    pub source_parent_retained: bool,
    pub host_activation_handoff_permitted: bool,
    pub runtime_boundary: &'static str,
}

/// One caller-owned predecessor child boundary.  It deliberately fixes the only
/// accepted identity (layer 0/BOS/position 0), so a caller cannot relabel a
/// different layer or token as the BOS layer predecessor.  Metal buffers are owned
...
pub struct DeepSeekV4BosLayerChildDeviceInput<'a> {
    metal: &'a MetalContext,
    child_hc_state_bf16: &'a metal::Buffer,
    queue_identity: usize,
    /// Layer index of the predecessor that produced this child HC state.
    predecessor_layer: usize,
}

impl<'a> DeepSeekV4BosLayerChildDeviceInput<'a> {
    /// Bind a device-resident predecessor child HC state. No activation copy.
    pub fn from_predecessor_child(
        metal: &'a MetalContext,
        child_hc_state_bf16: &'a metal::Buffer,
        predecessor_layer: usize,
    ) -> Result<Self> {
        if predecessor_layer >= 42 {
            return Err(bos_error(
                "BOS layer predecessor must be a base-layer child (0..41)",
            ));
        }
        validate_buffer_device(metal, child_hc_state_bf16, "predecessor child")?;
        if child_hc_state_bf16.length() != DSV4F_BOS_LAYER_HC_STATE_BF16_BYTES as u64 {
            return Err(bos_error(
                "predecessor child input must be an exact BF16[4,4096] Metal buffer",
            ));
        }
        Ok(Self {
            metal,
    ...
    /// Construct from a completed position-0 P7 child. Accepts any base layer
    /// 0..41 so the multi-layer loop can chain L0→L1→… without host handoff.
    pub fn from_p7_position0_child(
        metal: &'a MetalContext,
        output: &'a DeepSeekV4P7DeviceOutput,
    ) -> Result<Self> {
        output.validate()?;
        if output.token_id != DSV4F_BOS_LAYER_TOKEN_ID
            || output.token_position != DSV4F_BOS_LAYER_POSITION
        {
            return Err(bos_error(
                "BOS layer continuation accepts only position-0 / BOS P7 children",
            ));
        }
        Self::from_predecessor_child(metal, &output.child_hc_state_bf16, output.layer)
    }

    pub const fn source_layer(&self) -> usize {
        self.predecessor_layer
    }

    pub const fn token_id(&self) -> u32 {
        DSV4F_BOS_LAYER_TOKEN_ID
    }

    pub const fn token_position(&self) -> usize {
        DSV4F_BOS_LAYER_POSITION
    }

    fn validate_for(&self, metal: &MetalContext, expected_layer: usize) -> Result<()> {
        if !std::ptr::eq(self.metal, metal) || self.queue_identity != context_queue_identity(metal)
        {
            return Err(bos_error(
                "predecessor child must execute through its original caller-owned MetalContext/queue",
            ));
        }
        if self.predecessor_layer.checked_add(1) != Some(expected_layer) {
            return Err(bos_error(format!(
                "BOS layer {expected_layer} requires predecessor layer {}, got {}",
                expected_layer.saturating_sub(1),
                self.predecessor_layer
            )));
        }
        validate_buffer_device(metal, self.child_hc_state_bf16, "predecessor child")?;
        if self.child_hc_state_bf16.length() != DSV4F_BOS_LAYER_HC_STATE_BF16_BYTES as u64 {
            return Err(bos_error(
                "predecessor child buffer geometry changed after its BOS input binding",
            ));
        }
        Ok(())
    }
...
pub enum DeepSeekV4BosLayerAttentionDevicePhase {
    Prepared,
    Complete,
}

impl DeepSeekV4BosLayerAttentionDevicePhase {
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::Prepared => "prepared_bos_layer_attention_not_executed",
            Self::Complete => "complete_bos_layer_attention_device_only",
        }
    }
}

/// Device-only retained state after the bounded BOS layer attention graph.  The
/// buffers remain tied to the caller's Metal device; there is deliberately no
/// host activation/readback method on this type.
pub struct DeepSeekV4BosLayerAttentionDeviceOutput {
...
impl DeepSeekV4BosLayerAttentionDeviceOutput {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.attention_hc_state_bf16.length() != DSV4F_BOS_LAYER_HC_STATE_BF16_BYTES as u64
            || self.causal_kv_slot0_bf16.length() != DSV4F_BOS_LAYER_KV_SLOT0_BF16_BYTES as u64
            || self.layer == 0
            || self.layer >= 43
            || self.token_id != DSV4F_BOS_LAYER_TOKEN_ID
            || self.token_position != DSV4F_BOS_LAYER_POSITION
            || self.kv_rows != 1
            || self.actual_command_buffers != 1
            || self.actual_compute_encoders != DSV4F_BOS_LAYER_ATTENTION_DISPATCHES
            || self.actual_gpu_dispatches != DSV4F_BOS_LAYER_ATTENTION_DISPATCHES
            || self.actual_cpu_visible_waits != 1
            || self.host_intermediate_handoff_bytes != 0
        {
            return Err(bos_error(
                "BOS layer attention output has invalid state identity, geometry, or topology",
            ));
        }
        Ok(())
    }
...
struct VerifiedTensor {
    binding: DeepSeekV4BosLayerAttentionSourceTensorBinding,
    bytes: Vec<u8>,
}
...
impl LayerControls {
    ...
    fn bindings(&self) -> Vec<DeepSeekV4BosLayerAttentionSourceTensorBinding> {
        [
            &self.hc_fn,
            &self.hc_base,
            &self.hc_scale,
...
/// Prepared static BOS layer controls and device scratch for exactly one L0/BOS
...
pub struct DeepSeekV4BosLayerAttentionDeviceExecutor {
    source_bindings: DeepSeekV4BosLayerAttentionSourceBindings,
    context_queue_identity: usize,
    phase: DeepSeekV4BosLayerAttentionDevicePhase,
    hc_fn: metal::Buffer,
...
impl DeepSeekV4BosLayerAttentionDeviceExecutor {
    /// Verify and upload static `layers.{layer}.*` attention controls for one
    /// BOS/position-0 step. The caller-owned predecessor state is not read
    /// during preparation.
    pub fn prepare(
        metal: &MetalContext,
        ...
        if layer == 0 || layer >= 43 {
            return Err(bos_error(
                "BOS layer attention executor targets base layers 1..42 (layer 0 uses the P3A/P4A embed path)",
            ));
        }
        verify_bos_source_contract(reader, layer)?;
        ...
        let bindings = DeepSeekV4BosLayerAttentionSourceBindings {
            ...
            layer,
            token_id: DSV4F_BOS_LAYER_TOKEN_ID,
            token_position: DSV4F_BOS_LAYER_POSITION,
            compress_ratio,
            ...
            runtime_boundary: "caller-owned-context BOS/position-0 window-KV attention for one base layer; compressed slots are empty at BOS; no FFN/MoE, Engine, HCLI, serve, or TPS claim",
        };
        if bindings.controls.len() != 17
            || bindings
                .controls
                .iter()
                .any(|control| !control.name.starts_with(&layer_prefix))
        {
            return Err(bos_error(format!(
                "BOS layer {layer} preparation tried to stage a non-layer or incomplete source-control set"
            )));
        }
        ...
        Ok(Self {
            ...
            phase: DeepSeekV4BosLayerAttentionDevicePhase::Prepared,
            ...
            causal_kv_slot0_bf16: metal.new_buffer_checked(DSV4F_BOS_LAYER_KV_SLOT0_BF16_BYTES)?,
    ...
    pub fn source_bindings(&self) -> &DeepSeekV4BosLayerAttentionSourceBindings {
        &self.source_bindings
    }

    pub const fn phase(&self) -> DeepSeekV4BosLayerAttentionDevicePhase {
        self.phase
    }
    ...
    pub fn execute(
        &mut self,
        metal: &MetalContext,
        input: DeepSeekV4BosLayerChildDeviceInput<'_>,
    ) -> Result<DeepSeekV4BosLayerAttentionDeviceOutput> {
        if self.phase != DeepSeekV4BosLayerAttentionDevicePhase::Prepared {
            return Err(bos_error(
                "BOS layer BOS attention executor may execute only once per preparation",
            ));
        }
        if context_queue_identity(metal) != self.context_queue_identity {
            return Err(bos_error(
                "BOS layer BOS attention executor requires its preparation MetalContext/queue",
            ));
        }
        input.validate_for(metal, self.source_bindings.layer)?;
        ...
        let timing = metal.dispatch_batch_timed(|batch| {
            ...
            // and BOS layer `hc_attn_*` controls, never a P6/MoE buffer.
            dispatch_hc_post(
                batch,
                &self.wo_b.bf16,
                input.child_hc_state_bf16,
        ...
        if timing.command_buffers != 1
            || timing.compute_encoders as usize != DSV4F_BOS_LAYER_ATTENTION_DISPATCHES
            || timing.compute_dispatches as usize != DSV4F_BOS_LAYER_ATTENTION_DISPATCHES
        {
            return Err(bos_error(
                "BOS layer BOS attention completion topology differs from its declared ordered graph",
            ));
        }
        self.phase = DeepSeekV4BosLayerAttentionDevicePhase::Complete;
        let output = DeepSeekV4BosLayerAttentionDeviceOutput {
            attention_hc_state_bf16: self.attention_hc_state_bf16.to_owned(),
            causal_kv_slot0_bf16: self.causal_kv_slot0_bf16.to_owned(),
            layer: self.source_bindings.layer,
            token_id: DSV4F_BOS_LAYER_TOKEN_ID,
            token_position: DSV4F_BOS_LAYER_POSITION,
            kv_rows: 1,
            timing,
            actual_command_buffers: 1,
            actual_compute_encoders: DSV4F_BOS_LAYER_ATTENTION_DISPATCHES,
            actual_gpu_dispatches: DSV4F_BOS_LAYER_ATTENTION_DISPATCHES,
...
fn verify_bos_source_contract(reader: &DeepSeekV4FullStreamReader, layer: usize) -> Result<()> {
    if reader.source_identity().repository != PINNED_REPOSITORY
        || reader.source_identity().revision != PINNED_REVISION
    {
        return Err(bos_error(
            "BOS layer BOS attention requires the pinned DeepSeek-V4-Flash source identity",
        ));
    }
    ...
    {
        return Err(bos_error(
            "BOS layer source metadata hashes differ from the pinned source grammar",
        ));
    }
    let config: Value = serde_json::from_slice(
        &reader.read_verified_metadata_asset("inference/config.json", 64 * 1024)?,
    )
    .map_err(|error| bos_error(format!("BOS layer inference config JSON: {error}")))?;
    for (key, expected) in [
        ...
        if config.get(key).and_then(Value::as_u64) != Some(expected) {
            return Err(bos_error(format!(
                "BOS layer inference config {key:?} differs from the pinned geometry"
            )));
        }
    }
    ...
    {
        return Err(bos_error(
            "BOS layer inference config does not retain the native FP8/E8M0 grammar",
        ));
    }
    let ratios = config
        .get("compress_ratios")
        .and_then(Value::as_array)
        .ok_or_else(|| bos_error("BOS layer inference config lacks compress_ratios"))?;
    // Source lists 44 schedule entries (43 base + 1 MTP). Admit either length.
    if !(ratios.len() == 43 || ratios.len() == 44)
        || ratios.get(0).and_then(Value::as_u64) != Some(0)
        || ratios.get(1).and_then(Value::as_u64) != Some(0)
    {
        return Err(bos_error(
            "base layers 0 and 1 must remain ratio-zero in the source compress_ratios schedule",
        ));
    }
    let expected = expected_bos_compress_ratio(layer) as u64;
    if ratios.get(layer).and_then(Value::as_u64) != Some(expected) {
        return Err(bos_error(format!(
            "layer {layer} compression ratio disagrees with the pinned BOS schedule (expected {expected})"
        )));
    }

    let model_config: Value =
        serde_json::from_slice(&reader.read_verified_metadata_asset("config.json", 64 * 1024)?)
            .map_err(|error| bos_error(format!("BOS layer model config JSON: {error}")))?;
    for (key, expected) in [("rms_norm_eps", RMS_NORM_EPS), ("hc_eps", HC_EPS)] {
        ...
        if actual.is_none_or(|value| (value as f32).to_bits() != expected.to_bits()) {
            return Err(bos_error(format!(
                "BOS layer model config {key:?} differs from the pinned F32 control"
            )));
        }
    }
    ...
    for (asset, needle) in [
        ...
        if !contains_bytes(asset, needle) {
            return Err(bos_error("pinned BOS layer source grammar anchor is absent"));
        }
    }
    Ok(())
}
...
fn read_verified_tensor(
    ...
    if !name.starts_with("layers.") {
        return Err(bos_error(format!(
            "BOS layer static-control loader refuses non-layer tensor {name:?}"
        )));
    }
    ...
    {
        return Err(bos_error(format!(
            "{name} differs from the required BOS layer source-native geometry"
        )));
    }
...
fn read_fp8_pair(
    ...
    {
        return Err(bos_error(format!(
            "BOS layer native FP8/E8M0 pair geometry differs for {weight_name}"
        )));
    }
    Ok(Fp8Pair {
        weight: read_verified_tensor(
            ...
            rows.checked_mul(cols)
                .ok_or_else(|| bos_error("BOS layer FP8 weight byte geometry overflow"))?,
        )?,
        scale: read_verified_tensor(
            ...
            scale_rows
                ...
                .ok_or_else(|| bos_error("BOS layer FP8 scale byte geometry overflow"))?,
        )?,
    })
}
...
fn validate_required_pipelines(metal: &MetalContext) -> Result<()> {
    for (kernel, threads) in [
        ...
        if maximum < threads {
            return Err(bos_error(format!(
                "{kernel} supports only {maximum} threads, below BOS layer required {threads}",
            )));
        }
    }
    Ok(())
...
fn dispatch_fp8(
    ...
    if rows == 0 || cols == 0 || scale_cols != cols / ACT_QUANT_BLOCK as u32 {
        return Err(bos_error("BOS layer FP8 matvec source geometry is invalid"));
    }
...
fn dispatch_kv_qat(
    ...
    {
        return Err(bos_error("BOS layer ratio-zero KV QAT geometry is invalid"));
    }
    batch.dispatch_threads(
        KV_QAT_KERNEL,
...
fn dispatch_sparse_growing_kv(
    ...
    {
        return Err(bos_error(
            "BOS layer ratio-zero growing-KV sparse-attention geometry is invalid",
        ));
    }
...
fn bos_error(message: impl Into<String>) -> Error {
    Error::Gravity(format!(
        "DeepSeek-V4 BOS layer attention device primitive: {}",
        message.into()
    ))
}

/// Source compress schedule for base layers 0..42 (MTP excluded).
pub fn expected_bos_compress_ratio(layer: usize) -> usize {
    match layer {
        0 | 1 => 0,
...
mod tests {
    ...
    #[test]
    fn bos_geometry_and_topology_are_explicitly_bounded() {
        assert_eq!(DSV4F_BOS_LAYER_HC_STATE_BF16_BYTES, 4 * 4096 * 2);
        assert_eq!(DSV4F_BOS_LAYER_KV_SLOT0_BF16_BYTES, 512 * 2);
        assert_eq!(DSV4F_BOS_LAYER_ATTENTION_DISPATCHES, 22);
        assert_eq!(expected_bos_compress_ratio(1), 0);
        assert_eq!(expected_bos_compress_ratio(2), 4);
        assert_eq!(expected_bos_compress_ratio(3), 128);
    }
}
