...
impl PrefillKey {
    ...
    pub fn from_model_and_prompt(model_id: &str, tokenizer_signature: &[u8], prompt_tokens: &[u32]) -> Self {
        ...
        Self { model_hash, tokenizer_hash, prefix_hash, prompt_tokens: prompt_tokens.to_vec() }
    }
    ...
    pub fn rolling_prefix_hash(model_hash: &[u8; 32], tokenizer_hash: &[u8; 32], tokens: &[u32]) -> [u8; 32] {
...
impl PrefillDiskCache {
    pub fn open<P: AsRef<Path>>(root: P) -> Result<Self> {
        ...
        Ok(Self { root: root.as_ref().to_owned(), budget_bytes: None, index: Mutex::new(index) })
    }

    /// Open from `HAWKING_PREFIX_CACHE_DIR` if set. Returns `Ok(None)`
    /// if the env var is unset (which signals "feature disabled").
    pub fn open_from_env() -> Result<Option<Self>> {
        let Ok(dir) = std::env::var("HAWKING_PREFIX_CACHE_DIR") else {
            return Ok(None);
        };
        if dir.is_empty() {
            return Ok(None);
        ...
        if let Ok(bytes_str) = std::env::var("HAWKING_PREFIX_CACHE_BUDGET_MB") {
    ...
    pub fn lookup_longest_prefix(&self, model_hash: &[u8; 32], tokenizer_hash: &[u8; 32], prompt_tokens: &[u32]) -> Result<Option<PrefillHit>> {
        ...
        let path = self.root.join(hex32(model_hash)).join(format!("{}.kv", hex32(&prefix_hash)));
        ...
        let expected_body = meta.n_layers as usize * 2 * n_tokens * meta.n_kv_heads as usize * meta.head_dim as usize * 4;
        if mmap.len() < body_offset + expected_body {
            self.forget(model_hash, &prefix_hash);
            return Ok(None);
        }
        Ok(Some(PrefillHit { n_tokens, n_layers: meta.n_layers as usize, n_kv_heads: meta.n_kv_heads as usize, head_dim: meta.head_dim as usize, mmap, body_offset, tokens_offset }))
    }
    ...
    pub fn lookup(&self, key: &PrefillKey) -> Result<Option<Mmap>> {
        ...
        if &mmap[12..44] != key.model_hash || &mmap[44..76] != key.tokenizer_hash || &mmap[76..108] != key.prefix_hash {
            return Ok(None);
        }
        Ok(Some(mmap))
    }
    ...
    pub fn store(&self, key: &PrefillKey, kv: &KvCache) -> Result<()> {
        if kv.seq_len < key.prompt_tokens.len() {
            return Err(crate::Error::Model(format!("prefill_disk store: kv.seq_len={} < key.prompt_tokens.len()={}", kv.seq_len, key.prompt_tokens.len())));
        }
        ...
        self.store_raw(key, kv.n_layers, kv.n_kv_heads, kv.head_dim, &keys_refs, &vals_refs)
    }
    ...
    pub fn store_raw(&self, key: &PrefillKey, n_layers: usize, n_kv_heads: usize, head_dim: usize, keys_per_layer: &[&[f32]], values_per_layer: &[&[f32]]) -> Result<()> {
        let n_tokens = key.prompt_tokens.len();
        let want = n_tokens * n_kv_heads * head_dim;
        if keys_per_layer.len() != n_layers || values_per_layer.len() != n_layers {
            return Err(crate::Error::Model(format!("store_raw: layer mismatch (got K={}, V={}, want {})", keys_per_layer.len(), values_per_layer.len(), n_layers)));
        }
        for (li, (k, v)) in keys_per_layer.iter().zip(values_per_layer.iter()).enumerate() {
            if k.len() < want || v.len() < want {
                return Err(crate::Error::Model(format!("store_raw: layer {} too short (k={}, v={}, want {})", li, k.len(), v.len(), want)));
            }
        }
        let path = key.path(&self.root);
        ...
        std::fs::rename(&tmp_path, &path)?;
        {
            ...
            bucket.insert(key.prefix_hash, EntryMeta { n_tokens, n_layers: n_layers as u32, n_kv_heads: n_kv_heads as u32, head_dim: head_dim as u32 });
        }
    ...
    fn evict_if_over_budget(&self) {
        ...
        for (path, _, len) in entries {
            ...
            let prefix_hash_opt = path.file_stem().and_then(|s| s.to_str()).and_then(parse_hex32);
            let model_hash_opt = path.parent().and_then(|p| p.file_name()).and_then(|s| s.to_str()).and_then(parse_hex32);
    ...
    pub fn len_for_model(&self, model_hash: &[u8; 32]) -> usize {
        self.index.lock().unwrap().get(model_hash).map(|b| b.len()).unwrap_or(0)
    }
}
...
pub fn restore_hit_into_kv(hit: &PrefillHit, kv: &mut KvCache) -> Result<()> {
    ...
    if hit.n_tokens > kv.max_seq {
        return Err(crate::Error::Model(format!("restore_hit_into_kv: hit n_tokens {} > kv.max_seq {}", hit.n_tokens, kv.max_seq)));
    }
    let stride = kv.n_kv_heads * kv.head_dim;
...
fn read_header_meta(path: &Path) -> Option<EntryMeta> {
    ...
    Some(EntryMeta { n_tokens, n_layers, n_kv_heads, head_dim })
}
...
fn f32_slice_from_bytes(src: &[u8]) -> &[f32] {
    ...
    assert!(src.len() % 4 == 0);
    debug_assert_eq!((src.as_ptr() as usize) % std::mem::align_of::<f32>(), 0, "f32 slice misaligned");
...
mod tests {
    ...
    fn round_trip_basic() {
        ...
        let hit = cache.lookup_longest_prefix(&key.model_hash, &key.tokenizer_hash, &[1, 2, 3, 4, 5]).unwrap().expect("expected hit on prefix");
    ...
    fn restore_round_trip() {
        ...
        cache.store(&key, &kv).unwrap();
        let hit = cache.lookup_longest_prefix(&key.model_hash, &key.tokenizer_hash, &[1, 2, 3, 4, 9]).unwrap().unwrap();
    ...
    fn longest_prefix_picks_longest() {
        ...
        let hit = cache.lookup_longest_prefix(&key_a.model_hash, &key_a.tokenizer_hash, &[10, 11, 12, 13, 14]).unwrap().unwrap();
        assert_eq!(hit.n_tokens, 4);
    }
    ...
    fn lookup_does_not_return_full_prompt() {
        ...
        cache.store(&key, &kv).unwrap();
        assert!(cache.lookup_longest_prefix(&key.model_hash, &key.tokenizer_hash, &[1, 2, 3]).unwrap().is_none());
        let hit = cache.lookup_longest_prefix(&key.model_hash, &key.tokenizer_hash, &[1, 2, 3, 99]).unwrap().unwrap();
    ...
    fn tokenizer_change_invalidates() {
        ...
        let hit = cache.lookup_longest_prefix(&key_v2.model_hash, &key_v2.tokenizer_hash, &[5, 6, 7]).unwrap();
    ...
    fn lru_eviction_respects_budget() {
        ...
        let cache = PrefillDiskCache::open(tmp.path()).unwrap().with_budget_bytes(900);
        for i in 0..5u32 {
        ...
        assert!(total <= 900, "expected ≤900 bytes on disk after eviction, got {}", total);
        assert!(total > 0);
    }
    ...
    fn index_repopulates_on_reopen() {
        ...
        let hit = cache.lookup_longest_prefix(&key.model_hash, &key.tokenizer_hash, &[1, 2, 3]).unwrap();
        assert!(hit.is_some());
    }
}
