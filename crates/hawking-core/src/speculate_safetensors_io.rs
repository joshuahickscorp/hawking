...
impl SafeTensors {
    ...
    pub fn open(path: &Path) -> Result<Self> {
        ...
        if mmap.len() < 8 {
            return Err(Error::Model(format!("safetensors: file too short ({} bytes) at {}", mmap.len(), path.display())));
        }
        let hlen = u64::from_le_bytes(mmap[..8].try_into().unwrap()) as usize;
        if 8 + hlen > mmap.len() {
            return Err(Error::Model(format!("safetensors: header_len {hlen} overruns file len {} at {}", mmap.len(), path.display())));
        }
        let header_bytes = &mmap[8..8 + hlen];
        let header: Value = serde_json::from_slice(header_bytes).map_err(|e| Error::Model(format!("safetensors: header JSON parse failed at {}: {e}", path.display())))?;
        let header_obj = header.as_object().ok_or_else(|| Error::Model(format!("safetensors: header is not a JSON object at {}", path.display())))?;
        ...
        for (k, v) in header_obj.iter() {
            ...
            let obj = v.as_object().ok_or_else(|| Error::Model(format!("safetensors: entry '{k}' is not an object")))?;
            let dtype = obj.get("dtype").and_then(|x| x.as_str()).ok_or_else(|| Error::Model(format!("safetensors: entry '{k}' missing dtype")))?.to_string();
            let shape = obj
                .get("shape")
                .and_then(|x| x.as_array())
                .ok_or_else(|| Error::Model(format!("safetensors: entry '{k}' missing shape")))?
                .iter()
                .map(|d| d.as_u64().map(|u| u as usize).ok_or_else(|| Error::Model(format!("safetensors: entry '{k}' shape dim not u64"))))
                .collect::<Result<Vec<usize>>>()?;
            let offsets = obj.get("data_offsets").and_then(|x| x.as_array()).ok_or_else(|| Error::Model(format!("safetensors: entry '{k}' missing data_offsets")))?;
            if offsets.len() != 2 {
                return Err(Error::Model(format!("safetensors: entry '{k}' data_offsets len != 2")));
            }
            let a = offsets[0].as_u64().ok_or_else(|| Error::Model(format!("safetensors: '{k}' offset[0] not u64")))? as usize;
            let b = offsets[1].as_u64().ok_or_else(|| Error::Model(format!("safetensors: '{k}' offset[1] not u64")))? as usize;
            entries.insert(k.clone(), Entry { dtype, shape, offset_start: a, offset_end: b });
        }

        Ok(Self { mmap, data_start: 8 + hlen, entries, metadata })
    }
    ...
    pub fn shape(&self, name: &str) -> Result<&[usize]> {
        self.entries.get(name).map(|e| e.shape.as_slice()).ok_or_else(|| Error::Model(format!("safetensors: missing tensor '{name}'")))
    }
    ...
    pub fn read_f16(&self, name: &str, expected_shape: &[usize]) -> Result<Vec<f16>> {
        ...
        Ok(raw.chunks_exact(2).map(|b| f16::from_le_bytes([b[0], b[1]])).collect())
    }

    fn slice(&self, name: &str, expected_dtype: &str, expected_shape: &[usize], bytes_per_elem: usize) -> Result<&[u8]> {
        let e = self.entries.get(name).ok_or_else(|| Error::Model(format!("safetensors: missing tensor '{name}'")))?;
        if e.dtype != expected_dtype {
            return Err(Error::Model(format!("safetensors: '{name}' dtype is {} expected {expected_dtype}", e.dtype)));
        }
        if e.shape != expected_shape {
            return Err(Error::Model(format!("safetensors: '{name}' shape is {:?} expected {:?}", e.shape, expected_shape)));
        }
        ...
        if span != expected_bytes {
            return Err(Error::Model(format!("safetensors: '{name}' byte span {span} != expected {expected_bytes}")));
        }
        let abs_start = self.data_start + e.offset_start;
        let abs_end = self.data_start + e.offset_end;
        if abs_end > self.mmap.len() {
            return Err(Error::Model(format!("safetensors: '{name}' overruns file (end={abs_end} len={})", self.mmap.len())));
        }
