...
use crate::kernels::{add_inplace, embed_lookup, gelu_mul, logit_softcap_inplace, rope_inplace};
...
impl Gemma2Config {
    pub fn from_gguf(g: &GgufFile) -> Result<Self> {
        ...
        Ok(Self { n_layers, hidden, n_heads, n_kv_heads, head_dim, intermediate, vocab_size, rope_theta, rms_norm_eps, max_seq_len, attn_scale, attn_logit_softcap, final_logit_softcap, embed_scale })
    }
}
...
impl Gemma2 {
    ...
    pub(crate) fn forward_token(&mut self, token: u32, pos: usize) -> Result<Vec<f32>> {
        ...
        for li in 0..n_layers {
            ...
            mha_decode_step_gemma(&q_full, keys, values, n_heads, n_kv_heads, head_dim, mha_seq_len, attn_scale, attn_softcap, &mut attn_out)?;
    ...
    fn load(weights: &Path, config: EngineConfig) -> Result<Self> {
        ...
        for li in 0..cfg.n_layers {
            ...
            layers.push(Gemma2Layer {
                ...
                post_attention_norm: Self::dequant_norm_plus_one(&gguf, &lp("post_attention_norm.weight"))?,
        ...
        Ok(Self { config: cfg, tokenizer, model_id, gguf, embed, final_norm, layers, kv, sampler, kernel_profile, _weights_path: weights.to_path_buf(), metal_ctx })
    }
