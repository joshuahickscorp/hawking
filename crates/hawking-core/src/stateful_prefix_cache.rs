...
//!
//! `QwenDense::generate` (`crates/hawking-core/src/model_qwen_dense.rs`
...
impl PrefixKey {
    ...
    pub fn from_model_and_prompt(model_id: &str, tokenizer_signature: &[u8], prompt_tokens: &[u32]) -> Self {
        ...
        Self { model_hash: dk.model_hash, tokenizer_hash: dk.tokenizer_hash, prefix_hash: dk.prefix_hash, n_tokens: prompt_tokens.len() }
    }
    ...
    pub fn to_prefill_key(&self, prompt_tokens: &[u32]) -> PrefillKey {
        debug_assert_eq!(prompt_tokens.len(), self.n_tokens, "to_prefill_key: prompt_tokens length must match the key's n_tokens");
        PrefillKey { model_hash: self.model_hash, tokenizer_hash: self.tokenizer_hash, prefix_hash: self.prefix_hash, prompt_tokens: prompt_tokens.to_vec() }
    }
}
...
impl Default for PrefixCacheBudget {
    fn default() -> Self {
        Self { max_bytes: Some(DEFAULT_MAX_BYTES), max_entries: None }
    }
}
...
impl InMemoryPrefixCache {
    ...
    pub fn new() -> Self {
        Self { budget: PrefixCacheBudget::default(), entries: HashMap::new(), clock: 0, retained_bytes: 0, stats: PrefixCacheStats::default() }
    }
    ...
    pub fn insert_from_kv(&mut self, key: PrefixKey, kv: &KvCache) -> InsertOutcome {
        debug_assert_eq!(key.n_tokens, kv.seq_len, "insert_from_kv: key.n_tokens must equal kv.seq_len");
        let n_tokens = kv.seq_len;
        let stride = kv.n_kv_heads * kv.head_dim;
        let want = n_tokens * stride;
        let keys: Vec<Vec<f32>> = (0..kv.n_layers).map(|li| kv.keys[li][..want].to_vec()).collect();
        let values: Vec<Vec<f32>> = (0..kv.n_layers).map(|li| kv.values[li][..want].to_vec()).collect();
        let blocks = KvBlockRange { n_tokens, n_layers: kv.n_layers, n_kv_heads: kv.n_kv_heads, head_dim: kv.head_dim };
    ...
    pub fn restore_into(&self, key: &PrefixKey, query_tokens: &[u32], matched_len: usize, kv: &mut KvCache) -> crate::Result<usize> {
        let prefix_hash = rolling_prefix_hash_of(&key.model_hash, &key.tokenizer_hash, &query_tokens[..matched_len]);
        let entry = self.entries.get(&key.model_hash).and_then(|b| b.get(&prefix_hash)).ok_or_else(|| crate::Error::Model("prefix_cache restore_into: no entry for match".into()))?;
        if entry.n_layers != kv.n_layers || entry.n_kv_heads != kv.n_kv_heads || entry.head_dim != kv.head_dim {
            return Err(crate::Error::Model(format!(
                "prefix_cache restore_into: shape mismatch (entry={}x{}x{}, kv={}x{}x{})",
                entry.n_layers, entry.n_kv_heads, entry.head_dim, kv.n_layers, kv.n_kv_heads, kv.head_dim,
            )));
        }
        if entry.n_tokens() > kv.max_seq {
            return Err(crate::Error::Model(format!("prefix_cache restore_into: entry n_tokens {} > kv.max_seq {}", entry.n_tokens(), kv.max_seq)));
        }
        let stride = kv.n_kv_heads * kv.head_dim;
    ...
    fn insert_inner(&mut self, key: PrefixKey, blocks: KvBlockRange, keys: Vec<Vec<f32>>, values: Vec<Vec<f32>>) -> InsertOutcome {
        self.clock += 1;
        let tick = self.clock;
        ...
        let entry = CacheEntry { tokens: vec![0u32; key.n_tokens], n_layers: blocks.n_layers, n_kv_heads: blocks.n_kv_heads, head_dim: blocks.head_dim, keys, values, last_tick: tick };
        ...
        let still_present = self.entries.get(&key.model_hash).map(|b| b.contains_key(&key.prefix_hash)).unwrap_or(false);
        if !still_present {
            return InsertOutcome::RejectedOverBudget;
        }
...
fn rolling_prefix_hash_of(model_hash: &[u8; 32], tokenizer_hash: &[u8; 32], tokens: &[u32]) -> [u8; 32] {
...
impl PrefixCache for InMemoryPrefixCache {
    fn lookup(&self, key: &PrefixKey, query_tokens: &[u32]) -> Option<PrefixMatch> {
        ...
        Some(PrefixMatch { matched_len, blocks: KvBlockRange { n_tokens: entry.n_tokens(), n_layers: entry.n_layers, n_kv_heads: entry.n_kv_heads, head_dim: entry.head_dim } })
    }
    ...
    fn evict_to(&mut self, budget: PrefixCacheBudget) {
        ...
        loop {
            let over_bytes = budget.max_bytes.map(|max| self.retained_bytes > max).unwrap_or(false);
            let over_entries = budget.max_entries.map(|max| self.total_entries() > max).unwrap_or(false);
...
impl InMemoryPrefixCache {
    ...
    /// populated.
    pub fn lookup_counting(&mut self, key: &PrefixKey, query_tokens: &[u32]) -> Option<PrefixMatch> {
        ...
        if let Some(ref hit) = m {
            ...
            self.clock += 1;
            let tick = self.clock;
            let prefix_hash = rolling_prefix_hash_of(&key.model_hash, &key.tokenizer_hash, &query_tokens[..hit.matched_len]);
...
mod tests {
    ...
    fn key_is_byte_compatible_with_disk_tier() {
        ...
        assert_eq!(ram.prefix_hash, disk.prefix_hash, "rolling hash must match disk tier");
    ...
    fn lookup_never_returns_full_prompt() {
        ...
        assert!(c.lookup(&key, &p).is_none(), "must not match the whole prompt");
    ...
    fn cold_vs_warm_prefill_byte_identical() {
        ...
        c.insert_from_kv(PrefixKey::from_model_and_prompt("m", b"v1", &system), &kv_t1);
        ...
        let restored = c.restore_into(&key2, &turn2, m.matched_len, &mut kv_warm).unwrap();
    ...
    fn tokenizer_change_invalidates() {
        ...
        assert!(c.lookup(&key_v2, &q).is_none(), "tokenizer change must invalidate");
    }

    #[test]
    fn lru_eviction_respects_entry_budget() {
        let mut c = InMemoryPrefixCache::with_budget(PrefixCacheBudget { max_bytes: None, max_entries: Some(2) });
        for i in 0..5u32 {
    ...
    fn lru_eviction_respects_byte_budget() {
        ...
        let mut c = InMemoryPrefixCache::with_budget(PrefixCacheBudget { max_bytes: Some(200), max_entries: None });
        for i in 0..6u32 {
            let p: Vec<u32> = vec![i, i + 1, i + 2];
            c.insert_from_kv(PrefixKey::from_model_and_prompt("m", b"t", &p), &cold_prefill(&p, 1, 1, 4));
        }
    ...
    fn insert_past_byte_cap_evicts_oldest_keeps_newest() {
        ...
        // must evict the 1st.
        let cap = 250u64;
        let mut c = InMemoryPrefixCache::with_budget(PrefixCacheBudget { max_bytes: Some(cap), max_entries: None });
        ...
        assert!(c.stats().retained_bytes <= cap, "retained bytes {} must stay ≤ cap {cap}", c.stats().retained_bytes);
        ...
        let mut q0 = p0.clone();
        q0.push(99);
        assert!(c.lookup(&PrefixKey::from_model_and_prompt("m", b"t", &q0), &q0).is_none(), "oldest entry must have been evicted");
        let mut q2 = p2.clone();
        q2.push(99);
        let m2 = c.lookup(&PrefixKey::from_model_and_prompt("m", b"t", &q2), &q2).expect("newest entry must survive");
