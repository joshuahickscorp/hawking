...
use crate::kernels::{add_inplace, embed_lookup, rope_inplace_scaled, silu_mul, Llama3RopeScaling};
...
impl LlamaConfig {
    pub fn from_gguf(g: &GgufFile) -> Result<Self> {
        ...
        let rope_scaling = if get_str("llama.rope.scaling.type") == Some("llama3") {
            ...
            match (factor, low, high, orig) {
                (Some(factor), Some(low_freq_factor), Some(high_freq_factor), Some(orig_ctx)) => {
                    Some(Llama3RopeScaling { factor, low_freq_factor, high_freq_factor, original_max_position_embeddings: orig_ctx })
                }
                _ => None,
            }
        } else {
        ...
        Ok(Self { n_layers, hidden, n_heads, n_kv_heads, head_dim, intermediate, vocab_size, rope_theta, rms_norm_eps, max_seq_len, rope_scaling, arch })
    }
}
    ...
    pub(crate) fn forward_token(&mut self, token: u32, pos: usize) -> Result<Vec<f32>> {
        ...
        for li in 0..n_layers {
            ...
            for h_i in 0..n_heads {
                ...
                rope_inplace_scaled(&mut q_full[off..off + head_dim], pos as u32, rope_theta, rope_scaling);
            }
            for h_i in 0..n_kv_heads {
                let off = h_i * head_dim;
                rope_inplace_scaled(&mut k_token[off..off + head_dim], pos as u32, rope_theta, rope_scaling);
            }
...
impl Engine for LlamaDense {
    fn load(weights: &Path, config: EngineConfig) -> Result<Self> {
        ...
        Ok(Self { config: cfg, tokenizer, model_id, gguf, embed, final_norm, lm_head, layers, kv, sampler, kernel_profile, _weights_path: weights.to_path_buf(), metal_ctx })
    }
...
mod tests {
    ...
    fn rope_scaling_params_round_trip_into_struct() {
        let s = Llama3RopeScaling { factor: 32.0, low_freq_factor: 1.0, high_freq_factor: 4.0, original_max_position_embeddings: 8192 };
