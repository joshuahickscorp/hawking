            ...
            for (pos, &tid) in token_ids.iter().enumerate() {
                ...
                for (li, &layer) in layers_run.iter().enumerate() {
                    ...
                    let mut sites = json!({
                        ...
                        "attn_compute_encoders": attn_out.timing.compute_encoders,
                        "attn_compute_dispatches": attn_out.timing.compute_dispatches,
                        "attn_ordered_encoder": attn_out.ordered_encoder,
