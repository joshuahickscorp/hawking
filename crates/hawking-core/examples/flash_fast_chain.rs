...
mod macos {
    ...
    fn main_impl() -> Result<(), Box<dyn Error>> {
        ...
        let start: usize = arg_value(&argv, "--start-layer")
            .unwrap_or_else(|| "0".into())
            .parse()?;
        let end: usize = arg_value(&argv, "--end-layer")
            .unwrap_or_else(|| "47".into())
            .parse()?;
        if start > end || end >= 48 {
            return Err("expected 0 <= start <= end < 48".into());
        }
        let out_root = PathBuf::from(arg_value(&argv, "--out-dir").unwrap_or_else(|| {
            repo_root()
                .join("receipts/headless/flash_fast_chain")
                .display()
                .to_string()
        }));
        ...
        let mut previous_state: Option<PathBuf> =
            arg_value(&argv, "--base-state").map(PathBuf::from);
        ...
        while layer <= end {
            if FULL_LAYERS.contains(&layer) {
                ...
                let full_args = full::Args {
                    ...
                    compact_experts,
                    fused_route_accumulate,
                    fused_attention_gate,
                };
                if device_resident {
                    let prior_device_state = previous_device_state.take();
                    previous_device_state =
                        full::run_layer_device_input(full_args, prior_device_state.as_ref())?;
                } else {
                    full::run_layer(full_args)?;
                }
                if !state.is_file() {
                    return Err(format!("layer {layer} did not emit state").into());
                }
            ...
            for candidate in FULL_LAYERS {
                if candidate > layer {
                    run_end = run_end.min(candidate - 1);
                    break;
                }
            }
            ...
            if device_resident {
                let prior_device_state = previous_device_state.take();
                previous_device_state =
                    linear::run_layer_device_output(linear_args, prior_device_state.as_ref())?;
            } else {
                linear::run_layer(linear_args)?;
            }
            if !state.is_file() {
                return Err(format!("group {layer}..{run_end} did not emit state").into());
            }
            let group_doc: Value = serde_json::from_slice(&fs::read(&receipt)?)?;
            let top_level_passed = group_doc
                .get("status")
                .and_then(Value::as_str)
                .map(|status| status == "PASSED" || status.starts_with("PASSED_"))
                .unwrap_or(false);
            let rows_passed = group_doc
                .get("layers")
            ...
            if !passed {
                return Err(format!("linear group {layer}..{run_end} failed parity").into());
            }
        ...
        let terminal_receipt = if end == 47 {
            let state = previous_state
                .clone()
                .ok_or("terminal requested without final state")?;
        ...
        fs::write(
            out_root.join("FAST_CHAIN_SUMMARY.json"),
