...
mod macos {
    ...
    const ROUTED_DOWN_KERNEL_NAME: &str = "qwen_uniform_q4_group64_routed_down_geo_tpr64_tg128";
    ...
    fn parse_args() -> Result<Args, Box<dyn Error>> {
        ...
        if args.out == default_out {
            if args.source_layer_bridge.is_some() {
                ...
                args.out =
                    repo.join("receipts/headless/FLASH_NOETIC_ROUTED_GATE_UP_FUSED_NATIVE.json");
            } else if args.fused_routed_down {
                args.out =
                    repo.join("receipts/headless/FLASH_NOETIC_ROUTED_DOWN_FUSED_NATIVE.json");
            } else if args.fused_routed_moe {
                args.out = repo.join("receipts/headless/FLASH_NOETIC_ROUTED_MOE_FUSED_NATIVE.json");
    ...
    fn read_bridge_f32(
        ...
        if bytes.len() != expected_bytes || sha256_bytes(&bytes) != expected_sha256 {
            return Err(
                format!("source MoE bridge {name} payload does not match its receipt").into(),
            );
        }
        let values = bytes
    ...
    fn validate_source_moe_bridge(path: &Path) -> Result<SourceMoeBridge, Box<dyn Error>> {
        ...
        {
            return Err(
                "source MoE bridge linked receipt does not declare passed source parity".into(),
            );
        }
        let source_receipt_path =
            referenced_path(&canonical_path, source_receipt_ref, "source_layer_receipt")?;
        ...
        let mlp_input = read_bridge_f32(
            ...
            receipt
                .get("mlp_input")
                .ok_or("source MoE bridge has no MLP input")?,
            "mlp_input",
            HYPER_STATE_HIDDEN,
        )?;
        ...
        let (route_ids, route_weights) =
            parse_router_selection(route_selection, "source MoE bridge")?;
        if route_ids.len() != ROUTED_TOP_K
            || (route_weights.iter().copied().sum::<f32>() - 1.0).abs() > 2.0e-3
        {
            return Err(
                "source MoE bridge route selection is not a normalized top-k control".into(),
            );
        }
        ...
        if source_route_ids != route_ids {
            return Err(
                "source MoE bridge route IDs disagree with its linked exact source receipt".into(),
            );
        }
        Ok(SourceMoeBridge {
            path: canonical_path,
    ...
    fn weighted_routed_sum(
        ...
        for (route, weight) in route_weights.iter().enumerate() {
            ...
            for (destination, source) in sum
                .iter_mut()
                .zip(&route_outputs[start..start + rows_per_route])
            {
    ...
    fn parse_router_selection(
        ...
        {
            return Err(format!(
                "router receipt {selection_name} shape is outside bounded dispatch limits"
            )
            .into());
        }
        Ok((ids, weights))
    ...
    fn resolve_fused_route_selection(
        ...
        {
            return Err(
                "router receipt does not bind source-native selected ids to its reference".into(),
            );
        }
    ...
    fn run_fused_routed_gate_up(args: &Args) -> Result<Value, Box<dyn Error>> {
        ...
        let fused_output =
            context.new_buffer_checked(expected.len() * std::mem::size_of::<f32>())?;
    ...
    fn run_fused_routed_down(args: &Args) -> Result<Value, Box<dyn Error>> {
        ...
        if routed_specs
            .iter()
            .any(|spec| spec.down.rows != rows_per_route || spec.down.columns != columns)
        {
        ...
        let route_inputs_buffer =
            context.new_buffer_with_bytes_checked(&f32_bytes(&route_inputs))?;
        let fused_output =
            context.new_buffer_checked(expected.len() * std::mem::size_of::<f32>())?;
        ...
        for (route, spec) in routed_specs.iter().enumerate() {
            ...
            individual_outputs
                .push(context.new_buffer_checked(rows_per_route * std::mem::size_of::<f32>())?);
        }
    ...
    fn run_fused_routed_moe(args: &Args) -> Result<Value, Box<dyn Error>> {
        ...
        let source_bridge = args
            ...
            .transpose()?;
        let (selected_ids, selected_weights, route_authority) = if let Some(bridge) =
            source_bridge.as_ref()
        {
            (
        ...
        if gate_rows != down_columns {
            return Err(
                "routed gate/up activation width does not match routed down columns".into(),
            );
        }
        ...
        let (input, input_authority) = if let Some(bridge) = source_bridge.as_ref() {
            if bridge.mlp_input.len() != input_columns {
                return Err(
                    "source MoE bridge MLP activation does not match Q4 body columns".into(),
                );
            }
            (
        ...
        for spec in &routed_specs {
            ...
            individual_gate_scales.push(context.new_buffer_with_bytes_checked(&spec.gate.scales)?);
            ...
            individual_down_scales.push(context.new_buffer_with_bytes_checked(&spec.down.scales)?);
            individual_gate_up_outputs
                .push(context.new_buffer_checked(gate_rows * std::mem::size_of::<f32>())?);
            individual_down_outputs
                .push(context.new_buffer_checked(hidden * std::mem::size_of::<f32>())?);
        }
        ...
        let source_bridge_dependency = source_bridge
            .as_ref()
            .map(|bridge| {
                component_ref(&bridge.path, &bridge.receipt, Some(&bridge.receipt_sha256))
            })
            .unwrap_or(Value::Null);
