//! Persistent two-token, three-layer Flash linear-prefix session.

#[cfg(not(target_os = "macos"))]
fn main() -> Result<(), Box<dyn std::error::Error>> {
    Err(std::io::Error::other("Flash prefix session requires macOS Metal").into())
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let out = PathBuf::from(value("--out").unwrap_or_else(|| {
        "receipts/headless/FLASH_STATEFUL_LINEAR_PREFIX_SESSION.json".to_owned()
    }));
    let token_ids = value("--tokens").unwrap_or_else(|| "248044,248044".to_owned())
        .split(',').map(str::parse::<usize>).collect::<Result<Vec<_>, _>>()?;
    linear::run_stateful_linear_prefix_session_mode(
        root,
        &token_ids,
        out,
        argv.iter().any(|arg| arg == "--dense"),
        value("--union-from").map(PathBuf::from),
    )
}
