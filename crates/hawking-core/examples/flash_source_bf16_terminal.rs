...
//!
//! `TerminalExecutor` consumes the exact 48-layer final HyperConnection state
//! produced by the admitted session, performs the model-level HyperConnection
//! readout, and executes the source-BF16 lm_head and greedy argmax on Metal.
//! The standalone executable is deliberately fail-closed: it has no session
//! admission or protected-lane preflight and must not create a second native
//! launch surface.
...
mod macos {
    ...
    const STANDALONE_TERMINAL_DISABLED: &str =
        "standalone flash_source_bf16_terminal execution is disabled: it has no native-session admission or protected-lane preflight; use the admitted flash_stateful_complete_token_session path";

    /// This file is embedded by the admitted stateful session for
    /// `TerminalExecutor`. Its standalone executable formerly exposed a
    /// second, unadmitted source/Metal launch surface, so it now fails closed
    /// instead of attempting to replicate session admission here.
    fn reject_standalone_terminal_execution() -> Result<(), String> {
        Err(STANDALONE_TERMINAL_DISABLED.to_owned())
    }
    ...
    /// Terminal receipts live inside the session artifact namespace reserved
    /// by the caller.  `create_new` claims the final path atomically, so a
    /// competing writer can never replace a prior/racing terminal artifact.
    fn write_new_terminal_receipt(path: &Path, contents: &str) -> Result<(), String> {
        let mut receipt = fs::OpenOptions::new()
            ...
            .map_err(|error| {
                if error.kind() == std::io::ErrorKind::AlreadyExists {
                    format!(
                        "refusing to overwrite existing terminal receipt {}",
                        path.display()
                    )
                } else {
                    format!(
                        "cannot exclusively create terminal receipt {}: {error}",
                        path.display()
                    )
                }
            })?;
        receipt
            ...
            .map_err(|error| {
                format!(
                    "terminal receipt {} was exclusively created but could not be fully written: {error}",
                    path.display()
                )
            })
    }
    ...
    impl TerminalExecutor {
        ...
        pub(crate) fn run_state(
            ...
            if let Some(path) = out_path {
                write_new_terminal_receipt(&path, &format!("{serialized}\n"))?;
            }
            Ok((token, receipt))
        }
    ...
    /// There is intentionally no standalone terminal executor. Its caller
    /// must be the admitted complete-session path, which owns source admission,
    /// lane observation, receipt reservation, and the long-lived executor.
    pub(crate) fn run_with(
        _root: PathBuf,
        _state_path: PathBuf,
        _out_path: Option<PathBuf>,
    ) -> Result<(), String> {
        reject_standalone_terminal_execution()
    }
    ...
    mod tests {
        ...
        #[test]
        fn terminal_receipt_writer_preserves_existing_file() {
            let temp = tempfile::tempdir().expect("temporary directory");
            let path = temp.path().join("attempt.terminal.json");
            fs::write(&path, b"immutable-prior-receipt\n").expect("seed prior receipt");

            let error = write_new_terminal_receipt(&path, "{\"new\":true}\n")
                .expect_err("existing receipt must not be replaced");

            assert!(error.contains("refusing to overwrite existing terminal receipt"));
            assert_eq!(
                fs::read(&path).expect("read preserved receipt"),
                b"immutable-prior-receipt\n"
            );
        }

        #[test]
        fn standalone_terminal_entrypoints_fail_before_source_or_metal_setup() {
            let error = run_with(
                PathBuf::from("/definitely-not-a-model-root"),
                PathBuf::from("/definitely-not-a-terminal-state"),
                Some(PathBuf::from("/definitely-not-a-terminal-receipt")),
            )
            .expect_err("the standalone terminal run_with path must be disabled");
            assert_eq!(error, STANDALONE_TERMINAL_DISABLED);

            let cli_error = run().expect_err("the standalone terminal CLI path must be disabled");
            assert_eq!(cli_error, STANDALONE_TERMINAL_DISABLED);
        }
    }
}
