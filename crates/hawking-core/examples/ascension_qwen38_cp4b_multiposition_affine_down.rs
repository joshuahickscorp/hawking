//! CP4b — mlp_down, the second-largest organ, batched.
//!
//! CP4 batched `mlp_gate_up` (35.9% of per-step GPU ns) at 2.49x, bit-identical.
//! `mlp_down` is 20.9% and runs the GENERIC single-tensor affine matvec
//! `qwen_affine_q2_group32_matvec_geo_tpr64_tg128`, which branches on a runtime
//! `group_size`. It had no multi-position variant either. Together the two MLP
//! organs are 56.8% of the step.
//!
//! One tensor instead of a pair, so `R*K` accumulators rather than `2*R*K`:
//! live floats/thread ~= `R*K + K + 3*R`. CP4 REFUTED CP3's register threshold
//! (r4k4 won at 60 live floats where the q4 curve had collapsed by 50-56), so no
//! knee is predicted here. The grid is swept and the measurement decides —
//! including r4k8, which CP3's model would have called hopeless.
//!
//! Geometry is `mlp.down_proj.weight`: rows 5120, cols 17408 — the TRANSPOSE of
//! gate_up, so `groups_per_row` is 272 rather than 80 and each row is 3.4x
//! longer. That matters: the sweep loop strides `col += 512`, so a longer row is
//! more work per thread and a different occupancy regime.
//!
//! Build:
//!   cargo build --profile release-fast -p hawking-core \
//!     --example ascension_qwen38_cp4b_multiposition_affine_down
//! Correctness (safe under GPU contention):
//!   .../ascension_qwen38_cp4b_multiposition_affine_down --out /tmp/cp4b.json
//! Timing (REQUIRES zero loaded 27B residents, checked by RSS; run BOTH orders):
//!   ./tools/gpu_lane_lock.sh cp4b .../ascension_qwen38_cp4b_multiposition_affine_down \
//!     --timing --reps 9 --out receipts/runtime/CP4B_AFFINE_DOWN_FORWARD.json
...
mod macos {
    ...
    const SCHEMA: &str = "hawking.cp4b.multiposition_affine_down.v1";

    // From the artifact's own hgrafv01 header, not from a constant in this repo.
    const ROWS: usize = 5_120;
    const COLS: usize = 17_408;
    const GROUP: usize = 64;
    const CODE_BYTES: usize = 22_282_240; // rows*cols*2 bits -- same element count, transposed
    const PLANE_HALVES: usize = 1_392_640; // rows * groups_per_row = 5120 * 272
    ...
    const GRID: &[(usize, usize)] =
        &[(1, 1), (2, 1), (4, 1), (2, 2), (2, 4), (4, 2), (4, 4), (4, 8)];

    const BASELINE: &str = "qwen_affine_q2_group32_matvec_geo_tpr64_tg128";
    const GROUP_SIZE: u32 = 64; // runtime arg; the artifact header says group_size 64

    fn kernel_for(r: usize, k: usize) -> String {
        format!("qwen_affine_q2_matmul_r{r}k{k}_geo_tpr64_tg128")
    }
    ...
    struct Bufs { c: metal::Buffer, s: metal::Buffer, b: metal::Buffer }
    ...
    fn arm_seq(
        ...
        for i in 0..xs.len() {
            let t = ctx.dispatch_threads_timed(BASELINE, grid, tg, |enc| {
                enc.set_buffer(0, Some(&w.c), 0); enc.set_buffer(1, Some(&w.s), 0);
                enc.set_buffer(2, Some(&w.b), 0);
                enc.set_buffer(3, Some(&xs[i]), 0);
                enc.set_buffer(4, Some(&go[i]), 0);
                let r = ROWS as u32; let c = COLS as u32; let g = GROUP_SIZE;
                enc.set_bytes(5, 4, &r as *const u32 as *const _);
                enc.set_bytes(6, 4, &c as *const u32 as *const _);
                enc.set_bytes(7, 4, &g as *const u32 as *const _);
            })?;
    ...
    fn arm_mp(
        ...
        let t = ctx.dispatch_threads_timed(kernel, grid, tg, |enc| {
            enc.set_buffer(0, Some(&w.c), 0); enc.set_buffer(1, Some(&w.s), 0);
            enc.set_buffer(2, Some(&w.b), 0);
            enc.set_buffer(3, Some(xi), 0);
            enc.set_buffer(4, Some(go), 0);
            let rr = ROWS as u32; let c = COLS as u32; let g = GROUP_SIZE;
            enc.set_bytes(5, 4, &rr as *const u32 as *const _);
            enc.set_bytes(6, 4, &c as *const u32 as *const _);
            enc.set_bytes(7, 4, &g as *const u32 as *const _);
        })?;
        match (t.gpu_start_ns, t.gpu_end_ns) {
    ...
    pub fn run() -> Result<(), Box<dyn Error>> {
        ...
        let w = Bufs {
            c: ctx.new_buffer_with_bytes_checked(&codes)?,
            s: ctx.new_buffer_with_bytes_checked(&halves(PLANE_HALVES, 0.0020, 0.00011))?,
            b: ctx.new_buffer_with_bytes_checked(&halves(PLANE_HALVES, -0.0030, 0.00007))?,
        };
        ...
        for &(r, k) in &order {
            ...
            rows_out.push(json!({
                ...
                "scale_plus_bias_bytes": 2 * PLANE_HALVES * 2,
                "activation_bytes_moved": COLS * k * 4,
                "output_bytes": ROWS * k * 4,
                "interleave_transform_ns": interleave_ns,
                "temporary_bytes": COLS * k * 4 + ROWS * k * 4,
                "flops": 2.0 * (ROWS * COLS * k) as f64,
                "threadgroup_bytes": 16 * r * k,
                "accumulators_per_thread": r * k,
                "live_floats_per_thread": r * k + k + 3 * r,
            }));
...
neg_rejected={neg_rejected} uniform={uniform_holds}");
        ...
        let doc = json!({
            "schema": SCHEMA,
            "organ": "mlp_down",
            "organ_share_of_step_gpu_ns": 0.209,
...
for the correctness check, and both arms use it, so the comparison isolates batching.",
            ...
            "no_knee_predicted": "CP4 REFUTED CP3's register threshold -- r4k4 won at 60 live floats \
where the q4 curve had already collapsed by 50-56. So this sweep predicts nothing and includes r4k8 \
(live floats 44) precisely because CP3's model would have called it hopeless.",
