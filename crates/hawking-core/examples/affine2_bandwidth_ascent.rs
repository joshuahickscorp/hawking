//! Affine2 g64 kernel-geometry levers vs the tpr64 incumbent.
//!
//! Isolated GEMV (no 27B): no-op control, deliberately-bad control, and three
//! distinct geometries. Production decode on NOETIC_PARENT_A is optional
//! (`--artifact-root`) and never mutates the parent.
//!
//! ```text
//! cargo build --profile release-fast -p hawking-core --example affine2_bandwidth_ascent
//! ./tools/gpu_lane_lock.sh n018-bwascent \
//!   workspace/ops/build/rust/release-fast/examples/affine2_bandwidth_ascent \
//!   --isolated --reps 7 --out receipts/headless/_BANDWIDTH_ASCENT_isolated.json
//! ```
...
fn main() {
    ...
    let mut doc = json!({
        "schema": "hawking.headless.bandwidth_ascent.raw.v1",
        "isolated": isolated,
...
mod isolated {
    ...
    const ARMS: &[Arm] = &[
        ...
        Arm {
            ...
            role: "lever",
            lever: Some("qmv_fast tile: 8 rows/TG, 4-row x-reuse, TG 64, uint32 codes"),
            rows_per_tg: 8,
            tg: 64,
            bind_group_size: false,
        },
        Arm {
            ...
            role: "lever",
            lever: Some("whole-group uint4 loads, 32 threads/row, scale/bias once per 64"),
            rows_per_tg: 4,
            tg: 128,
            bind_group_size: false,
        },
        Arm {
            ...
            role: "lever",
            lever: Some("threadgroup-staged x K-tile 512, 8 rows/TG, TG 256, split-K simd_sum"),
            rows_per_tg: 8,
            tg: 256,
            bind_group_size: false,
        },
        Arm {
            ...
            lever: Some("same geometry as qmvfast; skip (q*scale+bias)*x"),
            rows_per_tg: 8,
            tg: 64,
            bind_group_size: false,
        },
