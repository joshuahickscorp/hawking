...
fn pq_matvec_matches_reference_for_all_fixtures() {
    ...
    for row in &manifest.fixtures {
        ...
        let h = parse_pq_header(&payload).unwrap_or_else(|e| panic!("{}: parse_pq_header: {e}", row.fixture));
        ...
        let y_got = pq_matvec(&payload, &x).unwrap_or_else(|e| panic!("{}: pq_matvec: {e}", row.fixture));
        ...
        for (i, (&a, &b)) in y_got.iter().zip(y_ref.iter()).enumerate() {
            ...
            assert!(diff <= tol, "{}: element {i}: got {a}, want {b}, |diff|={diff} > tol={tol}", row.fixture);
        }
        let (idx, _, diff, tol, got_val) = worst;
        assert!(diff <= tol, "{}: worst element {idx}: got {got_val}, want {}, |diff|={diff} > tol={tol}", row.fixture, y_ref[idx]);
    }
}
