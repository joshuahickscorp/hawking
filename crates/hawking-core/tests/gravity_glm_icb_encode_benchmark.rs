...
use hawking_core::gravity_glm::{
    GlmTrace, GPU_COMPACT_ATTENTION_ICB_ENV, GPU_COMPACT_MLA_ENV, GPU_DEVICE_DSA_ENV, GPU_DEVICE_ROUTER_ENV, GPU_EXPERT_TABLE_HIT_ENV,
    GPU_EXPERT_TABLE_ICB_ENV, GPU_EXPERT_WAVE_ENV, GPU_LM_HEAD_ENV, GPU_LM_HEAD_FULL_LOGITS_ENV, GPU_LM_HEAD_ICB_ENV,
};
...
const REPLAY_MODES: [ReplayMode; 5] = [
    ReplayMode { name: "direct", attention: false, expert: false, head: false },
    ReplayMode {
        name: "attention_only",
        attention: true,
        expert: false,
...
fn percentile(values: &[u64], percentile: usize) -> u64 {
    ...
    let rank = percentile.saturating_mul(sorted.len().saturating_sub(1)).saturating_add(99) / 100;
...
fn bucket(report: &TokenCostReport, name: &str) -> u64 {
    report
        .buckets_us
        .get(name)
        .unwrap_or_else(|| panic!("missing {name} bucket; available={:?}", report.buckets_us.keys().collect::<Vec<_>>()))
        .as_u64()
        .unwrap_or_else(|| panic!("missing {name} bucket"))
}
fn profiled_forward(model: &GravityGlmGpu, tokens: &[u32], mode: ReplayMode) -> (Vec<f32>, GlmTrace, TokenCostReport, u64) {
    ...
    let waits = model.last_resident_waits().expect("bounded resident wait count");
...
fn compact_glm_all_icb_flags_complete_token_encode_benchmark() {
    ...
    let authority: serde_json::Value =
        serde_json::from_slice(&std::fs::read(dir.join("ref_logits_f64.json")).expect("bounded FP64 authorities"))
    ...
    let model =
        GravityGlmGpu::open_dir_with_budget_resident(ctx, &dir, true, 512 * 1024 * 1024, true).expect("bounded compact resident model");
    ...
    let mut reports: Vec<Vec<TokenCostReport>> = REPLAY_MODES.iter().map(|_| Vec::with_capacity(SAMPLES)).collect();
    for iteration in 0..SAMPLES {
        for offset in 0..REPLAY_MODES.len() {
            ...
            assert_eq!(
                logits.iter().map(|value| value.to_bits()).collect::<Vec<_>>(),
                reference_logits.iter().map(|value| value.to_bits()).collect::<Vec<_>>(),
                "mode={} changed complete-token logits",
                mode.name
            );
            assert_eq!(trace.final_topk, reference_trace.final_topk, "mode={} changed DSA rank", mode.name);
            assert_eq!(trace.expert_choices, reference_trace.expert_choices, "mode={} changed expert choices", mode.name);
            assert_eq!(trace.sample_token, reference_trace.sample_token, "mode={} changed sampled token", mode.name);
    ...
    let summarize = |reports: &[TokenCostReport]| {
        ...
        let encode: Vec<u64> = reports.iter().map(|report| bucket(report, "metal_encode")).collect();
        let submit: Vec<u64> = reports.iter().map(|report| bucket(report, "metal_submit")).collect();
        let synchronize: Vec<u64> = reports.iter().map(|report| bucket(report, "metal_synchronize_cpu_wait")).collect();
        let gpu: Vec<u64> = reports.iter().map(|report| report.device.gpu_execution_us).collect();
        serde_json::json!({
            "samples": reports.len(),
