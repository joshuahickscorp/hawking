//! Explicit bounded complete-token benchmark for GLM replay flags.
//!
//! This is ignored by the ordinary suite because it requires the generated
//! direct-u8 compact fixture and a Metal device. It makes no production or
//! sealed-model claim.
...
fn compact_glm_all_icb_flags_complete_token_encode_benchmark() {
    ...
    eprintln!(
        "GLM_ALL_ICB_COMPLETE_TOKEN_BENCHMARK {}",
        serde_json::json!({
            "fixture": dir,
            "tokens": tokens,
            "waits": reference_waits,
            "modes": modes
        })
    );
}
