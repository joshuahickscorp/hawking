...
fn run_f32_4r(
    ...
    kernels::gemv_q4_k_v4_predec_pair_4r_pinned_tcb(
        &mut tcb, &model_buf, g_off, g_len, &gs_buf, 0, u_off, u_len, &us_buf, 0, rows, cols, &x_buf, &g_out, &u_out,
    )
    .unwrap();
...
fn pair_4r_f16s_rel_l2_quality_gate() {
    ...
    let cases: &[(usize, usize, u32)] = &[(11008, 2048, 0xD40A), (11008, 2048, 0xD40B), (1024, 2048, 0xD40C), (2048, 2048, 0xD40D)];
    for &(rows, cols, seed) in cases {
        ...
        let u_off = g_w.len();
        let x: Vec<f32> =
            (0..cols).map(|i| ((i as u32).wrapping_mul(1664525).wrapping_add(seed) as f32 / u32::MAX as f32) * 2.0 - 1.0).collect();
