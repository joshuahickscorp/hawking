...
fn gravity_llama_forward_matches_frozen_oracle() {
    ...
    let reference: Reference =
        serde_json::from_slice(&std::fs::read(dir.join("ref_3tok.json")).expect("read ref_3tok")).expect("parse ref_3tok");
    ...
    assert_eq!(model.tied_head, reference.tied_head, "head tying disagrees with the oracle");
    assert_eq!(model.arch.vocab_size, want.len(), "vocab_size vs reference logit count");
    ...
    for (i, &w) in reference.logits_head.iter().enumerate() {
        ...
        assert!(diff <= 1e-3 + 1e-4 * w.abs(), "logits_head[{i}]: got {}, want {w}, diff {diff}", got[i]);
    }
    let got_top5 = top_k(&got, 5);
    assert_eq!(got_top5[0], reference.argmax, "argmax: got {}, want {}", got_top5[0], reference.argmax);
...
fn gravity_llama_gpu_forward_matches_frozen_oracle() {
    ...
    assert_eq!(got_top5, reference.top5, "top-5");
    assert!(model.device_bytes < 150 * 1024 * 1024, "device_bytes {} suggests something was widened on upload", model.device_bytes);
}
#[cfg(target_os = "macos")]
#[test]
fn gravity_llama_incremental_decode_matches_full_replay() {
    ...
    let max_abs = got.iter().zip(want.iter()).map(|(a, b)| (a - b).abs()).fold(0f32, f32::max);
    assert!(max_abs <= 1e-4, "incremental decode diverged from full replay by {max_abs}");
}
