...
//! decode concatenates the raw bytes. hawking's `RwkvWorldTokenizer` mirrors
...
//! Needs the f32 reference GGUF (env `HAWKING_RWKV7_F32_GGUF`, else the
...
//! without `--no-bos`). hawking's `encode` likewise emits no BOS.
...
fn rwkv_world_tokenizer_parity_vs_llamacpp() {
    ...
    // leading id if hawking prepended one).
