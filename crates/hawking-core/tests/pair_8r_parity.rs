...
fn run_pair(
    ...
    run_predec_pair_combined(ctx, wg, wu, g_scales, u_scales, x, rows, cols, |tcb, c, wb, gs, us, xb, yg, yu| {
        let r = if is_8r {
            kernels::gemv_q4_k_v4_predec_pair_8r_pinned_tcb(tcb, c, 0, wb, gs, 0, wb, wb, us, 0, rows, cols, xb, yg, yu)
        } else {
            kernels::gemv_q4_k_v4_predec_pair_4r_pinned_tcb(tcb, c, 0, wb, gs, 0, wb, wb, us, 0, rows, cols, xb, yg, yu)
        };
...
fn e1_pair_8r_matches_pair_4r() {
    ...
    for &(rows, cols, seed) in cases {
        ...
        assert!(diff_g <= MAX_DIFF, "E1 rows={rows} cols={cols}: gate max_diff={diff_g:.2e}");
        assert!(diff_u <= MAX_DIFF, "E1 rows={rows} cols={cols}: up max_diff={diff_u:.2e}");
    }
}
