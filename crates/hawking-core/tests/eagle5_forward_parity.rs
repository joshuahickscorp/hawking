...
//!
//! Fixture path: `crates/hawking-core/tests/fixtures/eagle5_parity_q3b.json`.
//!
...
fn q1p5_head_path() -> Option<PathBuf> {
    ...
    let home = std::env::var_os("HOME")?;
    let candidate =
        PathBuf::from(home).join("Downloads/hawking_export/heads/q1p5_eagle6_long.safetensors");
    if candidate.exists() {
        Some(candidate)
    } else {
        None
...
fn eagle6_forward_matches_pytorch_q1p5() {
    let head = q1p5_head_path()
        .expect("set HAWKING_Q1P5_HEAD or place q1p5_eagle6_long.safetensors at ~/Downloads/hawking_export/heads/");
    let fixture_path =
