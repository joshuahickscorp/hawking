...
use common::*;
const ATOL: f32 = 1e-3;
fn run_v3w(wbuf: &PinnedBuffer, wlen: usize, sbuf: &PinnedBuffer, rows: usize, cols: usize, batch: usize, xbuf: &PinnedBuffer) -> Vec<f32> {
    let ctx = ctx();
    let ybuf = ctx.new_buffer(batch * rows * std::mem::size_of::<f32>());
    let mut tcb = TokenCommandBuffer::new(ctx);
    kernels::gemm_q4_k_m_batched_v3w_predec_pinned_tcb(&mut tcb, wbuf, 0, wlen, sbuf, 0, rows, cols, batch, xbuf, &ybuf)
        .expect("v3w encode");
    tcb.commit_and_wait().expect("v3w commit");
    let p = ybuf.contents() as *const f32;
    unsafe { std::slice::from_raw_parts(p, batch * rows) }.to_vec()
}
fn run_v4r(wbuf: &PinnedBuffer, wlen: usize, sbuf: &PinnedBuffer, rows: usize, cols: usize, batch: usize, xbuf: &PinnedBuffer) -> Vec<f32> {
    let ctx = ctx();
    let ybuf = ctx.new_buffer(batch * rows * std::mem::size_of::<f32>());
    let mut tcb = TokenCommandBuffer::new(ctx);
    kernels::gemm_q4_k_m_batched_v4r_predec_pinned_tcb(&mut tcb, wbuf, 0, wlen, sbuf, 0, rows, cols, batch, xbuf, &ybuf)
        .expect("v4r encode");
