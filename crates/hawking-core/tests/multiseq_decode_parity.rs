...
fn ms_solo(engine: &mut QwenDense, seed: u32, n: usize, max_seq: usize) -> Vec<u32> {
    ...
    for pos in 0..n {
        let next = engine.forward_tokens_multiseq(&[cur], &[pos], max_seq).expect("ms solo");
        seq.push(next[0]);
        cur = next[0];
    }
    seq
...
fn multiseq_batched_equals_solo_and_anchors_single() {
    ...
    for &s in &seeds {
        ...
        assert_eq!(single, ms, "anchor: B=1 multiseq token {ms} != single-stream {single} (seed {s})");
    }
    ...
    for pos in 0..n {
        ...
        let next = engine.forward_tokens_multiseq(&cur, &positions, max_seq).expect("ms batched");
        for bi in 0..b {
    ...
    for bi in 0..b {
        assert_eq!(solo[bi], batched[bi], "slot {bi}: batched {:?} != solo {:?}", batched[bi], solo[bi]);
    }
}
