...
fn run_4r(ctx: &MetalContext, w_q6: &[u8], gate: &[f32], up: &[f32], rows: usize, cols: usize) -> Vec<f32> {
    let model_buf = ctx.new_buffer_with_bytes(w_q6);
    let gate_buf = new_f32_buf(ctx, gate);
    let up_buf = new_f32_buf(ctx, up);
    let out_buf = ctx.new_buffer(rows * std::mem::size_of::<f32>());
    let mut tcb = TokenCommandBuffer::new(ctx);
    kernels::gemv_q6_k_swiglu_4r_direct_tcb(&mut tcb, &model_buf, 0, w_q6.len(), rows, cols, &gate_buf, &up_buf, &out_buf).unwrap();
    tcb.commit_and_wait().unwrap();
    read_f32_buf(&out_buf, rows)
}
#[test]
fn d8_q6k_swiglu_4r_matches_2r() {
    ...
    for &(rows, cols, seed) in cases {
        ...
        assert!(diff <= MAX_DIFF, "D8 rows={rows} cols={cols}: 4r vs 2r diff={diff:.2e} > {MAX_DIFF:.2e}");
    }
}
