...
fn solo_decode_after_prompt(engine: &mut QwenDense, prompt_ids: &[u32]) -> Vec<u32> {
    ...
    for (pos, &tok) in prompt_ids.iter().enumerate() {
        last_out = engine.forward_token_greedy_tcb(tok, pos).expect("solo prefill step");
    }
    let mut seq = vec![last_out];
    let mut cur = last_out;
    for step in 1..3usize {
        cur = engine.forward_token_greedy_tcb(cur, prompt_len + step - 1).expect("solo decode step");
        seq.push(cur);
    }
    seq
}
fn multiseq_decode_after_prefill(engine: &mut QwenDense, slot_id: usize, prompt_ids: &[u32]) -> Vec<u32> {
    let prompt_len = prompt_ids.len();
    let last_prompt_tok = *prompt_ids.last().unwrap();
    let returned = engine.prefill_slot(slot_id, prompt_ids).expect("prefill_slot");
    assert_eq!(returned, last_prompt_tok, "prefill_slot must return the last prompt token id");
    let mut cur = last_prompt_tok;
    let mut seq = Vec::with_capacity(3);
    for step in 0..3usize {
        let logits = engine.forward_multiseq_batched(&[cur], &[prompt_len + step], &[slot_id]).expect("multiseq decode step");
...
fn prefill_slots_parallel_parity() {
    ...
    for step in 0..3usize {
        let logits = engine.forward_multiseq_batched(&[cur_b], &[prompt_b.len() + step], &[3]).expect("serial decode slot 3");
        let tok = argmax(&logits[0]);
        expected_b.push(tok);
        cur_b = tok;
    }
    ...
    engine.kv.reset();
    engine.dense_arena = None;
    engine.prefill_slots_parallel(&[(0, prompt_a), (3, prompt_b)]).expect("prefill_slots_parallel");
    let mut cur_a = *prompt_a.last().unwrap();
    let mut parallel_a = Vec::with_capacity(3);
    for step in 0..3usize {
        let logits = engine.forward_multiseq_batched(&[cur_a], &[prompt_a.len() + step], &[0]).expect("parallel decode slot 0");
        let tok = argmax(&logits[0]);
        parallel_a.push(tok);
        cur_a = tok;
    }
    ...
    for step in 0..3usize {
        let logits = engine.forward_multiseq_batched(&[cur_b2], &[prompt_b.len() + step], &[3]).expect("parallel decode slot 3");
        let tok = argmax(&logits[0]);
        parallel_b.push(tok);
        cur_b2 = tok;
    }
    assert_eq!(expected_a, parallel_a, "slot 0 diverges between serial and parallel prefill");
    assert_eq!(expected_b, parallel_b, "slot 3 diverges between serial and parallel prefill");
}
