//! Independent-oracle parity: hawking's greedy decode agrees with llama.cpp's
//! on the same GGUF.
//!
//! Every other parity gate checks Metal against hawking's *own* CPU reference
...
//! gate runs wherever llama.cpp + a model fixture exist (set `HAWKING_LLAMA_CLI`
...
/// hawking greedy continuation, in-process.
...
fn hawking_greedy_matches_llama_cpp() {
    ...
    eprintln!("[oracle] hawking -> {dism:?}  (first word {dw:?})");
