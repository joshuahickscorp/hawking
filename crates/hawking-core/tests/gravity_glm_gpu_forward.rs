...
fn gravity_glm_gpu_forward_resets_between_requests() {
    ...
    let (second, _) = model.forward(&reference.tokens).expect("second request on the same resident model");
    assert_eq!(first, second, "two identical requests on one resident model must produce identical logits");
}
