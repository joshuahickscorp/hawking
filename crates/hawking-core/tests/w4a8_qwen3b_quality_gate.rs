...
fn run(weights: &PathBuf, w4a8: bool) -> Option<RunOut> {
    ...
    assert!(prompt_ids.len() >= 2, "prompt tokenized too short: {:?}", prompt_ids);
...
fn w4a8_quality_gate() {
    ...
    let first_div = f32_out.tokens.iter().zip(&w4a8_out.tokens).position(|(a, b)| a != b).unwrap_or(MAX_NEW);
    assert!(cos > 0.998, "logit cosine too low: {cos:.6} (need > 0.998)");
    assert!(first_div >= 8, "greedy divergence at token {first_div} (need >= 8)");
}
