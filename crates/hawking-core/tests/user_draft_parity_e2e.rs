...
//! `HAWKING_QWEN_USER_DRAFT`). Runs on the real Qwen-3B model; skips
...
//! A request with `HAWKING_QWEN_USER_DRAFT=1` must therefore produce the
...
fn make_engine(weights: &PathBuf) -> Box<dyn hawking_core::Engine> {
    ...
    // E3 (HAWKING_QWEN_PAIR_2R_INLINE, default-on) is bit-identical to old-2r
...
/// THE GATE: `HAWKING_QWEN_USER_DRAFT=1` greedy output must be
...
///
/// With the shipped vocab-pruned Q4_K LM head active (`HAWKING_QWEN_VOCAB_PRUNE`
/// + `HAWKING_QWEN_Q4K_LMHEAD`), `forward_tokens_verify` takes its GPU fast path
