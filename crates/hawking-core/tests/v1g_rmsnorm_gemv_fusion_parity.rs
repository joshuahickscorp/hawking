...
fn wedge_g_rmsnorm_gemv_f32_attn_pinned_tcb_matches_cpu() {
    ...
    kernels::rmsnorm_gemv_f32_attn_pinned_tcb(&mut tcb, &w_buf, &x_buf, &weight_buf, eps, &out_buf, rows, cols)
...
fn wedge_g_fused_pair_matches_cpu() {
    ...
    let mut tcb = TokenCommandBuffer::new(ctx);
    kernels::rmsnorm_gemv_f32_attn_pinned_tcb(&mut tcb, &w_a_buf, &x_buf, &weight_buf, eps, &out_a_buf, rows_a, cols)
        .expect("rmsnorm_gemv q_a");
    kernels::rmsnorm_gemv_f32_attn_pinned_tcb(&mut tcb, &w_b_buf, &x_buf, &weight_buf, eps, &out_b_buf, rows_b, cols)
