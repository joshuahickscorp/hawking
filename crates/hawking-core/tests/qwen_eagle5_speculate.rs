...
//!
//! 1. **Speculate=off equivalence:** running with `HAWKING_QWEN_TCB=1` and
//!    no Eagle5 must produce bit-identical greedy tokens to baseline. Eagle5
//!    must never silently perturb the default path.
//! 2. **Speculate=eagle5 engagement:** with `HAWKING_QWEN_EAGLE5=1` and
...
// Env flags `HAWKING_QWEN_TCB` / `HAWKING_QWEN_EAGLE5` are process-global;
...
fn run_greedy_capture_stats(
    weights: &PathBuf,
    cfg: hawking_core::EngineConfig,
) -> (Vec<u32>, hawking_core::GenStats) {
...
fn qwen_eagle5_speculate_off_bit_identical_to_baseline() {
    ...
    let cfg_a = hawking_core::EngineConfig {
    ...
    let cfg_b = hawking_core::EngineConfig {
        kernel_profile: profile.clone(),
        ..Default::default()
    };
...
fn qwen_eagle5_speculate_on_mock_head_engages_and_preserves_greedy() {
    ...
    std::env::set_var("HAWKING_QWEN_TCB", "1");
    let cfg_baseline = hawking_core::EngineConfig {
        kernel_profile: profile.clone(),
        ..Default::default()
    };
...
/// `HAWKING_QWEN_EAGLE5_BATCHED=1` flipped on. The batched-with-logits
...
///
/// With `HAWKING_QWEN_EAGLE5_CAPTURE=1` the per-layer forward inserts
