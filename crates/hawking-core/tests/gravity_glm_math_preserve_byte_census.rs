//! Pure arithmetic checks for the sealed H0.98 Math-Preserve header census.
//! This test does not open model artifacts or initialize Metal.
...
fn math_preserve_routed_census_is_route_conditioned_and_bounded() {
    // Header audit over every layer 3..77 expert candidate. Each layer has
    // exactly 243 R4 candidates and 13 protected candidates split R0/native.
    ...
    // Three projections for one selected expert.
    ...
    // Minimum resident extent selects eight R4 candidates in every layer.
    let route_min = 75 * 8 * R4_TRIPLET_RESIDENT;
    assert_eq!(route_min, 737_287_200);

    // Maximum resident extent selects native first, then R0. The layer-local
    // constraint yields 561 native + 39 R0 selections, not a global sort.
    ...
    // Source-level shader request extrema use a different ordering:
    // native < R4 < R0.
...
fn r0_geometry_is_only_a_historical_control() {
    ...
    // Correct Math-Preserve fixed work under the historical f32-native path:
    // fixed R4 PQ buffers plus indexer/router/head widened to f32.
