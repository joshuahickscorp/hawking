...
fn run_greedy() -> Vec<u32> {
    ...
    let req = GenerateRequest {
        ...
        sampling: SamplingParams { temperature: 0.0, seed: Some(42), ..Default::default() },
        stop: vec![],
        abort: None,
        max_stall_ms: 0,
        json_mode: false,
...
fn batched_prefill_matches_single_token_prefill() {
    ...
    assert_eq!(baseline.len(), batched.len(), "token count mismatch: baseline={} batched={}", baseline.len(), batched.len(),);
    assert_eq!(baseline, batched, "batched prefill must produce identical greedy tokens to single-token prefill");
}
