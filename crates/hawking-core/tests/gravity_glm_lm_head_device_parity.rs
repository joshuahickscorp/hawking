...
use hawking_core::numeric_parity::{format_score_line, matvec_bf16_f64_authority, score_pair, Bounds, SCHEMA};
fn activations(cols: usize) -> Vec<(&'static str, Vec<f32>)> {
    vec![
        ("ramp", (0..cols).map(|c| (c as f32) * 0.01 - 0.3).collect()),
        ("mod17", (0..cols).map(|c| ((c * 5 + 3) % 17) as f32 * 0.05 - 0.4).collect()),
        ("ones", vec![1.0; cols]),
        ("zeros", vec![0.0; cols]),
        ("block3", (0..cols).map(|c| if c % 3 == 0 { 0.5 } else { -0.25 }).collect()),
        ("sin", (0..cols).map(|c| ((c as f32).sin()) * 0.1).collect()),
        ("half", (0..cols).map(|c| if c < cols / 2 { 0.125 } else { -0.0625 }).collect()),
        ("mod31", (0..cols).map(|c| ((c * 13 + 7) % 31) as f32 * 0.02 - 0.3).collect()),
        (
            "gaussish",
            (0..cols)
                .map(|c| {
...
fn device_bf16_lm_head_v21_against_f64_over_several_vectors() {
    ...
    for &(rows, cols) in &shapes {
        ...
        let w_buf = ctx.new_buffer_with_bytes_checked(&weight).expect("upload weight");
        for (vi, (name, x)) in activations(cols).into_iter().enumerate() {
            let reference = matvec_bf16_f64_authority(&weight, cols, &x).unwrap_or_else(|e| panic!("f64 authority failed: {e}"));
            for accumulation in NativeBf16Accumulation::ALL {
                ...
                let device = hawking_core::gravity_glm::gpu::dispatch_gemv_native_bf16_accumulation(
                    &ctx,
                    &w_buf,
                    rows as u32,
                    cols as u32,
    ...
    assert!(!any_accurate_fail, "internal bookkeeping: accurate failure set but asserts passed");
}
