...
// ── Phase 5C.1: f16 KV cache variants ──────────────────────────────────────

// Phase 5C.1: f16 KV variant of mla_kv_append.
// Identical to kv_append_f32 except writes KV as half (f16) instead of float.
// Input activations arrive as f32; converted to f16 on write.
// Buffer layout:
//   0  src_c_kv_normed  (kv_lora_rank,)         f32  — c_kv_normed_buf
//   1  src_kv_a_out     (kv_a_dim,)              f32  — kv_a_out_buf (k_pe at [kv_lora_rank..])
//   2  dst_c_kv         (max_seq × kv_lora_rank) f16  — persistent GPU KV latent (f16 storage)
//   3  dst_k_pe         (max_seq × rope_dim)     f16  — persistent GPU RoPE K (f16 storage)
//   4  seq_slot         constant uint             — position index
//   5  kv_lora_rank     constant uint
//   6  qk_rope_head_dim constant uint
kernel void kv_append_f16(
    device const float* src_c_kv_normed  [[buffer(0)]],
    device const float* src_kv_a_out     [[buffer(1)]],
    device       half*  dst_c_kv         [[buffer(2)]],
    device       half*  dst_k_pe         [[buffer(3)]],
    constant ArgbufKvAppend& args        [[buffer(4)]],
    uint tid [[thread_position_in_grid]])
{
    uint64_t c_base  = (uint64_t)args.seq_slot * (uint64_t)args.kv_lora_rank;
    uint64_t pe_base = (uint64_t)args.seq_slot * (uint64_t)args.qk_rope_head_dim;
    if (tid < args.kv_lora_rank) {
        dst_c_kv[c_base + tid] = (half)src_c_kv_normed[tid];
    }
    if (tid < args.qk_rope_head_dim) {
        dst_k_pe[pe_base + tid] = (half)src_kv_a_out[(uint64_t)args.kv_lora_rank + tid];
    }
}

// Phase 5C.1: f16 KV variant of mla_decode_kernel (single-token decode).
// Identical to mla_decode_kernel except reads KV from device half* buffers.
// KV values are loaded as half, immediately promoted to float for accumulation.
// All internal accumulators (softmax, MAC) remain float.
kernel void mla_decode_kernel_f16(
    device const float* q          [[buffer(0)]],
    device const half*  c_kv       [[buffer(1)]],
    device const half*  k_pe       [[buffer(2)]],
    device const float* kv_b_proj  [[buffer(3)]],
    device       float* out        [[buffer(4)]],
    constant     uint&  n_heads             [[buffer(5)]],
    constant     uint&  qk_nope_head_dim    [[buffer(6)]],
    ...
    uint                tg_size [[threads_per_threadgroup]])
{
    ...
    // Phase 1: scores[t] = (q_nope_proj · c_kv[t] + q_rope · k_pe[t]) × scale
    // Load c_kv and k_pe as half, promote to float.
    for (uint t = tid; t < seq_len; t += tg_size) {
        device const half*  c_kv_t = c_kv + (uint64_t)t * kv_lora_rank;
        device const half*  k_pe_t = k_pe + (uint64_t)t * qk_rope_head_dim;

        float s = 0.0f;
        for (uint r = 0; r < kv_lora_rank; r++)      s += q_nope_proj[r] * (float)c_kv_t[r];
        for (uint r = 0; r < qk_rope_head_dim; r++)  s += q_rope[r] * (float)k_pe_t[r];
        scores[t] = s * scale;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    // Phase 2: softmax (thread 0 serial — stays float)
    if (tid == 0) {
        float mx = -INFINITY;
        for (uint t = 0; t < seq_len; t++) if (scores[t] > mx) mx = scores[t];
        float sum = 0.0f;
    ...
    // Phase 3: c_kv_wt[r] = Σ_t scores[t] × c_kv[t, r]
    // Load c_kv as half, promote to float.
    for (uint r = tid; r < kv_lora_rank; r += tg_size) {
        float acc = 0.0f;
        for (uint t = 0; t < seq_len; t++)
            acc += scores[t] * (float)c_kv[(uint64_t)t * kv_lora_rank + r];
        c_kv_wt[r] = acc;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
...
// Phase 5C.1: f16 KV variant of mla_decode_kernel_batched_slots (Phase 4A).
// Identical to mla_decode_kernel_batched_slots except reads KV from device half*.
// Load half → float at read time, compute entirely in float.
kernel void mla_decode_kernel_batched_slots_f16(
    device const float* q_batch     [[buffer(0)]],
    device const half*  c_kv        [[buffer(1)]],
    device const half*  k_pe        [[buffer(2)]],
    device const float* kv_b_proj   [[buffer(3)]],
    ...
    uint2               tg_size_v [[threads_per_threadgroup]])
{
    ...
    for (uint t = tid_v.x; t < seq_len_m; t += tg_size_v.x) {
        const uint kv_index = slot_base + t;
        device const half*  c_kv_t = c_kv + (uint64_t)kv_index * kv_lora_rank;
        device const half*  k_pe_t = k_pe + (uint64_t)kv_index * qk_rope_head_dim;
    ...
    for (uint r = tid_v.x; r < kv_lora_rank; r += tg_size_v.x) {
        float acc = 0.0f;
        for (uint t = 0; t < seq_len_m; t++) {
            acc += scores[t] * (float)c_kv[(uint64_t)(slot_base + t) * kv_lora_rank + r];
        }
        c_kv_wt[r] = acc;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
