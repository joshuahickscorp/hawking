...
//!
//! The deliverable for slices 1-2: dismantle's CPU-reference RWKV-7 forward must
...
//!    both sides, dismantle's forward reproduces llama.cpp's greedy decode
//!    **exactly** for all N tokens (the only residual is f32 op-order rounding,
//!    measured at <=0.03 max-abs logit diff). Gated on env
//!    `DISMANTLE_RWKV7_F32_GGUF` (or `/tmp/rwkv_ref/rwkv7-04-f32.gguf`); skips if
    ...
    if !f32_path.exists() {
        eprintln!(
            "skipping rwkv7_argmax_parity_f32_exact: no F32 RWKV-7 GGUF at {f32_path:?}\n  \
             ...
             /tmp/rwkv_ref/rwkv7-04-f32.gguf F32, or set DISMANTLE_RWKV7_F32_GGUF)"
        );
        return;
    }
...
fn rwkv7_loads_and_runs_q4k() {
    ...
    let boxed = dismantle_core::model::load_engine(&weights, EngineConfig::default())
