...
fn run_f32_2r(
    ...
    kernels::gemv_q4k_predec_qkv_rope_append_pinned_tcb(
        &mut tcb, &model,
        q_off, w.q_w.len(), &q_sc,
        k_off, w.k_w.len(), &k_sc,
        v_off, w.v_w.len(), &v_sc,
        q_rows, kv_rows, shape.cols,
        shape.n_q, shape.n_k, shape.hd,
        shape.pos, 10000.0, shape.kv_off,
        &x_buf, &q_buf, None, None, None,
        &k_cache, &v_cache,
    ).expect("f32 2r");
...
fn check_rel_l2(
    label: &str,
    ref_q: &[f32], ref_k: &[f32], ref_v: &[f32],
    got_q: &[f32], got_k: &[f32], got_v: &[f32],
) {
...
fn qkv_rope_append_f16s_2r_vs_4r_agree() {
    ...
    let x: Vec<f32> = (0..shape.cols)
        .map(|i| ((i as u32).wrapping_mul(1664525).wrapping_add(0xD500) as f32 / u32::MAX as f32) * 2.0 - 1.0)
        .collect();
