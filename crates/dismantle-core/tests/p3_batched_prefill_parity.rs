...
//! `DISMANTLE_QWEN_BATCH_PREFILL=1`) produces the same token sequence
...
fn batched_prefill_matches_single_token_prefill() {
    ...
    std::env::remove_var("DISMANTLE_QWEN_BATCH_PREFILL");
    let baseline = run_greedy();

    // Path B: batched B=4 TCB prefill.
    std::env::set_var("DISMANTLE_QWEN_BATCH_PREFILL", "1");
