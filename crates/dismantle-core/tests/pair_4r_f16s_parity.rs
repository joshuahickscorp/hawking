...
fn run_f32_4r(
    ctx: &MetalContext,
    model: &[u8],
    g_off: usize, g_len: usize,
    u_off: usize, u_len: usize,
    g_scales: &[f32], u_scales: &[f32],
    x: &[f32],
    ...
    kernels::gemv_q4_k_v4_predec_pair_4r_pinned_tcb(
        &mut tcb, &model_buf,
        g_off, g_len, &gs_buf, 0,
        u_off, u_len, &us_buf, 0,
        rows, cols, &x_buf, &g_out, &u_out,
    ).unwrap();
...
fn run_f16s_4r(
    ...
    g_off: usize, g_len: usize,
    u_off: usize, u_len: usize,
    g_scales_f16: &[u8], u_scales_f16: &[u8],
    x: &[f32],
    rows: usize, cols: usize,
) -> (Vec<f32>, Vec<f32>) {
    let model_buf = ctx.new_buffer_with_bytes(model);
...
fn pair_4r_f16s_rel_l2_quality_gate() {
    ...
    for &(rows, cols, seed) in cases {
        ...
        let (ref_g, ref_u) = run_f32_4r(ctx, &model, g_off, g_w.len(), u_off, u_w.len(),
                                         &g_sc, &u_sc, &x, rows, cols);
        let (got_g, got_u) = run_f16s_4r(ctx, &model, g_off, g_w.len(), u_off, u_w.len(),
                                          &g_sc_f16, &u_sc_f16, &x, rows, cols);
