...
//!   cargo test -p dismantle-core --features tq --test rwkv7_tq_loader -- --nocapture --ignored
//! ```
...
fn tq_artifact_loads_expected_names() {
    ...
    // dismantle_core::model::rwkv7::load_tq_artifact(&path)
    let _ = &path; // placeholder until the real loader is available
    panic!("STUB: wire dismantle_core::model::rwkv7::load_tq_artifact and check tensor names");

    // Expected structure once wired:
    //   let artifact = dismantle_core::model::rwkv7::load_tq_artifact(&path)
...
fn tq_artifact_shapes_match_04b() {
    ...
    panic!("STUB: wire dismantle_core::model::rwkv7::load_tq_artifact and check shapes");
