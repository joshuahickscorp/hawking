...
//! there is no IR/graph (dismantle decode is imperative), so routing is a
//! single `supports()` check per op. dismantle has no `ggml_backend_sched`
...
//! ```
//!
//! gated by the string env [`FORCE_CPU_OP_ENV`] (`DISMANTLE_FORCE_CPU_OP`,
...
pub const FORCE_CPU_OP_ENV: &str = "DISMANTLE_FORCE_CPU_OP";
...
impl Router {
    ...
    fn flush_and_reset<'a>(
        &self,
        rec: &mut MetalRecorder<'a>,
        ctx: &'a MetalContext,
    ) -> Result<()> {
