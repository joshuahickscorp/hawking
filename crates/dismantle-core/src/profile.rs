...
pub struct RuntimeLevers {
    ...
    /// Vocab-prune target size (`DISMANTLE_QWEN_VOCAB_PRUNE=N`), or None when
    ...
    /// ffn_down requantized Q6_K->Q4_K (`DISMANTLE_QWEN_FFN_DOWN_Q4K=1`).
    #[serde(default)]
    pub ffn_down_q4k: bool,
    /// Activation/predec scale dtype: "f16" when `DISMANTLE_QWEN_PREDEC_F16SCALES=1`,
    /// else "f32".
...
impl RuntimeLevers {
    ...
    ///   * vocab_prune  — `DISMANTLE_QWEN_VOCAB_PRUNE` (parse usize; >0 wins;
    ///                    "0"/empty/unparseable => None), matching qwen_dense.rs.
    ///   * ffn_down_q4k — `DISMANTLE_QWEN_FFN_DOWN_Q4K` via env_on.
    ///   * scale_dtype  — `DISMANTLE_QWEN_PREDEC_F16SCALES` via env_on => "f16".
    ///   * kv_dtype     — `DISMANTLE_QWEN_INT4_KV` => "int4", else
    ///                    `DISMANTLE_QWEN_F16_KV` => "f16", else "f32".
    ...
    pub fn from_env(profile_name: &str) -> Self {
        let vocab_prune = std::env::var("DISMANTLE_QWEN_VOCAB_PRUNE")
            .ok()
            .filter(|v| v != "0" && !v.is_empty())
            .and_then(|v| v.parse::<usize>().ok())
            .filter(|&n| n > 0);
        let ffn_down_q4k = crate::env_on("DISMANTLE_QWEN_FFN_DOWN_Q4K");
        let f16_scales = crate::env_on("DISMANTLE_QWEN_PREDEC_F16SCALES");
        let scale_dtype = if f16_scales { "f16" } else { "f32" }.to_string();
        let kv_dtype = if crate::env_on("DISMANTLE_QWEN_INT4_KV") {
            "int4"
        } else if crate::env_on("DISMANTLE_QWEN_F16_KV") {
            "f16"
        } else {
            "f32"
        }
        ...
        let q4k_head = crate::env_on("DISMANTLE_QWEN_Q4K_LMHEAD");
...
mod tests {
    ...
    fn build_deterministic_profile_populates_runtime_levers() {
        ...
        std::env::set_var("DISMANTLE_QWEN_PREDEC_F16SCALES", "1");
        std::env::set_var("DISMANTLE_QWEN_Q4K_LMHEAD", "1");
        std::env::set_var("DISMANTLE_QWEN_F16_KV", "1");
        std::env::remove_var("DISMANTLE_QWEN_INT4_KV");
        std::env::remove_var("DISMANTLE_QWEN_Q4K_PREDEC"); // default-on
        ...
        std::env::remove_var("DISMANTLE_QWEN_PREDEC_F16SCALES");
        std::env::remove_var("DISMANTLE_QWEN_Q4K_LMHEAD");
        std::env::remove_var("DISMANTLE_QWEN_F16_KV");
