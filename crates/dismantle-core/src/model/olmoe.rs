...
impl OlmoeConfig {
    pub fn from_gguf(g: &GgufFile) -> Result<Self> {
        ...
        let n_layers = get_u32("llama.block_count")
            .ok_or_else(|| Error::Model("olmoe: missing llama.block_count".into()))?
            as usize;
        let hidden = get_u32("llama.embedding_length")
            .ok_or_else(|| Error::Model("olmoe: missing llama.embedding_length".into()))?
            as usize;
        let n_heads = get_u32("llama.attention.head_count")
            .ok_or_else(|| Error::Model("olmoe: missing llama.attention.head_count".into()))?
            as usize;
        let n_kv_heads =
        ...
        let intermediate = get_u32("llama.feed_forward_length")
            .ok_or_else(|| Error::Model("olmoe: missing llama.feed_forward_length".into()))?
            as usize;
        let n_experts = get_u32("llama.expert_count")
            .ok_or_else(|| Error::Model("olmoe: missing llama.expert_count".into()))?
            as usize;
        ...
        if hidden % n_heads != 0 {
            return Err(Error::Model(format!(
                "olmoe: hidden={hidden} not divisible by n_heads={n_heads}"
            )));
        }
...
impl OlmoeEngine {
    ...
    fn fused_expert_refs(g: &GgufFile, name: &str, n_experts: usize) -> Result<Vec<TensorRef>> {
        let info = g
            .tensor(name)
            .ok_or_else(|| Error::Model(format!("olmoe: missing tensor `{name}`")))?;
        ...
        if total_elems % n_experts != 0 {
            return Err(Error::Model(format!(
                "olmoe tensor {name}: {total_elems} elems not divisible by {n_experts}"
            )));
        }
        if total_bytes % n_experts != 0 {
            return Err(Error::Model(format!(
                "olmoe tensor {name}: {total_bytes} bytes not divisible by {n_experts}"
            )));
        }
...
impl Engine for OlmoeEngine {
    ...
    fn load(weights: &Path, config: EngineConfig) -> Result<Self> {
        ...
        if arch != "olmoe" {
            return Err(Error::Model(format!(
                "olmoe loader: expected arch 'olmoe', got '{arch}'"
            )));
        }
        ...
        let sidecar = weights
            ...
            .filter(|p| p.exists());
        let tokenizer = if let Some(path) = sidecar {
        ...
        eprintln!(
            "[olmoe] loaded {} layers, {} experts top-{} (CPU path)",
