...
impl Mamba2LayerState {
    fn new(conv_kernel: usize, conv_width: usize, n_heads: usize, head_dim: usize, state_size: usize) -> Self {
        Self {
            ...
            conv_pos: 0,
            ssm_state: vec![0.0f32; n_heads * head_dim * state_size],
        }
    }
    ...
    fn forward_token(&mut self, token: u32) -> Result<Vec<f32>> {
        ...
        for li in 0..cfg.n_layers {
            ...
            // 5. Conv ring buffer update + depthwise conv.
            {
                ...
                state.conv_ring[pos * conv_width..pos * conv_width + conv_width]
                    .copy_from_slice(xbc);
...
impl Engine for Mamba2 {
    fn load(weights: &Path, config: EngineConfig) -> Result<Self> {
        ...
        let lm_head = if gguf.tensor("output.weight").is_some() {
            Some(super::weights::dequant_f16(&gguf, "output.weight")?)
        } else {
            None
        };
        ...
        let states: Vec<Mamba2LayerState> = (0..cfg.n_layers)
            .map(|_| {
                Mamba2LayerState::new(
                    cfg.conv_kernel,
                    cfg.conv_width(),
                    cfg.n_heads,
                    cfg.head_dim,
        ...
        #[cfg(target_os = "macos")]
        if !config.force_cpu {
            let ctx_result = if config.trace_dispatch {
            ...
            match ctx_result {
                ...
                Err(e) => eprintln!("dismantle: mamba2 Metal init failed ({e}), using CPU"),
            }
        }
