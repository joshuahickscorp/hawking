//! The one typed envelope for everything: events, state, programs, receipts, source and artifact
//! identities, readiness. Canonical serialization + identity + seal + parent linkage in one engine.
...
pub fn canonical(value: &serde_json::Value) -> String {
    // serde_json with a BTreeMap round-trip yields deterministic key ordering.
...
mod tests {
    ...
    fn seal_roundtrip_and_tamper() {
        let r = Record::new("program", serde_json::json!({"parent":"120B","rate":"4/5"})).sealed();
        assert!(r.verify().is_ok());
        // tamper the payload → seal must fail
        let mut t = r.clone();
        t.payload = serde_json::json!({"parent":"120B","rate":"1/1"});
        assert!(t.verify().is_err());
    }

    #[test]
    fn identity_is_content_addressed_and_canonical() {
        // key order in the payload must not change identity (canonical sort)
        let a = Record::new("event", serde_json::json!({"a":1,"b":2}));
        let b = Record::new("event", serde_json::json!({"b":2,"a":1}));
