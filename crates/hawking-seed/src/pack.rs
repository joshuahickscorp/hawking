...
//! plugin execution: Candidate A uses content-addressed verified packs (static registration).
...
pub struct PackManifest {
    ...
    /// Where the pack's content lives (offline cache dir), if separate from the manifest.
    #[serde(default)]
...
impl PackManifest {
    pub fn load(manifest_path: impl AsRef<Path>) -> Result<Self> {
        let s = std::fs::read_to_string(manifest_path.as_ref())?;
    ...
    /// Compatibility gate against the Seed's kernel compat token.
...
mod tests {
    ...
    fn scratch() -> PathBuf {
        let d = std::env::temp_dir().join(format!("seed-pack-{}", std::process::id()));
    ...
    fn verifies_offline_and_rejects_tamper_and_rollback() {
        let d = scratch();
        let sha = write(&d, "impl.rs", b"fn forge() {}\n");
        let man = PackManifest {
            pack: "hawking-forge-default".into(),
            version: "1.0.0".into(),
            compatibility: "seed-1".into(),
            source_commit: "abc".into(),
            offline_cache: d.to_string_lossy().into(),
            contents: vec![PackEntry { path: "impl.rs".into(), sha256: sha.clone() }],
        };
        assert_eq!(man.verify(d.join("manifest.json")).unwrap(), 1);
        assert!(man.compatible_with("seed-1"));
        // tamper the content → verify must fail (rollback = restore + re-verify)
        write(&d, "impl.rs", b"fn forge() { evil() }\n");
        assert!(man.verify(d.join("manifest.json")).is_err());
        write(&d, "impl.rs", b"fn forge() {}\n"); // rollback
