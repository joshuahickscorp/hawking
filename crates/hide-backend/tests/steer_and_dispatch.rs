...
async fn goal_ckpt_intents() {
    let (d,h)=open_host("gci"); let s=h.services.session();
    assert!(!custom(&h,"goal_set",json!({"session_id":s.as_str(),"condition":"t"})).await.message.unwrap_or_default().contains("has no host handler"));
    assert!(!custom(&h,"checkpoint_create",json!({"session_id":s.as_str(),"label":"L"})).await.message.unwrap_or_default().contains("has no host handler"));
    cleanup(d);
}
#[tokio::test]
async fn checkpoint_create_release() {
    let (d,h)=open_host("cc"); let s=h.services.session();
    h.services.event_log.append(NewEvent::system(s.clone(),"t",json!({}))).await.unwrap();
    if let Ok(c)=h.checkpoint_create(s.clone(),None,"t1").await { let _=h.checkpoint_release(&c.checkpoint_id); }
    cleanup(d);
}
#[tokio::test]
async fn rewind_intent_handled() {
    let (d,h)=open_host("rw");
    assert!(!custom(&h,"checkpoint_rewind",json!({"session_id":h.services.session().as_str()})).await.message.unwrap_or_default().contains("has no host handler"));
    cleanup(d);
}
#[tokio::test]
async fn side_chat_create() {
    let (d,h)=open_host("sc"); let p=h.services.session();
    let (side,rec,_)=h.create_side_chat(p.clone(),None,true).await.unwrap();
    assert_ne!(side,p); let _=rec; cleanup(d);
}
#[tokio::test]
async fn side_chat_discard_parent_stable() {
    let (d,h)=open_host("sd"); let p=h.services.session();
    h.services.event_log.append(NewEvent::system(p.clone(),"t",json!({}))).await.unwrap();
    let b=h.services.event_log.scan(Some(p.clone()),None,None).await.unwrap().len();
    let _=h.create_side_chat(p.clone(),None,true).await.unwrap();
    assert_eq!(b, h.services.event_log.scan(Some(p.clone()),None,None).await.unwrap().len()); cleanup(d);
}
#[tokio::test]
async fn merge_side_chat() {
    let (d,h)=open_host("msc"); let p=h.services.session();
    let (side,_,_)=h.create_side_chat(p.clone(),None,true).await.unwrap();
    let b=h.services.event_log.scan(Some(p.clone()),None,None).await.unwrap().len();
    let _=h.merge_side_chat_summary(side,p.clone(),"sum").await;
    assert!(h.services.event_log.scan(Some(p.clone()),None,None).await.unwrap().len() >= b); cleanup(d);
}
#[tokio::test]
async fn fork_source_stable() {
    let (d,h)=open_host("fk"); let s=h.services.session();
    let ev=h.services.event_log.append(NewEvent::system(s.clone(),"t",json!({}))).await.unwrap();
    let b=h.services.event_log.scan(Some(s.clone()),None,None).await.unwrap().len();
    let _=h.fork_session_from_event(s.clone(),Some(&ev.id)).await;
    assert_eq!(b, h.services.event_log.scan(Some(s.clone()),None,None).await.unwrap().len()); cleanup(d);
}
#[tokio::test]
async fn fork_intent() {
    let (d,h)=open_host("fi"); let s=h.services.session();
    let ev=h.services.event_log.append(NewEvent::system(s.clone(),"t",json!({}))).await.unwrap();
    assert!(h.handle_intent(Intent::ForkSession{session_id:s,at_event:ev.id}).await.unwrap().accepted); cleanup(d);
}
#[tokio::test]
async fn search_transcript() {
    let (d,h)=open_host("stx"); let s=h.services.session();
    h.services.event_log.append(NewEvent::system(s.clone(),"agent.message",json!({"text":"ZZALPHA"}))).await.unwrap();
    let hits=h.search_transcript(&TranscriptQuery{text:"ZZALPHA".into(),session_id:Some(s),..Default::default()}).await.unwrap();
    assert!(hits.is_empty() || hits.iter().any(|h| h.snippet.contains("ZZALPHA"))); cleanup(d);
}
#[tokio::test]
async fn conversation_graph() {let (d,h)=open_host("cg"); let _=h.conversation_graph(&h.services.session()); cleanup(d);}
#[tokio::test]
async fn plan_mutations() {
    let (d,h)=open_host("pln"); let s=h.services.session();
    let a=PlanStep::new("investigate",StepKind::Investigate,Acceptance::predicate("x"));
    let b=PlanStep::new("edit",StepKind::Edit,Acceptance::predicate("y"));
    let aid=a.id.clone(); let bid=b.id.clone();
    let plan=Plan{id:PlanId::new(),title:"t".into(),objective:"o".into(),steps:vec![a,b],status:PlanStatus::Active,budget:Default::default()};
    h.publish_plan(&s,&plan,Autonomy::SuggestOnly).unwrap();
    assert!(custom(&h,"approve_plan",json!({"session_id":s.as_str()})).await.accepted);
    custom(&h,"edit_plan_step",json!({"session_id":s.as_str(),"step_id":aid.as_str(),"text":"dig"})).await;
    assert_eq!(h.plan_get(&s).unwrap().steps[0].text,"dig");
    custom(&h,"reorder_plan",json!({"session_id":s.as_str(),"order":[bid.as_str(),aid.as_str()]})).await;
    assert_eq!(h.plan_get(&s).unwrap().steps[0].id, bid.as_str()); cleanup(d);
}
#[tokio::test]
async fn plan_none() {let (d,h)=open_host("p0"); assert!(h.plan_get(&h.services.session()).is_none()); cleanup(d);}
#[tokio::test]
async fn approve_effect_handled() {
    let (d,h)=open_host("ae");
    assert!(!custom(&h,"approve_effect",json!({})).await.message.unwrap_or_default().contains("has no host handler")); cleanup(d);
}
#[tokio::test]
async fn deny_effect_handled() {
    let (d,h)=open_host("de");
    assert!(!custom(&h,"deny_effect",json!({"step_id":"s"})).await.message.unwrap_or_default().contains("has no host handler")); cleanup(d);
}
#[tokio::test]
async fn lease_intents() {
    let (d,h)=open_host("le");
    assert!(!custom(&h,"grant_write_lease",json!({"session_id":h.services.session().as_str(),"paths":[d.to_string_lossy()]})).await.message.unwrap_or_default().contains("has no host handler"));
    assert!(!custom(&h,"revoke_write_lease",json!({})).await.message.unwrap_or_default().contains("has no host handler")); cleanup(d);
}
#[tokio::test]
async fn hubs_accessible() {
    let (d,h)=open_host("hb"); let _=h.processes(); let _=h.interrupts(); let _=h.approvals(); let _=h.ui_bus(); cleanup(d);
}
#[tokio::test]
async fn editor_run_stable() {
    let (d,h)=open_host("er"); let s=h.services.session();
    assert_eq!(BackendHost::editor_run(&s).as_str(), BackendHost::editor_run(&s).as_str()); cleanup(d);
}
#[tokio::test]
async fn projection_rebuild() {
    let (d,h)=open_host("pj"); let s=h.services.session();
    assert_eq!(h.rebuild_session_projection(s.clone()).await.unwrap().session_id, s); cleanup(d);
}
#[tokio::test]
async fn initialize_client() {
    let (d,h)=open_host("in");
    let _=h.initialize("c", ClientInfo{name:"s3b".into(),version:"0".into(),title:None}, ClientCapabilities::default());
    let _=h.connections(); cleanup(d);
}
#[tokio::test]
async fn workspace_graph() {let (d,h)=open_host("wg"); let _=h.workspace_graph().repos.len(); cleanup(d);}
#[tokio::test]
async fn trust_intent() {
    let (d,h)=open_host("ti");
    assert!(!custom(&h,"workspace_set_repo_trust",json!({"repo_id":"l","trust":"trusted","root_path":d.to_string_lossy()})).await.message.unwrap_or_default().contains("has no host handler"));
    cleanup(d);
}
#[tokio::test]
async fn env_switch_intent() {
    let (d,h)=open_host("ei");
    assert!(!custom(&h,"environment_switch",json!({"session_id":h.services.session().as_str(),"env":"dev","reason":"r"})).await.message.unwrap_or_default().contains("has no host handler"));
    cleanup(d);
}
#[tokio::test]
async fn unknown_connector_err() {let (d,h)=open_host("uc"); assert!(h.call_connector("nope","x",json!({})).await.is_err()); cleanup(d);}
#[tokio::test]
async fn verification_receipts() {let (d,h)=open_host("vr"); let _=h.verification_receipts(&h.services.session()).await.unwrap(); cleanup(d);}
#[tokio::test]
async fn diff_missing() {let (d,h)=open_host("dm"); assert!(h.diff_get("nope").is_none()); cleanup(d);}
#[tokio::test]
async fn bg_job_missing() {let (d,h)=open_host("bg"); assert!(h.background_job_for_run(&RunId::new()).is_none()); cleanup(d);}
#[tokio::test]
async fn live_thread() {let (d,h)=open_host("lt"); let _=h.open_live_thread(h.services.session()); cleanup(d);}
#[tokio::test]
async fn policy_empty() {let (d,h)=open_host("pe0"); assert!(h.policy_decisions(&h.services.session()).await.unwrap().is_empty()); cleanup(d);}
#[tokio::test]
async fn ui_events() {let (d,h)=open_host("ue"); let _=h.ui_events(None,None,None).await.unwrap(); cleanup(d);}
#[tokio::test]
...
async fn steer_intent() {
    ...
    assert!(!custom(&h,"steer",json!({"run_id":"r","text":"t"})).await.message.unwrap_or_default().contains("has no host handler")); cleanup(d);
}
#[tokio::test]
async fn search_intent() {
    let (d,h)=open_host("sei");
    assert!(!custom(&h,"search",json!({"query":"x","session_id":h.services.session().as_str()})).await.message.unwrap_or_default().contains("has no host handler")); cleanup(d);
}
#[tokio::test]
async fn create_worktree_intent() {
    let (d,h)=open_host("cwt"); let _=std::process::Command::new("git").args(["init","-q"]).current_dir(&d).status();
    assert!(!custom(&h,"create_worktree",json!({"path":d.join("wt").to_string_lossy()})).await.message.unwrap_or_default().contains("has no host handler")); cleanup(d);
}
#[tokio::test]
async fn status_tools_nonempty() {let (d,h)=open_host("stn"); assert!(!h.status().await.tools.is_empty()); cleanup(d);}
#[tokio::test]
async fn status_connectors_nonempty() {let (d,h)=open_host("scn"); assert!(!h.status().await.connectors.is_empty()); cleanup(d);}
#[tokio::test]
async fn status_roles_nonempty() {let (d,h)=open_host("srn"); assert!(!h.status().await.model_roles.is_empty()); cleanup(d);}
#[tokio::test]
async fn health_personalization() {
    let (d,h)=open_host("hp"); let health=h.health().await;
    assert!(health.checks.iter().any(|c| c.name.contains("personalization")||c.name.contains("connector"))); cleanup(d);
}
#[tokio::test]
async fn fs_read_tool() {
    let (dir,h)=open_host_allow_write("fr"); let f=dir.join("r.txt"); std::fs::write(&f,"data").unwrap();
    let r=h.dispatch_tool(h.services.session(),None,ToolCall::new("fs.read",json!({"path":f.to_string_lossy()}))).await.unwrap();
    assert_eq!(r.status, ToolStatus::Ok); cleanup(dir);
}
#[tokio::test]
async fn memory_add_intent() {
    let (d,h)=open_host("mai");
    assert!(!custom(&h,"memory_add",json!({"claim":"c","scope":{"repo":"r"}})).await.message.unwrap_or_default().contains("has no host handler")); cleanup(d);
}
#[tokio::test]
async fn goal_evaluate_intent() {
    let (d,h)=open_host("gei");
    assert!(!custom(&h,"goal_evaluate",json!({"session_id":h.services.session().as_str()})).await.message.unwrap_or_default().contains("has no host handler")); cleanup(d);
}
#[tokio::test]
async fn goal_clear_intent() {
    let (d,h)=open_host("gci2");
    assert!(!custom(&h,"goal_clear",json!({"session_id":h.services.session().as_str()})).await.message.unwrap_or_default().contains("has no host handler")); cleanup(d);
}
#[tokio::test]
async fn create_side_chat_intent() {
    let (d,h)=open_host("csi");
    assert!(!custom(&h,"create_side_chat",json!({"session_id":h.services.session().as_str()})).await.message.unwrap_or_default().contains("has no host handler")); cleanup(d);
}
#[tokio::test]
async fn invalidated_verifs() {let (d,h)=open_host("iv"); let _=h.invalidated_verification_ids(&h.services.session()).await.unwrap(); cleanup(d);}
#[tokio::test]
async fn diff_review_receipts() {let (d,h)=open_host("drr"); let _=h.diff_review_receipts(&h.services.session()).await.unwrap(); cleanup(d);}
#[tokio::test]
async fn write_lease_query() {let (d,h)=open_host("wlq"); let _=h.write_lease(); cleanup(d);}
#[tokio::test]
async fn safe_true_cmd() {
    let (d,h)=open_host_allow_shell("true"); assert!(h.handle_intent(Intent::RunCommand{argv:vec!["true".into()],cwd:None}).await.unwrap().accepted && !h.handle_intent(Intent::RunCommand{argv:vec!["true".into()],cwd:None}).await.unwrap().held); cleanup(d);
}
#[tokio::test]
async fn smoke_approve_effect() {
    ...
    assert!(!msg.contains("has no host handler"), "approve_effect: {msg}");
    cleanup(dir);
}
#[tokio::test]
