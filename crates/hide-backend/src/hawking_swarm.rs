//! Parallel, read-safe HAWKING analysis swarms.
//!
...
//! workspace.  The normal HAWKING profile therefore remains `SuggestOnly`: this
...
pub const HAWKING_SWARM_RECEIPT_SCHEMA: &str = "hawking.parallel_analysis_swarm.v1";
...
/// Configuration for a set of independently driven HAWKING agent lanes.
#[derive(Debug, Clone)]
pub struct HawkingSwarmConfig {
    pub goal: String,
...
impl Default for HawkingSwarmConfig {
    fn default() -> Self {
        let profile = HawkingProfile::Power;
        Self {
            goal: String::new(),
            model_url: None,
            profile,
...
pub async fn run_parallel_analysis_swarm(
    host: &BackendHost,
    config: HawkingSwarmConfig,
) -> Result<HawkingSwarmResult> {
    ...
    let mut receipt = json!({
        "schema": HAWKING_SWARM_RECEIPT_SCHEMA,
        ...
        "artifact_manifest": {
            "schema": "hawking.swarm.artifact_manifest.v1",
...
async fn synthesize_lane_receipts(base_url: &str, prompt: String) -> Result<SynthesisOutcome> {
    ...
    let request = InferenceRequest {
        task_kind: "hawking.swarm.reducer".to_string(),
        prompt,
        messages: Vec::new(),
