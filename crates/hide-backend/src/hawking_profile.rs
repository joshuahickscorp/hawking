//! Named, bounded compute profiles for HAWKING agent runs.
//!
//! A profile changes **how much exploration** an HAWKING run is allowed to use;
...
//! [`HawkingProfileSpec`] in its receipt and report realized subagent/model-call
//! counts separately.
...
/// HAWKING's explicit compute profiles, from everyday local use to a bounded
/// long-running research run.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum HawkingProfile {
...
impl Default for HawkingProfile {
    fn default() -> Self {
        Self::Balanced
    }
}
...
impl HawkingProfile {
    /// Stable CLI spellings accepted by HAWKING.
    ...
    /// A transparent snapshot suitable for `hawking run --json` and receipts.
    pub fn spec(self) -> HawkingProfileSpec {
...
impl std::fmt::Display for HawkingProfile {
...
mod tests {
    ...
    fn profile_parser_accepts_only_named_safe_presets() {
        assert_eq!(
            HawkingProfile::parse("balanced"),
            Some(HawkingProfile::Balanced)
        );
        assert_eq!(
            HawkingProfile::parse(" DEFAULT "),
            Some(HawkingProfile::Balanced)
        );
        assert_eq!(HawkingProfile::parse("POWER"), Some(HawkingProfile::Power));
        assert_eq!(HawkingProfile::parse("max"), Some(HawkingProfile::Maximum));
        assert_eq!(HawkingProfile::parse("unlimited"), None);
        assert_eq!(HawkingProfile::parse("full_auto"), None);
    }

    #[test]
    fn profiles_scale_compute_but_keep_effects_suggest_only() {
        let balanced = HawkingProfile::Balanced.spec();
        let power = HawkingProfile::Power.spec();
        let maximum = HawkingProfile::Maximum.spec();
    ...
    fn every_profile_has_finite_governor_caps() {
        for profile in HawkingProfile::ALL {
    ...
    fn applying_a_profile_changes_only_the_budget() {
        ...
        HawkingProfile::Maximum.apply_to_state(&mut state);

        assert_eq!(state.budget, HawkingProfile::Maximum.budget());
