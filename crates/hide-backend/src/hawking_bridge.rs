//! Transport-neutral JSONL machine-control contract for HAWKING.
//!
//! This module deliberately does **not** execute a model, start an agent, or
//! call the existing turn path.  It is the small, stable seam a future `hawking`
...
//! responsible for turning a validated [`HawkingOperation`] into a durable host
...
//!
//! ```json
//! {"protocol":"hawking.machine.v1","id":"req-7","method":"generate","params":{"prompt":"Explain this repository","runtime_url":"http://127.0.0.1:8787","durable":true}}
//! ```
//!
...
use crate::hawking_profile::HawkingProfile;
use crate::hawking_sources::MAX_CONTEXT_SOURCES;
...
pub const HAWKING_JSONL_PROTOCOL: &str = "hawking.machine.v1";
/// Input-only spelling used by persisted pre-consolidation JSONL records.
/// Parsing normalizes it immediately; newly emitted records always use
/// [`HAWKING_JSONL_PROTOCOL`].
const LEGACY_HCLI_JSONL_PROTOCOL: &str = "hcli.machine.v1";
/// Schema for the small, stable model-optimization identity advertised by
/// HAWKING capability surfaces. The canonical values are derived from the shared
/// `hawking/semantic_taxonomy.json` contract rather than redefined here.
pub const HAWKING_MODEL_OPTIMIZATION_IDENTITY_SCHEMA: &str =
    "hawking.model_optimization.identity.v1";
/// Schema for HAWKING's explicit non-execution boundary around Gravity/Condense.
///
/// This is deliberately separate from the shared Hawking taxonomy: the
/// taxonomy names the canonical artifact/runtime identity, while this HAWKING
/// contract states what this particular control surface can *not* execute.
pub const HAWKING_MODEL_OPTIMIZATION_EXECUTION_BOUNDARY_SCHEMA: &str =
    "hawking.model_optimization.execution_boundary.v1";
/// Stable JSONL error-code spelling returned when a caller asks HAWKING itself
/// to run Condense/Gravity model optimization.
pub const HAWKING_MODEL_OPTIMIZATION_NOT_AVAILABLE_CODE: &str = "model_optimization_not_available";
const HAWKING_MODEL_OPTIMIZATION_NOT_AVAILABLE_MESSAGE: &str =
    "HAWKING reports Gravity/Condense identity and planning boundaries, but cannot execute model optimization, Condense, packing, or artifact creation";
...
/// The HAWKING-facing projection of Hawking's canonical Gravity/Condense
...
pub struct HawkingModelOptimizationIdentity {
    pub schema: String,
    pub taxonomy_schema: String,
    pub canonical_identity: String,
    pub canonical_display_name: String,
    pub public_cli: HawkingGravityPublicCli,
    pub condense_engine_operation: HawkingCondenseEngineOperation,
    pub claim_limits: HawkingModelOptimizationClaimLimits,
    /// A typed, stable refusal contract.  This prevents consumers from
    /// inferring execution capability merely because HAWKING exposes the public
    /// Gravity naming and Hawking command references.
    pub execution_boundary: HawkingModelOptimizationExecutionBoundary,
    /// Compact forward-only projection of the taxonomy's legacy Condense
    /// schema/command mapping.  A taxonomy-derived default lets older HAWKING
    /// identity records decode and validate without pretending their legacy
    /// fields were absent from the current contract.
    #[serde(default)]
    pub legacy_compatibility: HawkingLegacyCompatibility,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct HawkingGravityPublicCli {
    pub command: String,
    ...
    pub compatibility_commands: Vec<HawkingGravityCompatibilityCommand>,
}

/// A retained compatibility spelling. It is descriptive only; callers should
/// prefer [`HawkingGravityPublicCli::command`] for new automation.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct HawkingGravityCompatibilityCommand {
    pub command: String,
    pub status: String,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct HawkingCondenseEngineOperation {
    pub id: String,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct HawkingModelOptimizationClaimLimits {
    #[serde(alias = "hcli_executes_optimization")]
    pub hawking_executes_optimization: bool,
    pub v4_runtime_claim: bool,
    pub artifact_identity_is_not_capability_evidence: bool,
}

/// HAWKING's current execution boundary for Gravity/Condense work.
///
/// `available=false` is a product capability fact, not a transient runtime
/// health state.  JSONL callers can use [`HawkingErrorCode::ModelOptimizationNotAvailable`]
/// and this fixed `refusal_code` instead of scraping explanatory prose.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct HawkingModelOptimizationExecutionBoundary {
    pub schema: String,
    pub available: bool,
...
/// HAWKING's projection of the shared Condense-to-Gravity compatibility mapping.
///
/// The values below are copied from `hawking/semantic_taxonomy.json`; this type is
/// deliberately descriptive and never rewrites a historical receipt or turns
/// a legacy command into an HAWKING optimization operation.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct HawkingLegacyCompatibility {
    ...
    pub schema_aliases: Vec<HawkingLegacySchemaAlias>,
    /// Deprecated command aliases retained by Hawking for compatibility.
    pub command_aliases: Vec<HawkingLegacyCommandAlias>,
}

/// One legacy schema namespace projected from the taxonomy.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct HawkingLegacySchemaAlias {
    pub prefix: String,
    pub status: String,
    pub deprecated: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct HawkingLegacyCommandAlias {
    pub command: String,
    pub status: String,
    pub deprecated: bool,
    pub superseded_by: String,
...
fn hawking_semantic_taxonomy() -> Value {
    serde_json::from_str(include_str!("../../../hawking/semantic_taxonomy.json"))
        .expect("checked-in Hawking semantic taxonomy must be valid JSON")
}

fn hawking_legacy_compatibility_from_taxonomy(taxonomy: &Value) -> HawkingLegacyCompatibility {
...
impl Default for HawkingLegacyCompatibility {
    fn default() -> Self {
        hawking_legacy_compatibility_from_taxonomy(&hawking_semantic_taxonomy())
    }
}

/// Build HAWKING's stable model-optimization identity from the shared semantic
...
pub fn hawking_model_optimization_identity() -> HawkingModelOptimizationIdentity {
    let taxonomy = hawking_semantic_taxonomy();
    ...
    let hawking_executes_optimization =
        boolean("/capability_claim_limits/hawking_executes_optimization");
    if hawking_executes_optimization {
        panic!(
            "HAWKING's model-optimization execution boundary must be deliberately implemented before the shared taxonomy can advertise execution"
        );
    }
    HawkingModelOptimizationIdentity {
        schema: HAWKING_MODEL_OPTIMIZATION_IDENTITY_SCHEMA.to_string(),
        ...
        public_cli: HawkingGravityPublicCli {
            ...
            compatibility_commands: taxonomy
                ...
                .map(|(index, command)| {
                    ...
                    HawkingGravityCompatibilityCommand {
        ...
        condense_engine_operation: HawkingCondenseEngineOperation {
        ...
        claim_limits: HawkingModelOptimizationClaimLimits {
            hawking_executes_optimization,
        ...
        execution_boundary: HawkingModelOptimizationExecutionBoundary {
            schema: HAWKING_MODEL_OPTIMIZATION_EXECUTION_BOUNDARY_SCHEMA.to_string(),
            available: false,
            refusal_code: HAWKING_MODEL_OPTIMIZATION_NOT_AVAILABLE_CODE.to_string(),
            message: HAWKING_MODEL_OPTIMIZATION_NOT_AVAILABLE_MESSAGE.to_string(),
            canonical_hawking_plan_command: string(
                "/canonical_identity/public_cli/plan_command",
            ),
        },
        legacy_compatibility: hawking_legacy_compatibility_from_taxonomy(&taxonomy),
    }
}

impl Default for HawkingModelOptimizationIdentity {
    fn default() -> Self {
        hawking_model_optimization_identity()
    }
}

impl HawkingModelOptimizationIdentity {
    fn validate(&self) -> HawkingBridgeResult<()> {
        let expected = hawking_model_optimization_identity();
        if self != &expected {
            return Err(HawkingBridgeError::validation(
...
/// Errors returned while parsing or validating an HAWKING JSONL record.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum HawkingBridgeError {
    /// The line was not a single valid JSON object matching the wire shape.
    Parse { message: String },
    /// A syntactically valid record violated an HAWKING contract invariant.
    Validation { field: String, message: String },
    /// Serializing a valid request/response failed unexpectedly.
    Encode { message: String },
}

impl HawkingBridgeError {
    ...
    /// Stable machine-readable error family suitable for [`HawkingError`].
    pub const fn code(&self) -> HawkingErrorCode {
        match self {
            Self::Parse { .. } => HawkingErrorCode::ParseError,
            Self::Validation { .. } => HawkingErrorCode::InvalidRequest,
            Self::Encode { .. } => HawkingErrorCode::Internal,
        }
    }
}

impl fmt::Display for HawkingBridgeError {
...
impl std::error::Error for HawkingBridgeError {}

pub type HawkingBridgeResult<T> = std::result::Result<T, HawkingBridgeError>;
...
/// Use [`HawkingRequest::operation`] to obtain the validated typed operation.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct HawkingRequest {
    pub protocol: String,
    pub id: String,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub client: Option<HawkingClient>,
    pub method: HawkingMethod,
...
impl HawkingRequest {
    ...
    pub fn new(id: impl Into<String>, operation: HawkingOperation) -> Self {
        let (method, params) = operation.into_wire();
        Self {
            protocol: HAWKING_JSONL_PROTOCOL.to_string(),
            id: id.into(),
            client: None,
            method,
            params,
    ...
    pub fn parse_jsonl_line(line: &str) -> HawkingBridgeResult<Self> {
        ...
        let mut request: Self = serde_json::from_str(line).map_err(HawkingBridgeError::parse)?;
        normalize_legacy_protocol(&mut request.protocol);
        request.validate()?;
        Ok(request)
    }
    ...
    pub fn to_jsonl_line(&self) -> HawkingBridgeResult<String> {
        self.validate()?;
        serde_json::to_string(self).map_err(HawkingBridgeError::encode)
    }

    /// Validate the envelope and its typed method parameters.
    pub fn validate(&self) -> HawkingBridgeResult<()> {
        if self.protocol != HAWKING_JSONL_PROTOCOL {
            return Err(HawkingBridgeError::validation(
                "protocol",
                format!("expected {HAWKING_JSONL_PROTOCOL:?}"),
            ));
        }
    ...
    pub fn operation(&self) -> HawkingBridgeResult<HawkingOperation> {
        let operation = HawkingOperation::from_wire(self.method, self.params.clone())?;
        operation.validate()?;
        Ok(operation)
    }

    /// Produce an owned, validated request convenient for a dispatcher.
    pub fn into_validated(self) -> HawkingBridgeResult<ValidatedHawkingRequest> {
        self.validate()?;
        let operation = HawkingOperation::from_wire(self.method, self.params)?;
        Ok(ValidatedHawkingRequest {
            id: self.id,
            client: self.client,
            operation,
        })
...
pub struct ValidatedHawkingRequest {
    pub id: String,
    pub client: Option<HawkingClient>,
    pub operation: HawkingOperation,
}
...
pub enum HawkingMethod {
    Capabilities,
    /// A deliberately non-executing request that returns HAWKING's stable
...
/// Typed request variants used by an HAWKING dispatcher after it validates the
/// outer JSONL envelope.
#[derive(Debug, Clone, PartialEq)]
pub enum HawkingOperation {
...
impl HawkingOperation {
    pub const fn method(&self) -> HawkingMethod {
        match self {
            Self::Capabilities(_) => HawkingMethod::Capabilities,
            Self::ModelOptimization(_) => HawkingMethod::ModelOptimization,
            Self::Generate(_) => HawkingMethod::Generate,
            Self::Agent(_) => HawkingMethod::Agent,
            Self::Swarm(_) => HawkingMethod::Swarm,
            Self::Session(_) => HawkingMethod::Session,
            Self::Status(_) => HawkingMethod::Status,
        }
    }

    fn into_wire(self) -> (HawkingMethod, Value) {
    ...
    fn from_wire(method: HawkingMethod, params: Value) -> HawkingBridgeResult<Self> {
        if !params.is_object() {
            return Err(HawkingBridgeError::validation(
                "params",
        ...
        match method {
            HawkingMethod::Capabilities => decode_params(params).map(Self::Capabilities),
            HawkingMethod::ModelOptimization => decode_params(params).map(Self::ModelOptimization),
            HawkingMethod::Generate => decode_params(params).map(Self::Generate),
            HawkingMethod::Agent => decode_params(params).map(Self::Agent),
            HawkingMethod::Swarm => decode_params(params).map(Self::Swarm),
            HawkingMethod::Session => decode_params(params).map(Self::Session),
            HawkingMethod::Status => decode_params(params).map(Self::Status),
        }
    }
...
/// `model_optimization` intentionally takes no controls.  HAWKING must refuse
...
#[serde(rename_all = "snake_case")]
pub enum HawkingAutonomy {
    ReadOnly,
    SuggestOnly,
    FullAuto,
}

impl Default for HawkingAutonomy {
    fn default() -> Self {
        Self::SuggestOnly
    }
}

/// Optional agent-level overrides on top of a named [`HawkingProfile`].  `None`
...
pub struct AgentBudgetOverride {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_context_tokens: Option<u32>,
    /// Informational compute target only.  Zero is rejected here so HAWKING never
...
pub struct AgentRequest {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub model: Option<String>,
    #[serde(default = "default_hawking_profile")]
    pub profile: HawkingProfile,
    #[serde(default)]
    pub budget: AgentBudgetOverride,
    /// Existing workspace-local HAWKING evidence references selected explicitly
    /// for this one agent run.  The adapter resolves them through the same
    /// bounded derivative-only path as `hawking agent --attach`; raw object bytes
    /// and URLs never enter the JSONL request or model prompt.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub attachments: Vec<String>,
    #[serde(default)]
    pub autonomy: HawkingAutonomy,
...
impl AgentRequest {
    pub fn validate(&self) -> HawkingBridgeResult<()> {
        ...
        if matches!(self.autonomy, HawkingAutonomy::FullAuto) && !self.acknowledge_effects {
            return Err(HawkingBridgeError::validation(
...
/// HAWKING response envelope.  `ok` means the control-plane request was accepted
/// and produced a typed result; it does not mean a run reached `completed`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct HawkingResponse {
    pub protocol: String,
    pub id: String,
    pub ok: bool,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub result: Option<HawkingResult>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub error: Option<HawkingError>,
}

impl HawkingResponse {
    pub fn success(id: impl Into<String>, result: HawkingResult) -> HawkingBridgeResult<Self> {
        let response = Self {
            protocol: HAWKING_JSONL_PROTOCOL.to_string(),
            id: id.into(),
            ok: true,
    ...
    pub fn failure(id: impl Into<String>, error: HawkingError) -> HawkingBridgeResult<Self> {
        let response = Self {
            protocol: HAWKING_JSONL_PROTOCOL.to_string(),
            id: id.into(),
            ok: false,
    ...
    pub fn parse_jsonl_line(line: &str) -> HawkingBridgeResult<Self> {
        ...
        let mut response: Self = serde_json::from_str(line).map_err(HawkingBridgeError::parse)?;
        normalize_legacy_protocol(&mut response.protocol);
        response.validate()?;
        Ok(response)
    }
    ...
    pub fn validate(&self) -> HawkingBridgeResult<()> {
        ...
        match (self.ok, &self.result, &self.error) {
            ...
            (true, None, _) => Err(HawkingBridgeError::validation(
                "result",
                "successful response requires one result and no error",
            )),
            (false, _, None) => Err(HawkingBridgeError::validation(
                "error",
                "failed response requires one error and no result",
            )),
            _ => Err(HawkingBridgeError::validation(
                "response",
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum HawkingErrorCode {
    ParseError,
    InvalidRequest,
    Unsupported,
    Unavailable,
    /// Stable, non-retryable refusal for a requested HAWKING-side
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct HawkingError {
    pub code: HawkingErrorCode,
    pub message: String,
    #[serde(default)]
    pub retryable: bool,
}
...
impl HawkingError {
    pub fn invalid_request(message: impl Into<String>) -> Self {
        Self {
            code: HawkingErrorCode::InvalidRequest,
            message: message.into(),
            retryable: false,
        }
    }

    /// The exact error returned by HAWKING's non-executing model-optimization
    /// control path.  Keep this paired with
    /// [`HAWKING_MODEL_OPTIMIZATION_NOT_AVAILABLE_CODE`].
    pub fn model_optimization_not_available() -> Self {
        Self {
            code: HawkingErrorCode::ModelOptimizationNotAvailable,
            message: HAWKING_MODEL_OPTIMIZATION_NOT_AVAILABLE_MESSAGE.to_string(),
            retryable: false,
        }
    }
...
/// planned but unwired HAWKING feature.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
impl Default for HawkingLimits {
    fn default() -> Self {
        Self {
...
pub struct CapabilitiesResponse {
    ...
    #[serde(default)]
    pub model_optimization_identity: HawkingModelOptimizationIdentity,
    #[serde(default)]
    pub limits: HawkingLimits,
    #[serde(default)]
...
pub struct AgentResponse {
    ...
    pub requested_profile: HawkingProfile,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub effective_profile: Option<HawkingProfile>,
    #[serde(default)]
...
fn json_value<T: Serialize>(value: T) -> Value {
    ...
    serde_json::to_value(value).expect("HAWKING bridge request must serialize")
}

fn decode_params<T: for<'de> Deserialize<'de>>(params: Value) -> HawkingBridgeResult<T> {
    serde_json::from_value(params).map_err(HawkingBridgeError::parse)
}
...
fn normalize_legacy_protocol(protocol: &mut String) {
    if protocol == LEGACY_HCLI_JSONL_PROTOCOL {
        *protocol = HAWKING_JSONL_PROTOCOL.to_string();
    }
}
...
fn default_hawking_profile() -> HawkingProfile {
    // HAWKING is the high-capability local surface.  The profile remains bounded
    // and effect-gated; callers can explicitly choose balanced or maximum.
    HawkingProfile::Power
}
...
fn validate_identifier(field: &str, value: &str) -> HawkingBridgeResult<()> {
...
fn validate_optional_identifier(field: &str, value: Option<&str>) -> HawkingBridgeResult<()> {
...
fn validate_optional_model(field: &str, value: Option<&str>) -> HawkingBridgeResult<()> {
    if let Some(value) = value {
...
fn validate_optional_runtime_url(field: &str, value: Option<&str>) -> HawkingBridgeResult<()> {
...
fn validate_optional_context_tokens(field: &str, value: Option<u32>) -> HawkingBridgeResult<()> {
    if let Some(value) = value {
...
/// Validate only the stable *wire identity* of an HAWKING evidence attachment.
///
...
/// `HawkingSourceStore::select_context` at dispatch time.  Keeping that lookup
...
fn validate_source_attachment_refs(field: &str, attachments: &[String]) -> HawkingBridgeResult<()> {
...
fn validate_metadata(field: &str, metadata: &BTreeMap<String, String>) -> HawkingBridgeResult<()> {
...
fn validate_notes(field: &str, notes: &[String]) -> HawkingBridgeResult<()> {
...
fn validate_short_text(field: &str, value: &str, maximum: usize) -> HawkingBridgeResult<()> {
    validate_text(field, value, maximum)
}

fn validate_text(field: &str, value: &str, maximum: usize) -> HawkingBridgeResult<()> {
    if value.trim().is_empty() {
        return Err(HawkingBridgeError::validation(field, "must not be blank"));
    }
...
fn ensure_no_nul(field: &str, value: &str) -> HawkingBridgeResult<()> {
    if value.contains('\0') {
        return Err(HawkingBridgeError::validation(
            field,
...
fn validate_range<T>(field: &str, value: T, minimum: T, maximum: T) -> HawkingBridgeResult<()>
where
...
fn ensure_unique<T>(field: &str, values: impl IntoIterator<Item = T>) -> HawkingBridgeResult<()>
where
    T: Ord,
{
    let mut seen = BTreeSet::new();
...
mod tests {
    ...
    fn request(method: &str, params: Value) -> String {
        json!({
            "protocol": HAWKING_JSONL_PROTOCOL,
            "id": "req-1",
    ...
    fn parses_a_durable_generate_request_and_round_trips_jsonl() {
        ...
        let parsed = HawkingRequest::parse_jsonl_line(&(line + "\n")).unwrap();
        let operation = parsed.operation().unwrap();
        let HawkingOperation::Generate(generate) = operation else {
        ...
        assert_eq!(HawkingRequest::parse_jsonl_line(&encoded).unwrap(), parsed);
    }

    #[test]
    fn normalizes_historic_jsonl_protocol_and_claim_limit_field() {
        let legacy_request = json!({
            "protocol": LEGACY_HCLI_JSONL_PROTOCOL,
            "id": "req-legacy",
            "method": "status",
            "params": {},
        })
        .to_string();
        let request = HawkingRequest::parse_jsonl_line(&legacy_request).unwrap();
        assert_eq!(request.protocol, HAWKING_JSONL_PROTOCOL);
        assert!(request
            .to_jsonl_line()
            .unwrap()
            .contains(HAWKING_JSONL_PROTOCOL));
        ...
        let legacy_response = json!({
            ...
            "error": {
                "code": "unavailable",
                "message": "historic input",
                "retryable": false,
            },
        })
        .to_string();
        let response = HawkingResponse::parse_jsonl_line(&legacy_response).unwrap();
        assert_eq!(response.protocol, HAWKING_JSONL_PROTOCOL);
        assert!(response
            .to_jsonl_line()
            .unwrap()
            .contains(HAWKING_JSONL_PROTOCOL));

        let claim_limits: HawkingModelOptimizationClaimLimits = serde_json::from_value(json!({
            "hcli_executes_optimization": false,
            "v4_runtime_claim": false,
            "artifact_identity_is_not_capability_evidence": true,
        }))
        .unwrap();
        assert!(!claim_limits.hawking_executes_optimization);
        let encoded = serde_json::to_value(claim_limits).unwrap();
        assert!(encoded.get("hcli_executes_optimization").is_none());
        assert_eq!(encoded["hawking_executes_optimization"], false);
    }
    ...
    fn rejects_full_auto_without_an_explicit_acknowledgement() {
        ...
        let error = HawkingRequest::parse_jsonl_line(&line).unwrap_err();
        assert!(matches!(
            error,
            HawkingBridgeError::Validation { ref field, .. }
    ...
    fn accepts_high_compute_agent_controls_without_equating_them_to_effects() {
        ...
        let request = HawkingRequest::parse_jsonl_line(&line).unwrap();
        let HawkingOperation::Agent(agent) = request.operation().unwrap() else {
            panic!("expected agent operation");
        };
        assert_eq!(agent.profile, HawkingProfile::Maximum);
        assert_eq!(agent.budget.max_subagents, Some(128));
        assert_eq!(agent.autonomy, HawkingAutonomy::SuggestOnly);
    }
    ...
    fn agent_attachment_refs_are_bounded_and_round_trip_without_source_bytes() {
        ...
        let parsed = HawkingRequest::parse_jsonl_line(&line).unwrap();
        let HawkingOperation::Agent(agent) = parsed.operation().unwrap() else {
            panic!("expected agent operation");
        };
        assert_eq!(
            agent.attachments,
        ...
        let error = HawkingRequest::parse_jsonl_line(&duplicate).unwrap_err();
        assert!(matches!(
            error,
            HawkingBridgeError::Validation { ref field, .. } if field == "params.attachments"
        ));
        ...
        let error = HawkingRequest::parse_jsonl_line(&overflow).unwrap_err();
        assert!(matches!(
            error,
            HawkingBridgeError::Validation { ref field, .. } if field == "params.attachments"
        ));
        ...
        let error = HawkingRequest::parse_jsonl_line(&malformed).unwrap_err();
        assert!(matches!(
            error,
            HawkingBridgeError::Validation { ref field, .. }
    ...
    fn validates_capabilities_session_and_status_requests() {
        for (method, params) in [
            ...
            HawkingRequest::parse_jsonl_line(&request(method, params)).unwrap();
        }

        let duplicate = request("capabilities", json!({ "include": ["runtime", "runtime"] }));
        assert!(HawkingRequest::parse_jsonl_line(&duplicate).is_err());
    }

    #[test]
    fn capability_identity_uses_gravity_as_the_public_identity_and_condense_as_engine() {
        let identity = hawking_model_optimization_identity();
        let taxonomy: Value =
            serde_json::from_str(include_str!("../../../hawking/semantic_taxonomy.json")).unwrap();
        assert_eq!(identity.schema, HAWKING_MODEL_OPTIMIZATION_IDENTITY_SCHEMA);
        ...
        assert!(!identity.claim_limits.hawking_executes_optimization);
        ...
        assert_eq!(
            ...
            HAWKING_MODEL_OPTIMIZATION_EXECUTION_BOUNDARY_SCHEMA
        );
        ...
        let historic_identity: HawkingModelOptimizationIdentity =
    ...
    fn model_optimization_is_a_typed_non_executing_protocol_refusal() {
        let parsed = HawkingRequest::parse_jsonl_line(&request("model_optimization", json!({})))
            .expect("the explicit boundary method must be parseable");
        assert_eq!(parsed.method, HawkingMethod::ModelOptimization);
        assert!(matches!(
            parsed.operation().unwrap(),
            HawkingOperation::ModelOptimization(ModelOptimizationRequest {})
        ));

        let error = HawkingError::model_optimization_not_available();
        assert_eq!(error.code, HawkingErrorCode::ModelOptimizationNotAvailable);
        ...
        let response = HawkingResponse::failure("req-1", error).unwrap();
        let encoded = response.to_jsonl_line().unwrap();
        assert_eq!(
            HawkingResponse::parse_jsonl_line(&encoded).unwrap(),
            response
        );
    }
    ...
    fn response_requires_truthful_tps_denominator_and_durable_event_reference() {
        let invalid = HawkingResponse::success(
            "req-1",
            HawkingResult::Generate(GenerateResponse {
        ...
        let response = HawkingResponse::success(
            "req-1",
            HawkingResult::Generate(GenerateResponse {
                status: OperationStatus::Completed,
                session_id: Some("ses_local".to_string()),
    ...
    fn rejects_multiline_and_unknown_outer_fields() {
        ...
        assert!(HawkingRequest::parse_jsonl_line(&multiline).is_err());
        ...
        .to_string();
        assert!(HawkingRequest::parse_jsonl_line(&unknown).is_err());
    }
}
