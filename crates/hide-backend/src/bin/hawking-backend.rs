//! HAWKING — Hawking's headless local-model product surface and HIDE backend.
//!
//! This is the current Rust backend authority behind the HAWKING product surface:
...
use hide_backend::hawking_bridge::{
    hawking_model_optimization_identity, AgentRealization, AgentResponse, CapabilitiesResponse,
    ComponentState, ComponentStatus, DecodeTelemetry, DurableTurnReceipt, GenerateResponse,
    HawkingAutonomy, HawkingError, HawkingErrorCode, HawkingMethod, HawkingOperation,
    HawkingRequest, HawkingResponse, HawkingResult, MethodCapability, OperationStatus,
    SessionAction, SessionRequest, SessionResponse, SessionSummary, StatusArea, StatusResponse,
    HAWKING_MODEL_OPTIMIZATION_NOT_AVAILABLE_CODE,
};
use hide_backend::hawking_profile::HawkingProfile;
use hide_backend::hawking_research::{run_hawking_research, HawkingResearchConfig};
use hide_backend::hawking_sources::{HawkingSourceContext, HawkingSourceStore};
use hide_backend::hawking_swarm::{run_parallel_analysis_swarm, HawkingSwarmConfig};
...
const HAWKING_SCHEMA: &str = "hawking.command.v1";
...
impl Options {
    ...
    /// existing local HAWKING evidence ref/hash and is validated by the object
...
fn usage() -> &'static str {
    "HAWKING — Hawking's headless local-model shell\n\
     \n\
     Usage:\n\
       hawking capabilities [--workspace PATH] [--model-url URL] [--json]\n\
       hawking model inspect [--workspace PATH] [--model-url URL] [--json]\n\
       hawking model optimize [--json]\n\
       hawking run --prompt TEXT [--session NAME] --model-url URL [--max-output-tokens N]
                [--attach OREF_OR_BLAKE3]...
                [--workspace PATH] [--json]\n\
       hawking repl --model-url URL [--session NAME] [--workspace PATH]\n\
       hawking agent --goal TEXT --model-url URL [--profile balanced|power|maximum]\n\
                  [--max-transitions N] [--attach OREF_OR_BLAKE3]... [--receipt PATH] [--allow-incomplete]\n\
       hawking swarm --goal TEXT --model-url URL [--profile balanced|power|maximum]\n\
                  [--lanes N] [--max-concurrency N] [--max-transitions N]\n\
                  [--no-synthesis] [--receipt PATH] [--allow-incomplete]\n\
       hawking research --topic TEXT --model-url URL [--per-query-limit N]\n\
                     [--read-budget N] [--max-rounds N] [--receipt PATH] [--allow-incomplete]\n\
       hawking source ingest (--file PATH | --stdin) [--label NAME] [--mime TYPE]\n\
                          [--workspace PATH] [--json]\n\
       hawking source list [--workspace PATH] [--json]\n\
       hawking source show (--ref OREF | --hash BLAKE3) [--workspace PATH] [--json]\n\
       hawking source context --attach OREF_OR_BLAKE3 [--attach OREF_OR_BLAKE3]... [--workspace PATH] [--json]\n\
       hawking processes [--workspace PATH] [--no-footprint] [--json]\n\
       hawking processes-server --workspace PATH\n\
       hawking bench --prompt TEXT --model-url URL [--warmup N] [--runs N]\n\
                  [--max-output-tokens N] [--receipt PATH]\n\
       hawking bridge jsonl [--workspace PATH] [--model-url URL]\n\
       hawking session show [--session NAME] [--workspace PATH]\n\
       hawking session fork --session NAME --at-seq N [--workspace PATH]\n\
       hawking tool list [--workspace PATH]\n\
       hawking tool run --name TOOL --args JSON [--session NAME] [--workspace PATH]\n\
       hawking serve (--weights PATH | --gravity PATH) [--addr HOST:PORT]\n\
                  [--hawking-bin PATH] [--profile fast|exact|race|efficient]\n\
     \n\
     Environment:\n\
       HAWKING_MODEL_URL     Default explicit local model URL\n\
       HAWKING_MODEL_BIN     Default Hawking executable for `hawking serve`\n\
     \n\
     ...
     operation. HAWKING reports that taxonomy but does not run optimization or\n\
     promote a live V4 diagnostic into a full-model, numeric-parity, Metal, or\n\
     TPS claim. `hawking serve --gravity` is a compatibility launcher for\n\
     an already-built Gravity artifact; inspect `model_optimization_identity`\n\
     for the canonical Hawking command and compatibility scope. `hawking model\n\
...
fn named_session(host: &BackendHost, options: &Options) -> hide_core::ids::SessionId {
    match options.value("session") {
        ...
        None => host.services.session_named("hawking"),
    }
}

fn profile(options: &Options, default: HawkingProfile) -> Result<HawkingProfile> {
    match options.value("profile") {
        Some(name) => HawkingProfile::parse(name).ok_or_else(|| {
...
fn selected_source_context(
    root: &std::path::Path,
    options: &Options,
) -> Result<Option<HawkingSourceContext>> {
...
/// Keep the canonical Rust process inspector resident for HAWKING daemon-owned
            ...
            Ok(request) => match request.get("op").and_then(Value::as_str).unwrap_or("") {
                ...
                "shutdown" => {
                    writeln!(
                        stdout,
                        "{}",
                        json!({"ok": true, "schema": "hawking.processes-server.shutdown.v1"})
                    )?;
                    stdout.flush()?;
                    return Ok(());
                }
...
async fn cmd_capabilities(options: &Options) -> Result<Value> {
    ...
    let runtime = match endpoint.as_deref() {
        ...
        None => json!({
            ...
            "note": "pass --model-url (or HAWKING_MODEL_URL) to inspect a local endpoint",
        }),
    };
    Ok(command_envelope(
        "capabilities",
        json!({
            ...
            "model_optimization_identity": hawking_model_optimization_identity(),
            "workspace": root,
            "backend": status,
            "runtime": runtime,
            "surfaces": {
                ...
                "machine_bridge": "available through hawking bridge jsonl",
            },
            "honesty_limits": {
                "model_roles": "one explicit local endpoint per HAWKING run; no realized multi-model role routing",
                "model_optimization": "Gravity is the canonical public identity and Condense is its engine operation; HAWKING reports the shared taxonomy but does not execute optimization. `hawking model optimize` and JSONL `model_optimization` return the stable model_optimization_not_available refusal. See model_optimization_identity.public_cli for the executable Hawking surface.",
                "uploads_to_context": "no HTTP upload route; explicit existing local evidence refs can be attached to one HAWKING run/chat/agent invocation",
                ...
                "deepseek_v4_gravity": "an explicit --model-url may expose a live V4 diagnostic; HAWKING never promotes that into a full-model, numeric-parity, Metal, or TPS claim",
...
/// Read-only, machine-consumable acknowledgement of HAWKING's optimization
...
fn cmd_model_optimization_status(options: &Options) -> Value {
    ...
    command_envelope(
        ...
        json!({
            ...
            "refusal": HawkingError::model_optimization_not_available(),
            "request": {
            ...
            "next": {
                ...
                "note": "The named Hawking command is the canonical planning surface; this HAWKING response does not assert a produced artifact, runnable V4 model, or benchmark result.",
            },
        }),
    )
}
...
async fn cmd_run(options: &Options) -> Result<Value> {
    ...
    let endpoint =
        model_url(options).ok_or_else(|| anyhow!("--model-url is required for hawking run"))?;
    let host = BackendHost::open_workspace(&root)?;
    ...
    let turn = host
        .hawking_turn_with_output_cap_and_source_context(
            session,
            endpoint.clone(),
            prompt,
...
async fn cmd_agent(options: &Options) -> Result<Value> {
    ...
    let profile = profile(options, HawkingProfile::Power)?;
    ...
    .await?;
    let receipt_path = path_or_default(&root, options, "hawking-agent");
...
async fn cmd_swarm(options: &Options) -> Result<Value> {
    ...
    .await?;
    let receipt_path = path_or_default(&root, options, "hawking-swarm");
    write_sealed_receipt(&receipt_path, &result.receipt)?;
    let value = command_envelope(
        "swarm",
        json!({
...
async fn cmd_research(options: &Options) -> Result<Value> {
    ...
    let endpoint = model_url(options);
    let host = BackendHost::open_workspace(&root)?;
    let result = run_hawking_research(
        &host,
        HawkingResearchConfig {
            topic,
            model_url: endpoint.clone(),
    ...
    .await?;
    let receipt_path = path_or_default(&root, options, "hawking-research");
    write_sealed_receipt(&receipt_path, &result.receipt)?;
    let value = command_envelope(
        "research",
        json!({
    ...
    match subcommand.unwrap_or("show") {
        "show" | "new" => {
            ...
            Ok(command_envelope(
                ...
                json!({
                    "workspace": root,
                    "session_name": options.value("session").unwrap_or("hawking"),
                    "session_id": session,
...
async fn cmd_source(subcommand: Option<&str>, options: &Options) -> Result<Value> {
    let root = workspace(options)?;
    let sources = HawkingSourceStore::open(&root)?;
    match subcommand.unwrap_or("list") {
        "ingest" => {
            ...
            Ok(command_envelope(
                ...
                json!({
                    ...
                    "receipt": result.receipt,
                    "context_note": "Ingestion never auto-attaches. Use repeatable --attach on hawking run/chat/agent to inject this source's bounded model-facing derivatives for one invocation.",
                }),
            ))
        }
        "list" => {
            ...
            Ok(command_envelope(
                ...
                json!({
                    ...
                    "context_note": "Listing evidence does not make it available to a model turn. Pass one or more listed refs to --attach on hawking run/chat/agent, or inspect the metadata-only pack with hawking source context.",
                }),
            ))
        }
        "show" => {
...
async fn cmd_bench(options: &Options) -> Result<Value> {
    let root = workspace(options)?;
    let endpoint =
        model_url(options).ok_or_else(|| anyhow!("--model-url is required for hawking bench"))?;
    let prompt = prompt(options)?;
...
impl BridgeDispatchError {
    ...
    fn unavailable(message: impl Into<String>) -> Self {
        Self {
            code: HawkingErrorCode::Unavailable,
            message: message.into(),
            retryable: true,
        }
    }
    ...
    fn model_optimization_not_available() -> Self {
        let refusal = HawkingError::model_optimization_not_available();
        Self {
            code: refusal.code,
    ...
    fn unsupported(message: impl Into<String>) -> Self {
        Self {
            code: HawkingErrorCode::Unsupported,
            message: message.into(),
            retryable: false,
        }
    }
    ...
    fn internal(error: impl std::fmt::Display) -> Self {
        Self {
            code: HawkingErrorCode::Internal,
            message: compact_bridge_message(error.to_string()),
            retryable: false,
        }
    }

    fn into_wire(self) -> HawkingError {
        HawkingError {
            code: self.code,
...
fn bridge_requires_suggest_only(
    autonomy: HawkingAutonomy,
) -> std::result::Result<(), BridgeDispatchError> {
    if matches!(autonomy, HawkingAutonomy::SuggestOnly) {
        Ok(())
    } else {
        Err(BridgeDispatchError::unsupported(
            "this HAWKING adapter exposes SuggestOnly agent autonomy only; ReadOnly and FullAuto are not silently remapped",
        ))
    }
}

fn bridge_sampler(
    request: &hide_backend::hawking_bridge::SamplingRequest,
) -> Option<SamplerProfile> {
...
async fn bridge_direct_generate(
    request: hide_backend::hawking_bridge::GenerateRequest,
    endpoint: String,
    ...
    let inference = InferenceRequest {
        task_kind: "hawking.machine.generate".to_string(),
...
async fn bridge_generate(
    ...
    let endpoint =
        bridge_runtime_url(request.runtime_url.as_deref(), options).ok_or_else(|| {
            BridgeDispatchError::unavailable(
                "generate requires params.runtime_url or --model-url/HAWKING_MODEL_URL",
            )
        })?;
    if !request.durable {
    ...
    let session_id = request
        ...
        .unwrap_or_else(|| host.services.session_named("hawking-bridge"));
    let turn = host
        .hawking_turn_with_output_cap(
            session_id,
            endpoint,
            request.prompt,
...
/// evidence path used by the direct HAWKING command.  A machine request carries
...
fn bridge_agent_source_context(
    root: &std::path::Path,
    attachments: &[String],
) -> std::result::Result<Option<HawkingSourceContext>, BridgeDispatchError> {
    if attachments.is_empty() {
        return Ok(None);
    }
    let sources = HawkingSourceStore::open(root).map_err(BridgeDispatchError::internal)?;
    sources
...
async fn bridge_agent(
    ...
    root: &std::path::Path,
    options: &Options,
    request: hide_backend::hawking_bridge::AgentRequest,
) -> std::result::Result<AgentResponse, BridgeDispatchError> {
    bridge_requires_suggest_only(request.autonomy)?;
    if request.dry_run {
        return Err(BridgeDispatchError::unsupported(
            "agent dry_run is not implemented; HAWKING refuses to present a model-free receipt as an executed agent run",
        ));
    }
    if request.model.is_some() {
        return Err(BridgeDispatchError::unsupported(
    ...
    let session_id = request
        ...
        .as_deref()
        .map(hide_core::ids::SessionId::from)
        .or_else(|| Some(host.services.session_named("hawking-bridge-agent")));
...
async fn bridge_session(
    ...
    match request {
        SessionRequest::Create { label } => {
            let label = label.unwrap_or_else(|| format!("hawking-{}", hide_core::ids::now_ms()));
        ...
        SessionRequest::Close { .. } => Err(BridgeDispatchError::unsupported(
            "HAWKING does not expose session deletion/closure; durable history is intentionally retained",
        )),
    }
}
...
async fn bridge_status(
    host: &BackendHost,
    options: &Options,
    request: hide_backend::hawking_bridge::StatusRequest,
    ...
    if include(StatusArea::Runtime) {
        ...
        let (state, detail, data) = match runtime {
            ...
            None => (
                ...
                "no explicit HAWKING runtime endpoint configured".to_string(),
                None,
            ),
        };
        components.insert(
    ...
    if include(StatusArea::Agents) {
        components.insert(
            ...
            ComponentStatus {
                ...
                data: Some(json!({ "profiles": HawkingProfile::ALL })),
            },
        );
    }
...
async fn bridge_dispatch(
    ...
    root: &std::path::Path,
    options: &Options,
    operation: HawkingOperation,
) -> std::result::Result<HawkingResult, BridgeDispatchError> {
    match operation {
        HawkingOperation::Capabilities(_) => Ok(HawkingResult::Capabilities(CapabilitiesResponse {
            operations: vec![
                MethodCapability { method: HawkingMethod::Capabilities, available: true, detail: Some("protocol and effective limits".to_string()) },
                MethodCapability { method: HawkingMethod::ModelOptimization, available: false, detail: Some(format!("always refuses with stable code {HAWKING_MODEL_OPTIMIZATION_NOT_AVAILABLE_CODE}; HAWKING reports the taxonomy but never executes Condense/Gravity optimization")) },
                MethodCapability { method: HawkingMethod::Generate, available: true, detail: Some("direct local generation and durable contextual turns".to_string()) },
                MethodCapability { method: HawkingMethod::Agent, available: true, detail: Some("SuggestOnly headless AgentKernel; profile plus transition cap".to_string()) },
                MethodCapability { method: HawkingMethod::Swarm, available: false, detail: Some("direct CLI analysis swarm exists, but this machine contract lacks a graph coordinator and hard shared budget governor".to_string()) },
                MethodCapability { method: HawkingMethod::Session, available: true, detail: Some("create, resume, and list; close is intentionally unavailable".to_string()) },
                MethodCapability { method: HawkingMethod::Status, available: true, detail: Some("host/runtime/storage status; optional runtime refresh".to_string()) },
            ],
            model_optimization_identity: hawking_model_optimization_identity(),
            limits: Default::default(),
            notes: vec![
                "HAWKING is local-endpoint oriented and does not proxy model traffic.".to_string(),
        ...
        HawkingOperation::ModelOptimization(_) => {
            Err(BridgeDispatchError::model_optimization_not_available())
        }
        HawkingOperation::Generate(request) => bridge_generate(host, options, request).await.map(HawkingResult::Generate),
        HawkingOperation::Agent(request) => bridge_agent(host, root, options, request).await.map(HawkingResult::Agent),
        HawkingOperation::Swarm(_) => Err(BridgeDispatchError::unsupported(
            "the JSONL swarm method is withheld until HAWKING has a real coordinator, evidence manifest, and hard shared model-call/concurrency governor; use `hawking swarm` only for the explicitly labeled independent analysis swarm",
        )),
        HawkingOperation::Session(request) => bridge_session(host, request).await.map(HawkingResult::Session),
        HawkingOperation::Status(request) => bridge_status(host, options, request).await.map(HawkingResult::Status),
    }
}
...
async fn cmd_bridge(subcommand: Option<&str>, options: &Options) -> Result<()> {
    ...
    for line in stdin.lock().lines() {
        let line = line?;
        let response = match HawkingRequest::parse_jsonl_line(&line) {
            Ok(request) => {
                ...
                match request.into_validated() {
                    Ok(validated) => {
                        match bridge_dispatch(&host, &root, options, validated.operation).await {
                            Ok(result) => {
                                HawkingResponse::success(id, result).unwrap_or_else(|error| {
                                    HawkingResponse::failure(
                                        "invalid",
                                        HawkingError {
                                            code: HawkingErrorCode::Internal,
                                    ...
                                    .expect("internal HAWKING error response must validate")
                                })
                            }
                            Err(error) => HawkingResponse::failure(id, error.into_wire())
                                .expect("bridge error response must validate"),
                        }
                    }
                    Err(error) => HawkingResponse::failure(
                        "invalid",
                        HawkingError {
                            code: HawkingErrorCode::InvalidRequest,
                            message: compact_bridge_message(error.to_string()),
            ...
            Err(error) => HawkingResponse::failure(
                "invalid",
                HawkingError {
                    code: HawkingErrorCode::ParseError,
                    message: compact_bridge_message(error.to_string()),
                    retryable: false,
                },
            )
...
/// A deliberately small, HAWKING-specific slash-command surface for the
...
/// protocol command catalog includes UI commands with no equivalent safe HAWKING
...
fn repl_help(topic: Option<&str>) -> String {
    const GENERAL: &str = "\
HAWKING slash commands (only real HAWKING operations are listed):
  /help [COMMAND]                 Show this help or details for one command
  /status                          Show current session, backend, and runtime health
  /capabilities                    Show the implemented REPL/HAWKING surface and limits
...
surface-specific UI commands that are not silently exposed through HAWKING.";
...
operation. This HAWKING build does not execute optimization or assert a V4
...
Reports HAWKING's explicit, machine-readable non-execution boundary for
...
leave HAWKING's execution boundary."
            .to_string(),
...
use `hawking source ingest --stdin` outside the REPL when that input mode is needed."
            .to_string(),
...
Tool execution goes through HAWKING's real registry, permission, schema, and
...
derivatives only for this invocation and records the selection in its receipt."
            .to_string(),
        Some(value) => format!("No HAWKING REPL command named {value:?}.\n\n{GENERAL}"),
    }
}

/// Tokenize command options without invoking a shell.  Quotes and backslash
/// escapes are only a convenience for HAWKING command arguments; no expansion,
    ...
    match subcommand.to_ascii_lowercase().as_str() {
        ...
        "ingest" => {
            ...
            if options.flag("stdin") {
                bail!("/source ingest --stdin is unavailable in the REPL because stdin carries commands; run `hawking source ingest --stdin` outside the REPL");
            }
    ...
    emit(
        json!({
            ...
            "explicit_source_context": source_context
                .map(HawkingSourceContext::receipt_json)
        ...
        ReplSlashCommand::Capabilities => {
            emit(
                json!({
                    ...
                    "catalog_scope_note": "The protocol command catalog also covers GUI and protocol surfaces. This REPL exposes only operations with an implemented HAWKING mapping.",
                }),
                false,
            )?;
        }
        ReplSlashCommand::Model => {
            emit(
                json!({
                    ...
                    "model_optimization_identity": hawking_model_optimization_identity(),
                    "runtime": runtime_preflight(endpoint).await,
                    "note": "Gravity is the canonical public model-optimization identity and Condense is its engine operation. This HAWKING surface reports naming and live endpoint facts only; it does not execute optimization or promote a live V4 diagnostic into a full-model, numeric-parity, Metal, or TPS result.",
                }),
                false,
            )?;
        }
...
async fn cmd_repl(options: &Options) -> Result<()> {
    let root = workspace(options)?;
    let endpoint =
        model_url(options).ok_or_else(|| anyhow!("--model-url is required for hawking repl"))?;
    let host = BackendHost::open_workspace(&root)?;
    let mut session = ReplSession::named(&host, options.value("session").unwrap_or("hawking"));
    eprintln!(
        "HAWKING REPL — session={}; /help for commands, /quit to leave.",
        session.label
    );
    let stdin = io::stdin();
    loop {
        eprint!("hawking[{}]> ", session.label);
        io::stderr().flush()?;
...
fn cmd_serve(options: &Options) -> Result<()> {
    ...
    if weights.is_some() == gravity.is_some() {
        bail!("hawking serve requires exactly one of --weights or --gravity");
    }
    let bin = options
        .value("hawking-bin")
        .map(str::to_string)
        .or_else(|| std::env::var("HAWKING_MODEL_BIN").ok())
    ...
    eprintln!(
        "Launching local Hawking runtime via {bin:?}; HAWKING does not proxy or alter model bytes. \
...
async fn main() -> Result<()> {
    ...
    let value = match command {
        ...
        other => bail!("unknown HAWKING command {other:?}\n\n{}", usage()),
    };
    emit(value, options.flag("json"))
}
...
mod tests {
    ...
    fn optimize_surfaces_are_explicit_machine_readable_non_execution_boundaries() {
        ...
        assert!(repl_help(Some("optimize")).contains(HAWKING_MODEL_OPTIMIZATION_NOT_AVAILABLE_CODE));
        ...
        assert!(usage().contains("hawking model optimize"));
    }
    ...
    async fn bridge_model_optimization_dispatch_uses_the_stable_refusal_code() {
        ...
        let error = bridge_dispatch(
            ...
            directory.path(),
            &Options::default(),
            HawkingOperation::ModelOptimization(Default::default()),
        )
        .await
        .unwrap_err()
        .into_wire();
    ...
    fn bridge_agent_source_attachments_select_bounded_provenance_without_source_text() {
        ...
        let selected_fact = "HAWKING_BRIDGE_SELECTED_FACT_5c31";
        std::fs::write(&source_path, selected_fact).unwrap();
        let sources = HawkingSourceStore::open(directory.path()).unwrap();
    ...
    async fn bridge_agent_rejects_missing_local_attachment_before_runtime_work() {
        ...
        let error = bridge_agent(
            ...
            directory.path(),
            &Options::default(),
            hide_backend::hawking_bridge::AgentRequest {
                ...
                autonomy: HawkingAutonomy::SuggestOnly,
        ...
        .unwrap_err()
        .into_wire();
        assert_eq!(error.code, HawkingErrorCode::InvalidRequest);
