//! Evidence-first data procurement for HAWKING.
//!
//! HAWKING research is a concrete path over the existing Research Lab FSM:
...
pub const HAWKING_RESEARCH_RECEIPT_SCHEMA: &str = "hawking.research.v1";

/// Bounded parameters for an evidence-backed research run.
#[derive(Debug, Clone)]
pub struct HawkingResearchConfig {
    pub topic: String,
...
impl Default for HawkingResearchConfig {
    fn default() -> Self {
        Self {
            topic: String::new(),
            model_url: None,
...
#[derive(Debug, Clone)]
pub struct HawkingResearchResult {
    pub complete: bool,
    pub receipt: Value,
}
...
pub async fn run_hawking_research(
    host: &BackendHost,
    config: HawkingResearchConfig,
) -> Result<HawkingResearchResult> {
    ...
    let mut receipt = json!({
        "schema": HAWKING_RESEARCH_RECEIPT_SCHEMA,
    ...
    let Some(model_url) = model_url else {
        ...
        seal(&mut receipt)?;
        return Ok(HawkingResearchResult {
            complete: false,
            receipt,
        });
    };
