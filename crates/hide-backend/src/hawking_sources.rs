//! Local, evidence-first source ingestion for HAWKING.
//!
...
//! an explicit bounded derivative pack for an HAWKING turn or agent, and receive
...
pub const HAWKING_SOURCE_RECEIPT_SCHEMA: &str = "hawking.source.ingest.v1";
pub const HAWKING_SOURCE_CONTEXT_SCHEMA: &str = "hawking.source.context.v1";
const SOURCE_OWNER: &str = "hawking-local";
const PREVIEW_CHARS: usize = 4_096;
/// This first attachment seam intentionally fits beneath the minimum HAWKING
...
/// A workspace-local, persistent object store used by `hawking source`.
///
...
#[derive(Debug, Clone)]
pub struct HawkingSourceIngestResult {
...
/// receipts. It is carried only in [`HawkingSourceContext::model_prompt`] on the
/// in-process model-call path; the durable audit surface gets this metadata,
/// identities, and counts instead.
#[derive(Debug, Clone, serde::Serialize)]
pub struct HawkingSourceContextSelection {
...
#[derive(Clone)]
pub struct HawkingSourceContext {
    model_prompt: String,
    model_prompt_tokens: usize,
    tokens_estimated: bool,
    selections: Vec<HawkingSourceContextSelection>,
}

impl std::fmt::Debug for HawkingSourceContext {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("HawkingSourceContext")
            .field(
...
impl HawkingSourceContext {
    ...
    pub fn selections(&self) -> &[HawkingSourceContextSelection] {
        &self.selections
    }
    ...
    pub fn receipt_json(&self) -> Value {
        json!({
            "schema": HAWKING_SOURCE_CONTEXT_SCHEMA,
            "status": "selected",
...
impl HawkingSourceStore {
    ...
    pub fn open(workspace_root: impl AsRef<Path>) -> Result<Self> {
        ...
        let store = ObjectStore::open(&root, StorageBudget::default())
            .map_err(|error| anyhow!("open HAWKING evidence object store: {error}"))?;
        Ok(Self { root, store })
    }
    ...
    ) -> Result<HawkingSourceIngestResult> {
    ...
    pub fn show(&self, identifier: &str) -> Result<Value> {
        ...
        let previews: Vec<Value> = view
            ...
            .collect();
        Ok(json!({
            "schema": "hawking.source.show.v1",
            "reference": source_summary(&reference, &record),
            "model_facing_derivative_previews": previews,
            "raw_body": {
                "included": false,
                "reason": "hawking source show does not expose the original byte stream. For textual inputs, a bounded text-extract derivative preview can reproduce source text up to its stated cap.",
            },
        }))
    }

    /// Resolve one or more explicit source IDs into a small model-facing
    /// derivative pack. This is the only HAWKING attachment path: it crosses the
    ...
    pub fn select_context(&self, identifiers: &[String]) -> Result<HawkingSourceContext> {
        if identifiers.is_empty() {
            bail!("at least one --attach source reference or content hash is required");
        }
        if identifiers.len() > MAX_CONTEXT_SOURCES {
            bail!(
                "at most {MAX_CONTEXT_SOURCES} explicit source attachments are allowed per HAWKING invocation"
            );
        }
        ...
        for identifier in identifiers {
            ...
            let derivative = view
                ...
                .ok_or_else(|| {
                    anyhow!(
                        "source {} has no non-empty inline model-facing text derivative; inspect it with `hawking source show --ref {}` before attaching",
                        reference.id.as_str(),
                        reference.id.as_str()
                    )
                })?;
            ...
            selections.push(HawkingSourceContextSelection {
    ...
    fn ingest_prepared_path(
        ...
        let mut receipt = json!({
            "schema": HAWKING_SOURCE_RECEIPT_SCHEMA,
            "status": "completed",
            "finished_ms": now_ms(),
            ...
            "context": {
                ...
                "note": "Ingestion never auto-attaches evidence. A later explicit HAWKING --attach selection may use bounded model-facing derivatives for one run or agent invocation.",
            },
            "limits": [
...
mod tests {
    ...
    fn local_file_ingest_is_durable_and_safe_to_list_after_reopen() {
        ...
        let first = HawkingSourceStore::open(workspace.path())
            .unwrap()
        ...
        let reopened = HawkingSourceStore::open(workspace.path()).unwrap();
    ...
    fn stdin_ingest_streams_to_the_same_durable_store() {
        ...
        let result = HawkingSourceStore::open(workspace.path())
            .unwrap()
            .ingest_stdin(
                &mut stdin,
        ...
        assert!(HawkingSourceStore::open(workspace.path())
            .unwrap()
    ...
    fn explicit_context_selection_uses_only_bounded_model_facing_derivatives() {
        ...
        .unwrap();
        let store = HawkingSourceStore::open(workspace.path()).unwrap();
