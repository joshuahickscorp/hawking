//! Host-wide process observation for the HAWKING operator surface.
//!
...
//! Rust makes the Python HAWKING skin an adapter instead of a second process
//! authority.
...
fn role_for(command: &str) -> Option<Role> {
    ...
    const ROLE_DEFS: &[(&str, &str, &str, bool, &str)] = &[
        (
            r"(?:^|/)(?:hawkingd|hawking)(?:\s+-P\s+-m\s+hawking\.hawkingd)?\s+serve\b",
            "hawkingd",
        ...
        (
            r"hawking\.agentos\.resident\b.*--supervise",
            "resident-supervisor",
            "ESSENTIAL_PERSISTENT",
            false,
...
fn claimed_worker_pids(workspace: &Path) -> BTreeSet<u32> {
    ...
    roots
        .into_iter()
        .filter_map(|root| {
            let path = root.join(".hawking").join("resident").join("state.json");
...
mod tests {
    ...
    fn classifier_preserves_specific_role_order_and_near_miss() {
        assert_eq!(
            role_for("Python -m hawking.agentos.resident --supervise")
                .unwrap()
                .name,
            "resident-supervisor"
        );
        ...
        assert_eq!(
            role_for(
                "/opt/hawking/current/hawkingd -P -m hawking.hawkingd serve \
...
Summary Footprint: 5242880 B\n";
    ...
    fn state_claims_are_workspace_scoped_and_union_with_cwd() {
        ...
        let state_dir = workspace.path().join(".hawking/resident");
