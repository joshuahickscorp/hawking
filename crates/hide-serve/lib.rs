...
fn error_frame(code: &str, message: &str) -> String {
    ...
    serde_json::to_string(&event).unwrap_or_else(|_| {
        json!({"seq":0,"session_id":null,"kind":{"type":"error","data":{"code":code,"message":message}}}).to_string()
    })
}
...
mod tests {
    ...
    async fn intent_round_trips_through_the_handler() {
        ...
        let body =
            serde_json::to_vec(&Intent::RunCommand { argv: vec!["printf".to_string(), "hide".to_string()], cwd: None })
                .unwrap();
        let resp = app
            .oneshot(
                Request::builder()
    ...
    async fn empty_intent_is_rejected_as_200_body() {
        ...
        let body = serde_json::to_vec(&Intent::RunCommand { argv: vec![], cwd: None }).unwrap();
        let resp = app
            .oneshot(
                Request::builder()
                    .method("POST")
    ...
    async fn events_plain_get_returns_a_json_array() {
        ...
        let resp = app
            .oneshot(Request::builder().method("GET").uri("/v1/hide/events?after_seq=0").body(Body::empty()).unwrap())
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let bytes = resp.into_body().collect().await.unwrap().to_bytes();
