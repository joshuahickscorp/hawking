...
pub trait KeyValueStore: Send + Sync {
    ...
    /// Enumerate every `(key, value)` pair currently in a table. Order is
    /// unspecified: callers that need a stable projection sort the result
    /// themselves. The default is empty so a store that does not support
    /// enumeration degrades to "no entries" rather than erroring; the in-tree
    /// stores override it. Used to walk the durable `session_records` namespace
    /// for the conversation-graph projection (bible sec 32-33).
...
impl KeyValueStore for InMemoryKeyValueStore {
    ...
    fn list(&self, table: &str) -> Result<Vec<(String, Value)>> {
        Ok(self
            .tables
            .lock()
            .get(table)
...
impl KeyValueStore for FileKeyValueStore {
    ...
    fn list(&self, table: &str) -> Result<Vec<(String, Value)>> {
        let dir = self.root.join(sanitize_component(table));
        if !dir.exists() {
            return Ok(Vec::new());
        }
        let mut out = Vec::new();
        for entry in std::fs::read_dir(&dir)? {
            let path = entry?.path();
            // Only committed `<key>.json` values; skip the transient
            // `<key>.json.tmp` write-and-rename staging files.
