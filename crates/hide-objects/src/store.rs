...
use crate::derivatives::{CompileObjectView, DerivativeSelection, RawBytesCap};
use crate::error::{ObjectError, Result};
use crate::hash::{ContentHash, CHUNK_SIZE};
use crate::permissions::{ObjectPermissions, Reader};
use crate::pipeline::{self, new_processing_record};
use crate::processors::ProcessorSet;
use crate::queue::{IngestJob, IngestQueue, JobStatus, Priority};
            ...
            .find(|s| {
                record
                    ...
                    .map(|r| {
                        matches!(
                            r.status,
                            crate::schema::StageStatus::Pending
                                | crate::schema::StageStatus::Partial
                                | crate::schema::StageStatus::Failed
                        )
                    })
                    .unwrap_or(true)
            })
