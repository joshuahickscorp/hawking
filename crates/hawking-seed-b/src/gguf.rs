//! A minimal GGUF v2/v3 reader — only what this fixture needs: header, metadata KVs (incl. string
//! arrays for the tokenizer), the tensor directory, and absolute tensor-data slices. Little-endian.
//! No external GGUF library; layout matches the predecessor's `gguf.rs`.
...
impl GgufFile {
    ...
    pub fn parse(data: Vec<u8>) -> Result<Self> {
        let (metadata, tensors) = {
            ...
            let magic = p.u32()?;
            if magic != GGUF_MAGIC {
                return Err(Error::Gguf(format!("bad magic {magic:#x}")));
            }
            let version = p.u32()?;
            ...
            for _ in 0..tensor_count {
                ...
                if n_dims > 8 {
                    return Err(Error::Gguf(format!("absurd n_dims {n_dims}")));
                }
            ...
            for (name, dims, dtype, local_offset) in infos {
                ...
                if abs + byte_size > data.len() as u64 {
                    return Err(Error::Gguf(format!("{name}: data past end of file")));
                }
                tensors.insert(name.clone(), TensorInfo { name, dims, dtype, data_offset: abs, byte_size });
            }
            (metadata, tensors)
        };
        Ok(GgufFile { data, metadata, tensors })
    }
    ...
    pub fn tensor_bytes(&self, name: &str) -> Result<&[u8]> {
        ...
        Ok(&self.data[t.data_offset as usize..(t.data_offset + t.byte_size) as usize])
    }
    ...
    pub fn meta_u32(&self, key: &str) -> Result<u32> {
        match self.metadata.get(key) {
            ...
            _ => Err(Error::Gguf(format!("missing/typed u32 metadata {key}"))),
        }
    }
    pub fn meta_f32(&self, key: &str) -> Result<f32> {
        match self.metadata.get(key) {
            Some(MetaValue::F32(x)) => Ok(*x),
            _ => Err(Error::Gguf(format!("missing/typed f32 metadata {key}"))),
        }
    }
    pub fn meta_str(&self, key: &str) -> Result<&str> {
        match self.metadata.get(key) {
            Some(MetaValue::String(s)) => Ok(s.as_str()),
            _ => Err(Error::Gguf(format!("missing/typed string metadata {key}"))),
        }
    }
    pub fn meta_str_array(&self, key: &str) -> Result<Vec<&str>> {
        match self.metadata.get(key) {
            Some(MetaValue::Array(a)) => Ok(a
                ...
                .collect()),
            _ => Err(Error::Gguf(format!("missing/typed array metadata {key}"))),
        }
    }
}
