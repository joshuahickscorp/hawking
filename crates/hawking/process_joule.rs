...
const COLLECTOR_CONTRACT_SHA256: &str = "0c832e80b1336a2ceb88cb401590981d01478f3cad2b75aa15c7381e44984dba";
const STRUCT_LAYOUT_SHA256: &str = "f6f0425fd8a4b3e054077ca68ea4856dd77c7771597bfd065aa26b3c36d66494";
...
fn stamp_value(mut value: Value, field: &str) -> Result<Value> {
    ...
    value.as_object_mut().context("only JSON objects can be stamped")?.insert(field.to_owned(), Value::String(digest));
    Ok(value)
}
...
fn validate_file_identity(value: Option<&Value>, label: &str) -> Result<()> {
    let map =
        value.and_then(Value::as_object).with_context(|| format!("{label} provenance identity must be an object"))?;
    ...
    let digest = map["sha256"].as_str().with_context(|| format!("{label} provenance hash is not a string"))?;
    if !is_lower_hex(digest, 64) {
        bail!("{label} provenance hash is not lowercase SHA-256");
    }
    if map["size_bytes"].as_u64().filter(|size| *size > 0).is_none() {
...
fn expected_layout_sha256() -> Result<String> {
    ...
    for (index, field) in RUSAGE_V6_U64_FIELDS.iter().enumerate() {
        offsets.insert((*field).to_owned(), json!(16 + index * mem::size_of::<u64>()));
    }
    offsets.insert("ri_reserved".to_owned(), json!(16 + RUSAGE_V6_U64_FIELDS.len() * mem::size_of::<u64>()));
...
fn read_stable_json(path: &Path) -> Result<Value> {
    ...
    let mut file = File::open(path).with_context(|| format!("open process-joule provenance {}", path.display()))?;
    ...
    let identity = |row: &fs::Metadata| {
        (row.dev(), row.ino(), row.len(), row.mtime(), row.mtime_nsec(), row.ctime(), row.ctime_nsec(), row.nlink())
    };
...
fn validate_provenance(value: &Value) -> Result<()> {
    let map = value.as_object().context("process-joule provenance must be an object")?;
    let expected = [
        "schema",
    ...
    if map["proc_libversion_major"].as_u64().is_none() || map["proc_libversion_minor"].as_u64().is_none() {
        bail!("process-joule provenance libproc version is invalid");
    }
    let cache_uuid =
        map["dyld_shared_cache_uuid"].as_str().context("process-joule shared-cache UUID is not a string")?;
    if !is_lower_hex(cache_uuid, 32) {
        bail!("process-joule shared-cache UUID is invalid");
    }
    for field in ["os_build", "machine"] {
        if map[field].as_str().filter(|value| !value.is_empty()).is_none() {
    ...
    let claimed = map["provenance_sha256"].as_str().context("process-joule provenance hash is not a string")?;
    if !is_lower_hex(claimed, 64) {
        bail!("process-joule provenance hash is invalid");
    }
    let mut unstamped = value.clone();
    unstamped.as_object_mut().expect("validated provenance object").remove("provenance_sha256");
...
impl ProcessJouleCollector {
    pub fn from_provenance_file(path: &Path) -> Result<Self> {
        ...
        Ok(Self { pid: std::process::id(), library_provenance })
    }

    pub fn snapshot(&self) -> Result<Snapshot> {
        let pid: c_int = self.pid.try_into().context("probe PID does not fit Darwin pid_t")?;
        ...
        let result =
            unsafe { proc_pid_rusage(pid, RUSAGE_INFO_V6, (&mut usage as *mut RUsageInfoV6).cast::<c_void>()) };
        let read_ended_at_continuous_ns = continuous_ns()?;
        let read_ended_at_unix_ns = wall_unix_ns()?;
        if result != 0 {
            bail!("proc_pid_rusage failed for probe PID {}: {}", self.pid, std::io::Error::last_os_error());
        }

        let process_uuid = usage.ri_uuid.iter().map(|byte| format!("{byte:02x}")).collect::<String>();
    ...
    pub fn phase_record(
        ...
        for (position, index) in MONOTONE_FIELD_INDEXES.iter().enumerate() {
            if after.monotone[position] < before.monotone[position] {
                bail!("direct process counter wrapped or decreased: {}", RUSAGE_V6_U64_FIELDS[*index]);
            }
        }
        let energy_nj_delta =
            after.energy_nj.checked_sub(before.energy_nj).context("direct process energy counter wrapped")?;
