...
use hawking_core::model::qwen38_hybrid_decode::{
    generate_greedy, generate_greedy_unmeasured, load_qwen38_tokenizer, render_qwen38_user_chat,
...
fn verify_contract_provenance(repo: &Path, raw: &str) -> Value {
    ...
    for (idx, item) in contracts.iter().enumerate() {
        ...
        if path.is_empty() || Path::new(path).is_absolute() {
            fail(format!(
                "contract[{idx}] canonical_path must be a repo-relative path"
            ));
        }
        let claimed_sha = row
            .get("sha256")
    ...
    out.insert("binding_sha256".into(), Value::String(measured_binding));
...
fn parse_args() -> Args {
    ...
    while let Some(flag) = args.next() {
        match flag.as_str() {
            ...
            "--genesis-contract-provenance" => {
                genesis_contract_provenance = Some(args.next().unwrap_or_else(|| fail(usage())));
            }
            "--max-seq-len" => {
                max_seq_len = args
                    .next()
    ...
    let genesis_contract_provenance = genesis_contract_provenance.unwrap_or_else(|| fail(usage()));
...
impl Body {
    fn load(
        ...
        let serve_untimed = matches!(env::var("HAWKING_QWEN38_SERVE_UNTIMED").as_deref(), Ok("1"));
