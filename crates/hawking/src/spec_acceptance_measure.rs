...
//
// Usage:
//   hawking-spec-acceptance-measure --weights <path> [--steps N]
//
...
use clap::Parser;
use hawking_core::{model::load_engine, EngineConfig};
use std::path::PathBuf;

#[derive(Parser, Debug)]
#[command(
    name = "hawking-spec-acceptance-measure",
...
fn main() -> Result<()> {
    ...
    let kernel_profile = cli
        .kernel_profile
        .map(|p| hawking_core::profile::KernelProfile::load(&p))
        .transpose()
    ...
    for (pi, &prompt) in PROMPTS.iter().enumerate() {
        ...
        //
        // NOTE: For real measurement, wire hawking_core::tokenizer directly.
