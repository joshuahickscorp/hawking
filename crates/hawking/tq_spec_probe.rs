...
mod macos {
    ...
    fn canonical_sha256(value: &Value) -> Result<String> {
        ...
        Ok(format!("{:x}", Sha256::digest(serde_json::to_vec(&ordered(value))?)))
    }
    ...
    impl PhaseRecorder {
        ...
        fn pair(
            ...
            let verifier_interval_id = self
                ...
                .iter()
                .find(|interval| {
                    interval.get("interval_sha256").and_then(Value::as_str) == Some(verifier_interval_sha256.as_str())
                })
                .and_then(|interval| interval.get("interval_id"))
                .and_then(Value::as_str)
        ...
        fn finish(mut self) -> Result<(Value, Value)> {
            ...
            self.energy_records.sort_by_key(|record| (record.batch, record.iteration));
    ...
    fn greedy_continuation(model: &mut QwenDense, prompt: &[u32], count: usize) -> Result<(Vec<u32>, Vec<u64>)> {
    ...
    fn verify_continuation(
        ...
        for (group, input) in inputs.chunks_exact(batch).enumerate() {
            ...
            let positions: Vec<usize> = (0..batch).map(|index| position0 + offset + index).collect();
            let start = Instant::now();
            ...
            if predictions.len() != batch {
                bail!("B={batch} verifier returned {} predictions for {batch} inputs", predictions.len());
            }
            mismatches +=
                predictions.iter().zip(&generated[offset..offset + batch]).filter(|(got, want)| got != want).count();
        }
        Ok((mismatches, total_ns))
    }

    fn baseline_corpus(model: &mut QwenDense, prompts: &[ResolvedPrompt], count: usize) -> Result<Vec<Vec<u32>>> {
        prompts
            .iter()
            .map(|prompt| greedy_continuation(model, &prompt.token_ids, count).map(|(generated, _)| generated))
            .collect()
    }
    ...
    fn baseline_mismatches(observed: &[Vec<u32>], canonical: &[Vec<u32>], count: usize) -> usize {
        observed
            .iter()
            .zip(canonical)
            .map(|(got, want)| got.iter().zip(&want[..count]).filter(|(left, right)| left != right).count())
            .sum()
    }
    ...
    fn balanced_batch_order(run_nonce: &str, phase: &str, round: usize) -> Vec<usize> {
        ...
        batches.sort_by_key(|batch| Sha256::digest(format!("{run_nonce}:{phase}:{round}:{batch}").as_bytes()).to_vec());
        batches
    }
    ...
    pub fn run() -> Result<()> {
        ...
        let runtime_path = hawking_core::TqRuntimePath::parse(&args.runtime_path).map_err(anyhow::Error::msg)?;
        let run_nonce =
        ...
        if prompt_set.tokenizer_sha256.len() != 64
            || !prompt_set.tokenizer_sha256.bytes().all(|byte| byte.is_ascii_hexdigit() && !byte.is_ascii_uppercase())
        {
        ...
        for prompt in &prompt_set.prompts {
            ...
            if prompt.text.is_some() == prompt.token_ids.is_some() {
                bail!("prompt {:?} must contain exactly one of text or token_ids", prompt.id);
            }
        }
        ...
        let tokenizer_path =
            args.weights.parent().map(|parent| parent.join("tokenizer.json")).filter(|path| path.is_file());
        let (tokenizer_sha256, tokenizer_source, tokenizer_size) = if let Some(path) = tokenizer_path.as_ref() {
        ...
        for prompt in &prompt_set.prompts {
            ...
            if token_ids.len() < 2 {
                bail!("prompt {:?} resolves to fewer than two token ids", prompt.id);
            }
            if token_ids.iter().any(|&token| token as usize >= model.config.vocab_size) {
                bail!("prompt {:?} contains an out-of-vocabulary token", prompt.id);
            }
            if token_ids.len() + args.generated_tokens + 7 > model.kv.max_seq {
                bail!("prompt {:?} plus verification window exceeds max_seq", prompt.id);
            }
        ...
        {
            bail!("TQ proof coverage is incomplete or uses the wrong runtime: {:?}", coverage);
        }

        let max_count = (1usize..=8).map(|batch| args.generated_tokens.div_ceil(batch) * batch).max().unwrap();
        ...
        let mut phase_recorder = PhaseRecorder::new(run_nonce.clone(), &args.process_joule_provenance)?;
        ...
        for warmup in 0..args.warmups_per_batch {
            for batch in balanced_batch_order(&run_nonce, "warmup", warmup) {
                ...
                let (baseline_mismatch, verifier_mismatch, baseline_marker, verifier_marker) = if baseline_first {
                    ...
                    (baseline_mismatch, verifier_mismatch, baseline_marker, verifier_marker)
                } else {
                    let (verifier_mismatch, _, verifier_marker) =
                        phase_recorder.measure("warmup", "verifier", batch, warmup, || {
                            verifier_corpus(&mut model, &resolved_prompts, &canonical, batch, count)
                ...
                warmup_markers[batch - 1].push(phase_recorder.pair(
                    ...
                    batch,
                    warmup,
                    if baseline_first { "baseline" } else { "verifier" },
                    baseline_marker,
                    verifier_marker,
                )?);
            }
        ...
        for repeat in 0..args.repeats_per_batch {
            for batch in balanced_batch_order(&run_nonce, "trial", repeat) {
                ...
                let (
                    ...
                    let baseline_mismatch = baseline_mismatches(&outputs, &canonical, count);
                    let (verifier_mismatch, verifier_wall_ns, verifier_marker) =
                        phase_recorder.measure("trial", "verifier", batch, repeat, || {
                            verifier_corpus(&mut model, &resolved_prompts, &canonical, batch, count)
                        })?;
                    (
                        baseline_mismatch,
        ...
        for batch in 1usize..=8 {
            ...
            let baseline_wall_ns: Vec<u64> = rows.iter().map(|row| row["baseline_wall_ns"].as_u64().unwrap()).collect();
            let verifier_wall_ns: Vec<u64> = rows.iter().map(|row| row["verifier_wall_ns"].as_u64().unwrap()).collect();
            let mismatches: u64 = rows.iter().map(|row| row["mismatches"].as_u64().unwrap()).sum();
