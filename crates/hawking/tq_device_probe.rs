...
mod macos {
    ...
    fn continuous_ns() -> Result<u64> {
        ...
        (ticks * info.numer as u128 / info.denom as u128).try_into().context("mach continuous nanoseconds overflow u64")
    }
    ...
    impl PhaseRecorder {
        fn new(run_nonce: String, process_joule_provenance: &Path) -> Result<Self> {
            Ok(Self {
                ...
                intervals: Vec::new(),
                pairs: Vec::new(),
                process_joule: ProcessJouleCollector::from_provenance_file(process_joule_provenance)?,
            ...
            F: FnOnce() -> Result<T>,
        {
            ...
            let output = output?;
            let elapsed_ns =
                continuous_ended_ns.checked_sub(continuous_started_ns).context("continuous clock moved backwards")?;
            if elapsed_ns == 0 {
        ...
        fn pair(
            ...
            let baseline_interval_id = self
                ...
                .iter()
                .find(|interval| {
                    interval.get("interval_sha256").and_then(Value::as_str) == Some(baseline_interval_sha256.as_str())
                })
            ...
            let candidate_interval_id = self
                ...
                .iter()
                .find(|interval| {
                    interval.get("interval_sha256").and_then(Value::as_str) == Some(comparison_interval_sha256.as_str())
                })
                .and_then(|interval| interval.get("interval_id"))
                .and_then(Value::as_str)
            ...
            if phase == "trial" {
                ...
                self.energy_records.push(PendingEnergyRecord { batch, iteration, value });
            }
            Ok(digest)
        }
        ...
        fn finish(mut self) -> Result<(Value, Value)> {
            ...
            self.energy_records.sort_by_key(|record| (record.batch.unwrap_or(0), record.iteration));
            let records = self.energy_records.into_iter().map(|record| record.value).collect();
    ...
    fn validate_admission() -> Result<()> {
        ...
        let raw = std::env::var("HAWKING_HEAVY_LEASE_FD").context("HAWKING_HEAVY_LEASE_FD is required")?;
        let fd: i32 = raw.parse().context("invalid HAWKING_HEAVY_LEASE_FD")?;
        let fd_meta = fs::metadata(format!("/dev/fd/{fd}")).context("inherited heavy-lease fd is not open")?;
    ...
    fn select_tensor<'a>(tensors: &'a [StrandTensor], requested: Option<&str>) -> Result<&'a StrandTensor> {
    ...
    fn activation(cols: usize) -> Vec<f32> {
        (0..cols).map(|index| ((index as f32 + 0.5) * 0.013_671_875).sin()).collect()
    }
    ...
    fn run_projection(
        ...
        match residual {
            Some(residual) => base.run_gemv_two_pass(ctx, residual).map_err(anyhow::Error::from),
            None => base.run_gemv(ctx).map_err(anyhow::Error::from),
        }
    }

    fn kernel_sequence(tensor: &StrandTensor, runtime_path: TqRuntimePath, accumulate: bool) -> Vec<String> {
        ...
        sequence.push(
            if accumulate { "strand_bitslice_reduce_rows_accum" } else { "strand_bitslice_reduce_rows" }.to_owned(),
        );
    ...
    pub fn run() -> Result<()> {
        ...
        if !(7..=64).contains(&args.source_commit.len())
            || !args.source_commit.bytes().all(|byte| byte.is_ascii_hexdigit() && !byte.is_ascii_uppercase())
        {
        ...
        let run_nonce =
            std::env::var("HAWKING_PHYSICAL_RUN_NONCE").context("HAWKING_PHYSICAL_RUN_NONCE is required")?;
        if run_nonce.len() != 64
            || !run_nonce.bytes().all(|byte| byte.is_ascii_hexdigit() && !byte.is_ascii_uppercase())
        {
        ...
        if args.matrix_cell_sha256.len() != 64
            || !args.matrix_cell_sha256.bytes().all(|byte| byte.is_ascii_hexdigit() && !byte.is_ascii_uppercase())
        {
            bail!("matrix_cell_sha256 must be 64 lowercase hexadecimal characters");
        }
        let bytes = fs::read(&args.artifact).with_context(|| format!("read artifact {}", args.artifact.display()))?;
        ...
        let residual_source = match (&args.residual_artifact, &args.residual_tensor) {
            (Some(path), Some(name)) => {
                let residual_bytes =
                    fs::read(path).with_context(|| format!("read residual artifact {}", path.display()))?;
        ...
        let residual_baseline = residual_tensor
            ...
            .transpose()?;
        let residual_candidate =
            residual_tensor.map(|tensor| TqDeviceHarness::prepare(&ctx, tensor, runtime_path, &x)).transpose()?;
        ...
        let q12_mismatches = gpu_q12.iter().zip(&cpu_q12).filter(|(got, want)| got != want).count();
            ...
            (residual_tensor, residual_candidate.as_ref())
        {
            ...
            let (gpu_residual_q12, _, marker) =
                phase_recorder.measure("parity", "candidate_residual_q12", None, 1, || {
                    residual_candidate.decode_q12(&ctx).map_err(anyhow::Error::from)
                })?;
            let mismatches = gpu_residual_q12.iter().zip(&cpu_residual_q12).filter(|(got, want)| got != want).count();
        ...
        let fused_phase_marker_sha256 =
            phase_recorder.pair("parity", None, 0, "baseline", baseline_parity_interval, candidate_parity_interval)?;
        ...
        for warmup in 0..args.warmups {
            ...
            let (baseline_interval, candidate_interval) = if baseline_first {
                let (_, _, baseline_interval) = phase_recorder.measure("warmup", "baseline", None, warmup, || {
                    run_projection(&ctx, &baseline, residual_baseline.as_ref()).map(|_| ())
                })?;
                let (_, _, candidate_interval) = phase_recorder.measure("warmup", "candidate", None, warmup, || {
            ...
            warmup_phase_marker_sha256.push(phase_recorder.pair(
                ...
                None,
                warmup,
                if baseline_first { "baseline" } else { "candidate" },
                baseline_interval,
                candidate_interval,
            )?);
        }
        ...
        for trial in 0..args.trials {
            if trial % 2 == 0 {
                let (_, wall, baseline_interval) = phase_recorder.measure("trial", "baseline", None, trial, || {
                    run_projection(&ctx, &baseline, residual_baseline.as_ref()).map(|_| ())
                })?;
        ...
        let rht_blocks = if tensor.rht_mode == RhtMode::Cols { tensor.in_features / 256 } else { 0 };
        let base_kernel_sequence = kernel_sequence(tensor, runtime_path, false);
        let residual_kernel_sequence = residual_tensor.map(|tensor| kernel_sequence(tensor, runtime_path, true));
        ...
        if dispatches != kernel_sequence.len() {
            bail!("observed dispatch count {dispatches} differs from exact pass sequence {}", kernel_sequence.len());
        }
        ...
        if let (Some((_, residual_sha256, _)), Some(residual_tensor), Some(sequence)) =
            (residual_source.as_ref(), residual_tensor, residual_kernel_sequence.as_ref())
        {
            ...
            let residual_rht_blocks =
                if residual_tensor.rht_mode == RhtMode::Cols { residual_tensor.in_features / 256 } else { 0 };
        ...
        let rht_cols_passes =
            pass_sequence.iter().filter(|pass| pass.get("rht_mode").and_then(Value::as_str) == Some("cols")).count();
        let outlier_corrected_passes = pass_sequence
            .iter()
            .filter(|pass| pass.get("outlier_count").and_then(Value::as_u64).unwrap_or(0) > 0)
            .count();
        ...
        let payload = traffic.payload_bytes + residual_traffic.map(|value| value.payload_bytes).unwrap_or(0);
        let metadata = traffic.metadata_bytes(runtime_path)
            + residual_traffic.map(|value| value.metadata_bytes(runtime_path)).unwrap_or(0);
        let codebook_staging = traffic.staged_codebook_bytes(runtime_path)
            + residual_traffic.map(|value| value.staged_codebook_bytes(runtime_path)).unwrap_or(0);
        let partial_roundtrip =
            traffic.partial_roundtrip_bytes + residual_traffic.map(|value| value.partial_roundtrip_bytes).unwrap_or(0);
        ...
        let total_projection_weights = traffic.weights + residual_traffic.map(|value| value.weights).unwrap_or(0);
        let residual_probe = if let (
            ...
            (residual_source.as_ref(), residual_tensor, residual_candidate.as_ref(), residual_q12_evidence.as_ref())
        {
            json!({
