...
| 3 | Router v1 (expected_gain arbiter) | FUNCTIONAL | `HAWKING_QWEN_EVENT_HORIZON` | `router.rs` ProposalRouter::plan() with per-slot EWMA CostModel, expected_gain_ns, margin hysteresis, plan_shape(context_confidence), observe_disabled; unit tests: fast-target auto-kill + slow-target enable + gated-slot denial |
