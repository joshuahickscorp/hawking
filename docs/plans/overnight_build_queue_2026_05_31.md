...
3. **f16s default-on — SETTLED = OPT-IN (Lane C drift sweep done).** Broad sweep (24 prompts): **8.2% corpus drift, category-correlated** — constrained (code/SQL/factual/lists/non-English) 0%, open-ended (math 18%, dialogue 30%, prose 23%) drifts where the argmax is a near-tie. NO-GO default-on. The LM-head-exclusion refinement is DE-PRIORITIZED: the FFN-pair f16s also perturbs the residual → drift cascades in open-ended gen regardless of the LM head, so excluding it wouldn't rescue default-on. f16s stays a sound **opt-in** (+6-9%) for constrained/latency-sensitive workloads.

## Lane C done (2026-05-31)
- **A6.5 f16s drift sweep:** opt-in confirmed (above). Tooling + result: `reports/bench/f16s_drift_sweep.py` / `_result.json` / `f16s_and_b2_longctx_findings.md`.
- **B2 long-ctx KV-working-set:** BLOCKED, not re-killed. The CPU attn-capture path is O(N²·logN)-ish — 1200 + 3200-tok attempts both hit the CPU-time governor (exit 144, ~45-55 min) before flushing JSON; no >586-tok data point obtainable. Type-1 NO-GO stands (no GO evidence). **Resurrection oracle is BLOCKED on a cheaper capture path** (GPU `scores` spill / cumulative-curve-only instrument / a box without the CPU cap) before a 16-32K test can run. New: frac99 ↔ layer-depth corr **+0.50** (deeper = more diffuse). **Hard-cap long-ctx captures next time.**
