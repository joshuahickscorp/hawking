...
| Preflight | 10.0 | `hawking studio preflight` checks RAM/disk/deps/compile plus the HIDE app Node/pnpm engine, writes refresh/ledger/launch-gate artifacts, signs a machine/network/power/thermal/developer-environment preflight summary with SHA-256, `hawking studio verify-summary` verifies that summary on demand, `environment-capture|environment-verify` captures the same launch environment independently, and the gate correctly stays red without licenses/disk or app-engine mismatch. | Keep the summary and environment schemas stable as new launch gates are added. |
