...
| **4.4 WgpuBackend implementation** | Cross-vendor reach (AMD/Intel/NVIDIA/Android) | Cargo.toml `wgpu = { version = "0.20", features = ["vulkan", "metal", "dx12"] }` needs user approval first. Then: scaffolding → cheap ops (WGSL) → attention → BackendGemv Q4_K. Scope in `reports/phase_4_4_cross_vendor_scope.md`. |
