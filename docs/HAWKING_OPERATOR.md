# HAWKING: the Hawking operator

`hawking` is the cognitive shell of the Hawking computer. One organism, different
authority. You state an objective; it does the engineering.

## Commands (no flags, any directory)

```bash
hawking web                 # browser chat on the current resident, read/research
hawking build               # the same chat WITH repo-scoped write authority
hawking use                 # list admitted bodies; use an exact name/path to switch
hawking serve               # the OpenAI endpoint only (Open WebUI points here)
hawking report              # measure the loaded body: cold prefill, warm reuse, decode
hawking stop                # put down what web/serve/build started
hawking --help              # every verb, grouped
```
...
| PROFILE | profile prefill and wall | `hawking report` |
...
`receipts/future/HAWKING_OPERATOR_LAWS.json`.
