...
`hawking/command_registry.py` is the single source of truth: `/help`, completion,
...
```bash
hawking "keep working on sub-2 and tell me only if the capability cliff changes"
```

## Which code does `hawking` actually run

There is one HAWKING product name with two deliberately different layers:

* Python `hawking` is the orchestration/resident skin. It owns the long-lived
  AgentOS loop and provider-specific campaign work.
* Rust `hawking` (`cargo run -p hide-backend --bin hawking`) is the current HIDE
  backend authority. It owns durable backend sessions, receipts, tools,
  JSONL control, and runtime-facing composition.

The visual/IDE/ACP surface is not active. Rebuild it only behind Hawking's
perception boundary; it must consume the Rust backend contract instead of
creating a second state or command authority.

Two different install paths exist, and they are not the same code:

| how you installed | what `hawking` runs |
|---|---|
| `python3 -m pip install -e .` | the repo, live — always current |
| `python3 -m hawking install-shims` | a stamped snapshot under `~/.local/share/hawking/` |

The snapshot is frozen at the moment you installed it. It drifted six days
behind the repo once and nothing said so, which meant `hawking` and
`PYTHONPATH=. python3 -m hawking` silently ran two different codebases. Startup now
compares the snapshot against the source it was copied from and says so if they
differ. Refresh it with:

```bash
python3 -m hawking install-shims
```

**One more wrinkle: your working directory decides.** The shim sets
`PYTHONPATH`, but Python puts the current directory first, so `hawking` run *from
inside the repo* imports the repo and `hawking` run from anywhere else imports the
...
|---|---|---|
| `<repo>/.hawking/` | missions, resident state, background jobs, locks | working state, rebuilt on demand |
| `<repo>/.hawking/pastes/` | cached pastes | **yes** — disposable by definition |
| `<repo>/receipts/` | receipts and evidence | **no** |
| `<repo>/civilization/` | roadmap and obligation state | **no** |
| `~/.local/share/hawking/` | stamped install snapshots | yes, except the `current` symlink target |
...
message records as
`<workspace>/.hawking/sessions/<session-id>.history.jsonl.gz`. Session shutdowns
and compactions also update the workspace prior-knowledge index at
`<workspace>/.hawking/knowledge.json`; its older bounded records are kept in
...
`.hawking/config.json` (`context_archive_root`); another machine can use
`HAWKING_CONTEXT_ARCHIVE_ROOT` or omit it and keep the archive beside the index.
...
The queue is workspace-scoped at `<workspace>/.hawking/goal-bank.json`, survives
new HAWKING sessions, and starts FIFO after a successful active goal. A failed or
cancelled goal stops promotion; its remaining bank stays intact. A dead HAWKING
...
```bash
hawking resident status                 # no model is opened by this
hawking resident start --goal "..."
hawking resident bank --mode mission "run the overnight campaign"
hawking resident stop
```

`hawking status` prints the machine-level view: background jobs, resource
ownership, recovery state.

## Leaving without stopping the work

Closing the CLI is not stopping the mission. A CLI session, an HAWKING mission, and
