...
**Version:** `HAWKING_NOMENCLATURE_V3`
...
surfaces. There is no public compatibility CLI or peer-product namespace:
implementation legs resolve through Hawking and are not product identities.
...
|---|---|---|---|---|---|---|---|---|
| SEMANTIC_CHANGE | Retired Python CLI and peer-backend labels | Hawking CLI / Hawking Web / `hawkingd` | `crates/hawking/src/main.rs`; `hawking/cli.py`, `hawking/serve.py`, `hawking/web.py`; OpenWebUI | An invocation resolves one admitted artifact revision and one supported action; unknown identities fail closed. | Hawking package entry points, `HAWKING_*` environment names, canonical port 8014, install stamps and browser integration. | No forwarding command or package is public; historical receipt schemas remain readable evidence. | `crates/hawking/src/main.rs::public_surface_tests`; Hawking CLI, web and serve tests. | IMPLEMENTED_PUBLICLY. `hawking` and `h` are the only Python command spellings. |
| OWNER_CONSOLIDATION | Prior peer orchestration name | Hawking internal capability and authority machinery | Private control legs, WorkUnits, resident gates, perception and supervisor callers. | Preserve explicit capability checks, mutation authority and bounded work execution. | Serialized WorkUnits, tool registry IDs, receipts and process ownership. | `from hawking import Hawking` and public operational symbols resolve lazily from the root. | Hawking namespace, restart, mutation and resident tests. | IMPLEMENTED_PUBLICLY. The implementation package remains private; no command requires its name. |
| OWNER_CONSOLIDATION | Multiple Gravity framings | Gravity with an explicit target, budget and policy | `hawking/gravity*.py`, foundry and condense tools, plans and historical receipts. | One bounded discovery/transformation function retains cancellation, budget, run history and qualification. | Campaign receipts, schemas, command aliases and protected model permissions. | Preserve historical schemas; emit the canonical definition in new public interfaces. | Gravity gauntlet, foundry lifecycle and nomenclature tests. | IN_PROGRESS. Remove a duplicate live engine only after every production caller uses the canonical owner. |
| RENAME_ONLY | Doctor / Tabula product framing | Hawking diagnosis and Gravity science sidecar | `hawking/perception/doctor.py`, `tools/future/tabula.py`, `hawking/tool_registry.py`. | Diagnose or propose; neither path grants execution qualification. | Tool IDs, saved invocations and historical receipts. | Keep exact tool entry points and describe them as Hawking/Gravity operations. | Existing doctor, Tabula and tool-registry tests. | IMPLEMENTED_PARTIAL. The runtime scorer is `hawking.behavior`; the numerical sidecar stays optional. |
| SEMANTIC_CHANGE | Transient Noetic pipeline vocabulary | NR revision -> physical plan -> NX candidate | `hawking/nomenclature.py`, native artifact loaders, `hawking/physical_graph.py`, receipts. | Preserve semantic revision identity separately from immutable deployment realization and instance state. | V1 schemas, hashes, artifact paths and sealed receipts. | Read V1 forever; emit V2 only at new serialization boundaries; never rewrite evidence. | Hawking nomenclature and science-boundary tests. | IN_PROGRESS. Old schemas remain historical compatibility; new callers must use the V2 interpretation. |
| RENAME_ONLY | Singularity / Singularity Profile | selected NX / execution profile | `hawking/nomenclature.py`, roadmap prose and historical selection records. | A selection binds an exact NX and machine-relative execution contract. | Persisted historical names and receipt hashes. | Read aliases as `SelectedNX` / `ExecutionProfile`; do not rewrite receipts. | Nomenclature and terminology-guard tests. | IN_PROGRESS. Remove new public use after the bounded terminology check is part of all release verification. |
