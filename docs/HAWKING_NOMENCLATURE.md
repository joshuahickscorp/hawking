...
| OWNER_CONSOLIDATION | Method registry copies | `tools/foundry/GRAVITY_METHOD_REGISTRY.json` with `gravity_potency.py` as its policy reader | Foundry lifecycle, acquisition and post-parent review tools. | Retrieve one sealed generation, reject duplicate IDs and apply scoped applicability/falsifier rules before reuse. | Potency ledger, negative-transfer atlas, review seals and parent evidence. | Preserve research imports and sealed generation bytes. | `tools/foundry/tests/test_foundry_tables_lifecycle.py`. | UNMET_IN_THIS_BRANCH. The research reader still defaults to a noncanonical directory and no future-work caller automatically invokes a selected method; fix and prove both before calling reuse complete. |
