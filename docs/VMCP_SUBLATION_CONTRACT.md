...
Whole package: 757 files, 298,913 lines. The port owes a fraction of that,
because `call_vmcp` only ever builds the core profile and calls one of 9 tools.

| scope | files | lines | % of package |
|---|---|---|---|
| shared baseline, paid on every call | 29 | 12,745 | 4.3% |
| 8 tools + vision.verify's common capture_id case | 34 | **15,229** | **5.1%** |
| + vision.verify's `receipt_path` argument | +33 | +20,204 | |
| + the rare `worldir.world` conditional branch | +144 | +54,764 | |
| primary union | 67 | 35,433 | 11.9% |
| full static union | 211 | 90,197 | 30.2% |

Five of the nine tools cost NOTHING beyond the baseline: system.doctor,
vision.capabilities, project.status, vision.list_artifacts. get_artifact adds
114 lines; query and progress add 980; observe adds 2,370 more.

**The whole heavy tail is vision.verify, and not the tool -- two of its
argument paths.** `verify(capture_id)` is free, reusing observe's bus and query.
`verify(receipt_path)` alone pulls acceptance/receipts.py and with it evidence,
visual_geometry, cameras, comparison, generative3d: 20,204 lines, more than the
other eight tools combined. The `worldir.world` branch fires only on a capture
written by a different profile and pulls 54,764 more, including a CAD kernel,
Metal GPU code and a browser backend.

Dropping both leaves ~15,229 lines. That is the difference between a weekend
and a month.

## Dependencies -- and visionmcp's own manifest is wrong about them

`visionmcp/pyproject.toml:24-27` states "numpy and Pillow are NOT absorbed...
because no core-server path reaches them." Measured, they are reached:
`perception/media.py:16-17` imports numpy and PIL at module level, and cv2 is
required unconditionally by `vision.observe` through `core/optional.py`'s
`lazy_optional` proxy, which is why a plain grep misses it
(`media.py:76-83` reads `cv2.__version__` on every observe;
`media.py:618-693` calls cvtColor/Canny/findContours).

| package | in hawking? |
|---|---|
| numpy | yes, informally (12 hcli files; not declared) |
| Pillow | **no** |
| opencv-python-headless | **no** -- native binary wheel |
| jsonschema | no -- only the rare worldir branch needs it |
| playwright | no -- only the rare worldir branch |

So a straight port of `vision.observe` adds Pillow AND OpenCV.

**But file_eye.py already classifies 12 formats with neither.** That is the
argument for REWRITING observe on the local organ rather than porting it: the
same capability without two heavy dependencies. What file_eye does not have is
the other half -- ProjectStore/ArtifactStore persistence, capture_id, the
adapter contract -- and its own docstring says so (`file_eye.py:797-799`).

Five tools have no local precedent at all (query, progress, list_artifacts,
get_artifact, project.status) and would be written fresh. They are also the
cheapest: mostly ProjectStore/SQLite plumbing.

## State: the 624 MB is not needed

Verified three ways. `core/config.py:67-71 default_projects_root()` resolves to
an OS user-data dir, not the checkout. `call_vmcp` already requires the caller
to pass `projects_root`, which hawking owns. No source path references the
repo-relative artifacts/ directory. All state is per-project SQLite plus
content-addressed blobs, created fresh under the supplied root.

The 624 MB artifacts/ tree is release and benchmark output, structurally
disconnected from all 9 tools.
