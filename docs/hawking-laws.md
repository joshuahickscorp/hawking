...
`hawking/tool_registry.py` those identifiers were `{hawking, tests, tool_registry}` --
...
Byte counts cannot answer "which 6027 characters". `HAWKING_DUMP_EVIDENCE` exists
...
`tools/hawking_laws.py`, which runs the workflow a remediation promises rather than
