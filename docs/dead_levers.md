...
**Reframe considered (ALIVE, Type-2):** the gap is real (llama.cpp = ~49 tps on this same M3 Pro vs dismantle ~30.5) and the trace says it is **kernel-bound**, not idle — so the surviving formulation is "**llama's `mul_mv` Metal kernel runs the same Q4_K bytes in less GPU-time** (higher effective BW and/or fewer GPU-µs) via a technique dismantle's pre-decoded GEMV doesn't match." This is a *different axis* from both the dead idle-reclaim form and the dead GEMV-micro-opt form (which tuned dismantle's OWN kernel, not llama's algorithm).
**Why it stays parked until its oracle fires:** the named, in-hand oracle is a **Metal System Trace (Instruments) per-token timeline diff of llama.cpp vs dismantle on M3 Pro** — does llama's GEMV sustain higher GiB/s per call, and by what mechanism (simdgroup_matrix tiling at M=1, a different weight layout, fused dequant)? Until that trace is run, the reframe is unproven on this hardware. **Caution / prior:** dismantle already killed simdgroup-MMA at M=1 ([[qwen3b-dead-levers]], occupancy) and GEMV micro-opt (memory-model optimum) — so the reframe is *alive but adverse*; it may reduce to those dead forms once traced. Effort: medium (build/trace llama) then medium-high (port the technique), uncertain payoff.
**Resurrection check:** do NOT re-spawn "reclaim inter-dispatch idle / push GPU-busy / command-buffer batching / ICB-at-scale" as a tps lever — production is already ~100% intra-token GPU-busy (measured 2026-06-02, Type-1). The ONLY surviving tps reframe is the **llama-kernel-technique**, and it is admissible only **behind the Metal System Trace diff oracle** above — a measured llama-vs-dismantle per-kernel GiB/s delta on M3 Pro. No trace → stays parked.
