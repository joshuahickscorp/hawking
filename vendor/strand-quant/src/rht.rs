...
impl RhtConfig {
    pub fn from_seed(seed: u64) -> Self {
        RhtConfig { seed, block: HADAMARD_BLOCK }
    }
}
...
mod tests {
    ...
    fn rht_round_trip_identity_pow2_multiple() {
        let n = 4864usize;
        let x: Vec<f32> = (0..n).map(|i| ((i as f32) * 0.013).sin() * 0.3 + ((i % 7) as f32) * 0.05).collect();
    ...
    fn rht_rows_round_trip_awkward_in_features() {
        ...
        for &(out_f, in_f) in &[(8usize, 896usize), (3, 896), (7, 100), (1, 100), (5, 768 + 128)] {
            ...
            let d = max_abs_diff(&x, &back);
            assert!(d < 1e-3, "row-aware RHT round-trip not identity (out={out_f}, in={in_f}): max abs diff {d}");
            let (ex, ey) = (l2(&x), l2(&fwd));
            assert!((ex - ey).abs() / ex.max(1e-12) < 1e-3, "row-aware RHT not energy-preserving (out={out_f}, in={in_f}): {ex} vs {ey}");
        }
    }
    ...
    fn rht_rows_matches_flat_when_in_features_256_aligned() {
        ...
        for &(out_f, in_f) in &[(8usize, 256usize), (4, 512), (6, 768), (3, 1024), (5, 4864)] {
            ...
            assert_eq!(flat, rows, "row-aware RHT diverged from flat at aligned in_features={in_f}");
        }
    }
    ...
    fn rht_is_energy_preserving() {
        ...
        assert!((ex - ey).abs() / ex.max(1e-12) < 1e-3, "RHT not energy-preserving: {ex} vs {ey}");
    }

    #[test]
    fn rht_gaussianizes_clustered_weights() {
        let n = 8192usize;
        let x: Vec<f32> = (0..n).map(|i| if i % 2 == 0 { 1.0 } else { -1.0 } + ((i % 13) as f32) * 0.01).collect();
        ...
        assert!((k_after - 3.0).abs() < (k_before - 3.0).abs(), "RHT did not move kurtosis toward Gaussian: before {k_before:.3} after {k_after:.3}");
    }
