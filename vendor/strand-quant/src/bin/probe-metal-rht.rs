...
fn parse_args() -> Args {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--staging-root" => staging_root = Some(args.next().expect("--staging-root needs PATH").into()),
            "--admission-receipt" => admission_receipt = Some(args.next().expect("--admission-receipt needs PATH").into()),
            "--lease-fd" => lease_fd = Some(args.next().expect("--lease-fd needs FD").parse().expect("--lease-fd must be an integer")),
            "--dispatch" => dispatch = true,
            other => panic!("unknown argument {other}"),
        }
    }
    Args { receipt: receipt.expect("--receipt is required"), staging_root: staging_root.expect("--staging-root is required"), admission_receipt, lease_fd, dispatch }
}
...
fn confined_path(root: &Path, path: &Path, must_exist: bool) -> Result<PathBuf, String> {
    if fs::symlink_metadata(root).map_err(|error| format!("inspect staging root: {error}"))?.file_type().is_symlink() {
        return Err("staging root must not be a symlink".into());
    }
    let root = fs::canonicalize(root).map_err(|error| format!("canonicalize staging root: {error}"))?;
    let resolved = if must_exist {
        fs::canonicalize(path).map_err(|error| format!("canonicalize staged input: {error}"))?
    } else {
        let parent = path.parent().unwrap_or_else(|| Path::new("."));
        fs::canonicalize(parent).map_err(|error| format!("canonicalize receipt parent: {error}"))?.join(path.file_name().ok_or("receipt path has no file name")?)
    };
...
fn scan_json_value(bytes: &[u8], start: usize) -> Result<usize, String> {
    match bytes.get(start) {
        ...
        Some(_) => {
            ...
            let end = (start..index).rev().find(|candidate| !bytes[*candidate].is_ascii_whitespace()).map(|candidate| candidate + 1).ok_or("empty JSON value")?;
            Ok(end)
        }
...
fn top_level_field<'a>(document: &'a str, requested: &str) -> Result<Option<TopLevelField<'a>>, String> {
    ...
    loop {
        ...
        let raw_key = document.get(index + 1..key_end - 1).ok_or("admission key is not valid UTF-8")?;
        if raw_key.contains('\\') {
        ...
        if raw_key == requested {
            ...
            found = Some(TopLevelField { raw_value: document.get(value_start..value_end).ok_or("admission value is not valid UTF-8")?, pair_start, pair_end: value_end });
        }
...
fn top_level_string(document: &str, field: &str) -> Result<Option<String>, String> {
    ...
    if !value.raw_value.starts_with('"') || !value.raw_value.ends_with('"') || value.raw_value[1..value.raw_value.len() - 1].contains('\\') {
        return Err(format!("top-level admission field {field} is not a plain string"));
    }
    Ok(Some(value.raw_value[1..value.raw_value.len() - 1].to_string()))
}
...
fn top_level_u128(document: &str, field: &str) -> Result<Option<u128>, String> {
    ...
    if value.raw_value.is_empty() || !value.raw_value.bytes().all(|byte| byte.is_ascii_digit()) {
        return Err(format!("top-level admission field {field} is not an unsigned integer"));
    }
    value.raw_value.parse().map(Some).map_err(|_| format!("top-level admission field {field} overflows u128"))
}
...
fn verify_document_self_hash(document: &str) -> Result<(), String> {
    let document = document.strip_suffix('\n').unwrap_or(document);
    let field = top_level_field(document, "document_sha256")?.ok_or("admission document self-hash field is absent")?;
    let value = top_level_string(document, "document_sha256")?.ok_or("admission document self-hash field is absent")?;
    if value.len() != 64 || !value.bytes().all(|byte| byte.is_ascii_hexdigit() && !byte.is_ascii_uppercase()) {
        return Err("admission document self-hash is not lowercase SHA-256".into());
    }
    let bytes = document.as_bytes();
    let (remove_start, remove_end) = if field.pair_start > 0 && bytes.get(field.pair_start - 1) == Some(&b',') {
...
fn verify_bound_source(document: &str, path_field: &str, sha_field: &str, expected: &Path) -> Result<(), String> {
    let recorded = top_level_string(document, path_field)?.ok_or_else(|| format!("admission {path_field} is missing"))?;
    let expected = fs::canonicalize(expected).map_err(|error| format!("resolve expected admission source: {error}"))?;
    let expected_digest = file_digest(&expected);
    if fs::canonicalize(&recorded).map_err(|error| format!("resolve recorded admission source: {error}"))? != expected
        || top_level_string(document, sha_field)?.as_deref() != Some(expected_digest.as_str())
    {
        return Err(format!("admission {path_field}/{sha_field} binding differs"));
    }
    Ok(())
}

fn verify_lease_owner(fd: i32, path: &str, expected_device: u128, expected_inode: u128) -> Result<(), String> {
...
"#;
    let status = Command::new("/usr/bin/python3")
        .args(["-c", SCRIPT, &fd.to_string(), path, &expected_device.to_string(), &expected_inode.to_string()])
        .status()
        .map_err(|error| format!("run lease ownership verifier: {error}"))?;
    if !status.success() {
        return Err(format!("inherited heavy lease is not exclusively owned ({status})"));
    }
    Ok(())
}

fn verify_dispatch_admission(args: &Args) -> Result<String, String> {
    let admission = args.admission_receipt.as_deref().ok_or("--dispatch requires --admission-receipt")?;
    ...
    let modified = fs::metadata(&admission).and_then(|metadata| metadata.modified()).map_err(|error| format!("inspect admission receipt time: {error}"))?;
    if SystemTime::now().duration_since(modified).map_err(|_| "admission receipt modification time is in the future")? > std::time::Duration::from_secs(60) {
        return Err("resource admission receipt file is stale".into());
    }
    let document = fs::read_to_string(&admission).map_err(|error| format!("read resource admission receipt: {error}"))?;
    verify_document_self_hash(&document)?;
    if top_level_string(&document, "schema")?.as_deref() != Some("hawking.strand.native-probe-admission.v1")
        || top_level_string(&document, "generator_schema")?.as_deref() != Some("hawking.strand.native-probe-launcher.v1")
    ...
    if std::env::var("HAWKING_NATIVE_PROBE_ADMITTED").as_deref() != Ok("1") || std::env::var("HAWKING_NATIVE_PROBE_ADMISSION_SHA256").as_deref() != Ok(digest.as_str()) {
        return Err("admission receipt environment authority differs".into());
    }
    let repository = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..").canonicalize().map_err(|error| format!("resolve repository root: {error}"))?;
    verify_bound_source(&document, "generator_path", "generator_sha256", &repository.join("vendor/strand-quant/tools/native_probe.py"))?;
    verify_bound_source(&document, "owner_pattern_source_path", "owner_pattern_source_sha256", &repository.join("tools/condense/doctor_v5_local_observer.py"))?;
    let now = SystemTime::now().duration_since(UNIX_EPOCH).map_err(|_| "clock before epoch")?.as_nanos();
    let generated = top_level_u128(&document, "generated_unix_ns")?.ok_or("admission time missing")?;
    let expires = top_level_u128(&document, "expires_unix_ns")?.ok_or("admission expiry missing")?;
    ...
    if fs::canonicalize(&lease_path).map_err(|error| format!("resolve heavy lease path: {error}"))?
        != fs::canonicalize(&expected_lease).map_err(|error| format!("resolve expected heavy lease: {error}"))?
    {
        return Err("admission receipt names a non-Studio heavy lease".into());
    }
    let staging_in_receipt = top_level_string(&document, "staging_root")?.ok_or("staging root missing")?;
    if fs::canonicalize(&staging_in_receipt).map_err(|error| format!("resolve admitted staging root: {error}"))?
        != fs::canonicalize(&args.staging_root).map_err(|error| format!("resolve requested staging root: {error}"))?
    {
        return Err("admission receipt is for a different staging root".into());
    }
    let admitted_probe = top_level_string(&document, "probe_sha256")?.ok_or("probe hash missing")?;
    let executable = std::env::current_exe().map_err(|error| format!("current executable: {error}"))?;
...
fn exact_bits(left: &[f32], right: &[f32]) -> bool {
    left.len() == right.len() && left.iter().zip(right).all(|(left, right)| left.to_bits() == right.to_bits())
}

fn main() {
    let args = parse_args();
    let receipt_path = confined_path(&args.staging_root, &args.receipt, false).unwrap_or_else(|error| panic!("probe receipt confinement: {error}"));
    let admission_sha256 = if args.dispatch { Some(verify_dispatch_admission(&args).unwrap_or_else(|error| panic!("physical dispatch admission: {error}"))) } else { None };
    ...
    let (status, device, rows_exact, cols_exact, dispatch_wall_ns) = if args.dispatch {
        ...
        let rows = probe.forward(&input, seed, 256, RhtAxis::Rows).unwrap_or_else(|error| panic!("Metal row RHT: {error}"));
        let cols = probe.forward(&input, seed, 256, RhtAxis::Cols).unwrap_or_else(|error| panic!("Metal col RHT: {error}"));
        let rows_exact = exact_bits(&cpu_rows, &rows);
        let cols_exact = exact_bits(&cpu_cols, &cols);
        (if rows_exact && cols_exact { "pass" } else { "fail" }, probe.device_name(), Some(rows_exact), Some(cols_exact), Some(started.elapsed().as_nanos()))
    } else {
    ...
    let option_u128 = |value: Option<u128>| value.map(|value| value.to_string()).unwrap_or_else(|| "null".into());
    let receipt = format!(
    ...
    strand_quant::native_io::write_preallocated(&receipt_path, receipt.as_bytes()).expect("atomically write Metal RHT receipt");
    print!("{receipt}");
    if status == "fail" {
        std::process::exit(2);
    }
...
mod tests {
    ...
    fn admission_fields_parse_exactly() {
        ...
        assert_eq!(top_level_string(document, "schema").unwrap().as_deref(), Some("admitted"));
        assert_eq!(top_level_u128(document, "active_heavy_owner_count").unwrap(), Some(0));
        assert_eq!(top_level_u128(document, "expires_unix_ns").unwrap(), Some(42));
        assert_eq!(top_level_bool(document, "resource_health_ok").unwrap(), Some(true));
    ...
    fn actual_launcher_shape_uses_only_top_level_authority() {
        let body =
            r#"{"active_heavy_owner_count":0,"resource_health":{"snapshot":{"schema":"hawking.studio_resource_snapshot.v1"}},"schema":"hawking.strand.native-probe-admission.v1","status":"admitted"}"#;
        let sealed = format!(
        ...
        assert_eq!(top_level_string(&sealed, "schema").unwrap().as_deref(), Some("hawking.strand.native-probe-admission.v1"));
        assert!(verify_document_self_hash(&sealed).is_ok());
        let duplicate = sealed.replace("\"status\":\"admitted\"", "\"schema\":\"forged\",\"status\":\"admitted\"");
    ...
    fn admission_document_self_hash_is_mandatory_and_exact() {
        ...
        let sealed = format!("{{\"active_heavy_owner_count\":0,\"document_sha256\":\"{}\",\"schema\":\"test\"}}\n", digest(body.as_bytes()));
        assert!(verify_document_self_hash(&sealed).is_ok());
        assert!(verify_document_self_hash(&sealed.replace("\"schema\":\"test\"", "\"schema\":\"tampered\"")).is_err());
        assert!(verify_document_self_hash(body).is_err());
    }

    #[test]
    fn receipt_path_cannot_escape_staging_root() {
        let root = std::env::temp_dir().join(format!("strand-metal-probe-stage-{}", std::process::id()));
        let outside = std::env::temp_dir().join(format!("strand-metal-probe-outside-{}.json", std::process::id()));
        let _ = fs::remove_dir_all(&root);
        fs::create_dir(&root).unwrap();
        let inside = root.join("receipt.json");
        assert_eq!(confined_path(&root, &inside, false).unwrap(), fs::canonicalize(&root).unwrap().join("receipt.json"));
    ...
    fn unheld_descriptor_cannot_self_admit_during_verification() {
        ...
        let path = std::env::temp_dir().join(format!("strand-metal-probe-unheld-{}.lock", std::process::id()));
        let _ = fs::remove_file(&path);
        let file = OpenOptions::new().read(true).write(true).create_new(true).open(&path).unwrap();
        let metadata = file.metadata().unwrap();
        assert!(verify_lease_owner(file.as_raw_fd(), path.to_str().unwrap(), metadata.dev() as u128, metadata.ino() as u128,).is_err());
        drop(file);
