...
fn run_identity(model: &str, max_tensors: usize, max_blocks: usize) -> bool {
    ...
    for (name, w) in &real {
        ...
        for (k, l) in [(2u32, 12u32), (3, 12)] {
            let cfg = TrellisConfig::new(l, k, 256);
            check(
                format!("REAL {name} [..{n_use}] k={k} L={l} default"),
                &w[..n_use],
                &cfg,
                &OPT_COMBOS[0].1,
            );
    ...
    println!(
        "  {cases} cases, {fails} mismatches; mean expansion ratio {:.4}",
        exp_sum / exp_n.max(1) as f64
    );
    if fails == 0 {
...
fn bench_pair(label: &str, w: &[f32], cfg: &TrellisConfig, iters: u32) {
    ...
    println!(
        "  {label:<44} live {mw_live:7.3} Mw/s | pruned {mw_pruned:7.3} Mw/s | speedup {:.2}x | nodes expanded {:.1}% (sink={sink})",
        mw_pruned / mw_live,
        100.0 * rep.expansion_ratio(),
    );
}
...
fn run_bench(model: &str, max_tensors: usize, max_blocks: usize) {
    ...
    for (name, k, l, n, iters) in [
        ("synthetic q2_l12", 2u32, 12u32, 49_152usize, 2u32),
        ("synthetic q3_l12", 3, 12, 24_576, 2),
        ("synthetic q4_l8", 4, 8, 262_144, 2),
    ] {
        let cfg = TrellisConfig::new(l, k, 256);
...
fn print_pcts(label: &str, vals: &mut Vec<f64>) {
    ...
    println!(
        "    {label:<28} min {:10.3e}  p10 {:10.3e}  p25 {:10.3e}  p50 {:10.3e}  p75 {:10.3e}  p90 {:10.3e}  max {:10.3e}",
    ...
    for (k, l) in [(2u32, 12u32), (3, 12)] {
        ...
        for (_name, w) in &real {
            ...
            for b in &rep.blocks {
                ...
                let energy: f64 = w[off..off + b.n as usize]
                    .iter()
