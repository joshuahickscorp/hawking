...
impl TropicalEncoder {
    ...
    pub fn encode_tensor(
        ...
        while bi_base < n_blocks {
            ...
            let out = self.run_batch_full(
                &weights[w_start..w_end], &blens, cfg, opts, n_sub_max, &lut_buf,
            )?;
            ...
            for (i, &blen) in blens.iter().enumerate() {
                ...
                blocks_meta.push(BlockMeta {
                    ...
                    sub_scales: pack_sub_scales(&out.mults[m_off..m_off + n_sub]),
    ...
    fn run_batch_full(
        ...
        let mut w_pad = vec![0.0f32; n * mbl];
        {
            let mut src = 0usize;
            for (i, &blen) in blens.iter().enumerate() {
                w_pad[i * mbl..i * mbl + blen]
                    .copy_from_slice(&batch_weights[src..src + blen]);
                src += blen;
            }
        }
...

        {
            ...
            enc.dispatch_thread_groups(
                MTLSize { width: n as NSUInteger, height: 1, depth: 1 },
                MTLSize { width: tg as NSUInteger, height: 1, depth: 1 },
            );
            enc.end_encoding();
        }

        {
            ...
            enc.dispatch_thread_groups(
                MTLSize { width: n as NSUInteger, height: 1, depth: 1 },
                MTLSize { width: tg_threads as NSUInteger, height: 1, depth: 1 },
            );
            enc.end_encoding();
        }
    ...
    pub fn encode_tensor_prep_cpu(
        ...
        let back_bytes_per_block = cfg.block_len * num_states;
        let batch_blocks = (MAX_BACK_BYTES / back_bytes_per_block.max(1))
            .clamp(1, n_blocks);
        ...
        while bi_base < n_blocks {
            ...
            let t0 = std::time::Instant::now();
            let preps = prep_blocks_parallel(
                &weights[w_start..w_end], cfg, opts, num_states, n_sub_max,
            );
    ...
    fn run_batch(
        ...
        let mut w_pad = vec![0.0f32; n * mbl];
        {
            ...
            for (i, p) in preps.iter().enumerate() {
                w_pad[i * mbl..i * mbl + p.blen]
                    .copy_from_slice(&batch_weights[src..src + p.blen]);
                src += p.blen;
            }
        }
        let w_buf = self.upload(&w_pad);
