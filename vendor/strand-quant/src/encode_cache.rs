...
impl Manifest {
    ...
    pub fn record(&mut self, name: &str, tensor_bytes_hash: &[u8; 32], bits: u32) {
        ...
        self.entries.insert(
            name.to_string(),
            ManifestEntry { name: name.to_string(), bits, sha256_hex: hex32(tensor_bytes_hash) },
        );
    }
    ...
    pub fn from_json(s: &str) -> Option<Manifest> {
        ...
        while i < b.len() {
            if b[i] == b'{' {
                ...
                m.entries.insert(
                    name.clone(),
                    ManifestEntry { name, bits: bits as u32, sha256_hex: sha },
                );
            } else {
                i += 1;
            }
...
mod tests {
    ...
    fn should_skip_requires_everything_to_match() {
        ...
        assert!(!should_skip("model.layers.0.mlp.down_proj.weight", &hash, 2,
                             "bits=2,l=13,rht=true,out=1.0@8", Some(&m)));
    ...
    fn load_missing_or_garbage_is_a_cache_miss() {
        ...
        assert!(Manifest::from_json("{\"version\": 999, \"config_key\": \"x\", \"tensors\": []}")
            .is_none());
    ...
    fn save_load_roundtrip_on_disk() {
        ...
        assert_eq!(m, back);
        assert!(should_skip("model.layers.5.mlp.down_proj.weight", &hash, 2,
                            "bits=2,l=12,out=1.0@8", Some(&back)));
