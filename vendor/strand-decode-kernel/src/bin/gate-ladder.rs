...
use strand_quant::encode::{
    encode_tensor, encode_tensor_with_lut, EncodeOpts, EncodedTensor,
};
...
fn main() {
    ...
    println!(
        "tensor: {rows} x {cols} = {total} weights ({:.2} Mw), in % 256 == 0 (STRICT); logical CPUs={cores}, rayon threads={rayon_threads}",
        total as f64 / 1e6
    );
    ...
    for rung in &rungs {
        ...
        println!(
            ...
            d,
            cfg.block_len,
            if is_vector { "VECTOR trellis" } else { "scalar trellis" }
        );
        ...
        let (enc, vec_lut): (EncodedTensor, Option<Vec<i32>>) = if is_vector {
            ...
            assert_eq!(
                lut.len(),
                cfg.num_states() * d,
                "learned vector LUT must be [2^L * d]"
            );
        ...
        assert_eq!(
            q_fast, q_ref,
            "{}: decode_q12_fast != decode_lean — DETERMINISM CONTRACT BROKEN",
            rung.label
        );
        assert_eq!(
            q_par, q_ref,
            "{}: decode_q12_par != decode_lean — DETERMINISM CONTRACT BROKEN",
            rung.label
        );
        assert_eq!(q_ref.len(), total, "decoded weight count mismatch");
        println!(
            "   bit-identity: fast == par == decode_lean  ... OK ({} weights, {})",
            q_ref.len(),
            if is_vector { "vector LUT path" } else { "scalar path" }
        );

        let bytes_per_weight = enc.total_bpw(&cfg) / 8.0;
        let floor_bytes = if is_vector {
            
            payload_bpw / 8.0 + SCALAR_SCALE_OVERHEAD_BPW_BYTES
        } else {
            k as f64 / 8.0 + SCALAR_SCALE_OVERHEAD_BPW_BYTES
        };
        ...
        println!(
            ...
            bytes_per_weight, payload_bpw, floor_bytes, pct_above_floor, if below_q4k { "YES" } else { "no" }
        );
        ...
        let mw_par = mw(s_par);
        println!(
            "   throughput: scalar {:.0} Mw/s, parallel {:.0} Mw/s ({:.2}x)",
            mw_scalar, mw_par, mw_par / mw_scalar
        );
        println!();
    ...
    println!(
        "{:<9} {:<8} {:>8} {:>9} {:>9} {:>11} {:>9} {:>9} {:>7}  {}",
        "rung", "kind", "payload", "B/w", "floor", "%>floor", "scalar", "parallel", "<Q4_K", "quality anchor (7B WT2 PTQ)"
    );
    println!(
        "{:<9} {:<8} {:>8} {:>9} {:>9} {:>11} {:>9} {:>9} {:>7}  {}",
        "", "", "bpw", "B/w", "B/w", "", "Mw/s", "Mw/s", "?", ""
    );
    println!("{}", "-".repeat(112));
    for r in &table {
        println!(
    ...
    println!("{}", "-".repeat(112));
    println!(
        "HEADLINE: STRAND is deterministic (fast==par==decode_lean, bit-for-bit) + near-density-floor at EVERY bleeding-edge rung."
    );
    println!(
        "          4/3-bit are quality-usable today (7.81 / 9.42 PPL); 2/1-bit are FORMAT + RUNTIME ready, quality training-gated"
    );
    println!(
        "          (BitNet/QAT frontier) — do NOT claim 2/1 quality-usable via PTQ. Every rung is denser than Q4_K (0.5625 B/w)."
    );
}
