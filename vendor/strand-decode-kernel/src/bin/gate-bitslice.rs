...
mod macos {
    ...
    use strand_decode_kernel::metal::{
        bake_bitslice_entries, bake_bitslice_entries_vec, bitslice_decode_q12,
        bitslice_decode_q12_computed, BitsliceGpu, StrandGpu,
    };
    ...
    fn identity_matrix(gpu: &BitsliceGpu) -> usize {
        ...
        
        {
            ...
            assert_eq!(
                bitslice_decode_q12_with_lut(gpu, &enc, &cfg, &lut),
                want,
                "bitslice vec fallback"
            );
            checked += 1;
        }
        checked
    ...
    fn bench(gpu: &BitsliceGpu, strand_gpu: Option<&StrandGpu>) {
        ...
        println!(
            "\n== bench (best-of-10 GPU dispatch, ffn_down {out_f}x{in_f} = {:.1}M weights) ==",
            total as f64 / 1e6
        );
        println!("  {}", machine_stamp());

        let peak = strand_gpu
            .map(|g| g.bench_peak_bw(64 << 20, 5))
            .unwrap_or(f64::NAN);
        println!("  measured streaming peak: {:.1} GB/s (grid-stride f32 sum, 256 MB)", peak / 1e9);

        for (cfg, label) in [
            (TrellisConfig::for_bpw(3.0), "3-bit deploy (k3 L7, 512 B TG LUT)"),
            (TrellisConfig::for_bpw_l(2.0, 12), "2-bit reopen (k2 L12, 16 KB TG LUT)"),
        ] {
            println!("  -- {label} --");
            ...
            let bytes =
                (total as f64) * (cfg.k_bits as f64) / 8.0 + (tbl.len() * 80) as f64 + (total * 4) as f64;
            ...
            println!(
                "  GPU bitslice : {:>7.2} ms   {:>6.2} Gw/s   {:>6.1} GB/s moved = {:>5.1}% of peak",
                dt * 1e3,
                gws,
                achieved / 1e9,
                pct
            ...
            println!(
                "  CPU rayon par: {:>7.2} ms   {:>6.2} Gw/s   (12-core baseline)   GPU/CPU = {:.2}x",
                cpu_best * 1e3,
                cpu_gws,
                gws / cpu_gws
            );
            ...
            for r in (0..out_f).step_by(997) {
                ...
                assert!(
                    (y_gpu[r] - acc).abs() / denom < 1e-3,
                    "fused y diverged beyond float tolerance at {label} row {r}: GPU {} vs CPU {acc}",
                    y_gpu[r]
                );
            }
            let dt_f = gpu.bench_matvec(
                &enc.bits,
                &tbl,
                lut,
                out_f as u32,
            ...
            let f_bytes = (total as f64) * (cfg.k_bits as f64) / 8.0
                + (tbl.len() * 80) as f64
                + (tbl.len() * 8) as f64
                + ((in_f + out_f) * 4) as f64;
            println!(
    ...
    fn bench_gemm_prompt(gpu: &BitsliceGpu) {
        ...
        let total = out_f * in_f;
        println!(
            "\n== fused GEMM (prompt phase): B in {{4,16,64}}, ffn_down {out_f}x{in_f} = {:.1}M weights ==",
            total as f64 / 1e6
        );
        println!("  {}", machine_stamp());
        ...
        for (cfg, label) in [
            ...
            for &batch in &[4usize, 16, 64] {
                ...
                let y = gpu.gemm(
                    &enc.bits, &tbl, lut, out_f as u32, in_f as u32, cfg.k_bits, cfg.l_bits,
                    &xs, batch,
                );
                for r in 0..out_f {
                    for (b, &c) in probe.iter().enumerate() {
                        ...
                        assert_eq!(
                            recovered, expected,
                            "GEMM one-hot Q12 diverged at {label} B={batch} r={r} c={c} — release blocker"
                        );
                    }
                }

                let xs: Vec<f32> = (0..batch * in_f).map(|i| ((i as f32) * 0.031).cos()).collect();
                let dt = gpu.bench_gemm(
                    &enc.bits, &tbl, lut, out_f as u32, in_f as u32, cfg.k_bits, cfg.l_bits,
                    &xs, batch, 5,
                );
                ...
                println!(
                    ...
                    dt * 1e3, eff_gws, gmacs, per_col_ms,
                    cpu_best * 1e3, cpu_gmacs, cpu_best / dt
                );
            }
        }
        println!(
    ...
    fn bench_prepared(gpu: &BitsliceGpu) {
        ...
        for (cfg, label) in [
            ...
            assert_eq!(
                ...
                want,
                "prepared GPU decode diverged at {label} — release blocker"
            );
        ...
        for (cfg, label) in [
            ...
            for (p, enc) in prepared.iter().zip(encs.iter()) {
                assert_eq!(
                    ...
                    "batched prepared dispatch diverged at {label} — release blocker"
                );
            }
    ...
    fn vec_d2_gate(gpu: &BitsliceGpu) -> bool {
        ...
        println!("  {}", machine_stamp());
        println!(
            "  host sizeof(BitsliceEntry) = {} B; GPU sizeof = {} B (probe)",
            std::mem::size_of::<strand_decode_kernel::metal::BitsliceEntry>(),
            gpu.gpu_entry_sizeof(),
        );
        ...
        let edge_ns: [usize; 18] = [
            1, 2, 3, 4, 5, 31, 32, 33, 63, 127, 255, 256, 257, 511, 512, 513, 999, 4097,
        ];
        let mut checked = 0usize;
        ...
        'outer: for (cfg, label) in &id_configs {
            ...
            for &n in &edge_ns {
                ...
                for seed in 0..3u64 {
                    let w: Vec<f32> = (0..n)
                        .map(|i| ((i as f32 + seed as f32 * 7.0) * 0.0137).sin() * 0.5)
                        .collect();
                    ...
                    for enc in &variants {
                        ...
                        let Some(tbl) = bake_bitslice_entries_vec(enc, cfg) else {
                            first_fail = Some(format!(
                                "{label} n={n} seed={seed}: bake_bitslice_entries_vec returned None"
                            ));
                            break 'outer;
                        };
                        let got = gpu.decode_q12_vec(
                            &enc.bits,
                            &tbl,
                            &vec_lut,
                            enc.total,
        ...
        println!(
            "  identity: {checked} GPU _vec cells byte-identical to decode_tensor_fixed_with_lut_vec ✓"
        );
        ...
        let peak = StrandGpu::new()
            .map(|g| g.bench_peak_bw(64 << 20, 5))
            .unwrap_or(f64::NAN);
        println!(
            "\n  -- fused y=Wx (B=1), ffn_down {out_f}x{in_f} = {:.1}M weights --",
            total as f64 / 1e6
        );
        ...
        let scalar_gws = {
            ...
            let x: Vec<f32> = (0..in_f).map(|i| (i as f32 * 0.05).sin()).collect();
            let dt = gpu.bench_matvec(
                &enc.bits, &tbl, lut, out_f as u32, in_f as u32, cfg.k_bits, cfg.l_bits, &x, 10,
            );
            let gws = total as f64 / dt / 1e9;
            let bytes = (total as f64) * (cfg.k_bits as f64) / 8.0
                + (tbl.len() * std::mem::size_of::<strand_decode_kernel::metal::BitsliceEntry>())
                    as f64
                + ((in_f + out_f) * 4) as f64;
            let pct = 100.0 * (bytes / dt) / peak;
            println!(
                "  SCALAR L12,d1 (incumbent): {:>7.3} ms   {:>6.2} Gw/s   {:>5.1}% of peak",
                dt * 1e3, gws, pct
            );
            gws
        };
        ...
        for (cfg, label) in [
            (TrellisConfig::for_bpw(3.0).with_vec_dim(D as u32), "L7,d2"),
            (TrellisConfig::for_bpw_l(3.0, 10).with_vec_dim(D as u32), "L10,d2"),
        ] {
            let enc = synth_encoded(total, cfg.k_bits, 256);
            let scalar_lut = codebook_lut(cfg.l_bits);
            ...
            let bytes = (total as f64 / D as f64) * (cfg.k_bits as f64) / 8.0
                + (tbl.len() * std::mem::size_of::<strand_decode_kernel::metal::BitsliceEntry>())
                    as f64
                + ((in_f + out_f) * 4) as f64;
            let pct = 100.0 * (bytes / dt) / peak;
            ...
            println!(
                "  VECTOR {label}: {:>7.3} ms   {:>6.2} Gw/s   {:>5.1}% of peak   (TG LUT {} B)",
                dt * 1e3,
                gws,
                pct,
                tg_lut_kb,
        ...
        let best = vec_results
            .iter()
            .cloned()
            .fold(("none".to_string(), 0.0f64, 0.0f64), |acc, x| {
        ...
        for (label, gws, _pct) in &vec_results {
            ...
            println!(
                "  {label}: {:.2} Gw/s  /  scalar {:.2} Gw/s = {:.2}x  => {v}",
                gws, scalar_gws, ratio
            );
        }
        ...
        println!(
            "  BEST vector cell: {} @ {:.2} Gw/s = {:.2}x scalar  =>  OVERALL VERDICT: {overall}",
            best.0, best.1, best_ratio
        );
        true
    }
    ...
    pub fn run() {
        ...
        println!(
            "gate-bitslice — G4 bitslice decode (probe: GPU sizeof(BitsliceEntry) == host {} ✓)",
            std::mem::size_of::<strand_decode_kernel::metal::BitsliceEntry>()
        );
        ...
        println!(
            "identity: {} GPU config×variant cells byte-identical to decode_tensor_fixed in {:.1}s ✓",
            checked,
