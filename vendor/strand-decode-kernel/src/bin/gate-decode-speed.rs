...
fn main() {
    ...
    println!(
        "tensor: {rows} x {cols} = {} weights ({:.2} Mw) in {n_blocks} blocks; logical CPUs={cores}, rayon threads={rayon_threads}",
        total,
        total as f64 / 1e6
    );
    println!();
    ...
    println!(
        "{:<26} {mw_par:>10.1} {:>11.3} {:>8.2}x",
        "parallel (rayon)",
        mw_par / 1e3,
        mw_par / mw_fast
    );
    println!(
        "{:<26} {mw_simd:>10.1} {:>11.3} {:>8.2}x",
        "SIMD (NEON 4-block)",
        mw_simd / 1e3,
        mw_simd / mw_fast
    );
    ...
    println!(
        "compute ceiling (cores x single-core): {:.1} Mw/s ({:.3} Gw/s); parallel reaches {:.0}% of it",
        ceiling_mw,
        ceiling_mw / 1e3,
        pct_ceiling
    );
    ...
    println!(
        "decoded output write rate: single {:.2} GB/s, parallel {:.2} GB/s (Q12 = 4 B/weight)",
        gbs_fast, gbs_par
    );
    println!();
    ...
    println!(
        "on-disk weight traffic: {:.4} B/weight (payload {:.3} + 16B/{}-block table {:.4})",
        on_disk_bpw, payload_bpw_bytes, cfg.block_len, table_bpw_bytes
    );
    println!(
        "compute->bandwidth FLIP point: {:.2} Gw/s @ {:.4} B/w  ( {:.2} Gw/s @ 0.42 B/w headline )",
        flip_gw_measured, on_disk_bpw, flip_gw_042
    );
    ...
    if par_gw < flip_gw_measured {
        println!(
            "verdict: parallel decode {:.3} Gw/s is {:.0}x BELOW the {:.2} Gw/s flip point => still COMPUTE-BOUND",
            par_gw, headroom, flip_gw_measured
        );
        println!("         (the byte savings are not yet the bottleneck; more cores / wider SIMD still help.)");
    } else {
        println!(
            "verdict: parallel decode {:.3} Gw/s has REACHED the {:.2} Gw/s flip point => now BANDWIDTH-BOUND",
            par_gw, flip_gw_measured
        );
    }
}
