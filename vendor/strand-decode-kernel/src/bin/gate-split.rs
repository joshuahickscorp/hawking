...
use strand_decode_kernel::split_decode::{decode_q12_split, decode_q12_split_par, decode_q12_split_par_with_lut, decode_q12_split_with_lut};
        ...
        for &(rows, cols) in &[(16usize, 256usize), (37, 300), (9, 1024)] {
            ...
            for opts in [EncodeOpts::default(), EncodeOpts { tail_biting: true, affine_min: true, ..Default::default() }] {
                ...
                for &batch in &[1usize, 4, 5, 16, 21, 64] {
                    ...
                    let budget: Vec<f32> = (0..batch).map(|b| xs[b * cols..(b + 1) * cols].iter().map(|v| v.abs()).sum::<f32>() / 4096.0).collect();
...
fn bench_decode() {
    ...
    println!("\n== dense decode bench (best-of-3, ffn_down {out_f}x{in_f} = {:.1}M weights) ==", total as f64 / 1e6);
    ...
    for (cfg, label) in [(TrellisConfig::for_bpw(3.0), "3-bit deploy (k=3,L=7)"), (TrellisConfig::for_bpw_l(2.0, 12), "2-bit reopen (k=2,L=12)")] {
        ...
        for (name, f, single) in rows {
            ...
            if *single {
                println!("  {name:<30} {:>8.1} ms   {:>6.3} Gw/s   ~{:>4.2} cyc/w", best * 1e3, gws, EST_CLOCK_GHZ / gws);
            } else {
