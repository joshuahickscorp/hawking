...
fn main() {
    ...
    println!("strand-decode-kernel bench — {out}x{inf} layer ({nw} weights), reference decode+matvec\n");
    ...
    for &bpw in &bpws {
        ...
        println!("  bpw={:.1}  {:6.2} GMAC/s   weights={:4} KB   {:4.1}x smaller than bf16  (~{:4.1}x less weight traffic)", bpw, gmacs, bytes / 1024, bf16 as f64 / bytes as f64, 16.0 / bpw);
    }
    println!(
