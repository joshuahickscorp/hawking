...
fn run_gate() -> Result<(), String> {
    ...
    for (cfg, label) in &configs {
        ...
        for &n in &[2048usize, 512, 257] {
            for seed in 0u64..3 {
                ...
                let enc_bonus0 = encode_tensor_with(&w, cfg, &EncodeOpts { silence_bonus: 0.0, ..Default::default() });

                if enc_default != enc_bonus0 {
                    return Err(format!("IDENTITY FAIL: bonus=0.0 != default for {label} n={n} seed={seed}"));
                }
                ...
                if mask_d.class != mask_b.class {
                    return Err(format!("SILENCE MASK MISMATCH: bonus=0.0 vs default {label} n={n} seed={seed}"));
                }
    ...
    println!("gate-silence-bonus IDENTITY: PASS ({id_checked} cells, bonus=0.0 is byte-identical to default)");
    ...
    for (cfg, label) in &configs {
        ...
        for (b, blk) in enc_bonus.blocks.iter().enumerate() {
            ...
            if let Some(consts) = &mask_bonus.consts[b] {
                ...
                for (sb, &c) in consts.iter().enumerate() {
                    ...
                    for &v in &vals[s..e] {
                        if v != c {
                            return Err(format!("BONUS SILENCE CONSTANT MISMATCH: {label} block {b} sb {sb} c={c} v={v}"));
                        }
                    }
                }
            }
        ...
        println!("{:<26} {:>8} {:>8} {:>+10}", label, sil0_base, sil0_bonus, sil0_bonus as i64 - sil0_base as i64);
    ...
    println!("{:<26} {:>8} {:>8} {:>+10}", "TOTAL", total_base, total_bonus, total_bonus as i64 - total_base as i64,);
    ...
    println!("silence rate: {:.4}% (base) → {:.4}% (bonus=1e-5)  delta={:+.4}%", rate_base, rate_bonus, rate_bonus - rate_base,);

    if total_bonus < total_base {
        return Err(format!("REGRESSION: bonus=1e-5 gave FEWER silent blocks ({total_bonus}) than base ({total_base})"));
    }
