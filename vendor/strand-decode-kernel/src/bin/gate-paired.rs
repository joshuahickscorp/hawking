...
use strand_decode_kernel::paired_lut::{decode_q12_paired, decode_q12_paired_par_with_table, decode_q12_paired_with_table, PairTable};
    ...
    for (cfg, label) in canonical_configs() {
        ...
        for seed in 0..24u64 {
            ...
            for enc in &variants {
                ...
                for (name, got) in
                    [("paired", decode_q12_paired(enc, &cfg)), ("paired+table", decode_q12_paired_with_table(enc, &cfg, &table)), ("paired-par", decode_q12_paired_par_with_table(enc, &cfg, &table))]
                {
                    assert_eq!(
                        got, reference,
...
fn bench() {
    ...
    for (cfg, label) in [(TrellisConfig::for_bpw(3.0), "3-bit deploy (k=3, L=7)"), (TrellisConfig::for_bpw_l(2.0, 12), "2-bit reopen (k=2, L=12)")] {
        println!("  -- {label} --");

        let tb = Instant::now();
        let table = PairTable::build(&cfg);
        let build_s = tb.elapsed().as_secs_f64();
        println!("  pair table: {} entries, {:.1} KB, built in {:.3} ms (load-time only)", 1usize << (cfg.l_bits + cfg.k_bits), table.size_bytes() as f64 / 1024.0, build_s * 1e3);
        ...
        for (i, (name, best)) in results.iter().enumerate() {
            ...
            println!("  {name:<26} {:>8.1} ms   {:>6.2} Gw/s   {:.2}x vs its baseline", best * 1e3, total as f64 / best / 1e9, ratio);
        }
    }
}
