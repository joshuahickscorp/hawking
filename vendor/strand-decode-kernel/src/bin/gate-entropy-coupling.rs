...
use strand_quant::encode::{
    compute_block_entropy, encode_tensor_with, extract_block_symbols, EncodeOpts,
};
...
fn run_gate() -> Result<(), String> {
        ...
        for &n in &[2048usize, 512, 257] {
            for seed in 0u64..3 {
                ...
                if enc_default != enc_psi_zero {
                    return Err(format!(
                        "IDENTITY FAIL: psi_scale=0.0 != default for {label} n={n} seed={seed}"
                    ));
                }
    ...
    println!(
        "gate-entropy-coupling IDENTITY: PASS ({id_checked} cells, psi_scale=0.0 is byte-identical to default)"
    );
    ...
    println!(
        "  summary: n_blocks={} mean={:.4} min={:.4} max={:.4}",
        n_blocks, mean_c, min_c, max_c
    );

    println!("\ngate-entropy-coupling SILENCE DELTA  (psi_scale=0.0 vs psi_scale=1e-4 two-pass):");
    println!(
        "{:<26} {:>8} {:>8} {:>8} {:>10}",
        "config", "n_blocks", "sil_base", "sil_psi", "delta"
    );

    let opts_psi = EncodeOpts {
        entropy_bonus_scale: 1e-4,
    ...
    for (cfg, label) in &configs {
            ...
            if let Some(consts) = &mask_psi.consts[b] {
                ...
                for (sb, &c) in consts.iter().enumerate() {
                    ...
                    for &v in &vals[s..e] {
                        if v != c {
                            return Err(format!(
                                "PSI DECODE CONSTANT MISMATCH: {label} block {b} sb {sb} c={c} v={v}"
                            ));
                        }
                    }
                }
        ...
        println!(
            ...
            sil_base,
            sil_psi,
            sil_psi as i64 - sil_base as i64
        );
    ...
    println!(
        ...
        total_sil_base,
        total_sil_psi,
        total_sil_psi as i64 - total_sil_base as i64,
    );
    ...
    println!(
        "silence rate: {:.4}% (base) → {:.4}% (psi_scale=1e-4 two-pass)  delta={:+.4}%",
        rate_base, rate_psi, rate_psi - rate_base,
    );

    println!("\ngate-entropy-coupling ONE-PASS ROLLING sanity:");
    let opts_psi_onepass = EncodeOpts {
        entropy_bonus_scale: 1e-4,
        entropy_bonus_two_pass: false,  
        ..Default::default()
    };
    ...
    if dec_op.len() != w_op.len() {
        return Err(format!(
            "ONE-PASS DECODE LEN MISMATCH: {} vs {}",
            dec_op.len(), w_op.len()
        ));
    }
        ...
        if let Some(consts) = &mask_op.consts[b] {
            ...
            for (sb, &c) in consts.iter().enumerate() {
                ...
                for &v in &vals[s..e] {
                    if v != c {
                        return Err(format!(
                            "ONE-PASS SILENCE CONSTANT MISMATCH: block {b} sb {sb} c={c} v={v}"
                        ));
                    }
                }
            }
    ...
    println!();
    println!(
        "NOTE: entropy_bonus_scale > 0.0 CHANGES THE EMITTED BITS (expected, by design)."
    );
    println!(
        "      Same licence as SCHISM Δ (silence_bonus) and STRAND_F32_METRIC: bit-changing,"
    );
    println!(
        "      off by default. All KATs and model roots under default opts remain valid."
    );
    println!(
        "      Ψ requires its own A/B (PPL measurement) before any production adoption."
    );
