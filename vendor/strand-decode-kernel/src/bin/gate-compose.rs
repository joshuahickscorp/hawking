...
use strand_decode_kernel::block_walk::gate_proto::{
    canonical_configs, machine_stamp, synth_encoded,
};
...
use strand_decode_kernel::paired_lut::{
    decode_q12_paired_par_with_table, decode_q12_paired_with_table, PairTable,
};
use strand_decode_kernel::prepared::{
    decode_q12_par_prepared, decode_q12_prepared_paired, decode_q12_prepared_paired_par,
    PreparedTensor,
};
    ...
    for (cfg, label) in canonical_configs() {
        ...
        for seed in 0..24u64 {
            ...
            for enc in &variants {
                ...
                for (name, got) in [
                    ("compose", decode_q12_prepared_paired(&p, &table)),
                    ("compose-par", decode_q12_prepared_paired_par(&p, &table)),
                ] {
                    assert_eq!(
                        got, reference,
...
fn bench() {
    ...
    for (cfg, label) in [
        ...
        for tail in [false, true] {
            ...
            println!(
                "    tail-biting {} (prepare {:.0} ms one-time, {:.4} B/w prepared):",
                if tail { "ON " } else { "off" },
                prep_ms,
