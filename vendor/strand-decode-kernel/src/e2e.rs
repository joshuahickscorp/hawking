...
mod tests {
    ...
    fn build_model() -> Vec<Synth> {
        let specs: &[(&'static str, usize, usize, f64)] =
            &[("blk.0.attn_q.weight", 32, 256, 3.0), ("blk.0.attn_k.weight", 16, 512, 2.0), ("blk.0.ffn_down.weight", 24, 768, 4.0), ("blk.0.ffn_up.weight", 40, 256, 3.0)];
        specs
            .iter()
            .enumerate()
    ...
    fn write_model(model: &[Synth]) -> (std::path::PathBuf, usize) {
        ...
        let pts: Vec<PackedTensorV2> = model
            ...
            .map(|(s, shape)| PackedTensorV2 {
                base: PackedTensor { name: s.name, shape, rht_seed: 0, l_bits: s.cfg.l_bits as u8, k_bits: s.cfg.k_bits as u8, vec_dim: s.cfg.vec_dim() as u8, enc: &s.enc },
    ...
    fn e2e_v2_runtime_decode_is_bit_exact() {
        ...
        assert_eq!(loaded.header().tensors.len(), model.len(), "tensor count round-trip");
        assert!(loaded.header().all_strict(), "all shapes are in % block_len == 0, so ALL_STRICT must be set");
        ...
        for s in &model {
            ...
            assert_eq!(q_loader, q_ref, "{}: loader-reconstructed decode_tensor_fixed != reference (DETERMINISM BREAK)", s.name);
            ...
            assert_eq!(q_runtime, q_ref, "{}: runtime decode_tensor_q12 (decode_lean) != reference (DETERMINISM BREAK)", s.name);

            println!("  {:<24} [{:>3}x{:<4}] bpw={:.1} k={} l={} n_blocks={} -> {} Q12 weights, bit-exact OK", s.name, s.out, s.inf, s.bpw, s.cfg.k_bits, s.cfg.l_bits, s.enc.blocks.len(), n);
        }
        ...
        println!("  total weights = {}  file = {} bytes  => {:.3} bytes/weight (whole archive, incl. headers + page padding)", total_weights, file_len, bytes_per_weight);
        println!("  CPU decode throughput (decode_tensor_q12 over all {} tensors, {} iters): {:.1} Mweights/s ({:.3} ms/full-pass)", model.len(), iters, mw_per_s, loop_secs / iters as f64 * 1e3);

        let _ = total_q12;
        println!("  (single-pass decode of all tensors: {} Q12 weights in {:.3} ms)\n", total_q12, decode_secs * 1e3);

        assert!(bytes_per_weight > 0.0 && bytes_per_weight < 8.0, "bytes/weight {bytes_per_weight} out of sane range");
    ...
    fn e2e_footprint_bytes_per_token_report() {
        ...
        println!("\n=== strand-decode-kernel E2E footprint / bytes-per-token ({} synthetic weights across {} tensors) ===", total_weights, model.len());
        ...
        for &bpw in &[3.0f64, 2.0, 1.5] {
            ...
            println!("  bpw={:.1}  {:>7} bytes/token  {:5.2}x smaller than bf16  (~{:.1}x less weight traffic/token)", bpw, bytes, shrink, 16.0 / bpw);
