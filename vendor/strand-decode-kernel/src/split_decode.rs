...
use crate::block_walk::{
    block_init_state, block_plans, exceeds_max_sub, SideInfo, WordReader,
};
use rayon::prelude::*;
...
pub fn decode_q12_split_with_lut(
    enc: &EncodedTensor,
    cfg: &TrellisConfig,
    lut: &[i32],
) -> Vec<i32> {
...
pub fn decode_q12_split_par_with_lut(
    enc: &EncodedTensor,
    cfg: &TrellisConfig,
    lut: &[i32],
) -> Vec<i32> {
...
unsafe fn scale_apply_neon(seg: &mut [i32], es: i32, off: i32) {
    ...
    while i < n4 {
        ...
        let r = vcombine_s32(
            vmovn_s64(vshrq_n_s64::<16>(plo)),
            vmovn_s64(vshrq_n_s64::<16>(phi)),
        );
...
mod tests {
    ...
    fn split_vector_trellis_fallback() {
        ...
        assert_eq!(
            decode_q12_split_par_with_lut(&enc, &cfg, &lut),
            want,
            "split-par vec fallback"
        );
    }
}
