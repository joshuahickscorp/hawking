...
A. Setup — clone repo, deps, fetch corpora (release) + rebuild frozen (HF GGUF,
   fingerprint-verified, cached to Drive across sessions). [proven]
B. HEAD MAXIMIZATION SWEEP [proven, default-ON, the core heavy compute] —
   ...
   the spec MULTIPLIER (accepted-prefix 1.6 -> target 2.5+). Subprocess output
   is STREAMED so progress is visible, and already-evaluated configs are
   SKIPPED on resume.
...
MODELS = {
    ...
    # q7b: bartowski is the only single-file Q4_K_M on HF (Qwen-official is
    # split-shard, which build_frozen_gguf.py can't read). The release's
    # frozen_fingerprints.json was generated from a LOCAL q7b_frozen.npz; if
    # bartowski's quant bytes don't match that build, the verify will fail
    # SAFE (slug dropped from READY, NO bad head trained). The verify print
    # tells the user the actual diff so they can decide to upload manually.
...
# SMOKE_MODE config: 1 model, 1 cheap entry, tiny epoch count + window count.
# Should complete in ~2 minutes on any GPU. Used to validate end-to-end
# (download/extract/build_frozen/train/eval/emit) before the full sweep burns
# 2-4 GPU-hours. Set SMOKE_MODE=True in the cell to use this.
SMOKE_MODEL = "q05b"
...
def build() -> dict:
    ...
    cells.append(md(
        ...
        "\n"
        "**Recommended path:**\n"
        "1. `Runtime → Run all` once with `SMOKE_MODE=True` in cell B0 → "
        "validates end-to-end in ~2 min.\n"
        "2. Set `SMOKE_MODE=False` in cell B0, re-run cell B1 → full sweep "
        "(2-4 GPU-hours). Already-evaluated configs from prior runs are "
    ...
    cells.append(md(
        ...
        "First session: downloads ~250MB corpora + ~12GB GGUFs, rebuilds frozen "
        "(~10 min). Subsequent sessions: restores frozen from Drive cache (~30s)."
    ))
    cells.append(code(
    ...
    cells.append(md(
        ...
        "\n"
        "**Cell B0** sets toggles. Set `SMOKE_MODE=True` for a ~2 min "
        "end-to-end validation, then flip to `False` for the full sweep.\n"
        "**Cell B1** runs it. Subprocess stdout STREAMS — you see progress as "
        "training runs. Configs with an existing `tau.json` are SKIPPED on "
        "resume (so a Colab disconnect mid-sweep doesn't re-train completed "
        "configs)."
    ))
    cells.append(code(
        "# ── B0. Toggles ───────────────────────────────────────────────────\n"
        "SMOKE_MODE = False  # True: ~2-min validation; False: full ~3 GPU-hour sweep\n"
        f"FULL_SWEEP = {json.dumps(SWEEP)}\n"
        f"SMOKE_SWEEP = {json.dumps(SMOKE_SWEEP)}\n"
        f"SMOKE_MODEL = {SMOKE_MODEL!r}\n"
        f"SMOKE_MAX_WINDOWS = {SMOKE_MAX_WINDOWS}\n"
        f"SMOKE_MAX_ROWS = {SMOKE_MAX_ROWS}\n"
        "if SMOKE_MODE:\n"
        "    SWEEP = SMOKE_SWEEP\n"
