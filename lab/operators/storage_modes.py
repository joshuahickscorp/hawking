"""Recomposed science module storage_modes (C-SCI-R1)."""
...
_IMMUTABLE_REVISION = re.compile('^[0-9a-f]{40}$')
...
def choose_mode(official_manifest_bytes: int, live_free_bytes: int, reserve_bytes: int, parent_id: str, *, working_window_bytes: int=DEFAULT_WORKING_WINDOW_BYTES) -> dict:
    ...
    """
    for name, value in (('official_manifest_bytes', official_manifest_bytes), ('live_free_bytes', live_free_bytes), ('reserve_bytes', reserve_bytes), ('working_window_bytes', working_window_bytes)):
    ...
    if official_manifest_bytes <= usable:
        ...
        reason = f'official manifest {official_manifest_bytes} B fits in usable {usable} B (free {live_free_bytes} - reserve {reserve_bytes})'
    elif usable >= working_window_bytes:
        mode = VULTURE_SHARD_SERIAL
        reason = f'official manifest {official_manifest_bytes} B exceeds usable {usable} B, but usable covers the {working_window_bytes} B shard-serial working window'
    else:
        mode = BOUNDED_REMOTE_RANGE
        reason = f'usable {usable} B is below the {working_window_bytes} B shard-serial working window: no shard may land, range windows only'
    return {'schema': SCHEMA_DECISION, 'parent_id': parent_id, 'mode': mode, 'reason': reason, 'official_manifest_bytes': official_manifest_bytes, 'live_free_bytes': live_free_bytes, 'reserve_bytes': reserve_bytes, 'usable_bytes': usable, 'headroom_bytes': usable - official_manifest_bytes, 'working_window_bytes': working_window_bytes, 'expert_cache_cap_bytes': EXPERT_CACHE_CAP_BYTES, 'decided_from': ['official_manifest_bytes', 'live_free_bytes', 'reserve_bytes'], 'never_decided_from': ['parameter_count', 'active_params', 'expert_count'], 'full_parent_coverage': True}
...
def vulture_coverage(manifest_tensors, packed_tensors) -> dict:
    ...
    return {'invariant': 'vulture_shard_serial_achieves_full_parent_coverage', 'manifest_count': len(manifest), 'packed_count': len(packed), 'missing': missing, 'unexpected': unexpected, 'complete': not missing and (not unexpected)}

def assert_full_coverage(manifest_tensors, packed_tensors) -> dict:
    result = vulture_coverage(manifest_tensors, packed_tensors)
    if not result['complete']:
        raise CoverageError(f'vulture pass is not total: {len(result['missing'])} missing {result['missing'][:5]}, {len(result['unexpected'])} unexpected {result['unexpected'][:5]}')
    return result
...
def assert_shard_release_ordered(shard_tensors, packed_at_release) -> None:
    ...
    if unpacked:
        raise CoverageError(f'raw shard released with {len(unpacked)} tensor(s) still unpacked: {unpacked[:5]}')
...
def load_ladder(path: Path | None=None) -> list[dict]:
    ...
    for r in doc.get('rungs') or []:
        ...
        if not total_gb:
            continue
        out.append({'parent_id': f'{r.get('rung', '?')}:{r.get('official_repo', '?')}', 'bytes': int(round(float(total_gb) * 10 ** 9)), 'basis': size.get('evidence', 'HAWKING_LADDER_V3 size.total_main_gb')})
    return out

def ladder_decisions(live_free_bytes: int, reserve_bytes: int, ladder: list[dict] | None=None) -> list:
    rungs = ladder if ladder is not None else load_ladder()
    return [dict(choose_mode(p['bytes'], live_free_bytes, reserve_bytes, p['parent_id']), basis=p['basis']) for p in rungs]
