"""Recomposed science module glm52_synthetic (C-SCI-R1)."""
from __future__ import annotations

from lab.operators.glm52_adapter import BoundedSafetensorsReader
from lab.operators.glm52_adapter import CORE
from lab.operators.glm52_adapter import MTP
from lab.operators.glm52_adapter import PROFILE_SYNTHETIC
from lab.operators.glm52_adapter import SYNTHETIC_CONFIG_CONTRACT
from lab.operators.glm52_adapter import SYNTHETIC_INDEXER_TYPES
from lab.operators.glm52_adapter import VIEW_CORE
from lab.operators.glm52_adapter import VIEW_FULL
from lab.operators.glm52_adapter import Inventory
from lab.operators.glm52_adapter import TensorSpec
from lab.operators.glm52_adapter import expected_tensor_specs
from lab.operators.glm52_adapter import schema_summary
from lab.operators.glm52_adapter import validate_config
from lab.operators.glm52_adapter import verify_checkpoint
import hashlib
import json
import os
import struct
...
class SyntheticFixture:
    ...
    def main_only_reader(self, *, max_tensor_bytes: int=256 * 1024 * 1024) -> BoundedSafetensorsReader:
        return BoundedSafetensorsReader(self.main_only_inventory, max_tensor_bytes=max_tensor_bytes)
...
def synthetic_config() -> dict[str, Any]:
    ...
    config.update({'eos_token_id': [60, 61, 62], 'pad_token_id': 60, 'initializer_range': 0.02, 'rope_parameters': {'rope_theta': 10000, 'rope_type': 'default'}, 'transformers_version': 'synthetic-offline-fixture'})
    return config

def _float32_to_bf16_bytes(values: np.ndarray) -> bytes:
    values = np.asarray(values, dtype='<f4')
    bits = values.view(np.uint32)
    rounding = np.uint32(32767) + (bits >> np.uint32(16) & np.uint32(1))
...
def deterministic_values(spec: TensorSpec) -> np.ndarray:
    ...
    if spec.organ == 'router_control':
        return np.arange(size, dtype=np.float32).reshape(spec.shape) / np.float32(65536.0)
    ...
    if spec.organ == 'embeddings':
        ...
        values = centered / np.float32(8192.0)
...
def tensor_payload(spec: TensorSpec) -> bytes:
    ...
    if len(payload) != spec.byte_count:
        raise AssertionError(f'synthetic payload length drift for {spec.name}: {len(payload)} != {spec.byte_count}')
    return payload
...
def _safetensors_bytes(specs: Mapping[str, TensorSpec]) -> tuple[bytes, int, str]:
    ...
    for name in sorted(specs):
        ...
        header[name] = {'dtype': spec.dtype, 'shape': list(spec.shape), 'data_offsets': [cursor, end]}
        payloads.append(payload)
        cursor = end
    encoded = json.dumps(header, sort_keys=True, ensure_ascii=False, separators=(',', ':')).encode('utf-8')
...
def _index_for_parts(parts: tuple[Mapping[str, TensorSpec], ...], shard_names: tuple[str, ...]) -> dict[str, Any]:
...
def build_synthetic_fixture(root: Path) -> SyntheticFixture:
    ...
    full_shards = ('model-00001-of-00003.safetensors', 'model-00002-of-00003.safetensors', 'model-00003-of-00003.safetensors')
    main_shards = ('model-00001-of-00002.safetensors', 'model-00002-of-00002.safetensors')
    ...
    for part, name in zip((core_early, core_late, mtp), full_shards, strict=True):
        ...
        shard_receipts.append({'name': name, 'section': MTP if part is mtp else CORE, 'tensor_count': len(part), 'payload_bytes': payload_bytes, 'file_bytes': len(body), 'sha256': digest})
    ...
    full_inventory = verify_checkpoint(full_dir, profile=PROFILE_SYNTHETIC, view=VIEW_FULL)
    main_inventory = verify_checkpoint(main_only_dir, profile=PROFILE_SYNTHETIC, view=VIEW_CORE)
    ...
    metadata = {'schema': 'hawking.glm52.synthetic_fixture.v1', 'deterministic': True, 'network_access': False, 'profile': PROFILE_SYNTHETIC, 'architecture': schema_summary(geometry), 'views': {'full': {'directory': 'full', 'tensor_count': full_inventory.tensor_count, 'payload_bytes': full_inventory.payload_bytes, 'shard_count': len(full_inventory.shards), 'index_sha256': _sha256(full_dir / 'model.safetensors.index.json')}, 'main_only': {'directory': 'main_only', 'tensor_count': main_inventory.tensor_count, 'payload_bytes': main_inventory.payload_bytes, 'shard_count': len(main_inventory.shards), 'index_sha256': _sha256(main_only_dir / 'model.safetensors.index.json'), 'is_exact_core_filter': True}}, 'shards': shard_receipts, 'routing_fixture': {'router_rows': 'constant positive BF16 fractions 1/256..256/256', 'all_positive_hidden_probe_is_tie_free': True, 'correction_bias_is_strictly_increasing_f32': True}, 'long_context_indexer_contract': {'maximum_positions': int(config['max_position_embeddings']), 'shape_probe_required': True, 'capability_claimed': False}}
    _write_json_once(root / 'fixture_metadata.json', metadata)
    return SyntheticFixture(root=root, full_dir=full_dir, main_only_dir=main_only_dir, config=config, index=index, main_only_index=main_index, metadata=metadata, full_inventory=full_inventory, main_only_inventory=main_inventory)
__all__ = ['SyntheticFixture', 'build_synthetic_fixture', 'deterministic_values', 'synthetic_config', 'tensor_payload']
