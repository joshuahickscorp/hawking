"""Recomposed science module gravity_bench_lab (C-SCI-R1)."""
...
MACHINE_FACTS: dict[str, Any] = {'model_identifier': 'Mac15,14', 'chip': 'Apple M3 Ultra', 'gpu_cores': 60, 'cpu_cores': 28, 'unified_memory_gib': 96, 'metal_version': 4, 'max_threadgroup_memory_bytes': 32768, 'thread_execution_width': 32, 'bandwidth_roof_gb_s': 736.0, 'compute_roof_gflop_s': 17703.0, 'command_buffer_fixed_cost_us': 215.8, 'marginal_dispatch_us': 0.71, 'ridge_flop_per_byte': 24.05}
BANDWIDTH_ROOF_GB_S: float = MACHINE_FACTS['bandwidth_roof_gb_s']
COMPUTE_ROOF_GFLOP_S: float = MACHINE_FACTS['compute_roof_gflop_s']
BASELINES: tuple[str, ...] = ('dense_fp16_mps', 'torch_mps_compact', 'cpu_authority', 'custom_v2', 'track_a', 'track_b', 'selected_hybrid')
COMPONENTS: tuple[str, ...] = ('gpu_execution', 'host_encode', 'command_buffer', 'end_to_end', 'cold_start', 'warm_steady_state')
...
class RefutedClaim:
    ...
    value: float
    reason: str
REFUTED_CLAIMS: tuple[RefutedClaim, ...] = (RefutedClaim('dense_fp16_9.012ms', 'milliseconds', 9.012, 'dense fp16 MPS was re-measured at 0.3674 ms; 9.012 ms is 24x wrong and is the baseline the retracted 35.9x speedup was divided by'), RefutedClaim('speedup_35.9x', 'ratio', 35.9, 'computed against the refuted 9.012 ms dense baseline; the matched single-call result is 0.727x at down and 0.329x at gate/up (custom is SLOWER)'), RefutedClaim('metal_parity_1.4e-6', 'parity', 1.4e-06, 'mislabelled: 1.4e-6 belongs to the torch fp32 path. Custom Metal parity is 2.1e-4, caused by gravity_metal.py:202 casting the codebook to fp16'))
_REFUTED_REL_TOL = 0.001
...
class BenchSpec:
    ...
    def to_json(self) -> dict[str, Any]:
        return {'rows': self.rows, 'cols': self.cols, 'batch': self.batch, 'input_seed': self.input_seed, 'input_dtype': self.input_dtype, 'output_dtype': self.output_dtype, 'warmup': self.warmup, 'reps': self.reps, 'sync_boundary': self.sync_boundary, 'dependency_shape': self.dependency_shape, 'pack_in_timed_region': self.pack_in_timed_region, 'unpack_in_timed_region': self.unpack_in_timed_region}
...
def require_matched(a: BenchSpec, b: BenchSpec, *, left: str='baseline', right: str='candidate') -> None:
    diff = mismatched_fields(a, b)
    if diff:
        raise MatchedBenchmarkError(f'unmatched BenchSpecs ({left} vs {right}): fields differ {diff}. The matched benchmark law forbids a speedup across differing specs.')
...
class TimingStats:
    ...
    def to_json(self) -> dict[str, Any]:
        return {'raw_samples_ms': list(self.raw_samples_ms), 'count': self.count, 'min_ms': self.min_ms, 'median_ms': self.median_ms, 'p95_ms': self.p95_ms, 'max_ms': self.max_ms, 'stddev_ms': self.stddev_ms, 'coefficient_of_variation': self.coefficient_of_variation, 'is_contended': self.is_contended, 'contention_cv_threshold': CONTENTION_CV_THRESHOLD}
...
class ComponentTimings:
    ...
    def primary(self) -> tuple[str, TimingStats]:
        for name in PRIMARY_TIMING_ORDER:
            ...
            if stats is not None:
                return (name, stats)
        raise MatchedBenchmarkError(f'no roofline-billable component measured; need one of {PRIMARY_TIMING_ORDER}')
    ...
    def from_json(cls, value: dict[str, Any]) -> 'ComponentTimings':
        return cls(**{name: None if value[name] == UNMEASURED else TimingStats.from_json(value[name]) for name in COMPONENTS})
...
class BenchResult:
    ...
    def __post_init__(self) -> None:
        if self.baseline not in BASELINES:
            raise MatchedBenchmarkError(f'unknown baseline {self.baseline!r}; register against one of {BASELINES}')

    def roofline(self) -> dict[str, Any]:
        source, stats = self.timings.primary()
        seconds = stats.median_ms / 1000.0
        out: dict[str, Any] = {'timing_source': source, 'seconds_used': seconds, 'bytes_moved': self.bytes_moved if self.bytes_moved is not None else UNMEASURED, 'flops': self.flops if self.flops is not None else UNMEASURED}
        if self.bytes_moved is None or seconds <= 0:
            out['achieved_gb_s'] = UNMEASURED
            out['fraction_of_bandwidth_roof'] = UNMEASURED
        else:
            gb_s = self.bytes_moved / seconds / 1000000000.0
        ...
        if self.flops is None or seconds <= 0:
            ...
            gflop_s = self.flops / seconds / 1000000000.0
            out['achieved_gflop_s'] = gflop_s
            out['fraction_of_compute_roof'] = gflop_s / COMPUTE_ROOF_GFLOP_S
        return out

    def to_json(self) -> dict[str, Any]:
        return {'baseline': self.baseline, 'reproduced': self.reproduced, 'spec': self.spec.to_json(), 'spec_fingerprint': self.spec.fingerprint, 'timings': self.timings.to_json(), 'roofline': self.roofline(), 'notes': self.notes}

    @classmethod
    def from_json(cls, value: dict[str, Any]) -> 'BenchResult':
        roof = value['roofline']
        return cls(baseline=value['baseline'], spec=BenchSpec.from_json(value['spec']), timings=ComponentTimings.from_json(value['timings']), reproduced=value['reproduced'], bytes_moved=None if roof['bytes_moved'] == UNMEASURED else roof['bytes_moved'], flops=None if roof['flops'] == UNMEASURED else roof['flops'], notes=value['notes'])

def assert_not_refuted(*, name: str | None=None, kind: str | None=None, value: float | None=None) -> None:
    ...
    for claim in REFUTED_CLAIMS:
        ...
        if math.isclose(value, claim.value, rel_tol=_REFUTED_REL_TOL):
            raise MatchedBenchmarkError(f'REFUTED claim {claim.name}: value {value!r} matches the disproved {claim.value} ({claim.kind}). {claim.reason}')
...
def measure(fn: Callable[[], Any], spec: BenchSpec) -> TimingStats:
    ...
    for _ in range(spec.reps):
        ...
        fn()
        samples.append((time.perf_counter_ns() - start) / 1000000.0)
...
def speedup(baseline: BenchResult, candidate: BenchResult) -> dict[str, Any]:
    ...
    """
    require_matched(baseline.spec, candidate.spec, left=f'baseline:{baseline.baseline}', right=f'candidate:{candidate.baseline}')
    if not baseline.reproduced:
        raise MatchedBenchmarkError(f'baseline {baseline.baseline!r} is flagged unreproduced; no speedup may be emitted against a baseline this harness did not measure')
    if not candidate.reproduced:
        raise MatchedBenchmarkError(f'candidate {candidate.baseline!r} is flagged unreproduced')
    ...
    if base_source != cand_source:
        raise MatchedBenchmarkError(f'component mismatch: baseline billed {base_source}, candidate billed {cand_source}. A speedup across different timed regions is unmatched.')
    ...
    return {'baseline': baseline.baseline, 'candidate': candidate.baseline, 'specs_matched': True, 'spec_fingerprint': baseline.spec.fingerprint, 'timing_source': base_source, 'statistic': 'median_ms', 'baseline_median_ms': base_stats.median_ms, 'candidate_median_ms': cand_stats.median_ms, 'speedup': ratio, 'slower_than_baseline': ratio < 1.0, 'either_side_contended': base_stats.is_contended or cand_stats.is_contended}

def build_report(results: Sequence[BenchResult], speedups: Sequence[dict[str, Any]], *, label: str, notes: str='') -> dict[str, Any]:
    ...
    return {'schema': SCHEMA, 'label': label, 'generated_at': utc_now(), 'notes': notes, 'machine': dict(MACHINE_FACTS, observed_platform=platform.platform(), observed_machine=platform.machine()), 'baselines': list(BASELINES), 'components': list(COMPONENTS), 'contention_cv_threshold': CONTENTION_CV_THRESHOLD, 'refuted_claims': [{'name': c.name, 'kind': c.kind, 'value': c.value, 'reason': c.reason} for c in REFUTED_CLAIMS], 'results': [r.to_json() for r in results], 'matched': list(speedups)}
...
def selftest(*, rows: int=256, cols: int=512) -> dict[str, Any]:
    ...
    """
    spec = BenchSpec(rows=rows, cols=cols, batch=1, input_seed=20260722, input_dtype='float32', output_dtype='float32', warmup=3, reps=15, sync_boundary='none_cpu_wall_clock', dependency_shape='independent_calls', pack_in_timed_region=False, unpack_in_timed_region=True)
    rng = np.random.default_rng(spec.input_seed)
    w = rng.standard_normal((rows, 8)).astype(np.float32) @ rng.standard_normal((8, cols)).astype(np.float32) * 0.1
    x = spec.make_input()[:, 0]
    art = gf.pack_product_quant(w, dim=8, subspaces=1, k=128, seed=0, iters=4)
    codes = art.config['pq_codes']
    nchunk = codes['nchunk']
    dense = BenchResult(baseline='cpu_authority', spec=spec, timings=ComponentTimings(end_to_end=measure(lambda: w @ x, spec)), bytes_moved=rows * cols * 4 + cols * 4 + rows * 4, flops=2 * rows * cols, notes='numpy float32 dense matvec, CPU')
    pq = BenchResult(baseline='custom_v2', spec=spec, timings=ComponentTimings(end_to_end=measure(lambda: gf.pq_execute(art, x), spec)), bytes_moved=rows * nchunk * codes['S'] + 128 * codes['sub'] * 4 + cols * 4 + rows * 4, flops=2 * rows * cols, notes='gravity_forge.pq_execute, CPU, D=8 k=128 subspaces=1 rotate=False (R0 geometry, reduced size)')
    report = build_report([dense, pq], [speedup(dense, pq)], label='gravity_bench_lab selftest (CPU only, no GPU work)', notes='Harness proof only. Both sides are CPU; neither figure is a production claim. packed_bpw and on-disk 7-bit index billing are not exercised here.')
    round_trip = [BenchResult.from_json(r).to_json() for r in report['results']]
    report['selftest'] = {'json_round_trip_stable': round_trip == report['results'], 'unmatched_comparison_refused': _refuses_unmatched(spec, dense), 'refuted_guard_live': _refuses_refuted()}
    return report
