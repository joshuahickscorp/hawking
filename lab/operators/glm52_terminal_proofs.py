"""Recomposed science module glm52_terminal_proofs (C-SCI-R1)."""
...
OFFICIAL_LICENSE_SHA256 = 'f4a18c6ae40b0a8e7d2b7667f52f6e1994e54a46430d2e172b73cb8c9b5eb0d7'
OFFICIAL_FILE_COUNT = 295
OFFICIAL_WEIGHT_SHARDS = 282
OFFICIAL_SOURCE_LOGICAL_BYTES = 1506693036946
OFFICIAL_WEIGHT_CONTAINER_BYTES = 1506667387408
OFFICIAL_NONWEIGHT_BYTES = 25649538
OFFICIAL_TENSOR_PAYLOAD_BYTES = 1506659919872
OFFICIAL_TENSOR_COUNT = 59585
OFFICIAL_LOGICAL_WEIGHTS = 753329940480
OFFICIAL_BF16_TENSOR_COUNT = 59509
OFFICIAL_BF16_LOGICAL_WEIGHTS = 753329921024
OFFICIAL_BF16_PAYLOAD_BYTES = 1506659842048
OFFICIAL_F32_TENSOR_COUNT = 76
OFFICIAL_F32_LOGICAL_WEIGHTS = 19456
OFFICIAL_F32_PAYLOAD_BYTES = 77824
...
EXTERNAL_BASELINE_PRODUCER_SHA256 = '5a8f216905578f64bf6573fa1598250531d31fcf1a2d77ae1dd2a9e68ba65477'
...
EXTERNAL_BASELINE_COMMON_SHA256 = '6e67ada54b6b4dac24b50682b3d900de349f33d545921fb10169625507201eb2'
INSTRUMENT_PRODUCTION_COMMIT = '2a7fddad866ba73dda933daf310a948469f25bf3'
ADAPTER_INSTRUMENT_LOCAL_SOURCE_SHA256: dict[str, str] = {'tools/condense/glm52_adapter.py': 'f4ca10c3f8f790fa35dac112fdadc58066fc745a2a4b260fbed9fc40e8f3903a', 'tools/condense/glm52_common.py': EXTERNAL_BASELINE_COMMON_SHA256, 'tools/condense/glm52_parity.py': '90b51e99095dbfb9e427f40b67ba7e7eb81557344e3d77060f43887af83d2332', 'tools/condense/glm52_reference.py': '18c480e5484ed8dae70bb7be8c1bc3e38124c0094e2c71d569e2a897611b21d7', 'tools/condense/glm52_synthetic.py': 'b4b83812cbe016333a75503f49d24a4077e5dbad8ab0dbcba6362fb4ee31433b', 'tools/condense/requirements-glm52.txt': 'b70154b77468197abe5e7986830b005286b1091deba5b8a2878b20bba5791835'}
CORPUS_BUILDER_PATH = 'tools/condense/glm52_corpus.py'
CORPUS_BUILDER_SHA256 = 'b08088608f97d72a1e773f436253db071830cad7bb5f7d31e84db0711d493a1e'
CORPUS_INSTRUMENT_SHA256: dict[str, str] = {'tools/condense/glm52_common.py': EXTERNAL_BASELINE_COMMON_SHA256, 'tools/condense/requirements-glm52.txt': 'b70154b77468197abe5e7986830b005286b1091deba5b8a2878b20bba5791835'}
CORPUS_TOKENIZERS_IMPORT_MODULE_SHA256 = '148ecb122f3fee03be9abb1fe213dd85fbc691bf2ad2b437ff531533781a6a3a'
KIMI_REPO = 'moonshotai/Kimi-K2.6'
KIMI_REVISION = '7eb5002f6aadc958aed6a9177b7ed26bb94011bb'
KIMI_SOURCE_LOGICAL_BYTES = 595204999341
KIMI_WEIGHT_BYTES = 595177988208
KIMI_WEIGHT_SHARDS = 64
KIMI_FILE_COUNT = 96
KIMI_SOURCE_ALLOCATED_BYTES = 595205144576
KIMI_TOTAL_CLEANUP_DELTA_BYTES = 597515915264
READY_STOP_CONDITIONS: tuple[str, ...] = ('kimi_final_evidence_verified', 'kimi_raw_source_safely_released', 'official_glm52_immutable_revision_sealed', 'bf16_source_manifest_complete', 'exact_logical_weight_ledger_sealed', 'gravity_pre_audit_complete', 'external_baseline_matrix_complete', 'adapter_twin_green', 'corpus_integrity_green')
_SHA256_RE = re.compile('[0-9a-f]{64}\\Z')
_SHA1_RE = re.compile('[0-9a-f]{40}\\Z')
...
class _Reader:
    ...
    def _open_absolute_root(self) -> tuple[list[int], list[tuple[str, tuple[int, int, int]]], os.stat_result]:
        flags = os.O_RDONLY | _CLOEXEC | _NOFOLLOW | getattr(os, 'O_DIRECTORY', 0)
        ...
        try:
            ...
            for component in (item for item in self._root_path.split('/') if item):
                ...
                if stat.S_ISLNK(named.st_mode) or not stat.S_ISDIR(named.st_mode):
                    _fail(f'terminal-proof root component is not a real directory: {component!r}')
                ...
                if self._identity(named) != self._identity(opened):
                    os.close(child)
                    _fail(f'terminal-proof root component changed while opening: {component!r}')
            ...
            if isinstance(exc, OSError):
                raise TerminalProofError(f'cannot open terminal-proof root: {exc}') from exc
            raise
    ...
    def _open_relative_directories(self, root_fd: int, components: Sequence[str]) -> tuple[list[int], list[tuple[str, tuple[int, int, int]]]]:
        ...
        try:
            for component in components:
                named = self._lstat_at(component, descriptors[-1])
                if stat.S_ISLNK(named.st_mode) or not stat.S_ISDIR(named.st_mode):
                    _fail(f'evidence directory is not a real directory: {component!r}')
                child = os.open(component, flags, dir_fd=descriptors[-1])
                try:
                    opened = os.fstat(child)
                except OSError:
            ...
            for descriptor in reversed(descriptors):
                os.close(descriptor)
            if isinstance(exc, OSError):
                raise TerminalProofError(f'cannot open terminal-proof evidence directory: {exc}') from exc
            raise
    ...
    def _parts(relative: str) -> tuple[str, ...]:
        ...
        if pure.is_absolute() or not pure.parts or any((part in {'', '.', '..'} for part in pure.parts)) or ('/'.join(pure.parts) != relative):
    ...
    def raw(self, relative: str) -> bytes:
        ...
        try:
            ...
            fd = os.open(leaf, os.O_RDONLY | _CLOEXEC | _NOFOLLOW | getattr(os, 'O_NONBLOCK', 0), dir_fd=parent_fd)
            before = os.fstat(fd)
            ...
            while True:
                ...
                total += len(chunk)
            after = os.fstat(fd)
            if self._fingerprint(after) != self._fingerprint(before) or total != int(after.st_size):
            ...
            self._verify_chain(directory_descriptors, directory_links, label='terminal-proof evidence directory')
            self._verify_chain(root_descriptors, root_links, label='terminal-proof root')
    ...
    def sealed(self, relative: str, *, schema: str | None=None, status: str | None | object=...) -> dict[str, Any]:
        if relative in self._loaded:
            ...
            self._loaded[relative] = _Loaded(path=relative, raw=raw, file_sha256=hashlib.sha256(raw).hexdigest(), value=value)
    ...
    def artifact_binding(self, relative: str) -> dict[str, Any]:
        ...
        return {'path': relative, 'file_sha256': loaded.file_sha256, 'bytes': len(loaded.raw), 'schema': loaded.value.get('schema'), 'status': loaded.value.get('status'), 'seal_sha256': loaded.value.get('seal_sha256')}

    def document_binding(self, relative: str) -> dict[str, Any]:
        raw = self.raw(relative)
        return {'path': relative, 'file_sha256': hashlib.sha256(raw).hexdigest(), 'bytes': len(raw)}

def validator_source_binding() -> dict[str, Any]:
    path = Path(__file__).resolve()
    named_pre = path.lstat()
    if stat.S_ISLNK(named_pre.st_mode) or not stat.S_ISREG(named_pre.st_mode) or int(named_pre.st_nlink) != 1:
    ...
    try:
        ...
        before = (int(opened_pre.st_dev), int(opened_pre.st_ino), int(opened_pre.st_size), int(opened_pre.st_nlink), int(opened_pre.st_mtime_ns), int(opened_pre.st_ctime_ns))
        named = (int(named_pre.st_dev), int(named_pre.st_ino), int(named_pre.st_size), int(named_pre.st_nlink), int(named_pre.st_mtime_ns), int(named_pre.st_ctime_ns))
        if before != named:
        ...
        after = (int(opened_post.st_dev), int(opened_post.st_ino), int(opened_post.st_size), int(opened_post.st_nlink), int(opened_post.st_mtime_ns), int(opened_post.st_ctime_ns))
        if after != before:
            _fail('terminal-proof validator source changed while reading')
    finally:
        os.close(descriptor)
    named_post = path.lstat()
    if (int(named_post.st_dev), int(named_post.st_ino), int(named_post.st_size), int(named_post.st_nlink), int(named_post.st_mtime_ns), int(named_post.st_ctime_ns)) != after:
    ...
    return {'path': 'tools/condense/glm52_terminal_proofs.py', 'sha256': hashlib.sha256(raw).hexdigest(), 'bytes': len(raw)}
...
def _stable_absolute_absence(path_value: Any) -> dict[str, Any]:
    ...
    before = (int(parent_pre.st_dev), int(parent_pre.st_ino), int(parent_pre.st_mtime_ns), int(parent_pre.st_ctime_ns))
    after = (int(parent_post.st_dev), int(parent_post.st_ino), int(parent_post.st_mtime_ns), int(parent_post.st_ctime_ns))
    if before != after:
...
def _validate_kimi_final(reader: _Reader) -> _SemanticResult:
    ...
    handoff = reader.sealed(handoff_path, schema='hawking.glm52.handoff_precheck.v1', status='PASS_WITH_SECURITY_AND_ROLLBACK_EXCEPTIONS')
    final = reader.sealed(final_path, schema='hawking.kimi_k26.gravity_final.v1', status='CLOSED')
    ...
    if not isinstance(bpw, (int, float)) or not math.isclose(float(bpw), 0.9085909525553385, rel_tol=0.0, abs_tol=1e-15):
        _fail('Kimi complete BPW mismatch')
    diagnosis = _require_object(final.get('causal_diagnosis'), 'Kimi diagnosis')
    _expect(diagnosis.get('diagnosis'), 'UPSTREAM_STATE_PRIMARY_ROUTE_SECONDARY_CONDITIONAL_AMPLIFIER', 'Kimi causal diagnosis')
    ...
    _expect(_require_object(fidelity.get('F2'), 'Kimi F2').get('promotable'), False, 'Kimi F2 promotable')
    ...
    _expect(handoff_science.get('primary_diagnosis'), 'UPSTREAM_STATE_PRIMARY_ROUTE_SECONDARY_CONDITIONAL_AMPLIFIER', 'handoff diagnosis')
    ...
    _expect(handoff_final.get('file_sha256'), hashlib.sha256(reader.raw(final_path)).hexdigest(), 'handoff final file hash')
    ...
    for evidence_id, (path, schema, status) in _KIMI_CHAIN.items():
        ...
        _expect(entry.get('seal_sha256'), FROZEN_ARTIFACT_SEALS[path], f'Kimi chain {evidence_id} seal')
        reader.sealed(path, schema=schema, status=status)
        artifact_paths.append(path)
        observed_chain_rows.append({'evidence_id': evidence_id, 'path': path, 'seal_sha256': entry['seal_sha256']})
    _expect(handoff_science.get('evidence_chain_verified'), observed_chain_rows, 'handoff evidence chain')
    ...
    if 'native functional student' not in transfer.lower() or 'teacher-hidden' not in transfer.lower():
        _fail('Kimi next-parent transfer omits required native/teacher-hidden action')
    handoff_final_md = _require_object(handoff_science.get('final_markdown'), 'handoff final Markdown')
    handoff_transfer = _require_object(handoff_science.get('next_parent_transfer'), 'handoff next-parent transfer')
    _expect(handoff_final_md.get('sha256'), FROZEN_RAW_DOCUMENT_SHA256['KIMI_K26_GRAVITY_FINAL.md'], 'handoff final Markdown hash')
    _expect(handoff_transfer.get('sha256'), FROZEN_RAW_DOCUMENT_SHA256['KIMI_K26_NEXT_PARENT_TRANSFER.md'], 'handoff transfer hash')
    _expect(handoff_final_md.get('integrity_authority'), '0210e5aa05f0e3c69d6f2022c539c9dc90cce322', 'handoff closure commit')
    _expect(handoff_transfer.get('integrity_authority'), '0210e5aa05f0e3c69d6f2022c539c9dc90cce322', 'handoff transfer commit')
    ...
    _expect(rollback.get('reproducibility_capsule_status'), 'DEGRADED_BY_PRIOR_BROAD_CLEANSE', 'handoff capsule exception')
    return _SemanticResult(artifact_paths=tuple(artifact_paths), document_paths=tuple(FROZEN_RAW_DOCUMENT_SHA256), facts={'terminal_outcome': 'OUTCOME_C', 'best_candidate': 'P1_DUAL_PATH_RECOVERY_R16X2', 'best_complete_bpw': float(bpw), 'f2_promotable': False, 'causal_diagnosis': diagnosis['diagnosis'], 'evidence_artifact_count': len(_KIMI_CHAIN), 'partial_prerequisite_evidence_preserved': True, 'next_parent_action': 'NATIVE_STUDENT_TEACHER_HIDDEN_FIRST'}, scope={'proves': 'sealed Kimi scientific closure and transfer evidence', 'does_not_prove': ['retained Kimi runtime or payload', 'GLM-5.2 body execution', 'GLM-5.2 capability']})
...
def _validate_kimi_release(reader: _Reader) -> _SemanticResult:
    ...
    release = reader.sealed(release_path, schema='hawking.kimi_k26.source_release_for_glm52.v1', status='RECONCILED_ALREADY_RELEASED')
    final = reader.sealed(final_path, schema='hawking.kimi_k26.gravity_final.v1', status='CLOSED')
    manifest = reader.sealed(manifest_path, schema='hawking.kimi_k26.official_manifest.v1', status=None)
    remediation = reader.sealed(remediation_path, schema='hawking.kimi_k26.credential_remediation_for_glm52.v1', status='SANITIZED_CURRENT_TREE_ROTATION_PENDING')
    cleanse = reader.sealed(cleanse_path, schema='hawking.kimi_k26.device_cleanse.final.v1', status=None)
    ...
    _expect(closure.get('byte_auction_seal_sha256'), FROZEN_ARTIFACT_SEALS['KIMI_K26_FINAL_BYTE_AUCTION.json'], 'Kimi byte-auction binding')
    source = _require_object(release.get('source'), 'Kimi release source')
    for key, expected in (('repo', KIMI_REPO), ('revision', KIMI_REVISION), ('manifest_files', KIMI_FILE_COUNT), ('weight_shards', KIMI_WEIGHT_SHARDS), ('logical_bytes', KIMI_SOURCE_LOGICAL_BYTES), ('weight_bytes', KIMI_WEIGHT_BYTES), ('exists_now', False)):
    ...
    _expect(release.get('credential_remediation_seal_sha256'), remediation['seal_sha256'], 'Kimi credential remediation binding')
    _expect(release.get('release_timing'), 'COMPLETED_BEFORE_THIS_GLM52_GOAL', 'Kimi release timing')
    ...
    runtime_observation = _stable_absolute_absence('/Users/scammermike/Library/Application Support/Hawking/KimiK26')
    plist_observation = _stable_absolute_absence('/Users/scammermike/Library/LaunchAgents/com.hawking.kimi-k26-doctor-prime.plist')
    allocated = _require_object(release.get('allocated_byte_reconciliation'), 'Kimi allocated reconciliation')
    _expect(allocated.get('exact_kimi_k2_6_source_root_allocated_bytes'), KIMI_SOURCE_ALLOCATED_BYTES, 'Kimi exact allocated bytes')
    _expect(allocated.get('total_cleanup_free_space_delta_bytes'), KIMI_TOTAL_CLEANUP_DELTA_BYTES, 'Kimi cleanup delta')
    _expect(allocated.get('total_delta_includes_runtime_and_other_targets'), True, 'Kimi delta scope')
    rollback = _require_object(release.get('rollback_boundary'), 'Kimi rollback boundary')
    expected_rollback = {'best_local_payload_preserved': False, 'runtime_preserved': False, 'scientific_reports_preserved': True, 'implementation_recoverable_from_git_history': True, 'reproducibility_capsule_status': 'DEGRADED_BY_PRIOR_BROAD_CLEANSE'}
    ...
    publication = _require_object(remediation.get('publication_gate'), 'Kimi publication gate')
    _expect(publication.get('telegram_delivery_allowed'), False, 'Kimi Telegram gate')
    cleanse_verification = _require_object(cleanse.get('verification'), 'Kimi cleanse verification')
    ...
    _expect(cleanse_deleted.get('kimi_k2_6_source_allocated_bytes'), KIMI_SOURCE_ALLOCATED_BYTES, 'Kimi cleanse source allocation')
    _expect(cleanse_deleted.get('kimi_runtime_allocated_bytes'), 2310508544, 'Kimi cleanse runtime allocation')
    _expect(cleanse_deleted.get('runtime_credentials_logs_captures_payloads_and_checkpoints'), True, 'Kimi broad runtime deletion')
    return _SemanticResult(artifact_paths=(release_path, final_path, manifest_path, remediation_path, cleanse_path), document_paths=(), facts={'source_repo': KIMI_REPO, 'source_revision': KIMI_REVISION, 'source_logical_bytes': KIMI_SOURCE_LOGICAL_BYTES, 'source_allocated_bytes': KIMI_SOURCE_ALLOCATED_BYTES, 'total_cleanup_delta_bytes': KIMI_TOTAL_CLEANUP_DELTA_BYTES, 'source_absence': live_observation, 'runtime_absence': runtime_observation, 'launchd_plist_absence': plist_observation, 'release_preceded_glm_goal': True, 'new_deletion_by_release_receipt': False, 'rollback_payload_preserved': False, 'rollback_runtime_preserved': False, 'rollback_capsule_status': 'DEGRADED_BY_PRIOR_BROAD_CLEANSE', 'scientific_reports_preserved': True, 'credential_rotation_pending': True, 'git_history_purge_pending': True, 'telegram_delivery_allowed': False, 'mop_touched': False, 'runtime_allocated_bytes_deleted': 2310508544, 'broad_runtime_payload_capture_checkpoint_deletion': True}, scope={'proves': 'current Kimi raw-source absence and retrospective accounting', 'exceptions_carried': ['best local payload was not preserved', 'runtime was not preserved', 'rollback capsule is degraded', 'credential rotation and history coordination remain pending'], 'does_not_prove': ['Part-I-compliant safe release', 'pre-delete exact-path/realpath/MOP gates', 'pre-delete mapped-reader and queue absence', 'healthy Kimi rollback', 'Telegram readiness'], 'remediation_required': ['rehydrate the exact immutable Kimi source', 'reconstruct and preserve the best payload and runtime capsule', 'repeat exact-path, resolved-realpath, outside-MOP, no-reader, no-queue, and official-rehydration gates', 'release only the exact dependency-closed source inventory', 'ground the exact allocated bytes recovered by that release']}, proven=False, blockers=('MISSING_GROUNDED_PREDELETE_EXACT_PATH_REALPATH_MOP_AUDIT', 'MISSING_GROUNDED_PREDELETE_READER_AND_QUEUE_AUDIT', 'REPRODUCIBILITY_CAPSULE_DEGRADED_PAYLOAD_AND_RUNTIME_ABSENT', 'RETROSPECTIVE_AGGREGATE_DELTA_NOT_EXACT_RELEASE_RECOVERY'))
...
def _validate_revision(reader: _Reader) -> _SemanticResult:
    ...
    admission = reader.sealed(admission_path, schema='hawking.glm52.source_admission.v1', status='ADMITTED_CONTROL_PLANE_HEADERS_AND_PLAN_BODY_PENDING')
    manifest = reader.sealed(manifest_path, schema='hawking.glm52.official_manifest.v1', status='PASS_CONTROL_PLANE_AND_HEADERS_BODY_PENDING')
    architecture = reader.sealed(architecture_path, schema='hawking.glm52.architecture_contract.v1', status='PASS_CONFIG_INDEX_AND_HEADERS')
    ledger = reader.sealed(ledger_path, schema='hawking.glm52.logical_weight_ledger.v1', status='PASS_HEADER_DERIVED')
    formats = reader.sealed(format_path, schema='hawking.glm52.source_format_ledger.v1', status='PASS_HEADER_DERIVED_BODY_PENDING')
    for artifact, label in ((admission, 'admission'), (manifest, 'manifest'), (architecture, 'architecture'), (ledger, 'weight ledger'), (formats, 'format ledger')):
    ...
    _expect(admission.get('main_matches_pinned_revision_at_admission'), True, 'main revision binding')
    ...
    _expect(evidence, {'architecture_contract_seal_sha256': architecture['seal_sha256'], 'dependency_graph_seal_sha256': FROZEN_ARTIFACT_SEALS['GLM52_SHARD_DEPENDENCY_GRAPH.json'], 'logical_weight_ledger_seal_sha256': ledger['seal_sha256'], 'official_manifest_seal_sha256': manifest['seal_sha256'], 'source_format_ledger_seal_sha256': formats['seal_sha256'], 'streaming_schedule_seal_sha256': FROZEN_ARTIFACT_SEALS['GLM52_STREAMING_SCHEDULE_PRE_AUTOTUNE.json']}, 'admission dependent seals')
    ...
    return _SemanticResult(artifact_paths=(admission_path, manifest_path, architecture_path, ledger_path, format_path), document_paths=(), facts={'repo': OFFICIAL_REPO, 'immutable_revision': OFFICIAL_REVISION, 'license_spdx': OFFICIAL_LICENSE, 'license_sha256': OFFICIAL_LICENSE_SHA256, 'immutable_tree_url': tree_url, 'body_shards_verified': 0}, scope={'proves': 'official repository, immutable revision, and license identity', 'does_not_prove': ['weight-body verification', 'BF16 reference forward']})

def _manifest_rows(manifest: Mapping[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
...
def _validate_manifest(reader: _Reader) -> _SemanticResult:
    ...
    for artifact, label in ((manifest, 'manifest'), (formats, 'format ledger')):
        _expect(artifact.get('repo'), OFFICIAL_REPO, f'{label} repo')
        _expect(artifact.get('revision'), OFFICIAL_REVISION, f'{label} revision')
    for key, expected in (('file_count', OFFICIAL_FILE_COUNT), ('weight_shards', OFFICIAL_WEIGHT_SHARDS), ('source_logical_bytes', OFFICIAL_SOURCE_LOGICAL_BYTES), ('weight_container_logical_bytes', OFFICIAL_WEIGHT_CONTAINER_BYTES), ('nonweight_logical_bytes', OFFICIAL_NONWEIGHT_BYTES)):
    ...
    expected_names = {f'model-{index:05d}-of-00282.safetensors' for index in range(1, OFFICIAL_WEIGHT_SHARDS + 1)}
    _expect({row.get('path') for row in weights}, expected_names, 'weight-shard names')
    _expect(sum((int(row.get('logical_bytes', -1)) for row in rows)), OFFICIAL_SOURCE_LOGICAL_BYTES, 'manifest row byte sum')
    _expect(sum((int(row.get('logical_bytes', -1)) for row in weights)), OFFICIAL_WEIGHT_CONTAINER_BYTES, 'manifest weight byte sum')
    for row in weights:
        path = str(row.get('path'))
        _expect(row.get('download_state'), 'NOT_FETCHED', f'{path} download state')
        _expect(row.get('verification_state'), 'HEADER_VERIFIED_BODY_NOT_FETCHED', f'{path} verification boundary')
        local = _require_object(row.get('local'), f'{path} local state')
        _expect(local.get('state'), 'HEADER_ONLY_REMOTE_RANGE_NO_LOCAL_WEIGHT_BODY', f'{path} local state')
    ...
    _expect(dtype, {'BF16': {'logical_weights': OFFICIAL_BF16_LOGICAL_WEIGHTS, 'source_payload_bytes': OFFICIAL_BF16_PAYLOAD_BYTES, 'tensor_count': OFFICIAL_BF16_TENSOR_COUNT}, 'F32': {'logical_weights': OFFICIAL_F32_LOGICAL_WEIGHTS, 'source_payload_bytes': OFFICIAL_F32_PAYLOAD_BYTES, 'tensor_count': OFFICIAL_F32_TENSOR_COUNT}}, 'source dtype summary')
    gates = _require_object(admission.get('admission_gates'), 'admission gates')
    _expect(gates.get('official_file_manifest_complete'), True, 'manifest gate')
    _expect(gates.get('body_stream'), False, 'body stream gate')
    return _SemanticResult(artifact_paths=(manifest_path, format_path, admission_path), document_paths=(), facts={'repo': OFFICIAL_REPO, 'revision': OFFICIAL_REVISION, 'official_files': OFFICIAL_FILE_COUNT, 'weight_shards': OFFICIAL_WEIGHT_SHARDS, 'source_logical_bytes': OFFICIAL_SOURCE_LOGICAL_BYTES, 'weight_container_bytes': OFFICIAL_WEIGHT_CONTAINER_BYTES, 'tensor_payload_bytes': OFFICIAL_TENSOR_PAYLOAD_BYTES, 'tensor_count': OFFICIAL_TENSOR_COUNT, 'bf16_tensor_count': OFFICIAL_BF16_TENSOR_COUNT, 'headers_verified': True, 'body_shards_fetched': 0, 'body_shards_sha256_verified': 0, 'body_verification_complete': False}, scope={'proves': 'complete immutable remote file manifest and verified tensor headers', 'manifest_completeness_is_not_body_verification': True, 'body_verification_belongs_to_stop': 'every_official_bf16_shard_fetched_verified', 'does_not_prove': ['any complete shard body', 'BF16 teacher execution']})
...
def _validate_weight_ledger(reader: _Reader) -> _SemanticResult:
    ...
    formats = reader.sealed(format_path, schema='hawking.glm52.source_format_ledger.v1', status='PASS_HEADER_DERIVED_BODY_PENDING')
    architecture = reader.sealed(architecture_path, schema='hawking.glm52.architecture_contract.v1', status='PASS_CONFIG_INDEX_AND_HEADERS')
    for artifact, label in ((ledger, 'weight ledger'), (formats, 'format ledger'), (architecture, 'architecture')):
        _expect(artifact.get('repo'), OFFICIAL_REPO, f'{label} repo')
        _expect(artifact.get('revision'), OFFICIAL_REVISION, f'{label} revision')
    for key, expected in (('tensor_count', OFFICIAL_TENSOR_COUNT), ('logical_weight_denominator', OFFICIAL_LOGICAL_WEIGHTS), ('source_payload_bytes', OFFICIAL_TENSOR_PAYLOAD_BYTES)):
        _expect(ledger.get(key), expected, f'weight ledger {key}')
    _expect(ledger.get('source_dtype_summary'), formats.get('dtype_summary'), 'ledger dtype cross-binding')
    architecture_weights = _require_object(architecture.get('weights'), 'architecture weights')
    _expect(architecture_weights.get('logical_elements'), OFFICIAL_LOGICAL_WEIGHTS, 'architecture logical weights')
    _expect(architecture_weights.get('tensor_count'), OFFICIAL_TENSOR_COUNT, 'architecture tensor count')
    _expect(architecture_weights.get('dtype_summary'), formats.get('dtype_summary'), 'architecture dtype cross-binding')
    views = _require_object(ledger.get('major_accounting_views'), 'accounting views')
    all_weights = _require_object(views.get('all_declared_model_weights'), 'all-declared view')
    _expect(all_weights, {'logical_weights': OFFICIAL_LOGICAL_WEIGHTS, 'source_payload_bytes': OFFICIAL_TENSOR_PAYLOAD_BYTES, 'tensor_count': OFFICIAL_TENSOR_COUNT}, 'all-declared accounting view')
    _expect(_require_object(ledger.get('mtp_policy'), 'MTP policy').get('included_in_complete_denominator'), True, 'MTP denominator inclusion')
    ...
    expected_budgets = {'hard_1_bpw': OFFICIAL_LOGICAL_WEIGHTS // 8, 'planned_0_98_bpw': OFFICIAL_LOGICAL_WEIGHTS * 49 // (50 * 8), 'rate_0_75_bpw': OFFICIAL_LOGICAL_WEIGHTS * 3 // (4 * 8), 'rate_0_50_bpw': OFFICIAL_LOGICAL_WEIGHTS // (2 * 8), 'rate_0_33_represented_as_one_third_bpw': OFFICIAL_LOGICAL_WEIGHTS // (3 * 8), 'rate_0_25_bpw': OFFICIAL_LOGICAL_WEIGHTS // (4 * 8)}
    for name, expected_bytes in expected_budgets.items():
        budget = _require_object(budgets.get(name), f'rate budget {name}')
        _expect(budget.get('maximum_complete_physical_bytes'), expected_bytes, f'rate budget {name} bytes')
    return _SemanticResult(artifact_paths=(ledger_path, format_path, architecture_path), document_paths=(), facts={'repo': OFFICIAL_REPO, 'revision': OFFICIAL_REVISION, 'logical_weight_denominator': OFFICIAL_LOGICAL_WEIGHTS, 'tensor_count': OFFICIAL_TENSOR_COUNT, 'bf16_logical_weights': OFFICIAL_BF16_LOGICAL_WEIGHTS, 'f32_router_bias_logical_weights': OFFICIAL_F32_LOGICAL_WEIGHTS, 'tensor_payload_bytes': OFFICIAL_TENSOR_PAYLOAD_BYTES, 'mtp_included': True, 'stored_aliases': 0}, scope={'proves': 'exact header-derived complete logical-weight denominator', 'does_not_prove': ['weight-body hashes', 'physical compact artifact size']})

def _validate_pre_audit(reader: _Reader) -> _SemanticResult:
    path = 'GRAVITY_COMPLETENESS_AUDIT_GLM52_PRE.json'
    audit = reader.sealed(path, schema='hawking.gravity_completeness_audit.glm52_pre.v1', status='PASS_FROZEN_PRE_CAMPAIGN_BASELINE')
    ...
    _expect(snapshot.get('repository_head'), '8d634af9702aa965728f057661b5d1fad1883f45', 'pre-audit repository head')
    _expect(snapshot.get('at_utc'), '2026-07-21T20:29:51.121903Z', 'pre-audit timestamp')
    ...
    _expect(set(scores), {'GLM52_PRE', 'GPT_OSS_120B', 'KIMI_K26', 'QWEN3_235B'}, 'pre-audit score inventory')
    ...
    for model, rows_value in evidence.items():
        ...
        for index, row_value in enumerate(rows):
            ...
            _expect(hashlib.sha256(raw).hexdigest(), row.get('sha256'), f'pre-audit evidence hash {evidence_path}')
    ...
    return _SemanticResult(artifact_paths=(path,), document_paths=tuple(document_paths), facts={'audit_boundary': 'FROZEN_PRE_CAMPAIGN', 'axis_count': len(_PRE_AUDIT_AXES), 'glm52_pre_total': 16, 'glm52_pre_maximum': 105, 'evidence_rows': evidence_rows, 'later_glm_artifacts_excluded': True, 'glm_scientific_result_at_entry': False}, scope={'proves': 'frozen pre-campaign Gravity maturity baseline', 'does_not_prove': ['post-campaign maturity', 'any GLM scientific result']})

def _validate_external_matrix(reader: _Reader) -> _SemanticResult:
    path = 'GRAVITY_EXTERNAL_BASELINE_MATRIX.json'
    matrix = reader.sealed(path, schema='hawking.gravity_external_baseline_matrix.v1', status='PASS_PRIMARY_SOURCE_COMPARISON')
    ...
    for index, method_value in enumerate(methods):
        ...
        for source_value in sources:
            ...
            if kind == 'paper':
                identity = _require_object(source.get('content_identity'), f'paper identity {name}')
    ...
    _expect(names, ['BitNet b1.58 / b1.58 2B4T', 'ParetoQ', 'QuIP', 'QuIP#', 'AQLM', 'VPTQ', 'GPTVQ', 'BiLLM', 'STBLLM', 'QMoE', 'BTC-LLM', 'NanoQuant', 'LittleBit'], 'external method inventory')
    qmoe = next((method for method in methods if method.get('method') == 'QMoE'))
    _expect(_require_object(qmoe.get('rate'), 'QMoE rate').get('canonical_artifact_bpw'), 0.807, 'QMoE canonical artifact BPW')
    ...
    _expect(set(taxonomy), {'canonical_artifact_bpw', 'decoded_tensor_payload_bpw', 'nominal_or_method_bpw', 'ranking_rule'}, 'rate taxonomy levels')
    binding = _require_object(matrix.get('instrument_binding'), 'matrix instrument')
    _expect(binding.get('generator'), EXTERNAL_BASELINE_PRODUCER_PATH, 'external matrix generator path')
    _expect(binding.get('generator_sha256'), EXTERNAL_BASELINE_PRODUCER_SHA256, 'external matrix generator hash')
    _expect(binding.get('repository_base_commit'), EXTERNAL_BASELINE_COMMON_COMMIT, 'external matrix repository base commit')
    _expect(binding.get('common_sha256'), EXTERNAL_BASELINE_COMMON_SHA256, 'external matrix common hash')
    ...
    return _SemanticResult(artifact_paths=(path,), document_paths=(), facts={'method_count': len(methods), 'paper_source_count': paper_sources, 'commit_pinned_code_source_count': code_sources, 'primary_source_only': True, 'rate_taxonomy_levels_distinguished': True, 'qmoe_canonical_artifact_bpw': 0.807, 'matched_cross_paper_leaderboard_claimed': False, 'historical_producer_path': EXTERNAL_BASELINE_PRODUCER_PATH, 'historical_common_path': EXTERNAL_BASELINE_COMMON_PATH, 'historical_common_commit': EXTERNAL_BASELINE_COMMON_COMMIT}, scope={'proves': 'primary-source external method and accounting comparison', 'does_not_prove': ['GLM result superiority', 'matched model/hardware comparison', 'campaign capability']})

def _validate_adapter(reader: _Reader) -> _SemanticResult:
    path = 'GLM52_ADAPTER_TWIN.json'
    parity_path = 'GLM52_REFERENCE_PARITY.json'
    manifest_path = 'GLM52_OFFICIAL_MANIFEST.json'
    twin = reader.sealed(path, schema='hawking.glm52.adapter_twin.v1', status='PASS_SYNTHETIC_TWIN_AND_OFFICIAL_HEADER_TOKENIZER_SCHEMA')
    manifest = reader.sealed(manifest_path, schema='hawking.glm52.official_manifest.v1', status='PASS_CONTROL_PLANE_AND_HEADERS_BODY_PENDING')
    parity = reader.sealed(parity_path, schema='hawking.glm52.reference_parity.v1', status='PASS_SYNTHETIC_MAIN_AND_MTP_SELF_CONSISTENCY_SOURCE_PARENT_PENDING')
    ...
    _expect(binding.get('source_class'), ['OFFICIAL_BF16_TEACHER', 'VULTURE_XET_STREAMING', 'TEXT_GENERATION'], 'adapter source class')
    ...
    for key in ('expert_gate_then_up', 'hardlinked_core_shards', 'main_only_is_exact_core_filter', 'mtp_has_full_indexer', 'shared_layers_have_no_indexer_tensors'):
    ...
    for key, expected in (('status', 'PASS'), ('actual_tensor_count', OFFICIAL_TENSOR_COUNT), ('expected_tensor_count', OFFICIAL_TENSOR_COUNT), ('actual_logical_elements', OFFICIAL_LOGICAL_WEIGHTS), ('expected_logical_elements', OFFICIAL_LOGICAL_WEIGHTS)):
    ...
    streaming = _require_object(checks.get('streaming_window_admission'), 'adapter streaming admission')
    ...
    long_context = _require_object(fixture.get('long_context_indexer_contract'), 'adapter long-context scope')
    ...
    source_hashes = _require_object(instrument.get('local_source_sha256'), 'adapter source hashes')
    _expect(dict(sorted(source_hashes.items())), dict(sorted(ADAPTER_INSTRUMENT_LOCAL_SOURCE_SHA256.items())), 'adapter instrument historical pin map')
    for source_path, expected_hash in ADAPTER_INSTRUMENT_LOCAL_SOURCE_SHA256.items():
        _expect(source_hashes.get(source_path), expected_hash, f'adapter instrument hash {source_path}')
    parity_instrument = _require_object(parity.get('instrument_binding'), 'reference parity instrument')
    _expect(parity_instrument.get('local_source_sha256'), source_hashes, 'adapter/reference instrument identity')
    _expect(parity.get('synthetic_fixture'), twin.get('synthetic_fixture'), 'adapter/reference synthetic fixture')
    claim_boundary = _require_object(parity.get('claim_boundary'), 'reference claim boundary')
    _expect(claim_boundary.get('capability'), 'NOT_CLAIMED', 'reference capability')
    _expect(claim_boundary.get('official_bf16_parent_forward'), 'PENDING_FIRST_ADMITTED_SOURCE_WINDOW', 'reference BF16 boundary')
    main_metrics = _require_object(parity.get('official_transformers_main_vs_numpy_reference'), 'reference main metrics')
    ...
    if float(main_metrics.get('maximum_absolute_error')) > float(thresholds.get('maximum_absolute_logit_error')) or float(main_metrics.get('cosine')) < float(thresholds.get('minimum_logit_cosine')) or float(main_metrics.get('top1_agreement')) < float(thresholds.get('minimum_top1_agreement')) or (float(main_metrics.get('relative_frobenius_error')) > float(thresholds.get('relative_frobenius_logit_error'))):
        _fail('reference main metrics exceed their frozen thresholds')
    _expect(parity.get('reference_deterministic_exact_replay'), True, 'reference deterministic replay')
    ...
    _expect(_require_object(metal.get('metrics'), 'reference Metal metrics').get('status'), 'PASS', 'reference Metal status')
    ...
    long_probe = _require_object(parity.get('long_context_indexer_shape_probe'), 'reference long-context probe')
    ...
    tokenizer = _require_object(checks.get('official_tokenizer_chat_assembly'), 'adapter tokenizer')
    ...
    return _SemanticResult(artifact_paths=(path, parity_path, manifest_path), document_paths=(), facts={'repo': OFFICIAL_REPO, 'revision': OFFICIAL_REVISION, 'official_schema_tensor_count': OFFICIAL_TENSOR_COUNT, 'official_schema_logical_weights': OFFICIAL_LOGICAL_WEIGHTS, 'synthetic_twin_green': True, 'official_header_schema_green': True, 'official_tokenizer_chat_assembly_green': True, 'body_backed_parent_parity': False, 'bf16_reference_forward_validated': False, 'capability_claimed': False, 'historical_instrument_commit': INSTRUMENT_PRODUCTION_COMMIT, 'historical_instrument_paths': sorted(ADAPTER_INSTRUMENT_LOCAL_SOURCE_SHA256)}, scope={'proves': 'synthetic adapter twin plus official header/tokenizer schema', 'body_backed': False, 'does_not_prove': ['real BF16 tensor-value parity', 'BF16 reference forward', 'model capability']})

def _validate_corpus(reader: _Reader) -> _SemanticResult:
    path = 'GLM52_CORPUS_INTEGRITY.json'
    manifest_path = 'GLM52_OFFICIAL_MANIFEST.json'
    corpus = reader.sealed(path, schema='hawking.glm52.corpus_integrity.v2', status='PASS')
    manifest = reader.sealed(manifest_path, schema='hawking.glm52.official_manifest.v1', status='PASS_CONTROL_PLANE_AND_HEADERS_BODY_PENDING')
    scope = _require_object(corpus.get('scope'), 'corpus scope')
    expected_scope = {'model_payload_downloaded': False, 'network_access_used': False, 'capability_claim_permitted': False, 'real_model_scores_required_for_capability_claim': True}
    ...
    for key, expected in (('status', 'PASS'), ('record_count', 171), ('core_record_count', 135), ('long_context_record_count', 36), ('atomic_segment_count', 33804), ('unique_prompt_hashes', 171), ('unique_context_window_hashes', 171), ('unique_source_document_ids', 171), ('unique_source_document_hashes', 171), ('unique_segment_hashes', 33804)):
    ...
    ladder_by_rung = {_require_object(row, 'corpus ladder row').get('rung'): row for row in ladder}
    ...
    _expect(ladder_by_rung['256K'].get('admission'), 'NOT_ADMITTED_RESOURCE_VALIDATION_PENDING', 'corpus 256K boundary')
    _expect(ladder_by_rung['1M'].get('admission'), 'NOT_ADMITTED_EXACT_RUNTIME_PENDING', 'corpus 1M boundary')
    tokenizer = _require_object(corpus.get('official_tokenizer'), 'corpus tokenizer')
    for key, expected in (('repository', OFFICIAL_REPO), ('revision', OFFICIAL_REVISION), ('file', 'tokenizer.json'), ('sha256', '19e773648cb4e65de8660ea6365e10acca112d42a854923df93db4a6f333a82d'), ('bytes', 20217442), ('vocabulary_size', 154856), ('local_files_only', True), ('host_path_sealed', False)):
    ...
    _expect(builder.get('builder_sha256'), CORPUS_BUILDER_SHA256, 'corpus builder hash')
    ...
    for instrument_path, expected_hash in CORPUS_INSTRUMENT_SHA256.items():
        _expect(instruments.get(instrument_path), expected_hash, f'corpus instrument hash {instrument_path}')
    _expect(instruments.get('tokenizers_import_module'), CORPUS_TOKENIZERS_IMPORT_MODULE_SHA256, 'corpus tokenizers_import_module pin')
    quality = _require_object(corpus.get('quality_metric_contract'), 'corpus quality contract')
    _expect(quality.get('corpus_manifest_is_not_a_model_quality_result'), True, 'corpus quality boundary')
    _expect(quality.get('capability_claim_requires_real_scored_execution'), True, 'corpus capability boundary')
    return _SemanticResult(artifact_paths=(path, manifest_path), document_paths=(), facts={'repo': OFFICIAL_REPO, 'revision': OFFICIAL_REVISION, 'record_count': 171, 'partition_count': 9, 'domain_count': 15, 'integrity_gate_count': 12, 'admitted_context_rungs': ['2K', '8K', '32K', '128K'], 'withheld_context_rungs': ['256K', '1M'], 'network_access_used': False, 'model_payload_downloaded': False, 'capability_claimed': False, 'historical_instrument_commit': INSTRUMENT_PRODUCTION_COMMIT, 'historical_builder_path': CORPUS_BUILDER_PATH}, scope={'proves': 'offline deterministic corpus integrity and split hygiene', 'host_paths_sealed': False, 'does_not_prove': ['model quality', '256K execution safety', '1M execution safety', 'capability']})
_VALIDATORS: dict[str, Callable[[_Reader], _SemanticResult]] = {'kimi_final_evidence_verified': _validate_kimi_final, 'kimi_raw_source_safely_released': _validate_kimi_release, 'official_glm52_immutable_revision_sealed': _validate_revision, 'bf16_source_manifest_complete': _validate_manifest, 'exact_logical_weight_ledger_sealed': _validate_weight_ledger, 'gravity_pre_audit_complete': _validate_pre_audit, 'external_baseline_matrix_complete': _validate_external_matrix, 'adapter_twin_green': _validate_adapter, 'corpus_integrity_green': _validate_corpus}
...
def _derive(reader: _Reader, stop_condition: str) -> dict[str, Any]:
    ...
    return {'schema': PROOF_SCHEMA, 'status': 'PASS' if result.proven else 'BLOCKED', 'proven': result.proven, 'blockers': list(result.blockers), 'stop_condition': stop_condition, 'campaign_id': CAMPAIGN_ID, 'source_repo': OFFICIAL_REPO, 'source_revision': OFFICIAL_REVISION, 'validator_source': validator_source_binding(), 'artifact_bindings': {path: reader.artifact_binding(path) for path in sorted(result.artifact_paths)}, 'document_bindings': {path: reader.document_binding(path) for path in sorted(set(result.document_paths))}, 'facts': result.facts, 'scope': result.scope}

def derive_stop_proof(root: str | os.PathLike[str], stop_condition: str) -> dict[str, Any]:
    """Derive one deterministic, non-receipt semantic proof from trusted files."""
    return _derive(_Reader(root), stop_condition)

def derive_all_ready_stop_proofs(root: str | os.PathLike[str]) -> dict[str, dict[str, Any]]:
...
def validate_stop_proof(root: str | os.PathLike[str], stop_condition: str, proof: Mapping[str, Any]) -> dict[str, Any]:
    ...
    expected_fields = {'schema', 'status', 'proven', 'blockers', 'stop_condition', 'campaign_id', 'source_repo', 'source_revision', 'validator_source', 'artifact_bindings', 'document_bindings', 'facts', 'scope'}
    ...
    if type(proven) is not bool or status not in {'PASS', 'BLOCKED'} or (status == 'PASS') is not proven:
        _fail('terminal proof status/proven relation is invalid')
    blockers = proof.get('blockers')
    if not isinstance(blockers, list) or (proven and blockers) or (not proven and (not blockers)):
    ...
    _expect(proof.get('validator_source'), validator_source_binding(), 'terminal proof validator-source bytes')
...
def validate_all_ready_stop_proofs(root: str | os.PathLike[str], proofs: Mapping[str, Mapping[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    return {condition: validate_stop_proof(root, condition, proofs[condition]) for condition in READY_STOP_CONDITIONS}
