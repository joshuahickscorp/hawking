"""Recomposed science module glm52_pack (C-SCI-R1)."""
...
def serialize(artifact: forge.PackedArtifact) -> bytes:
    ...
    header = MAGIC + struct.pack('<HHHHIIIIH?B', int(codes['D']), int(codes['S']), int(codes['sub']), int(codebooks[0].shape[0]), int(codes['rows']), int(codes['cols']), int(codes['nchunk']), int(codes['seed']), int(bits), bool(codes['rotate']), len(codebooks))
    ...
    if len(blob) != billed:
        raise ValueError(f'serialized {len(blob)} bytes but ledger bills {billed}; the BPW claim and the file must agree exactly')
    return blob
...
def deserialize(blob: bytes) -> dict[str, Any]:
    ...
    for _ in range(n_codebooks):
        span = card * sub * 2
        codebooks.append(np.frombuffer(blob[offset:offset + span], dtype=np.float16).astype(np.float32).reshape(card, sub))
        offset += span
    count = rows * nchunk * S
    indices = unpack_indices(blob[offset:], count, bits).reshape(rows * nchunk, S)
    return {'codebooks': codebooks, 'indices': indices.astype(np.int64), 'D': D, 'S': S, 'sub': sub, 'rows': rows, 'cols': cols, 'nchunk': nchunk, 'rotate': bool(rotate), 'seed': seed}
...
LADDER = ({'rung': 'R0', 'dim': 8, 'k': 128, 'nominal_bpw': 0.876}, {'rung': 'R2', 'dim': 16, 'k': 256, 'nominal_bpw': 0.505}, {'rung': 'R4', 'dim': 32, 'k': 256, 'nominal_bpw': 0.261})
PRODUCTION_RUNG = 'R0'
...
def pack_tensor_ladder(weights: np.ndarray, *, ladder=LADDER, seed: int=0, rungs: frozenset[str] | None=None) -> list[dict]:
    ...
    for rung in ladder:
        if rungs is not None and rung['rung'] not in rungs:
            rows.append({'rung': rung['rung'], 'dim': rung['dim'], 'k': rung['k'], 'admitted': False, 'sampled_out': True, 'reason': 'NOT_IN_THIS_TENSOR_LADDER_SAMPLE', 'artifact': None})
            continue
        if not rung_is_admissible(rung, weights.size):
            rows.append({'rung': rung['rung'], 'dim': rung['dim'], 'k': rung['k'], 'admitted': False, 'reason': 'FIXED_COST_CANNOT_AMORTIZE_UNDER_CEILING', 'artifact': None})
            continue
        artifact = forge.pack_product_quant(weights, dim=rung['dim'], subspaces=1, k=rung['k'], seed=seed)
        if artifact.whole_artifact_bpw >= BPW_CEILING:
            rows.append({'rung': rung['rung'], 'dim': rung['dim'], 'k': rung['k'], 'admitted': False, 'bpw': artifact.whole_artifact_bpw, 'reason': 'MEASURED_BPW_AT_OR_OVER_CEILING', 'artifact': None})
            continue
        rows.append({'rung': rung['rung'], 'dim': rung['dim'], 'k': rung['k'], 'admitted': True, 'bpw': artifact.whole_artifact_bpw, 'relative_frobenius_error': round(forge._rel_error(weights, artifact.recon), 6), 'artifact': artifact})
    return rows
...
def _candidate_families() -> list[dict]:
    ...
    """
    return [{'family': 'product_quant', 'dim': 16, 'k': 256}, {'family': 'product_quant', 'dim': 16, 'k': 64}, {'family': 'product_quant', 'dim': 32, 'k': 256}, {'family': 'transform_pq', 'dim': 16, 'k': 256}, {'family': 'transform_pq', 'dim': 32, 'k': 256}, {'family': 'pq_protected_islands', 'dim': 16, 'k': 256, 'budget_frac': 0.03}, {'family': 'pq_doctor', 'dim': 16, 'k': 256, 'doctor_frac': 0.25, 'strategy': 'residual_codebook'}, {'family': 'shared_grammar', 'dim': 16, 'k': 256, 'cluster': 8}]

def _run_candidate(spec: dict, weights: np.ndarray, cluster: list[np.ndarray] | None) -> forge.PackedArtifact | None:
    family = spec['family']
    ...
    if family == 'pq_protected_islands':
        return forge.pack_pq_protected_islands(weights, dim=spec['dim'], subspaces=1, k=spec['k'], seed=0, budget_frac=spec['budget_frac'])
    if family == 'pq_doctor':
        ...
        report = forge.doctor_pq(weights, base, byte_budget=budget, strategy=spec['strategy'])
...
def run_tournament(samples: list[np.ndarray], *, cluster: list[np.ndarray] | None=None, target_bpw: float=0.5) -> dict:
    ...
    for spec in _candidate_families():
        ...
        for index, weights in enumerate(samples):
            try:
                ...
                results.append({**spec, 'status': 'ERROR', 'error': f'{type(exc).__name__}: {exc}'})
                ok = False
                break
            if artifact is None:
                ok = False
            ...
            if recon.ndim == 3:
                ...
                errors.append(float(np.mean([forge._rel_error(reference[i], recon[i]) for i in range(recon.shape[0])])))
            else:
                errors.append(forge._rel_error(weights, recon))
            rates.append(artifact.whole_artifact_bpw)
        if not ok or not errors:
            continue
        results.append({**spec, 'status': 'OK', 'mean_bpw': float(np.mean(rates)), 'mean_relative_frobenius_error': float(np.mean(errors))})
    admissible = [r for r in results if r['status'] == 'OK' and r['mean_bpw'] <= target_bpw + 1e-06]
    winner = min(admissible, key=lambda r: r['mean_relative_frobenius_error']) if admissible else None
    return {'schema': 'hawking.glm52.frozen_pack_program.v1', 'target_bpw': target_bpw, 'samples': len(samples), 'evidence_level': 'F1_WEIGHT_SPACE_PROXY_ONLY', 'not_evidence_of': 'output divergence or capability', 'results': sorted(results, key=lambda r: r.get('mean_relative_frobenius_error', 9000000000.0)), 'winner': winner}

def pack_shard(shard_path: Path, rows: list[dict], out_dir: Path, *, production_rung: str=PRODUCTION_RUNG, seed: int=0, rate_override: dict[str | tuple[int, int], str] | None=None, telemetry: dict[str, Any] | None=None) -> dict:
    ...
    def category(row: dict) -> dict[str, Any]:
        ...
        if name not in category_stats:
            category_stats[name] = {'tensors': 0, 'weights': 0, 'source_bytes': 0, 'artifact_bytes': 0, 'native_tensors': 0, 'packed_tensors': 0, 'stage_seconds': defaultdict(float)}
    ...
    def carry_native(row: dict, raw: bytes, elements: int, reason: str) -> None:
        ...
        descriptor = {'name': row['name'], 'category': row['category'], 'layer': row.get('layer'), 'expert': row.get('expert'), 'shape': row['shape'], 'codec': f'native.{str(row['dtype']).lower()}', 'terminal_state': 'PROTECTED_SOURCE_NATIVE', 'elements': int(elements), 'bpw': len(raw) * 8 / max(1, elements), 'reason': reason}
    ...
    with open(shard_path, 'rb', buffering=0) as source:
        for row in ordered:
            ...
            if row['dtype'] != 'BF16':
                ...
                continue
            started = time.perf_counter()
            weights = probe._bf16_to_f32(np.frombuffer(raw, dtype=np.uint16)).reshape(row['shape']).astype(np.float32)
            ...
            if override == 'native':
                reason = 'PROMETHEUS_COALITION_PROTECTED' if row.get('expert') is not None else 'PROMETHEUS_PROFILE_PROTECTED'
            ...
            if not is_routed or LADDER_SAMPLE_EVERY <= 1:
                ...
                rungs = all_rungs if routed_seen % LADDER_SAMPLE_EVERY == 0 else frozenset({target_rung})
                routed_seen += 1
            ...
            chosen = next((r for r in ladder_rows if r['rung'] == target_rung and r['admitted']), None)
            if chosen is None:
            ...
            descriptor = {'name': row['name'], 'category': row['category'], 'layer': row['layer'], 'expert': row['expert'], 'shape': row['shape'], 'codec': 'gravity-pq', 'terminal_state': 'PACKED_IN_CORE_ARTIFACT', 'elements': int(weights.size), 'rung': chosen['rung'], 'bpw': chosen['bpw'], 'relative_frobenius_error': chosen['relative_frobenius_error'], 'ladder': [{k: v for k, v in r.items() if k != 'artifact'} for r in ladder_rows]}
    ...
    if absent:
        raise PackCoverageError(f'{shard_path.name}: {len(absent)} of {len(ordered)} source tensors would be billed but not written, including {absent[:3]}. Refusing to emit an artifact that authorizes eviction of weights it does not contain.')
    ...
    gravity_format.write_shard(partial_path, payloads, model={'repo': 'zai-org/GLM-5.2', 'revision': 'b4734de4facf877f85769a911abafc5283eab3d9', 'source_shard': shard_path.name}, architecture={'type': 'GlmMoeDsaForCausalLM', 'hidden_layers': 78, 'routed_experts': 256, 'shared_experts': 1, 'hidden_size': 6144}, tokenizer={'kind': 'reference', 'source': 'zai-org/GLM-5.2'}, compression={'codec': 'gravity-pq', 'production_rung': production_rung, 'packed_bpw': sum((len(b) for _, b in compressed)) * 8 / max(1, packed_weights), 'complete_bpw': sum((len(b) for _, b in payloads)) * 8 / max(1, complete_weights), 'whole_shard_bpw': compact_bits / max(1, total_weights), 'native_tensors': len(native), 'native_bytes': sum((len(b) for _, b in native)), 'compressed_tensors': len(compressed), 'fit_kernel': forge.FIT_KERNEL, 'ladder_survey': {'schedule': 'v2_target_rung_always_survey_every_nth_routed_expert', 'target_rung_coverage': 'ALL_TENSORS', 'other_rungs_sampled_every_nth_routed_expert': LADDER_SAMPLE_EVERY, 'non_routed_tensors_fully_surveyed': True, 'tensors_fully_surveyed': surveyed, 'routed_expert_tensors_seen': routed_seen}, 'protected_budget_class': PROTECTED_BUDGET_CLASS, 'evidence_level': 'F0_PHYSICAL_AND_F1_WEIGHT_SPACE_PROXY_ONLY', 'not_evidence_of': 'output divergence, capability, or end-to-end behaviour'}, shard={'source': shard_path.name, 'of': 282}, telemetry=format_telemetry)
    stage_seconds['hash_manifest'] += float(format_telemetry.get('hash_manifest_seconds', 0.0))
    stage_seconds['file_write'] += float(format_telemetry.get('file_write_seconds', 0.0))
    started = time.perf_counter()
    ...
    if telemetry is not None:
        ...
        for name, stats in sorted(category_stats.items()):
            ...
            clean_categories[name] = {**{k: v for k, v in stats.items() if k != 'stage_seconds'}, 'stage_seconds': {key: round(value, 6) for key, value in sorted(stats['stage_seconds'].items())}, 'measured_seconds': round(cat_seconds, 6), 'tensors_per_second': stats['tensors'] / max(total_seconds, 1e-12), 'weights_per_second': stats['weights'] / max(total_seconds, 1e-12)}
        source_bytes = sum((int(row['payload_bytes']) for row in ordered))
        artifact_file_bytes = gravity_path.stat().st_size
        telemetry.update({'schema': 'hawking.glm52.pass3_pack_telemetry.v1', 'shard': shard_path.name, 'timing_side_channel_only': True, 'excluded_from_artifact_and_canonical_receipts': True, 'total_seconds': total_seconds, 'stage_seconds': {key: round(value, 6) for key, value in sorted(stage_seconds.items())}, 'stage_time_percentage': {key: round(value * 100.0 / max(total_seconds, 1e-12), 4) for key, value in sorted(stage_seconds.items())}, 'categories': clean_categories, 'tensors': len(entries), 'weights': total_weights, 'source_bytes': source_bytes, 'artifact_payload_bytes': offset, 'artifact_file_bytes': artifact_file_bytes, 'tensors_per_second': len(entries) / max(total_seconds, 1e-12), 'weights_per_second': total_weights / max(total_seconds, 1e-12), 'source_mb_per_second_end_to_end': source_bytes / 1000000 / max(total_seconds, 1e-12), 'artifact_mb_per_second_end_to_end': artifact_file_bytes / 1000000 / max(total_seconds, 1e-12), 'source_read_mb_per_second': source_bytes / 1000000 / max(stage_seconds['source_read'], 1e-12), 'format': format_telemetry})
    return {'schema': 'hawking.glm52.compact_shard_index.v1', 'shard': shard_path.name, 'gravity': gravity_path.name, 'production_rung': production_rung, 'tensors': len(entries), 'weights': total_weights, 'compact_bytes': offset, 'ladder_tensors_fully_surveyed': surveyed, 'ladder_sample_every_nth_routed_expert': LADDER_SAMPLE_EVERY, 'complete_bpw': sum((len(b) for _, b in payloads)) * 8 / max(1, complete_weights), 'packed_bpw': sum((len(b) for _, b in compressed)) * 8 / max(1, packed_weights), 'native_tensors': len(native), 'compressed_tensors': len(compressed), 'tensor_coverage': 'COMPLETE_EVERY_DECLARED_TENSOR_PHYSICALLY_PRESENT', 'whole_shard_bpw': compact_bits / max(1, total_weights), 'evidence_level': 'F0_PHYSICAL_AND_F1_WEIGHT_SPACE_PROXY_ONLY', 'not_evidence_of': 'output divergence, capability, or end-to-end behaviour'}
...
def selftest() -> int:
    ...
    for dim, k in ((8, 16), (4, 256), (8, 256)):
        ...
        assert abs(on_disk_bpw - artifact.whole_artifact_bpw) < 1e-09, on_disk_bpw
        codes = deserialize(blob)
        ...
        assert gap < 0.002, f'execute drifted after round trip: {gap}'
    ...
    print(json.dumps({'selftest': 'PASS', 'schema': PACK_SCHEMA, 'header_bytes': HEADER_BYTES, 'ladder_rungs': len(LADDER), 'all_rungs_sub_one_bpw': True, 'size_equals_ledger': True, 'executes_from_disk': True}, indent=2))
    return 0
