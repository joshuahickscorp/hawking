"""Source-fetch authority body (C-SCI-R1); CLI remains tools/condense/glm52_source_fetch.py."""
from __future__ import annotations

from lab.operators import glm52_pack as pack
from lab.operators import glm52_shard_probe as probe
...
def _pack_shard(name: str, rows: list[dict]) -> dict:
    ...
    return {'shard': name, 'status': 'PACKED', 'compact_bytes': receipt['compact_bytes'], 'whole_shard_bpw': round(receipt['whole_shard_bpw'], 6), 'tensors': receipt['tensors'], 'pack_seconds': round(time.time() - started, 1), 'at': _now()}
...
def _probe_shard(name: str, rows: list[dict]) -> dict:
    ...
    return {'shard': name, 'status': 'PROBED', 'tensors': result['tensor_count'], 'entropy_bits_per_weight': result['shard_zeroth_order_entropy_bits_per_weight'], 'probe_seconds': round(time.time() - started, 2), 'at': _now()}
EVICTION_PAUSED = os.environ.get('GLM52_EVICTION_PAUSED', '1') == '1'

def _evict(names: list[str], *, verified: set[str], probed: set[str], packed: set[str], protected: set[str], authorized: set[str]) -> list[dict]:
    ...
    freed = []
    for name in names:
        if name in protected or name not in verified or name not in authorized or (name not in probed) or (name not in packed):
            continue
...
def _deferred() -> set[str]:
    ...
    if not DEFERRED.exists():
        return set()
    try:
        return {name for name in _read_json(DEFERRED)['shards'] if (SOURCE_ROOT / name).exists()}
...
def _teacher_authority(candidates: set[str], window: str) -> dict:
    ...
    try:
        ...
        return {'authorized': []}
    still_refused = sorted(set(authority['refused_uncaptured_but_capturable']) | set(authority['refused_incomplete_organs']))
    _write_json(DEFERRED, {'shards': still_refused, 'window': window, 'at': _now()})
    _append_ledger({'event': 'TEACHER_GATE', 'window': window, 'authorized': sorted(authority['authorized']), 'refused_uncaptured_but_capturable': authority['refused_uncaptured_but_capturable'], 'refused_incomplete_organs': authority['refused_incomplete_organs'], 'authorized_with_unrecoverable_organs': authority['authorized_with_unrecoverable_organs'], 'capture_outcome': authority.get('capture_outcome', {}), 'at': _now()})
    return authority
...
def _fetch_one(row: dict, repo: str, revision: str) -> dict:
    ...
    name = row['path']
    started = time.time()
    got_path = Path(hf_hub_download(repo_id=repo, filename=name, revision=revision, local_dir=str(SOURCE_ROOT), token=False))
    ...
    if size != row['logical_bytes']:
        ...
        return {'shard': name, 'status': 'SIZE_MISMATCH', 'expected_bytes': row['logical_bytes'], 'observed_bytes': size, 'quarantined': str(quarantine), 'at': _now()}
    observed = _sha256_file(got_path)
    if observed != row['lfs_sha256']:
        quarantine = got_path.with_suffix(got_path.suffix + '.badhash')
        os.replace(got_path, quarantine)
        return {'shard': name, 'status': 'HASH_MISMATCH', 'expected_sha256': row['lfs_sha256'], 'observed_sha256': observed, 'quarantined': str(quarantine), 'at': _now()}
    return {'shard': name, 'status': 'VERIFIED', 'bytes': size, 'sha256': observed, 'seconds': round(elapsed, 2), 'megabits_per_second': round(size * 8 / elapsed / 1000000.0, 1), 'completed_at': _now()}
...
def run() -> int:
        ...
        for name in list(names):
            ...
            if future is not None:
                try:
                    ...
                    _append_ledger({'shard': name, 'status': 'PROBE_ERROR', 'error': f'{type(exc).__name__}: {exc}', 'at': _now()})
            ...
            if future is not None:
                try:
                    ...
                    _append_ledger({'shard': name, 'status': 'PACK_ERROR', 'error': f'{type(exc).__name__}: {exc}', 'at': _now()})
    ...
    def _harvest_finished() -> None:
        ...
        """
        done = {name for name, future in list(probe_futures.items()) + list(pack_futures.items()) if future.done()}
        if done:
            _drain_probes(done)
    for index, window in enumerate(windows):
        wid = window['window_id']
        resident = _resident(manifest)
        owed = [n for n in window['new_fetch_shards'] if n not in probed or n not in packed] if MODE == 'stream' else [n for n in window['new_fetch_shards'] if n not in verified]
            ...
            while queue or running:
                while queue and len(running) < WORKERS and (not stopped_for_disk):
                    ...
                    if free - nxt['logical_bytes'] < DISK_FLOOR_BYTES:
                        _append_ledger({'event': 'DISK_FLOOR_STOP', 'window': wid, 'free_bytes': free, 'floor_bytes': DISK_FLOOR_BYTES, 'at': _now()})
                        stopped_for_disk = True
                        break
                    if MODE == 'resident' and resident_bytes + nxt['logical_bytes'] > residency_ceiling:
                        _append_ledger({'event': 'RESIDENCY_CEILING_STOP', 'window': wid, 'resident_bytes': resident_bytes, 'ceiling_bytes': residency_ceiling, 'at': _now()})
                        stopped_for_disk = True
                        break
                    queue.pop(0)
                ...
                for future in as_completed(list(running)):
                    ...
                    try:
                        ...
                        failed += 1
                        result = {'shard': row['path'], 'status': 'ERROR', 'window': wid, 'error': f'{type(exc).__name__}: {exc}', 'at': _now()}
                    else:
                        result['window'] = wid
                        if result['status'] == 'VERIFIED':
                            verified.add(result['shard'])
                            bytes_this_run += result['bytes']
                            _submit_probe(result['shard'])
                        else:
                            failed += 1
                    _append_ledger(result)
                    elapsed = max(time.time() - wall0, 1e-06)
                    _write_json(PROGRESS, {'state': 'RUNNING', 'mode': MODE, 'window': wid, 'window_index': index, 'window_count': len(windows), 'repo': repo, 'revision': revision, 'shards_verified_total': len(verified), 'shards_probed_total': len(probed_shards()), 'shards_packed_total': len(packed_shards()), 'total_source_shards': 282, 'source_fraction_verified': round(sum((by_path[n]['logical_bytes'] for n in verified)) / total_weight, 4), 'resident_bytes': sum((by_path[n]['logical_bytes'] for n in _resident(manifest))), 'evicted_bytes_this_run': evicted_bytes, 'failed_this_run': failed, 'aggregate_megabits_per_second': round(bytes_this_run * 8 / elapsed / 1000000.0, 1), 'free_disk_bytes': _free_bytes(), 'updated_at': _now()})
                    break
            if stopped_for_disk and MODE == 'resident':
                break
        if MODE == 'stream':
            candidates = set(window.get('evict_after_seal_shards', [])) | _deferred()
            _drain_probes(candidates)
            _harvest_finished()
            authority = _teacher_authority(candidates, wid)
            freed = _evict(sorted(candidates), verified=verified, probed=probed, packed=packed, protected=_protected_after(schedule, index), authorized=set(authority['authorized']))
            if freed:
                evicted_bytes += sum((f['bytes'] for f in freed))
                _append_ledger({'event': 'EVICT', 'window': wid, 'basis': 'VERIFIED_PROBED_PACKED_TEACHER_CAPTURED', 'note': 'hash-verified against the sealed manifest, probed, packed, and every still-capturable layer it carries is sealed in a teacher capsule; layers already destroyed under the pre-capture policy are listed as unrecoverable, not captured', 'unrecoverable_organs': authority.get('authorized_with_unrecoverable_organs', {}), 'shards': [f['shard'] for f in freed], 'freed_bytes': sum((f['bytes'] for f in freed)), 'at': _now()})
    _drain_probes(set(probe_futures) | set(pack_futures))
    probe_pool.shutdown(wait=True)
    pack_pool.shutdown(wait=True)
    elapsed = max(time.time() - wall0, 1e-06)
    _write_json(PROGRESS, {'state': 'ALL_SHARDS_VERIFIED' if len(verified) == 282 and (not failed) else 'COMPLETED_WITH_FAILURES' if failed else 'STOPPED_EARLY', 'mode': MODE, 'repo': repo, 'revision': revision, 'shards_verified_total': len(verified), 'shards_probed_total': len(probed_shards()), 'total_source_shards': 282, 'source_fraction_verified': round(sum((by_path[n]['logical_bytes'] for n in verified)) / total_weight, 4), 'resident_bytes': sum((by_path[n]['logical_bytes'] for n in _resident(manifest))), 'evicted_bytes_this_run': evicted_bytes, 'failed_this_run': failed, 'aggregate_megabits_per_second': round(bytes_this_run * 8 / elapsed / 1000000.0, 1), 'free_disk_bytes': _free_bytes(), 'finished_at': _now()})
    try:
        rollup()
    except Exception as exc:
        _append_ledger({'event': 'ROLLUP_ERROR', 'error': f'{type(exc).__name__}: {exc}', 'at': _now()})
    return 0

def status() -> int:
    print(json.dumps(_read_json(PROGRESS) if PROGRESS.exists() else {'state': 'NOT_STARTED'}, indent=2, sort_keys=True))
    return 0
...
def rollup() -> int:
    ...
    def _bucket() -> dict:
        return {'tensors': 0, 'elements': 0, 'entropy_weighted': 0.0, 'entropy_elements': 0, 'absmax': 0.0, 'max_exponent_span': 0}
    ...
    for path in files:
        ...
        shards.append({'shard': probe['shard'], 'tensors': probe['tensor_count'], 'elements': probe['elements'], 'entropy_bits_per_weight': probe['shard_zeroth_order_entropy_bits_per_weight']})
        for tensor in probe['tensors']:
            for group, key in ((by_category, tensor['category']), (by_budget_class, tensor['budget_class']), (by_section, tensor['section'])):
                ...
                for target in (bucket, total) if group is by_category else (bucket,):
                    ...
                    target['max_exponent_span'] = max(target['max_exponent_span'], tensor['exponent_span_log2'])
                    if tensor.get('zeroth_order_entropy_bits') is not None:
                        target['entropy_weighted'] += tensor['zeroth_order_entropy_bits'] * tensor['elements']
                        target['entropy_elements'] += tensor['elements']

    def _finish(bucket: dict) -> dict:
        entropy = bucket['entropy_weighted'] / bucket['entropy_elements'] if bucket['entropy_elements'] else None
        return {'tensors': bucket['tensors'], 'elements': bucket['elements'], 'zeroth_order_entropy_bits_per_weight': round(entropy, 6) if entropy else None, 'absmax': bucket['absmax'], 'max_exponent_span_log2': bucket['max_exponent_span']}
    atlas = {'schema': 'hawking.glm52.source_weight_atlas.v1', 'note': "Zeroth-order entropy is a context-free coder's rate on the BF16 values as stored. It is an empirical reference point, NOT an achievability bound and NOT a capability result: it models no weight structure and no error tolerance, so a real representation may legally land either side of it.", 'shards_probed': len(files), 'total_source_shards': 282, 'generated_at': _now(), 'total': _finish(total), 'by_category': {k: _finish(v) for k, v in sorted(by_category.items())}, 'by_budget_class': {k: _finish(v) for k, v in sorted(by_budget_class.items())}, 'by_section': {k: _finish(v) for k, v in sorted(by_section.items())}, 'shards': shards}
...
TELEGRAM_SERVICES = ('com.hawking.glm52.gravity.telegram.bot-token', 'com.hawking.glm52.gravity.telegram.chat-id')
...
def _host_health() -> dict:
    ...
    try:
        ...
        match = re.search('used\\s*=\\s*([0-9]+(?:\\.[0-9]+)?)\\s*([BKMGTPE])', swap_text, re.I)
        if match is None:
    ...
    return {'free_disk_bytes': usage.free, 'disk_floor_bytes': DISK_FLOOR_BYTES, 'disk_above_floor': usage.free > DISK_FLOOR_BYTES, 'physical_memory_bytes': int(_run(['/usr/sbin/sysctl', '-n', 'hw.memsize']) or 0), 'swap_used_bytes': swap_used, 'swap_raw': swap_text.strip(), 'thermal_green': 'No thermal warning level has been recorded' in thermal}
...
def _controller_environment(pid: int) -> dict[str, str]:
    ...
    return dict((token.split('=', 1) for token in text.split() if token.startswith('GLM52_') and '=' in token))

def _controller() -> dict:
    pids = [int(value) for value in _run(['/usr/bin/pgrep', '-f', 'glm52_source_fetch.py run']).split()]
    ...
    if live:
        try:
            ...
            pgid = None
    beats = [path.stat().st_mtime for path in (PROGRESS, LEDGER, COMPACT, PROBES) if path.exists()]
    heartbeat = round(time.time() - max(beats), 1) if beats else None
    environment = _controller_environment(live[0]) if live else {}
    return {'pids': live, 'pgid': pgid, 'launchd_label': 'com.hawking.glm52.source-fetch', 'launchd_loaded': bool(label.strip()), 'caffeinate_active': caffeinate, 'lease_path': str(LOCK), 'lease_held': bool(live), 'heartbeat_age_seconds': heartbeat, 'environment': environment, 'eviction_paused': environment.get('GLM52_EVICTION_PAUSED', '1') == '1' if live else None}
...
def safe_to_leave() -> int:
    ...
    faults = [row for row in rows if str(row.get('status', '')).endswith('ERROR') or str(row.get('event', '')).endswith('ERROR') or row.get('status') in {'HASH_MISMATCH', 'SIZE_MISMATCH'}]
    ...
    telegram = 'OPTIONAL_CONFIGURED' if all((_run(['/usr/bin/security', 'find-generic-password', '-s', service]).strip() for service in TELEGRAM_SERVICES)) else 'OPTIONAL_NOT_CONFIGURED'
    blockers = []
    ...
    if any((row.get('event') == 'TEACHER_CAPTURE_FAILED' for row in teacher.recent_ledger(limit=20))):
    ...
    status_obj = {'schema': 'hawking.glm52.safe_to_leave.v1', 'endpoint': 'SAFE_TO_LEAVE' if not blockers else 'NOT_SAFE_TO_LEAVE', 'blockers': blockers, 'controller': controller, 'source': {'state': progress.get('state'), 'current_window': progress.get('window'), 'shards_verified': progress.get('shards_verified_total'), 'shards_probed': progress.get('shards_probed_total'), 'shards_packed': progress.get('shards_packed_total'), 'total_source_shards': 282, 'resident_shards': len(resident), 'resident_bytes': sum((by_path[n]['logical_bytes'] for n in resident)), 'fraction_verified': progress.get('source_fraction_verified')}, 'teacher': {'capsule_count': teacher_status['capsule_count'], 'captured_layers': teacher_status['captured_layers'], 'capsule_bytes_total': teacher_status['capsule_bytes_total'], 'latest_capsule_seal': teacher_status['latest_capsule_seal'], 'ledger_rows': ledger_rows, 'ledger_path': teacher_status['ledger_path'], 'uncaptured_resident_layers': teacher_status['capturable_now'], 'policy_sealed': teacher_status['policy_sealed']}, 'eviction': {'paused': controller['eviction_paused'], 'gate': 'VERIFIED + PROBED + PACKED + TEACHER_CAPTURED + NOT_CARRIED_FORWARD', 'events': len(evictions), 'last_eviction_at': evictions[-1]['at'] if evictions else None, 'last_eviction_basis': evictions[-1].get('basis') if evictions else None, 'last_eviction_shards': evictions[-1].get('shards', []) if evictions else []}, 'compact': {'format': '.gravity', 'shards': len(packed), 'bytes': sum((path.stat().st_size for path in packed)), 'mean_whole_shard_bpw': round(sum((row['whole_shard_bpw'] for row in pack_rows)) / len(pack_rows), 6) if pack_rows else None, 'root': str(COMPACT)}, 'host': health, 'telegram': telegram, 'current_experiment': f'source traversal {progress.get('window')} with teacher capture armed', 'next_experiment': 'BASELINE_A block-level trajectory verdict (REACHABLE / REPAIRABLE / DEAD / INVALID)', 'last_fault': faults[-1] if faults else None, 'faults_total': len(faults), 'updated_at': _now()}
    _write_json(SAFE_TO_LEAVE_JSON, status_obj)
    lines = ['# GLM-5.2 safe-to-leave status', '', f'- endpoint: **{status_obj['endpoint']}**', f'- blockers: {(', '.join(blockers) if blockers else 'none')}', f'- controller pids: {controller['pids']} pgid {controller['pgid']} launchd {('loaded' if controller['launchd_loaded'] else 'ABSENT')} caffeinate {('active' if controller['caffeinate_active'] else 'ABSENT')}', f'- lease: {controller['lease_path']} held={controller['lease_held']} heartbeat age {controller['heartbeat_age_seconds']}s', f'- source: {status_obj['source']['shards_verified']}/282 verified, {status_obj['source']['resident_shards']} resident, window {status_obj['source']['current_window']}, state {status_obj['source']['state']}', f'- teacher: {teacher_status['capsule_count']} capsules, layers {teacher_status['captured_layers']}, ledger {ledger_rows} rows, latest seal {teacher_status['latest_capsule_seal']}', f'- eviction: paused={controller['eviction_paused']}, {len(evictions)} events, last {status_obj['eviction']['last_eviction_at']}', f'- compact: {status_obj['compact']['shards']} .gravity shards, {status_obj['compact']['bytes']} bytes, mean BPW {status_obj['compact']['mean_whole_shard_bpw']}', f'- disk: {health['free_disk_bytes']} free against a {health['disk_floor_bytes']} floor', f'- swap: {health['swap_raw']} | thermal green: {health['thermal_green']}', f'- telegram: {telegram}', f'- next experiment: {status_obj['next_experiment']}', f'- updated: {status_obj['updated_at']}', '']
...
def reconcile() -> int:
    ...
    receipted |= {row['shard'] for row in _ledger_rows() if row.get('event') == 'PACK_RECONCILED'}
    orphans = sorted((path for path in COMPACT.glob('*.gravity') if path.stem + '.safetensors' not in receipted))
    results = []
    for path in orphans:
        ...
        try:
            ...
            row = {'event': 'PACK_RECONCILED', 'shard': shard, 'gravity': path.name, 'status': 'PACKED' if report['ok'] else 'PACK_CORRUPT', 'file_bytes': path.stat().st_size, 'body_bytes': report['body_bytes'], 'tensors': report['tensors'], 'body_sha256_ok': report['body_sha256_ok'], 'bad_tensors': report['bad_tensors'], 'rate_self_consistent': report['rate_self_consistent'], 'whole_shard_bpw': header['compression'].get('whole_shard_bpw'), 'note': "receipt recovered from the artifact's own header after its pack future went uncollected", 'at': _now()}
        except Exception as exc:
            row = {'event': 'PACK_RECONCILED', 'shard': shard, 'gravity': path.name, 'status': 'PACK_UNREADABLE', 'error': f'{type(exc).__name__}: {exc}', 'at': _now()}
        _append_ledger(row)
        results.append(row)
    print(json.dumps({'orphans': len(orphans), 'reconciled': sum((1 for r in results if r['status'] == 'PACKED')), 'corrupt': [r['shard'] for r in results if r['status'] != 'PACKED'], 'shards': [r['shard'] for r in results]}, indent=2, sort_keys=True))
    return 0
...
def selftest() -> int:
    ...
    print(json.dumps({'selftest': 'PASS', 'shards': len(by_path), 'windows': len(windows), 'peak_resident_gb': round(peak / 1000000000.0, 1), 'residency_ceiling_gb': round(ceiling / 1000000000.0, 1), 'full_traversal_fits': True, 'source_tb': round(sum((by_path[n]['logical_bytes'] for n in by_path)) / 1000000000000.0, 3)}, indent=2))
    return 0
