"""Recomposed science module glm52_reference (C-SCI-R1)."""
...
def linear(x: np.ndarray, weight: np.ndarray) -> np.ndarray:
    ...
    if weight.ndim != 2 or x.shape[-1] != weight.shape[1]:
        raise Glm52ReferenceError(f'linear shape mismatch: x={x.shape}, weight={weight.shape}')
...
def layernorm(x: np.ndarray, weight: np.ndarray, bias: np.ndarray, eps: float) -> np.ndarray:
...
def rope_cos_sin(position_ids: np.ndarray, rotary_dim: int, rope_theta: float) -> tuple[np.ndarray, np.ndarray]:
...
def apply_interleaved_rope(q: np.ndarray, k: np.ndarray, cos: np.ndarray, sin: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    def rotate(value: np.ndarray) -> np.ndarray:
        ...
        return np.concatenate([first * half_cos - second * half_sin, second * half_cos + first * half_sin], axis=-1)
...
def indexer_topk(hidden_states: np.ndarray, q_resid: np.ndarray, *, wq_b: np.ndarray, wk: np.ndarray, k_norm_weight: np.ndarray, k_norm_bias: np.ndarray, weights_proj: np.ndarray, position_ids: np.ndarray, cos: np.ndarray, sin: np.ndarray, n_heads: int, head_dim: int, rotary_dim: int, topk: int, attention_mask: np.ndarray | None=None, past_keys: np.ndarray | None=None) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    k = layernorm(linear(hidden_states, wk), k_norm_weight, k_norm_bias, 1e-06)
    ...
    q_rot_bhsd, k_rot_bhsd = apply_interleaved_rope(q_rot_bhsd, k_rot_bhsd, cos, sin)
    ...
    scores = np.matmul(q.astype(np.float32, copy=False), all_k.transpose(0, 2, 1)[:, None].astype(np.float32, copy=False))
    ...
    if attention_mask is not None:
        ...
        if mask.shape != index_scores.shape:
            raise Glm52ReferenceError(f'indexer mask {mask.shape} != score shape {index_scores.shape}')
...
def router_topk(hidden_states: np.ndarray, weight: np.ndarray, correction_bias: np.ndarray, *, top_k: int, num_groups: int, topk_groups: int, normalize: bool, scaling_factor: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    return (logits.reshape(*original, experts), weights.reshape(*original, top_k), indices.reshape(*original, top_k))

def dense_mlp(hidden_states: np.ndarray, source: Mapping[str, np.ndarray] | TensorSource, prefix: str) -> np.ndarray:
...
def routed_moe(hidden_states: np.ndarray, source: Mapping[str, np.ndarray] | TensorSource, prefix: str, config: Mapping[str, Any], *, retain_per_expert: bool=False) -> tuple[np.ndarray, dict[str, np.ndarray]]:
    ...
    """
    router_logits, topk_weights, topk_indices = router_topk(hidden_states, _tensor(source, f'{prefix}.gate.weight'), _tensor(source, f'{prefix}.gate.e_score_correction_bias'), top_k=int(config['num_experts_per_tok']), num_groups=int(config['n_group']), topk_groups=int(config['topk_group']), normalize=bool(config['norm_topk_prob']), scaling_factor=float(config['routed_scaling_factor']))
    ...
    trace = {'router_logits': router_logits, 'topk_weights': topk_weights, 'topk_indices': topk_indices, 'routed_output': routed.reshape(hidden_states.shape), 'shared_output': shared}
    if retain_per_expert:
        trace['per_expert'] = per_expert
    return (output, trace)

def _append_cache(mapping: dict[int, np.ndarray], layer: int, current: np.ndarray, sequence_axis: int) -> np.ndarray:
...
def attention_forward(hidden_states: np.ndarray, source: Mapping[str, np.ndarray] | TensorSource, layer: int, config: Mapping[str, Any], position_ids: np.ndarray, cache: ReferenceCache, *, indexer_type: str, previous_topk: np.ndarray | None, additive_mask: np.ndarray | None=None) -> tuple[np.ndarray, np.ndarray, dict[str, np.ndarray]]:
    ...
    k_latent = rmsnorm(k_latent, _tensor(source, f'{prefix}.kv_a_layernorm.weight'), float(config['rms_norm_eps']))
    ...
    cos, sin = rope_cos_sin(position_ids, rope, float(config['rope_parameters']['rope_theta']))
    ...
    if indexer_type == 'full':
        ...
        topk, index_scores, index_keys = indexer_topk(hidden_states, q_resid, wq_b=_tensor(source, f'{idx_prefix}.wq_b.weight'), wk=_tensor(source, f'{idx_prefix}.wk.weight'), k_norm_weight=_tensor(source, f'{idx_prefix}.k_norm.weight'), k_norm_bias=_tensor(source, f'{idx_prefix}.k_norm.bias'), weights_proj=_tensor(source, f'{idx_prefix}.weights_proj.weight'), position_ids=position_ids, cos=cos, sin=sin, n_heads=int(config['index_n_heads']), head_dim=int(config['index_head_dim']), rotary_dim=rope, topk=int(config['index_topk']), attention_mask=index_mask, past_keys=cache.indexer_keys.get(layer))
    ...
    return (output, topk, {'q_resid': q_resid, 'queries': queries, 'keys': keys, 'values': values_all, 'index_scores': index_scores, 'topk_indices': topk, 'attention_probabilities': probabilities, 'attention_output': output})

def decoder_layer(hidden_states: np.ndarray, source: Mapping[str, np.ndarray] | TensorSource, layer: int, config: Mapping[str, Any], position_ids: np.ndarray, cache: ReferenceCache, *, mlp_type: str, indexer_type: str, previous_topk: np.ndarray | None, additive_mask: np.ndarray | None=None, retain_per_expert: bool=False) -> tuple[np.ndarray, np.ndarray, dict[str, Any]]:
    prefix = f'model.layers.{layer}'
    before = np.asarray(hidden_states, dtype=np.float32)
    attention_input = rmsnorm(before, _tensor(source, f'{prefix}.input_layernorm.weight'), float(config['rms_norm_eps']))
    attention_output, topk, attention_trace = attention_forward(attention_input, source, layer, config, position_ids, cache, indexer_type=indexer_type, previous_topk=previous_topk, additive_mask=additive_mask)
    post_attention = before + attention_output
    mlp_input = rmsnorm(post_attention, _tensor(source, f'{prefix}.post_attention_layernorm.weight'), float(config['rms_norm_eps']))
    if mlp_type == 'dense':
        mlp_output = dense_mlp(mlp_input, source, f'{prefix}.mlp')
        mlp_trace: dict[str, Any] = {'kind': 'dense'}
    elif mlp_type == 'sparse':
        mlp_output, mlp_trace = routed_moe(mlp_input, source, f'{prefix}.mlp', config, retain_per_expert=retain_per_expert)
        mlp_trace = {'kind': 'sparse', **mlp_trace}
    else:
        raise Glm52ReferenceError(f'unknown MLP type {mlp_type!r}')
    output = post_attention + mlp_output
    return (output, topk, {'layer': layer, 'input': before, 'attention_input': attention_input, 'attention': attention_trace, 'post_attention': post_attention, 'mlp_input': mlp_input, 'mlp': mlp_trace, 'output': output})

def _embedding_rows(source: Mapping[str, np.ndarray] | TensorSource, input_ids: np.ndarray) -> np.ndarray:
...
def main_forward(input_ids: np.ndarray, source: Mapping[str, np.ndarray] | TensorSource, config: Mapping[str, Any], *, cache: ReferenceCache | None=None, position_ids: np.ndarray | None=None, additive_mask: np.ndarray | None=None) -> tuple[np.ndarray, ReferenceCache, dict[str, Any]]:
    ...
    if position_ids is None:
        ...
        position_ids = np.broadcast_to(np.arange(start, start + ids.shape[1], dtype=np.int64)[None, :], ids.shape).copy()
    ...
    for layer in range(main_layers):
        hidden, previous_topk, trace = decoder_layer(hidden, source, layer, config, position_ids, cache, mlp_type=mlp_types[layer], indexer_type=indexer_types[layer], previous_topk=previous_topk, additive_mask=additive_mask)
        layers.append(trace)
    pre_final = hidden
    final = rmsnorm(pre_final, _tensor(source, 'model.norm.weight'), float(config['rms_norm_eps']))
    logits = _linear(source, final, 'lm_head.weight')
    return (logits, cache, {'position_ids': np.asarray(position_ids), 'layers': layers, 'pre_final_hidden': pre_final, 'final_hidden': final, 'logits': logits, 'final_main_topk': previous_topk})

def mtp_forward(previous_main_hidden: np.ndarray, shifted_input_embeddings: np.ndarray, source: Mapping[str, np.ndarray] | TensorSource, config: Mapping[str, Any], position_ids: np.ndarray, *, cache: ReferenceCache | None=None, speculative_step: int=0, additive_mask: np.ndarray | None=None) -> tuple[np.ndarray, ReferenceCache, dict[str, Any]]:
    ...
    output, topk, layer_trace = decoder_layer(fused, source, layer, config, positions, cache, mlp_type='sparse', indexer_type='shared' if reuse else 'full', previous_topk=cache.mtp_iteration_topk if reuse else None, additive_mask=additive_mask)
    if not reuse:
        cache.mtp_iteration_topk = topk.copy()
    final = rmsnorm(output, _tensor(source, f'{prefix}.shared_head.norm.weight'), float(config['rms_norm_eps']))
    logits = _linear(source, final, 'lm_head.weight')
    return (logits, cache, {'speculative_step': speculative_step, 'reused_step_zero_topk': reuse, 'masked_shifted_embeddings': shifted, 'enorm': enorm, 'hnorm': hnorm, 'fused_input': fused, 'layer': layer_trace, 'pre_shared_head': output, 'shared_head_hidden': final, 'logits': logits, 'topk_indices': topk})

def selfcheck() -> dict[str, Any]:
    positions = np.array([[0, 1]], dtype=np.int64)
    cos, sin = rope_cos_sin(positions, 4, 10000.0)
    ...
    logits, weights, indices = router_topk(np.ones((1, 1, 2), dtype=np.float32), np.array([[1, 0], [0, 1], [-1, 0], [0, -1]], dtype=np.float32), np.array([0, 0.1, 0, 0], dtype=np.float32), top_k=2, num_groups=1, topk_groups=1, normalize=True, scaling_factor=2.5)
    if not np.allclose(weights.sum(axis=-1), 2.5):
        raise AssertionError('router scaling selfcheck failed')
    return {'status': 'PASS', 'rope_position_zero_layout': rotated[0, 0, 0].tolist(), 'router_indices': indices.tolist(), 'router_logits_shape': list(logits.shape), 'router_weight_sum': float(weights.sum())}
