"""Recomposed science module glm52_telegram (C-SCI-R1)."""
...
from lab.operators.glm52_state import TelegramAuthConfig
from lab.operators.glm52_state import make_telegram_delivery_receipt
from lab.operators.glm52_state import telegram_chat_identity_digest
from lab.operators.glm52_state import validate_telegram_delivery_receipt as _state_validate_telegram_delivery_receipt
from lab.operators.glm52_state import validate_transition_intent as _state_validate_transition_intent
import argparse
import base64
import fcntl
import getpass
...
TOKEN_RE = re.compile('^[0-9]{6,16}:[A-Za-z0-9_-]{20,}$')
SAFE_NAME_RE = re.compile('^[A-Za-z0-9][A-Za-z0-9_.:/@+\\-]{0,127}$')
CLAIM_RE = re.compile('^[A-Za-z0-9][A-Za-z0-9._:/@+\\-]{7,255}$')
RATE_RE = re.compile('^[0-9]+(?:\\.[0-9]+)?$')
...
NORMAL_DELIVERY_LIFECYCLE = ('OUTBOX_PREPARED', 'SEND_STARTED', 'DELIVERY_RECEIPT')
LEDGER_ENTRY_KINDS = frozenset({*NORMAL_DELIVERY_LIFECYCLE, 'AMBIGUOUS_BLOCKED', 'DUPLICATE_RETRY_AUTHORIZED'})
...
STATUS_KEYS = frozenset({'state', 'source_coverage_percent', 'shards', 'network_bytes', 'throughput_bytes_per_second', 'eta_seconds', 'current', 'candidate_rates', 'best_metrics', 'resources', 'process'})
...
_DELIVERY_BINDING_KEYS = frozenset({'schema', 'transition_intent', 'transition_intent_sha256', 'event_kind', 'claim_id', 'from_state', 'to_state', 'dedupe_key', 'canonical_status', 'canonical_status_sha256', 'rendered_message', 'rendered_message_sha256', 'rendered_message_utf8_bytes', 'controller_anchor', 'controller_anchor_sha256'})
_LEDGER_ENTRY_KEYS = frozenset({'schema', 'seq', 'kind', 'recorded_at', 'dedupe_key', 'binding', 'receipt', 'reconciliation', 'prev_chain_sha256', 'chain_sha256', 'hmac_sha256'})
_LEDGER_HEAD_KEYS = frozenset({'schema', 'entry_count', 'head_chain_sha256', 'head_entry_hmac_sha256', 'hmac_sha256'})
_RECONCILIATION_AUTH_KEYS = frozenset({'schema', 'status', 'action', 'authorization_claim_id', 'transition_intent_sha256', 'dedupe_key', 'canonical_status_sha256', 'rendered_message_sha256', 'controller_anchor_sha256', 'ambiguous_entry_seq', 'ambiguous_entry_chain_sha256', 'send_started_seq', 'send_started_chain_sha256', 'attempt', 'reason', 'authorized_at', 'seal_sha256', 'hmac_sha256'})
_AMBIGUITY_BLOCK_KEYS = frozenset({'schema', 'status', 'attempt', 'send_started_seq', 'send_started_chain_sha256', 'reason'})
_LIFECYCLE_NEXT_KINDS: dict[str | None, frozenset[str]] = {None: frozenset({'OUTBOX_PREPARED'}), 'OUTBOX_PREPARED': frozenset({'SEND_STARTED'}), 'SEND_STARTED': frozenset({'AMBIGUOUS_BLOCKED', 'DELIVERY_RECEIPT'}), 'AMBIGUOUS_BLOCKED': frozenset({'DUPLICATE_RETRY_AUTHORIZED', 'DELIVERY_RECEIPT'}), 'DUPLICATE_RETRY_AUTHORIZED': frozenset({'SEND_STARTED'}), 'DELIVERY_RECEIPT': frozenset()}
...
class TelegramTransport(Protocol):
    ...
    def call(self, token: str, method: str, payload: Mapping[str, Any]) -> TelegramHTTPResponse:
...
class MacOSKeychain:
    ...
    def __init__(self, *, native_reader: Callable[[str, str], str | None] | None=None, native_writer: Callable[[str, str, str], None] | None=None) -> None:
...
class UrllibTelegramTransport:
    ...
    def __init__(self, *, opener: Callable[..., Any]=urllib.request.urlopen, timeout_seconds: int=20) -> None:
        self._opener = opener
    ...
    def call(self, token: str, method: str, payload: Mapping[str, Any]) -> TelegramHTTPResponse:
        ...
        request = urllib.request.Request(f'https://api.telegram.org/bot{token}/{method}', data=encoded, headers={'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'hawking-glm52-gravity/1'}, method='POST')
        try:
...
def _secret_digest(kind: str, value: str | bytes) -> str:
    ...
    return hashlib.sha256(canonical({'schema': 'hawking.glm52.telegram_secret_identity.v1', 'kind': kind, 'secret_sha256': hashlib.sha256(raw).hexdigest()})).hexdigest()
...
def _notification_bot_identity_digest(token: str) -> str:
    ...
    if separator != ':' or not bot_id.isascii() or (not bot_id.isdigit()):
        raise TelegramSecurityError('Telegram bot credential shape is invalid')
    return hashlib.sha256(b'hawking.glm52.telegram-bot-identity.v1\x00' + bot_id.encode('ascii')).hexdigest()

def _bot_identity_digest(identity: Mapping[str, Any]) -> str:
    bot_id = identity.get('id')
    username = identity.get('username')
    if isinstance(bot_id, bool) or not isinstance(bot_id, int) or bot_id <= 0 or (identity.get('is_bot') is not True) or (not isinstance(username, str)) or (not username):
        raise TelegramSecurityError('Telegram getMe identity is invalid')
    return hashlib.sha256(canonical({'schema': 'hawking.glm52.telegram_bot_identity.v1', 'bot_id': bot_id, 'username': username})).hexdigest()
...
def _transport_call(transport: TelegramTransport, token: str, method: str, payload: Mapping[str, Any]) -> TelegramHTTPResponse:
    if method not in ALLOWED_METHODS:
        raise TelegramSecurityError('unrecognized Telegram Bot API method')
    try:
...
def configure_token(keychain: Keychain, transport: TelegramTransport, *, hidden_prompt: Callable[[str], str]=getpass.getpass) -> dict[str, bool | str]:
    ...
    return {'token_configured': True, 'bot_identity_digest': identity_digest}

def discover_private_chat(keychain: Keychain, transport: TelegramTransport) -> dict[str, bool | str]:
    ...
    response = _transport_call(transport, token, 'getUpdates', {'timeout': 0, 'allowed_updates': '["message"]'})
    ...
    for update in updates:
        ...
        if isinstance(chat_id, bool) or not isinstance(chat_id, int) or chat_id <= 0 or (sender_id != chat_id):
            continue
        chat_ids.add(str(chat_id))
    if not chat_ids:
        raise TelegramSecurityError('no safe private chat was found; send this bot one direct message and retry')
    if len(chat_ids) != 1:
    ...
    return {'private_chat_configured': True, 'chat_identity_digest': telegram_chat_identity_digest(chat_id)}

def configure_hmac_key(keychain: Keychain, *, random_bytes: Callable[[int], bytes]=secrets.token_bytes) -> dict[str, bool | str]:
    ...
    if existing is not None:
        ...
        return {'hmac_key_configured': True, 'hmac_key_identity_digest': _secret_digest('receipt_hmac_key', decoded)}
    try:
    ...
    return {'hmac_key_configured': True, 'hmac_key_identity_digest': _secret_digest('receipt_hmac_key', generated)}
...
def credential_status(keychain: Keychain) -> dict[str, bool | str]:
    ...
    if valid_token is not None:
        ...
        result['notification_bot_identity_digest'] = _notification_bot_identity_digest(valid_token)
...
def load_telegram_auth(keychain: Keychain) -> TelegramAuthConfig:
    ...
    try:
        ...
        return TelegramAuthConfig(hmac_key=hmac_key, expected_chat_identity_digest=telegram_chat_identity_digest(chat_id))
...
def validate_campaign_status(status: Mapping[str, Any]) -> dict[str, Any]:
    ...
    normalized_shards = {key: _nonnegative_integer(shards[key], f'shards.{key}') for key in sorted(SHARD_KEYS)}
    if not normalized_shards['evicted'] <= normalized_shards['verified'] <= normalized_shards['fetched'] <= normalized_shards['total']:
    ...
    if not isinstance(candidate_rates, list) or len(candidate_rates) > 20 or any((not isinstance(rate, str) or RATE_RE.fullmatch(rate) is None for rate in candidate_rates)):
    ...
    if any((not isinstance(key, str) or SAFE_NAME_RE.fullmatch(key) is None for key in metric_names)):
    ...
    normalized_resources = {key: _nonnegative_integer(resources[key], f'resources.{key}') for key in sorted(RESOURCE_KEYS)}
    process = status['process']
    if not isinstance(process, Mapping) or set(process) != PROCESS_KEYS or (not isinstance(process['lease_held'], bool)):
        raise TelegramSecurityError('campaign status process/lease fields are invalid')
    normalized_process = {'pid': _nonnegative_integer(process['pid'], 'process.pid', positive=True), 'lease_held': process['lease_held'], 'lease_owner': _safe_name(process['lease_owner'], 'process.lease_owner')}
    eta = status['eta_seconds']
    if eta is not None:
        eta = _nonnegative_integer(eta, 'eta_seconds')
    return {'state': state, 'source_coverage_percent': coverage, 'shards': normalized_shards, 'network_bytes': _nonnegative_integer(status['network_bytes'], 'network_bytes'), 'throughput_bytes_per_second': _finite_number(status['throughput_bytes_per_second'], 'throughput'), 'eta_seconds': eta, 'current': {'window': window, 'layer': layer}, 'candidate_rates': list(candidate_rates), 'best_metrics': normalized_metrics, 'resources': normalized_resources, 'process': normalized_process}
...
def compose_message(event: str, dedupe_key: str, status: Mapping[str, Any]) -> str:
    ...
    text = '\n'.join(['GLM-5.2 Gravity', f'event={event}', f'dedupe_key={dedupe_key}', f'state={value['state']}', f'source_coverage_percent={value['source_coverage_percent']:.6f}', f'shards fetched={shards['fetched']} verified={shards['verified']} evicted={shards['evicted']} total={shards['total']}', f'network_bytes={value['network_bytes']} throughput_bytes_per_second={value['throughput_bytes_per_second']:.6f} eta_seconds={(value['eta_seconds'] if value['eta_seconds'] is not None else 'UNAVAILABLE')}', f'current_window={(current['window'] if current['window'] is not None else 'UNAVAILABLE')} current_layer={(current['layer'] if current['layer'] is not None else 'UNAVAILABLE')}', f'candidate_rates={rates}', f'best_metrics={metrics}', f'disk_free_bytes={resources['disk_free_bytes']} ram_available_bytes={resources['ram_available_bytes']} swap_used_bytes={resources['swap_used_bytes']}', f'pid={process['pid']} lease_held={str(process['lease_held']).lower()} lease_owner={process['lease_owner']}'])
...
def make_delivery_binding(transition_intent: Mapping[str, Any], auth: TelegramAuthConfig) -> dict[str, Any]:
    ...
    return {'schema': DELIVERY_BINDING_SCHEMA, 'transition_intent': intent, 'transition_intent_sha256': intent['seal_sha256'], 'event_kind': intent['event_kind'], 'claim_id': intent['claim_id'], 'from_state': intent['from_state'], 'to_state': intent['to_state'], 'dedupe_key': intent['dedupe_key'], 'canonical_status': intent['canonical_status'], 'canonical_status_sha256': intent['canonical_status_sha256'], 'rendered_message': rendered, 'rendered_message_sha256': intent['rendered_message_sha256'], 'rendered_message_utf8_bytes': len(rendered.encode('utf-8')), 'controller_anchor': intent['controller_anchor'], 'controller_anchor_sha256': intent['controller_anchor']['anchor_sha256']}
...
def _validate_delivery_binding(value: Any, auth: TelegramAuthConfig) -> dict[str, Any]:
    ...
    if set(binding) != _DELIVERY_BINDING_KEYS or binding.get('schema') != DELIVERY_BINDING_SCHEMA:
...
def make_reconciliation_authorization(transition_intent: Mapping[str, Any], *, ambiguity_entry: Mapping[str, Any], auth: TelegramAuthConfig, action: str, authorization_claim_id: str, reason: str, authorized_at: str | None=None) -> dict[str, Any]:
    ...
    if not isinstance(authorization_claim_id, str) or CLAIM_RE.fullmatch(authorization_claim_id) is None:
        raise TelegramSecurityError('Telegram reconciliation claim id is invalid')
    if not isinstance(reason, str) or not reason or reason != reason.strip() or (len(reason) > 512) or ('\n' in reason) or ('\r' in reason):
    ...
    body = {'schema': RECONCILIATION_AUTH_SCHEMA, 'status': 'AUTHORIZED', 'action': action, 'authorization_claim_id': authorization_claim_id, 'transition_intent_sha256': binding['transition_intent_sha256'], 'dedupe_key': binding['dedupe_key'], 'canonical_status_sha256': binding['canonical_status_sha256'], 'rendered_message_sha256': binding['rendered_message_sha256'], 'controller_anchor_sha256': binding['controller_anchor_sha256'], 'ambiguous_entry_seq': blocked['seq'], 'ambiguous_entry_chain_sha256': blocked['chain_sha256'], 'send_started_seq': block['send_started_seq'], 'send_started_chain_sha256': block['send_started_chain_sha256'], 'attempt': block['attempt'], 'reason': reason, 'authorized_at': timestamp}
    sealed = {**body, 'seal_sha256': hashlib.sha256(canonical(body)).hexdigest()}
    return {**sealed, 'hmac_sha256': auth.authenticate(sealed)}

def _validate_reconciliation_authorization(value: Any, binding: Mapping[str, Any], auth: TelegramAuthConfig, *, ambiguity_entry: Mapping[str, Any]) -> dict[str, Any]:
    authorization = _canonical_object(value, 'Telegram reconciliation authorization', nonempty=True)
    if set(authorization) != _RECONCILIATION_AUTH_KEYS or authorization.get('schema') != RECONCILIATION_AUTH_SCHEMA or authorization.get('status') != 'AUTHORIZED' or (authorization.get('action') != 'AUTHORIZE_DUPLICATE_RETRY'):
        raise TelegramSecurityError('Telegram reconciliation authorization fields are invalid')
    blocked = _validate_standalone_ambiguity_entry(ambiguity_entry, binding, auth)
    block = blocked['reconciliation']
    expected_bindings = {'transition_intent_sha256': binding['transition_intent_sha256'], 'dedupe_key': binding['dedupe_key'], 'canonical_status_sha256': binding['canonical_status_sha256'], 'rendered_message_sha256': binding['rendered_message_sha256'], 'controller_anchor_sha256': binding['controller_anchor_sha256'], 'ambiguous_entry_seq': blocked['seq'], 'ambiguous_entry_chain_sha256': blocked['chain_sha256'], 'send_started_seq': block['send_started_seq'], 'send_started_chain_sha256': block['send_started_chain_sha256'], 'attempt': block['attempt']}
    if any((authorization.get(key) != expected for key, expected in expected_bindings.items())) or isinstance(authorization.get('ambiguous_entry_seq'), bool) or (not isinstance(authorization.get('ambiguous_entry_seq'), int)) or (authorization['ambiguous_entry_seq'] < 0) or isinstance(authorization.get('send_started_seq'), bool) or (not isinstance(authorization.get('send_started_seq'), int)) or (authorization['send_started_seq'] < 0) or isinstance(authorization.get('attempt'), bool) or (not isinstance(authorization.get('attempt'), int)) or (authorization['attempt'] <= 0) or (not _is_sha256(authorization.get('ambiguous_entry_chain_sha256'))) or (not _is_sha256(authorization.get('send_started_chain_sha256'))) or (not isinstance(authorization.get('authorization_claim_id'), str)) or (CLAIM_RE.fullmatch(authorization['authorization_claim_id']) is None) or (not isinstance(authorization.get('reason'), str)) or (not authorization['reason']) or (authorization['reason'] != authorization['reason'].strip()) or (len(authorization['reason']) > 512) or ('\n' in authorization['reason']) or ('\r' in authorization['reason']) or (not isinstance(authorization.get('authorized_at'), str)) or (not authorization['authorized_at']):
        raise TelegramSecurityError('Telegram reconciliation authorization binding is invalid')
    unsigned = {key: item for key, item in authorization.items() if key not in {'seal_sha256', 'hmac_sha256'}}
...
def _ambiguity_block(attempt: int, reason: str, *, send_started_seq: int, send_started_chain_sha256: str) -> dict[str, Any]:
    if isinstance(attempt, bool) or not isinstance(attempt, int) or attempt <= 0 or isinstance(send_started_seq, bool) or (not isinstance(send_started_seq, int)) or (send_started_seq < 0) or (not _is_sha256(send_started_chain_sha256)) or (reason not in {'PROCESS_RECOVERY_FOUND_SEND_STARTED', 'SENDER_FAILED_AFTER_SEND_STARTED'}):
        raise TelegramSecurityError('Telegram ambiguity block evidence is invalid')
    return {'schema': AMBIGUITY_BLOCK_SCHEMA, 'status': 'BLOCKED_AMBIGUOUS_SEND', 'attempt': attempt, 'send_started_seq': send_started_seq, 'send_started_chain_sha256': send_started_chain_sha256, 'reason': reason}
...
def _validate_ambiguity_block(value: Any) -> dict[str, Any]:
    ...
    return _ambiguity_block(block.get('attempt'), block.get('reason'), send_started_seq=block.get('send_started_seq'), send_started_chain_sha256=block.get('send_started_chain_sha256'))

def _validate_standalone_ambiguity_entry(value: Any, binding: Mapping[str, Any], auth: TelegramAuthConfig) -> dict[str, Any]:
    ...
    if set(entry) != _LEDGER_ENTRY_KEYS or entry.get('schema') != DELIVERY_LEDGER_SCHEMA or isinstance(entry.get('seq'), bool) or (not isinstance(entry.get('seq'), int)) or (entry['seq'] < 0) or (entry.get('kind') != 'AMBIGUOUS_BLOCKED') or (not isinstance(entry.get('recorded_at'), str)) or (not entry['recorded_at']) or (entry.get('dedupe_key') != binding['dedupe_key']) or (entry.get('receipt') is not None) or (not _is_sha256(entry.get('prev_chain_sha256'))):
    ...
    unsigned = {key: item for key, item in entry.items() if key not in {'chain_sha256', 'hmac_sha256'}}
    expected_chain = hashlib.sha256(canonical(unsigned)).hexdigest()
    signed = {**unsigned, 'chain_sha256': expected_chain}
    if entry.get('chain_sha256') != expected_chain or not auth.verify(signed, entry.get('hmac_sha256')):
...
def _strict_json_object(raw: bytes, label: str) -> dict[str, Any]:
    ...
    def reject_duplicate_keys(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        return result
    try:
        value = json.loads(raw.decode('utf-8'), parse_constant=reject_constant, object_pairs_hook=reject_duplicate_keys)
...
class TelegramDeliveryLedger:
    ...
    def __init__(self, path: str | os.PathLike[str], *, fault_injector: Callable[[str], None] | None=None, clock: Callable[[], str]=utc_now) -> None:
        self.path = Path(path)
        if not self.path.is_absolute() or self.path.anchor != os.sep or any((part in {'', '.', '..'} for part in self.path.parts[1:])) or (self.path.name in {'', '.', '..'}):
    ...
    @staticmethod
    def _open_regular_leaf(parent_fd: int, name: str, flags: int, mode: int=384) -> int:
        try:
            descriptor = os.open(name, flags | os.O_NOFOLLOW | getattr(os, 'O_CLOEXEC', 0), mode, dir_fd=parent_fd)
    ...
    def _locked_files(self) -> Iterator[tuple[int, int]]:
        ...
        try:
            ...
            lock_fd = self._open_regular_leaf(parent_fd, self._lock_name, os.O_RDWR | os.O_CREAT)
            try:
                fcntl.flock(lock_fd, fcntl.LOCK_EX)
            except OSError:
                raise TelegramSecurityError('Telegram ledger concurrency lock failed') from None
            ledger_fd = self._open_regular_leaf(parent_fd, self.path.name, os.O_RDWR | os.O_APPEND | os.O_CREAT)
            try:
    ...
    def _read_head(self, parent_fd: int, auth: TelegramAuthConfig) -> dict[str, Any] | None:
        ...
        if set(head) != _LEDGER_HEAD_KEYS or head.get('schema') != DELIVERY_LEDGER_HEAD_SCHEMA or isinstance(head.get('entry_count'), bool) or (not isinstance(head.get('entry_count'), int)) or (head['entry_count'] < 0) or (not _is_sha256(head.get('head_chain_sha256'))) or (not _is_sha256(head.get('head_entry_hmac_sha256'))):
    ...
    def _head_value(entries: Sequence[Mapping[str, Any]], auth: TelegramAuthConfig) -> dict[str, Any]:
        ...
        body = {'schema': DELIVERY_LEDGER_HEAD_SCHEMA, 'entry_count': len(entries), 'head_chain_sha256': chain, 'head_entry_hmac_sha256': entry_hmac}
        return {**body, 'hmac_sha256': auth.authenticate(body)}

    def _write_head(self, parent_fd: int, entries: Sequence[Mapping[str, Any]], auth: TelegramAuthConfig) -> None:
        try:
        ...
        descriptor = -1
        try:
            descriptor = self._open_regular_leaf(parent_fd, temporary, os.O_WRONLY | os.O_CREAT | os.O_EXCL)
    ...
    @staticmethod
    def _validate_receipt(receipt: Any, binding: Mapping[str, Any], auth: TelegramAuthConfig) -> dict[str, Any]:
        value = _canonical_object(receipt, 'Telegram signed delivery receipt', nonempty=True)
        try:
            validated = _state_validate_telegram_delivery_receipt(value, binding['transition_intent'], auth)
        except StateError:
            raise TelegramSecurityError('Telegram signed delivery receipt binding/HMAC is invalid') from None
        if validated.get('schema') != TELEGRAM_RECEIPT_SCHEMA or validated.get('status') != 'DELIVERED' or validated.get('response_validated') is not True or (validated.get('bot_api_http_status') != 200):
            raise TelegramSecurityError('Telegram outbox requires a validated successful Bot API receipt')
        return _canonical_object(validated, 'Telegram signed delivery receipt', nonempty=True)

    def _verified_entries(self, parent_fd: int, ledger_fd: int, auth: TelegramAuthConfig) -> list[dict[str, Any]]:
        ...
        for seq, line in enumerate(complete_raw.splitlines()):
            ...
            if set(entry) != _LEDGER_ENTRY_KEYS or entry.get('schema') != DELIVERY_LEDGER_SCHEMA or isinstance(entry.get('seq'), bool) or (entry.get('seq') != seq) or (entry.get('kind') not in LEDGER_ENTRY_KINDS) or (not isinstance(entry.get('recorded_at'), str)) or (not entry['recorded_at']) or (not _is_sha256(entry.get('dedupe_key'))) or (entry.get('prev_chain_sha256') != previous):
            ...
            if prior_binding != encoded_binding:
                raise TelegramSecurityError('Telegram dedupe key is rebound to a different delivery intent')
            ...
            if kind in {'OUTBOX_PREPARED', 'SEND_STARTED'}:
                ...
                started = history[-1]
                expected_block = _ambiguity_block(sum((item['kind'] == 'SEND_STARTED' for item in history)), block['reason'], send_started_seq=started['seq'], send_started_chain_sha256=started['chain_sha256'])
                if canonical(block) != canonical(expected_block):
                    raise TelegramSecurityError('Telegram ambiguity block does not identify the prior send attempt')
            elif kind == 'DUPLICATE_RETRY_AUTHORIZED':
                if receipt is not None:
                    raise TelegramSecurityError('Telegram retry authorization may not claim delivery')
                authorization = _validate_reconciliation_authorization(reconciliation, binding, auth, ambiguity_entry=history[-1])
                claim_id = authorization['authorization_claim_id']
                if claim_id in authorization_claims:
                    raise TelegramSecurityError('Telegram retry authorization claim was already consumed')
                authorization_claims.add(claim_id)
            elif kind == 'DELIVERY_RECEIPT':
                if reconciliation is not None:
                    raise TelegramSecurityError('validated Bot API receipt may not carry operator delivery evidence')
        ...
        if torn_tail:
            if head is None or head != expected_head:
                raise TelegramSecurityError('Telegram delivery ledger torn tail lacks an authenticated recovery head')
            try:
                os.ftruncate(ledger_fd, len(complete_raw))
                os.fsync(ledger_fd)
                os.fsync(parent_fd)
            except OSError:
                raise TelegramSecurityError('Telegram delivery ledger torn-tail recovery failed') from None
            self._fault('after_torn_tail_recovery_fsync')
            return entries
        if head is None:
            if entries:
                raise TelegramSecurityError('Telegram delivery ledger lacks its authenticated durable head')
    ...
    def _append_entry(self, parent_fd: int, ledger_fd: int, entries: Sequence[Mapping[str, Any]], *, kind: str, binding: Mapping[str, Any], receipt: Mapping[str, Any] | None, reconciliation: Mapping[str, Any] | None, auth: TelegramAuthConfig) -> list[dict[str, Any]]:
        ...
        matching = [entry for entry in entries if entry['dedupe_key'] == validated_binding['dedupe_key']]
        if any((canonical(entry['binding']) != canonical(validated_binding) for entry in matching)):
            raise TelegramSecurityError('Telegram dedupe key is already bound to a different delivery intent')
        ...
        if kind in {'OUTBOX_PREPARED', 'SEND_STARTED'}:
            ...
            started = matching[-1]
            expected_block = _ambiguity_block(sum((entry['kind'] == 'SEND_STARTED' for entry in matching)), validated_reconciliation['reason'], send_started_seq=started['seq'], send_started_chain_sha256=started['chain_sha256'])
            if canonical(validated_reconciliation) != canonical(expected_block):
                raise TelegramSecurityError('Telegram ambiguity block does not identify the prior send attempt')
        elif kind == 'DUPLICATE_RETRY_AUTHORIZED':
            if receipt is not None:
                raise TelegramSecurityError('Telegram retry authorization may not claim delivery')
            validated_reconciliation = _validate_reconciliation_authorization(reconciliation, validated_binding, auth, ambiguity_entry=matching[-1])
            claim_id = validated_reconciliation['authorization_claim_id']
            if any((entry['kind'] == 'DUPLICATE_RETRY_AUTHORIZED' and entry['reconciliation']['authorization_claim_id'] == claim_id for entry in entries)):
                raise TelegramSecurityError('Telegram retry authorization claim was already consumed')
        elif kind == 'DELIVERY_RECEIPT':
        ...
        unsigned: dict[str, Any] = {'schema': DELIVERY_LEDGER_SCHEMA, 'seq': len(entries), 'kind': kind, 'recorded_at': recorded_at, 'dedupe_key': validated_binding['dedupe_key'], 'binding': validated_binding, 'receipt': validated_receipt, 'reconciliation': validated_reconciliation, 'prev_chain_sha256': entries[-1]['chain_sha256'] if entries else LEDGER_GENESIS_HASH}
    ...
    def _append_ambiguity_block(self, parent_fd: int, ledger_fd: int, entries: Sequence[Mapping[str, Any]], binding: Mapping[str, Any], auth: TelegramAuthConfig, *, reason: str) -> list[dict[str, Any]]:
        matching = [entry for entry in entries if entry['dedupe_key'] == binding['dedupe_key']]
        if not matching or matching[-1]['kind'] != 'SEND_STARTED':
            raise TelegramSecurityError('Telegram ambiguity block requires the current SEND_STARTED attempt')
        started = matching[-1]
        evidence = _ambiguity_block(sum((entry['kind'] == 'SEND_STARTED' for entry in matching)), reason, send_started_seq=started['seq'], send_started_chain_sha256=started['chain_sha256'])
        return self._append_entry(parent_fd, ledger_fd, entries, kind='AMBIGUOUS_BLOCKED', binding=binding, receipt=None, reconciliation=evidence, auth=auth)
    ...
    def deliver_or_replay(self, binding: Mapping[str, Any], *, auth: TelegramAuthConfig, sender: Callable[[], Mapping[str, Any]], duplicate_retry_authorization: Mapping[str, Any] | None=None) -> dict[str, Any]:
        if not isinstance(auth, TelegramAuthConfig):
            raise TelegramSecurityError('Telegram receipt authenticator is required')
        validated_binding = _validate_delivery_binding(binding, auth)
        with self._locked_files() as (parent_fd, ledger_fd):
            ...
            matching = [entry for entry in entries if entry['dedupe_key'] == validated_binding['dedupe_key']]
            if matching and any((canonical(entry['binding']) != canonical(validated_binding) for entry in matching)):
                raise TelegramSecurityError('Telegram dedupe key is already bound to a different delivery intent')
            ...
            if latest_kind == 'SEND_STARTED':
                self._append_ambiguity_block(parent_fd, ledger_fd, entries, validated_binding, auth, reason='PROCESS_RECOVERY_FOUND_SEND_STARTED')
                raise TelegramSecurityError('Telegram delivery outcome is ambiguous and durably blocked; a bound HMAC duplicate-retry authorization is required')
            if latest_kind == 'AMBIGUOUS_BLOCKED':
                if duplicate_retry_authorization is None:
                    raise TelegramSecurityError('Telegram delivery outcome is ambiguous and durably blocked; a bound HMAC duplicate-retry authorization is required')
                entries = self._append_entry(parent_fd, ledger_fd, entries, kind='DUPLICATE_RETRY_AUTHORIZED', binding=validated_binding, receipt=None, reconciliation=duplicate_retry_authorization, auth=auth)
                ...
                if duplicate_retry_authorization is not None and canonical(_canonical_object(duplicate_retry_authorization, 'Telegram reconciliation authorization', nonempty=True)) != canonical(durable_authorization):
                    raise TelegramSecurityError('Telegram retry authorization differs from the durable authorization')
            elif duplicate_retry_authorization is not None:
                raise TelegramSecurityError('Telegram duplicate-retry authorization has no current ambiguity block')
            if latest_kind is None:
                entries = self._append_entry(parent_fd, ledger_fd, entries, kind='OUTBOX_PREPARED', binding=validated_binding, receipt=None, reconciliation=None, auth=auth)
            elif latest_kind not in {'OUTBOX_PREPARED', 'DUPLICATE_RETRY_AUTHORIZED'}:
                raise TelegramSecurityError('Telegram delivery outbox lifecycle is invalid')
            entries = self._append_entry(parent_fd, ledger_fd, entries, kind='SEND_STARTED', binding=validated_binding, receipt=None, reconciliation=None, auth=auth)
            self._fault('before_network_send')
            try:
                receipt = sender()
                validated_receipt = self._validate_receipt(receipt, validated_binding, auth)
            except Exception:
                self._append_ambiguity_block(parent_fd, ledger_fd, entries, validated_binding, auth, reason='SENDER_FAILED_AFTER_SEND_STARTED')
                raise TelegramSecurityError('Telegram sender failed after SEND_STARTED; outcome is durably blocked') from None
            self._fault('after_send_success_before_receipt_append')
            self._append_entry(parent_fd, ledger_fd, entries, kind='DELIVERY_RECEIPT', binding=validated_binding, receipt=validated_receipt, reconciliation=None, auth=auth)
            return validated_receipt

    def reconcile_ambiguous(self, transition_intent: Mapping[str, Any], receipt: Mapping[str, Any], *, auth: TelegramAuthConfig) -> dict[str, Any]:
        ...
        with self._locked_files() as (parent_fd, ledger_fd):
            entries = self._verified_entries(parent_fd, ledger_fd, auth)
            matching = [entry for entry in entries if entry['dedupe_key'] == binding['dedupe_key']]
            if matching and any((canonical(entry['binding']) != canonical(binding) for entry in matching)):
                raise TelegramSecurityError('Telegram dedupe key is already bound to a different delivery intent')
            latest_kind = matching[-1]['kind'] if matching else None
            if latest_kind == 'DELIVERY_RECEIPT':
                stored = self._validate_receipt(matching[-1]['receipt'], binding, auth)
                if canonical(stored) != canonical(validated_receipt):
                    raise TelegramSecurityError('Telegram reconciliation receipt differs from the durable receipt')
                return stored
            if latest_kind == 'SEND_STARTED':
                entries = self._append_ambiguity_block(parent_fd, ledger_fd, entries, binding, auth, reason='PROCESS_RECOVERY_FOUND_SEND_STARTED')
                latest_kind = 'AMBIGUOUS_BLOCKED'
            if latest_kind != 'AMBIGUOUS_BLOCKED':
                raise TelegramSecurityError('Telegram reconciliation requires a durably blocked ambiguous send')
            self._append_entry(parent_fd, ledger_fd, entries, kind='DELIVERY_RECEIPT', binding=binding, receipt=validated_receipt, reconciliation=None, auth=auth)
            return validated_receipt

def send_campaign_status(transition_intent: Mapping[str, Any], *, ledger: TelegramDeliveryLedger, keychain: Keychain, transport: TelegramTransport, duplicate_retry_authorization: Mapping[str, Any] | None=None) -> dict[str, Any]:
    ...
    def sender() -> dict[str, Any]:
        response = _transport_call(transport, credentials.token, 'sendMessage', {'chat_id': credentials.chat_id, 'text': text, 'disable_web_page_preview': 'true'})
        ...
        try:
            return make_telegram_delivery_receipt(binding['transition_intent'], auth=credentials.auth, bot_api_response=response.body, http_status=response.status)
        except StateError:
            raise TelegramSecurityError('Telegram sendMessage response failed prepared-intent receipt validation') from None
    return ledger.deliver_or_replay(binding, auth=credentials.auth, sender=sender, duplicate_retry_authorization=duplicate_retry_authorization)
