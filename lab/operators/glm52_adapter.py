"""Recomposed science module glm52_adapter (C-SCI-R1)."""
...
OFFICIAL_TOKENIZER_ASSETS: Mapping[str, tuple[int, str]] = {'tokenizer.json': (20217442, '19e773648cb4e65de8660ea6365e10acca112d42a854923df93db4a6f333a82d'), 'tokenizer_config.json': (761, '98b1271574f41abf89427ae2dda030d94dc9478f0edc5a8bd240db213c6fd5fc'), 'chat_template.jinja': (5076, '172dc74a35e1752df75ecfb2b2cf9326d2852bb1379868ebeec9571654489679'), 'generation_config.json': (194, 'ac76b43d8683d3b930126870fc8be73d8679308fe752fa1f381096d8354f6a55')}
GATE_UP_ORDER = ('gate_proj', 'up_proj')
_SAFE_SHARD_RE = re.compile('^[A-Za-z0-9][A-Za-z0-9._-]*\\.safetensors$')
_LAYER_RE = re.compile('^model\\.layers\\.(\\d+)\\.(.+)$')
_EXPERT_RE = re.compile('^mlp\\.experts\\.(\\d+)\\.(gate_proj|up_proj|down_proj)\\.weight$')
...
class OfficialTokenizerAssembly:
    ...
    def assemble_chat(self, messages: Iterable[Mapping[str, Any]], *, tools: Iterable[Mapping[str, Any]] | None=None, add_generation_prompt: bool=True, enable_thinking: bool=True, reasoning_effort: Literal['high', 'max']='max', clear_thinking: bool=True) -> dict[str, Any]:
        if not all((isinstance(value, bool) for value in (add_generation_prompt, enable_thinking, clear_thinking))):
        ...
        try:
            rows = json.loads(json.dumps([dict(message) for message in messages], ensure_ascii=False, allow_nan=False))
        ...
        for index, row in enumerate(rows):
            if not isinstance(row, dict) or 'role' not in row or 'content' not in row:
                raise Glm52AdapterError(f'chat message {index} must contain role/content')
            ...
            if unknown:
                raise Glm52AdapterError(f'chat message {index} has unsupported fields: {unknown}')
            content = row['content']
            if content is None and row['role'] != 'assistant':
                raise Glm52AdapterError(f'chat content {index} may be null only for assistant tool calls')
            if content is not None and (not isinstance(content, (str, list))):
                raise Glm52AdapterError(f'chat content {index} must be text, a content-part list, or assistant null')
            if 'reasoning_content' in row and (not isinstance(row['reasoning_content'], str)):
                raise Glm52AdapterError(f'assistant reasoning_content {index} must be text')
            if 'tool_calls' in row:
                calls = row['tool_calls']
                if not isinstance(calls, list) or not calls:
                    raise Glm52AdapterError(f'assistant tool_calls {index} must be a non-empty list')
                for call_index, call in enumerate(calls):
                    if not isinstance(call, dict):
                        raise Glm52AdapterError(f'assistant tool call {index}/{call_index} must be an object')
                    function = call.get('function', call)
                    if not isinstance(function, dict) or not isinstance(function.get('name'), str) or (not function['name']):
                        raise Glm52AdapterError(f'assistant tool call {index}/{call_index} lacks a function name')
                    if not isinstance(function.get('arguments'), dict):
                        raise Glm52AdapterError(f'assistant tool call {index}/{call_index} arguments must be an object')
        try:
            tool_rows = None if tools is None else json.loads(json.dumps([dict(tool) for tool in tools], ensure_ascii=False, allow_nan=False))
        except (TypeError, ValueError) as exc:
        ...
        rendered = self.tokenizer.apply_chat_template(rows, tokenize=False, tools=tool_rows, add_generation_prompt=add_generation_prompt, enable_thinking=enable_thinking, reasoning_effort=reasoning_effort, clear_thinking=clear_thinking)
        ...
        if not isinstance(token_ids, list) or any((isinstance(value, bool) or not isinstance(value, int) for value in token_ids)):
            raise Glm52AdapterError('official tokenizer returned invalid token IDs')
        return {'rendered_sha256': hashlib.sha256(rendered.encode('utf-8')).hexdigest(), 'token_ids_sha256': hashlib.sha256(json.dumps(token_ids, separators=(',', ':')).encode('utf-8')).hexdigest(), 'token_count': len(token_ids), 'begins_with_glm_mask_sop': rendered.startswith('[gMASK]<sop>'), 'ends_with_generation_think': rendered.endswith('<|assistant|><think>' if enable_thinking else '<|assistant|><think></think>') if add_generation_prompt else None, 'tool_count': len(tool_rows or []), 'contains_tool_catalog': '<tools>' in rendered, 'contains_tool_call': '<tool_call>' in rendered, 'contains_tool_response': '<tool_response>' in rendered, 'thinking_enabled': enable_thinking, 'reasoning_effort': reasoning_effort, 'generation_eos_token_ids': list(self.generation_eos_token_ids), 'generation_pad_token_id': self.generation_pad_token_id}
...
def load_official_tokenizer_assembly(control_root: Path) -> OfficialTokenizerAssembly:
    ...
    if IMMUTABLE_REVISION not in str(control_root):
        raise Glm52AdapterError('official tokenizer root is not bound to the immutable source revision')
    ...
    for name, (expected_bytes, expected_sha256) in OFFICIAL_TOKENIZER_ASSETS.items():
        ...
        if observed_bytes != expected_bytes or digest != expected_sha256:
            raise Glm52AdapterError(f'official tokenizer asset identity mismatch for {name}: bytes/sha256={observed_bytes}/{digest}')
        asset_hashes[name] = digest
    try:
        from transformers import AutoTokenizer
        tokenizer = AutoTokenizer.from_pretrained(control_root, local_files_only=True, trust_remote_code=False)
    ...
    generation_eos = (154820, 154827, 154829)
    if not isinstance(generation, dict) or tuple(generation.get('eos_token_id', ())) != generation_eos or generation.get('pad_token_id') != 154820:
        raise Glm52AdapterError('official generation stop-token contract mismatch')
    expected = {'vocabulary_size': 154856, 'padded_model_vocabulary_size': 154880, 'eos_token_id': 154820, 'pad_token_id': 154820, 'model_max_length': 1048576}
    observed = {'vocabulary_size': len(tokenizer), 'padded_model_vocabulary_size': OFFICIAL_CONFIG_CONTRACT['vocab_size'], 'eos_token_id': tokenizer.eos_token_id, 'pad_token_id': tokenizer.pad_token_id, 'model_max_length': tokenizer.model_max_length}
    if observed != expected or tokenizer.chat_template != template:
        raise Glm52AdapterError(f'official tokenizer/chat-template contract mismatch: {observed}')
    return OfficialTokenizerAssembly(root=control_root, tokenizer=tokenizer, asset_sha256=asset_hashes, generation_eos_token_ids=generation_eos, generation_pad_token_id=154820, **expected)

def _official_indexer_types() -> tuple[str, ...]:
    return tuple(('full' if layer in {0, 1, 2} or (layer >= 6 and (layer - 6) % 4 == 0) else 'shared' for layer in range(78)))
OFFICIAL_INDEXER_TYPES = _official_indexer_types()
SYNTHETIC_INDEXER_TYPES = ('full', 'full', 'full', 'shared', 'shared', 'shared', 'full')
OFFICIAL_CONFIG_CONTRACT: Mapping[str, Any] = {'architectures': [EXPECTED_ARCHITECTURE], 'attention_bias': False, 'attention_dropout': 0.0, 'dtype': 'bfloat16', 'ep_size': 1, 'first_k_dense_replace': 3, 'head_dim': 192, 'hidden_act': 'silu', 'hidden_size': 6144, 'index_head_dim': 128, 'index_n_heads': 32, 'index_share_for_mtp_iteration': True, 'index_skip_topk_offset': 3, 'index_topk': 2048, 'index_topk_freq': 4, 'index_topk_pattern': None, 'intermediate_size': 12288, 'kv_lora_rank': 512, 'max_position_embeddings': 1048576, 'model_type': EXPECTED_MODEL_TYPE, 'moe_intermediate_size': 2048, 'moe_layer_freq': 1, 'moe_router_dtype': 'float32', 'n_group': 1, 'n_routed_experts': 256, 'n_shared_experts': 1, 'norm_topk_prob': True, 'num_attention_heads': 64, 'num_experts_per_tok': 8, 'num_hidden_layers': 78, 'num_key_value_heads': 64, 'num_nextn_predict_layers': 1, 'pretraining_tp': 1, 'q_lora_rank': 2048, 'qk_head_dim': 256, 'qk_nope_head_dim': 192, 'qk_rope_head_dim': 64, 'rms_norm_eps': 1e-05, 'rope_interleave': True, 'routed_scaling_factor': 2.5, 'scoring_func': 'sigmoid', 'tie_word_embeddings': False, 'topk_group': 1, 'topk_method': 'noaux_tc', 'use_cache': True, 'v_head_dim': 256, 'vocab_size': 154880}
SYNTHETIC_CONFIG_CONTRACT: Mapping[str, Any] = {'architectures': [EXPECTED_ARCHITECTURE], 'attention_bias': False, 'attention_dropout': 0.0, 'dtype': 'bfloat16', 'ep_size': 1, 'first_k_dense_replace': 3, 'head_dim': 8, 'hidden_act': 'silu', 'hidden_size': 32, 'index_head_dim': 4, 'index_n_heads': 4, 'index_share_for_mtp_iteration': True, 'index_skip_topk_offset': 3, 'index_topk': 2, 'index_topk_freq': 4, 'index_topk_pattern': None, 'intermediate_size': 64, 'kv_lora_rank': 8, 'max_position_embeddings': 1048576, 'model_type': EXPECTED_MODEL_TYPE, 'moe_intermediate_size': 16, 'moe_layer_freq': 1, 'moe_router_dtype': 'float32', 'n_group': 1, 'n_routed_experts': 256, 'n_shared_experts': 1, 'norm_topk_prob': True, 'num_attention_heads': 4, 'num_experts_per_tok': 8, 'num_hidden_layers': 7, 'num_key_value_heads': 4, 'num_nextn_predict_layers': 1, 'pretraining_tp': 1, 'q_lora_rank': 16, 'qk_head_dim': 12, 'qk_nope_head_dim': 8, 'qk_rope_head_dim': 4, 'rms_norm_eps': 1e-05, 'rope_interleave': True, 'routed_scaling_factor': 2.5, 'scoring_func': 'sigmoid', 'tie_word_embeddings': False, 'topk_group': 1, 'topk_method': 'noaux_tc', 'use_cache': True, 'v_head_dim': 8, 'vocab_size': 64}
...
class Geometry:
    ...
    def section_for_layer(self, layer: int) -> str:
        ...
        if layer == self.mtp_layer:
            return MTP
        raise Glm52AdapterError(f'layer {layer} is outside 0..{self.mtp_layer} (physical MTP boundary)')
...
def validate_config(config: Mapping[str, Any], *, profile: str=PROFILE_OFFICIAL) -> Geometry:
    ...
    mismatches = {key: {'expected': expected, 'actual': config.get(key)} for key, expected in contract.items() if config.get(key) != expected}
    if mismatches:
    ...
    if not isinstance(actual_indexers, list) or tuple(actual_indexers) != expected_indexers:
        raise Glm52AdapterError(f'{profile} IndexShare pattern mismatch: expected={expected_indexers!r} actual={actual_indexers!r}')
    expected_mlp = tuple(('dense' if layer < int(contract['first_k_dense_replace']) else 'sparse' for layer in range(int(contract['num_hidden_layers']))))
    actual_mlp = config.get('mlp_layer_types')
    if not isinstance(actual_mlp, list) or tuple(actual_mlp) != expected_mlp:
        raise Glm52AdapterError(f'{profile} dense/sparse layer pattern mismatch: expected={expected_mlp!r} actual={actual_mlp!r}')
    if int(config['qk_head_dim']) != int(config['qk_nope_head_dim']) + int(config['qk_rope_head_dim']):
    ...
    return Geometry(profile=profile, hidden_size=int(config['hidden_size']), vocab_size=int(config['vocab_size']), num_hidden_layers=int(config['num_hidden_layers']), first_k_dense_replace=int(config['first_k_dense_replace']), intermediate_size=int(config['intermediate_size']), moe_intermediate_size=int(config['moe_intermediate_size']), num_attention_heads=int(config['num_attention_heads']), num_key_value_heads=int(config['num_key_value_heads']), q_lora_rank=int(config['q_lora_rank']), kv_lora_rank=int(config['kv_lora_rank']), qk_nope_head_dim=int(config['qk_nope_head_dim']), qk_rope_head_dim=int(config['qk_rope_head_dim']), qk_head_dim=int(config['qk_head_dim']), v_head_dim=int(config['v_head_dim']), index_n_heads=int(config['index_n_heads']), index_head_dim=int(config['index_head_dim']), index_topk=int(config['index_topk']), n_routed_experts=int(config['n_routed_experts']), n_shared_experts=int(config['n_shared_experts']), num_experts_per_tok=int(config['num_experts_per_tok']), routed_scaling_factor=float(config['routed_scaling_factor']), scoring_func=str(config['scoring_func']), topk_method=str(config['topk_method']), norm_topk_prob=bool(config['norm_topk_prob']), indexer_types=expected_indexers)

def _top_spec(geometry: Geometry, name: str) -> TensorSpec | None:
    shapes = {'model.embed_tokens.weight': ((geometry.vocab_size, geometry.hidden_size), 'embeddings'), 'model.norm.weight': ((geometry.hidden_size,), 'normalization'), 'lm_head.weight': ((geometry.vocab_size, geometry.hidden_size), 'lm_head')}
    row = shapes.get(name)
    if row is None:
        return None
    shape, organ = row
...
def tensor_spec(geometry: Geometry, name: str) -> TensorSpec:
    ...
    normalizations = {'input_layernorm.weight': (h,), 'post_attention_layernorm.weight': (h,), 'self_attn.q_a_layernorm.weight': (geometry.q_lora_rank,), 'self_attn.kv_a_layernorm.weight': (geometry.kv_lora_rank,)}
    if suffix in normalizations:
        return TensorSpec(name, normalizations[suffix], 'BF16', 'normalization', section, layer, None, source)
    attention = {'self_attn.q_a_proj.weight': (geometry.q_lora_rank, h), 'self_attn.q_b_proj.weight': (geometry.num_attention_heads * geometry.qk_head_dim, geometry.q_lora_rank), 'self_attn.kv_a_proj_with_mqa.weight': (geometry.kv_lora_rank + geometry.qk_rope_head_dim, h), 'self_attn.kv_b_proj.weight': (geometry.num_attention_heads * (geometry.qk_nope_head_dim + geometry.v_head_dim), geometry.kv_lora_rank), 'self_attn.o_proj.weight': (h, geometry.num_attention_heads * geometry.v_head_dim)}
    if suffix in attention:
        return TensorSpec(name, attention[suffix], 'BF16', 'attention', section, layer, None, source)
    indexer = {'self_attn.indexer.k_norm.bias': (geometry.index_head_dim,), 'self_attn.indexer.k_norm.weight': (geometry.index_head_dim,), 'self_attn.indexer.weights_proj.weight': (geometry.index_n_heads, h), 'self_attn.indexer.wk.weight': (geometry.index_head_dim, h), 'self_attn.indexer.wq_b.weight': (geometry.index_n_heads * geometry.index_head_dim, geometry.q_lora_rank)}
    if suffix in indexer:
        if geometry.indexer_type(layer) != 'full':
            raise Glm52AdapterError(f'shared IndexShare layer {layer} must omit stored indexer tensor {name!r}')
        return TensorSpec(name, indexer[suffix], 'BF16', 'indexer', section, layer, None, source)
    ...
    if expert_match is not None:
        ...
        shape = (h, geometry.moe_intermediate_size) if projection == 'down_proj' else (geometry.moe_intermediate_size, h)
        return TensorSpec(name, shape, 'BF16', 'routed_expert', section, layer, expert, source)
    if suffix in {'mlp.shared_experts.gate_proj.weight', 'mlp.shared_experts.up_proj.weight', 'mlp.shared_experts.down_proj.weight'}:
        if section == CORE and layer < geometry.first_k_dense_replace:
            raise Glm52AdapterError(f'shared expert appears on dense layer: {name!r}')
        shape = (h, geometry.moe_intermediate_size) if suffix.endswith('down_proj.weight') else (geometry.moe_intermediate_size, h)
        return TensorSpec(name, shape, 'BF16', 'shared_expert', section, layer, None, source)
    if suffix == 'mlp.gate.weight':
        if section == CORE and layer < geometry.first_k_dense_replace:
            raise Glm52AdapterError(f'router appears on dense layer: {name!r}')
        return TensorSpec(name, (geometry.n_routed_experts, h), 'BF16', 'router', section, layer, None, source)
    if suffix == 'mlp.gate.e_score_correction_bias':
        if section == CORE and layer < geometry.first_k_dense_replace:
            raise Glm52AdapterError(f'router correction appears on dense layer: {name!r}')
        return TensorSpec(name, (geometry.n_routed_experts,), 'F32', 'router_control', section, layer, None, source)
    if section == MTP:
        mtp_shapes = {'eh_proj.weight': ((h, 2 * h), 'mtp_projection'), 'enorm.weight': ((h,), 'mtp_normalization'), 'hnorm.weight': ((h,), 'mtp_normalization'), 'shared_head.norm.weight': ((h,), 'mtp_head_norm')}
...
def _layer_names(geometry: Geometry, layer: int) -> Iterable[str]:
    ...
    for suffix in ('input_layernorm.weight', 'post_attention_layernorm.weight', 'self_attn.q_a_layernorm.weight', 'self_attn.kv_a_layernorm.weight', 'self_attn.q_a_proj.weight', 'self_attn.q_b_proj.weight', 'self_attn.kv_a_proj_with_mqa.weight', 'self_attn.kv_b_proj.weight', 'self_attn.o_proj.weight'):
        yield (base + suffix)
    if geometry.indexer_type(layer) == 'full':
        for suffix in ('self_attn.indexer.k_norm.bias', 'self_attn.indexer.k_norm.weight', 'self_attn.indexer.weights_proj.weight', 'self_attn.indexer.wk.weight', 'self_attn.indexer.wq_b.weight'):
            yield (base + suffix)
...
def expected_tensor_specs(geometry: Geometry, *, view: str=VIEW_FULL) -> dict[str, TensorSpec]:
    ...
    expected_count = 59585 if geometry.profile == PROFILE_OFFICIAL else 3978
    if view == VIEW_CORE:
        expected_count -= 791
    if len(specs) != expected_count:
        raise Glm52AdapterError(f'internal {geometry.profile}/{view} tensor census drift: expected={expected_count} actual={len(specs)}')
    return specs
...
def _pread_exact(fd: int, count: int, offset: int, *, context: str) -> bytes:
    ...
    if len(data) != count:
        raise Glm52AdapterError(f'short bounded read for {context}: expected={count} actual={len(data)}')
    return data

def _parse_shard(path: Path, shard_name: str, expected_names: set[str], expected_specs: Mapping[str, TensorSpec]) -> tuple[ShardRecord, dict[str, TensorRecord]]:
    ...
    try:
        ...
        if actual_names != expected_names:
            raise Glm52AdapterError(f'index/header tensor set mismatch in {shard_name}: missing={sorted(expected_names - actual_names)[:5]} unknown={sorted(actual_names - expected_names)[:5]}')
        ...
        for name, entry in header.items():
            ...
            if not isinstance(shape_raw, list) or not shape_raw or any((not isinstance(dim, int) or isinstance(dim, bool) or dim <= 0 for dim in shape_raw)):
            ...
            if start < 0 or end <= start or end - start != expected_extent:
                raise Glm52AdapterError(f'shape/dtype extent mismatch: {shard_name}:{name}: shape={shape} dtype={dtype} offsets={offsets}')
            ...
            if shape != spec.shape or dtype != spec.dtype:
                raise Glm52AdapterError(f'schema mismatch for {name}: expected={spec.dtype}{spec.shape} actual={dtype}{shape}')
            records[name] = TensorRecord(spec=spec, shard=shard_name, relative_start=start, relative_end=end, absolute_start=data_start + start, absolute_end=data_start + end)
            extents.append((start, end, name))
        cursor = 0
        for start, end, name in sorted(extents):
            if start != cursor:
                relation = 'overlap' if start < cursor else 'gap'
                raise Glm52AdapterError(f'safetensors payload {relation} before {shard_name}:{name}: cursor={cursor} start={start}')
            cursor = end
        if data_start + cursor != st.st_size:
            raise Glm52AdapterError(f'safetensors payload does not close at EOF: {shard_name}: data_start={data_start} payload={cursor} file={st.st_size}')
        return (ShardRecord(name=shard_name, path=path.resolve(), file_bytes=st.st_size, header_bytes=header_bytes, data_start=data_start, payload_bytes=cursor, header_sha256=hashlib.sha256(prefix + header_raw).hexdigest(), device=st.st_dev, inode=st.st_ino, mtime_ns=st.st_mtime_ns), records)
    finally:
        os.close(fd)

def verify_checkpoint(root: Path, *, profile: str=PROFILE_OFFICIAL, view: str=VIEW_FULL) -> Inventory:
    ...
    if indexed_names != expected_names:
        raise Glm52AdapterError(f'checkpoint tensor census mismatch: missing={sorted(expected_names - indexed_names)[:5]} unknown={sorted(indexed_names - expected_names)[:5]}')
    ...
    if index['metadata']['total_size'] != expected_payload_bytes:
        raise Glm52AdapterError(f'complete source index total_size mismatch before window admission: index={index['metadata']['total_size']} expected={expected_payload_bytes}')
    shard_names = set(weight_map.values())
    disk_shards = {path.name for path in root.glob('*.safetensors')}
    if disk_shards != shard_names:
        raise Glm52AdapterError(f'checkpoint shard set mismatch: missing={sorted(shard_names - disk_shards)} unindexed={sorted(disk_shards - shard_names)}')
    ...
    for shard_name in sorted(shard_names):
        shard, records = _parse_shard(root / shard_name, shard_name, by_shard[shard_name], expected_specs)
    ...
    if payload_bytes != index['metadata']['total_size']:
        raise Glm52AdapterError(f'index total_size mismatch: index={index['metadata']['total_size']} headers={payload_bytes}')
    ...
    return Inventory(root=root, profile=profile, view=view, geometry=geometry, config=config, index=index, tensors=tensors, shards=shards)

def _streaming_window_shards(window_contract: Mapping[str, Any], *, indexed_shards: set[str]) -> set[str]:
    ...
    for key in ('source_shards', 'carry_in_shards', 'new_fetch_shards', 'refetch_shards', 'carry_out_shards', 'evict_after_seal_shards'):
        raw = window_contract.get(key)
        if not isinstance(raw, list):
            raise Glm52AdapterError(f'{window_id}.{key} must be a list')
        if any((not isinstance(item, str) or _SAFE_SHARD_RE.fullmatch(item) is None or Path(item).name != item for item in raw)):
    ...
    if any((acquisition_sets[left] & acquisition_sets[right] for left in range(len(acquisition_sets)) for right in range(left + 1, len(acquisition_sets)))):
        raise Glm52AdapterError(f'{window_id} carry/new-fetch/refetch shard sets must be disjoint')
    if source != carry_in | new_fetch | refetch:
        raise Glm52AdapterError(f'{window_id}.source_shards must equal carry-in + new-fetch + refetch')
    ...
    if evict != source - carry_out:
        raise Glm52AdapterError(f'{window_id}.evict_after_seal_shards must equal source minus carry-out')
    unknown = source - indexed_shards
    if unknown:
        raise Glm52AdapterError(f'{window_id} references shards outside the complete official index: {sorted(unknown)[:3]}')
    return source

def verify_streaming_window(control_root: Path, hydrated_root: Path, window_contract: Mapping[str, Any], *, profile: str=PROFILE_OFFICIAL, view: str=VIEW_FULL) -> Inventory:
    ...
    indexed_names = set(weight_map)
    if indexed_names != expected_names:
        raise Glm52AdapterError(f'complete source index tensor census mismatch before window admission: missing={sorted(expected_names - indexed_names)[:5]} unknown={sorted(indexed_names - expected_names)[:5]}')
    expected_payload_bytes = sum((spec.byte_count for spec in expected_specs.values()))
    if index['metadata']['total_size'] != expected_payload_bytes:
        raise Glm52AdapterError(f'complete source index total_size mismatch before window admission: index={index['metadata']['total_size']} expected={expected_payload_bytes}')
    indexed_shards = set(weight_map.values())
    resident = _streaming_window_shards(window_contract, indexed_shards=indexed_shards)
    disk_shards = {path.name for path in hydrated_root.glob('*.safetensors')}
    if disk_shards != resident:
        raise Glm52AdapterError(f'hydrated streaming shard set mismatch: missing={sorted(resident - disk_shards)} unexpected={sorted(disk_shards - resident)}')
    by_shard: dict[str, set[str]] = {name: set() for name in resident}
    for name, shard in weight_map.items():
        if shard in resident:
            by_shard[shard].add(name)
    expected_window_names = {name for name, shard in weight_map.items() if shard in resident}
    shards: dict[str, ShardRecord] = {}
    tensors: dict[str, TensorRecord] = {}
    for shard_name in sorted(resident):
        shard, records = _parse_shard(hydrated_root / shard_name, shard_name, by_shard[shard_name], expected_specs)
        overlap = set(tensors).intersection(records)
        if overlap:
            raise Glm52AdapterError(f'duplicate tensors across streaming shards: {sorted(overlap)[:5]}')
        shards[shard_name] = shard
        tensors.update(records)
    ...
    if sum((shard.payload_bytes for shard in shards.values())) != sum((record.byte_count for record in tensors.values())):
        raise Glm52AdapterError('streaming window payload totals differ')
    return Inventory(root=hydrated_root, profile=profile, view=view, geometry=geometry, config=config, index=index, tensors=tensors, shards=shards)
...
def expert_gate_up_names(layer: int, expert: int) -> tuple[str, str]:
    ...
    return tuple((base + projection + '.weight' for projection in GATE_UP_ORDER))

def pack_expert_gate_up(reader: BoundedSafetensorsReader, layer: int, expert: int, *, max_bytes_per_tensor: int | None=None) -> np.ndarray:
    ...
    if gate_record is None or up_record is None:
        raise Glm52AdapterError(f'cannot pack missing routed expert gate/up pair: layer={layer} expert={expert}')
...
def schema_summary(geometry: Geometry) -> dict[str, Any]:
    ...
    return {'profile': geometry.profile, 'main_layers': geometry.num_hidden_layers, 'physical_layers': geometry.physical_layer_count, 'mtp_physical_layer': geometry.mtp_layer, 'full_tensor_count': len(full), 'core_tensor_count': len(core), 'mtp_tensor_count': len(mtp), 'core_logical_elements': sum((spec.element_count for spec in core.values())), 'mtp_logical_elements': sum((spec.element_count for spec in mtp.values())), 'indexer_types_main': list(geometry.indexer_types), 'mtp_indexer_type': geometry.indexer_type(geometry.mtp_layer), 'index_topk': geometry.index_topk, 'routed_expert_count': geometry.n_routed_experts, 'shared_expert_count': geometry.n_shared_experts, 'experts_per_token': geometry.num_experts_per_tok, 'router_selection': f'top-{geometry.num_experts_per_tok}-of-{geometry.n_routed_experts}', 'router_weight_sum_after_scaling': geometry.routed_scaling_factor, 'router_scoring_function': geometry.scoring_func, 'router_topk_method': geometry.topk_method, 'router_normalizes_topk_probability': geometry.norm_topk_prob, 'gate_up_order': list(GATE_UP_ORDER), 'source_dtypes': sorted({spec.dtype for spec in full.values()})}
__all__ = ['BoundedSafetensorsReader', 'CORE', 'DEFAULT_MAX_TENSOR_BYTES', 'DTYPE_BYTES', 'GATE_UP_ORDER', 'Geometry', 'Glm52AdapterError', 'IMMUTABLE_REVISION', 'Inventory', 'MTP', 'OFFICIAL_CONFIG_CONTRACT', 'OFFICIAL_INDEXER_TYPES', 'OFFICIAL_TOKENIZER_ASSETS', 'OfficialTokenizerAssembly', 'PROFILE_OFFICIAL', 'PROFILE_SYNTHETIC', 'REPO_ID', 'SYNTHETIC_CONFIG_CONTRACT', 'SYNTHETIC_INDEXER_TYPES', 'TensorRecord', 'TensorSpec', 'VIEW_CORE', 'VIEW_FULL', 'expected_tensor_specs', 'expert_gate_up_names', 'load_index', 'load_json_strict', 'load_official_tokenizer_assembly', 'pack_expert_gate_up', 'schema_summary', 'split_expert_gate_up', 'tensor_spec', 'validate_config', 'verify_checkpoint', 'verify_streaming_window']
