"""Recomposed science module glm52_evidence_auth (C-SCI-R1)."""
from __future__ import annotations
from lab.operators.glm52_common import Glm52Error
from lab.operators.glm52_state import EvidenceAuthConfig
...
class MacOSKeychain:
    ...
    def __init__(self, *, runner: Callable[..., subprocess.CompletedProcess[str]]=subprocess.run, native_reader: Callable[[str, str], str | None] | None=None, native_writer: Callable[[str, str, str], None] | None=None) -> None:
        self._runner = runner
...
def _set_generic_password_native(service: str, account: str, value: str) -> None:
    ...
    find.argtypes = [void_p, uint32, ctypes.c_char_p, uint32, ctypes.c_char_p, ctypes.POINTER(uint32), ctypes.POINTER(void_p), ctypes.POINTER(void_p)]
    find.restype = ctypes.c_int32
    add = security.SecKeychainAddGenericPassword
    add.argtypes = [void_p, uint32, ctypes.c_char_p, uint32, ctypes.c_char_p, uint32, void_p, ctypes.POINTER(void_p)]
    ...
    item = void_p()
    status = find(None, len(service_bytes), service_bytes, len(account_bytes), account_bytes, ctypes.byref(password_length), ctypes.byref(password_data), ctypes.byref(item))
    try:
        if status == 0:
            ...
            status = add(None, len(service_bytes), service_bytes, len(account_bytes), account_bytes, len(secret_bytes), secret_pointer, None)
        if status != 0:
...
def _get_generic_password_native(service: str, account: str) -> str | None:
    ...
    password_length = uint32()
    password_data = void_p()
    status = find(None, len(service_bytes), service_bytes, len(account_bytes), account_bytes, ctypes.byref(password_length), ctypes.byref(password_data), None)
    if status == -25300:
        return None
...
def _identity_digest(key: bytes) -> str:
    return hashlib.sha256(b'hawking.glm52.evidence-key-public-identity.v1\x00' + key).hexdigest()
...
def configure_hmac_key(keychain: Keychain, *, random_bytes: Callable[[int], bytes]=secrets.token_bytes) -> dict[str, Any]:
    ...
    if existing is not None:
        ...
        return {'status': 'ALREADY_CONFIGURED', 'configured': True, 'key_identity_digest': _identity_digest(key)}
    try:
    ...
    return {'status': 'CONFIGURED', 'configured': True, 'key_identity_digest': _identity_digest(key)}
...
def credential_status(keychain: Keychain) -> dict[str, Any]:
    ...
    try:
        ...
        return {'configured': False, 'ready': False}
    return {'configured': True, 'ready': True, 'key_identity_digest': _identity_digest(key)}

def load_evidence_auth(keychain: Keychain, *, campaign_id: str, source_revision: str) -> EvidenceAuthConfig:
    encoded = keychain.get(EVIDENCE_HMAC_SERVICE)
    if encoded is None:
        raise EvidenceSecurityError('scientific evidence authentication is not configured')
    try:
        return EvidenceAuthConfig(hmac_key=_decode(encoded), campaign_id=campaign_id, source_revision=source_revision)
