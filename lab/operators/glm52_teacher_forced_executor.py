...
class LayerWeightPlan:
    ...
    def shards_for_layer(self, layer: int) -> set[str]:
        return set(self.layer_to_shards.get(layer, set())) | set(self.global_shards)
    ...
    def missing_for_layer(self, layer: int) -> list[str]:
        return sorted(
            name
            for name in self.shards_for_layer(layer)
...
def run_teacher_forced(cfg: ExecutorConfig) -> dict[str, Any]:
    ...
    if profile == PROFILE_SYNTHETIC:
        corpus = freeze_corpus(
            level=cfg.corpus_level,
            mode="synthetic",
            max_sequence=min(cfg.max_sequence, int(config.get("index_topk", 2)) if cfg.mode == "synthetic" else cfg.max_sequence),
            vocab_size=int(config["vocab_size"]),
            pad_id=0,
        )
        # DSA index_topk for synthetic is 2; keep sequences ≤ index_topk for trivial complete selection
        # OR allow longer with real selection. Reference handles both. Prefer ≤ index_topk for determinism.
        if corpus.max_sequence > int(config["index_topk"]):
            # re-freeze with shorter sequences for index-complete smoke
            corpus = freeze_corpus(
                level=cfg.corpus_level,
                mode="synthetic",
    ...
    # Embedding requires global shards.
    embed_missing = [
        s for s in plan.global_shards if not (plan.root / s).is_file()
    ]
    # Also need layer-0 shards for first forward.
    ...
    # Admit all currently resident shards at start (synthetic has full body tiny).
    ...
    for layer in range(n_layers):
        ...
        missing = plan.missing_for_layer(layer)
        if missing:
            deepest_blocker = (
        ...
        # Prefetch mark for N+1 (synthetic: already resident; official: would stream).
        if db.n_plus_1 is not None:
            prefetch_missing = plan.missing_for_layer(db.n_plus_1)
            db_row["prefetch_status"] = (
                "RESIDENT" if not prefetch_missing else f"MISSING_{len(prefetch_missing)}"
            )
        else:
            db_row["prefetch_status"] = "NONE"

        # Refresh admitted set to currently resident.
        source.admitted_shards = plan.resident_shards()
        plan.verify_window_hashes(plan.shards_for_layer(layer) & source.admitted_shards)
    ...
    if len(layers_captured) == n_layers and source.resident("model.norm.weight"):
        try:
            if source.resident("lm_head.weight"):
                ...
                if cfg.allow_eviction:
                    ...
                    # Evict everything still resident that only served completed main layers.
                    victims = shards_evictable_after(
                        plan,
    ...
    receipt = seal(
        {
            ...
            "note": (
                ...
                "Official 78-layer BF16 body required for PASS_FULL_STACK on "
                "profile=official; synthetic proves the mechanism end-to-end."
            ),
        }
    )
