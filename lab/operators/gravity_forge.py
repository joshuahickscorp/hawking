"""Recomposed science module gravity_forge (C-SCI-R1)."""
...
def _argmin_chunked(v, v2, cb, step: int):
    ...
    for start in range(0, n, step):
        stop = min(start + step, n)
        out[start:stop] = (v2[start:stop] - 2.0 * (v[start:stop] @ cb_t) + cb_sq).argmin(1)
    return out
...
def pack_transform_pq(w: np.ndarray, *, dim: int, subspaces: int, k: int, seed: int=0, iters: int=10) -> PackedArtifact:
    ...
    recon = (recon_r @ R).reshape(rows, cols)
    base_bits = ledger.total_bits() - _METADATA_BYTES * 8
    codes = _make_codes(codebooks_np, np.stack(idx_cols, axis=1), D=D, S=S, sub=sub, rows=rows, cols=cols, rotate=True, seed=seed)
    return PackedArtifact('transform_pq', recon.detach().cpu().numpy().astype(np.float32), w.size, ledger, base_bits, 0, {'dim': D, 'subspaces': S, 'k': k, 'seed': seed, 'sub': sub, 'rotate': True, 'pq_codes': codes})
...
def pack_transform_pq_actaware(w: np.ndarray, acts: np.ndarray, *, dim: int, subspaces: int, k: int, alpha: float=0.5, seed: int=0, iters: int=10) -> PackedArtifact:
    ...
    cols = w.shape[1]
    a = np.abs(acts).mean(axis=0).astype(np.float32)
    ...
    recon = art.recon / s[None, :]
    art.ledger.add_fp16(cols)
    return PackedArtifact('transform_pq_actaware', recon.astype(np.float32), w.size, art.ledger, art.base_bits, 0, {**art.config, 'alpha': alpha, 'act_scaled': True})

def pack_shared_grammar(experts: list[np.ndarray], *, dim: int, k: int, stages: int, corr_rank: int=0, iters: int=8, seed: int=0) -> PackedArtifact:
    ...
    vs = [torch.from_numpy(np.ascontiguousarray(w, dtype=np.float32)).to(dev).reshape(-1, D) for w in experts]
    pool = torch.cat(vs, 0)
    residual = pool.clone()
    cbs = []
    ...
    return PackedArtifact('shared_expert_grammar', recon_stack, total_weights, ledger, base_bits_total, doctor_bits_total, {'dim': D, 'k': k, 'stages': stages, 'corr_rank': corr_rank, 'n_experts': len(experts)})

def pack_repairability_shaped(w: np.ndarray, *, base_dim: int, base_k: int, corr_rank: int, sparse_rows: int, iters: int=8, seed: int=0) -> PackedArtifact:
    ...
    if sparse_rows > 0:
        ...
        cbits = sparse_rows * (cols * 16 + math.ceil(math.log2(max(2, rows))))
        ledger.add('doctor_sparse_rows', cbits)
        doctor_bits += cbits
    return PackedArtifact('repairability_shaped', recon.detach().cpu().numpy().astype(np.float32), w.size, ledger, base_bits, doctor_bits, {'base_dim': D, 'base_k': base_k, 'corr_rank': corr_rank, 'sparse_rows': sparse_rows})
...
def pack_ternary_factor(w: np.ndarray, *, rank: int, keep_frac: float=0.6, iters: int=6, seed: int=0) -> PackedArtifact:
    ...
    for j in range(rank):
        ...
        for _ in range(iters):
            ...
            a = _ternarize(resid @ b / denom_b, keep_frac)
            denom_a = float((a * a).sum()) or 1.0
            b = _ternarize(resid.t() @ a / denom_a, keep_frac)
        num = float(a @ (resid @ b))
        den = float((a * a).sum()) * float((b * b).sum()) or 1.0
        s = num / den
        recon = recon + s * torch.outer(a, b)
        resid = t - recon
        ledger.add_bits('ternary_factors', rows + cols, 2)
        ledger.add_fp16(1)
    base_bits = rank * ((rows + cols) * 2 + 16)
    return PackedArtifact('ternary_factor', recon.detach().cpu().numpy().astype(np.float32), w.size, ledger, base_bits, 0, {'rank': rank, 'keep_frac': keep_frac, 'ternary_bits': 2})

def pack_naive_rvq(w: np.ndarray, *, dim: int, k: int, stages: int, iters: int=10, seed: int=0) -> PackedArtifact:
    torch = _torch()
    dev = _device()
    rows, cols = w.shape
    D = dim if cols % dim == 0 else _largest_pow2_divisor(cols)
    ...
    for m in range(stages):
        ...
        ledger.add_fp16(cb.shape[0] * D)
    base_bits = ledger.total_bits() - _METADATA_BYTES * 8
    return PackedArtifact('naive_rvq', recon.reshape(rows, cols).detach().cpu().numpy().astype(np.float32), w.size, ledger, base_bits, 0, {'dim': D, 'k': k, 'stages': stages})
...
def output_divergence(reader, block: int, pack_fn: Callable[[np.ndarray], np.ndarray], *, n_inputs: int=8, top_k: int=4, seed: int=0) -> dict[str, Any]:
    ...
    for x in xs:
        ...
        rel.append(float(np.linalg.norm(y0 - y1) / (np.linalg.norm(y0) + 1e-09)))
    return {'n_inputs': n_inputs, 'n_experts_exercised': len(routed), 'mean_output_rel_div': round(float(np.mean(rel)), 5), 'max_output_rel_div': round(float(np.max(rel)), 5), 'min_output_rel_div': round(float(np.min(rel)), 5), 'signal': 'proxy_output_synthetic_activations', 'capability_parity': False}
...
def _make_codes(codebooks: list[np.ndarray], indices: np.ndarray, *, D: int, S: int, sub: int, rows: int, cols: int, rotate: bool, seed: int) -> dict[str, Any]:
    return {'codebooks': [np.ascontiguousarray(cb, dtype=np.float32) for cb in codebooks], 'indices': np.ascontiguousarray(indices, dtype=np.int64), 'D': int(D), 'S': int(S), 'sub': int(sub), 'rows': int(rows), 'cols': int(cols), 'nchunk': int(cols // D), 'rotate': bool(rotate), 'seed': int(seed)}
...
def _pq_encode(w: np.ndarray, *, D: int, S: int, sub: int, k: int, seed: int, iters: int, rotate: bool) -> tuple[np.ndarray, list[np.ndarray], np.ndarray, int]:
...
def pack_product_quant(w: np.ndarray, *, dim: int, subspaces: int, k: int, seed: int=0, iters: int=10) -> PackedArtifact:
    ...
    recon, codebooks, indices, N = _pq_encode(w, D=D, S=S, sub=sub, k=k, seed=seed, iters=iters, rotate=False)
    ledger = ByteLedger()
    for cb in codebooks:
        ledger.add_index(N, cb.shape[0])
        ledger.add_fp16(cb.shape[0] * sub)
    base_bits = ledger.total_bits() - _METADATA_BYTES * 8
    codes = _make_codes(codebooks, indices, D=D, S=S, sub=sub, rows=rows, cols=cols, rotate=False, seed=seed)
    return PackedArtifact('product_quant', recon, w.size, ledger, base_bits, 0, {'dim': D, 'subspaces': S, 'k': k, 'seed': seed, 'sub': sub, 'rotate': False, 'pq_codes': codes})
...
def pq_inspect(w: np.ndarray) -> dict[str, Any]:
    ...
    eff_rank = int(np.searchsorted(cume, 0.9) + 1)
    ...
    return {'rows': int(rows), 'cols': int(cols), 'valid_subvector_dims': valid, 'effective_rank_90': eff_rank, 'effective_rank_frac': round(eff_rank / max(1, min(rows, cols)), 4), 'residual_kurtosis': round(kurt, 4), 'spectral_tail_ratio': round(float(s[-1] / (s[0] or 1.0)), 6)}

def pq_fit(w: np.ndarray, *, dim: int, subspaces: int=1, k: int=16, seed: int=0, iters: int=10, rotate: bool=False) -> dict[str, Any]:
    ...
    rows, cols = w.shape
    D, S, sub = _pq_geometry(cols, dim, subspaces)
    _, codebooks, indices, N = _pq_encode(w, D=D, S=S, sub=sub, k=k, seed=seed, iters=iters, rotate=rotate)
    return {'codebooks': codebooks, 'indices': indices, 'D': D, 'S': S, 'sub': sub, 'N': N, 'k': k, 'seed': seed, 'rotate': rotate}

def pq_pack(w: np.ndarray, *, dim: int, subspaces: int, k: int, seed: int=0, iters: int=10, rotate: bool=False) -> PackedArtifact:
...
def pq_measure(w: np.ndarray, artifact: PackedArtifact) -> dict[str, Any]:
    ...
    return {'rel_error': round(err, 6), 'verdict': _verdict(err), 'whole_artifact_bpw': round(artifact.whole_artifact_bpw, 5), 'base_bpw': round(artifact.base_bpw, 5), 'doctor_bpw': round(artifact.doctor_bpw, 5), 'physical_bytes': int(artifact.physical_bytes), 'total_bits': int(artifact.ledger.total_bits())}
...
def pq_execute(artifact: PackedArtifact, x: np.ndarray) -> np.ndarray:
    ...
    B = x.shape[1]
    xc = x.reshape(nchunk, D, B)
    if codes['rotate']:
        R = _pq_rotation_np(D, codes['seed'])
        xc = np.einsum('jk,ckb->cjb', R, xc, optimize=True)
    idx = codes['indices']
    y = np.zeros((rows, B), dtype=np.float32)
    for s in range(S):
        cb = codes['codebooks'][s]
        dec = cb[idx[:, s]].reshape(rows, nchunk, sub)
        xslice = xc[:, s * sub:(s + 1) * sub, :]
    ...
    if island_rows is not None and len(island_rows) > 0:
        y[island_rows] = codes['island_vals'].astype(np.float32) @ x
    return y[:, 0] if onedim else y

def pq_validate(w: np.ndarray, artifact: PackedArtifact, x: np.ndarray, *, tol: float=0.001) -> dict[str, Any]:
    ...
    return {'within_tol': rel_vs_recon <= tol, 'tol': tol, 'rel_err_vs_recon': round(rel_vs_recon, 8), 'max_abs_err': round(max_abs, 8), 'rel_err_vs_original': round(rel_vs_orig, 6), 'no_dense_reconstruction': True}
...
def pq_repairability(w: np.ndarray, artifact: PackedArtifact) -> dict[str, Any]:
    ...
    return {'residual_rel_energy': round(rn / wn, 6), 'rank1_capture': round(cap(1), 5), 'rank4_capture': round(cap(4), 5), 'rank8_capture': round(cap(8), 5), 'sparse_row_capture': round(sparse_cap, 5), 'sparse_rows_used': int(n_rows), 'repair_score': round(score, 5)}
...
class PQFamily:
    ...
    def __init__(self, *, dim: int=32, subspaces: int=4, k: int=16, seed: int=0, iters: int=10, rotate: bool=False) -> None:
        self.dim = dim
    ...
    def fit(self, w: np.ndarray, *, dim: int | None=None, k: int | None=None, seed: int | None=None) -> dict[str, Any]:
        return pq_fit(w, dim=self.dim if dim is None else dim, subspaces=self.subspaces, k=self.k if k is None else k, seed=self.seed if seed is None else seed, iters=self.iters, rotate=self.rotate)

    def pack(self, w: np.ndarray) -> PackedArtifact:
        return pq_pack(w, dim=self.dim, subspaces=self.subspaces, k=self.k, seed=self.seed, iters=self.iters, rotate=self.rotate)
    ...
    def validate(self, w: np.ndarray, artifact: PackedArtifact, x: np.ndarray, *, tol: float=0.001) -> dict[str, Any]:
...
def select_protected_islands(w: np.ndarray, residual: np.ndarray | None=None, *, strategy: str='magnitude', budget_frac: float=0.03, activation: np.ndarray | None=None, sensitivity: np.ndarray | None=None) -> dict[str, Any]:
    ...
    return {'strategy': strategy, 'budget_frac': budget_frac, 'n_islands': int(n), 'row_indices': rowsel, 'score_captured': float(np.asarray(score)[rowsel].sum())}

def pack_pq_protected_islands(w: np.ndarray, *, dim: int, subspaces: int, k: int, strategy: str='residual_energy', budget_frac: float=0.03, seed: int=0, iters: int=10, rotate: bool=False, activation: np.ndarray | None=None, sensitivity: np.ndarray | None=None) -> PackedArtifact:
    ...
    isl = select_protected_islands(w, resid, strategy=strategy, budget_frac=budget_frac, activation=activation, sensitivity=sensitivity)
    rowsel = isl['row_indices']
    recon = base.recon.copy()
    recon[rowsel] = w[rowsel].astype(np.float32)
    ledger = ByteLedger()
    for name, bits in base.ledger.items.items():
        ledger.add(name, bits)
    island_bits = int(len(rowsel)) * (cols * 16 + max(1, math.ceil(math.log2(max(2, rows)))))
    ledger.add('protected_islands', island_bits)
    ...
    cfg = {**base.config, 'strategy': strategy, 'budget_frac': budget_frac, 'n_islands': int(len(rowsel)), 'protected': True, 'pq_codes': codes}
    return PackedArtifact('pq_protected_islands', recon.astype(np.float32), w.size, ledger, base_bits, 0, cfg)
_DOCTOR_TREATMENTS = ('residual_codebook', 'sparse_residual', 'per_channel_scale', 'protected_island_expansion')

def doctor_pq(w: np.ndarray, base_artifact: PackedArtifact, *, byte_budget: int, strategy: str, seed: int=0, iters: int=8) -> dict[str, Any]:
    ...
    if strategy == 'per_channel_scale':
    ...
    elif strategy == 'residual_codebook':
        D = int(base_artifact.config.get('dim', 32))
        chosen = None
        for sub_s in (base_artifact.config.get('subspaces', 4), max(1, D // 4), D):
            for kk in (4, 8, 16, 2):
                art_r = pack_product_quant(resid, dim=D, subspaces=int(sub_s), k=kk, seed=seed, iters=iters)
        ...
        evidence = {'stage2_bpw': round(chosen.whole_artifact_bpw, 5), 'stage2_k': chosen.config['k'], 'stage2_subspaces': chosen.config['subspaces']}
    else:
        D = int(base_artifact.config.get('dim', 32))
    ...
    return {'treatment': strategy, 'added_bytes': int(added_bytes), 'new_whole_bpw': round(new_whole_bpw, 6), 'quality_delta': round(err0 - err1, 8), 'err_before': round(err0, 8), 'err_after': round(err1, 8), 'within_budget': added_bytes <= byte_budget, 'evidence': evidence}
...
def _pq_dual_fit(w: np.ndarray, *, D: int, S: int, sub: int, k: int, seed: int, iters: int) -> dict[str, Any]:
    ...
    return {'relerr_cpu': _rel_error(v_all, recon_cpu), 'relerr_metal': _rel_error(v_all, recon_metal), 'assignment_agreement': float(np.mean(agree))}

def pq_cpu_metal_parity(w: np.ndarray, *, dim: int, k: int, seed: int=0, iters: int=10, subspaces: int=1, tol: float=0.002) -> dict[str, Any]:
    ...
    return {'device': _device().type, 'authoritative': 'cpu', 'D': int(D), 'S': int(S), 'k': int(k), 'candidate_ks': [int(c) for c in cand_ks], 'codebook_relerr_cpu': round(main['relerr_cpu'], 6), 'codebook_relerr_metal': round(main['relerr_metal'], 6), 'relerr_delta': round(float(delta), 8), 'tol': tol, 'assignment_agreement': round(main['assignment_agreement'], 5), 'ranking_cpu': [int(r) for r in rank_cpu], 'ranking_metal': [int(r) for r in rank_metal], 'ranking_match': rank_cpu == rank_metal, 'verdict_cpu': _verdict(main['relerr_cpu']), 'verdict_metal': _verdict(main['relerr_metal']), 'pass_match': _verdict(main['relerr_cpu']) == _verdict(main['relerr_metal']), 'within_tol': float(delta) <= tol, 'note': 'CPU (numpy) authoritative; MPS reductions may reorder near-ties'}

def selftest() -> dict[str, Any]:
    rng = np.random.default_rng(0)
    lr = rng.standard_normal((256, 8)).astype(np.float32) @ rng.standard_normal((8, 256)).astype(np.float32) * 0.1
    ...
    a = pack_transform_pq(lr, dim=32, subspaces=4, k=16)
    ...
    val = fam.validate(lr, art_pq, x)
    ins = fam.inspect(lr)
    pq_det = fam.pack(lr).physical_bytes == fam.pack(lr).physical_bytes
    isl_art = pack_pq_protected_islands(lr, dim=32, subspaces=4, k=16, budget_frac=0.05, strategy='residual_energy')
    islands_increase = isl_art.whole_artifact_bpw > art_pq.whole_artifact_bpw and 'protected_islands' in isl_art.ledger.items and (isl_art.whole_artifact_bpw >= isl_art.base_bpw + isl_art.doctor_bpw - 1e-06)
    ...
    det = pack_naive_rvq(lr, dim=32, k=64, stages=2, seed=7).physical_bytes == pack_naive_rvq(lr, dim=32, k=64, stages=2, seed=7).physical_bytes
    acct_ok = all((p.physical_bytes > 0 and p.whole_artifact_bpw >= p.base_bpw + p.doctor_bpw - 1e-06 and np.isfinite(p.recon).all() for p in (a, gram, rep, tern, art_pq, isl_art)))
    return {'ok': True, 'device': _device().type, 'lowrank_family_relerr': round(err_lr, 5), 'random_family_relerr': round(err_rnd, 5), 'compressible_beats_random': err_lr < err_rnd and err_lr < 0.05, 'transform_pq_bpw': round(a.whole_artifact_bpw, 4), 'shared_grammar_bpw': round(gram.whole_artifact_bpw, 4), 'repairability_bpw': round(rep.whole_artifact_bpw, 4), 'repairability_base_bpw': round(rep.base_bpw, 4), 'repairability_doctor_bpw': round(rep.doctor_bpw, 4), 'ternary_factor_bpw': round(tern.whole_artifact_bpw, 4), 'families_available': 4, 'deterministic_bytes': det, 'accounting_invariant_holds': acct_ok, 'pq_family_verbs': len(PQFamily.VERBS), 'pq_whole_bpw': round(art_pq.whole_artifact_bpw, 4), 'pq_effective_rank_90': ins['effective_rank_90'], 'pq_execute_within_tol': val['within_tol'], 'pq_execute_rel_err_vs_recon': val['rel_err_vs_recon'], 'pq_deterministic_bytes': pq_det, 'pq_islands_increase_bpw': islands_increase, 'pq_doctor_reduces_error': doctor_ok, 'pq_doctor_within_budget': bool(doc['within_budget']), 'pq_cpu_metal_within_tol': parity_ok}
