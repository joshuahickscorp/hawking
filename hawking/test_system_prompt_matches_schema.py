...
`HAWKING_RESULT_SCHEMA` lists `tool_calls` in `required`, but two of the three
...
from hawking.backends import validate_against_schema
from hawking.engine import HAWKING_RESULT_SCHEMA, _SYSTEM_PROMPT
...
def test_each_worked_example_validates(index):
    ...
    err = validate_against_schema(example, HAWKING_RESULT_SCHEMA)
    assert err is None, (
...
def test_the_probe_reads_the_live_schema_not_a_copy():
    ...
    assert probe.RESULT_SCHEMA is HAWKING_RESULT_SCHEMA
