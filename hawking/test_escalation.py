"""Tests for hawking/escalation.py and its two tool_registry.py registrations.
...
from hawking.escalation import (
    MAX_ARTIFACTS,
    MAX_SWARM_LANES,
...
from hawking.grok_bridge import GrokContractError, GrokNotAvailable
from hawking.tool_registry import COSTLY, READ_ONLY, RESEARCH, REVERSIBLE_REPO, REVERSIBLE_RUNTIME, default_tool_registry
...
def test_propose_swarm_is_unbounded_by_default_and_keeps_explicit_policy():
    assert MAX_SWARM_LANES is None
    lanes = [{"name": f"lane{i}", "objective": "x", "write_scope": ["a"], "verify_command": "pytest a"} for i in range(16)]
    assert propose_swarm("problem", lanes)["lane_count"] == 16
    with pytest.raises(SwarmBoundsError):
        propose_swarm("problem", lanes, max_lanes=4)
    dupes = [
...
def test_escalate_fails_closed_with_no_credential_and_never_calls_the_network(monkeypatch):
    ...
    monkeypatch.setattr("hawking.escalation.urllib.request.urlopen", _must_not_be_called)
...
def test_escalate_wraps_a_successful_call_as_an_unverified_hypothesis(monkeypatch):
    ...
    monkeypatch.setattr("hawking.escalation.urllib.request.urlopen", _fake_urlopen)
