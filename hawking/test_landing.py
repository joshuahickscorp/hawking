"""The one remaining self-hosting gap: HAWKING could read and test its own code
...
These tests drive the REAL `hawking.landing` module and the REAL `ToolRegistry`
...
from hawking._test_git import scratch_repo

from hawking.landing import LandingService
from hawking.tool_registry import DESTRUCTIVE, READ_ONLY, default_tool_registry
...
def _repo(tmp_path: Path) -> Path:
    ...
    return scratch_repo(
        tmp_path / "repo",
        email="hawking-landing-test@example.com",
        name="hawking-landing-test",
        filename="README.md",
        body="scratch repo for hawking.landing tests\n",
    )
...
def _propose(repo: Path, **overrides):
    args = {
        ...
        "message": "feat(hawking): add the feature file",
    }
...
def test_clean_proposal_lands_a_real_commit(tmp_path):
    ...
    assert "feat(hawking): add the feature file" in body
    assert "Landed-By: hawking-landing-service" in body
...
def test_refusal_path_touches_governance_source(tmp_path):
    ...
    result = _propose(repo, allowed_paths=["hawking/landing.py"])
...
def test_every_file_that_can_weaken_the_check_is_refused(tmp_path):
    ...
    hawking/tool_registry.py LANDED, and that file defines every tool's mutation
    ...
    """
    from hawking.landing import _ALWAYS_REFUSED_PREFIXES, IntegrationVerifier

    for target in (
        "hawking/landing.py",
        "hawking/tool_registry.py",
        "hawking/verifier_pipeline.py",
        "hawking/executors.py",
    ):
        assert target in _ALWAYS_REFUSED_PREFIXES, target
        repo = _repo(_sub(tmp_path, target.replace("/", "_")))
        (repo / "hawking").mkdir(exist_ok=True)
...
def _landing_proposal(repo: Path, **overrides):
    from hawking.landing import LandingProposal
...
def test_a_test_command_cannot_smuggle_a_governance_file_into_the_commit(tmp_path):
    ...
    feature.txt landed a commit containing hawking/landing.py overwritten with
    ...
    """
    from hawking.landing import LandingProposal, LandingService

    repo = _repo(_sub(tmp_path, "tamper"))
    (repo / "hawking").mkdir(exist_ok=True)
    (repo / "hawking" / "landing.py").write_text("GOVERNANCE = 'strict'\n", encoding="utf-8")
    ...
    attack = (
        ...
        'Path("hawking/landing.py").write_text("PWNED_GOVERNANCE\\n"); '
        'subprocess.check_call(["git","add","--","hawking/landing.py"]); print("ok")'
    )
