"""`hawking report` -- what is this resident actually like to use?

RUNNABLE, NOT TRANSCRIBED. Every performance claim about HAWKING so far has been a
...
def render(report: Dict[str, Any]) -> str:
    ...
    out.append(f"HAWKING REPORT   resident {resident.get('resident', 'unknown')}"
...
def main(argv: Optional[list] = None) -> int:
    ...
    # POSITIONAL, so `hawking report Qwen3-14B` measures that body: it switches to
    ...
    if a.model:
        ...
        # one will do, and the switch is the same door `hawking use` knocks on.
    ...
    if resident is None:
        print(f"nothing is answering on {a.base}/health.\n"
              f"Start one with:  python -m hawking serve --port "
              f"{a.base.rsplit(':', 1)[-1] or '8011'}", file=sys.stderr)
        return 2

    skip = {s.strip() for s in a.skip.split(",") if s.strip()}
    report: Dict[str, Any] = {
        "schema": "hawking.report.v2",
        "timing_unit": "ns",
        "base_url": a.base,
        "resident": resident,
