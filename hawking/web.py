...
it up instead: start `hawking serve` (the OpenAI surface over the persistent
...
from .cli import (
    DEFAULT_ENDPOINT_HOST as DEFAULT_HOST,
    DEFAULT_ENDPOINT_MODEL as DEFAULT_MODEL,
    DEFAULT_ENDPOINT_PORT as DEFAULT_PORT,
)
from .process_identity import branded_python_entrypoint

WEBUI_PORT = 8080
STATE_DIR = Path.home() / ".hawking" / "web"
...
class OwnedWebUIManager:
    ...
    Python process, so letting ``hawking web`` spawn it made the process tree lie
    ...
    def start(self, port: int, base_url: str,
        ...
        # the daemon must not reap the UI when `hawking web` exits.  Positive
        ...
        with self._lock:
            ...
            "owner": "hawkingd" if requester_pid == 0 else "hawking-client",
                "data_dir": data_dir,
                "log": log,
                "base_url": base_url,
            }
    ...
    def _row(row: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "owner": str(row.get("owner") or "hawking-client"),
...
def surface_health(host: str, port: int) -> Optional[Dict[str, Any]]:
    ...
    if isinstance(body, dict) and isinstance(body.get("resident"), str) and body.get("resident") and body.get("state", "ready") == "ready":
        return body
    return None


def daemon_health_for_stop(host: str, port: int) -> Optional[Dict[str, Any]]:
    """Identify this Hawking daemon for controlled recovery, even when waiting.

    A canonical daemon with no admitted native artifact deliberately reports
    ``waiting_for_native_runtime`` and has no resident identity.  That state is
    not a reusable cognition surface, but its lease plus health owner is still
    sufficient to identify the exact Hawking daemon that a qualified restart
    path may stop.
    """
    body = _get_json(f"http://{host}:{port}/health")
    ...
    try:
        ...
        pid = 0
    if str(owner.get("daemon") or "") != "hawkingd" or pid <= 1:
        return None
    state = str(body.get("state") or "").strip().lower()
    if state == "waiting_for_native_runtime":
        return body
    return surface_health(host, port)


def reconcile_requested_resident(existing: Dict[str, Any], requested: Any,
                                 base_url: str, timeout: float) -> Dict[str, Any]:
    """Make an explicit Web selection true or refuse before opening the UI."""
    if isinstance(existing.get("canonical_runtime"), dict):
        # Canonical hawkingd owns Web as a client of the same native-only
        # surface.  Select a logical role or immutable native artifact ID;
        # never pass a caller-supplied resolved action back through /v1.
        ...
        selector = requested.name if isinstance(requested, ResolvedAction) else str(requested)
        try:
            route = CanonicalRoleRouter().route(selector)
        except RuntimeRoleUnavailable as exc:
            raise RuntimeError(str(exc)) from exc
        raw_loaded = existing.get("resolved_action")
        try:
            loaded = ResolvedAction.from_wire(raw_loaded) if isinstance(raw_loaded, dict) else None
        except (TypeError, ValueError):
            loaded = None
        if loaded is not None and route.action.has_same_binding(loaded):
            return existing
        control_base = base_url.rstrip("/")
        if control_base.endswith("/v1"):
            control_base = control_base[:-3]
        status, body = post_json(
            control_base + "/api/runtime/switch", {"model": selector}, timeout,
        )
        if status != 200:
        ...
        updated.update(body)
        updated["resident"] = body.get("resident") or route.action.name
        updated["model"] = route.action.path
        if isinstance(body.get("resolved_action"), dict):
            updated["resolved_action"] = body["resolved_action"]
        return updated
    from .catalog import (
        ResolvedAction,
        resolve_action,
    ...
    try:
        target = (
            revalidate_action(requested, action="web")
            if isinstance(requested, ResolvedAction)
            else resolve_action(str(requested), "web")
        )
    ...
    live = existing.get("resolved_action")
    try:
        loaded = revalidate_action(ResolvedAction.from_wire(live)) \
            if isinstance(live, dict) else None
        if loaded is not None and loaded.action not in {"serve", "web"}:
            loaded = None
    except (LookupError, PermissionError, ValueError):
        loaded = None
    if (
        loaded is not None
        and loaded.action == target.action
        and target.has_same_binding(loaded)
    ):
        return existing
    status, body = post_json(base_url.rstrip("/") + "/switch",
                             {"resolved_action": target.to_wire()}, timeout)
    if status != 200:
        message = (body.get("error") or {}).get("message") or body
        raise RuntimeError(f"resident switch refused ({status}): {message}")
    updated = dict(existing)
...
def revalidate_resident_for_web(
    existing: Dict[str, Any], *, base_url: str, timeout: float,
) -> Dict[str, Any]:
    """Prove that an implicit resident remains admitted for the Web surface."""
    if isinstance(existing.get("canonical_runtime"), dict):
        from .canonical_runtime import CanonicalRoleRouter, RuntimeRoleUnavailable
        from .catalog import ResolvedAction

        raw = existing.get("resolved_action")
        if not isinstance(raw, dict):
            raise RuntimeError(
                "the canonical running resident does not publish an exact native action binding"
            )
        try:
            loaded = ResolvedAction.from_wire(raw)
            route = CanonicalRoleRouter().route(loaded.name)
        except (RuntimeRoleUnavailable, TypeError, ValueError) as exc:
            raise RuntimeError(
                f"the running canonical resident is no longer admitted: {exc}"
            ) from exc
        if not route.action.has_same_binding(loaded):
            raise RuntimeError(
                "the running canonical resident no longer matches its current native admission binding"
            )
        return existing
    ...
    if not isinstance(raw, dict):
        raise RuntimeError(
            "the running resident does not publish a resolved action binding; "
            "select an admitted Web artifact explicitly"
        )
    try:
        loaded = revalidate_action(ResolvedAction.from_wire(raw))
        if loaded.action not in {"serve", "web"}:
            raise PermissionError(
                f"the running contract is {loaded.action!r}, not a resident action"
            )
        web_action = resolve_action(loaded.path, "web")
    except (LookupError, PermissionError, ValueError) as exc:
        raise RuntimeError(f"the running resident is no longer admitted for Web: {exc}") from exc
    if web_action is None or not web_action.has_same_binding(loaded):
        raise RuntimeError(
            "the running resident no longer has the current admitted Web binding; "
            "select an admitted Web artifact explicitly"
        )
    # A same-body serve contract is not a Web contract.  Re-enter the one
    # resident switch boundary with the full Web record; it will avoid a
    # reload when the physical binding matches, while publishing the action
    # that this caller actually requested.
    return reconcile_requested_resident(existing, web_action, base_url, timeout)
...
def start_surface(model: str, host: str, port: int, log_dir: Path,
                  write: bool = False, resolved_action: Any = None) -> tuple:
    ...
    daemon_argv = (
        [daemon, "serve"]
        if daemon
        else [sys.executable, "-P", "-m", "hawking.hawkingd", "serve"]
    )
    if resolved_action is not None:
        from .catalog import ResolvedAction, resolved_action_json, revalidate_action
        if not isinstance(resolved_action, ResolvedAction):
            raise TypeError("start_surface requires a ResolvedAction value")
        resolved_action = revalidate_action(resolved_action, action="web")
        model = resolved_action.path
        selection_argv = ["--resolved-action-json", resolved_action_json(resolved_action)]
    else:
        selection_argv = ["--model", model]
    proc = subprocess.Popen(
        [*daemon_argv, *selection_argv,
         "--host", host, "--port", str(port), *(["--write"] if write else [])],
        # The surface inherits the directory the USER ran `hawking web` in, not the
...
def start_webui(base_url: str, port: int, log_dir: Path,
    ...
    env.update({
        ...
        "WEBUI_AUTH": "False",
        # Keep the existing Open WebUI shell, but make the Hawking review
        # surface legible on first launch. Kimi P0 is displayed as Hawking;
        # its legacy runtime remains withheld by canonical hawkingd.
        "WEBUI_NAME": "Hawking",
        "DEFAULT_MODELS": "KIMI_P0_OPERATIONAL",
        "DEFAULT_PINNED_MODELS": "KIMI_P0_OPERATIONAL",
        "ENABLE_OLLAMA_API": "False",
        # Open WebUI injects a built-in "Arena Model" even when the upstream
        # model endpoint returns only admitted bodies.  HAWKING's normal surface
        ...
        # the real turn for GPU time and each entered HAWKING's tool loop as if it
        ...
        # process environment above. HAWKING owns this constrained local surface,
...
def stop_daemon_webui(base_url: str, port: int, requester_pid: int | None = None) -> None:
    """Ask the daemon to stop the UI client created by this HAWKING session."""
    root = base_url.rsplit("/v1", 1)[0].rstrip("/")
    request = urllib.request.Request(
...
def next_webui_port(host: str, requested: int = WEBUI_PORT) -> int:
    ...
    for port in range(int(requested), int(requested) + 64):
        ...
        # second HAWKING client never mistakes an occupied UI port for a free
        # one.
        try:
...
def enforce_gravity_only_webui(
    ...
    arena policy can leave a healthy UI dialing a stale HAWKING port.  The
    per-port database belongs to HAWKING; bind it to the exact health-checked
...
def main(argv: Optional[list] = None) -> int:
    ...
    # POSITIONAL, because `hawking web Qwen3-14B` is what a person types. --model
    ...
    ap.add_argument("--write", action="store_true",
                    help="grant repo-scoped write authority (see `hawking build`)")
    ...
    if a.resolved_action_json:
        ...
        try:
            selected_action = resolved_action_from_json(
                a.resolved_action_json, action="web"
            )
            ...
            # hawkingd re-resolves immutable IDs through its native-only
            # public router; a caller-supplied path is not a provider selector.
    ...
    from .canonical_runtime import CanonicalRoleRouter, RuntimeRoleUnavailable
    try:
        canonical_route = CanonicalRoleRouter().route(model)
    except RuntimeRoleUnavailable as exc:
        print(f"REFUSED: canonical Hawking Web cannot start {model!r}: {exc}",
              file=sys.stderr)
        return 2

    # ``hawking build`` is this launcher with ``--write`` appended.  Check the
    # exact native artifact authority before inspecting or creating a resident.
    # The canonical builder repeats this at provider construction.
    if a.write:
        from .catalog import require_write_session_authority

        try:
            if canonical_route.action.body is None:
                raise LookupError(f"{model!r} has no admitted native artifact body")
            require_write_session_authority(canonical_route.action.body)
        except (LookupError, PermissionError, ValueError) as exc:
            print(f"REFUSED: {exc}", file=sys.stderr)
            return 2
    ...
    if existing:
        if a.model:
            ...
            try:
                existing = revalidate_resident_for_web(
                    existing, base_url=base_url, timeout=a.switch_timeout,
                )
            except RuntimeError as exc:
                print(f"REFUSED: {exc}", file=sys.stderr)
                return 2
        ...
        target = canonical_route.action
        model = target.name
        print(f"starting native resident {model} ...", flush=True)
    ...
    print(json.dumps({
        ...
        # The browser chat and the bounded Odyssey operator are served by the
        # same Hawking-owned endpoint.  Keep this in the machine-readable
        # launcher result so scripts do not have to reconstruct the route.
        "goal_operator_url": f"http://{a.host}:{a.port}/api/goal/ui",
    ...
    if a.no_webui:
        ...
        print(f"Bounded Goal operator: http://{a.host}:{a.port}/api/goal/ui")
        return 0

    # Never attach a new `hawking web` invocation to another UI process.  Pick a
    ...
    # wrapper per browser/blind session.  `hawking stop` and the daemon control
...
def stop_main(argv: Optional[list] = None) -> int:
    """`hawking stop` -- put down what `hawking web` or `hawking serve` started.
    ...
    ap = argparse.ArgumentParser(
        prog="hawking stop", description="Stop the browser chat and its endpoint.")
    ap.add_argument("--port", type=int, default=DEFAULT_PORT)
    ap.add_argument("--webui-port", type=int, default=WEBUI_PORT)
    a = ap.parse_args(list(argv or []))
    ...
    # hawking.hawkingd`` in argv, so pgrep-by-title missed the real daemon and
    ...
    from .hawkingd import daemon_lease_metadata
    incumbent = daemon_lease_metadata()
    health = daemon_health_for_stop(DEFAULT_HOST, a.port)
    # The canonical default is :8014, but an already-running explicitly
    # selected port remains the same single daemon owner.  Prefer a live
    # health observation at the lease's port to an argv fallback; this is both
    # safer and makes the port migration non-destructive for a running user.
    try:
        hinted_port = int(incumbent.get("port") or 0)
    except (TypeError, ValueError):
        hinted_port = 0
    if hinted_port and hinted_port != a.port:
        hinted_health = daemon_health_for_stop(DEFAULT_HOST, hinted_port)
        if hinted_health is not None:
            a.port = hinted_port
            health = hinted_health
    try:
    ...
    for label, pattern in (("endpoint", f"hawkingd serve :{a.port}"),
                           ("endpoint", f"hawking serve .*--port {a.port}"),
                           ("endpoint", "hawking serve"),
