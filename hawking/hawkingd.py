...
`hawking` is the client; hawkingd is what keeps running after the client exits.
The split matters for more than tidiness: `ps` should describe the process, and
"python3 -m hawking.resident --supervise <state>" describes the
...
``hawking.resident`` because the daemon needs a stable, model-neutral body
module rather than an implementation-leg package.
...
long-lived launch path; ``hawking serve`` remains a compatibility alias that
...
second model into unified memory.  Canonical providers are private native
children or IPC peers created and stopped by this daemon; historical MLX
adapters remain outside the public resident path and never own a Hawking
launch surface.
"""
...
from hawking.resident import daemon_main


DEFAULT_DAEMON_LOCK = Path.home() / ".hawking" / "hawkingd.lock"
...
def acquire_daemon_lease(role: str, metadata: Optional[dict] = None) -> DaemonLease:
    ...
    controlled isolated runs may override it with ``HAWKING_DAEMON_LOCK``.
    """
    raw_path = os.environ.get("HAWKING_DAEMON_LOCK")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if raw and raw[0] == "serve":
        from hawking.serve import main as serve_main
