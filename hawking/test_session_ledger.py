...
`hawking.landing.IntegrationVerifier`'s git subprocess wrapper the same way
`SessionLedger` does, so one real git repo fixture serves every test in this
file.

Runnable two ways:

    python3 -m pytest hawking/test_session_ledger.py -q
    python3 hawking/test_session_ledger.py
"""
...
from hawking.session_ledger import SessionLedger, changed_paths, discover_repo_root
...
def test_discover_repo_root_finds_the_scratch_repo_not_the_hawking_checkout(tmp_path):
    """`find_repo_root` (paths.py) is shaped to find HAWKING's own checkout and
...
def _with_a_passing_test(repo: Path) -> None:
    """/land's default test_command is `python3 -m pytest hawking/ -q` -- give
    the scratch repo a trivial `hawking/` so that command has something real to
    ...
    directory is one `git status --porcelain` line ("?? hawking/"), not one per
    ...
    """
    hawking_dir = repo / "hawking"
    hawking_dir.mkdir(exist_ok=True)
    keep = hawking_dir / ".gitkeep"
    if not keep.exists():
        keep.write_text("", encoding="utf-8")
        subprocess.run(["git", "-C", str(repo), "add", "hawking/.gitkeep"], check=True, capture_output=True)
        subprocess.run(["git", "-C", str(repo), "commit", "-q", "-m", "seed hawking dir"], check=True, capture_output=True)
    (hawking_dir / "test_ok.py").write_text("def test_ok():\n    assert True\n", encoding="utf-8")
