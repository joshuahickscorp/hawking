...
from hawking.engine import HAWKING_RESULT_SCHEMA, Engine
...
def test_the_schema_lets_the_model_ask_for_what_the_executor_will_run():
    ...
    declared = HAWKING_RESULT_SCHEMA["properties"]["tool_calls"]["maxItems"]
    assert declared == Engine.MAX_TOOL_CALLS_PER_ROUND


def test_the_prompt_tells_the_model_what_a_round_costs():
    from hawking.engine import _SYSTEM_PROMPT
...
def test_a_repeated_FAILURE_says_it_will_keep_failing():
    ...
    eng = _engine(reg)
    eng._run_tool_calls([_call("hawking")], "g")
    again = eng._run_tool_calls([_call("hawking")], "g")
