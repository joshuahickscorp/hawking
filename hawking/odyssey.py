...
already SEALED. HAWKING exposes a resident-facing Odyssey surface through this
...
* **inspect** -- ``status``, ``queue``, ``value``, ``economics``,
  ...
  reason is recorded at its definition. The execution boundary is therefore
  explicit: this module never exposes a verb that can reclaim disk or mutate
  persistent Odyssey state without ``confirm=True``. The boundary is enforced
  at the registry layer, not merely documented here: every mutating verb is
  registered with a ``confirm`` gate that raises ``PermissionError`` unless
  ``confirm=True`` is passed explicitly. ``admit_check`` remains unregistered
  for exactly this reason and must not be re-added without a ``confirm`` gate.
  The registry layer is the single enforcement point: a mutating verb that is
  absent from the registry cannot be reached through this module at all, so the
  boundary holds even if a caller passes ``confirm=True``.
...
* **continue** -- ``harvest``, ``write_packet``, ``run``, ``cycle``,
  ...
  in hawking/tool_registry.py (``if args.get("confirm") is not True: raise
...
* **own** -- ``ingest``, ``add_to_eligibility``, ``park_specimen``,
  ...
  ``create_adversarial_probe``. These give HAWKING a campaign of its own
  ...
  verbs HAWKING calls once it has decided that for itself. All writes land in
  ``HAWKING_LEDGER.json``, a file this module owns exclusively, alongside but
...
#: Verbs that mutate persistent Odyssey state, spawn a subprocess, or start a
#: download. Every name here must be registered with a ``confirm`` gate; the
#: registry layer is the single enforcement point for the execution boundary.
#: ``MUTATING_VERBS`` is the authoritative, importable list of those names; the
#: registry is checked against it at import time so a mutating verb can never be
#: registered without a ``confirm`` gate, and a gated verb can never be dropped
...
MUTATING_VERBS: tuple[str, ...] = (
    ...
    "record_law",
    "record_scar",
    "create_transfer_probe",
    "create_adversarial_probe",
)

#: Verbs that are read-only and must never be registered with a ``confirm``
#: gate, because they cannot mutate persistent Odyssey state or spend a
#: resource. ``admit_check`` is deliberately absent from the registry entirely
#: (see the module docstring): it is not read-only, so it is neither here nor
#: registered.
...
def _assert_execution_boundary(registry: dict[str, Any]) -> None:
    """Fail closed if the registry violates the execution boundary.

    Called at import time with the live registry. Raises ``RuntimeError`` if a
    mutating verb is registered without a ``confirm`` gate, if a read-only verb
    is registered with one, or if ``admit_check`` is registered at all. This is
    the executable form of the boundary the module docstring describes: the
    check runs on every import, so a regression cannot ship silently.
    """
    for name in MUTATING_VERBS:
        entry = registry.get(name)
        if entry is None:
            raise RuntimeError(
                f"execution boundary: mutating verb {name!r} is not registered"
            )
        if not entry.get("confirm"):
            raise RuntimeError(
                f"execution boundary: mutating verb {name!r} lacks a confirm gate"
            )
    for name in READ_ONLY_VERBS:
        entry = registry.get(name)
        if entry is None:
            raise RuntimeError(
                f"execution boundary: read-only verb {name!r} is not registered"
            )
        if entry.get("confirm"):
            raise RuntimeError(
                f"execution boundary: read-only verb {name!r} must not gate on confirm"
            )
    if "admit_check" in registry:
        raise RuntimeError(
            "execution boundary: admit_check must not be registered; it can "
            "reclaim disk via reclaim_if_tight()"
        )
...
#: Verbs that are deliberately absent from the registry because they can
#: reclaim disk or otherwise mutate state without a ``confirm`` gate. They must
#: never be reachable through this module.
UNREGISTERED_VERBS = frozenset({"admit_check"})


# cli-only: registry-construction audit; registering it separately creates a circular authority surface.
def audit_execution_boundary(registry: Any) -> dict:
    """Audit a verb registry against the declared execution boundary.

    ``registry`` is any mapping of verb name to callable (the shape
    ``hawking.tool_registry`` builds). The audit is read-only: it inspects the
    registry and returns a report, never mutating it.

    A verb is *gated* when its callable declares a ``confirm`` parameter, which
    is how every mutating verb in this module enforces ``confirm=True``. The
    report is ``ok`` only when every mutating verb is present and gated, and no
    unregistered verb is reachable at all.
    """
    import inspect

    names = set(registry)
    missing = sorted(MUTATING_VERBS - names)
    ungated = []
    for name in sorted(MUTATING_VERBS & names):
        try:
        ...
        if "confirm" not in params:
            ungated.append(name)
    leaked = sorted(UNREGISTERED_VERBS & names)
    return {
        "ok": not missing and not ungated and not leaked,
        "missing": missing,
        "ungated": ungated,
        "leaked": leaked,
        "mutating": sorted(MUTATING_VERBS),
        "unregistered": sorted(UNREGISTERED_VERBS),
    }
...
LEDGER_PATH = ODYSSEY_DIR / "HAWKING_LEDGER.json"  # HAWKING-owned; this module is the only writer
...
def _run(args: list[str], timeout_s: float = 60.0) -> dict[str, Any]:
    ...
    return {
        ...
        # read-only HAWKING call, not whether admission was granted.
...
def patient(oxx: str) -> dict:
    ...
    if not path.is_file():
        return {"schema": "hawking.odyssey.patient.v1", "oxx": oxx, "found": False, "path": str(path)}
    return {
        "schema": "hawking.odyssey.patient.v1",
        "oxx": oxx,
        "found": True,
        "path": str(path),
...
def run(confirm: bool = False, dry_run: bool = True, max_lanes: int = 2, grok_lanes: int = 0) -> dict:
    if grok_lanes:
        raise PermissionError("Grok lanes are retired; Odyssey is Hawking-native only")
    args = ["run", "--max-lanes", str(max_lanes)]
    if dry_run:
        return _run([*args, "--dry-run"])
    _require_confirm(confirm, "run --go (Hawking-owned Odyssey work)")
...
def cycle(
    ...
    if grok_lanes:
        raise PermissionError("Grok lanes are retired; Odyssey is Hawking-native only")
    args = ["cycle", "--max-lanes", str(max_lanes)]
...
def gravity_gauntlet(
    ...
    HAWKING owns the checkpoint and next-candidate decision; the existing patient
...
# --------------------------------------------------------------------------
# own -- HAWKING's own ledger on top of the driver's real state. ingest() reads
# both; everything else writes only HAWKING_LEDGER.json and gates on confirm.
# --------------------------------------------------------------------------
...
def _writer_identity() -> dict:
    ...
    ``hawking_owned`` is true only when the writing process shares the live
    resident worker's session. The supervisor spawns that worker with
    ``start_new_session=True`` (hawking/agentos/resident.py), so the worker leads
    ...
    HAWKING's own or a person's. A failure to determine ownership stamps false,
    never true -- the safe direction.
    """
    owned = False
    try:
        state = json.loads((REPO / ".hawking" / "resident" / "state.json").read_text())
        ...
        owned = False
    return {"writer_pid": os.getpid(), "hawking_owned": owned}
...
def _require_known_oxx(oxx: str) -> None:
    """Refuse to record a specimen HAWKING invented. Skipped only when the
...
def _empty_ledger() -> dict:
    return {"schema": "hawking.odyssey.ledger.v1", **{k: [] for k in _LEDGER_LISTS}}
...
def ingest() -> dict:
    """The real, mid-flight Odyssey state plus HAWKING's own ledger, in one
    shape HAWKING can reason over. Read-only: never seeds, re-runs, or touches
    a sealed patient -- O003 stays exactly as sealed as it was found."""
    specimens = _real_specimens()
    ledger = _load_ledger()
    return {
        "schema": "hawking.odyssey.ingest.v1",
...
def add_to_eligibility(oxx: str, note: str = "", confirm: bool = False) -> dict:
    """Mark a real specimen eligible for HAWKING-driven work. HAWKING decides what
    ...
    _save_ledger(ledger)
    return {"schema": "hawking.odyssey.ledger.v1", "list": "eligible", "entry": entry}
...
def park_specimen(oxx: str, reason: str, confirm: bool = False) -> dict:
    ...
    _save_ledger(ledger)
    return {"schema": "hawking.odyssey.ledger.v1", "list": "parked", "entry": entry}


def record_law(text: str, evidence: str = "", source_oxx: Optional[str] = None,
               domain: str = "", reopen_when: str = "", confirm: bool = False) -> dict:
    """Record a claimed compiler/architecture law. HAWKING decides what counts
    ...
    _save_ledger(ledger)
    return {"schema": "hawking.odyssey.ledger.v1", "list": "laws", "entry": entry}
...
def record_scar(law_id: str, description: str, reopen_when: str = "",
    ...
    _save_ledger(ledger)
    return {"schema": "hawking.odyssey.ledger.v1", "list": "scars", "entry": entry}
...
def create_transfer_probe(law_id: str, target_oxx: str, confirm: bool = False) -> dict:
    ...
    _save_ledger(ledger)
    return {"schema": "hawking.odyssey.ledger.v1", "list": "transfer_probes", "entry": entry}
...
def create_adversarial_probe(law_id: str, attack: str, confirm: bool = False) -> dict:
    ...
    _save_ledger(ledger)
    return {"schema": "hawking.odyssey.ledger.v1", "list": "adversarial_probes", "entry": entry}
