...
Covers, against fixture data (no file I/O, no subprocess):
  * SUB2 parked with a recorded wake_condition reads as WAITING, exactly
    the state receipts/future/HAWKING_MISSION_KERNEL.json is in today.
...
Runnable two ways:
    ...
    python3 hawking/test_frontier_scheduler.py
"""
...
def test_waiting_requires_wake_condition():
    """A WAITING state without a wake_condition cannot be constructed."""
    with pytest.raises((ValueError, AssertionError, TypeError)):
        fs.FrontierState(name="SUB2_EBPW", state="WAITING", wake_condition=None)


def test_waiting_with_wake_condition_is_allowed():
    """A WAITING state carrying a specific wake_condition is legal."""
    st = fs.FrontierState(
        name="SUB2_EBPW",
        state="WAITING",
        wake_condition="receipts/future/HAWKING_MISSION_KERNEL.json",
    )
    assert st.state == "WAITING"
    assert st.wake_condition == "receipts/future/HAWKING_MISSION_KERNEL.json"


def test_waiting_with_wake_condition_is_allowed_placeholder():
    """A WAITING state carrying a specific wake_condition is legal."""
    st = fs.FrontierState(
        name="SUB2_EBPW",
        state="WAITING",
        wake_condition="a landed receipt, a new scar, or an operator-set frontier",
    )
    assert st.state == "WAITING"
    assert st.wake_condition
...
def test_running_frontier_is_left_alone_even_if_something_scores_higher():
    states = [
        ...
        fs.FrontierState("HAWKING_SELF", fs.ELIGIBLE, 1000.0, None),
    ]
    decision = fs.select_next(states)
