...
These tests were watched FAILING against the unmodified HAWKING tree
...
from hawking.context_budget import (
...
_CTX_ENV = ("HAWKING_CTX_SIZE", "HAWKING_MODEL_TOKENS", "HAWKING_EVIDENCE_CHAR_BUDGET")
...
class TestPrecedence(unittest.TestCase):
    def test_precedence_override(self):
        with tempfile.TemporaryDirectory() as tmp:
            home = Path(tmp)
            (home / ".config" / "hawking").mkdir(parents=True)
            (home / ".config" / "hawking" / "machine_genome.json").write_text(
    ...
    def test_override_does_not_leak(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with _CleanCtxEnv():
                with patch.dict(
                    os.environ,
                    {"HOME": str(home), "HAWKING_CTX_SIZE": "65536"},
                    clear=False,
                ):
                ...
                with patch.dict(os.environ, {"HOME": str(home)}, clear=False):
                    os.environ.pop("HAWKING_CTX_SIZE", None)
    ...
    def test_no_http_on_preflight_failure(self):
        with tempfile.TemporaryDirectory() as tmp:
            with _CleanCtxEnv():
                with patch.dict(
                    os.environ,
                    {"HOME": str(tmp), "HAWKING_CTX_SIZE": "4096"},
                    clear=False,
                ):
...
class TestSingleAuthorityGuard(unittest.TestCase):
    def test_single_authority_guard(self):
        import hawking as hawking_pkg

        hawking_dir = Path(hawking_pkg.__file__).resolve().parent
        modules = ("config.py", "backends.py", "engine.py", "runtime.py")
        env_default = re.compile(
            r"""HAWKING_CTX_SIZE["']\s*,\s*["']?\d+["']?"""
            r"""|os\.environ\.get\(\s*["']HAWKING_CTX_SIZE["']"""
        )
        ...
        offenders = []
        for name in modules:
            text = (hawking_dir / name).read_text(encoding="utf-8")
            if env_default.search(text):
                offenders.append(f"{name}: HAWKING_CTX_SIZE default/read")
