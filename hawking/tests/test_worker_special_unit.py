...
from hawking.worker.special_unit import (
    DEFAULT_TOOL_NAMES,
    TOOL_CALL_CLOSE,
    TOOL_CALL_OPEN,
    TOOL_MAX_NEW_TOKENS,
...
from hawking.layout import REPO_ROOT
from hawking.verification_authority import AuthorityPrincipal, SelfPromotionError
...
def test_read_directory_returns_bounded_nonrecursive_listing(unit: SpecialUnit) -> None:
    listing = unit.tool("read", {"path": "hawking/worker", "limit": 8})
    assert listing.ok
...
def test_pytest_tool_runs_real_lab_test(unit: SpecialUnit) -> None:
    result = unit.tool("pytest", {"target": "hawking/tests/test_worker_option_c.py", "timeout": 90})
...
def test_prepare_candidate_stages_only_an_owned_unqualified_request(
    ...
    import hawking.lineage.lifecycle as lifecycle
...
def test_native_refuse_task_is_pass_not_skip(tmp_path: Path) -> None:
    from hawking.worker.offload_bench import _t_native_refuse_protected
...
def test_offload_catalog_cites_real_repo_files() -> None:
    from hawking.worker.offload_bench import G015, PRODUCER_HARNESS, PRODUCER_MODEL, TASKS
...
def test_goal_md_task_skips_naming_missing_path(tmp_path: Path) -> None:
    from hawking.worker.offload_bench import GOAL_MD, _t_read_goal_md_obligations
...
def test_write_and_pytest_still_requires_pytest_call() -> None:
    ...
    from hawking.worker.offload_bench import _t_write_and_pytest
...
def test_displacement_metric_is_not_rescored_to_model_expected() -> None:
    ...
    from hawking.worker.offload_bench import run_bench
...
def test_parse_tool_calls_well_formed_single() -> None:
    text = _call("grep", {"pattern": "proposed_complete", "path": "hawking/worker"})
...
def test_act_executes_scripted_well_formed_call(unit: SpecialUnit) -> None:
    unit.backend = ScriptedBackend(
        [_call("grep", {"pattern": "proposed_complete", "path": "hawking/worker", "max_hits": 10})]
    )
    result = unit.act("Search hawking/worker for proposed_complete.", known_tools=["grep"])
...
def test_scripted_act_is_not_counted_as_model(unit: SpecialUnit) -> None:
    ...
    assert result.ok
    assert result.produced_by == "scripted"
    from hawking.worker.offload_bench import PRODUCER_MODEL

    assert result.produced_by != PRODUCER_MODEL


def test_act_does_not_let_sandbox_verify(unit: SpecialUnit) -> None:
    unit.backend = ScriptedBackend([_call("grep", {"pattern": "x", "path": "hawking/worker"})])
...
def test_render_tool_prompt_is_raw_chat() -> None:
    prompt = render_tool_prompt("Search hawking/worker for proposed_complete.", DEFAULT_TOOL_NAMES)
...
def test_render_tool_prompt_for_resident_leaves_system_role_to_socket_contract() -> None:
    prompt = render_tool_prompt("Search hawking/worker.", ["grep"], user_only=True)
            ...
            json.dumps(
                {
                    "generated_text": _call(
                        "grep", {"pattern": "proposed_complete", "path": "hawking/worker"}
                    ),
                    "fallbacks": 0,
                }
            )
...
def test_act_refuses_protected_lock_without_invoke(tmp_path: Path) -> None:
    ...
    with pytest.raises(NativeDecodeRefused, match="protected"):
        unit.act("Search hawking/worker for proposed_complete.")
    assert called == []


def test_bench_skip_is_not_a_pass() -> None:
    from hawking.worker.offload_bench import _is_skip
...
def test_bench_unpack_preserves_measured_producer() -> None:
    from hawking.worker.offload_bench import _unpack_run
...
def test_resident_act_uses_injected_client_not_native_runner(tmp_path: Path) -> None:
    ...
    call = (
        f"{TOOL_CALL_OPEN}\n"
        + json.dumps({"name": "grep", "arguments": {"pattern": "proposed_complete", "path": "hawking/worker"}}, separators=(",", ":"))
        + f"\n{TOOL_CALL_CLOSE}\n"
    )
...
def test_resident_no_reply_does_not_fall_back_to_native(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import hawking.worker.special_unit as su
    ...
    with pytest.raises(ResidentRefused, match="resident client returned no result"):
        backend.complete("Say hi.", {})
    assert backend.last_receipt is None
    assert NativeQwen38Backend.lock_acquisitions == 0
    assert NativeQwen38Backend.generates_completed == 0
