...
from hawking.backends import CompletionResult
...
LIMIT_ENV = (
    ...
    "HAWKING_CTX_SIZE",
    "HAWKING_GENOME_STALENESS_HORIZON_S",
...
class FakeBackend:
    def __init__(self, model_path, port, n_slots=1, index=0, **_kwargs):
        ...
        self.index = index
        self.ctx_size = int(os.environ.get("HAWKING_CTX_SIZE", "8192"))
        self.process = None
        self.pid = None
    ...
    def endpoint(self):
        """Expose the canonical backend endpoint expected by RuntimePool."""
...
class AuthorityTestCase(unittest.TestCase):
    def setUp(self):
        ...
        os.environ.pop("HAWKING_GENOME_STALENESS_HORIZON_S", None)
        os.environ["HAWKING_CTX_SIZE"] = "8192"
...
class TestOversizedSlotAllocation(AuthorityTestCase):
    def test_five_slots_for_one_decode_surfaces_single_stream_cost(self):
        os.environ["HAWKING_RESIDENT_RUNTIME_LIMIT"] = "5"
        os.environ["HAWKING_ACTIVE_DECODE_LIMIT"] = "1"
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
                # Admission width now follows MEASURED model-call overlap,
