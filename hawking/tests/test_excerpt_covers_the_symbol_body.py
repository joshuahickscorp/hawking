...
naming _read_file (line 514 of hawking/tool_registry.py) received lines 457-571 --
...
from hawking.engine import _focused_excerpt

TARGET = pathlib.Path("hawking/tool_registry.py")
GOAL = (
    "Add a total_lines key to the whole-file return dict of _read_file in "
    "hawking/tool_registry.py so that the test passes."
)
...
def test_the_excerpt_reaches_the_code_the_goal_names():
    ...
    # The live _read_file implementation grew with bounded-read and digest
    # provenance handling; keep enough budget to reach the named body rather
    # than pinning this fixture to the retired shorter implementation.
    excerpt = _focused_excerpt(src, GOAL, 12000, str(TARGET))
...
def test_the_most_specific_name_wins_not_the_earliest(tmp_path):
    ...
    `path` instead. Measured on hawking/tool_registry.py: the window came out as
