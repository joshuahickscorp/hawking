...
from hawking.controller import Controller, _http_json
...
class TestFormatStatus(unittest.TestCase):
    def test_one_screen_contains_required_fields(self):
        ...
        self.assertIn("Hawking health=ok resident=4 active=1", text)
        self.assertIn("n_ctx=262144", text)
        self.assertIn("Cognition providers=deepseek external=true", text)
    ...
    def test_hawking_down_does_not_echo_intended_count(self):
        text = format_status(
            {
                "hawking": {
                    "health": "down",
        ...
        self.assertIn("Hawking health=down resident=0", text)
        self.assertNotIn("failed_restarting", text)


class TestHawkingProbe(unittest.TestCase):
    def test_unreachable_endpoint_is_down(self):
        with tempfile.TemporaryDirectory() as tmp:
            controller = Controller(workspace=tmp, runtime_count=1)
            controller._hawking_endpoint = lambda: "http://127.0.0.1:1"
            snap = controller._hawking_pool_status()
    ...
    def test_slots_and_health_are_used_when_present(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("hawking.controller._http_json", side_effect=fake_http):
                snap = controller._hawking_pool_status()
...
class TestMutationStaleOwner(unittest.TestCase):
    def test_owner_is_stale_when_lock_record_exists_but_not_held(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            text = format_status({"mutation": snap, "hawking": {"health": "down"}})
...
class TestHttpJson(unittest.TestCase):
    ...
    def test_live_http_server_is_probed_not_assumed(self):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmp:
                controller = Controller(workspace=tmp, runtime_count=1)
                controller._hawking_endpoint = lambda: f"http://{host}:{port}"
                snap = controller._hawking_pool_status()
                self.assertEqual(snap["health"], "ok")
