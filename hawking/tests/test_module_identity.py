"""One module identity for the HAWKING control plane (incident F24).

Python loaded the same files under two dotted names:

* ``hawking.engine``               with sys.path = <repo>/tools/haider
* ``tools.haider.hawking.engine``  with sys.path = <repo>
...
# Built so a mechanical ``tools.haider.hawking`` -> ``hawking`` rewrite cannot
# delete the name this test is guarding against.
_FOSSIL_ENGINE = "tools" + ".haider.hawking.engine"


class TestOneModuleIdentity(unittest.TestCase):
    def test_canonical_engine_imports(self):
        import hawking.engine as canonical

        self.assertEqual(canonical.__name__, "hawking.engine")
    ...
    def test_sys_modules_has_one_engine(self):
        ...
        self.assertIn("hawking.engine", engines)
