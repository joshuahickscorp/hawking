...
from hawking import backends as backends_mod
from hawking import machine as machine_mod
from hawking import mutation as mutation_mod
from hawking import persist as persist_mod
from hawking import resources as resources_mod
from hawking import runtime as runtime_mod
from hawking import scheduler as scheduler_mod
from hawking import verifier_pipeline as verifier_mod
from hawking import workunit as workunit_mod
from hawking.machine import MachineGenome, resolve_runtime_limits
from hawking.persist import atomic_write_json, atomic_write_text
from hawking.resources import resolve_active_decode_limit
...
class TestLimitOneAuthority(unittest.TestCase):
    ...
    def test_resources_no_longer_walks_genome_files(self):
        ...
        self.assertNotIn("~/.config/hawking/machine_genome.json", src)
...
class TestMachineGenomeIsNotAdmission(unittest.TestCase):
    ...
    def test_default_path_is_not_the_live_genome(self):
        ...
        self.assertNotEqual(
            path, Path.home() / ".config" / "hawking" / "machine_genome.json"
        )
...
class TestEntropyExecuted(unittest.TestCase):
    ...
    def test_hawking_index_source_removed_from_this_tree(self):
        # An editable install of another checkout can still satisfy
        # ``import hawking.index``. This tree's source is the identity.
        self.assertFalse((REPO / "hawking" / "index.py").is_file())
        spec = importlib.util.find_spec("hawking.index")
        if spec is not None and spec.origin:
            self.assertNotEqual(
                Path(spec.origin).resolve(),
                (REPO / "hawking" / "index.py").resolve(),
            )
    ...
    def test_verifier_main_guard_lives_on_engine(self):
        ...
        from hawking import engine as engine_mod
