...
the field, must validate against HAWKING_RESULT_SCHEMA, retry a bounded
...
from hawking.backends import StructuredOutputExhausted
from hawking.config import Config
from hawking.engine import Engine, HAWKING_RESULT_SCHEMA
...
class TestEngineSchemaDegrade(unittest.TestCase):
    ...
    def test_retry_exhaustion_is_explicit_failure_with_last_violation(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            engine._post_completion = stub
            with patch.dict(
                os.environ, {"HAWKING_STRUCTURED_OUTPUT_ATTEMPTS": "2"}, clear=False
            ):
                result = engine.execute("say ok")
    ...
    def test_retry_is_bounded_not_unbounded(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            engine._post_completion = stub
            with patch.dict(
                os.environ, {"HAWKING_STRUCTURED_OUTPUT_ATTEMPTS": "3"}, clear=False
            ):
    ...
    def test_receipt_enforced_shape(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                set(schema["required"]), set(HAWKING_RESULT_SCHEMA["required"])
            )
            receipt = _receipt(result)
            so = receipt["structured_output"]
