...
"applying your operation to hawking/perception/store.py would not compile:
...
from hawking.engine import _python_syntax_violation as _violation
...
def test_a_fragment_sent_as_a_whole_file_names_the_right_operation():
    msg = _reject([_op("replace_file", "hawking/perception/store.py", FRAGMENT)])
...
def test_create_on_an_existing_file_keeps_its_own_better_message():
    ...
    """
    msg = _reject([_op("create", "hawking/perception/store.py", FRAGMENT)])
...
def test_a_replace_fragment_is_still_allowed(tmp_path):
    ...
    """
    assert _reject([_op("replace", "hawking/perception/store.py", FRAGMENT)]) is None
