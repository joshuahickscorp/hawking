...
from hawking.grok_bridge import (  # noqa: E402
    GrokBridge,
    GrokContractError,
    GrokNotAvailable,
    GrokRunError,
...
create hawking/grok_bridge.py

# VERIFY
`pytest hawking/tests -q` must exit 0
...
class _BridgeTest(unittest.TestCase):
    def setUp(self):
        ...
        self.which_patch = patch(
            "hawking.grok_bridge.shutil.which",
    ...
    def _patch_run(self, stdout="", stderr="", returncode=0, collector=None):
        ...
        return patch(
            "hawking.grok_bridge.subprocess.run",
...
class TestMissingContractDoesNotSpawn(_BridgeTest):
    def test_empty_contract_does_not_call_subprocess(self):
        ...
        with patch("hawking.grok_bridge.subprocess.run", side_effect=boom):
            with patch(
                "hawking.grok_bridge.subprocess.Popen", side_effect=boom
            ):
                with self.assertRaises(GrokContractError):
...
class TestGrokNotAvailable(_BridgeTest):
    ...
    def test_which_none_raises_specific_error(self):
        self.which_patch.stop()
        with patch("hawking.grok_bridge.shutil.which", return_value=None), patch(
            "hawking.grok_bridge.DEFAULT_GROK_RUN_HINT", self.ABSENT_HINT
        ):
    ...
    def test_installed_binary_is_found_without_being_on_path(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            fake.chmod(0o755)
            with patch("hawking.grok_bridge.shutil.which", return_value=None), patch(
                "hawking.grok_bridge.DEFAULT_GROK_RUN_HINT", fake
            ):
                os.environ.pop("GROK_RUN", None)
...
class TestArgvConstruction(_BridgeTest):
    def test_delegate_flags(self):
        calls = []
        stdout = _dry_stdout("hawking-unit-20260101-000000", cwd=str(self.root))
        ...
        self.assertEqual(argv[argv.index("--task") + 1], "hawking-unit")
        ...
        self.assertEqual(handle.task_id, "hawking-unit-20260101-000000")
        self.assertEqual(handle.command_run, argv)
        self.assertTrue(handle.dry_run)

    def test_audit_flags(self):
        calls = []
        stdout = _dry_stdout("hawking-aud-20260101-000000")
        with self._patch_run(stdout=stdout, collector=calls):
            handle = self.bridge.audit(
                "hawking-aud", VALID_CONTRACT, background=True, dry_run=True
            )
        argv = calls[0][0]
        self.assertIn("audit", argv)
        self.assertEqual(argv[argv.index("--task") + 1], "hawking-aud")
        self.assertTrue(Path(argv[argv.index("--contract") + 1]).is_file())
        self.assertIn("--background", argv)
        self.assertNotIn("--no-worktree", argv)
        self.assertNotIn("--profile", argv)
        self.assertEqual(handle.task_id, "hawking-aud-20260101-000000")
    ...
    def test_verify_preserves_structured_provider_receipt_without_accepting_it(self):
        ...
        self.assertTrue(out["ok"])
        self.assertFalse(out["hawking_acceptance"])
...
class TestReceipts(_BridgeTest):
    def test_delegate_and_audit_write_receipts(self):
        ...
        path = self.root / ".hawking" / "grok" / f"{handle.task_id}.json"
        ...
        apath = self.root / ".hawking" / "grok" / f"{audit.task_id}.json"
    ...
    def test_v2_mission_records_exact_plan_and_receipt(self):
        ...
        self.assertEqual(receipt["schema"], "hawking.grok_mission.v1")
        self.assertTrue(receipt["dry_run"])
        self.assertTrue(
...
class TestReceiptCompletenessAndSuccess(_BridgeTest):
    ...
    def test_status_refreshes_receipt_completeness(self):
        ...
        with patch("hawking.grok_bridge.subprocess.run", side_effect=fake_run):
    ...
    def test_done_exit_1_is_not_successful_consult_223811_shape(self):
        ...
        from hawking.grok_bridge import grok_succeeded
