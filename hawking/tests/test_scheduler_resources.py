...
from hawking.scheduler import NO_PROGRESS, Scheduler, UnverifiedCompletion
...
class TestActiveDecodeLimit(unittest.TestCase):
    ...
    def test_delegates_to_resolve_runtime_limits(self):
        from hawking.machine import resolve_runtime_limits
...
class TestResourceDispatch(unittest.TestCase):
    ...
    def test_hawking_has_no_fixed_scheduler_slot_ceiling(self):
        limits = ResourceLimits.resolve()
        occupied = Counter({"HAWKING": 10000})
        self.assertIsNone(limits.hawking)
        self.assertTrue(can_admit("HAWKING", occupied, limits))
        self.assertEqual(next_class_slot("HAWKING", {"HAWKING": {0, 1, 2}}, limits), 3)
