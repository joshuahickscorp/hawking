...
class TestReadWindow(unittest.TestCase):
    def test_search_then_read_reaches_a_deep_function(self):
        reg = _reg()
        matches = _value(
            reg.invoke("fs.search", {"pattern": "def _record_model_call", "path": "hawking"})
        ).get("matches") or []
        ...
        window = _value(
            reg.invoke("fs.read", {"path": "hawking/engine.py",
    ...
    def test_the_window_reports_where_it_is(self):
        ...
        w = _value(_reg().invoke(
            "fs.read", {"path": "hawking/engine.py", "start_line": 10, "end_line": 12}
        ))
        ...
        anchor = w.get("source_anchor") or {}
        self.assertTrue(str(anchor.get("anchor") or "").startswith("SRC-"))
        self.assertEqual(anchor.get("path"), "hawking/engine.py")

    def test_a_whole_file_read_is_unchanged(self):
        """Negative control: the default path must keep its old behaviour."""
        w = _value(_reg().invoke("fs.read", {"path": "hawking/engine.py"}))
        self.assertTrue(w.get("truncated"))
        self.assertIsNone(w.get("start_line"))

    def test_an_out_of_range_window_is_not_an_error(self):
        w = _value(_reg().invoke(
            "fs.read", {"path": "hawking/engine.py", "start_line": 10**9}
        ))
    ...
    def test_an_out_of_range_window_SAYS_it_is_out_of_range(self):
        ...
        past the end of hawking/mutation.py and walked the window forward looking for
        ...
        """
        w = _value(_reg().invoke(
            "fs.read", {"path": "hawking/mutation.py", "start_line": 1620, "end_line": 1720}
        ))
    ...
    def test_a_window_inside_the_file_is_not_flagged(self):
        ...
        source = Path(REPO / "hawking/mutation.py").read_text(encoding="utf-8")
        line = next(
            number for number, text in enumerate(source.splitlines(), 1)
            if text.startswith("def _apply_replace")
        )
        w = _value(_reg().invoke(
            "fs.read", {"path": "hawking/mutation.py", "start_line": line, "end_line": line + 10}
        ))
