...
from hawking.max_policy import (
    grok_pool_snapshot,
    load_equilibrium,
    next_grok_rung,
    record_rung,
...
class TestRecordRung(unittest.TestCase):
    ...
    def test_run_grok_ramp_stops_when_rate_does_not_beat(self):
        with tempfile.TemporaryDirectory() as tmp:
            saved = os.environ.get("HAWKING_GROK_ADMITTED")
            ...
            try:
                ...
                if saved is None:
                    os.environ.pop("HAWKING_GROK_ADMITTED", None)
                else:
                    os.environ["HAWKING_GROK_ADMITTED"] = saved
            self.assertEqual(payload["grok_admitted"], 2)
