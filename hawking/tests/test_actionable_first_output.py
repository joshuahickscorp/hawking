...
AUDITED_TOOLS = {
    ...
    "browser.changed": "compliant",
    "browser.click": "compliant",
    "browser.close": "compliant",
    "browser.console": "compliant",
    "browser.downloads": "compliant",
    "browser.find": "compliant",
    "browser.goto": "compliant",
    "browser.health": "compliant",
    "browser.key": "compliant",
    "browser.network": "compliant",
    "browser.observe": "compliant",
    "browser.open": "compliant",
    "browser.pages": "compliant",
    "browser.screenshot": "compliant",
    "browser.scroll": "compliant",
    "browser.select": "compliant",
    "browser.type": "compliant",
    "browser.verify": "compliant",
    "browser.wait": "compliant",
    ...
    "lake": "compliant",
    "lake.catalog": "compliant",
    "macos.changed": "compliant",
    "macos.find": "compliant",
    "macos.health": "compliant",
    "macos.lease.acquire": "compliant",
    "macos.lease.preempt": "compliant",
    "macos.lease.release": "compliant",
    "macos.observe": "compliant",
    "macos.press": "compliant",
    "macos.verify": "compliant",
    ...
    "permissions.explain": "compliant",
    "permissions.require": "compliant",
    "permissions.status": "compliant",
    "processes": "compliant",
    "receipt": "compliant",
    "shell.exec": "compliant",
    "shell.readonly": "compliant",
    "source.ast_query": "compliant",
    "source.affected_tests": "compliant",
    "source.outline": "compliant",
    "source.owner": "compliant",
    "source.references": "compliant",
    "source.rewrite_preview": "compliant",
    "source.symbol": "compliant",
    "tests.list": "compliant",
    "tests.run": "fix",
    "tools.catalog": "fix",
    "perception": "compliant",
...
def test_auditor_tools_lead_with_the_verdict(tmp_path):
    """HAWKING's adversarial-auditor doors put the JUDGEMENT first. [S002]
...
def test_lake_catalog_summary_survives_budget(tmp_path, monkeypatch):
    """A compact catalog must reveal its usable capacity before its file path.

    This explicitly exercises the bounded JSON reader, rather than permitting
    an optional native helper to hide a slow catalog walk behind this test.
    """
    from hawking import tool_registry

    monkeypatch.setattr(tool_registry, "native_helpers_allowed", lambda: False)
    catalog = tmp_path / "catalog.json"
    catalog.write_text(json.dumps({
        "schema": "hawking.modellake.index.catalog.v1",
        "n_specimens": 17,
        "n_partial": 2,
        "tier2_used_bytes": 123456789,
        "tier2_budget": 987654321,
        "over_budget": False,
        "specimens": [],
    }))
    result = _registry(tmp_path).invoke("lake.catalog", {"catalog": str(catalog)})
    assert result.ok, result.error
    head = _head(result.value)
    assert '"backend"' in head
    assert '"n_specimens"' in head
    assert '"tier2_used_bytes"' in head
    assert result.value["n_specimens"] == 17
    assert result.value["over_budget"] is False
...
def test_specimens_registry_ids_survive_budget(tmp_path, monkeypatch):
    ...
    manifests.mkdir()
    monkeypatch.setenv("HAWKING_MODEL_LAKE_ROOT", str(lake))
    monkeypatch.setenv("HAWKING_SPECIMEN_MANIFEST_DIR", str(manifests))
    ids = []
    for i in range(16):
...
def test_processes_list_pids_survive_budget(tmp_path, monkeypatch):
    ...
    class _Proc:
        def __init__(self, pid):
            ...
            self.command = "/usr/bin/python3 -m hawking.resident --workspace " + "x" * 220
...
def test_acquisition_propose_recommended_survives_budget(tmp_path, monkeypatch):
    ...
    payload = {
        "schema": "hawking.acquisition.propose.v1",
    ...
    fake = types.ModuleType("hawking.acquisition")
    fake.propose = lambda: payload
    monkeypatch.setitem(sys.modules, "hawking.acquisition", fake)
...
def test_tests_run_verified_survives_budget(tmp_path, monkeypatch):
    import hawking.tool_registry as tr
