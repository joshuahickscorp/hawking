...
class TestOneVocabulary(unittest.TestCase):
    def test_aliases_are_callable_but_not_separate_capabilities(self):
        ...
        self.assertTrue(
            registry.invoke("filesystem.read", {"path": "hawking/__init__.py"}).ok
        )
    ...
    def test_search_by_path_finds_what_is_there(self):
        result = _registry().invoke(
            "fs.search", {"pattern": "os.waitpid", "path": "hawking"}
        )
    ...
    def test_root_still_works(self):
        ...
        result = _registry().invoke(
            "fs.search", {"pattern": "os.waitpid", "root": "hawking"}
        )
        self.assertTrue(getattr(result, "ok", False), getattr(result, "error", None))
    ...
    def test_both_names_agree(self):
        reg = _registry()
        by_path = reg.invoke("fs.search", {"pattern": "os.waitpid", "path": "hawking"})
        by_root = reg.invoke("fs.search", {"pattern": "os.waitpid", "root": "hawking"})
        self.assertEqual(
