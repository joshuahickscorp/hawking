...
from hawking.events import Event
...
class TestEngineConnector(unittest.TestCase):
    ...
    def test_mutation_without_proving_test_is_rejected_before_write(self):
        """The current RED-before-GREEN gate rejects an ungrounded edit."""
        workspace = Workspace(str(self.workspace_path))
        controller = Controller(workspace=workspace, model="test-model")
        ...
        self.assertIn("UNGROUNDED_MUTATION", result["error"])
        self.assertEqual((self.workspace_path / "main.py").read_text(), "print('hello')")

    def test_ungrounded_mutation_failure_preserves_original(self):
        """A rejected mutation leaves the source unchanged."""
        workspace = Workspace(str(self.workspace_path))
        controller = Controller(workspace=workspace, model="test-model")
    ...
    def test_receipt_creation(self):
        ...
        receipts_dir = self.workspace_path / ".hawking" / "receipts"
