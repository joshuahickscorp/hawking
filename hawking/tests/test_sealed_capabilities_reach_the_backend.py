...
from hawking.hawking_native import HawkingNativeConfig, config_for_model_path
...
if __name__ == "__main__":
    unittest.main()
def test_hawking_gate_admits_sealed_capability():
    from hawking.native_gate import _hawking_gate_admits

    class _Gate:
        withheld = False
        sealed_capabilities = frozenset({"cpu.native"})

    assert _hawking_gate_admits(_Gate(), "cpu.native") is True
    assert _hawking_gate_admits(_Gate(), "gpu.cuda") is False


def test_hawking_gate_withheld_denies():
    from hawking.native_gate import _hawking_gate_admits
    ...
    assert _hawking_gate_admits(_Gate(), "cpu.native") is False