...
something:
  ...
  * `--max-cycles` and `--max-turns` are parsed by hawking/cli.py and read by
  ...
  * `qwen27-mlp-ab` reads args.fusion_env, which its own subparser never
...
class TestBoundsThatDoNotExistAreRefused(unittest.TestCase):
    ...
    def _parse(self, argv):
        ...
        return parse_hawking_args(argv)
...
class TestConstraintsReachTheWorker(unittest.TestCase):
    def test_a_constraint_appears_in_the_goal(self):
        ...
        from hawking.delegate import _goal_with_constraints
...
class TestEveryHawkingControlSubcommandParses(unittest.TestCase):
