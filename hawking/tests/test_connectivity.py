...
from hawking.connectivity import _public_probe
...
def test_public_probe_writes_integer_nanoseconds(monkeypatch):
    monkeypatch.setattr(
        "hawking.connectivity.urllib.request.urlopen",
        lambda *_args, **_kwargs: _Response(),
    )

    with patch("hawking.connectivity.now_ns", side_effect=[10_000, 10_417]), \
         patch("hawking.connectivity.since_ns", return_value=417):
