...
from hawking.backends import CompletionResult, LlamaServerBackend
from hawking.machine import (
    GIB,
    MemGate,
...
from hawking.resources import pid_is_alive, process_start_token
from hawking.runtime import RuntimePool, allocate_port
...
LIMIT_ENV = (
    ...
    "HAWKING_ACTIVE_DECODE_LIMIT",
    "RESIDENT_RUNTIME_LIMIT",
    "ACTIVE_DECODE_LIMIT",
    "HAWKING_MAX_RUNTIMES",
    "HAWKING_OBSERVED_MODEL_OVERLAP",
    "HAWKING_DECODE_TOPOLOGY",
...
class RuntimePoolTestCase(unittest.TestCase):
    def setUp(self):
        ...
        os.environ.pop("HAWKING_MAX_RUNTIMES", None)
        os.environ.pop("HAWKING_OBSERVED_MODEL_OVERLAP", None)
...
class TestLimitResolution(RuntimePoolTestCase):
    def test_env_wins_independently(self):
        os.environ["HAWKING_RESIDENT_RUNTIME_LIMIT"] = "3"
        os.environ["HAWKING_ACTIVE_DECODE_LIMIT"] = "1"
    ...
    def test_genome_then_receipt_then_equilibrium_then_fallback(self):
        os.environ.pop("HAWKING_RESIDENT_RUNTIME_LIMIT", None)
        os.environ.pop("HAWKING_ACTIVE_DECODE_LIMIT", None)
        os.environ.pop("RESIDENT_RUNTIME_LIMIT", None)
        os.environ.pop("ACTIVE_DECODE_LIMIT", None)
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            home = root / "home"
            genome_dir = home / ".config" / "hawking"
            genome_dir.mkdir(parents=True)
            (genome_dir / "machine_genome.json").write_text(
                json.dumps(
                    {
                        "schema": "hawking.machine_genome.v1",
            ...
            eq_dir = root / ".hawking-legacy" / "bootstrap-director-v6"
    ...
    def test_topology_receipt_prefers_slot(self):
        os.environ.pop("HAWKING_DECODE_TOPOLOGY", None)
...
class TestPoolLifecycle(RuntimePoolTestCase):
    ...
    def test_second_runtime_marginal_not_full_model(self):
        os.environ["HAWKING_RESIDENT_RUNTIME_LIMIT"] = "2"
        os.environ["HAWKING_ACTIVE_DECODE_LIMIT"] = "1"
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            model = _dummy_model(root)
    ...
    def test_reaper_kills_recorded_orphan_only(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                time.sleep(0.15)
                hawking = root / ".hawking"
                hawking.mkdir()
                (hawking / "runtime_pool.json").write_text(
                    json.dumps(
                        {
                            "schema": "hawking.runtime_pool.v1",
    ...
    def test_unmeasured_overlap_admits_one_runtime(self):
        os.environ["HAWKING_RESIDENT_RUNTIME_LIMIT"] = "4"
        os.environ["HAWKING_ACTIVE_DECODE_LIMIT"] = "2"
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            pool = RuntimePool(
                _dummy_model(root),
    ...
    def test_stored_overlap_raises_admission_width(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            from hawking.runtime import store_observed_overlap
