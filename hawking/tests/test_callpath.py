...
from hawking.config import Config, coerce_bool, coerce_on_off
...
class TestCallPolicyConfig(unittest.TestCase):
    ...
    def test_env_wins_over_project(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.dict(
                os.environ,
                {
                    "HAWKING_ENABLE_THINKING": "true",
                    "HAWKING_RESPONSE_SCHEMA": "off",
                },
                clear=False,
            ):
    ...
    def test_model_tokens_source_env_config_derived(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            engine = _engine(root)
            with patch.dict(os.environ, {"HAWKING_MODEL_TOKENS": "1234"}, clear=False):
            ...
            env = os.environ.copy()
            env.pop("HAWKING_MODEL_TOKENS", None)
            with patch.dict(os.environ, env, clear=True):
                ...
                (root / ".hawking" / "config.json").write_text("{}", encoding="utf-8")
                with patch.dict(
                    os.environ, {"HAWKING_CTX_SIZE": "4096"}, clear=False
                ):
...
class TestCallPath(unittest.TestCase):
    def test_default_payload_is_structured(self):
        with tempfile.TemporaryDirectory() as tmp:
            with patch.dict(os.environ, {}, clear=False):
                os.environ.pop("HAWKING_ENABLE_THINKING", None)
                os.environ.pop("HAWKING_RESPONSE_SCHEMA", None)
            ...
            self.assertEqual(
                set(required),
                set(HAWKING_RESULT_SCHEMA["required"]),
            )
    ...
    def test_length_error_names_budget(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.dict(
                os.environ, {"HAWKING_MODEL_TOKENS": "6500"}, clear=False
            ):
