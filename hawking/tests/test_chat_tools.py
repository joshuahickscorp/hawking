...
class TestTheMenuMatchesTheTools(unittest.TestCase):
    ...
    def test_every_offered_tool_is_served_by_exactly_one_owner(self):
        # Two classes of door: registry tools, and retrieval doors HAWKING serves
        ...
        from hawking.chat_tools import LOCAL_TOOLS
    ...
    def test_every_offered_tool_advertises_its_real_required_arguments(self):
        from hawking.chat_tools import LOCAL_SHAPES, LOCAL_TOOLS
        for name in CHAT_TOOLS:
...
class TestDetachedGoalProjection(unittest.TestCase):
    """Main chat reads detached Goals from the canonical Goal owner."""

    def test_goals_list_returns_bounded_goal_and_result_projection(self):
        from hawking.goal_surface import create_goal, write_result
        from hawking.chat_tools import run_local_tool

        with tempfile.TemporaryDirectory() as tmp:
            goal = create_goal(Path(tmp), objective="disposable markdown fixture")
            write_result(
                Path(tmp), goal["goal_id"], status="COMPLETE",
                summary="Attached markdown was read and verified.",
                files_changed=[],
            )
            observed = run_local_tool("goals.list", {"limit": 4}, workspace=tmp)

        self.assertTrue(observed.ok, observed.error)
        self.assertEqual(observed.value["authority"], "canonical Hawking Goal files")
        self.assertEqual(observed.value["goals"][0]["goal_id"], goal["goal_id"])
        self.assertEqual(
            observed.value["goals"][0]["result"]["summary"],
            "Attached markdown was read and verified.",
        )

    def test_goal_state_question_gets_a_canonical_read_before_synthesis(self):
        from hawking.goal_surface import create_goal

        with tempfile.TemporaryDirectory() as tmp:
            goal = create_goal(Path(tmp), objective="most recent disposable Goal")
            registry = build_registry(tmp, tmp)
            replies = iter([
                "I do not have access to canonical Goal state.",
                f"Goal {goal['goal_id']} is the most recent disposable Goal.",
            ])

            answer, trace = run_with_tools(
                lambda _conversation: next(replies),
                [{
                    "role": "user",
                    "content": "Summarize the most recent disposable Goal and what it did.",
                }],
                registry,
            )

        self.assertIn(goal["goal_id"], answer)
        self.assertTrue(any(
            row.get("tool") == "goals.list"
...
class TestParsing(unittest.TestCase):
    ...
    def test_an_object_buried_in_prose(self):
        got = parse_call('Let me look.\n{"tool": "fs.list", "arguments": {"path": "hawking"}}\nOK?')
    ...
    def test_batched_json_actions_are_consumed_in_order(self):
        ...
        self.assertEqual(parse_calls(text), [
            ("fs.search", {"pattern": "x"}),
            ("fs.list", {"path": "hawking"}),
        ])

    def test_typed_mutation_proposal_bridges_to_repo_edit(self):
        proposal = json.dumps({
            ...
            "required_tests": ["hawking/tests/test_example.py"],
        })
        self.assertEqual(parse_call(proposal), (
            "repo.edit",
            {
                "operations": [{
                    "op": "replace",
                ...
                "tests": ["hawking/tests/test_example.py"],
            },
        ))
    ...
    def test_compact_builder_dialect_maps_to_authority_checked_doors(self):
        ...
        self.assertEqual(
            parse_call('{"op":"test","paths":["hawking/tests/test_x.py"]}'),
            ("tests.run", {"paths": ["hawking/tests/test_x.py"]}),
        )

    def test_compact_multi_edit_becomes_one_typed_transaction(self):
        from hawking.chat_tools import coerce_arguments

        call = parse_call(
            '{"op":"edit","edits":['
            '["hawking/cli.py","old source","new source"],'
            '["hawking/test_cli.py","old test","new test"]],'
            '"test":"hawking/test_cli.py"}'
        )
        ...
        self.assertEqual(coerce_arguments(None, name, arguments), {
            "operations": [
                {"op": "replace", "path": "hawking/cli.py",
                 "old_lines": ["old source"], "new_lines": ["new source"]},
                {"op": "replace", "path": "hawking/test_cli.py",
                 "old_lines": ["old test"], "new_lines": ["new test"]},
            ],
            "tests": ["hawking/test_cli.py"],
        })
    ...
    def test_compact_edit_survives_template_marker_and_fstring_anchor(self):
        ...
        call = parse_call(
            '{"op":"edit","edits":[{"path":"hawking/test_cli.py",'
            '"old":"assert argv, f\\"bad: {argv}\\"",'
            '"new":"assert argv[0], f\\"bad: {argv}\\""}],'
            '"test":"hawking/test_cli.py"}<|im_end|>'
        )
        self.assertIsNotNone(call)
        name, arguments = call
        self.assertEqual(name, "repo.edit")
    ...
    def test_selected_lines_survive_a_truncated_structural_observation(self):
        from hawking.chat_tools import _selected_observation_lines
    ...
    def test_nested_compact_operation_tag_is_removed_only_for_its_tool(self):
        ...
        self.assertEqual(
            coerce_arguments(None, "fs.read",
                             {"path": "hawking/serve.py", "line": "728"}),
            {"path": "hawking/serve.py", "start_line": 728},
        )
        self.assertEqual(
...
class TestTheLoop(unittest.TestCase):
    ...
    def test_plural_hawking_tools_is_an_explicit_observation_obligation(self):
        registry = _Registry()
        text, trace = run_with_tools(
            _scripted(["--- fs.search\n" * 1000, "I still only planned it."]),
            [{"role": "user", "content": (
                "Use HAWKING tools to inspect the repository, then report exact "
    ...
    def test_capability_surface_curveball_requires_a_real_observation(self):
        from hawking.chat_tools import explicit_tool_observation_required
    ...
    def test_blind_write_mission_cannot_finish_before_mutation_and_test(self):
        ...
        text, trace = run_with_tools(
            _scripted([
                '{"tool":"tools.catalog","arguments":{"focus":"HAWKING"}}',
                ...
                '"append","path":"hawking/example.py","new_lines":["x = 1"]}],'
                '"tests":["hawking/tests/test_chat_tools.py"]}}',
                '{"mission":"FAIL","tools":{"successful_invocations":'
                '["repo.edit","tools.catalog"]},"mutation":{"accepted":true,'
                '"paths":["hawking/example.py"]},"verification":{"passed":false,'
                '"returncode":null}}',
                '{"tool":"tests.run","arguments":{"paths":'
                '["hawking/tests/test_chat_tools.py"]}}',
                '{"mission":"PASS","tools":{"successful_invocations":'
                '["repo.edit","tests.run","tools.catalog"]},"mutation":'
                '{"accepted":true,"paths":["hawking/example.py"]},'
    ...
    def test_explicit_accepted_mission_does_not_leave_unproven_edit(self):
        ...
        text, trace = run_with_tools(
            _scripted([
                ...
                '{"op":"append","path":"hawking/example.py",'
                '"new_lines":["x = 1"]}]}}',
                '{"tool":"repo.edit","arguments":{"operations":['
                '{"op":"append","path":"hawking/example.py",'
                '"new_lines":["x = 1"]}],"tests":['
                '"hawking/tests/test_example.py"]}}',
                '{"tool":"tests.run","arguments":{"paths":['
                '"hawking/tests/test_example.py"]}}',
    ...
    def test_blind_write_pass_claim_without_trace_is_rejected(self):
        from hawking.chat_tools import _trace_grounding_issues
        claimed = json.dumps({
            "mission": "PASS",
    ...
    def test_runtime_ownership_requires_one_hawkingd_ancestry_tree(self):
        from hawking.chat_tools import _runtime_tree_preserved
    ...
    def test_compaction_safe_ledger_replays_prior_success_before_synthesis(self):
        ...
        ledger = "\n".join(seen[1])
        self.assertIn("HAWKING OBSERVATION LEDGER", ledger)
    ...
    def test_explicit_evidence_categories_remain_open_until_observed(self):
        from hawking.chat_tools import _evidence_obligations, _pending_evidence
        messages = [{"role": "user", "content": (
            "Use actual tools. Discover the repository and HAWKING capabilities, "
    ...
    def test_readonly_controller_routes_one_explicit_category_after_retry(self):
        ...
        self.assertEqual(routed["evidence_class"], "HAWKING_SERVING_CONTRACT")
    ...
    def test_source_controller_reads_the_exact_search_hit_window(self):
        ...
        text, trace = run_with_tools(
            ...
            [{"role": "user", "content": (
                "Use actual tools to reproduce the installed HAWKING shadowing "
    ...
    def test_discriminating_mission_reads_a_test_before_running_it(self):
        path = "/repo/hawking/test_install_shims.py"
        registry = _OfferingRegistry({
            "fs.search": _Result(value={
    ...
    def test_source_window_projection_keeps_absolute_behavior_lines(self):
        from hawking.chat_tools import _content_selection
    ...
    def test_broad_interactive_test_run_is_redirected_to_a_focused_file(self):
        ...
        text, trace = run_with_tools(
            _scripted([
                '{"tool":"tests.run","arguments":{"paths":["hawking/tests"]}}',
                '{"tool":"tests.run","arguments":{"paths":["hawking/tests/test_chat_state.py"]}}',
        ...
        self.assertEqual(registry.calls, [
            ("tests.run", {"paths": ["hawking/tests/test_chat_state.py"]}),
        ])
    ...
    def test_the_conversation_never_exceeds_a_given_prompt_budget(self):
        ...
        # with no information HAWKING could act on). A body that never gets a
    ...
    def test_trace_repairs_only_mechanical_catalog_fields_after_model_retry(self):
        ...
        self.assertEqual(trace[-1]["evidence_class"], "HAWKING_SERVING_CONTRACT")
    ...
    def test_final_grounding_cannot_drop_current_authority_or_read_receipt(self):
        ...
        from hawking.chat_tools import _repair_trace_redundant_fields
    ...
    def test_a_batch_of_calls_in_one_reply_all_execute(self):
        ...
        # .hawking/selfdev/evidence/cycle-0008.txt: three <tool_call> blocks in a
...
class TestTruncatedSearchIsNotEvidenceOfAbsence(unittest.TestCase):
    def test_a_truncated_thin_search_escalates(self):
        ...
        # .hawking/receipts/*.json, files_seen 5001, truncated True -- the budget
        # was spent on generated data before reaching hawking/*.py, and the model
    ...
    def test_filename_search_keeps_receipts_in_the_fallback_scope(self):
        from hawking.chat_tools import _search_fallback_glob
        self.assertEqual(
            _search_fallback_glob({"pattern": "KIMI_P0_OPERATIONAL.json"}),
            "*.json")
        self.assertEqual(
            _search_fallback_glob({"path": "docs/HAWKING_WEB.md"}),
            "*.md")
    ...
    def test_zero_result_prose_phrase_falls_back_to_source_tokens(self):
        from hawking.chat_tools import escalate_search
        ...
        class PhraseRegistry:
            ...
            def invoke(self, name, arguments):
                ...
                matches = ([{"path": "hawking/serve.py", "line": 1}]
    ...
    def test_phrase_fallback_prefers_active_source_over_archaeology(self):
        ...
        result = escalate_search(
            RankedRegistry(),
            {"pattern": "globally installed HAWKING must"},
            initial,
        )
        self.assertEqual(result.value["pattern"], "installed")
        self.assertEqual(result.value["matches"][0]["path"], "hawking/cli.py")
...
class TestBoundedPathFeedback(unittest.TestCase):
    def test_root_escape_explains_the_repository_relative_retry(self):
        from hawking.chat_tools import _render
    ...
    def test_receipt_escape_explains_the_receipt_namespace(self):
        from hawking.chat_tools import _render
        result = _Result(error="PermissionError: receipt path must be under receipts or .hawking state",
                         value=None)
        result.ok = False
...
class TestOneSystemMessage(unittest.TestCase):
    ...
    def test_the_contract_merges_into_an_existing_system_message(self):
        from hawking.chat_tools import prepend_system
...
class TestTheBodysOwnDialect(unittest.TestCase):
    ...
    def test_the_exact_reply_sealed_3_14_produced_is_understood(self):
        self.assertEqual(parse_call(self.OBSERVED), ("fs.list", {"path": "hawking"}))
...
class TestTheXmlDialect(unittest.TestCase):
    ...
    def test_the_opener_missing_its_leading_bracket_still_parses(self):
        ...
        # intact (od -c confirmed in .hawking/selfdev evidence, 3x in one reply).
    ...
    def test_the_opener_missing_the_whole_function_token_still_parses(self):
        ...
        # after that fix deployed (.hawking/selfdev/evidence/cycle-0012.txt,
        ...
        self.assertEqual(
            parse_call(observed),
            ("fs.read", {"path": "hawking/engine.py",
    ...
    def test_a_doubled_equals_opener_still_parses(self):
        ...
        observed = ("<tool_call>\n==fs.read>\n<parameter=path>\n"
                    "hawking/tests/test_splicing_ops_are_not_falsely_rejected.py\n"
                    "</parameter>\n</function>\n</tool_call>")
        self.assertEqual(
            parse_call(observed),
            ("fs.read", {"path": "hawking/tests/test_splicing_ops_are_not_falsely_rejected.py"}))
...
class TestObservationsAreDiskFirst(unittest.TestCase):
    ...
    def test_the_line_truncation_would_have_destroyed_is_retrievable(self):
        from hawking.chat_tools import RESULT_CHARS, run_local_tool
        big = self._big()
        obs = self._observe(big)
...
class TestCapabilityDoorsAreServedLocally(unittest.TestCase):
    """reverse.identify and forensics.snapshot are HAWKING-served, one owner each."""
