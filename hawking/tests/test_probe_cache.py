...
HAWKING invocation and every test shard paid it again: measured 1.14 s inside a
...
from hawking.backends import _probe_cached, _probe_key, _probe_store
...
class TestProbeCache(unittest.TestCase):
    ...
    def test_an_unwritable_cache_is_not_fatal(self):
        ...
        old = os.environ.get("HAWKING_PROBE_CACHE")
        try:
            os.environ["HAWKING_PROBE_CACHE"] = "/dev/null/nope"
            _probe_store("/bin/sh", "help", "x")  # must not raise
        finally:
            if old is None:
                os.environ.pop("HAWKING_PROBE_CACHE", None)
            else:
                os.environ["HAWKING_PROBE_CACHE"] = old
