...
from hawking.ledger import Ledger
    ...
    def test_compiled_obligations_have_no_unfailable_verify_commands(self):
        ...
        with tempfile.TemporaryDirectory(prefix="p0-1-") as tmp:
            ...
            try:
                ...
                goal_md = (Path(tmp) / ".hawking" / "GOAL.md").read_text(
                    encoding="utf-8"
                )
            finally:
                ctrl.shutdown()
...
class TestP0_3HawkingTerminalState(unittest.TestCase):
    def test_legacy_grok_alias_uses_hawking_engine_and_preserves_failure(self):
        from hawking.executors import WorkUnitExecutor

        class FakeEngine:
            def execute_workunit(self, unit, context):
                del context
                return {
                    "validation": {"ok": False, "reason": "provider_failed"},
                    "unit_id": unit.id,
                }

        with tempfile.TemporaryDirectory(prefix="p0-3-fail-") as tmp:
            wu = _wu("g-fail", preferred_backend="grok")
            raw = WorkUnitExecutor(tmp, engine=FakeEngine()).execute(wu, {})
            self.assertEqual(raw["backend"], "hawking")
            self.assertFalse(raw["validation"]["ok"], raw)


class TestP0_11ErrorPathEvidence(unittest.TestCase):
    def test_engine_source_has_no_empty_evidence_literal(self):
        import hawking.engine as engine_mod
    ...
    def test_many_obligations_produce_many_traceable_workunits(self):
        ...
        with tempfile.TemporaryDirectory(prefix="p0-12-") as tmp:
            ...
            try:
                result = ctrl.start_ultragoal(text)
                dag = json.loads(
                    (Path(tmp) / ".hawking" / "dag.json").read_text(encoding="utf-8")
                )
                goal_md = (Path(tmp) / ".hawking" / "GOAL.md").read_text(
                    encoding="utf-8"
                )
