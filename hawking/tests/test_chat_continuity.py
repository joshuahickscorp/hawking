...
from hawking.chat_state import (
    checkpoint,
    compact,
    estimate_tokens,
    resume,
...
from hawking.chat_state import ChatSession
...
class TestCheckpointAndResume(unittest.TestCase):
    ...
    def test_a_moved_repository_is_reported_not_replayed(self):
        ...
        from hawking.chat_state import _head_commit
    ...
    def test_build_checkpoint_carries_budget_and_dirty_source_revision(self):
        self.session.tool_budget = {
        ...
        record = checkpoint(
            self.session,
            source_revision="abc123",
            tool_budget=self.session.tool_budget,
            state="CHECKPOINT",
            next_action="reconcile current Git evidence",
        )
        restored = ChatSession.load(self._tmp.name, "s1")
        restored.tool_budget = dict(self.session.tool_budget)
        restored.save()
        reloaded = ChatSession.load(self._tmp.name, "s1")
        self.assertEqual(record["source_revision"], "abc123")
        self.assertEqual(record["state"], "CHECKPOINT")
        self.assertEqual(record["tool_budget"]["remaining"], 17)
        self.assertEqual(reloaded.tool_budget["source_revision"], "abc123")
