...
from hawking.mission import Mission, MissionCorruptError
...
class TestMissionLoop(unittest.TestCase):
    ...
    def test_atomic_write_and_truncated_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                os.replace = real
            state = Path(tmp) / ".hawking" / "mission" / "state.json"
    ...
    def test_cancel_checkpoints(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((Path(tmp) / ".hawking" / "mission" / "state.json").is_file())
    ...
    def test_controller_status_reports_hawking_pool_separately_from_wu_flags(self):
        with tempfile.TemporaryDirectory() as tmp:
            ws = Path(tmp) / "ws"
            ws.mkdir()
            ...
            self.assertIn("hawking", snap)
            self.assertNotIn("qwen", snap)
            self.assertNotIn("grok", snap)
            ...
            self.assertIsInstance(snap["hawking"], dict)
