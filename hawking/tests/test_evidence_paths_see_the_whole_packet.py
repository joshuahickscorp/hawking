...
Measured, receipt `.hawking/receipts/16c237d7-d2e0-40b4-9b62-742e6edf2824.json`:
a mutation WorkUnit was compiled with `tools/test_hawking_metric_never_zeroes_unknown.py`
...
The run was under `HAWKING_NO_TOOLS=1`, where evidence is the ONLY channel to a
...
lines: "your old_text for hawking/engine.py matches nothing in the file -- not one
    ...
    packet = _packet(
        {
            ...
            "invariants": [
                "Change it so that tools/test_hawking_metric_never_zeroes_unknown.py passes."
            ],
            "acceptance_criteria": [],
            "referenced_files": [],
        }
    )
    assert "tools/test_hawking_metric_never_zeroes_unknown.py" in packet.evidence_paths, (
    ...
    packet = _packet(
        {
            ...
            "invariants": [],
            "acceptance_criteria": [
                "the named tests collect at least one case: tools/hawking_metric.py"
            ],
            "referenced_files": ["tools/hawking_metric.py"],
        }
    )
    assert "tools/hawking_metric.py" in packet.evidence_paths, (
...
def test_the_rendered_prompt_does_not_say_none_when_it_names_a_file():
    packet = _packet(
        {
            "goal": "make the dashboard honest",
            "invariants": ["Do not weaken tools/hawking_metric.py."],
            "acceptance_criteria": [],
            "referenced_files": [],
        }
    )
    assert "EVIDENCE_PATHS:\n(none)" not in packet.prompt, (
        ...
        "HAWKING_NO_TOOLS that leaves the model a filename and no bytes"
    )
...
def test_a_referenced_file_the_unit_never_re_mentions_is_still_shown():
    ...
    The root goal named `tools/hawking_metric.py`; the compiler captured it in
    ...
    packet = _packet(
        {
            ...
            "acceptance_criteria": [],
            "referenced_files": [
                "tools/hawking_metric.py",
                "tools/test_hawking_metric_never_zeroes_unknown.py",
            ],
        }
    )
    assert "tools/hawking_metric.py" in packet.evidence_paths, (
...
def test_implementation_source_is_first_when_engine_and_test_are_declared():
    unit = WorkUnit(
        ...
        role="implementation",
        description=(
            "Use hawking/test_engine_tool_loop.py to prove the next executable "
            "step in hawking/engine.py."
        ),
    )
    packet = compile_worker_context(
        unit,
        {
            ...
            "referenced_files": [
                "hawking/engine.py",
                "hawking/test_engine_tool_loop.py",
            ],
        },
        phase="running",
        units={},
    ...
    assert packet.evidence_paths[0] == "hawking/engine.py"
