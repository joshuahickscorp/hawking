...
and then said:
    ...
    What I could not check: I attempted the broader `hawking/tests` suite to check
    neighbours, but the tool timed out at 30s

It was doing exactly what the loop asks -- step 8 of the arc is "check
neighbours" -- and the tool made it impossible. `hawking/tests` takes 167 seconds.
...
class TestTestsRunCanRunASuite(unittest.TestCase):
    ...
    def test_the_handler_still_intends_a_five_minute_default(self):
        ...
        src = inspect.getsource(
            __import__("hawking.tool_registry", fromlist=["_tests_run"])._tests_run)
