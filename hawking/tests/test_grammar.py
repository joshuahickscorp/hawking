...
from hawking.cli import parse_hawking_args, resolve_resident_runtime_limit
from hawking.machine import live_machine_identity
...
class TestGrammar(unittest.TestCase):
    ...
    def test_zero_rejected(self):
        with self.assertRaises(SystemExit):
            parse_hawking_args(["0", "p"])

    def test_large_runtime_count_is_not_clamped_by_cli(self):
        args = parse_hawking_args(["64", "p"])
        self.assertEqual(args.runtime_count, 64)

    def test_max_resolves_and_sets_source(self):
        saved = os.environ.pop("HAWKING_RESIDENT_RUNTIME_LIMIT", None)
        try:
            args = parse_hawking_args(["max", "p"])
        finally:
            if saved is not None:
                os.environ["HAWKING_RESIDENT_RUNTIME_LIMIT"] = saved
        self.assertEqual(args.prompt, "p")
        self.assertFalse(args.interactive)
    ...
    def test_max_env_wins(self):
        with patch.dict(os.environ, {"HAWKING_RESIDENT_RUNTIME_LIMIT": "2"}, clear=False):
            args = parse_hawking_args(["max", "hello"])
        self.assertEqual(args.runtime_count, 2)
        self.assertEqual(args.max_source, "HAWKING_RESIDENT_RUNTIME_LIMIT")
    ...
    def test_max_genome_then_equilibrium_then_fallback(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            eq_dir = workspace / ".hawking-legacy" / "bootstrap-director-v6"
