...
def test_profile_echo_is_a_real_invocation():
    ...
    assert receipt["schema"] == "hawking.perception.tool_receipt.v1"
...
def test_profile_missing_tool_is_absent_not_empty_success():
    rec = profile(["perception-no-such-tool-abcxyz"])
...
def test_report_lists_e3_classes_from_hawking_local_organs():
    doc = report()
