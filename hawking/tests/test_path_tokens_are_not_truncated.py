...
from hawking.engine import _PATH_TOKEN_RE
...
class TestPathTokens(unittest.TestCase):
    def test_jsonl_survives(self):
        self.assertEqual(
            tokens("read .hawking/school/COMPILE_ECONOMICS.jsonl now"),
            [".hawking/school/COMPILE_ECONOMICS.jsonl"],
        )
    ...
    def test_ordinary_source_paths_are_unaffected(self):
        self.assertEqual(
            tokens("edit hawking/engine.py and crates/x/src/lib.rs"),
            ["hawking/engine.py", "crates/x/src/lib.rs"],
        )
...
class TestPacketFileExtractor(unittest.TestCase):
    ...
    def test_ordinary_paths_still_extract(self):
        """Negative control."""
        got = self._files("edit hawking/engine.py and docs/x.md")
        self.assertIn("hawking/engine.py", got)
