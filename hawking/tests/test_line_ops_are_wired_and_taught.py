...
`old_lines` / `new_lines` were added to HAWKING_RESULT_SCHEMA precisely because the
...
`.hawking/receipts/97444aca-9035-4dab-85e4-7cd27a794d4d.json` is what it costs:
...
2. `hawking/mutation.py::apply_mutation_operations` reads `op["old_text"]` and
   `op["new_text"]` directly and has no idea `*_lines` exists. It is not on the
   live path today -- `hawking/engine.py::_apply_operations` is -- but
...
from hawking import engine as eng
from hawking.mutation import apply_mutation_operations
...
def test_the_resolver_is_shared_so_two_readers_cannot_disagree():
    ...
    import hawking.mutation as mut
    assert hasattr(mut, "operation_text"), (
        "hawking/mutation.py has no line-form resolver of its own and does not "
