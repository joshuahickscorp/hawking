...
# These vendor-owned surfaces were retired during the HCLI/Grok
# consolidation. They are intentionally absent, not hidden aliases: keeping
# them callable would revive the obsolete authority boundary.
RETIRED = {
    "grok.swarm.launch",
    "grok.swarm.propose",
    "vmcp.capabilities",
...
def test_every_pre_consolidation_name_still_resolves(tmp_path):
    ...
    lost = [name for name in BEFORE if reg.get(name) is None and name not in RETIRED]
    assert lost == [], f"consolidation DELETED capability, not surface: {lost}"
    assert all(reg.get(name) is None for name in RETIRED)
...
def test_the_surface_actually_shrank(tmp_path):
    ...
    absorbed = {n for n in BEFORE if n not in visible and n not in RETIRED}
