...
from hawking.dag_store import DagCorruptError, DagStore, atomic_write_json
...
class TestDagStore(unittest.TestCase):
    def test_workunit_execution_identity_and_capability_lease_roundtrip(self):
        with tempfile.TemporaryDirectory() as tmp:
            wu = _wu(
                "identity",
                required_capabilities=["workspace_read", "process_execution"],
                capability_lease={"lease_id": "lease-1", "state": "READY"},
                worker_id="hawking-worker-1",
                worker_attempt_id="attempt-1",
                context_packet_digest="a" * 64,
            )
            store = DagStore(tmp)
            store.save({wu.id: wu})
            loaded = store.load()[wu.id]
            self.assertEqual(loaded.worker_id, "hawking-worker-1")
            self.assertEqual(loaded.worker_attempt_id, "attempt-1")
            self.assertEqual(loaded.context_packet_digest, "a" * 64)
            self.assertEqual(
                loaded.required_capabilities,
                ["WORKSPACE_READ", "PROCESS_EXECUTION"],
            )
            self.assertEqual(loaded.capability_lease["state"], "READY")
...
class TestGrokRestartAdoption(unittest.TestCase):
    ...
    def test_live_grok_unit_is_interrupted_without_external_adoption(self):
        with tempfile.TemporaryDirectory() as tmp:
            task_id = "consult-20260101-111111"
            wu = _running_grok_unit(task_id=task_id)
            store = self._roundtrip(tmp, wu)
            def liveness(tid: str):
                raise AssertionError(f"external liveness must not be consulted: {tid}")
                return {
                    "state": "running",
                    "exit_code": None,
                    "process_alive": True,
    ...
    def test_done_nonzero_grok_unit_is_interrupted_without_external_adoption(self):
        with tempfile.TemporaryDirectory() as tmp:
