...
and resent it whole, so HAWKING could not remember a plan or resolve "apply it".
...
from hawking.chat_state import (ChatSession, Plan, record_tool_trace, session_key,
                             working_set)
...
class TestPromotionIsNarrow(unittest.TestCase):
    ...
    def test_asking_for_a_plan_creates_one(self):
        from hawking.chat_state import remember_turn
...
class TestApplyIntentIsImperative(unittest.TestCase):
    ...
    def test_every_case(self):
        from hawking.chat_state import asked_to_apply
...
class TestBrowserToolTrace(unittest.TestCase):
    def test_streaming_trace_and_final_are_durable(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(value["schema"], "hawking.chat.tool-trace.v1")
