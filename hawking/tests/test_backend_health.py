...
from hawking.engine import NoOpMutation
from hawking.grok_bridge import GrokContractError, GrokNotAvailable, GrokRunError
from hawking.mutation import MutationError
from hawking.resources import ResourceLimits, can_admit
...
class TestRetryBudget(HealthTest):
    ...
    def test_grok_not_available_does_not_burn_budget_but_trips_circuit(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            health.record_failure("hawking", ctx)
            health.record_failure("hawking", ctx)
            self.assertEqual(health.state("hawking"), STATE_CIRCUIT_OPEN)
            self.assertFalse(health.allows_new_assignments("hawking"))
...
class TestCircuitBreaker(HealthTest):
    ...
    def test_n_failures_open_one_backend_leave_the_other_healthy(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for _ in range(n):
                health.record_failure("hawking", ctx)
            hawking = health.snapshot("hawking")
            cpu = health.snapshot("cpu")
            self.assertEqual(hawking["consecutive_failures"], n)
            self.assertEqual(hawking["state"], STATE_CIRCUIT_OPEN)
            self.assertFalse(hawking["allows_new"])
            self.assertFalse(health.allows_new_assignments("hawking"))
    ...
    def test_breaker_reopens_after_cooling_with_injected_clock(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(health.state("cpu"), STATE_HEALTHY)
            self.assertTrue(health.allows_new_assignments("cpu"))
    ...
    def test_success_closes_the_breaker(self):
        with tempfile.TemporaryDirectory() as tmp:
            clock = FakeClock()
            health = _health(tmp, clock=clock, failure_threshold=2)
            health.record_failure("hawking", GrokRunError("blip"))
            health.record_failure("hawking", GrokRunError("blip"))
            self.assertEqual(health.state("hawking"), STATE_CIRCUIT_OPEN)
            clock.advance(1.0)
            snap = health.record_success("hawking")
            self.assertEqual(snap["state"], STATE_HEALTHY)
            self.assertEqual(snap["consecutive_failures"], 0)
    ...
    def test_below_threshold_is_degraded_not_open(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(health.allows_new_assignments("cpu"))
            self.assertEqual(health.state("hawking"), STATE_HEALTHY)
...
class TestHealthPersistence(HealthTest):
    ...
    def test_survives_in_process_reload(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path = Path(tmp) / ".hawking" / HEALTH_FILENAME
    ...
    def test_written_by_one_process_read_by_another(self):
        with tempfile.TemporaryDirectory() as tmp:
            script = (
                "import sys\n"
                ""
                "from hawking.resources import BackendHealth\n"
                f"h = BackendHealth({tmp!r}, failure_threshold=3, cooling_seconds=30.0)\n"
                "h.record_failure('hawking', {'error': 'GrokNotAvailable'})\n"
                "h.record_failure('hawking', {'error': 'GrokNotAvailable'})\n"
            ...
            hawking = reader.snapshot("hawking")
            cpu = reader.snapshot("cpu")
            self.assertEqual(hawking["consecutive_failures"], 2)
            self.assertEqual(hawking["state"], STATE_DEGRADED)
            self.assertIsNotNone(hawking["last_failure_time"])
            self.assertIsNotNone(cpu["last_success_time"])
            self.assertEqual(cpu["consecutive_failures"], 0)
            self.assertEqual(hawking["consecutive_failures"], 2)
    ...
    def test_atomic_write_uses_temp_and_replace(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                health = _health(tmp)
                health.record_success("hawking")
            finally:
                os.replace = real
            self.assertTrue(calls)
            src, dst = calls[-1]
            self.assertIn(".tmp", Path(src).name)
            self.assertEqual(Path(dst), Path(tmp) / ".hawking" / HEALTH_FILENAME)
...
class TestSchedulerAccessor(HealthTest):
    ...
    def test_known_backends_are_hawking_and_cpu(self):
        self.assertEqual(KNOWN_BACKENDS, ("hawking", "cpu"))
