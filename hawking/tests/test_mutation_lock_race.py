...
from hawking.resources import MutationLock, process_start_token
...
# racers, two tests), and each one imported hawking.resources before it could
...
class _RacerPair:
    ...
    def race(self, workspace: str, mode: str) -> tuple[bool, bool]:
        Path(workspace, ".hawking", "mutation.lock").unlink(missing_ok=True)
...
class TestMutationLockStaleHolder(unittest.TestCase):
    ...
    def test_dead_holder_is_recoverable(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(ready.read_text(encoding="utf-8"), "1")
            self.assertTrue((Path(tmp) / ".hawking" / "mutation.lock").is_file())
