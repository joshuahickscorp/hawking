...
`load_artifact` from `hawking.engine`. That function has never existed there --
`git log -S"def load_artifact" -- hawking/` is empty, and the only definition in the
repo is an unrelated `load_artifact(blob)` in research/lab/operators/glm52_pack.py.
The import raised at COLLECTION time, so `pytest hawking/` aborted before running
