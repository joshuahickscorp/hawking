...
round as a CODE MUTATION, so it demanded red-before-green with an admissible pytest. HAWKING
...
def test_one_source_file_anywhere_keeps_the_strict_contract():
    ...
    assert Engine._is_evidence_only([
        Path("receipts/future/X.json"), Path("hawking/engine.py")]) is False
...
def test_a_receipt_outside_receipts_is_not_evidence():
    ...
    assert Engine._is_evidence_only([Path("hawking/config.json")]) is False
...
def test__validate_ACTUALLY_ROUTES_to_the_evidence_path():
    ...
    """
    src = (REPO / "hawking" / "engine.py").read_text()
