"""The hands: one mutation transaction, reusable by any HAWKING surface.

`Engine.execute()` already owned snapshot -> apply -> prove -> validate ->
accept-or-roll-back, but only as part of its own model loop. General HAWKING (the
...
class TestTheTransaction(unittest.TestCase):
    ...
    def test_missing_required_test_rolls_back_no_evidence_edit(self):
        """A required but uncollectable proving test cannot leave a live edit."""
        target = self.root / "mod.py"
        target.write_text("VALUE = 1\n", encoding="utf-8")
        got = self.engine.apply_typed_mutation(
            [{"op": "replace", "path": "mod.py",
