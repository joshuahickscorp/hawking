...
from hawking.flash_next import PINNED_REVISION
from hawking.agentos.fpga_preboard import simulate_partition
from hawking.agentos.protected_benchmark_watcher import _classify_blockers
from hawking.agentos.protected_accelerator_benchmark import _aggregate, _request_record
...
def test_flash_executable_scaffold_writes_honest_budgets(tmp_path):
    repo = tmp_path / "repo"
    repo.mkdir()
    source = str(preferred_flash_science_receipt("/Users/scammermike/Downloads/hawking"))
...
def test_flash_executable_prefers_current_modellake_receipt_names(tmp_path, monkeypatch):
    ...
    # Both names are intentionally present: the sealed legacy name remains
    # compatibility receipts, while the descriptive names are the current
    # source-specimen observations.
    (receipts / MODELLAKE_CENSUS_RECEIPT_NAMES[1]).write_text(json.dumps({
        "status": "PASSED",
    ...
    (receipts / MODELLAKE_SUPERVISION_RECEIPT_NAMES[1]).write_text(json.dumps({
        "status": "LEGACY",
    }), encoding="utf-8")
...
def test_flash_graph_component_compiles_validated_noetic_receipts_without_promotion(tmp_path):
    ...
    transform = {
        "schema": "hawking.agentos.flash_transform_parity.v1",
        "status": "PASSED",
        "repo": "Qwen/Qwen3.8-Flash-Next",
    ...
    descriptor = {
        "schema": "hawking.noetic.representation_descriptor.v1",
        "candidate_id": "independent_q4_g64",
        "source_tensor": tensor,
        "full_transform_reference": candidate,
    }
    loader = {
        "schema": "hawking.agentos.flash_loader_roundtrip.v1",
        "status": "PASSED",
        "repo": "Qwen/Qwen3.8-Flash-Next",
        "pinned_revision": revision,
        "root": specimen,
    ...
    kernel = {
        ...
        "noetic_descriptor": {"schema": "hawking.noetic.representation_descriptor.v1"},
...
def test_flash_component_campaign_composes_two_source_independent_blocks(tmp_path):
    ...
    for index, row_start in enumerate((0, 128)):
        ...
        body = {
            "schema": "hawking.agentos.flash_noetic_component_body.v1",
            "status": "PASSED",
            "repo": "Qwen/Qwen3.8-Flash-Next",
            "pinned_revision": revision,
            "root": specimen,
...
def test_canonical_nomenclature_is_versioned_without_renaming_legacy_terms():
    assert NOMENCLATURE_VERSION == "HAWKING_NOMENCLATURE_V3"
...
def test_watcher_never_classifies_modellake_as_pausable():
    classes = _classify_blockers([
        ...
        {"job_id": "bench", "label": "hawking-diagnostic", "argv": ["python3", "-m", "hawking", "agentos", "accelerator-regression"], "pid": 8},
    ])
    assert classes["blockers"][0]["kind"] == "MODELLAKE_UNTOUCHABLE"
    assert classes["pausable_hawking_jobs"][0]["job_id"] == "bench"
