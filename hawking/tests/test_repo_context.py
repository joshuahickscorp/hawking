"""The folder you opened HAWKING in, and what it is allowed to do to an answer.

Two failures are guarded here, and they pull in opposite directions:

  * a session opened in a repo that knows nothing about it (what `hawking web` did
...
from hawking.repo_context import RepoContext, inject, prewarm_native_gravity_index


def _repo(tmp: Path, git: bool = True) -> Path:
    (tmp / "hawking").mkdir(parents=True)
    (tmp / "tools").mkdir(parents=True)
    (tmp / "README.md").write_text("# widget\n\nA thing that widgets.\n")
    (tmp / "hawking" / "catalog.py").write_text("CATALOG_MARKER = 'the real catalog'\n")
    (tmp / "tools" / "catalog.py").write_text("OTHER_MARKER = 'a different catalog'\n")
    (tmp / "hawking" / "serve.py").write_text("SERVE_MARKER = 'serving'\n")
    if git:
...
class TestDetection(unittest.TestCase):
    def test_a_git_repo_is_detected_from_a_subdirectory(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            found = RepoContext.detect(str(root / "hawking"))
    ...
    def test_prewarm_starts_native_child_without_a_query(self):
        ...
        with patch.dict("os.environ", {"HAWKING_NATIVE_GRAVITY": "1"}, clear=False), \
             patch("hawking.repo_context._native_gravity_binary", return_value=binary), \
             patch("hawking.repo_context._NativeGravityIndex") as spawned, \
             patch("hawking.repo_context._NATIVE_GRAVITY_INDEXES", {}):
    ...
    def test_native_binary_cache_drops_stale_paths_and_sees_later_install(self):
        import hawking.repo_context as repo_context
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            binary.chmod(0o755)
            with patch.dict("os.environ", {"HAWKING_GRAVITYD_BIN": str(binary)}, clear=False):
...
class TestRetrievalPrecision(unittest.TestCase):
    ...
    def test_a_named_path_returns_that_file_alone(self):
        # THE DEFECT: asking about hawking/catalog.py returned it AND every other
        # catalog.py, and the answer was a fluent blend of files that describes
        # none of them.
        picked = [p for p, _ in self.ctx.files_for("what is hawking/catalog.py for?")]
        self.assertEqual(picked, ["hawking/catalog.py"], picked)

    def test_a_named_path_beats_a_stem_match_elsewhere(self):
        picked = [p for p, _ in self.ctx.files_for("how does hawking/serve.py work")]
        self.assertEqual(picked[0], "hawking/serve.py")
    ...
    def test_goal_only_question_uses_distinctive_content_for_orientation(self):
        target = self.root / "hawking" / "session_transport.py"
        target.write_text(
        ...
        self.assertEqual(picked[0], "hawking/session_transport.py")
...
class TestInjection(unittest.TestCase):
    ...
    def test_the_repo_is_named_and_the_file_is_included(self):
        out = inject([{"role": "user", "content": "what is hawking/catalog.py for?"}],
                     self.ctx)
    ...
    def test_the_stable_block_is_byte_identical_across_questions(self):
        ...
        first = inject([{"role": "user", "content": "what is hawking/serve.py"}],
                       self.ctx)[0]["content"]
        second = inject([{"role": "user", "content": "what is hawking/catalog.py"}],
                        self.ctx)[0]["content"]
