"""Program-level Hawking checkpoint and maturity census.
...
from hawking.persist import (
    atomic_write_json,
    read_json_object_or_none as _read_object,
)
from hawking.providers import (
    CAPABILITY_SCHEMA,
    FAILURE_SCHEMA,
...
SCHEMA = "hawking.program_checkpoint.v1"
DEFAULT_NAME = "HAWKING_CHECKPOINT.json"
...
def _gate_summary(workspace: Path, repo_root: Path) -> Dict[str, Any]:
    recovery_candidates = [
        workspace / ".hawking" / "receipts" / "recovery-gate.json",
        repo_root / "receipts" / "headless" / "recovery-gate.json",
        repo_root / "receipts" / "headless" / "HAWKING_AGENTOS_RECOVERY_GATE.json",
    ]
    research_candidates = [
        workspace / ".hawking" / "receipts" / "research-gate.json",
        repo_root / "receipts" / "headless" / "research-gate.json",
        repo_root / "receipts" / "headless" / "HAWKING_AGENTOS_RESEARCH_GATE.json",
    ]
    perception_candidates = [
        workspace / ".hawking" / "receipts" / "perception-gate.json",
        repo_root / "receipts" / "headless" / "HAWKING_PERCEPTION_GATE.json",
        repo_root / "receipts" / "headless" / "perception-gate.json",
        repo_root / "receipts" / "headless" / "HAWKING_AGENTOS_PERCEPTION_GATE.json",
    ]
    native_candidates = [
        workspace / ".hawking" / "receipts" / "native-gate.json",
        repo_root / "receipts" / "headless" / "HAWKING_NATIVE_GATE.json",
        repo_root / "receipts" / "headless" / "native-gate.json",
        repo_root / "receipts" / "headless" / "HAWKING_AGENTOS_NATIVE_GATE.json",
    ]
    resident_candidates = [
        workspace / ".hawking" / "receipts" / "resident-gate.json",
        repo_root / "receipts" / "headless" / "resident-gate.json",
        repo_root / "receipts" / "headless" / "HAWKING_RESIDENT_GATE.json",
        repo_root / "receipts" / "headless" / "HAWKING_AGENTOS_RESIDENT_GATE.json",
    ]
    mission_candidates = [
        workspace / ".hawking" / "receipts" / "native-mission-gate.json",
        repo_root / "receipts" / "headless" / "native-mission-gate.json",
        repo_root / "receipts" / "headless" / "HAWKING_NATIVE_MISSION_GATE.json",
    ]
    accelerator_candidates = [
        workspace / ".hawking" / "receipts" / "accelerator-native-smoke.json",
        repo_root / "receipts" / "headless" / "accelerator-native-smoke.json",
        repo_root / "receipts" / "headless" / "HAWKING_ACCELERATOR_NATIVE_SMOKE.json",
    ]
    autonomy_candidates = [
        workspace / ".hawking" / "receipts" / "autonomy-gate.json",
        repo_root / "receipts" / "headless" / "HAWKING_AUTONOMY_GATE.json",
        repo_root / "receipts" / "headless" / "autonomy-gate.json",
        repo_root / "receipts" / "headless" / "HAWKING_AGENTOS_AUTONOMY_GATE.json",
    ]
    unattended_candidates = [
        workspace / ".hawking" / "receipts" / "unattended-window.json",
        repo_root / "receipts" / "headless" / "HAWKING_UNATTENDED_WINDOW.json",
        repo_root / "receipts" / "headless" / "unattended-window.json",
        repo_root / "receipts" / "headless" / "HAWKING_AGENTOS_UNATTENDED_WINDOW.json",
    ]
    accelerator_regression_candidates = [
        workspace / ".hawking" / "receipts" / "accelerator-regression.json",
        repo_root / "receipts" / "headless" / "accelerator-regression.json",
        repo_root / "receipts" / "headless" / "HAWKING_ACCELERATOR_REGRESSION.json",
    ]
    fusion_source_audit_candidates = [
        workspace / ".hawking" / "receipts" / "qwen38-fusion-audit.json",
        repo_root / "receipts" / "headless" / "qwen38-fusion-audit.json",
        repo_root / "receipts" / "headless" / "HAWKING_QWEN38_FUSION_SOURCE_AUDIT.json",
    ]
    modellake_candidates = [
        workspace / ".hawking" / "receipts" / "modellake-census.json",
        repo_root / "receipts" / "headless" / MODELLAKE_CENSUS_RECEIPT_NAMES[0],
        repo_root / "receipts" / "headless" / "modellake-census.json",
        repo_root / "receipts" / "headless" / MODELLAKE_CENSUS_RECEIPT_NAMES[1],
    ]
    flash_science_candidates = [
        workspace / ".hawking" / "receipts" / "flash-science.json",
        repo_root / "receipts" / "headless" / "flash-science.json",
        *(repo_root / "receipts" / "headless" / name for name in FLASH_SCIENCE_RECEIPT_NAMES),
    ]
    preboard_candidates = [
        workspace / ".hawking" / "receipts" / "preboard.json",
        repo_root / "receipts" / "headless" / "preboard.json",
        repo_root / "receipts" / "headless" / "HAWKING_AGENTOS_PREBOARD.json",
    ]
    charge_candidates = [
        workspace / ".hawking" / "receipts" / "initial-charge.json",
    ...
    ab_candidates = [repo_root / "receipts" / "headless" / "HAWKING_DENSE_VS_NF_AB_SCAFFOLD.json"]
    fpga_candidates = [repo_root / "receipts" / "headless" / "HAWKING_FPGA_PREBOARD.json"]
    lake_supervision_candidates = [
        repo_root / "receipts" / "headless" / MODELLAKE_SUPERVISION_RECEIPT_NAMES[0],
        repo_root / "receipts" / "headless" / MODELLAKE_SUPERVISION_RECEIPT_NAMES[1],
    ]
    ...
    for path in research_candidates:
        ...
        if value is not None:
            ...
            break
    perception = None
    for path in perception_candidates:
        value = _read_object(path)
        if value is not None:
            perception = {
                "status": value.get("status"),
...
def build_program_checkpoint(
    ...
    *,
    ...
    from hawking.connectivity import probe_connectivity
    ...
    if gates["perception_gate"]["status"] != "PASSED":
        blockers.append("PERCEPTION operational evidence-boundary gate has not passed")
    if gates["native_gate"]["status"] != "PASSED":
        blockers.append("live native HAWKING A1-A6 ladder has not passed")
    ...
    perception = connectivity.get("surfaces", {}).get("perception", {})
    if perception.get("status") not in {"AVAILABLE", "AUTHENTICATED"}:
        blockers.append("PERCEPTION public surface is not fully importable/selected")
    ...
    return {
        ...
        "provider_neutral_contracts": {
            ...
            "role_policy_source": "hawking.providers.RoleRouter",
        ...
        "receipts": _receipt_inventory(
            (repo / "receipts", ws / ".hawking" / "receipts"),
        ),
        "maturity": {
            ...
            "perception": gates["perception_gate"]["qualification"],
            "native_hawking": gates["native_gate"]["qualification"],
