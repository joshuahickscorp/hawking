"""WorkUnit backend execution. One WorkUnit identity, one Hawking cognition path.
...
Provider output never marks a WorkUnit complete; the central verifier must.
"""
...
BACKEND_HAWKING = "hawking"
# Compatibility spellings are data adapters only. They never select a vendor
# process or a second scheduler.
BACKEND_QWEN = BACKEND_HAWKING
BACKEND_GROK = BACKEND_HAWKING
BACKEND_CPU = "cpu"
BACKEND_TOOL = "tool"
...
def _provider_receipt_request(request_payload: Mapping[str, Any]) -> Dict[str, Any]:
    """Keep provider receipts structural; WorkUnit prompts are not receipts."""
    if "messages" in request_payload:
        ...
        return {
            ...
            "has_response_schema": bool(request_payload.get("response_schema")),
        }
    return {
        "unit_id": request_payload.get("unit_id"),
        "role": request_payload.get("role"),
        "description_chars": len(str(request_payload.get("description") or "")),
        "context_keys": sorted(
            str(key)
            for key in (request_payload.get("context") or {})
            if str(key) not in {"prompt", "context_memory", "packet"}
        ),
    }
...
def select_backend_name(wu: Any) -> str:
    ...
    if pref:
        lowered = pref.lower()
        if lowered == "grok":
            return BACKEND_HAWKING
        if lowered.startswith("grok:"):
            return "hawking:" + pref.split(":", 1)[1]
        return pref if ":" in pref else lowered
    rc = str(getattr(wu, "resource_class", "") or "")
    if rc in {"GROK", "HAWKING"}:
        return BACKEND_HAWKING
...
class WorkUnitExecutor:
    def __init__(
        ...
        self.engine = engine
        # Accepted for old callers, deliberately ignored. Hawking cognition
        # is native/provider-neutral and never calls the Grok adapter.
        self._legacy_bridge = grok_bridge
        self._tools = tool_registry
        self.repo_root = Path(repo_root) if repo_root else None
        self.providers = dict(providers or {})

    def tool_registry(self):
        """The registry, built lazily with the Hawking control plane.
    ...
    def execute(self, wu: Any, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if provider is None:
            from .remote_cognition import is_remote_selector, provider_for_selector

            if is_remote_selector(name):
                provider = provider_for_selector(name, workspace=self.workspace, engine=self.engine)
        # A provider may intentionally be the same object as the engine (the
        # AgentOS ``resident`` alias is the normal example).  Only the
        # historical implicit native route is special; every explicitly
        ...
        # The Hawking engine path is the one backend that already emits its own
    ...
    def _emit_wrapped(self, wu, backend_name, context, fn):
        """provider/tool/cpu paths never touch ``self.engine`` in their own
    ...
    def _run_provider(
        ...
        """Invoke a provider without teaching Hawking its transport."""
        started = time.time()
        ...
        if callable(workunit_call):
            ...
            if (
                getattr(provider, "is_remote_cognition", False)
                and provider_context.get("hawking_tools") is not False
            ):
                # A Hawking-owned WorkUnit may receive an explicitly projected
                # registry. The provider never receives a shell escape or a
                # client-defined tool contract; ToolRegistry remains the sole
                # dispatcher and its permissions/mutation lock remain final.
                try:
                    provider_context.setdefault("tool_registry", self.tool_registry())
                    provider_context.setdefault(
                        "tool_write_authority",
                        bool(provider_context.get("tool_write_authority", False)),
                    )
                except Exception:
                    provider_context["hawking_tools"] = False
            try:
                raw = workunit_call(wu, provider_context)
            except Exception as exc:
                if not getattr(provider, "is_remote_cognition", False):
                    raise
                failure = getattr(provider, "failure_result", None)
        ...
        try:
            ...
            raw.setdefault(
                ...
                ProviderReceipt(
                    ...
                    request=(
                        _provider_receipt_request(request_payload)
                        if getattr(provider, "is_remote_cognition", False)
    ...
    def _run_engine(
        ...
        context: Dict[str, Any],
        *,
        backend_name: str = BACKEND_HAWKING,
    ) -> Dict[str, Any]:
        engine = self.engine
        raw: Any = {}
    ...
    # private helper before the Hawking-native rename.
    ...
    def _run_cpu(self, wu: Any, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        timeout = float(os.environ.get("HAWKING_CPU_TIMEOUT", "120"))
        try:
            proc = subprocess.run(
                cmd,
                shell=True,
    ...
    def _run_hawking(self, wu: Any, context: Dict[str, Any]) -> Dict[str, Any]:
        from .remote_cognition import HawkingNativeProvider

        provider = self.providers.get(BACKEND_HAWKING)
        if provider is None:
            provider = HawkingNativeProvider(
                workspace=self.workspace,
                engine=self.engine,
                model_id=str(context.get("model") or "native-resident"),
            )
        return self._run_provider(provider, BACKEND_HAWKING, wu, context)
...
def gather_evidence_paths(
    ...
    " ".join(paths) -- so for hawking/tool_registry.py the shared identifiers were
    {hawking, tests, tool_registry}, none of which are symbols in that file. The
...
def consult_worker(provider: Any, packet: WorkerPacket, **kwargs: Any) -> Any:
    """A compatibility consult sees only the compiled packet, never the goal."""
    if not isinstance(packet, WorkerPacket):
        raise EngineError("Hawking consult for a worker requires a WorkerPacket")
    if "GOAL:" in packet.prompt:
        raise EngineError("worker packet must not contain GOAL:")
    consult = getattr(provider, "consult", None)
    if not callable(consult):
        raise EngineError("Hawking provider is missing consult()")
    return consult(packet.prompt, **kwargs)


def _install_engine_hooks() -> None:
    current = getattr(Engine, "_build_model_payload", None)
    if current is not None and not getattr(current, "_hawking_worker_payload", False):
        try:
        ...
        if "del compiled" in src and "GOAL:" in src:
            ...
            _build_model_payload._hawking_worker_payload = True  # type: ignore[attr-defined]
