...
# HAWKING_EVENT_COMPAT_V1

def _hawking_event_from_parts(event_type, payload=None):
...
_HAWKING_ORIGINAL_EVENTBUS_EMIT = EventBus.emit


def _hawking_compatible_eventbus_emit(self, event, payload=None):
    """Accept both native Event objects and emit(type, payload)."""

    if isinstance(event, str):
        event = _hawking_event_from_parts(
            event,
            payload,
        )

    return _HAWKING_ORIGINAL_EVENTBUS_EMIT(
        self,
        event,
    )


_hawking_compatible_eventbus_emit._hawking_event_compat = True
EventBus.emit = _hawking_compatible_eventbus_emit
