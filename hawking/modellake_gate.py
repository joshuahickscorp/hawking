...
from hawking.flash_next import EXPECTED_BYTES, EXPECTED_FILE_COUNT, PINNED_REVISION, REPO_ID
from hawking.persist import (
    atomic_write_json,
    read_json_object_or_none as _read_json,
    sha256_file_or_none as _sha256,
)
from hawking.processes import NativeProcessError, live_processes


SCHEMA = "hawking.modellake_census.v1"
...
def _write(report: Dict[str, Any], emit: Optional[str], repo_root: Path) -> None:
    destination = Path(emit).expanduser() if emit else repo_root / "receipts" / "headless" / "MODELLAKE_FLASH_NEXT_CENSUS.json"
...
def record_modellake_causality(report: Dict[str, Any], **kwargs: Any) -> dict[str, Any]:
    ...
    return sc.stamp_gate(
        report,
        source="hawking/modellake_gate.py::run_modellake_census",
        **payload,
    )
...
class _ExecutionBoundary:
    """P16 execution boundary: a fresh attempt must not replay a prior provider turn."""

    allow_provider_replay = False


EXECUTION_BOUNDARY = _ExecutionBoundary()