"""Physical, bounded Hawking recovery gate.
...
SCHEMA = "hawking.recovery_gate.v1"
    ...
    def generate(self, request: Any, *, timeout: Optional[float] = None) -> GenerationResponse:
        ...
        if self.mode == "host":
            ...
            child = subprocess.Popen(
                [sys.executable, "-m", "hawking.recovery_gate", "--fixture-resident", str(self.workspace)],
...
def _malformed_json_case() -> Dict[str, Any]:
    from hawking.hawking_native import HawkingNativeProtocolError, ResidentProcess
...
def causality_payload(report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "probe_performed": (
            "disposable fixture: spawn Hawking host + fixture resident, SIGKILL resident "
...
def record_recovery_causality(report: Dict[str, Any], **kwargs: Any) -> dict[str, Any]:
    ...
    return sc.stamp_gate(
        report,
        source="hawking/recovery_gate.py::run_recovery_gate",
        **payload,
    )
...
def run_recovery_gate(
    ...
    *,
    ...
    root = Path(workspace).expanduser().resolve() if workspace else Path(tempfile.mkdtemp(prefix="hawking-recovery-gate-"))
    root.mkdir(parents=True, exist_ok=True)
    started = time.time()
    host = subprocess.Popen(
        [sys.executable, "-m", "hawking.recovery_gate", "--fixture-host", str(root)],
        cwd=str(root),
        env=_module_environment(),
        start_new_session=True,
        stdout=subprocess.DEVNULL,
    ...
    report: Dict[str, Any] = {
        ...
        "recovery": recovery,
        "controls": controls,
        "claim_boundary": "This proves the tested Hawking control-plane failure paths with a disposable fixture; it is not proof of production resident sovereignty.",
    ...
    receipt_path = root / ".hawking" / "receipts" / _GATE_RECEIPT
