"""`hawking use` -- see admitted Gravity bodies, and change which one is answering.
...
follows the change with no reconnect, no restart and no new port. `hawking use` and
...
with no explanation -- is a bad way to learn it. `hawking use qwen3-14b` pays the
...
from .cli import DEFAULT_ENDPOINT_BASE_URL
...
def render(bodies: List[Body], current: Optional[str], notes: List[str]) -> str:
    ...
    if current:
        ...
        lines.append("")
        lines.append(" nothing is loaded. `hawking web` or `hawking serve` starts one.")
    for note in notes:
...
def main(argv: Optional[list] = None) -> int:
    ...
    ap.add_argument("--base", default=DEFAULT_ENDPOINT_BASE_URL)
    ...
    ap.add_argument("--resolve-only", action="store_true",
                    help="resolve one admitted exact action without contacting a resident")
    ap.add_argument("--action", choices=sorted(ACTION_IDS), default=None,
                    help="requested Hawking action for --resolve-only")
    ...
    if a.resolve_only:
        if not a.model:
            ...
            return 2
        if not a.action:
            print("--resolve-only requires --action", file=sys.stderr)
            return 2
        try:
            target = resolve_action(a.model, a.action, bodies)
        except (LookupError, PermissionError, ValueError) as exc:
            print(str(exc), file=sys.stderr)
            return 2
        if target is None:
            print(f"no admitted body named {a.model!r}; `hawking models` lists admitted bodies",
                  file=sys.stderr)
            return 2
        print(json.dumps(target.to_wire(), sort_keys=True))
        return 0
    ...
    try:
        target = resolve_action(a.model, "serve", bodies)
    except (LookupError, PermissionError, ValueError) as exc:
        print(str(exc), file=sys.stderr)
        return 2
    if target is None:
        print(f"no body named {a.model!r}. `hawking use` lists what is available.",
              file=sys.stderr)
        return 2
    ...
    if current is None:
        print(f"nothing is running on {a.base}, so there is nothing to switch.\n"
              ...
              f"    python -m hawking web --model {target.path}", file=sys.stderr)
        return 2
    print(f"checking {current} -> {target.name} ...", flush=True)
    status, body = post_json(a.base.rstrip("/") + "/v1/switch",
                             {"resolved_action": target.to_wire()}, a.timeout)
    if status != 200:
