...
from hawking.engine import Engine, _CHARS_PER_TOKEN
from hawking.goal import GoalCompiler

PROFILE = "/Users/scammermike/Downloads/hawking/hawking/hawking-native.sealed-3.14.json"
...
SMALL = "Read hawking/paths.py and report the directory names it defines."
...
def test_the_exact_source_is_preserved_hashed_and_reachable(tmp_path):
    ...
    rel = [w for w in block.split() if w.startswith(".hawking/sources/")]
...
def test_the_kernel_fits_the_budget_the_raw_goal_blew(tmp_path):
    from hawking.context_budget import preflight
...
def test_the_same_source_is_written_once(tmp_path):
    ...
    eng._goal_block(BIG, compiled)
    files = list((Path(tmp_path) / ".hawking" / "sources").glob("goal-*.txt"))
...
def test_build_model_payload_actually_uses_the_compiler(tmp_path):
    ...
    assert ".hawking/sources/" in user, "no retrievable source handle in the payload"
