"""Tool doctor (roadmap E.3 / E.4) hosted on Hawking.
...
dangerous argv are refused before exec. subprocess never uses shell=True.
    ...
    from hawking.perception.doctor import profile, report
"""
...
from hawking.perception.receipts import (
    argv_list,
    basename_of,
    content_digest,
    dangerous_command,
...
from hawking.latency import now_ns, since_ns
...
# E.3 required classes. CONNECTED means Hawking hosts a real local
# implementation. PARKED names the concrete local prerequisite that remains.
E3_CLASSES: tuple[tuple[str, str, str], ...] = (
    ("file classifier", "CONNECTED", "hawking.perception.file_eye.observe"),
    ("hashing", "CONNECTED", "hashlib"),
    ("archive/compression", "CONNECTED", "zipfile/tarfile/gzip"),
    ("browser/CDP", "PARKED", "Hawking web backend + host Chrome"),
    ("HTML/DOM capture", "PARKED", "Hawking web capture backend"),
    ("CSS parser", "PARKED", "Hawking CSS parser backend"),
    ("source-map parser", "PARKED", "Hawking source-map opener"),
    ("visual diff", "PARKED", "Hawking visual comparison backend"),
    ("image handling", "CONNECTED", "PNG/JPEG/GIF magic in hawking.perception.file_eye (no PIL)"),
    ("OBJ/GLTF parser", "PARKED", "Hawking 3D parser backend"),
    ("spatial validator", "PARKED", "Hawking 3D backend + Blender"),
    ("independent renderer/viewer", "PARKED", "Blender CLI"),
    ("PTY capture", "PARKED", "openpty EPERM in this sandbox; hawking.perception.terminal"),
    ("process inspection", "CONNECTED", "hawking.perception.doctor.profile"),
    ...
    *,
    ...
    if refused:
        ...
        receipt = tool_receipt(
            ...
            limitations=[refused],
            verifier="hawking.perception.doctor.profile",
        )
        return {
            "act": "check",
            "organ": "tool_doctor",
...
def report(*, arguments: Mapping[str, Any] | None = None) -> dict[str, Any]:
    """E.3 class table for this host and Hawking's local organs."""
    del arguments
    from hawking.perception.terminal import probe as pty_probe
    ...
    for name, disposition, hosted_by in E3_CLASSES:
        ...
        if name == "PTY capture":
            if probed.get("ok"):
                ...
                row["hosted_by"] = "hawking.perception.terminal.capture"
            else:
    ...
    return {
        ...
        "note": "Hawking-hosted doctor of local classes; PARKED rows name their reopening prerequisites.",
    }
