...
def _default_native_profile() -> Optional[str]:
    ...
    machine while keeping a checkout or installation without the resident
        fully usable for explicit Hawking-native selections.
    """
...
def _model_info_for_resolved_action(resolved_action: Any) -> ModelInfo:
    """Project an admitted catalog body for display without selecting it again."""
    body = getattr(resolved_action, "body", None)
    if body is None:
        raise ValueError("resolved action has no revalidated catalog body")
    detail = body.detail or {}
    provider = "native" if body.kind == "noetic_native" else body.source
    return ModelInfo(
...
def _live_mission_identity(workspace_root: Any) -> Optional[Dict[str, Any]]:
    ...
    """
    try:
        path = Path(str(workspace_root)) / ".hawking" / "mission" / "state.json"
        if not path.is_file():
            return None
...
class Controller:
    """HAWKING product controller and owner of the native Engine."""
    ...
    def __init__(
        ...
        self.canonical_execution = bool(canonical_execution)
        ...
        if resolved_action is not None:
            from .catalog import ResolvedAction, revalidate_action

            if not isinstance(resolved_action, ResolvedAction):
                raise TypeError("Controller requires a ResolvedAction value")
            resolved_action = revalidate_action(resolved_action, action="execute")
        self.resolved_action = resolved_action
        self.model = (
            resolved_action.path if resolved_action is not None else model
        )
        ...
        self._native_execution_refusal: Optional[str] = None
        ...
        default_native = None
        if not self.model and not project_model and not global_model and not migration_env:
            default_native = _default_native_profile()

        if self.resolved_action is not None:
            # The transport contract already chose and revalidated this exact
            # catalog body.  ModelRegistry remains an inventory/UI adapter;
            # letting it resolve a path here would recreate a second selector.
            self.model_info = _model_info_for_resolved_action(self.resolved_action)
            try:
                self.registry.selected = self.model_info
            except Exception:
                pass
        elif self.canonical_execution:
            # The ordinary Hawking application is a canonical native surface.
            # It may display goals and status while unbound, but must not ask
            # ModelRegistry to auto-discover an MLX/remote body just because a
            # caller did not select an admitted artifact.
            candidate = (
                self.model
                or project_model
                or global_model
            ...
            if candidate:
                try:
                    ...
                    selected_action = resolve_action(str(candidate), "execute")
                except (LookupError, PermissionError, ValueError) as exc:
                    self._native_execution_refusal = str(exc)
                    selected_action = None
                if selected_action is not None:
                    self.resolved_action = selected_action
                    self.model = selected_action.path
                    self.model_info = _model_info_for_resolved_action(selected_action)
                    try:
                        self.registry.selected = self.model_info
                    except Exception:
                        pass
                elif self._native_execution_refusal is None:
                    self._native_execution_refusal = (
                        f"{candidate!r} does not name an admitted Hawking-native "
                        "artifact for execution"
                    )
            else:
                self._native_execution_refusal = (
                    "no admitted Hawking-native artifact is bound"
                )
        else:
            self.model_info = self.registry.resolve(
                explicit=self.model or default_native,
                project_config=project_model,
                global_config=global_model,
                env=migration_env,
            )
            if self.model_info is not None:
                # Config/default/interactive adapters may still hand us an
                # admitted Gravity path. Normalize that value here, before
                # any runtime can use it, so the catalog remains the policy
                # owner for every ordinary artifact execution path.
                try:
                    from .catalog import resolve_action

                    selected_action = resolve_action(self.model_info.path, "execute")
        ...
        if self.model_info is not None:
            self.model = self.resolved_action.path if self.resolved_action is not None \
                else self.model_info.path
            self.session.model = self.model
        elif self.resolved_action is not None:
            self.model = self.resolved_action.path
            self.session.model = self.resolved_action.path
    ...
    def status(
        ...
        if self.resolved_action is not None:
            snapshot["resolved_action"] = self.resolved_action.to_wire()
        ...
        snapshot["hawking"] = runtime_status
        snapshot["cognition"] = self._cognition_status(units)
        ...
        snapshot.setdefault("hawking", self._hawking_pool_status())
    ...
    def _cognition_status(self, units=None) -> dict:
        """Report WorkUnit cognition without a second worker/pool census."""
        if units is None:
            _, units = self._scheduler_units()
        counts = {}
        for unit in units or []:
            provider = str(
                getattr(unit, "assigned_backend", None)
                or getattr(unit, "preferred_backend", None)
                or "hawking"
            )
            node = counts.setdefault(provider, {"pending": 0, "running": 0, "completed": 0, "failed": 0})
            status = str(getattr(unit, "status", "pending"))
            node[status] = int(node.get(status, 0)) + 1
        return {
            "authority": "hawking.mission.WorkUnit",
            "providers": counts,
            "external_worker": False,
        }

    def _max_status(self) -> dict:
        # The old max-policy census owned a vendor-process pool.  Hawking's
        # only live cognition census is the WorkUnit/provider snapshot above;
        # keep this status slot truthful for older renderers without probing
        # external processes or legacy task directories.
        return {
            "authority": "hawking",
    ...
    def _hawking_endpoint(self) -> str:
    ...
    def _last_hawking_tps(self) -> Optional[float]:
    ...
    def _hawking_pool_status(self) -> dict:
        pool = self.runtime_pool
        ...
        if runtimes:
            ...
            if endpoint and not endpoint.startswith(("http://", "https://")):
                ...
                return {
                    ...
                    "prompt_tokens": None,
                    "tps": self._last_hawking_tps(),
                    "backend": identity.get("backend"),
                    "endpoint": endpoint,
                    "details": details,
                }
        base = self._hawking_endpoint().rstrip("/")
    ...
    def _runtime_status(self) -> dict:
        ...
        runtimes = list(getattr(pool, "runtimes", []) or []) if pool is not None else []
        if not runtimes:
            status = self._hawking_pool_status()
    ...
    def select_model(
        ...
        try:
            ...
            selected_action = resolve_action(selected.path, "execute")
        except PermissionError as exc:
            self._emit("warning", {"message": str(exc)})
            return False
        except (LookupError, ValueError):
            # A registry with explicit test/research roots may still expose a
            # legacy provider.  Normal admitted catalog rows are normalized
            # above; preserve the documented compatibility adapter here.
            selected_action = None
        if self.runtime_pool is not None:
            self.runtime_pool.stop()
            self.runtime_pool = None
        if selected_action is not None:
            selected = _model_info_for_resolved_action(selected_action)
            try:
                self.registry.selected = selected
            except Exception:
                pass

        self.model_info = selected
        self.resolved_action = selected_action
        self.model = selected_action.path if selected_action is not None else selected.path
        self.session.model = self.model
        self._persist_session()
    ...
    def prior_knowledge_snapshot(
        ...
        *,
        ...
        if self._knowledge_error:
            return {
                "schema": "hawking.workspace_knowledge.v1",
                "available": False,
    ...
    def ensure_runtime_pool(
        ...
        if self.canonical_execution and self.resolved_action is None:
            raise RuntimeError(
                "Canonical Hawking execution is waiting for an admitted native "
                f"artifact: {self._native_execution_refusal or 'no artifact binding'}"
            )
        ...
        if self.runtime_pool is None:
            ...
            if self.canonical_execution:
                from .runtime_iface import make_canonical_backend_for_model

                backend_factory = make_canonical_backend_for_model
            pool = RuntimePool(
                model_path,
    ...
    def _memory_text(value: Any, limit: int = _MEMORY_TEXT_LIMIT) -> str:
        ...
        # this is where control tokens have to die. HAWKING's own error text names
    ...
    def _compact_receipts(self) -> list[Dict[str, Any]]:
        directory = Path(self.workspace_root) / ".hawking" / "receipts"
    ...
    def start_ultragoal(self, goal_text: str) -> dict:
        ...
        goal_md = Path(self.workspace_root) / ".hawking" / "GOAL.md"
