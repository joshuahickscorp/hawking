...
HAWKING records failed results into workspace knowledge and injects them as
...
from hawking.knowledge import KnowledgeStore, sanitise_knowledge

HAWKING_THINKING_ERROR = (
...
def test_snapshot_neutralises_the_hawking_thinking_error(tmp_path):
    store = KnowledgeStore(tmp_path)
    recorded = store.record_result(
        "do the 76-char task",
        {"status": "failed", "reason": HAWKING_THINKING_ERROR},
    )
...
def test_snapshot_scrubs_poison_already_on_disk_without_rewriting_it(tmp_path):
    ...
    assert disk_reason == HAWKING_THINKING_ERROR
    assert "<think>" in disk_reason


def test_sanitise_is_idempotent():
    once = sanitise_knowledge(HAWKING_THINKING_ERROR)
    twice = sanitise_knowledge(once)
    assert once == twice
    _assert_not_steering(once)

    record = {
        "data": {"reason": HAWKING_THINKING_ERROR, "nested": ["<THINK>", "</think>"]},
        "kind": "result_claim",
    }
