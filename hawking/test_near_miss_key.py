...
from hawking.backends import _edit_distance, _near_miss_key, validate_against_schema
...
from hawking.backends import (StructuredOutputContract, SchemaViolation,
...
def test_the_call_timeout_is_configurable_and_defaults_to_180():
    ...
    on one generate."""
    import os
    from hawking.delegate import _delegate_timeout
    old = os.environ.pop("HAWKING_DELEGATE_TIMEOUT_S", None)
    try:
        assert _delegate_timeout() == 180.0
        os.environ["HAWKING_DELEGATE_TIMEOUT_S"] = "600"
        assert _delegate_timeout() == 600.0
        os.environ["HAWKING_DELEGATE_TIMEOUT_S"] = "not a number"
        assert _delegate_timeout() == 180.0, "a bad value must fall back, not raise"
        os.environ["HAWKING_DELEGATE_TIMEOUT_S"] = "0"
        assert _delegate_timeout() >= 1.0, "a zero timeout would fail every call"
    finally:
        os.environ.pop("HAWKING_DELEGATE_TIMEOUT_S", None)
        if old is not None:
            os.environ["HAWKING_DELEGATE_TIMEOUT_S"] = old
...
def test_the_fixture_really_is_malformed_and_really_does_salvage():
    ...
    from hawking.backends import extract_json_object
