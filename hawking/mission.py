...
# Mission.phase; hawking.resident re-exports it.
...
def mission_dir(workspace: Union[str, Path]) -> Path:
    return Path(workspace) / ".hawking" / MISSION_DIRNAME
...
class Mission:
    ...
    def __init__(
        ...
        *,
        ...
        # Kept as an empty compatibility field for persisted callers. Hawking
        # never adopts an external provider task across a restart.
        ...
        if scheduler is not None:
            ...
            try:
                ...
                # superseded graph (renamed under .hawking/dag-retired/, never
    ...
    def from_workspace(cls, workspace: Any, **kwargs: Any) -> "Mission":
        ...
        # Recovery is Hawking-owned and in-process. Anything still ``running``
        # after the DAG load belonged to a worker that died with its parent;
        # it is failed here so the normal bounded repair/requeue policy owns the
        # next attempt. No external provider process is adopted.
    ...
    # mission never reached its first model call: `hawking resident start` sat at
    ...
    def checkpoint(self) -> Path:
        ...
        and therefore why a legacy external task got relaunched instead of
        being owned by the bounded Hawking WorkUnit path.
    ...
    def _finish(self) -> None:
        if self._evacuating.is_set() and not self._cancel.is_set():
            ...
            self._join_inflight()
            # ponytail: kills every child pid, same as cancel(). All cognition
            # workers are owned by this mission or its Hawking runtime pool.
            self._stop_children()
            self.checkpoint()
    ...
    def _run_unit(self, wu: WorkUnit) -> Dict[str, Any]:
        ...
        # Every cognition unit uses the one Hawking WorkUnit choke point.
        ...
        if backend != "hawking" or backend in self.providers:
    ...
    def _unit_context(self, wu: WorkUnit) -> Dict[str, Any]:
        ...
        packet_cap = 4000
        configured_packet_cap = os.environ.get("HAWKING_WORKER_PACKET_CHAR_CAP")
        ...
        return {
            ...
            # The provider may propose mutations, but it may not claim the
            # existing single-writer lock. This flag is an observation of the
            # scheduler's durable lock record for this exact WorkUnit.
            "mutation_lock_held": self._mutation_lock_held(wu),
        }

    def _mutation_lock_held(self, wu: WorkUnit) -> bool:
        if normalize_resource_class(getattr(wu, "resource_class", "")) != "MUTATION":
            return False
        lock = getattr(self.scheduler, "mutation_lock", None)
        if lock is None:
            return False
        try:
            record = lock.read()
            ...
            return False
        return isinstance(record, Mapping) and str(record.get("unit_id") or "") == str(wu.id)
    ...
    def _record_evidence(
        ...
        """Persist evidence needed for evidence-derived refill.

        Child count is intentionally unbounded here; downstream admission is
        still governed by the scheduler, dependencies, writer scopes, and
        real resource/budget guards. Per-item descriptions remain bounded.
        """
