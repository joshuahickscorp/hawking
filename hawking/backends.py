...
def llama_server_binary() -> str:
    explicit = os.environ.get("HAWKING_LLAMA_SERVER")
    if explicit:
        return explicit
...
def http200_semantic_gate_v5(response_status: int, body: Any) -> Tuple[bool, str]:
    """P18_HTTP200_SEMANTIC_GATE_V5: a 200 is not a success by itself.

    A model server can answer HTTP 200 with an error envelope, an empty
    completion, or a body that is not the JSON object the caller asked for.
    Treating the status code as the verdict lets that pass silently. This gate
    inspects the decoded body and returns (ok, reason); ok is True only when
    the status is 200 AND the body carries a non-empty completion payload.
    """
    if response_status != 200:
        return False, f"http_status_{response_status}"
    if body is None:
    ...
    if isinstance(body, Mapping):
        ...
        for key in ("choices", "content", "text", "response"):
            if key in body:
                value = body[key]
                if value is None:
                ...
                if isinstance(value, str) and not value.strip():
                    return False, "empty_completion"
                if isinstance(value, (list, tuple, Mapping)) and not value:
                    return False, "empty_completion"
                return True, "ok"
        return False, "missing_completion"
    return False, "unexpected_body_type"
...
#: helps a process that asks twice. Every HAWKING invocation and every test shard
...
_PROBE_CACHE_DIR = Path(os.environ.get("HAWKING_PROBE_CACHE", "")) if os.environ.get(
    "HAWKING_PROBE_CACHE"
) else Path.home() / ".cache" / "hawking" / "probes"
...
def llama_version_text(binary: Optional[str] = None) -> str:
    ...
    if cached is not None:
        return cached
    on_disk = _probe_cached(path, "llama-version")
    if on_disk is not None:
        _VERSION[path] = on_disk
        return on_disk
    text = _capture(path, ["--version"])
    _VERSION[path] = text
    _probe_store(path, "llama-version", text)
    return text


def mlx_server_binary() -> str:
    explicit = os.environ.get("HAWKING_MLX_SERVER")
    if explicit:
        return explicit
...
def mlx_vlm_server_binary() -> str:
    ...
    explicit = os.environ.get("HAWKING_MLX_VLM_SERVER")
    if explicit:
        return explicit
...
def _capability_response_has_semantics(payload: Any) -> bool:
    """Return whether a successful capability probe actually generated.

    A reachable OpenAI-compatible server can still return an empty JSON
    envelope with HTTP 200 (the historical orphaned-server failure). Status
    is transport evidence only; readiness needs one non-empty assistant/text
    value or a real tool/function call in the response body.
    """
    ...
    def nonempty(value: Any) -> bool:
        ...
        if isinstance(value, list):
            return any(
                isinstance(item, Mapping) and nonempty(item.get("text"))
                for item in value
            )
        return False

    def callable_tool(value: Any) -> bool:
    ...
    if isinstance(choices, list) and choices and isinstance(choices[0], Mapping):
        choice = choices[0]
        if nonempty(choice.get("text")):
            return True
        message = choice.get("message")
        if isinstance(message, Mapping):
            if nonempty(message.get("content")) or nonempty(message.get("refusal")):
                return True
            calls = message.get("tool_calls")
            if isinstance(calls, list) and any(callable_tool(call) for call in calls):
                return True
            if callable_tool(message.get("function_call")):
                return True

    return any(nonempty(payload.get(key)) for key in ("text", "generated_text", "content"))
...
def structured_output_attempts() -> int:
    raw = os.environ.get("HAWKING_STRUCTURED_OUTPUT_ATTEMPTS", "")
    if str(raw).strip() == "":
...
def repair_tool_argument_values(instance: Any, log: List[str], path: str = "$") -> Any:
    ...
    HAWKING's result contract deliberately represents tool arguments as flat
...
def validate_against_schema(
    ...
    Covers the JSON Schema subset HAWKING actually ships: type, properties,
...
class _HttpChildLifecycle:
    """Lifecycle authority shared by local HTTP child backends.

    Providers keep ownership of command construction, payload handling, and
    concurrency semantics.  This mixin owns the common process state contract:
    endpoint availability, health polling, log inspection, and idempotent
    teardown of attached or recorded detached children.
    """
    ...
    def _terminate_recorded_child(
        self, pid: int, report: Dict[str, Any]
    ) -> None:
        identity_status = process_identity_status(pid, self.start_time)
        if identity_status != "matches":
            report["gone"] = identity_status == "pid_reused_start_time_mismatch"
            report["skipped"] = [
    ...
    def stop(self) -> Dict[str, Any]:
        ...
        if self._stopped and (self.process is None or self.process.poll() is not None):
            if self.pid and pid_is_alive(self.pid):
                self._terminate_recorded_child(self.pid, report)
                if report.get("gone"):
                    self.pid = None
                    self.process = None
            return report
        ...
        if pid and pid_is_alive(pid):
            self._terminate_recorded_child(int(pid), report)
        self.process = None
        if report.get("gone"):
            self.pid = None
        self._stopped = True
...
class LlamaServerBackend(_HttpChildLifecycle, RuntimeBackend):
    def __init__(
        self,
        model_path: str,
        ...
        env_layers = os.environ.get("HAWKING_N_GPU_LAYERS")
    ...
    def identity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            binary = self._binary or os.environ.get("HAWKING_LLAMA_SERVER") or ""
        try:
            model_bytes = (
    ...
    def command(self, port: Optional[int] = None, n_slots: Optional[int] = None) -> List[str]:
        ...
        if (os.environ.get("HAWKING_LLAMA_DEVICE") or "").strip().lower() == "none":
            cmd.append("--no-warmup")
        device = (os.environ.get("HAWKING_LLAMA_DEVICE") or "").strip()
        if device:
            cmd.extend(["--device", device])
        fit = (os.environ.get("HAWKING_LLAMA_FIT") or "").strip()
        if fit:
            cmd.extend(["--fit", fit])
    ...
    def spawn(self, **kwargs: Any) -> None:
        ...
        handle = tempfile.NamedTemporaryFile(
            prefix="hawking-llama-", suffix=".log", delete=False
        )
    ...
    def complete(
        ...
        limit = float(
            ...
            else os.environ.get("HAWKING_MODEL_TIMEOUT", "1800")
        )
        data = _post_json(url, prepared, limit, "llama-server")
        return self._parse_response(data, degraded)

class MlxServerBackend(_HttpChildLifecycle, RuntimeBackend):
    ...
    def __init__(
        ...
        env_tokens = os.environ.get("HAWKING_MODEL_TOKENS")
        if max_tokens is not None:
            ...
            self.max_tokens = 8192
        env_cache = os.environ.get("HAWKING_MLX_PROMPT_CACHE_SIZE")
    ...
    def identity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            binary = self._binary or os.environ.get("HAWKING_MLX_SERVER") or ""
    ...
    def spawn(self, **kwargs: Any) -> None:
        ...
        handle = tempfile.NamedTemporaryFile(
            prefix="hawking-mlx-", suffix=".log", delete=False
        )
        self._log_path = handle.name
        self._log_handle = handle
        self.process = subprocess.Popen(
...
class MlxVlmServerBackend(MlxServerBackend):
    ...
    the HAWKING resident can keep one backend contract while choosing the loader
...
class OpenAICompatibleBackend(RuntimeBackend):
    ...
    ``HAWKING_REMOTE_SUPPORTS_*``.  Unknown remote behaviour therefore follows
    ...
    def __init__(
        ...
        **_ignored: Any,
        ...
        self.model_name = (
            os.environ.get("HAWKING_REMOTE_MODEL")
            or query_model
            or fragment_model
            or "remote"
        )
        ...
        self._capability_failure_reason: Optional[str] = None
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {"Content-Type": "application/json", "User-Agent": "hawking/0.1"}
        token = (
            os.environ.get("HAWKING_REMOTE_API_KEY")
    ...
    def supports(self, feature: str) -> bool:
        canon = canonical_feature(feature)
        names = {
            "response_format": "HAWKING_REMOTE_SUPPORTS_RESPONSE_FORMAT",
            "grammar": "HAWKING_REMOTE_SUPPORTS_GRAMMAR",
            "chat_template_kwargs": "HAWKING_REMOTE_SUPPORTS_CHAT_TEMPLATE_KWARGS",
            "prefix_cache": "HAWKING_REMOTE_SUPPORTS_PREFIX_CACHE",
            "slots": "HAWKING_REMOTE_SUPPORTS_SLOTS",
            "vision": "HAWKING_REMOTE_SUPPORTS_VISION",
            "tool_calling": "HAWKING_REMOTE_SUPPORTS_TOOL_CALLING",
            "streaming": "HAWKING_REMOTE_SUPPORTS_STREAMING",
        }
    ...
    def ready(self, timeout: float) -> bool:
        if not self._started:
            return False
        if os.environ.get("HAWKING_REMOTE_SKIP_HEALTH", "").strip() == "1":
            return True
    ...
    def _prove_capability(self, timeout: float) -> bool:
        ...
        The probe must parse the successful body. A transport HTTP 200 with no
        choices, no text, and no tool/function call is not generation evidence.
        """
        if os.environ.get("HAWKING_REMOTE_SKIP_CAPABILITY_PROBE", "").strip() == "1":
        ...
        try:
            with urllib.request.urlopen(req, timeout=max(5.0, float(timeout))) as resp:
                if not 200 <= resp.status < 300:
                    ok = False
                    self._capability_failure_reason = f"http_status_{resp.status}"
                else:
                    try:
                        payload = json.loads(resp.read().decode("utf-8", errors="replace"))
                    except (UnicodeError, json.JSONDecodeError):
                        payload = None
                    ok = _capability_response_has_semantics(payload)
                    self._capability_failure_reason = (
                        None if ok else "http_success_without_generation_semantics"
                    )
        except urllib.error.HTTPError as exc:
            # The server answered with an opinion, so it is alive and serving.
            ok = exc.code in {400, 401, 403, 422}
            self._capability_failure_reason = (
                None if ok else f"http_status_{exc.code}"
            )
        except Exception:
            ok = False
            self._capability_failure_reason = "capability_probe_transport_failure"
    ...
    def complete(
        ...
        prepared, degraded = self._prepare_payload(payload)
        # HAWKING's common payload carries the model selector in ``model``. For
        ...
        limit = float(timeout if timeout is not None else os.environ.get("HAWKING_MODEL_TIMEOUT", "1800"))
        try:
        ...
        if not _capability_response_has_semantics(data):
            raise RuntimeError(
                "remote provider HTTP 200 without generation semantics"
            )
        return self._parse_response(data, degraded)
...
class NoeticNativeBackend(RuntimeBackend):
    ...
    def __init__(
        ...
        **_ignored: Any,
        ...
        self._configured = bool(
            not model_path
            or os.environ.get("HAWKING_NATIVE_CONFIG")
            or candidate_exists
        )
        try:
    ...
    def spawn(self, **kwargs: Any) -> None:
        ...
        self.connector.start(
            timeout=float(os.environ.get("HAWKING_READY_TIMEOUT", "300"))
        )
...
# already present in HAWKING receipts and capability snapshots.  These aliases
# expose the actual abstraction to new callers: the provider/profile may be
# Hawking's current resident today, while the HAWKING runtime contract remains
...
def _http200_semantic_gate(response):
    """Return True only when an HTTP 200 response carries a non-empty semantic body."""
    ...
    return bool(body)
    """Return True only when an HTTP 200 response carries a semantically valid body."""
    if response is None:
        return False
