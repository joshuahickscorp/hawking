"""`hawking census` — HAWKING surveys ModelLake specimens as bounded WorkUnits.
...
DISPOSITIONS = {
    ...
    "HAWKING RESIDENT CANDIDATE", "PULSAR CANDIDATE", "MAGNETAR CANDIDATE",
...
def pending_specimens(repo: Path) -> List[str]:
    ...
    if not census.exists():
        return out
    done = {p.stem.replace("HAWKING_CENSUS_", "")
            for p in (repo / "receipts/hawking-census").glob("HAWKING_CENSUS_*.json")}
...
def write_receipt(repo: Path, u: Unit) -> Path:
    d = repo / "receipts/hawking-census"
    d.mkdir(parents=True, exist_ok=True)
    p = d / f"HAWKING_CENSUS_{u.specimen.split('@')[0]}.json"
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.pending:
        ...
        print(f"{len(rest)} specimens with no HAWKING census verdict:")
        for n in rest:
            print(f"  {n}")
        return 0
    if not todo:
