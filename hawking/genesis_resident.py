...
Production body is the Hawking Rust binary (one process, one weight set,
...
live socket answering), never a status file.
    ...
    python3 hawking/genesis_resident.py serve
    python3 hawking/genesis_resident.py serve --stub
    python3 hawking/genesis_resident.py health
    python3 hawking/genesis_resident.py propose --prompt "..."
"""
...
from hawking.genesis_contract import (  # noqa: E402
...
def discover_body_bin(repo: Path | None = None) -> Path | None:
    # The Hawking-scoped setting is canonical. The short legacy setting is
    # input-only so a sealed/current lineage can still be inspected without
    # silently substituting a body.
    ...
    candidates = [
        ...
        root / "target" / "release" / "genesis-resident",
...
def _exec_body(ns: argparse.Namespace) -> int:
    ...
    if binary is None:
        ...
        print(
            "build: CARGO_TARGET_DIR=/tmp/hawking-genesis-target "
            "cargo build --release -p hawking --bin genesis-resident",
            file=sys.stderr,
        )
        return 2
