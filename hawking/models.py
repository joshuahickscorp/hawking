...
def discover_models(roots: Optional[List[str]] = None) -> List[ModelInfo]:
    ...
    for root in roots:
        ...
        if is_hawking_native_path(root):
            ...
            continue
        if _is_mlx_dir(root):
            # MLX directories are historical source specimens, not Hawking
            # execution choices.  Keep them out of the selectable inventory.
            continue
        for dirpath, dirnames, filenames in os.walk(root):
            ...
            if _is_mlx_dir(dirpath):
                # Do not descend into a retired vendor artifact or expose it
                # as a model the native runtime cannot load.
                dirnames[:] = []
                continue
            for fn in filenames:
                low = fn.lower()
                ...
                if low.endswith((".nx", ".noetic", ".hawking", ".hawking.json")) or (
                    ...
                    if info is not None:
                        ...
                        models.append(info)
                    continue
                # GGUF is a retired llama.cpp artifact.  It remains readable
                # in historical receipts but is never a Hawking model choice.
    return models
...
class ModelRegistry:
    ...
    ambiguous; HAWKING does not silently choose one of several installed models.
    ...
    MLX weight directories and GGUF files are historical source specimens,
    not selectable Hawking runtime bodies.  The native profile/catalog is the
    only local execution identity.
    """

    def __init__(self, roots: Optional[List[str]] = None) -> None:
        # The interactive HAWKING selector is an execution surface, not a raw
        ...
        # Gravity catalog in hawking.catalog.
    ...
    def select(self, selector: str) -> Optional[ModelInfo]:
        ...
        # Explicit paths are still required to resolve through the native
        # profile/catalog contract; retired vendor artifacts do not get an
        # implicit loader here.
