"""Live Hawking autonomy qualification and unattended-work runner.

This module is an evidence harness around the existing HAWKING authorities.  It
does not add a scheduler: Mission remains the DAG/receipt authority, Hawking
...
* A4 kills the HAWKING control process after a completed dependency and resumes
...
from hawking.persist import (
    ...
    json_compatible_copy as _json_safe,
)
from hawking.workunit import WorkUnit, identify_ready


SCHEMA = "hawking.autonomy_gate.v1"
WINDOW_SCHEMA = "hawking.unattended_window.v1"
...
def _profile_path(repo: Path, value: Optional[str | os.PathLike[str]]) -> Path:
    if value:
        return Path(value).expanduser().resolve()
    configured = os.environ.get("HAWKING_NATIVE_CONFIG")
    if configured:
        return Path(configured).expanduser().resolve()
    return (repo / "hawking" / DEFAULT_PROFILE_NAME).resolve()
...
def _generation_env(*, tokens: int = 64, attempts: int = 1) -> Iterable[None]:
    names = (
        ...
        "HAWKING_STRUCTURED_OUTPUT_ATTEMPTS",
        "HAWKING_MODEL_TIMEOUT",
    )
    old = {name: os.environ.get(name) for name in names}
    os.environ["HAWKING_MODEL_TOKENS"] = str(max(1, int(tokens)))
    os.environ["HAWKING_STRUCTURED_OUTPUT_ATTEMPTS"] = str(max(1, int(attempts)))
    os.environ.setdefault("HAWKING_MODEL_TIMEOUT", "180")
    try:
        yield
    finally:
    ...
    rows = [
        {
            ...
            "path": "receipts/headless/HAWKING_CHECKPOINT.json",
            "tool": "receipt.read",
            "arguments": {"path": "receipts/headless/HAWKING_CHECKPOINT.json"},
        },
        {
            "label": "native ladder",
            "path": "receipts/headless/HAWKING_NATIVE_GATE.json",
            "tool": "receipt.read",
            "arguments": {"path": "receipts/headless/HAWKING_NATIVE_GATE.json"},
        },
        {
            "label": "resident proof",
            "path": "receipts/headless/HAWKING_RESIDENT_GATE.json",
            "tool": "receipt.read",
            "arguments": {"path": "receipts/headless/HAWKING_RESIDENT_GATE.json"},
        },
        {
            "label": "native mission",
            "path": "receipts/headless/HAWKING_NATIVE_MISSION_GATE.json",
            "tool": "receipt.read",
            "arguments": {"path": "receipts/headless/HAWKING_NATIVE_MISSION_GATE.json"},
        },
        {
            "label": "recovery proof",
            "path": "receipts/headless/HAWKING_RECOVERY_GATE.json",
            "tool": "receipt.read",
            "arguments": {"path": "receipts/headless/HAWKING_RECOVERY_GATE.json"},
        },
        {
            "label": "research proof",
            "path": "receipts/headless/HAWKING_RESEARCH_GATE.json",
            "tool": "receipt.read",
            "arguments": {"path": "receipts/headless/HAWKING_RESEARCH_GATE.json"},
        },
        {
            "label": "PERCEPTION proof",
            "path": "receipts/headless/HAWKING_PERCEPTION_GATE.json",
            "tool": "receipt.read",
            "arguments": {"path": "receipts/headless/HAWKING_PERCEPTION_GATE.json"},
        },
        {
            "label": "Flash identity",
            "path": "receipts/headless/HAWKING_FLASH_NEXT_IDENTITY.json",
            "tool": "receipt.read",
            "arguments": {"path": "receipts/headless/HAWKING_FLASH_NEXT_IDENTITY.json"},
        },
        {
            "label": "accelerator smoke",
            "path": "receipts/headless/HAWKING_ACCELERATOR_NATIVE_SMOKE.json",
            "tool": "receipt.read",
            "arguments": {"path": "receipts/headless/HAWKING_ACCELERATOR_NATIVE_SMOKE.json"},
        },
        {
        ...
        },
        {
            ...
            "path": "hawking/hawking-native.sealed-3.14.json",
            "tool": "architecture.inspect",
            "arguments": {"path": "hawking/hawking-native.sealed-3.14.json", "max_tensors": 64},
        },
    ]
...
def _build_census_units(repo: Path, workspace: Path, count: int, prefix: str) -> tuple[Dict[str, WorkUnit], List[Dict[str, Any]]]:
    ...
    if not catalog:
        raise RuntimeError("no canonical HAWKING evidence targets are available")
    ...
    for index in range(max(1, int(count))):
        ...
        source["derived_content"] = json.dumps(
            {
                "schema": "hawking.evidence_observation.v1",
                "unit_id": unit_id,
                "source": source["path"],
...
def run_census_mission(
    *,
    ...
    workspace = Path(tempfile.mkdtemp(prefix=f"hawking-{prefix}-"))
    ...
    controller = None
    agent = None
    old_timeout = os.environ.get("HAWKING_MODEL_TIMEOUT")
    try:
        controller = Controller(workspace, model=str(profile))
        agent = Hawking(workspace, engine=controller.engine, controller=controller, repo_root=repo_root)
        ...
        if old_timeout is not None:
            os.environ["HAWKING_MODEL_TIMEOUT"] = old_timeout
        else:
            os.environ.pop("HAWKING_MODEL_TIMEOUT", None)
...
class _CrashOnceNativeProvider:
    ...
    def __init__(self, profile: Path) -> None:
        from hawking.hawking_native import HawkingNativeConfig, HawkingNativeConnector
    ...
    def start(self) -> None:
        self.connector.start(timeout=float(os.environ.get("HAWKING_READY_TIMEOUT", "300")))
    ...
    def generate(self, request: Any, *, timeout: Optional[float] = None) -> Any:
        from hawking.providers import GenerationResponse
...
def run_resident_kill(
    *, repo_root: Path, profile: Path
    ...
    workspace = Path(tempfile.mkdtemp(prefix="hawking-resident-kill-"))
    ...
    agent = Hawking(workspace, engine=object(), repo_root=repo_root, providers={"crash-resident": provider})
    unit = WorkUnit(
...
def _process_host(workspace: Path, repo: Path) -> int:
    ...
    first_path = (repo / "receipts/headless/HAWKING_CHECKPOINT.json").resolve()
    second_path = (repo / "receipts/headless/HAWKING_FLASH_NEXT_IDENTITY.json").resolve()
    first = WorkUnit(
    ...
    agent = Hawking(workspace, engine=object(), repo_root=repo)
    mission = agent.start_mission("Continue a real evidence DAG after HAWKING process death.", units={first.id: first, second.id: second})
...
def run_process_kill(*, repo_root: Path) -> Dict[str, Any]:
    ...
    workspace = Path(tempfile.mkdtemp(prefix="hawking-process-kill-"))
    host = subprocess.Popen(
        [sys.executable, "-m", "hawking.autonomy_gate", "--process-host", str(workspace), "--repo-root", str(repo_root)],
        cwd=str(workspace),
    ...
    try:
        state = _read_json(workspace / ".hawking/mission/state.json") or {}
        before = {
            "phase": state.get("phase"),
            "units": state.get("units"),
            "checkpoint_id": state.get("checkpoint_id"),
        }
        restarted = Hawking(workspace, engine=object(), repo_root=repo_root)
...
def run_idempotency_crash(*, repo_root: Path) -> Dict[str, Any]:
    ...
    workspace = Path(tempfile.mkdtemp(prefix="hawking-idempotency-"))
    content = f"hawking-idempotency-proof:{time.time_ns()}\n"
    host = subprocess.Popen(
        [sys.executable, "-m", "hawking.autonomy_gate", "--mutation-host", str(workspace), "--repo-root", str(repo_root), "--mutation-content", content],
        cwd=str(workspace),
        env=_module_environment(),
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    ...
    try:
        ...
        agent = Hawking(workspace, engine=object(), repo_root=repo_root)
...
def record_autonomy_causality(report: Dict[str, Any], **kwargs: Any) -> dict[str, Any]:
    ...
    return sc.stamp_gate(
        report,
        source="hawking/autonomy_gate.py::run_autonomy_gate",
        **payload,
    )
    ...
    *,
    ...
    try:
        ...
        if selected in {"all", "a4", "process-kill"}:
            report["stages"]["A4_hawking_process_kill"] = run_process_kill(repo_root=repo)
        ...
        report["checks"] = {
            ...
            "a4_process_recovery": report["stages"].get("A4_hawking_process_kill", {}).get("status") == "PASSED" if "A4_hawking_process_kill" in report["stages"] else True,
    ...
    destination = Path(emit).expanduser().resolve() if emit else repo / "receipts" / "headless" / "HAWKING_AUTONOMY_GATE.json"
...
def run_unattended_window(
    *,
    ...
    repo = _repo_root(repo_root)
    native_profile = _profile_path(repo, profile)
    root = Path(workspace).expanduser().resolve() if workspace else Path(tempfile.mkdtemp(prefix="hawking-unattended-window-"))
    ...
    try:
        with _generation_env(tokens=64, attempts=1):
            ...
            while time.time() < started + duration:
                ...
                cycle_index += 1
                agent: Optional[Hawking] = None
                cycle: Dict[str, Any] = {"index": cycle_index, "started_at": cycle_started, "status": "RUNNING"}
                try:
                    agent = Hawking(root, engine=controller.engine, controller=controller, repo_root=repo)
    ...
    destination = Path(emit).expanduser().resolve() if emit else repo / "receipts" / "headless" / "HAWKING_UNATTENDED_WINDOW.json"
    atomic_write_json(destination, report)
    report["receipt_path"] = str(destination)
    return report
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--mutation-content", default="hawking-idempotency-proof\n")
