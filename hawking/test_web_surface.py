...
from hawking.web import enforce_gravity_only_webui, next_webui_port


def test_provider_catalog_is_bounded_unless_explicitly_searched(monkeypatch):
    from hawking.serve import _select_openrouter_catalog_rows

    monkeypatch.delenv("HAWKING_OPENROUTER_VISIBLE_MODELS", raising=False)
    configured = [{"id": "z-ai/glm-5.3"}]
    catalog = [
    ...
    assert _select_openrouter_catalog_rows(configured, catalog) == []
    assert [
        row["id"]
        for row in _select_openrouter_catalog_rows(
            configured, catalog, search="glm"
        )
    ] == ["z-ai/glm-5.3"]
...
def test_build_refuses_legacy_kimi_before_creating_a_daemon_launcher(
        monkeypatch, capsys):
    """The human ``hawking build`` front door cannot revive a legacy runtime."""
    import hawking.catalog as catalog
    import hawking.web as web

    kimi = catalog.Body(
        name="KIMI_P0_OPERATIONAL",
        path="/tmp/kimi",
        kind="mlx",
        revision="a" * 64,
        source="fixture",
    ...
    monkeypatch.setattr(
        catalog,
        "catalog",
        lambda extra_roots=None, *, research=False: [kimi],
    )
    monkeypatch.setattr(web, "surface_health", lambda *_args, **_kwargs: None)

    def must_not_start(*_args, **_kwargs):
        raise AssertionError("Kimi write gate created a daemon launcher")

    monkeypatch.setattr(web, "start_surface", must_not_start)

    assert web.main([
        "KIMI_P0_OPERATIONAL", "--write", "--no-webui",
    ]) == 2
    assert "no admitted Hawking artifact has that immutable identity" in (
...
def test_stop_uses_live_health_and_lease_when_process_title_is_unavailable(
        monkeypatch, capsys):
    import hawking.hawkingd as hawkingd
    import hawking.web as web
...
def test_surface_health_rejects_starting_resident(monkeypatch):
    import hawking.web as web

    monkeypatch.setattr(
        web, "_get_json",
        lambda *_args, **_kwargs: {"resident": "loaded", "state": "starting"},
    )
    assert web.surface_health("127.0.0.1", 8011) is None


def test_surface_health_rejects_non_string_resident(monkeypatch):
    import hawking.web as web

    monkeypatch.setattr(
        web, "_get_json",
        lambda *_args, **_kwargs: {"resident": 123, "state": "ready"},
    )
    assert web.surface_health("127.0.0.1", 8011) is None


def test_surface_health_accepts_ready_resident(monkeypatch):
    import hawking.web as web

    monkeypatch.setattr(
        web, "_get_json",
        lambda *_args, **_kwargs: {"resident": "loaded", "state": "ready"},
    )
    assert web.surface_health("127.0.0.1", 8011)["resident"] == "loaded"
