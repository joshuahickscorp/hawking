...
HAWKING must perceive "I can inspect source", not "I have ripgrep" -- and when it
...
def _profiler_reachable() -> bool:
    try:
        import importlib
        importlib.import_module("hawking.prefill_profile")
        return True
    except Exception:
        return False
...
def capabilities() -> List[Capability]:
    ...
    return [
        ...
        Capability(
            ...
            tools=[],
            ladder=[
                Rung("report", "hawking report", "moderate",
...
def capability_map(workspace: Optional[str | Path] = None) -> Dict[str, Any]:
    """The whole surface, including fresh machine authority observations.

    Domain capabilities answer what Hawking can do.  ``permissions`` answers
    what the current machine and stable native owner can do right now.  They
    are intentionally separate: a daemon with FDA is not thereby authorized
    to hand an entire disk to a Goal.
    """
    owned = capabilities()
    result: Dict[str, Any] = {
        "schema": "hawking.capabilities.v1",
    ...
    try:
        ...
        machine_root = Path(workspace).expanduser().resolve() if workspace else Path.cwd()
        machine = PermissionsService(machine_root).status(refresh=True)
        result["permissions"] = machine
        result["machine_ready"] = all(
            isinstance(row, dict) and row.get("state") == "READY"
            for name, row in (machine.get("capabilities") or {}).items()
            if name in ("WORKSPACE_READ", "WORKSPACE_WRITE", "NETWORK", "PROCESS_EXECUTION")
        )
    except Exception as exc:
        result["permissions"] = {
            "schema": "hawking.permissions.v1",
            "state": "UNAVAILABLE",
            "reason": f"permission registry unavailable: {type(exc).__name__}",
        }
        result["machine_ready"] = False
    return result
...
def capture(workspace: str, *, note: str = "",
    ...
    events = root / ".hawking" / "mission" / "events.jsonl"
    if events.is_file():
        try:
    ...
    out = root / ".hawking" / "forensics" / f"snapshot-{int(snap.at)}-{reason}.json"
    try:
...
def identify(path: str) -> Dict[str, Any]:
    ...
    try:
        from .perception.file_eye import classify_bytes  # type: ignore
