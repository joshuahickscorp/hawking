...
COMMANDS: Tuple[Command, ...] = (
    ...
    Command("/tools", "list typed Hawking tools", False, "read_only", "/tools"),
    Command(
        "/provider",
    ...
    Command(
        "/cognition",
        "show the Hawking-native cognition contract and provider census",
        False,
        "read_only",
        "/cognition",
    ),
...
LAND_DEFAULT_TEST_COMMAND: Tuple[str, ...] = (
    "python3", "-m", "pytest", "hawking/", "-q", "-p", "no:cacheprovider",
)
...
def format_status(snapshot: Dict[str, Any]) -> str:
    ...
    if not isinstance(runtime, dict):
        runtime = None
    hawking = snap.get("hawking")
    if not isinstance(hawking, dict):
        hawking = runtime
    provider_status = hawking
    provider_label = "Hawking"
    ...
    if not provider_status:
        hawking_line = f"{provider_label} unknown"
    else:
        ...
        hawking_line = head + _truncate(tail, budget)

    if not cognition:
        cognition_line = "Cognition unknown"
    else:
        providers = cognition.get("providers") or {}
        cognition_line = (
            f"Cognition providers={','.join(sorted(str(key) for key in providers)) or 'none'} "
            f"external={str(cognition.get('external_worker', False)).lower()}"
        )
...
RESIDENT_STATE_REL = (".hawking", "resident", "state.json")
...
def _status_roots(
    ...
    this also tries `resolve_workspace()` -- the existing HAWKING_WORKSPACE /
    `.hawking` ancestor walk-up, already built for exactly this and
...
# stray `.hawking` left in the system tmp dir did too. On any developer box that
...
# snapshot has no receipts, and a stray `.hawking` has no receipts beside it.
_DECISIVE_MARKERS = (Path("civilization") / "ROADMAP_STATE.json",)
_CORROBORATING_MARKERS = (Path("receipts"), Path(".hawking"))
...
def _load_mission_state(controller: Any) -> Optional[Dict[str, Any]]:
    ...
    if root is None:
        return None
    path = root / ".hawking" / "mission" / "state.json"
    if not path.is_file():
        return None
    try:
...
def _find_ledger(controller: Any) -> Any:
    ...
    if root is None:
        return None
    for candidate in (root / ".hawking" / "GOAL.md", root / "GOAL.md"):
...
def enrich_status_snapshot(controller: Any, snap: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if last_checkpoint and last_checkpoint > 0:
        ...
        root = _workspace_root(controller)
        path = None if root is None else root / ".hawking" / "mission" / "state.json"
...
def _status_has_observed_fields(snap: Dict[str, Any]) -> bool:
    ...
    if snap.get("watchdog") not in (None, "", "unknown"):
        return True
    if snap.get("runtime") or snap.get("hawking") or snap.get("cognition"):
        return True
    return False
...
# advertised by help and missing here. hawking/command_registry.py is the source.
...
class CommandHandler:
    """Canonical HAWKING slash-command dispatcher.
    ...
    def _cmd_cognition(self, arg: str) -> str:
        if (arg or "").strip():
            return "Usage: /cognition"
        status_fn = getattr(self.controller, "status", None)
        snapshot = status_fn() if callable(status_fn) else {}
        if not isinstance(snapshot, dict):
            snapshot = {}
        cognition = snapshot.get("cognition") or {}
        self.last_value = cognition
        providers = cognition.get("providers") or {}
        names = ", ".join(sorted(str(name) for name in providers)) or "none"
        return (
            "Hawking cognition is native and WorkUnit-bounded. "
            f"providers={names} external_worker={bool(cognition.get('external_worker', False))}"
        )

    def _cmd_grok(self, arg: str) -> str:
        del arg
        text = (
            "The /grok adapter is retired. Hawking owns cognition now; "
            "use /mission, /cognition, or an explicit WorkUnit provider selector."
        )
        self.last_value = text
        return text
    ...
    def _cmd_context(self, arg: str) -> str:
        ...
        resolve to a direct child of ``<root>/.hawking/pastes``, so there is no
    ...
    def _cmd_receipts(self, arg: str) -> str:
        """Durable run receipts, newest first. hawking.engine writes one per goal."""
        limit = 10
        if arg.strip():
            try:
        ...
        directory = None if root is None else root / ".hawking" / "receipts"
    ...
    def _cmd_land(self, arg: str) -> str:
        ...
        dirty through `hawking.landing`, which re-verifies and re-runs the test
    ...
    def _land_push(self) -> str:
        """The explicit, operator-typed push. `hawking.landing` never pushes on
    ...
    def _verifier_run(repo_root: Path, *args: str, timeout: float = 30.0):
        ...
        the same subprocess wrapper `hawking.landing.IntegrationVerifier` uses
