"""Bounded public-research operational gate for Hawking.
...
from hawking.persist import (
    atomic_write_json,
    json_compatible_copy as _safe_json,
)
...
SCHEMA = "hawking.research_gate.v1"
...
def record_research_causality(report: Dict[str, Any], **kwargs: Any) -> dict[str, Any]:
    ...
    return sc.stamp_gate(
        report,
        source="hawking/research_gate.py::run_research_gate",
        **payload,
    )
...
def run_research_gate(
    ...
    *,
    ...
    root = Path(workspace or tempfile.mkdtemp(prefix="hawking-research-gate-")).expanduser().resolve()
    ...
    with tempfile.TemporaryDirectory(prefix="hawking-hf-metadata-", dir=str(root)) as download_root:
        if revision:
            hf_fetch = call(
    ...
    report: Dict[str, Any] = {
        ...
        "next_action": "feed source-backed hypotheses to Doctor/PERCEPTION and validate them with protected local measurements",
    }
    ...
    receipt = root / ".hawking" / "receipts" / "research-gate.json"
