"""Read-only physical connectivity census for the HAWKING/AgentOS boundary."""
...
SCHEMA = "hawking.agentos.connectivity.v1"
...
def _public_probe(url: str) -> Dict[str, Any]:
    ...
    request = urllib.request.Request(
        url, headers={"User-Agent": "hawking-agentos-connectivity/1"}, method="GET"
    )
    try:
...
def _perception_surface(repo: Path) -> Dict[str, Any]:
    """Probe Hawking's native perception boundary, not a marker directory."""
    try:
        from .perception_adapter import inspect_perception

        report = inspect_perception(repo)
        return {
            "status": report.get("status", "PARTIAL"),
            "native_module": "hawking.perception",
            "external_runtime_required": bool(report.get("external_runtime_required")),
            "tool_count": len(report.get("tools") or []),
...
def probe_connectivity(
    ...
    *,
    ...
    surfaces: Dict[str, Any] = {
        ...
        "metal": metal,
        "perception": _perception_surface(repo),
        "modellake": {
