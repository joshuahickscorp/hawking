from hawking.tool_registry import ToolContext, ToolRegistry, ToolSpec, default_tool_registry
...
def test_hawking_agentos_focus_discovers_the_live_capability_plane(tmp_path):
    """A product-level capability question must not look like zero tools."""
    registry = default_tool_registry(tmp_path, repo_root=tmp_path)
    result = registry.describe("HAWKING/AgentOS", max_results=21)
    assert result["shown"] > 0
    names = set(result["names"])
...
def test_native_catalog_ranking_keeps_python_contracts_and_schema(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("hawking.repo_context.native_tool_catalog", native)
...
def test_native_dispatch_denial_is_final_before_python_handler(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "hawking.repo_context.native_tool_dispatch_admit",
...
def test_native_dispatch_snapshot_is_reused_until_registry_changes(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("hawking.repo_context.native_tool_dispatch_admit", native)
...
def test_native_filesystem_list_preserves_content_free_discovery_contract(monkeypatch, tmp_path):
    from hawking.tool_registry import ToolContext, _list_files
    ...
    monkeypatch.setattr("hawking.repo_context.native_filesystem_list", native)
...
def test_native_filesystem_search_preserves_scoped_match_contract(monkeypatch, tmp_path):
    from hawking.tool_registry import ToolContext, _search_files
    ...
    def native(root, path, *, needle, glob, max_results, max_per_file):
        ...
        return {
            ...
            "matches": [{"path": "hawking/main.py", "line": 9, "text": "needle"}],
            "files_seen": 1, "skipped_large": 0, "truncated": False,
            "elapsed_ns": 73,
        }

    monkeypatch.setattr("hawking.repo_context.native_filesystem_search", native)
    result = _search_files(context, {
        "root": ".", "pattern": "needle", "glob": "*.py",
        "max_results": 7, "max_per_file": 2,
        # Native Gravity owns the unpartitioned scope. Named source/test
        # corpora intentionally stay on the scoped Python path so their
        # exclusions remain explicit and auditable.
        "corpus": "all-authorized",
    })
    assert result["provenance"] == "hawking-gravityd.search-files.v1"
    assert result["elapsed_ns"] == 73
    assert result["matches"][0]["path"] == str(tmp_path / "hawking/main.py")
    assert seen["root"] == tmp_path.resolve()
    assert seen["path"] == tmp_path.resolve()
...
def test_native_file_body_transport_is_lab_opt_in_until_it_earns_promotion(monkeypatch, tmp_path):
    from hawking.tool_registry import ToolContext, _read_file
    ...
    monkeypatch.setenv("HAWKING_NATIVE_GRAVITY", "1")
    monkeypatch.delenv("HAWKING_NATIVE_GRAVITY_READ", raising=False)
    monkeypatch.setattr("hawking.repo_context.native_filesystem_read", native)
    context = ToolContext(tmp_path, tmp_path)
    ...
    monkeypatch.setenv("HAWKING_NATIVE_GRAVITY_READ", "1")
    result = _read_file(context, {"path": "sample.py"})
...
def test_repo_context_accepts_native_direct_path_rank_without_full_inventory(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "hawking.repo_context.native_rank_code_paths",
        lambda root, *, suffixes, words, max_results: {
            "paths": ["hawking/serve.py"],
            "files_seen": 10_604,
            "complete": True,
            "elapsed_ns": 91,
        },
    ...
    assert context.paths_for("inspect hawking/serve.py", limit=4) == ["hawking/serve.py"]


def test_native_code_path_validation_keeps_only_canonical_relative_paths():
    from hawking.repo_context import _validated_native_code_path

    assert _validated_native_code_path("hawking/serve.py") == "hawking/serve.py"
    assert _validated_native_code_path("src/.hidden.py") == "src/.hidden.py"
    for path in ("/tmp/serve.py", "../serve.py", "hawking/../serve.py",
                 "hawking/./serve.py", "hawking\\serve.py", "hawking/serve.txt", ".py"):
...
def test_repo_context_accepts_native_multi_term_rank_without_inventory(monkeypatch, tmp_path):
    ...
    def native(root, *, terms, scope_prefix, test_focus, max_results):
        ...
        return {
            "paths": ["hawking/session_transport.py"],
            "files_seen": 1,
            "terms_used": 2,
            "elapsed_ns": 73,
        }

    monkeypatch.setattr("hawking.repo_context.native_rank_content_paths", native)
    monkeypatch.setattr(
        context,
        "_candidate_files",
    ...
    assert context.paths_for(
        "Determine whether HAWKING streaming preserves durable session state",
        limit=4,
    ) == ["hawking/session_transport.py"]
    assert seen["scope_prefix"] == "hawking/"
