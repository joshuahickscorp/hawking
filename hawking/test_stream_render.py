"""hawking/stream_render.py: the pure formatter shared by the TUI and any
resident watcher. See the module docstring for why this exists.
"""

from hawking.stream_render import event_phase, is_terminal_event, render_event

# event dict -> expected render_event() output, exact strings (not "truthy").
# One row per event type TUI._on_event handles today (31 arms in hawking/tui.py).
TABLE = [
