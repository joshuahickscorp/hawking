...
"""
import re
from hawking.engine import Engine  # noqa: F401  (import guards a syntax break)
...
def test_admit_test_still_accepts_the_published_forms():
    for form in ("hawking/test_engine_tool_loop.py",
                 "pytest hawking/test_engine_tool_loop.py",
                 "python -m pytest hawking/test_engine_tool_loop.py"):
...
def test_all_three_forms_really_do_resolve_to_pytest():
    ...
    e.root = Path(__file__).resolve().parent.parent
    for form in ("hawking/test_engine_tool_loop.py",
                 "python hawking/test_engine_tool_loop.py",
                 "pytest hawking/test_engine_tool_loop.py"):
