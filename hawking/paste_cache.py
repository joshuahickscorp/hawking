...
A paste is stored EXACTLY as handed over, under ``<root>/.hawking/pastes/<id>.txt``
...
* writes go through ``hawking.persist.atomic_write_text``, so a crash leaves the
...
class PasteCache:
    """``<root>/.hawking/pastes/``. ``root`` defaults to the repo root."""

    def __init__(self, root: Optional[Union[str, Path]] = None) -> None:
        self.root = Path(root) if root is not None else find_repo_root()
        self.dir = self.root / ".hawking" / PASTES_DIRNAME
