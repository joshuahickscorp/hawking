...
HAWKING_RESULT_SCHEMA: Dict[str, Any] = {
    "type": "object",
    "properties": {
...
# turn.  HAWKING supplies omitted operations/tests/tool_calls as empty lists after
# validation; mutation support remains available when those fields are used.
HAWKING_COMPACT_RESULT_SCHEMA: Dict[str, Any] = {
...
# the wrong task.
_AGENTIC_SYSTEM_PROMPT = """HAWKING worker for one bounded WorkUnit. Disk state and deterministic evidence are authority.
...
No reasoning, markdown, or essay. Paths are workspace-relative; never modify .git. HAWKING supplies receipts."""
...
def _is_contract_example_echo(result: Any) -> bool:
    """Detect a valid JSON shape that only copied HAWKING's teaching example."""
...
_SYSTEM_PROMPT = """You are the HAWKING engineering worker.
...
{"kind":"mutation","content":"what changed",
 ...
 "tests":["hawking/test_engine_tool_loop.py"]}
...
the identical failed call. Do not invent `hawking/tests/test_engine.py` or
`hawking/tests/__init__.py`: the production module is `hawking/engine.py`, and the
tool error's directory listing/suggestion is authoritative.

PROOF MUST NAME AN EXISTING FILE. For this self-repair loop,
`hawking/test_engine_tool_loop.py` is an admitted proving test; use that exact
path unless the observations name a more specific existing test. Never invent
anything under `hawking/tests/`. A mutation must change executable behavior or
add/repair a test; comment-only, marker-only, whitespace-only, and no-op edits
are invalid and will be rolled back.

`hawking/engine.py` already exists. To change it, use `replace` with an exact
...
- operations and tests are needed only for a mutation; tool_calls is needed
  only for a tool_use; omitted arrays are treated as empty by HAWKING
...
_NOT_ADMITTED_REASON = (
    ...
    "Accepted: 'hawking/tests/test_x.py', 'pytest hawking/tests/test_x.py', "
    "'python -m pytest hawking/tests/test_x.py', 'python hawking/tests/test_x.py'. "
...
def completion_evidence_met(
    validation: Any,
    *,
    changed_files: Optional[Iterable[Mapping[str, Any]]] = None,
    diff_inspected: bool = False,
    result_packet: bool = False,
    obligations: Optional[Mapping[str, Any]] = None,
) -> bool:
    """Apply the typed completion predicate at the final mutation edge.

    ``validation.ok`` is necessary but not sufficient: an evidence receipt has
    no code test, and a model can claim a result without a grounded changed
    artifact or diff.  The default obligations are the ones emitted by
    ``build_mutation_request``.  Read-only evidence may explicitly set
    ``require_change``/``require_tests`` to false; it still needs its own
    validator and result packet.
    """
    if not isinstance(validation, dict) or validation.get("ok") is not True:
        return False
    declared = dict(obligations or {})
    require_change = bool(
        declared.get("require_change", validation.get("kind") not in {"read_only", "evidence"})
    )
    if require_change:
        if not any(
            isinstance(item, Mapping) and item.get("changed") is True
            for item in (changed_files or ())
        ):
            return False
    require_tests = declared.get("require_tests")
    if require_tests is None:
        require_tests = bool(declared.get("tests"))
    if require_tests:
        checks = validation.get("checks")
        if not isinstance(checks, list) or not any(
            ...
            and item.get("collected") != 0
            and item.get("reason") not in {"NO_EVIDENCE", "TEST_FAILED"}
            for item in checks
        ):
            return False
    if declared.get("inspect_diff", True) and not diff_inspected:
        return False
    if declared.get("result_packet", True) and not result_packet:
        return False
    return True
...
def is_accepted_measurement(validation: Any) -> bool:
    ...
    But the campaign also asks HAWKING to be a scientist, and a scientist MEASURES. Under
...
# The resident reloads ``hawking.engine`` when a loop-budget environment knob is
...
_PROTECTED_PATH_PREFIXES = frozenset({".git", ".hawking"})
...
def _focused_excerpt(content: str, prompt: str, limit: int, path: str) -> str:
    ...
    if path.endswith(".py"):
        ...
        # A function/class named by the objective is the semantic owner even
        # when a longer local variable is mentioned alongside it (for example
        # `_list_files` and its `directories_seen` result field).  The symbol
        # cache also records assignments for useful fallback targeting, so
        # distinguish definitions here instead of letting a local name steal
        # the evidence window from the requested owner.
        try:
            ...
            definitions = {}
        named_definitions = [(definitions[w], w) for w in wanted if w in definitions]
        if named_definitions:
            named = named_definitions
        if named:
    ...
    if os.environ.get("HAWKING_DUMP_EVIDENCE"):
        try:
            with open(os.environ["HAWKING_DUMP_EVIDENCE"] + ".focus", "a") as h:
...
def _operation_text(operation: Dict[str, Any], field: str) -> Optional[str]:
    ...
    """
    # Delegates. The resolver lives in hawking/mutation.py so the applier there and
...
class Engine:
    """Native HAWKING execution boundary.
    ...
    def __init__(
        ...
        # All Hawking-owned repository writes share this precondition, atomic
        # publish and crash-intent owner.  The model loop still decides WHAT
        # to propose; this object decides whether and how bytes may land.
        self.mutation_executor = MutationExecutor(self.root)
    ...
    def candidate_manifest(
        ...
        *,
        ...
        """Bind a fresh state/resource/budget snapshot to the current root.

        This is intentionally an adapter over the existing Git/resource
        authorities.  It does not create a second Goal or scheduler store.
        """
        ...
        if persist:
            persist_candidate_manifest(self.root, manifest)
        return manifest
    ...
    def complete_text(
        ...
        the HAWKING result schema so a provider's raw text can be qualified
    ...
    MAX_TOOL_ROUNDS = int(os.environ.get("HAWKING_MAX_TOOL_ROUNDS", "6"))
    ...
    # otherwise; raise it per run with HAWKING_MAX_TOOL_OBSERVATIONS. The failed-call
    # and all-repeat closers below are untouched and still fire first.
    MAX_TOOL_OBSERVATIONS = int(os.environ.get("HAWKING_MAX_TOOL_OBSERVATIONS", "1"))
    ...
    TOOL_FAILURE_TOLERANCE = int(os.environ.get("HAWKING_TOOL_FAILURE_TOLERANCE", "1"))
    ...
    def _run_tool_calls(
        ...
        if os.environ.get("HAWKING_NO_TOOLS") == "1" and calls:
            return [{
    ...
    def _compact_tool_catalog(
        ...
        *,
        ...
        keyword_families = {
            ...
            "hawking": {"hawking", "native", "cognition", "worker"},
            "acquisition": {"acquisition", "disk", "ssd", "headroom"},
            "roadmap": {"roadmap", "civilization"},
            "perception": {"perception", "vision", "multimodal"},
    ...
    def _prompt_with_observations(
        ...
        *,
        ...
        if os.environ.get("HAWKING_NO_TOOLS") == "1" or not tools_allowed:
            ...
            if (
                ...
                parts.append(
                    ...
                    "hawking/test_engine_tool_loop.py proving test. Never emit a "
                    "comment-only or invented-test mutation. `hawking/engine.py` "
    ...
    def _clamp_observation(self, text: str) -> str:
        ...
        # a new costume: hawking/tool_registry.py is 2,341 lines and a whole-file
    ...
    def execute(
        ...
        *,
        ...
        # "hawking/tool_registry.py hawking/tests/test_...py" -- so focusing on it
        ...
        try:
            ...
            # path on its FIRST call. HAWKING_NO_TOOLS used to suppress the catalog
            ...
            tools_closed = os.environ.get("HAWKING_NO_TOOLS") == "1"
            ...
            for round_index in range(self.MAX_TOOL_ROUNDS):
                ...
                if _is_contract_example_echo(result):
                    ...
                    if schema_echo_nudged:
                        raise EngineError(
                            "CONTRACT_EXAMPLE_ECHO: the resident repeated HAWKING's "
                    ...
                    conversation_history.append({
                        "role": "user",
                        "content": (
                            "That reply copied HAWKING's JSON teaching example; "
                ...
                if grounding_errors:
                    ...
                    conversation_history.append({
                        "role": "user",
                        "content": (
                            "HAWKING rejected that proposal before mutation because "
    ...
    def _evidence_char_budget(self) -> int:
        ...
        This is intentionally approximate because HAWKING must not require a
    ...
    def _prefix_key(self, payload: Dict[str, Any]) -> str:
        ...
        System text is shared across HAWKING calls; the leading user bytes keep
    ...
    def _schema_contract(
        ...
        active_schema = schema or HAWKING_RESULT_SCHEMA
    ...
    def _persist_goal_source(self, prompt: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            directory = self.root / ".hawking" / "sources"
    ...
    def _build_model_payload(
        ...
        *,
        ...
        if os.environ.get("HAWKING_DUMP_EVIDENCE"):
            try:
                with open(os.environ["HAWKING_DUMP_EVIDENCE"], "a") as handle:
    ...
    def _call_model(
        ...
        *,
        ...
        if self.model_client is not None:
            ...
            # into HAWKING's existing structured result boundary here.
            ...
            if callable(generate):
                ...
                with self._model_call_scope(est):
                    ...
                    response = generate(
                        ...
                        timeout=float(os.environ.get("HAWKING_MODEL_TIMEOUT", "1800")),
                    )
                    wall_ns = _since_ns(started)
        ...
        if degrade:
            contract = self._schema_contract(
                backend,
                schema=(
                    HAWKING_COMPACT_RESULT_SCHEMA
                    if history is not None
                    else HAWKING_RESULT_SCHEMA
                ),
            )
    ...
    def apply_typed_mutation(
        ...
        request: Optional[Mapping[str, Any] | MutationRequest] = None,
    ) -> Dict[str, Any]:
        ...
        HAWKING (the browser surface) already has its own loop and its own tool
        ...
        try:
            mutation_request = build_mutation_request(
                ...
                capability_id=(
                    ...
                    else str((request or {}).get("capability_id") or "repo.edit")
                ),
            )
            prepared = self.mutation_executor.begin(mutation_request, paths)
            # Recheck immediately before the Engine's operation applier gets
            # control.  The applier remains the single multi-operation owner;
            # this fence prevents a stale model proposal from landing.
            self.mutation_executor.recheck(prepared)
        except Exception as exc:
            return {
                "status": "rejected",
                ...
                "rolled_back": False,
                "mutation_request": (
                    mutation_request.to_dict()
                    if "mutation_request" in locals() else None
                ),
            }

        snapshot = dict(prepared.snapshots)
        before = {str(p): (p.read_text(encoding="utf-8", errors="replace")
                           if p.is_file() else None) for p in paths}
        applied = False
        intent_status = "NOT_STARTED"
        try:
            # Move the intent across the crash boundary before the first
            # operation is published.  A process death inside a later
            # operation must reconcile as partial/unknown, never as an
            # untouched request.
            self.mutation_executor.finish(
                prepared,
                "APPLIED",
                outcome={"stage": "commit_started", "applied": False},
            )
            ...
            self.mutation_executor.finish(
                prepared,
                "APPLIED",
                outcome={"stage": "atomic_publish", "applied": True},
            )
            ...
            if destructive_replaces:
                ...
                self.mutation_executor.finish(
                    prepared,
                    "NOT_STARTED",
                    outcome={"stage": "rolled_back", "reason": "blast_radius"},
                )
                ...
                return {
                    ...
                    "mutation_request": mutation_request.to_dict(),
            ...
            candidate_files = list((apply_result or {}).get("files") or [])
            if not candidate_files:
                candidate_files = self.mutation_executor._files(paths, snapshot)
            base_files = self.mutation_executor._files(paths, snapshot)
            base_tree_digest = mutation_tree_digest(
                base_files,
                hash_field="sha256_before",
            )
            candidate_tree_digest = mutation_tree_digest(candidate_files)
            validation = self._validate(paths, tests, pre_mutation=pre_validation)
            if isinstance(validation, dict):
                validation["base_tree_digest"] = base_tree_digest
                validation["candidate_tree_digest"] = candidate_tree_digest
                validation["test_candidate_tree_digest"] = candidate_tree_digest
                observed_files = self.mutation_executor._files(paths, snapshot)
                observed_tree_digest = mutation_tree_digest(observed_files)
                validation["validated_tree_digest"] = observed_tree_digest
                validation["files"] = observed_files
                if observed_tree_digest != candidate_tree_digest:
                    validation["ok"] = False
                    validation["reason"] = "TREE_CHANGED_DURING_VALIDATION"
            ok = bool(validation.get("ok")) if isinstance(validation, dict) else False
            reason = str((validation or {}).get("reason") or "")
            # An edit with no proving obligation may remain unproven for a
            # caller that explicitly requested a source-only change.  A typed
            # WorkUnit that supplied required tests is different: a collection
            # failure, empty proving surface, or other NO_EVIDENCE result must
            # roll the transaction back, otherwise a rejected provider edit
            # remains in the canonical tree and poisons the next canary.
            retain_unproven = reason == "NO_EVIDENCE" and not tests
            if ok or retain_unproven:
                after = {str(p): (p.read_text(encoding="utf-8", errors="replace")
                                  if p.is_file() else None) for p in paths}
                diff = _unified_diff(before, after, self.root)
                changed_files = list((validation or {}).get("files") or candidate_files)
                if ok and not completion_evidence_met(
                    validation,
                    changed_files=changed_files,
                    diff_inspected=True,
                    result_packet=True,
                    obligations=mutation_request.verification_obligations,
                ):
                    self._restore(snapshot)
                    intent_status = "NOT_STARTED"
                    self.mutation_executor.finish(
                        ...
                        outcome={
                            "stage": "rolled_back",
                            "reason": "INCOMPLETE_COMPLETION_EVIDENCE",
                        },
                    )
                    return {
                        "status": "rejected",
                ...
                if ok:
                    ...
                    intent_status = "APPLIED"
                completion = {
                    "schema": "hawking.completion.packet.v1",
                    "status": "VERIFIED" if ok else "APPLIED",
                    "authorized_change": True,
                    "changed_files": changed_files,
                    "tests": list((validation or {}).get("checks") or []),
                    "exit_status": [
                        item.get("exit_code")
                    ...
                    "verification": {
                        ...
                        "validated_tree_digest": (validation or {}).get("validated_tree_digest"),
                    },
                    "limitations": ([] if ok else [reason or "NO_EVIDENCE"]),
                    "next_action": (
                        "continue from the verified checkpoint"
                        if ok else "supply an applicable deterministic test"
                    ),
                }
                return {
            ...
            self.mutation_executor.finish(
                prepared,
                "NOT_STARTED",
                outcome={"stage": "rolled_back", "reason": reason or "validation"},
            )
            ...
            # ``_apply_operations`` can write an earlier operation and then
            # fail on a later one (for example an empty append).  The old
            # ``applied`` flag was set only after the helper returned, so that
            # partial transaction escaped rollback and left a Goal's source
            # half-landed.  A typed mutation is atomic: the snapshot exists
            # before the helper, therefore restore it on every helper error.
            commit_started = intent_status in {"APPLIED", "VERIFIED"}
            if intent_status == "APPLIED":
                intent_status = "UNKNOWN_OUTCOME"
                try:
                    self.mutation_executor.finish(
                        prepared,
                        "UNKNOWN_OUTCOME",
            ...
            if snapshot:
                ...
                try:
                    self.mutation_executor.finish(
                        prepared,
                        "NOT_STARTED",
                        outcome={"stage": "rolled_back_after_exception", "error": type(exc).__name__},
                    )
                except Exception:
                    pass
            return {"status": "rejected", "applied": applied or commit_started,
                    "rolled_back": bool(snapshot),
                    "reason": f"{type(exc).__name__}: {exc}",
                    "paths": [str(p) for p in paths],
                    "mutation_request": mutation_request.to_dict(),
                    "intent_status": intent_status}
    ...
    def _restore(
        ...
        for raw_path, original in snapshot.items():
            ...
            if original is None:
                self.mutation_executor.remove_file(path)
            ...
            self.mutation_executor.write_bytes(path, original)
    ...
    def _apply_operations(
        ...
        for index, operation in enumerate(
            ...
            if op == "create":
                ...
                self.mutation_executor.write_text(path, new_text)
            ...
            self.mutation_executor.write_text(path, text)
    ...
    def _test_subprocess_env(self) -> Dict[str, str]:
        ...
        for key in _TEST_ENV_KEYS:
            ...
            if value:
                env[key] = value
        # Contained test subprocesses must see the same `hawking` this
        # process loaded. The package parent is repo-root (editable) or
        # site-packages (installed). Not a fossil-name sys.path hack.
        import hawking as _hawking_pkg

        container = str(Path(_hawking_pkg.__file__).resolve().parent.parent)
    ...
    def _admit_test(
        ...
        *,
        ...
        # Measured 2026-09-05: HAWKING named tools/sovereign/test_g002_attribution.py
    ...
    def _validate(
        ...
        for raw_path in path_list:
            ...
            if path.suffix == ".rs":
                ...
                # .is_none()) was ACCEPTED on `test hawking/test_engine_tool_loop.py
