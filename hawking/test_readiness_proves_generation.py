...
from hawking.backends import OpenAICompatibleBackend  # noqa: E402
...
class _Empty200(_Corpse):
    """The exact false-success: generation returns HTTP 200 with no answer."""
    def do_POST(self):                                   # noqa: N802
...
class _MalformedTool200(_Corpse):
    """HTTP 200 with a tool finish but no callable tool payload."""
    def do_POST(self):                                   # noqa: N802
        body = json.dumps({
            "choices": [{
...
def test_http_200_without_generation_semantics_is_not_ready():
    srv, url = _serve(_Empty200)
    try:
        connector = _connector(url)
        assert connector.ready(timeout=3.0) is False
        assert connector._capability_proven is False
        assert connector._capability_failure_reason == (
            "http_success_without_generation_semantics"
        )
        with pytest.raises(RuntimeError, match="HTTP 200 without generation semantics"):
            connector.complete({
...
def test_http_200_tool_finish_without_callable_payload_is_not_ready():
    srv, url = _serve(_MalformedTool200)
    try:
        connector = _connector(url)
        assert connector.ready(timeout=3.0) is False
        assert connector._capability_failure_reason == (
...
def test_the_escape_hatch_still_works():
    import os
    srv, url = _serve(_Corpse)
    os.environ["HAWKING_REMOTE_SKIP_CAPABILITY_PROBE"] = "1"
    try:
        assert _connector(url).ready(timeout=3.0) is True
    finally:
        del os.environ["HAWKING_REMOTE_SKIP_CAPABILITY_PROBE"]
        srv.shutdown()
