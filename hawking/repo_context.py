...
CWD IS CONTEXT, NOT IDENTITY. Opening HAWKING inside a repository must not turn
...
SKIP_DIRS = {
    ...
    "receipts", "workspace", "evidence", ".hawking",
}
...
class _NativeGravityIndex:
    ...
    the historical `RepoContext` result shape while the wider HAWKING migration is
    in flight. It is enabled only by the daemon, so an ordinary short-lived
    `hawking` client cannot accidentally create a competing resident service.
    """
...
def _native_gravity_binary(root: Path) -> Optional[Path]:
    ...
    if cached is not None:
        ...
        # a later install can become visible without restarting HAWKING.
...
def _native_content_matches(root: Path, term: str) -> Optional[List[str]]:
    if os.environ.get("HAWKING_NATIVE_GRAVITY") != "1":
        return None
...
def native_tool_catalog(root: Path, *, entries: Sequence[Dict[str, str]],
    ...
    """Rank an HAWKING-owned immutable tool catalog in the resident Rust child."""
    if os.environ.get("HAWKING_NATIVE_GRAVITY") != "1":
        return None
    index = _native_gravity_index(root)
    if index is None:
...
def prewarm_native_gravity_index(root: Path) -> bool:
    ...
    idempotent, and the process remains governed by the parent HAWKING daemon's
...
class RepoContext:
    ...
    def detect(cls, start: Optional[str] = None) -> Optional["RepoContext"]:
        """The repository the user opened HAWKING in, or None if this is not one."""
    ...
    def paths_for(self, question: str, limit: int = 4) -> List[str]:
        ...
        # "hawking/serve.py" is one token AND its parts: the question names a path,
        ...
        if not path_like and limit > 0 and content_terms:
            try:
                native = native_rank_content_paths(
                    ...
                    scope_prefix=("hawking/" if re.search(r"\bhawking\b", question or "", re.IGNORECASE) else None),
        ...
        if path_like:
            ...
            # a word with it. Measured: asking about "hawking/serve.py and the
            # stream flag" returned an unrelated visual stream module, because a stem
        ...
        if re.search(r"\bhawking\b", lowered_question):
            for rel in list(scores):
                if rel.startswith("hawking/"):
                    scores[rel] += 4
        ...
        # Measured: asking about hawking/catalog.py returned it AND three other
