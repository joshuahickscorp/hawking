...
`python3 -m hawking.resident --supervise <state>`, which says nothing
...
`hawking` remains the client that talks to it.
"""
...
import hawking.resident as R
import hawking.hawkingd as D
from hawking.cli import DAEMON_SHIM, install_shims
...
def test_daemon_argv_falls_back_when_the_shim_is_absent(tmp_path, monkeypatch):
    ...
    assert argv[:3] == [sys.executable, "-m", "hawking.hawkingd"], argv
...
def test_daemon_lease_blocks_a_second_long_lived_surface(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HAWKING_DAEMON_LOCK", str(lock))
...
def test_install_writes_a_daemon_shim_that_is_not_the_client():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "-m hawking.hawkingd" in dtext, (
        ...
        assert ".local/share/hawking/current" in dtext, "daemon shim lost its PYTHONPATH"
        assert "-P -m hawking.hawkingd" in dtext, (
            "daemon shim can be shadowed by an older checkout"
        )
        deployed = Path(tmp) / ".local/share/hawking/current/hawkingd"
