...
from hawking.gravity_gauntlet import (
    BUDGET_EXHAUSTED,
    PROVEN_UNABLE,
    TARGET_HIT,
    GravityGauntlet,
...
def test_hawking_wrapper_is_the_mutation_boundary(tmp_path):
...
def test_unparseable_spec_is_not_given_a_fabricated_precision():
    ...
    from hawking.gravity_gauntlet import _spec_bits, _spec_group
...
def test_specs_without_precision_do_not_outrank_measured_ones():
    ...
    from hawking.gravity_gauntlet import _candidate_priority, Candidate
...
def test_gravity_gauntlet_score_mean_residual():
    from hawking.gravity_gauntlet import gravity_gauntlet_score

    samples = [{"residual": 2.0}, {"residual": -4.0}, {"residual": 0.0}]
    assert gravity_gauntlet_score(samples) == 2.0


def test_gravity_gauntlet_score_empty_is_zero():
    from hawking.gravity_gauntlet import gravity_gauntlet_score

    assert gravity_gauntlet_score([]) == 0.0