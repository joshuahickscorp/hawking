...
evidence live in sibling directories under the same ``.hawking`` root, and no
spelling of ``/context drop`` may reach them.

Runnable two ways:

    python3 -m pytest hawking/test_context_pastes.py -q
    python3 hawking/test_context_pastes.py
"""
...
def fresh():
    ...
    for name in ("receipts", "mission", "evidence"):
        (root / ".hawking" / name).mkdir(parents=True, exist_ok=True)
        (root / ".hawking" / name / "keep.json").write_text("{}", encoding="utf-8")
    return CommandHandler(FakeController(root)), PasteCache(root), root


def neighbours_intact(root: Path) -> bool:
    return all(
        (root / ".hawking" / name / "keep.json").is_file()
        for name in ("receipts", "mission", "evidence")
    )
...
def test_no_traversal_reaches_a_sibling_directory():
    ...
    assert neighbours_intact(root)
    assert (root / ".hawking" / "receipts" / "keep.json").is_file()
