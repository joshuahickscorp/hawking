"""GrokBridge — HAWKING's thin wrapper around the `grok-run` binary.
...
``GrokRunHandle``
  ...
  ``verify(task_id) -> dict``     provider-side structured receipt; not HAWKING acceptance
...
4. For ``delegate``, **must** pass ``mutation_lock=`` wrapping
   ``hawking.resources.MutationLock`` acquire/release (see below). Omitting
...
5. Returns a human string (task id, state, report excerpt). Structured
   ...
   ``<workspace>/.hawking/grok/<task_id>.json``.
...
``hawking/resources.py``, ``ResourceClass.MUTATION``, cap 1,
...
``<workspace>/.hawking/grok/<task_id>.json`` (directory created if needed)
...
inside HAWKING is as auditable as one of HAWKING's own mutations.
...
NO_MISSION_LOCK_WARNING = (
    ...
    "no mutation-serialization was applied. A live HAWKING mission MUST pass "
...
def find_grok_run() -> str:
    ...
    # export PATH made HAWKING refuse to drive Grok on a machine where the binary
...
class GrokBridge:
    """Thin, honest wrapper around ``grok-run`` for HAWKING."""
    ...
    def __init__(
        ...
        if receipts_dir is None:
            self.receipts_dir = self.workspace / ".hawking" / "grok"
        else:
    ...
    def delegate(
        ...
        *,
        ...
        ``mutation_lock`` serializes this call against HAWKING's MUTATION
    ...
    def revise(
        ...
        *,
        ...
        """Resume one Grok task with a new contract under HAWKING's write lock."""
        validate_contract_text(contract_text)
        ...
        out = {
            ...
            "limitations": [
                "A revision is provider work; HAWKING acceptance still requires its own verifier.",
            ],
        }
    ...
    def verify(self, task_id: str, *, timeout: float = 120.0) -> Dict[str, Any]:
        """Read Grok's structured receipt without promoting it to HAWKING evidence."""
        self._require_task_id(task_id)
        ...
        return {
            ...
            "stdout": raw[-12000:],
            "stderr": (result.stderr or "")[-12000:],
            "hawking_acceptance": False,
            "hawking_acceptance_reason": "provider verify output is advisory until HAWKING independently verifies the workspace",
        }
    ...
    def mission(
        ...
        *,
        ...
        """Expose Grok V2's DAG mission compiler behind HAWKING's receipt boundary.

        The V2 scheduler remains an adapter: HAWKING records the exact invocation,
        does not treat its stdout as verification, and serializes the launch
        against the HAWKING mutation resource unless the caller explicitly runs a
        dry plan.
        """
        ...
        try:
            ...
            raise GrokContractError(
                "mission file must be inside the HAWKING workspace"
            ) from exc
        if not path.is_file():
        ...
        payload = {
            "schema": "hawking.grok_mission.v1",
            "mode": selected,
            ...
            "limitations": [
                "V2 stdout is a provider report, not HAWKING verification.",
                "HAWKING must independently inspect artifacts and run its own verifier before acceptance.",
            ],
        }
