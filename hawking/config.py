...
class Config:
    def __init__(self, workspace: str, global_path: Optional[str] = None):
        ...
        self.global_path = global_path or os.path.expanduser("~/.config/hawking/config.json")
        self.project_path = os.path.join(workspace, ".hawking", "config.json")
    ...
    def enable_thinking(self, default: bool = False) -> bool:
        return coerce_bool(
            self.value("enable_thinking", "HAWKING_ENABLE_THINKING", None),
            default=default,
        )

    def response_schema_on(self, default: bool = True) -> bool:
        raw = self.value("response_schema", "HAWKING_RESPONSE_SCHEMA", None)
        if raw is None:
            return default
    ...
    def model_tokens(self) -> Tuple[Optional[int], Optional[str]]:
        ...
        """
        raw_env = os.environ.get("HAWKING_MODEL_TOKENS")
