...
def _active_hawking_workers() -> list[str]:
    """Return no vendor lanes; Hawking workers are mission-owned threads."""
    return []
...
def snapshot() -> dict:
    ...
    return {
        ...
        "load_1m": _load_1m(),
        "active_hawking_workers": _active_hawking_workers(),
    }
...
def clean_box_ok(snap: dict | None = None, min_free_gib: float = 15.0) -> tuple[bool, str]:
    ...
    s = snap or snapshot()
    lanes = s.get("active_hawking_workers") or []
    if lanes:
...
if __name__ == "__main__":
    ...
    assert isinstance(snap["active_hawking_workers"], list)
