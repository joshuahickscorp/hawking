"""HAWKING could make exactly ONE conditioned tool round, no matter how well it went.
...
def test_the_default_is_unchanged_behaviour():
    ...
    assert int(os.environ.get("HAWKING_MAX_TOOL_OBSERVATIONS", "1")) == 1 or True
    src = (pathlib.Path(__file__).resolve().parents[1] / "hawking" / "engine.py").read_text()
    assert 'HAWKING_MAX_TOOL_OBSERVATIONS", "1"' in src, "default drifted from 1 silently"
...
def test_the_env_override_is_honoured(monkeypatch):
    ...
    monkeypatch.setenv("HAWKING_MAX_TOOL_OBSERVATIONS", "5")
    import importlib

    import hawking.engine as E
    importlib.reload(E)
    try:
        assert E.Engine.MAX_TOOL_OBSERVATIONS == 5
    finally:
        monkeypatch.delenv("HAWKING_MAX_TOOL_OBSERVATIONS", raising=False)
        importlib.reload(E)
