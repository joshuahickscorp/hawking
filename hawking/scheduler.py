...
LOG = logging.getLogger("hawking.scheduler")
...
# Measurement copy: tools/headless/hawking_scheduler_quality.py::remaining_depth.
...
class Scheduler:
    ...
    def from_workspace(
        ...
        **kwargs: Any,
        ...
        """Rebuild from disk using Hawking-owned in-process recovery only.

        ``grok_liveness`` remains an ignored keyword solely so old callers can
        reconstruct a scheduler while the historical adapter is retired.  A
        running legacy external unit is interrupted and re-routed through the
        normal Hawking WorkUnit path; no subprocess is adopted or relaunched.
        """
        del grok_liveness
        store = DagStore(workspace)
        units = store.load(recover_running=True, grok_liveness=None)
