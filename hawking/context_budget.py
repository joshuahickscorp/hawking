"""Single authority for HAWKING llama.cpp context-window arithmetic.

llama-server takes `--ctx-size TOTAL --parallel N` and then gives each
slot `ceil(TOTAL / N / 256) * 256` tokens. Every budget in HAWKING must be
...
def _generation_reserve(explicit: Optional[int]) -> int:
    if explicit is not None:
        try:
            ...
            if value >= 0:
                return value
    raw = os.environ.get("HAWKING_MODEL_TOKENS")
...
def _profile_genome_path() -> Path:
    return Path.home() / ".config" / "hawking" / "machine_genome.json"
...
def resolve(
    *,
    ...
    # HAWKING_MODEL_TOKENS still wins, which is what `_generation_reserve` checks.
    ...
    if override_val is None:
        raw = os.environ.get("HAWKING_CTX_SIZE")
        if raw is not None and str(raw).strip() != "":
            try:
                parsed = int(raw)
            except (TypeError, ValueError):
            ...
            if parsed is not None and parsed > 0:
                override_val = parsed
                override_source = "override:HAWKING_CTX_SIZE"
    ...
    if override_val is not None and override_source is not None:
        ...
        provenance["override"] = _lost(
            "no ctx_size argument and HAWKING_CTX_SIZE unset"
        )
        provenance["profile"] = {
            "won": True,
            "value": profile_val,
    ...
    *,
    ...
    if ok:
        ...
        remedy = (
            ...
            f"raise HAWKING_CTX_SIZE/--ctx-size (currently {budget.total_ctx}); "
