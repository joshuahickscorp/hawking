...
def _metal_budget_module():
    ...
    if not path.is_file():
        return None
    spec = importlib.util.spec_from_file_location("hawking_metal_budget", path)
...
class MemGate:
    ...
    def __init__(
        ...
        env_overhead = os.environ.get("HAWKING_PER_RUNTIME_OVERHEAD_GIB")
        if per_runtime_overhead_bytes is not None:
            ...
            self.per_runtime_overhead_bytes = DEFAULT_PER_RUNTIME_OVERHEAD_BYTES
        env_head = os.environ.get("HAWKING_GPU_HEADROOM_FRAC")
        ...
        env_reserve = os.environ.get("HAWKING_MEM_RESERVE_BYTES") or os.environ.get(
            "HAWKING_RESERVE_GIB"
        )
        if self.reserve_bytes is None and env_reserve:
            try:
                if os.environ.get("HAWKING_MEM_RESERVE_BYTES"):
                    self.reserve_bytes = int(os.environ["HAWKING_MEM_RESERVE_BYTES"])
                else:
    ...
    def _swap_ceiling(self) -> int:
        ...
        env = os.environ.get("HAWKING_SWAP_CEILING_GIB")
        if env:
            try:
...
# ~/.config/hawking/machine_genome.json is produced by
...
# receipts/headless/QWEN_MAX_EQUILIBRIUM*.json (pools spawned by HAWKING,
...
def genome_staleness_horizon_s(override: Optional[int] = None) -> int:
    if override is not None:
        ...
        return max(1, n)
    raw = os.environ.get("HAWKING_GENOME_STALENESS_HORIZON_S")
    if raw:
        n = _read_positive_int(str(raw).strip())
        if n is not None:
            return n
...
def resolve_runtime_limits(
    ...
    *,
    ...
    1. env HAWKING_RESIDENT_RUNTIME_LIMIT / HAWKING_ACTIVE_DECODE_LIMIT
    2. ~/.config/hawking/machine_genome.json  (FRESH genomes only)
    3. <repo>/receipts/headless/MACHINE_GENOME.json  (FRESH if genome-shaped)
    4. <repo>/.hawking-legacy/bootstrap-director-v6/worker-equilibrium.json
    ...
    resident: Optional[Tuple[int, str]] = _env_int(
        "HAWKING_RESIDENT_RUNTIME_LIMIT", "RESIDENT_RUNTIME_LIMIT"
    )
    active: Optional[Tuple[int, str]] = _env_int(
        "HAWKING_ACTIVE_DECODE_LIMIT", "ACTIVE_DECODE_LIMIT"
    )

    home_genome = Path.home() / ".config" / "hawking" / "machine_genome.json"
    ...
    for directory in _walk_parents(start):
        candidate = (
            directory / ".hawking-legacy" / "bootstrap-director-v6" / "worker-equilibrium.json"
        )
    ...
    if eq_path is None:
        eq_path = (
            root / ".hawking-legacy" / "bootstrap-director-v6" / "worker-equilibrium.json"
        )
...
def resolve_decode_topology(
    ...
    """
    env = (os.environ.get("HAWKING_DECODE_TOPOLOGY") or "").strip().lower()
    if env in {"slot", "process"}:
        return env, "env:HAWKING_DECODE_TOPOLOGY"
    root = Path(repo_root) if repo_root is not None else default_repo_root()
...
class MachineGenome:
    ...
    ``resolve_runtime_limits``, which reads ``~/.config/hawking/machine_genome.json``
    ...
    ``HAWKING_HOME`` so the two files cannot alias.

    Persistence is covered by sealed ``receipts/headless/HAWKING_PERSISTENCE_AUDIT.json``;
...
def get_machine_genome_path() -> Path:
    ...
    base = Path(os.environ.get("HAWKING_HOME", Path.home() / ".local" / "share" / "hawking"))
