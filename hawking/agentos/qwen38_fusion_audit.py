...
question from the checked-in Rust source and current HAWKING profile without
...
SCHEMA = "hawking.agentos.qwen38_fusion_source_audit.v1"
...
def run_qwen38_fusion_source_audit(
    *,
    ...
    destination = Path(emit).expanduser() if emit else repo / "receipts" / "headless" / "HAWKING_QWEN38_FUSION_SOURCE_AUDIT.json"
    ...
    try:
        ...
        # This experiment is explicitly a source/profile semantics audit; it
        # must not require a runnable native binary or artifact just to derive
        # the selected dispatch graph. Physical execution remains a separate
        # qualified experiment and is recorded as NOT_MEASURED below.
