...
from hawking.agentos.accelerator_regression import (
    _bench,
    _profile_path,
    _quiescence,
    _repo_root,
...
from hawking.nomenclature import NOMENCLATURE_VERSION
from hawking.persist import atomic_write_json


SCHEMA = "hawking.agentos.protected_accelerator_benchmark.v1"
...
def _lock_path(repo: Path) -> Path:
    path = repo / ".hawking" / "locks" / LOCK_NAME
...
def run_protected_accelerator_benchmark(
    *,
    ...
    destination = Path(emit).expanduser() if emit else repo / "receipts" / "headless" / "HAWKING_PROTECTED_ACCELERATOR_BENCHMARK.json"
