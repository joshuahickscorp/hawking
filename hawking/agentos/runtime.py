"""Operational Hawking facade over the canonical mission authorities."""
...
from hawking.providers import ResidentProfile, RoleRouter, profile_from_backend
from hawking.result_envelope import build_result_envelope
from hawking.runtime_iface import model_semantics_for
from hawking.tool_registry import ToolRegistry, ToolResult, default_tool_registry
from hawking.background import BackgroundJobStore


CHECKPOINT_SCHEMA = "hawking.control_checkpoint.v1"


def _find_repo_root(start: Path) -> Path:
    for path in (start, *start.parents):
        if (path / "hawking").is_dir() and (path / "pyproject.toml").is_file():
            return path
    ...
    # installed HAWKING project instead of silently turning that workspace into
...
class Hawking:
    ...
    they were before this facade existed.  Hawking owns composition: it gives
    ...
    def __init__(
        ...
        *,
        ...
        self.tools: ToolRegistry = default_tool_registry(
            ...
            mission_root=self.workspace / ".hawking" / "mission",
    ...
    def checkpoint_path(self) -> Path:
        return self.workspace / ".hawking" / "control" / "checkpoint.json"

    def _next_action(self, mission: Optional[Mission] = None) -> str:
        active = mission or self.mission
        if active is None:
            return "start a mission with Hawking.start_mission(goal)"
    ...
    def prior_knowledge_snapshot(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "available": False,
                "path": str(getattr(self.knowledge, "path", self.workspace / ".hawking")),
    ...
    def _context_memory(self) -> Optional[Dict[str, Any]]:
        ...
        if not prior.get("available") or not prior.get("records"):
            return None
        return {
            "schema": "hawking.context.memory.v1",
            "generation": 0,
    ...
    def _persist_control_checkpoint(self, *, event: str, next_action: Optional[str] = None) -> Path:
        ...
        payload = {
            ...
            "continuation": {
                "reconstruct": "Hawking(workspace).recover_mission()",
                "continue": "Hawking(workspace).continue_mission()",
    ...
    def start_mission(
        ...
        *,
        ...
        if self.engine is None:
            raise RuntimeError("Hawking requires an Engine or Controller")
    ...
    def register_provider(self, name: str, provider: Any) -> None:
        """Attach a Hawking provider under an exact policy/selector name."""
        key = str(name or "").strip()
        if not key or any(char.isspace() for char in key):
            raise ValueError("provider name must be a non-empty token")
        self.providers[key] = provider

    def register_cognition(self, selector: str, **kwargs: Any) -> Any:
        """Register one lazy provider-neutral cognition selector.

        OpenRouter remains opt-in and fail-closed; Hawking-native cognition is
        the default local provider. Provider construction performs no network
        call and does not create a resident or scheduler.
        """
        from hawking.remote_cognition import provider_for_selector
        ...
        parsed = str(selector).split(":", 1)[0].strip().lower()
        model = str(selector).split(":", 1)[1].strip()
        self.register_provider(f"{parsed}:{model}", provider)
        return provider
    ...
    def invoke_tool(self, name: str, arguments: Optional[Dict[str, Any]] = None) -> ToolResult:
        ...
        # Tool results are durable evidence.  This write is Hawking-owned and
        # stays under the mission's .hawking receipt directory; the tool itself
        # still cannot acquire a write permission by accident.
        destination = self.workspace / ".hawking" / "receipts" / "tools" / f"{result.invocation_id}.json"
...
# Compatibility spelling for historical imports. It is not a separate
# lifecycle, provider registry, scheduler, receipt, or public product.
AgentOS = Hawking

__all__ = ["Hawking", "AgentOS"]
