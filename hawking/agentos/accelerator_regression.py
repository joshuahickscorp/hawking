...
to become HAWKING's accelerator contract.  This gate records the current and
...
from hawking.agentos.benchmark_boundary import classify_window
...
SCHEMA = "hawking.agentos.accelerator_regression.v1"
...
def _profile_path(repo: Path, value: Optional[str | os.PathLike[str]]) -> Path:
    chosen = value or os.environ.get("HAWKING_NATIVE_CONFIG")
    return Path(chosen).expanduser().resolve() if chosen else (repo / "hawking" / DEFAULT_PROFILE_NAME).resolve()
...
def _run_one_request(
    ...
    *,
    ...
    from hawking.hawking_native import HawkingNativeConnector
...
def _write(report: Dict[str, Any], emit: Optional[str], repo: Path) -> None:
    destination = Path(emit).expanduser() if emit else repo / "receipts" / "headless" / "HAWKING_ACCELERATOR_REGRESSION.json"
...
def run_accelerator_regression(
    *,
    ...
    from hawking.hawking_native import HawkingNativeConfig
    ...
    try:
        ...
        prior_smoke = _read_json(repo / "receipts" / "headless" / "HAWKING_ACCELERATOR_NATIVE_SMOKE.json")
        live = _run_one_request(config, max(0.1, float(timeout_s)))
        report["identity"] = {
            "experiment": {
                "id": "hawking-current-resident-one-request",
                "class": "ACCEL-DEVICE",
        ...
        report["prior_receipts"] = {
            "native_smoke": {
                "path": str(repo / "receipts" / "headless" / "HAWKING_ACCELERATOR_NATIVE_SMOKE.json"),
