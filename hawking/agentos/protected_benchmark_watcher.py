...
optionally pauses only clearly identified HAWKING benchmark jobs for the duration
...
from hawking.agentos.accelerator_regression import _profile_path, _quiescence, _repo_root
...
from hawking.flash_next import REPO_ID
from hawking.persist import (
    atomic_write_json,
    json_compatible_copy as _safe,
)


SCHEMA = "hawking.agentos.qwen_protected_benchmark_watcher.v1"
...
def _job_is_pausable_hawking(job: Mapping[str, Any]) -> bool:
    if _job_is_model_lake(job):
        return False
    label = str(job.get("label") or "").lower()
    argv = " ".join(str(item) for item in (job.get("argv") or [])).lower()
    return "hawking" in label and any(marker in argv or marker in label for marker in PAUSABLE_MARKERS)
...
def _classify_blockers(jobs: Iterable[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    for job in jobs:
        ...
        elif _job_is_pausable_hawking(job):
            pausable.append(_safe(job))
        else:
            blockers.append({"kind": "OTHER_RUNNING_JOB", "job": _safe(job)})
    return {"blockers": blockers, "pausable_hawking_jobs": pausable}


def _lock_path(repo: Path) -> Path:
    path = repo / ".hawking" / "locks" / "qwen-protected-bench.lock"
...
def run_protected_benchmark_watcher(
    *,
    ...
    report: Dict[str, Any] = {
        ...
        "polls": [],
        "runs": [],
        "claim_boundary": "This watcher governs benchmark readiness only. The diagnostic result remains NOT_FOR_PROMOTION; no process outside explicitly identified HAWKING benchmark jobs is signaled.",
    }
        ...
        while time.time() < deadline:
            ...
            poll: Dict[str, Any] = {
                ...
                "eligible_without_pause": (
                    ...
                    and (pause_known_jobs or not job_classes["pausable_hawking_jobs"])
                ),
            }
                ...
                if lock_handle is None:
                    ...
                    try:
                        ...
                        locked_eligible = (
                            ...
                            and (pause_known_jobs or not locked_classes["pausable_hawking_jobs"])
                        )
                        if not locked_eligible:
                            poll["decision"] = "RECHECK_FAILED"
                        else:
                            if pause_known_jobs:
                                paused = _pause_jobs(locked_classes["pausable_hawking_jobs"])
