...
from hawking.nomenclature import preferred_flash_science_receipt
from hawking.persist import atomic_write_json


SCHEMA = "hawking.agentos.dense_vs_nf_ab.v1"
...
def _model_identity(repo: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        profile = {}
    try:
        flash = json.loads(preferred_flash_science_receipt(repo).read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError):
        flash = {}
    return {
        "model_a": {
...
def run_ab_scaffold(
    *,
    ...
    destination = Path(emit).expanduser().resolve() if emit else repo / "receipts" / "headless" / "HAWKING_DENSE_VS_NF_AB_SCAFFOLD.json"
