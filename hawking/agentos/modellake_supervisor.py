...
from hawking.flash_next import EXPECTED_BYTES, PINNED_REVISION, REPO_ID
...
SCHEMA = "hawking.agentos.modellake_supervision.v1"
...
def _job(repo: Path, job_id: Optional[str]) -> Optional[Dict[str, Any]]:
    ...
    if any(char not in "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_" for char in token):
        return None
    return _read(repo / ".hawking" / "background" / "jobs" / f"{token}.json")
...
def run_model_lake_supervision(
    *,
    ...
    destination = Path(emit).expanduser().resolve() if emit else repo / "receipts" / "headless" / "MODELLAKE_FLASH_NEXT_SUPERVISION.json"
