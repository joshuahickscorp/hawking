...
``agent.recover_mission()`` (via ``hawking.mission.load_state``) already raises
...
(``hawking.mission.load_state``), so a corrupt file is classified as "has work"
...
from hawking.resident import _mission_has_work


def _mission_dir(tmp_path: Path) -> Path:
    d = tmp_path / ".hawking" / "mission"
