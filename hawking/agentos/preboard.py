...
from hawking.persist import (
    atomic_write_json,
    read_json_object_or_none as _read,
)


SCHEMA = "hawking.agentos.preboard.v1"
NEGATIVE_RECEIPTS = (
...
def _fpga_surface(repo: Path) -> Dict[str, Any]:
    ...
    try:
        result = subprocess.run(
            ["rg", "--files", str(repo / "hawking"), str(repo / "tools"), str(repo / "crates")],
            capture_output=True,
            text=True,
            timeout=8.0,
            check=False,
...
def _preboard_plan() -> Dict[str, Any]:
    interfaces = [
        {"name": "kernel_genome", "schema": "hawking.fpga.kernel_genome.v1", "status": "INTERFACE_DEFINED", "fields": ["kernel_id", "input_layout", "output_layout", "dtype", "resources", "proof_hash"]},
        {"name": "hwir", "schema": "hawking.fpga.hwir.v1", "status": "INTERFACE_DEFINED", "fields": ["nodes", "buffers", "dependencies", "synchronization", "placement_constraints"]},
        {"name": "link", "schema": "hawking.fpga.link.v1", "status": "INTERFACE_DEFINED", "fields": ["symbols", "memory_regions", "relocations", "artifact_hash"]},
        {"name": "partitioner", "schema": "hawking.fpga.partitioner.v1", "status": "INTERFACE_DEFINED", "fields": ["source_graph", "partitions", "cut_bytes", "cut_synchronization", "device_assignment"]},
        {"name": "cache", "schema": "hawking.fpga.cache.v1", "status": "INTERFACE_DEFINED", "fields": ["content_address", "kernel_genome", "toolchain_identity", "target_identity", "verification"]},
        {"name": "simulator", "schema": "hawking.fpga.simulator.v1", "status": "CONTRACT_ONLY", "fields": ["hwir", "cycle_trace", "resource_usage", "numerical_output", "negative_controls"]},
        {"name": "harness", "schema": "hawking.fpga.harness.v1", "status": "CONTRACT_ONLY", "fields": ["input_fixture", "reference_output", "candidate_output", "tolerance", "trace"]},
        {"name": "verifier", "schema": "hawking.fpga.verifier.v1", "status": "INTERFACE_DEFINED", "fields": ["identity", "numerical_parity", "resource_bounds", "capability_contract", "receipt"]},
    ]
    return {
...
def _write(report: Dict[str, Any], emit: Optional[str], repo: Path) -> None:
    destination = Path(emit).expanduser() if emit else repo / "receipts" / "headless" / "HAWKING_AGENTOS_PREBOARD.json"
