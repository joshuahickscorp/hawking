...
from hawking.flash_next import (
...
from hawking.nomenclature import preferred_modellake_census_receipt
...
SCHEMA = "hawking.flash-next.pre-runtime-science.v1"
...
def _fetch(name: str, limit: int, timeout_s: float) -> Dict[str, Any]:
    url = RAW_BASE + name
    request = urllib.request.Request(url, headers={"User-Agent": "hawking-hawking/1"})
    try:
...
def _accelerator_primitive_plan() -> Dict[str, Any]:
    ...
    rows = [
        ...
        ("complete_token_acceptance_meter", "mtp", "HAWKING receipt/verifier and unattended complete-work accounting exist", "Flash-native accepted-token and rollback fields are not wired to a runtime", "P0"),
    ...
    return {
        ...
        "existing_capability_sources": [
            "receipts/headless/HAWKING_FPGA_PREBOARD.json",
            "receipts/headless/QWEN27_FPGA_ORGAN_MAP.json",
            "receipts/headless/FLASH_NEXT_FPGA_ORGAN_MAP.json",
            "receipts/headless/HAWKING_QWEN38_FUSION_SOURCE_AUDIT.json",
        ],
...
def _active_compute_bounds(
    ...
    bandwidth_gbps = _number(os.environ.get("HAWKING_FLASH_BANDWIDTH_GBPS"))
    roof: Dict[str, Any] = {
...
def _write(report: Dict[str, Any], emit: Optional[str], repo: Path) -> None:
    destination = Path(emit).expanduser() if emit else repo / "receipts" / "headless" / "FLASH_NEXT_PRE_RUNTIME_SCIENCE.json"
