...
from hawking.persist import (
    atomic_write_json,
    sha256_file_or_none as _sha256,
)
...
SCHEMA = "hawking.agentos.initial_charge.v1"
CHARGE_ID = "HAWKING_INITIAL_CHARGE"
DEFAULT_WORKSPACE_NAME = ".hawking/initial-charge"
...
def _specs(repo: Path) -> list[Dict[str, Any]]:
    ...
    census_receipt = str(preferred_modellake_census_receipt(repo).relative_to(repo))
    flash_receipt = str(preferred_flash_science_receipt(repo).relative_to(repo))
    return [
        {
        ...
        },
        {
            ...
            "evidence_paths": ["receipts/headless/HAWKING_ACCELERATOR_REGRESSION.json", "receipts/headless/NOETIC_DISPATCH_FUSION.json"],
            "required_verifier": "source receipt presence, source hash, classification boundary, and deterministic map fingerprint",
            "receipts": ["receipts/headless/HAWKING_ACCELERATOR_REGRESSION.json", "receipts/headless/NOETIC_DISPATCH_FUSION.json"],
        },
        {
            ...
            "receipts": ["receipts/headless/HAWKING_ACCELERATOR_REGRESSION.json"],
        },
        {
            ...
            "evidence_paths": [flash_receipt, "receipts/headless/FLASH_NEXT_PRECEDENT_MAP.json"],
        ...
        },
        {
            ...
            "evidence_paths": ["receipts/headless/HAWKING_AGENTOS_PREBOARD.json", flash_receipt],
        ...
        {
            "id": "P2-negative-science-doctor-perception-prep",
            "priority": "P2",
            "role": "evidence_science",
            "dependencies": ["P0-C-precedent-and-transfer-index"],
            "resource_class": "LIGHT_CONTROL",
            "next_experiment": "Use negative receipts to constrain the next experiment and prepare Doctor/PERCEPTION evidence surfaces without touching protected systems or starting Odyssey.",
            ...
            "allowed_mutations": ["receipts/headless/**"],
            "evidence_paths": ["receipts/headless/HAWKING_AGENTOS_PREBOARD.json"],
            "required_verifier": "negative receipt census, explicit boundary, no PERCEPTION/protected-system mutation, no Odyssey launch",
            "receipts": [],
        },
    ]
...
def create_initial_charge(
    *,
    ...
    from hawking.agentos.runtime import AgentOS
    ...
    goal = (
        ...
        "preboard, research/evidence/negative science, and HAWKING self-repair only when a "
    ...
    agent.checkpoint()
    state_path = root / ".hawking" / "mission" / "state.json"
    report = {
        "schema": SCHEMA,
        "charge_id": CHARGE_ID,
        "status": "CREATED",
