...
SCHEMA = "hawking.agentos.fpga_preboard.v1"
VERIFIED = "[V]"
DERIVED = "[D]"
SIMULATED = "[S]"
...
class FPGADeviceGenome:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"schema": "hawking.fpga.device_genome.v1", "label": DERIVED, **self.__dict__}
...
class HBMGenome:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"schema": "hawking.fpga.hbm_genome.v1", "label": DERIVED, **self.__dict__}
...
class MockFPGAProvider(FPGAProvider):
    ...
    def execute(self, hwir: Mapping[str, Any]) -> Dict[str, Any]:
        return {
            "schema": "hawking.fpga.receipt.v1",
            "status": "SIMULATED",
            "label": SIMULATED,
...
def simulate_partition(model: str, simulator: Optional[TransportLinkSimulator] = None) -> Dict[str, Any]:
    ...
    return {
        "schema": "hawking.fpga.partition_simulation.v1",
        "model": model,
        "label": SIMULATED,
...
def partitioner(model: str, organs: Iterable[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    return {"schema": "hawking.fpga.partitioner.v1", "model": model_key, "partitions": rows, "status": "CANDIDATE_ONLY"}
...
def _map(model: str) -> Dict[str, Any]:
    ...
    experiment_dag = {
        "schema": "hawking.fpga.experiment_dag.v1",
        "nodes": [
    ...
    return {
        "schema": "hawking.fpga.organ_map.v1",
        "model": model,
        "label": DERIVED,
        "model_identity": (
            {"label": "Qwen3.8-27B sealed resident / NOETIC_PARENT_A", "profile": "hawking/hawking-native.sealed-3.14.json", "artifact_identity": "NOETIC_PARENT_A"}
        ...
        "module_cache": {
            "schema": "hawking.fpga.module_cache.v1",
        ...
        "reproducible_vivado_vitis_harness": {
            "schema": "hawking.fpga.harness.v1",
...
def run_fpga_preboard(
    *,
    ...
    destination = Path(emit).expanduser().resolve() if emit else repo / "receipts" / "headless" / "HAWKING_FPGA_PREBOARD.json"
