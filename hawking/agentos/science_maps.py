...
TRANSFER_SCHEMA = "hawking.agentos.qwen38_accelerator_transfer_map.v1"
PRECEDENT_SCHEMA = "hawking.agentos.flash_next_precedent_map.v1"
...
def build_transfer_map(repo_root: Optional[str | os.PathLike[str]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        profile = {}
    flash_receipt = _read_receipt(preferred_flash_science_receipt(repo)) or {}
    regression = _read_receipt(repo / "receipts" / "headless" / "HAWKING_ACCELERATOR_REGRESSION.json") or {}
    common = [
    ...
    payload = {
        ...
        "model_a": {
            ...
            "identity_receipts": _find(repo, ["HAWKING_ACCELERATOR_REGRESSION.json", "NOETIC_DISPATCH_FUSION.json", "HAWKING_ACCELERATOR_NATIVE_SMOKE.json"]),
        },
        "model_b": {
            ...
            "identity_receipts": _find(
                repo,
                [*FLASH_SCIENCE_RECEIPT_NAMES, *MODELLAKE_CENSUS_RECEIPT_NAMES],
            ),
        },
...
def write_science_maps(
    *,
    ...
    return {
        "schema": "hawking.agentos.science_maps.v1",
        "status": "PASSED",
