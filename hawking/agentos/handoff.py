...
from hawking.nomenclature import (
    NOMENCLATURE_VERSION,
    preferred_flash_science_receipt,
    preferred_modellake_census_receipt,
    preferred_modellake_supervision_receipt,
)
from hawking.persist import atomic_write_json


SCHEMA = "hawking.agentos.overnight_handoff.v1"
...
def _window_summary(repo: Path) -> Dict[str, Any]:
    final_path = repo / "receipts" / "headless" / "HAWKING_AGENTOS_UNATTENDED_WINDOW.json"
    final = _read_object(final_path)
    candidates = [
        repo / ".hawking" / "autonomy-window-verified" / "window-progress.json",
        repo / ".hawking" / "autonomy-window" / "window-progress.json",
        repo / ".hawking" / "unattended-qwen-long" / "window-progress.json",
    ]
...
def _model_lake_summary(repo: Path) -> Dict[str, Any]:
    census_path = preferred_modellake_census_receipt(repo)
    supervision_path = preferred_modellake_supervision_receipt(repo)
...
def _flash_summary(repo: Path) -> Dict[str, Any]:
    flash = _read_object(preferred_flash_science_receipt(repo)) or {}
...
def _qwen27_summary(repo: Path) -> Dict[str, Any]:
    regression = _receipt(repo, "HAWKING_ACCELERATOR_REGRESSION.json") or {}
    fusion_audit = _receipt(repo, "HAWKING_QWEN38_FUSION_SOURCE_AUDIT.json") or {}
    ...
    profile_path = repo / "hawking" / "hawking-native.sealed-3.14.json"
    ...
    return {
        ...
        "fusion_source_audit": {
            ...
            "receipt_path": str(repo / "receipts" / "headless" / "HAWKING_QWEN38_FUSION_SOURCE_AUDIT.json"),
...
def _fpga_summary(repo: Path) -> Dict[str, Any]:
    preboard = _receipt(repo, "HAWKING_FPGA_PREBOARD.json") or {}
    return {
        "preboard": {
...
def build_handoff(repo_root: Optional[str | os.PathLike[str]] = None, *, emit: Optional[str | os.PathLike[str]] = None) -> Dict[str, Any]:
    ...
    fusion_status = (_receipt(repo, "HAWKING_QWEN38_FUSION_SOURCE_AUDIT.json") or {}).get("status")
    blockers = [
    ...
    payload: Dict[str, Any] = {
        ...
        "hawking": {
            ...
            "current_default_profile": str(repo / "hawking" / "hawking-native.sealed-3.14.json"),
            ...
            "dense_vs_nf_scaffold": str(repo / "receipts" / "headless" / "HAWKING_DENSE_VS_NF_AB_SCAFFOLD.json"),
        },
        ...
        "verification": {
            ...
            "receipt_gates": {
                "autonomy": (_receipt(repo, "HAWKING_AGENTOS_AUTONOMY_GATE.json") or {}).get("status"),
                "accelerator_regression": (_receipt(repo, "HAWKING_ACCELERATOR_REGRESSION.json") or {}).get("status"),
                "qwen38_fusion_audit": fusion_status,
                "preboard": (_receipt(repo, "HAWKING_AGENTOS_PREBOARD.json") or {}).get("status"),
                "flash_pre_runtime": (_read_object(preferred_flash_science_receipt(repo)) or {}).get("status"),
        ...
        "blockers": blockers,
        "continuation": {
            "inspect_status": f"python3 -m hawking status --workspace {repo} --repo-root {repo}",
            "refresh_checkpoint": f"python3 -m hawking checkpoint --repo-root {repo} --workspace {repo} --emit {repo / 'receipts/headless/HAWKING_CHECKPOINT.json'}",
            "supervise_modellake": f"python3 -m hawking modellake-supervise --repo-root {repo} --job-id {MODEL_LAKE_JOB} --emit {repo / 'receipts/headless/MODELLAKE_FLASH_NEXT_SUPERVISION.json'}",
            "modellake_job_status": f"python3 -m hawking background status --workspace {repo} --repo-root {repo} {MODEL_LAKE_JOB}",
            "unattended_job_status": f"python3 -m hawking background status --workspace {repo} --repo-root {repo} {UNATTENDED_JOB}",
            "flash_transform_job_status": f"python3 -m hawking background status --workspace {repo} --repo-root {repo} {FLASH_TRANSFORM_JOB}",
            "refresh_initial_charge": f"python3 -m hawking initial-charge --repo-root {repo} --workspace {repo / '.hawking/initial-charge'} --emit {repo / 'receipts/headless/HAWKING_INITIAL_CHARGE.json'}",
            "refresh_maps": f"python3 -m hawking science-maps --repo-root {repo} --transfer-emit {repo / 'receipts/headless/QWEN38_ACCELERATOR_TRANSFER_MAP.json'} --precedent-emit {repo / 'receipts/headless/FLASH_NEXT_PRECEDENT_MAP.json'}",
            "refresh_ab": f"python3 -m hawking ab-scaffold --repo-root {repo} --emit {repo / 'receipts/headless/HAWKING_DENSE_VS_NF_AB_SCAFFOLD.json'}",
            "refresh_fpga_preboard": f"python3 -m hawking fpga-preboard --repo-root {repo} --emit {repo / 'receipts/headless/HAWKING_FPGA_PREBOARD.json'}",
            "run_full_tests": "pytest -q",
            "run_qwen38_fusion_audit": f"python3 -m hawking qwen38-fusion-audit --repo-root {repo} --profile {repo / 'hawking/hawking-native.sealed-3.14.json'} --emit {repo / 'receipts/headless/HAWKING_QWEN38_FUSION_SOURCE_AUDIT.json'}",
            "run_qwen27_runtime_archaeology": f"python3 -m hawking qwen27-runtime-archaeology --repo-root {repo} --profile {repo / 'hawking/hawking-native.sealed-3.14.json'}",
            "run_qwen27_mlp_diagnostic": f"python3 -m hawking qwen27-mlp-ab --repo-root {repo} --profile {repo / 'hawking/hawking-native.sealed-3.14.json'} --resident-binary {repo / '.hawking/instrumented/ascension_qwen38_resident'}",
            "watch_protected_qwen_window": f"python3 -m hawking protected-bench-watch --repo-root {repo} --profile {repo / 'hawking/hawking-native.sealed-3.14.json'} --resident-binary {repo / '.hawking/instrumented/ascension_qwen38_resident'} --duration-s 21600 --interval-s 60",
            "build_flash_executable_scaffold": f"python3 -m hawking flash-executable --repo-root {repo}",
            "run_flash_component_body": f"python3 -m hawking flash-component-body --root /Volumes/corpdrive/hawking-modellake/specimens/Qwen--Qwen3.8-Flash-Next@34567a4712bc --repo-root {repo} --emit {repo / 'receipts/headless/FLASH_NOETIC_ROUTED_EXPERT_BODY.json'}",
            "run_flash_component_campaign": f"python3 -m hawking flash-component-campaign --repo-root {repo} --emit {repo / 'receipts/headless/FLASH_NOETIC_ROUTED_EXPERT_COMPONENT_CAMPAIGN.json'}",
            "run_flash_graph_component": f"python3 -m hawking flash-graph-component --repo-root {repo} --emit {repo / 'receipts/headless/FLASH_NOETIC_ROUTED_EXPERT_GRAPH.json'}",
            "run_flash_router_component_body": f"python3 -m hawking flash-matrix-body --root /Volumes/corpdrive/hawking-modellake/specimens/Qwen--Qwen3.8-Flash-Next@34567a4712bc --repo-root {repo} --tensor-name model.language_model.layers.0.mlp.gate.weight --component-kind router --emit {repo / 'receipts/headless/FLASH_NOETIC_ROUTER_COMPONENT_BODY.json'}",
            "run_flash_router_graph": f"python3 -m hawking flash-router-graph --repo-root {repo} --emit {repo / 'receipts/headless/FLASH_NOETIC_ROUTER_GRAPH.json'}",
            "run_flash_router_selection": f"python3 -m hawking flash-router-selection --repo-root {repo} --body-receipt {repo / 'receipts/headless/FLASH_NOETIC_ROUTER_COMPONENT_FULL_BODY.json'} --kernel-receipt {repo / 'receipts/headless/FLASH_NOETIC_ROUTER_COMPONENT_FULL_KERNEL_PARITY.json'} --emit {repo / 'receipts/headless/FLASH_NOETIC_ROUTER_SELECTION.json'}",
            ...
            "run_flash_router_representation_ab": f"python3 -m hawking flash-router-representation-ab --repo-root {repo} --root /Volumes/corpdrive/hawking-modellake/specimens/Qwen--Qwen3.8-Flash-Next@34567a4712bc --emit {repo / 'receipts/headless/FLASH_NOETIC_ROUTER_REPRESENTATION_AB.json'}",
            "run_flash_tensor_probe": f"python3 -m hawking flash-tensor-probe --emit {repo / 'receipts/headless/FLASH_FIRST_TENSOR_PROBE.json'}",
            "run_flash_representation_experiment": f"python3 -m hawking flash-representation-experiment --emit {repo / 'receipts/headless/FLASH_ROUTED_EXPERT_REPRESENTATION_EXPERIMENT.json'}",
            "run_flash_representation_replication": f"python3 -m hawking flash-representation-experiment --expert-indices 32,33,34,35,36,37,38,39 --row-start 64 --row-count 16 --emit {repo / 'receipts/headless/FLASH_ROUTED_EXPERT_REPRESENTATION_EXPERIMENT_DISJOINT.json'}",
            "run_flash_transform_parity": f"python3 -m hawking flash-transform-parity --root /Volumes/corpdrive/hawking-modellake/specimens/Qwen--Qwen3.8-Flash-Next@34567a4712bc --chunk-rows 128 --emit {repo / 'receipts/headless/FLASH_FULL_TENSOR_TRANSFORM_PARITY.json'}",
            "run_flash_bounded_loader_roundtrip": f"python3 -m hawking flash-loader-roundtrip --root /Volumes/corpdrive/hawking-modellake/specimens/Qwen--Qwen3.8-Flash-Next@34567a4712bc --repo-root {repo} --candidate independent_q4_g64 --emit {repo / 'receipts/headless/FLASH_ROUTED_EXPERT_LOADER_ROUNDTRIP.json'}",
            "run_flash_bounded_loader_roundtrip_quality_alternate": f"python3 -m hawking flash-loader-roundtrip --root /Volumes/corpdrive/hawking-modellake/specimens/Qwen--Qwen3.8-Flash-Next@34567a4712bc --repo-root {repo} --candidate shared_bf16_basis_nf4_residual --emit {repo / 'receipts/headless/FLASH_ROUTED_EXPERT_LOADER_ROUNDTRIP_SHARED.json'}",
            "run_flash_native_kernel_parity": f"{repo / '.hawking/flash-kernel-build/release-fast/release/examples/flash_noetic_q4_kernel_parity'} --root /Volumes/corpdrive/hawking-modellake/specimens/Qwen--Qwen3.8-Flash-Next@34567a4712bc --descriptor {repo / 'receipts/headless/FLASH_ROUTED_EXPERT_LOADER_ROUNDTRIP.json'} --candidate-body {repo / '.hawking/flash-component/flash-routed-expert-independent-q4-g64-e0-r0-128.bin'} --body-receipt {repo / 'receipts/headless/FLASH_NOETIC_ROUTED_EXPERT_BODY.json'} --reps 7 --warmup 2 --out {repo / 'receipts/headless/FLASH_NOETIC_Q4_BODY_KERNEL_PARITY.json'}",
            "resume_modellake_only_if_interrupted": f"python3 -m hawking background resume --workspace {repo} --repo-root {repo} {MODEL_LAKE_JOB}",
            "do_not_start": "Do not launch or promote a new Odyssey; continue only through the existing HAWKING/ModelLake authorities and governed windows.",
        },
    }
