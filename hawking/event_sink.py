...
``<workspace>/.hawking/mission/events.jsonl``, so a reader in another process
(``hawking resident watch``) can tail real activity by byte offset instead of
...
class EventSink:
    ...
    (hawking/events.py EventBus.emit has no per-subscriber error handling), so a
    ...
    def __init__(self, workspace: Union[str, Path], *, max_bytes: int = 8 * 1024 * 1024) -> None:
        self._path = Path(workspace) / ".hawking" / "mission" / _EVENTS_FILENAME
