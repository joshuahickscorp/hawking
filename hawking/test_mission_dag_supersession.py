...
``DagStore`` is ONE workspace-global ``.hawking/dag.json``, so before this the
...
def test_the_superseded_dag_is_archived_not_deleted(tmp_path):
    ...
    before = json.loads((tmp_path / ".hawking" / "dag.json").read_text())
    ...
    live = json.loads((tmp_path / ".hawking" / "dag.json").read_text())
...
def test_a_compatible_dag_is_never_retired(tmp_path):
    ...
    assert not (tmp_path / ".hawking" / "dag-retired").exists()
