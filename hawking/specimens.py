"""The specimen registry HAWKING reasons over: every sealed specimen, from disk.

47 sealed specimens sit under ``/Volumes/corpdrive/hawking-modellake/specimens``
today (override: ``HAWKING_MODEL_LAKE_ROOT``), one directory per
...
``HAWKING_SPECIMEN_MANIFEST_DIR``), not on the ModelLake volume itself.
...
SCHEMA = "hawking.specimens.registry.v1"
...
def _manifest_dir() -> Path:
    configured = os.environ.get("HAWKING_SPECIMEN_MANIFEST_DIR")
    if configured:
