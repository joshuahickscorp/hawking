...
def _decision_core_path() -> Path:
    ...
    The HAWKING package is copied into ``~/.local/share/hawking/build-*`` by
    ...
    breaks an ordinary ``hawking web`` launch.  Keep the source module as the
    single canonical owner, but resolve it through the install stamp when the
    package is detached from the checkout.
    """
    candidates = []
    configured = os.environ.get("HAWKING_SOURCE_ROOT")
    if configured:
