...
def test_genome_matches_refuses_on_digest_drift():
    """The NX load gate must REFUSE a machine whose genome digest differs from the
    sealed digest, and accept only an exact digest match. This exercises the
    production predicate tools.nx_genome.genome_matches directly."""
    ...
    nx_genome = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(nx_genome)

    sealed = {"genome_digest": "a" * 64}
    same = {"genome_digest": "a" * 64}
    drifted = {"genome_digest": "b" * 64}

    assert nx_genome.genome_matches(sealed, same) is True
    assert nx_genome.genome_matches(sealed, drifted) is False
    assert nx_genome.genome_matches(sealed, {}) is False
    assert nx_genome.genome_matches(None, same) is False
    assert nx_genome.genome_matches(sealed, None) is False