...
HAWKING historically called its local model ``qwen`` even when the selected
...
The concrete transports remain in :mod:`hawking.backends`.  This module does not
...
CAPABILITY_SCHEMA = "hawking.provider.capabilities.v1"
ROLE_SCHEMA = "hawking.provider.roles.v1"
GENERATION_REQUEST_SCHEMA = "hawking.provider.generation_request.v1"
GENERATION_RESPONSE_SCHEMA = "hawking.provider.generation_response.v1"
HEALTH_SCHEMA = "hawking.provider.health.v1"
FAILURE_SCHEMA = "hawking.provider.failure.v1"
RECEIPT_SCHEMA = "hawking.provider.receipt.v1"
...
class GenerationRequest:
    ...
    be HTTP or to use a particular model family. Edge/provider options stay in
    the bounded ``transport_options`` mapping; Hawking-owned receipt metadata
    remains in ``metadata`` and is never promoted to a provider payload.
    """
    ...
    # Provider-neutral request fields that are standard at the OpenAI edge but
    # transport-specific downstream (for example OpenRouter ``provider``
    # routing, tool_choice, or stream_options).  They are copied explicitly;
    # arbitrary metadata never crosses the provider boundary.
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "GenerationRequest":
        ...
        if not tools and isinstance(value.get("functions"), (list, tuple)):
            tools = tuple(
                {"type": "function", "function": dict(item)}
        ...
        legacy_tool_choice: Dict[str, Any] = {}
        function_call = value.get("function_call")
        if "function_call" in value and "tool_choice" not in value:
            if isinstance(function_call, str) and function_call in {"auto", "none"}:
                legacy_tool_choice = {"tool_choice": function_call}
            elif isinstance(function_call, Mapping) and function_call.get("name"):
                legacy_tool_choice = {
                    "tool_choice": {
                        "type": "function",
        ...
        return cls(
            ...
            transport_options={
                **(
                    dict(value.get("transport_options") or {})
                    if isinstance(value.get("transport_options"), Mapping)
                    else {}
                ),
                **{
                    key: value[key]
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "transport_options": _copy(dict(self.transport_options)),
        }
    ...
    def to_payload(self) -> Dict[str, Any]:
        ...
        payload.pop("transport_options", None)
        for key, value in self.transport_options.items():
            if (
                key in {"model", "messages", "max_tokens", "temperature", "tools", "metadata"}
                or str(key).startswith("hawking_")
            ):
                continue
...
DEFAULT_ROLE_POLICIES: Dict[str, RolePolicy] = {
    ...
    "vision": RolePolicy(
        "vision", ("perception", "multimodal", "remote", "local"), ("vision",)
    ),
...
class RoleRouter:
    ...
    def _matches_preference(provider: str, preference: str) -> bool:
        ...
        aliases = {
            ...
            "local": {"native", "noetic_native", "hawking_native", "hawking-native"},
