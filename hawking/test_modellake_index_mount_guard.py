...
def test_modellake_execution_boundary_rejects_provider_replay():
    """P16 boundary guard: a fresh attempt must not replay a prior provider turn."""
    from hawking import modellake_gate

    boundary = getattr(modellake_gate, "EXECUTION_BOUNDARY", None)
    assert boundary is not None, "modellake_gate must declare EXECUTION_BOUNDARY"
    assert getattr(boundary, "allow_provider_replay", True) is False, (
        "execution boundary must forbid replaying a prior provider turn"
    )