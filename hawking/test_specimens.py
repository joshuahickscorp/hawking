"""hawking.specimens: the disk-derived registry, never a hand-maintained list.
...
from hawking.specimens import get, registry
...
def _lab(tmp_path: Path, monkeypatch) -> tuple[Path, Path]:
    ...
    module reads in production (HAWKING_MODEL_LAKE_ROOT / _MANIFEST_DIR)."""
    lake = tmp_path / "lake"
...
def test_unmounted_lake_is_not_reported_as_zero_specimens(tmp_path, monkeypatch):
    monkeypatch.setenv("HAWKING_MODEL_LAKE_ROOT", str(tmp_path / "nothing-mounted-here"))
    monkeypatch.setenv("HAWKING_SPECIMEN_MANIFEST_DIR", str(tmp_path / "manifests"))
    doc = registry()
...
def test_real_lake_enumeration_is_cheap(monkeypatch):
    ...
    monkeypatch.delenv("HAWKING_MODEL_LAKE_ROOT", raising=False)
    monkeypatch.delenv("HAWKING_SPECIMEN_MANIFEST_DIR", raising=False)
