...
WHAT WAS MISSING. hawking/serve.py reached ZERO tools: it stripped everything but a
...
CHAT_TOOLS: Dict[str, str] = {
    ...
    "workspace.identity": "identify the bound Hawking workspace",
    "fs.read": "read a file in this project",
    "fs.list": "list a directory in this project",
    "fs.search": "find text in this project's source",
    "source.owner": "find the canonical owner of a repository concern",
    "source.outline": "inspect the bounded outline of a source file",
    "source.symbol": "inspect a named source symbol",
    "source.references": "find bounded references to a source symbol",
    "git.status": "inspect the grounded working-tree status",
    "git.diff": "inspect the grounded working-tree diff",
    "processes.list": "inspect Hawking-owned process state",
    "permissions.status": "inspect current Hawking machine capabilities",
    "capabilities.status": "inspect the Hawking capability readiness map",
    "runtime.status": "inspect the current Hawking runtime boundary",
    "goals.list": "inspect current and recent detached Hawking Goals",
    "artifact.read": "read one explicit Hawking artifact by artifact id",
    "artifact.search": "search workspace-scoped Hawking artifacts",
    "memory.search": "search workspace-scoped H-NOTES memory",
...
def _registry_menu(
        registry: Any, *, allowed_names: Optional[Collection[str]] = None,
) -> Dict[str, str]:
...
def session_menu(
    ...
    ``allowed_names`` is a server-selected request fence, not a prompt hint.
    It narrows an otherwise write-built registry for a bounded worker request;
    callers must pass the same fence to the dispatcher as well as the prompt.
    """
    allowed = (set(allowed_names) if allowed_names is not None else None)
    menu = _registry_menu(registry, allowed_names=allowed)
    ...
    menu.update({
        name: description for name, description in CHAT_TOOLS.items()
        if allowed is None or name in allowed
    })
    if write:
        menu.update({
            name: description for name, description in BUILDER_TOOLS.items()
            if allowed is None or name in allowed
        })
    return menu
...
def prompt_menu_names(
    ...
    wandered through perception and unrelated session files. Keep high-frequency
    ...
    """
    offered = session_menu(registry, write=write, allowed_names=allowed_names)
...
_GOAL_STATE_QUERY = re.compile(
    r"\b(?:current|active|recent|latest|most\s+recent|detached|disposable|running)\b"
    r".{0,100}\b(?:goal|workunit|worker|hawking)\b|"
    r"\b(?:goal|workunit|worker)\b.{0,100}\b(?:status|found|did|doing|result|state)\b",
    re.I | re.S,
)
...
def explicit_tool_observation_required(messages: Sequence[Dict[str, str]]) -> bool:
    ...
    return bool(
        ...
        or _GOAL_STATE_QUERY.search(latest)
    )


def goal_state_query_required(text: str) -> bool:
    """Whether an operator query needs canonical detached-Goal context."""
    return bool(_GOAL_STATE_QUERY.search(str(text or "")))
...
LOCAL_SHAPES = {
    ...
    "runtime.status": '{}',
    "capabilities.status": '{}',
    "artifact.read": '{"artifact_id": <string>}   optional: max_chars',
    "artifact.search": '{"query": <string>}   optional: session_id, limit',
    "memory.search": '{"query": <string>}   optional: session_id, limit',
    "goals.list": '{"limit": <integer>}   optional: status, goal_id, goal_ids',
...
def system_block(names: Optional[Sequence[str]] = None,
                 registry: Any = None, write: bool = False,
                 allowed_names: Optional[Collection[str]] = None) -> str:
    """The contract, stated once. Stable text, so it stays in the prefix cache."""
    menu = session_menu(registry, write=write, allowed_names=allowed_names)
    rows = []
    for name in (names or prompt_menu_names(
            registry, write=write, allowed_names=allowed_names)):
        if name not in menu:
            continue
    ...
    return (
        ...
        '  {"tool": "web.search", "arguments": {"query": "Hawking native runtime"}}\n'
        "\nAvailable tools:\n" + "\n".join(rows) + "\n"
        "\nWhich to reach for:\n"
        ...
        "  about current/recent/detached Hawking Goals or workers  ->  goals.list\n"
        ...
        "must start with receipts/ or come from .hawking state. For a recent "
        ...
        "inspect the broad HAWKING/AgentOS surface, use that phrase directly. "
...
def _repair(raw: str) -> Optional[Dict[str, Any]]:
    ...
    sealed-3.14 emitted `{"function": "fs.list", "arguments": {"path": "hawking"}}}`
...
def _call_from_object(parsed: Dict[str, Any]) -> Optional[Tuple[str, Dict[str, Any]]]:
    ...
    # Durable Goal workers may return a typed MutationProposal as the
    # provider's HTTP-200 assistant object.  It is still provider data, not
    # authority, but the existing Hawking builder door is the correct place
    # to validate and execute its operations.  Without this bridge the object
    # had no ``tool``/``op`` discriminator, so run_with_tools treated a valid
    # proposal as prose and the WorkUnit eventually dead-lettered it.
    if (
        str(parsed.get("status") or "").strip().upper() == "MUTATION_PROPOSED"
        and isinstance(parsed.get("operations"), list)
    ):
        arguments = {
        ...
        if tests is None:
            tests = parsed.get("required_tests")
        if tests is not None:
            arguments["tests"] = tests
        for key in ("mutation_request", "expected_base_hashes", "canonical_root"):
            if key in parsed:
                arguments[key] = parsed[key]
        return "repo.edit", arguments
...
def parse_call(text: str) -> Optional[Tuple[str, Dict[str, Any]]]:
    ...
    protocol; it speaks Qwen's, and under real use it emitted
        ...
        <tool_call>
        {"function": "fs.list", "arguments": {"path": "hawking"}}}
        </tool_call>
    ...
    # A typed MutationProposal is intentionally discriminated by
    # ``status``/``operations`` rather than a tool-name field. Try the
    # complete JSON document before the legacy brace-fragment parser so
    # nested source text and operation arrays are not mistaken for prose.
    try:
        parsed = json.loads(text.strip())
    except (TypeError, ValueError, json.JSONDecodeError):
        parsed = None
    if isinstance(parsed, dict):
        direct_call = _call_from_object(parsed)
...
def parse_calls(text: str) -> List[Tuple[str, Dict[str, Any]]]:
    ...
    (.hawking/selfdev/evidence/cycle-0008.txt). parse_call (singular) returns only
    ...
    if not ok:
        ...
        if "receipt path must be under receipts" in error_text:
            return (f"{name} FAILED: {error_text}\n"
                    ...
                    "starting with receipts/ (or a returned .hawking state path); "
        ...
        if failure == "MUTATION_BUNDLE":
            ...
            try:
                ...
                encoded = str(value)
            if len(encoded) > 5200:
                encoded = encoded[:5200] + "…"
            return (
                f"{name} BUNDLE_INCOMPLETE: {error_text}; no file was changed.\n"
                f"bundle={encoded}\n"
                "Emit only the missing operation named by missing_requirements; "
                "Hawking preserves valid source operations and will join them "
                "before the engine transaction.")
...
def _search_needs_escalation(result: Any) -> bool:
    ...
    in .hawking/receipts/*.json, with files_seen 5001 and truncated True -- the
    per-call file budget was spent walking thousands of generated receipts
    before the walk ever reached hawking/*.py, where the function is defined and
...
def _source_search_score(result: Any) -> int:
    ...
    for match in matches:
        ...
        if path.endswith((".py", ".rs", ".go", ".ts", ".tsx", ".js")):
            score += 30
        if path.startswith(("hawking/", "src/", "crates/", "tools/", "tests/")):
            score += 50
        if any(part in path for part in (
                "/receipts/", "receipts/", "/.hawking/", ".hawking/",
...
def _invoke_registry(
    ...
    *,
    suppress_native_helpers: bool = False,
) -> Any:
    """Dispatch one registry call under the request's process policy.

    The normal registry may use a daemon-owned Rust helper as an optimization.
    A restricted worker request explicitly promises not to start helper
    processes, so it enters the registry's context-local suppression fence for
    this one call.  The registry remains the sole dispatcher; this wrapper
    does not add a compatibility execution path.
    """
    if not suppress_native_helpers:
        return registry.invoke(name, arguments)
    from .tool_registry import suppress_restricted_worker_capabilities as _suppress
    with _suppress():
        return registry.invoke(name, arguments)
...
def escalate_search(registry: Any, arguments: Dict[str, Any],
                    result: Any = None, *,
                    suppress_native_helpers: bool = False) -> Optional[Any]:
    ...
    if _search_needs_escalation(current) and not arguments.get("glob"):
        ...
        try:
            candidate = _invoke_registry(
                registry, "fs.search", narrowed,
                suppress_native_helpers=suppress_native_helpers,
            )
        except Exception:
            return None
...
def _interactive_test_scope_refusal(arguments: Dict[str, Any]) -> Optional[str]:
    ...
    broad = {".", "hawking", "hawking/tests", "tests", "tools"}
...
_EVIDENCE_OBLIGATION_RULES: Tuple[
    ...
    ("detached goal state", _GOAL_STATE_QUERY,
     frozenset({"goals.list"})),
...
def _passing_test_row(
        trace: Sequence[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    """The latest focused test invocation that actually passed.

    A typed Goal bundle's engine transaction invokes the named tests during
    validation.  That is a real Hawking-owned test dispatch, not model prose;
    count its structured validation alongside an optional follow-up
    ``tests.run`` so the Goal does not demand a redundant second process.
    """
    return next((
        row for row in reversed(trace)
        if (
            ...
            and row.get("dispatched") and row.get("ok")
            and row.get("applied") is True
            and row.get("engine_test_passed") is True
        )
    ), None)
...
def _trace_grounding_issues(
    ...
    if isinstance(tools, dict):
        ...
        if isinstance(invocations, list) and not invocations and successful:
            issues.append(
                "tools.successful_invocations is empty but HAWKING recorded "
    ...
    if isinstance(tool_coverage, dict):
        ...
        if isinstance(recorded, list) and not recorded and successful:
            issues.append(
                "tool_coverage.successful is empty but HAWKING recorded successful "
    ...
    if "real_action_performed" in document and successful and not str(
        ...
        issues.append(
            "real_action_performed is empty although HAWKING recorded successful dispatches")
    ...
    if isinstance(mutation, dict) and "accepted" in mutation:
        ...
        if mutation.get("accepted") is not True and accepted_mutation is not None:
            issues.append(
                "mutation.accepted is false although HAWKING recorded an applied "
    ...
    if isinstance(verification, dict) and "passed" in verification:
        ...
        if verification.get("passed") is not True and passing_test is not None:
            issues.append(
                "verification.passed is false although HAWKING recorded a passing "
    ...
    if (isinstance(mission_verdict, str)
        ...
        issues.append(
            ...
            "is present in HAWKING's dispatch trace")
...
def _repair_trace_redundant_fields(
    ...
    """Repair receipt fields that merely restate HAWKING's dispatch ledger.
    ...
    judgments: HAWKING already knows exactly which calls succeeded and what each
...
_OBSERVATION_LEDGER_PREFIX = "[HAWKING OBSERVATION LEDGER -- MECHANICAL, NOT CONCLUSIONS]"
...
def _observation_ledger(trace: Sequence[Dict[str, Any]], *, limit: int = 2400) -> str:
    ...
    for item in selected:
        ...
        rows.append(row)
    rows.append(
        "These are calls already dispatched by HAWKING. Preserve them when "
...
def _mutation_bundle_recovery_nudge(value: Any) -> str:
    """Ask for only the missing operation of a rejected mutation bundle."""
    doc = value if isinstance(value, dict) else {}
    missing = ", ".join(str(item) for item in (doc.get("missing_requirements") or []))
    preserved = doc.get("preserved_operations") or []
    preserved_text = json.dumps(preserved, separators=(",", ":"), default=str)
    if len(preserved_text) > 2400:
        preserved_text = preserved_text[:2400] + "…"
    return (
        "MUTATION_BUNDLE_INCOMPLETE: HAWKING rejected the proposal before any "
        "file changed. Missing requirement(s): " + (missing or "bundle completeness") + ". "
        "The valid operations below are preserved by Hawking and will be joined "
        "automatically. Emit ONLY one repo.edit call containing the missing "
        "regression-test mutation; do not repeat or replace the preserved source "
        "operation, do not add an unrelated path, and do not claim acceptance. "
        "Use the exact observed test path and exact observed anchor/text. The "
        "existing focused tests list is also preserved, so omit it if doing so "
        "keeps the repair small. PRESERVED_OPERATIONS=" + preserved_text
    )
...
def run_with_tools(
    ...
    *,
    ...
    bundle_state: Optional[Dict[str, Any]] = None,
    ...
    bare 502 with nothing HAWKING could act on. A body that never gets a
    ...
    offered = session_menu(
        registry, write=engine is not None, allowed_names=allowed_names,
    )
    ...
    mutation_progress_nudged = False
    controller_routed_label: Optional[str] = None
    bundle_repair_value: Optional[Dict[str, Any]] = None
    ...
    def _evidence_nudge(pending: Dict[str, frozenset[str]], *, repeated: bool) -> str:
        ...
        # call; HAWKING supplies neither evidence nor an answer here.
        preferred = {
            "repository": ("git.status",),
            "filesystem": ("fs.list",),
    ...
    def _finalize(candidate: Any) -> Tuple[str, List[Dict[str, Any]]]:
        ...
        if parse_calls(repaired):
            ...
            return (
                "HAWKING could not satisfy the requested final JSON contract: "
        ...
        trace.append({
            ...
            **({
                ...
                "evidence_class": "HAWKING_SERVING_CONTRACT",
    ...
    for _ in range(max(0, round_limit)):
        ...
        if not calls:
            ...
            # Durable Goal/Build turns have a declared mutation contract. If
            # the provider has already produced a real bounded read but stops
            # before the mutation door, use one Hawking-owned continuation
            # nudge based on observed evidence rather than waiting for the
            # model's prose to mention the contract correctly. No operation,
            # path, or result is supplied here; repo.edit remains the only
            # admitted writer and the dispatcher remains authoritative.
            read_observed = any(
                row.get("dispatched") and row.get("ok")
            ...
            mutation_seen = any(
                row.get("dispatched") and str(row.get("tool") or "") == "repo.edit"
                for row in trace
            )
            if (
                engine is not None
                and mutation_bundle is not None
                and read_observed
                and not mutation_seen
                and not mutation_progress_nudged
            ):
                conversation.append({"role": "assistant", "content": str(text or "")[:1000]})
                conversation.append({
                    "role": "user",
                    "content": (
                        "Hawking has a successful bounded read for this mutation "
                        "WorkUnit. Do not continue archaeology or answer in prose. "
                        "The next turn must emit exactly one admitted repo.edit "
                        "call grounded only in the observed source and declared "
                        "mutation bundle. Do not claim the edit until Hawking "
                        "returns its dispatcher result."
                    ),
                })
                mutation_progress_nudged = True
                continue
            ...
            routed = None
            if pending_label == "detached goal state" and not dispatched:
                # Detached Goal state is a canonical operator fact, not a
                # model-memory preference.  A prose answer that says it lacks
                # access must not be returned when the local projection door
                # is available; make the first read deterministic and keep
                # the final synthesis in the model.
                routed = _controller_evidence_call(pending_label)
            elif (pending_label is not None and dispatched
            ...
            if not calls and tool_observation_required and not dispatched:
                ...
                if not tool_obligation_nudged:
                    ...
                    if len(str(text or "")) > len(excerpt):
                        excerpt += "\n[unsupported reply truncated by HAWKING]"
                    ...
                    continue
                return (
                    "HAWKING could not satisfy the required tool observation: no "
            ...
            if not calls and pending_mission:
                ...
                if attempts >= allowed_attempts:
                    return (
                        "HAWKING could not satisfy the explicit mission gate: "
        ...
        for name, arguments in calls[:max(1, max_calls)]:
            ...
            if dispatch_name in MUTATION_TOOLS:
                ...
                result = run_local_tool(
                    dispatch_name, dispatch_arguments, cache=cache,
                    knowledge=knowledge,
                    workspace=getattr(getattr(registry, "context", None),
                                      "workspace", None),
                )
                ...
                result = type("InteractiveTestRefusal", (), {
                    ...
                    "provenance": {"source": "hawking.chat_tools"},
                    "failure_class": "INVALID_ARGUMENTS",
                })()
            else:
                result = _invoke_registry(
                    registry, dispatch_name, dispatch_arguments,
                    suppress_native_helpers=suppress_native_helpers,
                )
            escalated = None
            if dispatch_name == "fs.search" and (
                ...
                escalated = escalate_search(
                    registry, arguments, result,
                    suppress_native_helpers=suppress_native_helpers,
                )
            ...
            if controller_routed_label is not None:
                ...
                entry["evidence_class"] = "HAWKING_SERVING_CONTRACT"
            ...
            if dispatch_name in MUTATION_TOOLS:
                ...
                if isinstance(value, dict):
                    ...
                    if isinstance(validation, dict):
                        entry["engine_test_validation"] = validation
                        entry["engine_test_paths"] = list(value.get("tests") or [])
                        entry["engine_test_passed"] = bool(validation.get("ok"))
                    if value.get("schema") == MUTATION_BUNDLE_SCHEMA or (
                            isinstance(value.get("missing_requirements"), list)
                            and "bundle_complete" in value):
                        entry["bundle_complete"] = bool(value.get("bundle_complete"))
                        entry["missing_requirements"] = list(
                            value.get("missing_requirements") or [])
                        entry["preserved_operations"] = list(
                            value.get("preserved_operations") or [])
                        entry["bundle_operations"] = list(
                            value.get("bundle_operations") or
                            value.get("operations") or [])
            ...
            elif getattr(result, "failure_class", "") == "MUTATION_BUNDLE":
                # Deficiency observations are evidence too: the next model turn
                # must see the machine-readable missing requirement and the
                # preserved operation, even though no registry result was ok.
                entry["observation_excerpt"] = observation[:1200]
                entry["observation"] = observation
                entry["observation_chars"] = len(observation)
                entry["observation_sha256"] = hashlib.sha256(
                    observation.encode("utf-8", "replace")).hexdigest()
        ...
        if repeated_names:
            ...
            conversation.append({
                "role": "user",
                "content": (
                    "HAWKING did not rerun these identical requests: "
                    + ", ".join(unique_repeats)
        ...
        if recovery_needed and not recovery_nudged and not stop_early:
            ...
            conversation.append({"role": "user", "content": (
                _mutation_bundle_recovery_nudge(bundle_repair_value)
                if bundle_repair_value is not None else
    ...
    if parse_calls(final):
        ...
        return (
            "HAWKING reached the bounded tool budget before the objective was "
...
def build_registry(workspace: str, repo_root: Optional[str] = None, *,
    ...
    HAWKING authority switch: it admits reversible/workspace/repository work and
...
def prepend_system(messages: Sequence[Dict[str, str]], text: str) -> List[Dict[str, str]]:
    ...
    of the repo context produced exactly that, and only on the native body --
        the retired legacy backend tolerated it, so every acceptance test passed
        while the flagship resident 502'd.
...
def openai_schemas(registry: Any,
                   ...
                   *, write: bool = False,
                   allowed_names: Optional[Collection[str]] = None) -> List[Dict[str, Any]]:
...
#: Doors served by HAWKING itself rather than by the tool registry: retrieval of
...
LOCAL_TOOLS = (
    ...
    "observation.expand", "debug.diagnose", "reverse.identify",
    "forensics.snapshot",
)


def run_local_tool(name: str, arguments: Dict[str, Any], *,
                   cache: Any = None, knowledge: Any = None,
                   workspace: Any = None, runtime: Any = None) -> Any:
    ...
    if name == "goals.list":
        if workspace is None:
            return _R(False, error="goals.list needs a Hawking workspace")
        try:
            from .goal_surface import list_goals, load_result
            root = Path(workspace).expanduser().resolve()
            raw_limit = arguments.get("limit", 12)
            try:
                limit = min(32, max(1, int(raw_limit)))
            except (TypeError, ValueError):
                return _R(False, error="goals.list limit must be an integer")
            wanted_status = str(arguments.get("status") or "").strip().upper()
            wanted_id = str(arguments.get("goal_id") or "").strip().upper()
            scoped_goal_ids = {
                str(item).strip().upper()
                for item in (arguments.get("goal_ids") or [])
                if str(item).strip()
            } if "goal_ids" in arguments else None
            rows = []
            for goal in reversed(list_goals(root, limit=32)):
                goal_id = str(goal.get("goal_id") or "").strip()
                if scoped_goal_ids is not None and goal_id.upper() not in scoped_goal_ids:
                    continue
                status = str(goal.get("status") or "UNKNOWN").upper()
                if wanted_status and status != wanted_status:
                    continue
                if wanted_id and goal_id.upper() != wanted_id:
                    continue
                result = load_result(root, goal_id) if goal_id else None
                row: Dict[str, Any] = {
                    "goal_id": goal_id,
                    "objective": str(goal.get("objective") or "")[:800],
                    "status": status,
                    "phase": str(goal.get("phase") or "")[:160],
                    "workunit_id": str(goal.get("workunit_id") or "")[:160],
                    "worker_model": str(goal.get("resident_assignment") or "")[:160],
                ...
                if isinstance(result, Mapping):
                    row["result"] = {
                        ...
                        "files_changed": [str(item)[:240] for item in
                                           (result.get("files_changed") or [])[:12]],
                    }
                rows.append(row)
                if len(rows) >= limit:
                    break
            return _R(True, value={
                "schema": "hawking.goals.projection.v1",
                "count": len(rows),
                "goals": rows,
                "authority": "canonical Hawking Goal files",
            }, provenance={"source": "hawking.goal_surface.list_goals",
                           "workspace": str(root)})
        except Exception as exc:
            return _R(False, error=f"canonical Goal state unavailable: {type(exc).__name__}: {exc}")

    if name == "workspace.identity":
        if workspace is None:
            return _R(False, error="workspace.identity needs a Hawking workspace")
        try:
            root = Path(workspace).expanduser().resolve()
            from .web_artifacts import workspace_id
            return _R(True, value={
                "schema": "hawking.workspace.identity.v1",
                "workspace_id": workspace_id(root),
                "name": root.name or str(root),
                "kind": "git" if (root / ".git").exists() else "directory",
                "authority": "Hawking workspace boundary",
            }, provenance={"source": "hawking.workspace", "workspace_id": workspace_id(root)})
        except Exception as exc:
            return _R(False, error=f"workspace identity unavailable: {type(exc).__name__}")
    ...
    if name == "runtime.status":
        ...
        return _R(True, value={
            "schema": "hawking.runtime.status.v1",
            "status": str(health.get("status") or "unknown")[:80],
            "authority": str(health.get("authority") or "")[:120],
            "owner_pid": health.get("owner_pid"),
            "session_id": str(snapshot.get("session_id") or "")[:80],
            "requested_model": str(snapshot.get("requested_model") or "")[:160],
            "route_scope": str(snapshot.get("route_scope") or "")[:40],
            "auto_scope": str(snapshot.get("auto_scope") or "")[:40],
            "owner": "hawkingd",
        }, provenance={"source": "hawkingd.health"})

    if name == "capabilities.status":
        try:
            from .permissions import PermissionsService
            status = PermissionsService(workspace).status(refresh=True)
            capabilities = status.get("capabilities") if isinstance(status, Mapping) else {}
            return _R(True, value={
                "schema": "hawking.capabilities.status.v1",
                "capabilities": {
                    str(key): str(value.get("state") or "UNKNOWN")
                    for key, value in (capabilities or {}).items()
                    if isinstance(value, Mapping)
                },
                "checked_at": status.get("checked_at"),
                "fresh": status.get("fresh", True),
                "owner": "hawking.permissions",
            }, provenance={"source": "hawking.permissions.PermissionsService"})
        except Exception as exc:
            return _R(False, error=f"capability status unavailable: {type(exc).__name__}")

    if name in {"artifact.read", "artifact.search", "memory.search"}:
        if workspace is None:
            return _R(False, error=f"{name} needs a Hawking workspace")
        try:
            from .web_artifacts import memory_search, read as read_artifact
            if name == "artifact.read":
                artifact_id = str(arguments.get("artifact_id") or arguments.get("id") or "").strip()
                if not artifact_id:
                    return _R(False, error="artifact.read needs artifact_id")
                item = read_artifact(workspace, artifact_id)
                ref = dict(item["attachment"])
                text = str(item.get("text") or "")
                try:
                    limit = min(16000, max(1, int(arguments.get("max_chars", 12000))))
                except (TypeError, ValueError):
                    limit = 12000
                return _R(True, value={
                    "schema": "hawking.artifact.read.v1",
                    "artifact": ref,
                    "text": text[:limit],
                    "truncated": len(text) > limit,
                    "original_preserved": True,
                }, provenance={"source": "hawking.web_artifacts", "artifact_id": artifact_id})
            query = str(arguments.get("query") or "").strip()
            if not query:
                return _R(False, error=f"{name} needs query")
            session_id = str(arguments.get("session_id") or "").strip() or None
            try:
                limit = min(8, max(1, int(arguments.get("limit", 8))))
            except (TypeError, ValueError):
                limit = 8
            hits = memory_search(workspace, query, session_id=session_id, limit=limit)
            return _R(True, value={
                "schema": "hawking.memory.search.v1",
                "query": query[:500],
                "hits": hits,
                "workspace_scoped": True,
            }, provenance={"source": "hawking.web_artifacts.memory_search", "session_id": session_id})
        except Exception as exc:
            return _R(False, error=f"{name} unavailable: {type(exc).__name__}")
        ...
        try:
            if arguments.get("query"):
                ...
                return _R(True, value={"id": paste_id, "matches": [
                    ...
                    provenance={"source": "hawking.paste_cache", "paste": paste_id})
    ...
    if name == "debug.diagnose":
        ...
        return _R(True, value=failure.to_dict(),
                  provenance={"source": "hawking.capabilities", "paste": paste_id})
    ...
    if name == "reverse.identify":
        ...
        return _R("error" not in result, value=result,
                  ...
                  provenance={"source": "hawking.capabilities"})
...
BUILDER_SHAPES = {
    "repo.edit": (
        ...
        'atomically. The typed operation enum is {"op": '
        '"create|replace|replace_file|insert_before|insert_after|append"}. '
        'The authority layer implements all six forms, but the browser contract uses '
...
# Goal mutations have a stronger terminal than an ordinary browser edit.  The
# source change and its regression-test change are one proposal even when a
# small resident emits them over two repair turns.  This is deliberately a
# compact, serializable contract so the Web/owner boundary can carry it without
# giving the model another authority surface.
MUTATION_BUNDLE_SCHEMA = "hawking.mutation_bundle.v1"


def _test_file_from_reference(value: Any) -> str:
    """Return the repository path portion of a focused test reference."""
    text = str(value or "").strip()
    if not text:
        return ""
    # Accept the common ``path::node`` and ``path.py -k expr`` forms, but do
    # not invent a path from a free-form command.
    text = text.split("::", 1)[0].strip()
    if " -k " in text:
        text = text.split(" -k ", 1)[0].strip()
    return text


def _operation_identity(operation: Any) -> str:
    return json.dumps(operation, sort_keys=True, separators=(",", ":"), default=str)


def validate_mutation_bundle(
    ...
    """Validate and, when complete, assemble a Goal mutation bundle.

    This stage is intentionally before ``Engine.apply_typed_mutation``.  It
    validates *completeness* only; operation semantics, anchors, syntax,
    red-before-green, and rollback remain owned by the engine.  A pending
    source proposal is accepted only when the repair contributes the missing
    test-file operation and stays within the Goal's allowed files.
    """
    spec = dict(bundle or {})
    raw_pending = dict(pending or {})
    proposed = [item for item in (operations or ()) if isinstance(item, dict)]
    preserved = [item for item in (raw_pending.get("operations") or ())
                 if isinstance(item, dict)]
    if not preserved:
        preserved = [item for item in (raw_pending.get("source_operations") or ())
                     if isinstance(item, dict)]
    test_paths = {
        _test_file_from_reference(item)
        for item in (spec.get("test_mutation_paths") or
                     spec.get("focus_tests") or ())
        if _test_file_from_reference(item)
    }
    allowed = {
        str(item).strip()
        for item in (spec.get("allowed_files") or spec.get("focus_files") or ())
        if str(item).strip()
    }
    allowed.update(test_paths)

    def is_test_operation(operation: Mapping[str, Any]) -> bool:
        path = str(operation.get("path") or "").strip()
        if path in test_paths:
            return True
        # When a Goal omitted focus_tests, retain a conservative fallback for
        # conventional test filenames.  The explicit contract always wins.
        name = Path(path).name.lower()
        return name.startswith("test_") or name.endswith("_test.py")

    source_proposed = [item for item in proposed if not is_test_operation(item)]
    test_proposed = [item for item in proposed if is_test_operation(item)]
    ...
    # A repair may repeat the exact preserved source operation, but it may not
    # smuggle a different source file or a second behavior change through the
    # missing-test turn.  Exact duplicates are removed before assembly.
    if preserved:
        preserved_ids = {_operation_identity(item) for item in preserved}
        unexpected_source = [
            item for item in source_proposed
            if _operation_identity(item) not in preserved_ids
        ]
        if unexpected_source:
            disallowed.extend(unexpected_source)
        source_proposed = [
            item for item in source_proposed
            if _operation_identity(item) not in preserved_ids
        ]

    combined: List[Dict[str, Any]] = [*preserved, *source_proposed, *test_proposed]
    # Preserve order while dropping exact duplicates.  Order is part of the
    # engine's transaction and deterministic repair must not apply a source
    # twice merely because the body echoed it.
    ...
    source_operations = [item for item in unique if not is_test_operation(item)]
    test_operations = [item for item in unique if is_test_operation(item)]
    missing: List[str] = []
    if bool(spec.get("require_source_mutation", True)) and not source_operations:
        missing.append("MISSING_SOURCE_MUTATION")
    if bool(spec.get("require_regression_test_mutation", True)) and not test_operations:
        missing.append("MISSING_REQUIRED_TEST_OPERATION")
    if bool(spec.get("require_focused_test_invocation", True)) and not (
            isinstance(tests, (list, tuple)) and any(str(item).strip() for item in tests)):
        # A pending bundle may already carry the invocation from the first
        # proposal.  It is part of the preserved bundle, not a new model duty.
        if not (isinstance(raw_pending.get("tests"), list)
                and any(str(item).strip() for item in raw_pending.get("tests") or [])):
            missing.append("MISSING_FOCUSED_TEST_INVOCATION")
    if disallowed:
        missing.insert(0, "DISALLOWED_MUTATION_OPERATION")

    retained_tests = list(tests or ())
    if not retained_tests and isinstance(raw_pending.get("tests"), list):
        retained_tests = list(raw_pending.get("tests") or [])
    return {
        ...
        "test_mutation_paths": sorted(test_paths),
...
def run_builder_tool(name: str, arguments: Dict[str, Any], *,
                     engine: Any = None, require_tests: bool = False,
                     mutation_bundle: Optional[Mapping[str, Any]] = None,
                     bundle_state: Optional[Dict[str, Any]] = None) -> Any:
    ...
    class _R:
        def __init__(self, ok, value=None, error=None, provenance=None,
            ...
            self.ok, self.value, self.error = ok, value, error
            self.provenance = provenance or {}
            self.failure_class = failure_class or (None if ok else "INVALID_ARGUMENTS")
    ...
    if not isinstance(operations, list) or not operations:
        return _R(False, error=(
            "repo.edit `operations` must be a non-empty list. "
            f"{BUILDER_SHAPES['repo.edit']}"
        ))
    tests = arguments.get("tests")
    if isinstance(tests, str):
        tests = [tests]
    # Small residents sometimes express an append as a replace whose
    # old_lines and new_lines are the same complete test block.  That is not a
    # valid engine mutation (and must never be accepted as a no-op), but the
    # intent is unambiguous when it targets the contract's required regression
    # test file.  Normalize only this exact typed shape into an explicit
    # append; source operations and all other replace operations remain strict.
    bundle_syntax_repairs: List[str] = []
    if isinstance(mutation_bundle, Mapping):
        test_paths = {
            str(path) for path in (mutation_bundle.get("test_mutation_paths") or [])
            if str(path).strip()
        }
        normalized: List[Any] = []
        for operation in operations:
            if (
                ...
                and str(operation.get("path") or "") in test_paths
                and str(operation.get("op") or "") == "replace"
                and isinstance(operation.get("old_lines"), list)
                and isinstance(operation.get("new_lines"), list)
                and operation.get("old_lines") == operation.get("new_lines")
                and operation.get("new_lines")
            ):
                repaired = dict(operation)
                repaired.pop("old_lines", None)
                repaired["op"] = "append"
                bundle_syntax_repairs.append("test_replace_to_append")
    ...
    if mutation_bundle is not None:
        bundle_verdict = validate_mutation_bundle(
            operations,
            tests if isinstance(tests, list) else None,
            bundle=mutation_bundle,
            pending=bundle_state,
        )
        if not bundle_verdict.get("bundle_complete"):
            # Only valid source operations survive a deficiency cycle.  The
            # state is intentionally small and JSON-compatible so ChatSession
            # can persist it between HTTP requests as well as within one loop.
            if bundle_state is not None:
                bundle_state.clear()
                bundle_state.update({
                    "schema": MUTATION_BUNDLE_SCHEMA,
                    "operations": list(bundle_verdict.get("source_operations") or []),
                    "source_operations": list(bundle_verdict.get("source_operations") or []),
                    "tests": list(bundle_verdict.get("tests") or []),
                    "missing_requirements": list(bundle_verdict.get("missing_requirements") or []),
                    "allowed_files": list(bundle_verdict.get("allowed_files") or []),
                    "test_mutation_paths": list(bundle_verdict.get("test_mutation_paths") or []),
                })
            return _R(
                False,
                value=bundle_verdict,
                error="MISSING_REQUIRED_TEST_OPERATION" if "MISSING_REQUIRED_TEST_OPERATION" in (
                    bundle_verdict.get("missing_requirements") or []) else
                    ";".join(bundle_verdict.get("missing_requirements") or []) or
                    "MUTATION_BUNDLE_INCOMPLETE",
                provenance={"source": "hawking.mutation_bundle.validate",
                            "schema": MUTATION_BUNDLE_SCHEMA},
                failure_class="MUTATION_BUNDLE",
            )
        # The engine receives the assembled proposal, not a partial repair.
        operations = list(bundle_verdict.get("operations") or [])
        tests = list(bundle_verdict.get("tests") or [])
    ...
    mutation_request = arguments.get("mutation_request")
    if isinstance(mutation_request, Mapping):
        ...
        # Preserve the pre-typed-mutation Engine seam for existing test and
        # extension adapters.  A request is only omitted when the caller did
        # not provide one, so the canonical Engine still receives every typed
        # authority binding that exists.
        verdict = engine.apply_typed_mutation(
            operations,
            tests if isinstance(tests, list) else None,
        )
    if bundle_state is not None and mutation_bundle is not None:
        # A complete bundle has crossed into the engine.  On a semantic
        # rejection after a two-turn repair, retain the already-valid source
        # so the next turn can repair only the test operation; an initial
        # all-at-once proposal has no preserved source and therefore clears.
        if (str(verdict.get("status") or "").lower() == "rejected"
                and bundle_verdict.get("preserved_operations")):
            bundle_state.clear()
            bundle_state.update({
                "schema": MUTATION_BUNDLE_SCHEMA,
                "operations": list(bundle_verdict.get("preserved_operations") or []),
                "source_operations": list(bundle_verdict.get("preserved_operations") or []),
                "tests": list(bundle_verdict.get("tests") or []),
                "missing_requirements": ["MISSING_REQUIRED_TEST_OPERATION"],
                "allowed_files": list(bundle_verdict.get("allowed_files") or []),
                "test_mutation_paths": list(bundle_verdict.get("test_mutation_paths") or []),
            })
        else:
    ...
    if bundle_verdict is not None:
        verdict = {
            **verdict,
            ...
            "preserved_operations": list(bundle_verdict.get("preserved_operations") or []),
            "bundle_operations": list(bundle_verdict.get("operations") or []),
        }
        if bundle_syntax_repairs:
            verdict["syntax_repairs"] = list(dict.fromkeys(bundle_syntax_repairs))
    status = verdict.get("status")
    repair_hint = None
    if str(status).lower() == "rejected" and isinstance(operations, list):
        # A small resident often copies the compact shape's illustrative
        # anchor literally.  Return a bounded, real source anchor with the
        # rejection so the next attempt can repair itself without another
        # broad repository search or a U001 scratch fallback.
        try:
            first = operations[0] if operations else {}
            raw_path = str(first.get("path") or "") if isinstance(first, dict) else ""
            target = (Path(engine.root) / raw_path).resolve()
            target.relative_to(Path(engine.root).resolve())
            lines = target.read_text(encoding="utf-8").splitlines()
            marker = next((i for i, line in enumerate(lines)
                           if "def test_gate6_goal_surface_roundtrip" in line), None)
            if marker is None:
                marker = next((i for i, line in enumerate(lines)
                               if line.startswith("def test_")), None)
            if marker is not None:
                repair_hint = {
                    "path": raw_path,
                    "exact_anchor_lines": lines[marker:min(len(lines), marker + 6)],
                    "instruction": "copy one exact existing line; do not use this field as a placeholder",
                }
    ...
    return _R(True, value={
        ...
        "schema": (MUTATION_BUNDLE_SCHEMA if bundle_verdict is not None else None),
        "bundle_complete": (True if bundle_verdict is not None else None),
        "missing_requirements": ([] if bundle_verdict is not None else None),
        "preserved_operations": (
            list(bundle_verdict.get("preserved_operations") or [])
            if bundle_verdict is not None else []),
        "bundle_operations": (
            list(bundle_verdict.get("operations") or [])
            if bundle_verdict is not None else []),
        "syntax_repairs": list(dict.fromkeys(bundle_syntax_repairs)),
        "validation": verdict.get("validation"),
        "tests": list(tests or []),
        "diff": (verdict.get("diff") or "")[:2000],
        "paths": verdict.get("paths"),
        "repair_hint": repair_hint,
        "mutation_request": verdict.get("mutation_request"),
        "intent_status": verdict.get("intent_status"),
        "changed_files": verdict.get("changed_files") or [],
        "completion_packet": verdict.get("completion_packet"),
    }, provenance={"source": "hawking.engine.apply_typed_mutation",
                   "status": status})


def _declared_schema(name: str) -> Dict[str, Any]:
    """A JSON-Schema for a door HAWKING serves itself.
    ...
    if name == "artifact.read":
        return {"type": "object", "required": ["artifact_id"], "properties": {
            "artifact_id": {"type": "string"}, "max_chars": {"type": "integer"}}}
    if name in {"artifact.search", "memory.search"}:
