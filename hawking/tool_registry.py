...
filesystem, Git, public research, receipt, ModelLake, PERCEPTION, Doctor, Gravity,
...
from .persist import sha256_bytes as _sha256_bytes
...
TOOL_SCHEMA = "hawking.agentos.tool.v1"


# A restricted worker request must not lazily start Hawking's Rust helper just
# because an otherwise-read-only tool has an optional native fast path.  This
# is context-local rather than an environment toggle: one daemon can serve a
# normal session and a constrained research request concurrently without the
# latter changing global process policy.  The default preserves the existing
# normal-session behavior.
_NATIVE_HELPERS_ALLOWED: ContextVar[bool] = ContextVar(
    "hawking_native_helpers_allowed", default=True,
)
_NETWORK_CREDENTIALS_ALLOWED: ContextVar[bool] = ContextVar(
    "hawking_network_credentials_allowed", default=True,
)


def native_helpers_allowed() -> bool:
    """Whether this call may use an optional native helper child."""
    return bool(_NATIVE_HELPERS_ALLOWED.get())


def network_credentials_allowed() -> bool:
    """Whether a tool may attach ambient account credentials to a request."""
    return bool(_NETWORK_CREDENTIALS_ALLOWED.get())


@contextmanager
def suppress_native_helpers():
    """Prevent optional native helper process creation for one tool call."""
    token = _NATIVE_HELPERS_ALLOWED.set(False)
    try:
        yield
    finally:
        _NATIVE_HELPERS_ALLOWED.reset(token)


@contextmanager
def suppress_restricted_worker_capabilities():
    """Run one constrained-worker dispatch without child helpers or secrets.

    Public-research handlers normally use a local GitHub or Hugging Face token
    when one exists. A worker may request public research, but must never turn
    an ambient token into access to private/account-scoped material.
    """
    native_token = _NATIVE_HELPERS_ALLOWED.set(False)
    credential_token = _NETWORK_CREDENTIALS_ALLOWED.set(False)
    try:
        yield
    finally:
        _NETWORK_CREDENTIALS_ALLOWED.reset(credential_token)
        _NATIVE_HELPERS_ALLOWED.reset(native_token)
...
_SAFE_SHELL_COMMANDS = frozenset(
    {
        ...
        # through this tool before it was removed. hawking/delegate.py's
...
class ToolContext:
    ...
    # Authority is an immutable request-local projection supplied by the
    # enclosing Hawking Goal.  Tool handlers may narrow from it, never create
    # or widen it.  Keeping it here avoids trusting model-supplied authority
    # fields in action arguments.
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "authority", dict(self.authority) if isinstance(self.authority, Mapping) else {})
    ...
    def resolve_write_path(self, raw: Any) -> Path:
        ...
        if relative_parts.intersection({".git", ".hawking"}):
...
class ToolSpec:
    ...
    provenance: str = "hawking.tool_registry"
...
class ToolResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "schema": "hawking.agentos.tool.result.v1",
            "tool": self.tool,
...
class ToolRegistry:
    ...
    def describe(self, focus: str = "", *, max_results: int = 12) -> Dict[str, Any]:
        ...
        return {
            ...
            "focus": query,
            "provenance": "hawking.tool_registry.ToolRegistry.describe",
        }
    ...
    def invoke(self, name: str, arguments: Optional[Mapping[str, Any]] = None) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                ...
                failure_class=str(getattr(exc, "code", "") or type(exc).__name__), mutation=spec.mutation,
                deterministic=spec.deterministic,
                provenance={"source": spec.provenance},
                started_at=started, finished_at=time.time(),
                started_ns=started_ns, finished_ns=now_ns(),
...
def _read_bounded_bytes(path: Path, *, maximum: int, label: str) -> bytes:
    """Read at most ``maximum`` bytes, rejecting a growing/oversized file.

    Checking ``stat`` alone is not a bound: a writer can grow a file between
    that check and ``read``. The extra byte makes the post-open condition
    explicit and prevents a read-only worker from turning a large artifact
    into an unbounded allocation before output clipping happens.
    """
    try:
    ...
    if size > maximum:
        raise ValueError(
            f"{label} is {size} bytes, above the bounded read limit of {maximum} bytes"
        )
    try:
        with path.open("rb") as handle:
    ...
    if len(data) > maximum:
        raise ValueError(
            f"{label} grew beyond the bounded read limit of {maximum} bytes"
        )
    return data
...
_FOCUS_TERM_ALIASES: Dict[str, Tuple[str, ...]] = {
    ...
    # A live P0 asked for "HAWKING/AgentOS" and received zero catalog matches even
    ...
    "hawking": ("tool", "repository", "receipt", "process", "campaign"),
    ...
    # Security-focused retrieval should prefer the read-only shell primitive
    # over the reversible shell executor when the catalog is bounded.  The
    # explicit term keeps `shell.readonly` in the actionable window instead
    # of allowing the lexicographic tie with `shell.exec` to hide it.
    "security": ("audit", "capability", "claim", "shell", "readonly", "git", "tests"),
...
def _read_file(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if path.is_dir():
        # NOT FileNotFoundError. Measured: the model listed `hawking`, called
    ...
    if not path.is_file():
        ...
        # identical FileNotFoundError on hawking/tests/test_tool_registry.py in
    ...
    if (
        ...
        and os.environ.get("HAWKING_NATIVE_GRAVITY_READ") == "1"
        ...
        if native is not None:
            ...
            try:
                from .compact_worker import create_source_anchor
                native["source_anchor"] = create_source_anchor(
                    context.repo_root, path,
                    start_line=int(native.get("start_line") or start or 1),
                    end_line=int(native.get("end_line") or end or native.get("total_lines") or 1),
                ).to_dict()
    ...
    if line_window:
        raw = _read_bounded_bytes(
            path, maximum=_MAX_READ_BYTES, label="line-window file"
        )
        ...
        # A successful bounded source read creates a durable reference.  The
        # provider receives the anchor identity; Hawking retains the exact
        # source slice and verifies it again before compiling a mutation.
        try:
            from .compact_worker import create_source_anchor
            anchor = create_source_anchor(
                context.repo_root, path, start_line=first, end_line=last,
            )
            window["source_anchor"] = anchor.to_dict()
        except (OSError, UnicodeError, ValueError):
            # Anchor creation is an optimization of the read surface, never a
            # reason to turn an otherwise valid observation into a failure.
            pass
        if out_of_range:
    ...
    # The ordinary head read needs no whole-file allocation. Preserve a full
    # digest only when we actually read every byte; a prefix digest must never
    # masquerade as evidence for a multi-gigabyte artifact.
    if total_bytes <= _MAX_READ_BYTES:
        raw = _read_bounded_bytes(path, maximum=_MAX_READ_BYTES, label="file")
        clipped = raw[:limit]
        full_sha256: Optional[str] = _sha256_bytes(raw)
        sha256_scope = "full_file"
    else:
        try:
            with path.open("rb") as handle:
                clipped = handle.read(limit)
        except OSError as exc:
            raise FileNotFoundError(path) from exc
        full_sha256 = None
        sha256_scope = "not_computed_file_exceeds_bounded_read_limit"
    result = {
        "path": str(path),
        "bytes": total_bytes,
        **_truncation_fields(len(clipped), total_bytes),
        "sha256": full_sha256,
    ...
    if not result.get("truncated") and full_sha256:
        try:
            from .compact_worker import create_source_anchor
            line_count = len(raw.decode(encoding, errors="replace").splitlines())
            result["source_anchor"] = create_source_anchor(
                context.repo_root, path, start_line=1, end_line=max(1, line_count),
            ).to_dict()
        except (OSError, UnicodeError, ValueError):
            pass
    return result
...
_SEARCH_SKIP_DIRS = {
    ...
    "target", ".worktrees", ".pytest_cache", ".mypy_cache", "build", "dist",
    "workspace",
}
...
# A corpus is an intentional search domain, not a hidden access restriction.
# `all-authorized` keeps the compatibility behavior; the named partitions keep
# routine repository questions out of generated campaign output while still
# returning an explicit expansion route.
SEARCH_CORPORA = (
    "source", "tests", "design", "evidence", "campaign", "dependencies",
    "all-authorized",
)

_CORPUS_ROOTS = {
    "source": ("hawking", "crates", "tools", "app", "native", "visionmcp"),
    "tests": ("hawking", "crates", "tools", "app", "tests"),
    "design": ("docs", "civilization", "AGENTS.md", "H-MANIFESTO.md", "CLAUDE-CODEX-EXODUS.md", "README.md"),
    "evidence": ("receipts", ".hawking/receipts", "workspace/evidence"),
    "campaign": ("workspace/campaign", "tools/odyssey", "workspace/ops", "campaign"),
    "dependencies": ("Cargo.toml", "Cargo.lock", "pyproject.toml", "requirements.txt", "package.json", "package-lock.json", "workspace/vendor", "vendor"),
}

_CORPUS_EXCLUDED = {
    "source": ("generated receipts/campaign evidence", "tests unless explicitly selected"),
    "tests": ("production source outside test files", "generated receipts/campaign evidence"),
    "design": ("source and generated artifacts"),
    "evidence": ("source and executable artifacts"),
    "campaign": ("production source outside campaign tooling"),
    "dependencies": ("installed environments and model payloads"),
    "all-authorized": (),
}
...
def _is_test_path(path: Path) -> bool:
    ...
    return (
        ...
        or lower.endswith(("_test.py", ".test.js", ".spec.js", ".test.ts", ".spec.ts"))
...
def _corpus_roots(
    ...
    *,
    ...
    """Resolve named roots without removing the caller's authorized read scope."""
    repo = Path(getattr(context, "repo_root", requested_root)).expanduser().resolve()
    if corpus == "all-authorized" or not explicit_root:
        if corpus == "all-authorized":
            return [requested_root], []
        roots: List[Path] = []
        for raw in _CORPUS_ROOTS.get(corpus, ()):
            candidate = repo / raw
            if candidate.exists():
                roots.append(candidate)
        # A temporary fixture or a nonstandard project has no named roots;
        # retain the normal caller root rather than manufacturing no results.
        if not roots and requested_root.exists():
            roots = [requested_root]
        return roots, list(_CORPUS_EXCLUDED.get(corpus, ()))
    # An explicit sub-root is a deliberate narrowing.  Keep it even when its
    # name is not in the standard corpus map, then apply the corpus predicate
    # below; this preserves `path=one-known-file` and fixture behavior.
    return [requested_root], list(_CORPUS_EXCLUDED.get(corpus, ()))


def _scope_revision(
    paths: Iterable[Tuple[Path, int, int, Optional[str]]], corpus: str
) -> str:
    digest = hashlib.sha256()
    digest.update(corpus.encode("utf-8"))
    for path, size, mtime_ns, content_hash in sorted(paths, key=lambda item: str(item[0])):
        digest.update(str(path).encode("utf-8", "surrogateescape"))
        digest.update(f"\0{size}\0{mtime_ns}".encode("ascii"))
        # Metadata is useful for a cheap scope hint, but it is not an artifact
        # identity: a same-sized replacement can preserve both size and mtime.
        # Include the bounded bytes read for this search whenever available;
        # an unreadable file is explicitly distinct from a verified digest.
        digest.update(b"\0content:")
        digest.update((content_hash or "unavailable").encode("ascii"))
    return f"fs-scope-v1:{digest.hexdigest()}"


def _search_files(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    # `path` remains a compatibility alias for `root`, while `corpus` makes
    # broad repository search intentional.  A plain repo query defaults to
    # source; explicit `corpus=all-authorized` is the expansion route for
    # generated evidence and campaign material.
    raw_location = args.get("root") if args.get("root") is not None else args.get("path")
    raw_location = raw_location if raw_location not in (None, "") else "."
    requested = context.resolve_read_path(raw_location)
    explicit_root = str(raw_location).strip() not in {"", "."}
    single_file: Optional[Path] = requested if requested.is_file() else None
    if single_file is not None:
        requested = single_file.parent
    elif not requested.is_dir():
        raise NotADirectoryError(requested)
    needle = str(args.get("pattern") or "")
    if not needle:
        raise ValueError("pattern is required")
    has_repo = getattr(context, "repo_root", None) is not None
    corpus = str(args.get("corpus") or ("source" if has_repo else "all-authorized")).strip().casefold()
    if corpus not in SEARCH_CORPORA:
        raise ValueError(f"corpus must be one of: {', '.join(SEARCH_CORPORA)}")
    ...
    max_per_file = max(1, min(1000, int(max_per_file_raw))) if max_per_file_raw is not None else None
    roots, corpus_excluded = _corpus_roots(
        context,
        requested,
        corpus,
    ...
    if single_file is not None:
        roots = [requested]
    roots = [Path(root).expanduser().resolve() for root in roots if Path(root).exists()]
    if not roots:
        roots = [requested]
    repo = Path(getattr(context, "repo_root", requested)).expanduser().resolve()
    excluded = list(corpus_excluded) + [
        f"directories:{','.join(sorted(_SEARCH_SKIP_DIRS))}",
        f"suffixes:{','.join(sorted(_SEARCH_SKIP_SUFFIXES))}",
    ]
    expansion_route: Dict[str, Any] = {
        "next_corpus": "all-authorized" if corpus != "all-authorized" else None,
        "query": {"corpus": "all-authorized", "root": str(requested)},
    }

    def glob_matches(path: Path, base: Path) -> bool:
        relative = Path(os.path.relpath(path, base))
        if relative.match(glob) or Path(path.name).match(glob):
            return True
        return glob.startswith("**/") and Path(path.name).match(glob[3:])

    def corpus_accept(path: Path) -> bool:
        if single_file is not None:
            return path.resolve() == single_file.resolve()
        if corpus == "tests":
    ...
    # Native search stays available only for the unpartitioned scope.  Named
    # corpora must carry their own searched-root and exclusion proof.
    native = None
    if corpus == "all-authorized" and len(roots) == 1 and roots[0].is_dir() and native_helpers_allowed():
        try:
            from .repo_context import native_filesystem_search
            native = native_filesystem_search(
                context.repo_root, roots[0], needle=needle, glob=glob,
    ...
    if native is not None:
        ...
        for row in native.get("matches") or []:
            ...
            matches.append(item)
        native["root"] = str(requested)
        native["pattern"] = needle
        native["corpus"] = corpus
        native["searched_roots"] = [str(root) for root in roots]
        native["excluded_source_kinds"] = excluded
        native["expansion_route"] = expansion_route
        native_revision = native.get("index_revision")
        if not isinstance(native_revision, str) or not native_revision:
            # Older resident Gravity helpers expose a monotonic coherent
            # generation but not the newer content-derived revision. Preserve
            # that explicit native revision rather than falling back to a root
            # mtime/size stamp, which is not a durable artifact identity.
            generation = native.get("generation")
            native["index_revision"] = (
                f"gravity-generation-v1:{generation}"
                if isinstance(generation, int) and not isinstance(generation, bool)
                else "gravity-generation-v1:unavailable"
            )
            native["index_revision_source"] = "resident-generation"
    ...
    files_seen = 0
    skipped_large = 0
    revision_rows: List[Tuple[Path, int, int, Optional[str]]] = []

    def result(truncated: bool) -> Dict[str, Any]:
        return {
            "root": str(requested),
            "pattern": needle,
            "corpus": corpus,
            ...
            "expansion_route": expansion_route,
            "index_revision": _scope_revision(revision_rows, corpus),
            "matches": matches,
    ...
    for base in roots:
        if base.is_file():
            walk_items = ((base.parent, [base]),)
        else:
            def _walk_items() -> Iterable[Tuple[Path, List[Path]]]:
                for dirpath, dirnames, filenames in os.walk(base):
                    dirnames[:] = sorted(name for name in dirnames if name not in _SEARCH_SKIP_DIRS)
                    yield Path(dirpath), [Path(dirpath) / filename for filename in sorted(filenames)]
            walk_items = _walk_items()
        for dirpath, file_paths in walk_items:
            for candidate in file_paths:
                if not glob_matches(candidate, base) or not corpus_accept(candidate):
                    continue
                try:
                ...
                try:
                    ...
                    revision_rows.append((path, int(stat.st_size), int(stat.st_mtime_ns), None))
                    continue
                content_hash = hashlib.sha256(raw).hexdigest()
                revision_rows.append(
                    (path, int(stat.st_size), int(stat.st_mtime_ns), content_hash)
                )
...
def _list_files(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for name in sorted(dirnames):
            if name in {".git", ".venv", "__pycache__", "node_modules"}:
                continue
            try:
                # Do not advertise a directory symlink that escapes the
                # caller's read roots. os.walk will not descend it, but its
                # name alone would still be an unbounded external discovery
                # surface for a restricted worker.
                context.resolve_read_path(Path(dirpath) / name)
            except (OSError, PermissionError, ValueError):
                continue
            if not _within(Path(dirpath) / name, (root,)):
                continue
        ...
        for filename in sorted(filenames):
            ...
            if len(entries) >= limit:
                ...
                continue
            try:
                path = context.resolve_read_path(Path(dirpath) / filename)
            except (OSError, PermissionError, ValueError):
                continue
...
def _fetch(
    ...
    *,
    ...
    request_headers = {"User-Agent": "hawking-agentos-research/1"}
    ...
    handlers: List[Any] = [_CheckedRedirectHandler()]
    if not network_credentials_allowed():
        # Default urllib discovery imports HTTP(S)_PROXY / ALL_PROXY, including
        # proxy credentials. Constrained research is public and unauthenticated:
        # fail closed when direct egress is unavailable rather than borrowing
        # an ambient account/proxy route.
...
def _github_search(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    token = (
        ...
        if network_credentials_allowed() else None
    )
    ...
    return {
        ...
        "retrieved_at": fetched.get("provenance", {}).get("retrieved_at"),
        "auth_available": bool(
            token or (network_credentials_allowed() and shutil.which("gh"))
        ),
...
def _huggingface_resolve(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    token = (
        os.environ.get("HF_TOKEN") or os.environ.get("HF_ACCESS_TOKEN")
        if network_credentials_allowed() else None
    )
...
def _model_lake_roots(context: ToolContext) -> Tuple[Path, ...]:
    configured = os.environ.get("HAWKING_MODEL_LAKE_ROOT")
...
def _resolve_download_path(context: ToolContext, raw: Any) -> Path:
    ...
    if _within(candidate, (context.workspace,)):
        try:
            ...
            if set(relative.parts).intersection({".git", ".hawking"}):
...
def _huggingface_download(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    headers = {"User-Agent": "hawking-agentos-research/1"}
...
def _receipt_read(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not candidate.is_absolute() and (
        ...
        path = context.resolve_read_path(context.repo_root / candidate)
    else:
        path = context.resolve_read_path(raw)
    allowed = (
        context.repo_root / "receipts",
        context.workspace / ".hawking" / "receipts",
        context.workspace / ".hawking" / "mission",
        # ``roadmap.read`` is a named, fixed evidence target rather than a
        # broad grant to civilization/. Keep the one durable roadmap readable
        # without making adjacent project state an arbitrary receipt namespace.
        context.repo_root / "civilization" / "ROADMAP_STATE.json",
    )
    if not _within(path, allowed):
        raise PermissionError(
            "receipt path must be under receipts, .hawking state, or the persisted roadmap"
        )
    if not path.is_file():
        raise FileNotFoundError(path)
    raw = _read_bounded_bytes(
        path,
        maximum=_text_limit(args.get("max_bytes")),
        label="receipt",
    )
    try:
...
def _target_receipt(name: str) -> Callable[[ToolContext, Dict[str, Any]], Dict[str, Any]]:
    def handler(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
        return _receipt_read(
            context,
            {**args, "path": args.get("path") or _RECEIPT_TARGETS[name]},
        )
    return handler
...
def _list_tests(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if raw is None or not str(raw).strip():
        root = context.resolve_read_path(context.repo_root / "hawking/tests")
    else:
    ...
    for path in root.rglob("test_*.py"):
        if len(found) >= limit:
            ...
            break
        try:
            safe_path = context.resolve_read_path(path)
        except (OSError, PermissionError, ValueError):
            continue
        if _within(safe_path, (root,)) and safe_path.is_file():
            found.append(str(safe_path))
    found.sort()
    return {
        "root": str(root),
        "count": len(found),
...
def _filesystem_write(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # `filesystem.write` and `repo.edit` have different compatibility names,
    # but they now share the same Hawking-owned precondition/intent/atomic
    # executor.  The registry remains the permission boundary; this handler
    # only adapts its expected-content shape into the typed request.
    from .mutation import MutationExecutor, build_mutation_request

    base_root = context.workspace
    if not _within(path, (base_root,)) and _within(path, (context.repo_root,)):
        base_root = context.repo_root
    try:
        relative = str(path.relative_to(base_root)).replace(os.sep, "/")
    except ValueError as exc:
        raise PermissionError(f"write target is outside the canonical root: {path}") from exc
    metadata = dict(args.get("mutation_request") or {}) if isinstance(args.get("mutation_request"), Mapping) else {}
    metadata.setdefault("request_id", args.get("request_id"))
    metadata.setdefault("goal_id", args.get("goal_id"))
    metadata.setdefault("workunit_id", args.get("workunit_id"))
    metadata.setdefault("worker_attempt_id", args.get("worker_attempt_id"))
    metadata.setdefault("worktree_id", args.get("worktree_id"))
    metadata.setdefault("lease_id", args.get("lease_id"))
    expected = dict(metadata.get("expected_base_hashes") or {})
    if args.get("expected_base_sha256") is not None:
        expected[relative] = args.get("expected_base_sha256")
    metadata["expected_base_hashes"] = expected
    operation = {
        "op": "create" if not path.exists() else "replace_file",
        "path": relative,
        "new_text": content,
    }
    ...
    return MutationExecutor(base_root).execute_file(
        request,
        path,
        content,
        overwrite=overwrite,
...
def _perception_inspect(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    from .perception_adapter import inspect_perception

    return inspect_perception(context.repo_root, profile=str(args.get("profile") or "core"))
...
def _read_continuation(context: ToolContext) -> Optional[Dict[str, Any]]:
    """Read the fixed continuation only after the normal read-root check.

    ``campaign.state`` is a convenience assembler, not a privilege to follow
    a symlink out of the checkout.  Its fixed filenames still need the same
    containment rule as caller-provided fs.read paths.
    """
    try:
        path = context.resolve_read_path(context.repo_root / CONTINUATION)
    except PermissionError as exc:
        return {"unreadable": f"{type(exc).__name__}: {exc}"}
    if not path.is_file():
        return None
    try:
        return json.loads(_read_bounded_bytes(
            path, maximum=_MAX_READ_BYTES, label="campaign continuation"
        ))
...
def _campaign_state(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {}
    sovereign_rel = "civilization/sovereign-goal.txt"
    sovereign: Optional[Dict[str, Any]] = None
    try:
        sovereign_path = context.resolve_read_path(root / sovereign_rel)
    except PermissionError as exc:
        sovereign = {
            "path": sovereign_rel,
        ...
        sovereign_path = None
    if sovereign_path is not None and sovereign_path.is_file():
        try:
            sovereign_text = _read_bounded_bytes(
                sovereign_path, maximum=_MAX_READ_BYTES,
                label="sovereign campaign state",
            ).decode("utf-8")
    ...
    ledger_rel = "receipts/future/G034_ODYSSEY_LEDGER.json"
    try:
        led = context.resolve_read_path(root / ledger_rel)
    except PermissionError:
        led = None
    axes: Dict[str, Any] = {}
    owed_depth: List[str] = []
    if led is not None and led.is_file():
        try:
            doc = json.loads(_read_bounded_bytes(
                led, maximum=_MAX_READ_BYTES, label="Odyssey ledger"
            ))
    ...
    try:
        rdir = context.resolve_read_path(root / "receipts" / "future")
    except PermissionError:
        rdir = None
    if rdir is not None and rdir.is_dir():
        newest: List[Tuple[float, Path]] = []
        for candidate in rdir.glob("*.json"):
            try:
                # Do the containment check before stat(): stat follows a
                # file symlink and used to expose outside metadata here.
                q = context.resolve_read_path(candidate)
                if not _within(q, (rdir,)):
                    continue
                newest.append((q.stat().st_mtime, q))
            except (OSError, PermissionError, ValueError):
                continue
        for mtime, q in sorted(newest, key=lambda row: row[0], reverse=True)[:6]:
    ...
    cont = _read_continuation(context)
    ...
    out = {
        ...
        "ledger": ledger_rel if led is not None and led.is_file() else None,
...
def _perception_capabilities(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    """The LIVE perception surface, not a frozen receipt.

    This verb used to read receipts/headless/PERCEPTION_CAPABILITY_SURFACE.json, a
    ...
    return inspect_perception(context.repo_root)


def _perception_tools(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    """Which Hawking perception tools can run, and why the rest are refused.

    Hawking exposes a typed local read/evidence surface.
    Without this, a resident holding `perception` has a door and no idea what is
    ...
    """
    from .perception_adapter import PERCEPTION_READ_ONLY_TOOLS

    callable_names = sorted(PERCEPTION_READ_ONLY_TOOLS)
    return {
        "callable": callable_names,
        "n_callable": len(callable_names),
        "how": "perception op=query tool=<name> arguments={...}",
        "refused_mechanism": (
            "everything outside this list raises PermissionError from "
            "hawking/perception_adapter.py::call_perception -- Hawking does not "
            "expose an untyped escape hatch; mutations and experimental tools "
            "stay behind Hawking's governed interfaces"),
        "reopen_when": (
            "a specific Hawking perception tool earns a read-only case and is added to "
            "PERCEPTION_READ_ONLY_TOOLS; the allowlist is the mechanism, not a bug"),
    }


def _perception_query(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    from .perception_adapter import call_perception
...
def _architecture_inspect(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(native, Mapping) and isinstance(native.get("plan"), Mapping):
        ...
        if isinstance(graph, Mapping):
            plan = dict(native["plan"])
            # Older already-built gravityd binaries emitted the temporary hcli
            # namespace. The public Python/Rust contract is Hawking-owned; map
            # that legacy wire label at the process boundary until every local
            # binary has been rebuilt from the corrected Rust constant.
            if plan.get("schema") == "hcli.physical_graph.mega_kernel.v1":
                plan["schema"] = "hawking.physical_graph.mega_kernel.v1"
            graph["native_mega_kernel"] = plan
...
def _load_architecture_atlas_for_context(context: ToolContext) -> Optional[Mapping[str, Any]]:
    ...
    HAWKING snapshots intentionally package only the runtime package, not the
    ...
    # Prefer the source-owned atlas builder whenever it is available. A stored
    # receipt is evidence of a prior build, not authority to override the
    # current compiler vocabulary; otherwise the same checkout can project two
    # different fingerprints depending on which entry point is used.
    try:
...
def _doctor_query(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result: Dict[str, Any] = {
        "schema": "hawking.doctor.query.v1",
        "operation": operation,
        "status": "PROPOSED",
...
def _gravity_experiment(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    result: Dict[str, Any] = {
        "schema": "hawking.gravity.experiment.v1",
        "status": "HYPOTHESIS",
    ...
    if args.get("execute"):
        from hawking.agentos.flash_representation_experiment import (
        ...
        result["producer"] = "hawking.agentos.flash_representation_experiment.run_flash_representation_experiment"
...
def _accelerator_benchmark(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result: Dict[str, Any] = {
        "schema": "hawking.accelerator.benchmark.v1",
        "status": "RECEIPT_ONLY",
        "physical_execution": False,
...
def _future(name: str):
    ...
    campaign_memory_guard -- and registering under a private `_hawking_future_` key means
    ...
    The repo root stays off: that would expose `hawking`, `tools`, `receipts` and the rest.
...
def _odyssey_ledger(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    state HAWKING needs to choose what to work on next without a human naming it.
    """
...
def _acquisition_propose(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Avoid retaining a stale package attribute in long-lived workers. The
    # import system is the authority for the currently admitted module.
    import importlib
    acquisition = importlib.import_module("hawking.acquisition")
...
def _processes_list(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    hawking/agentos/resident.py (_owned_signal), which checks a
...
def _processes_orphaned(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    hawking/runtime.py._reap_orphans_once, not something the model can trigger
...
def _browser_world(context: ToolContext, args: Dict[str, Any], operation: str) -> Dict[str, Any]:
    """Run one typed browser capability through Hawking's WorldState owner.

    The local Node sidecar supplies Playwright mechanics only.  It receives no
    Goal authority, permission grants, or receipt writer; this registry has
    already made the only admissibility decision before reaching the adapter.
    """
    from .browser_world import browser_tool

    return browser_tool(context.workspace, operation, args)


def _macos_world(context: ToolContext, args: Dict[str, Any], operation: str) -> Dict[str, Any]:
    """Run Hawking's scoped macOS WorldState/action capability."""
    from .macos_world import macos_tool

    authority = context.authority.get("macos") if isinstance(context.authority, Mapping) else None
    return macos_tool(context.workspace, operation, args, authority=authority)


def _permissions(context: ToolContext, args: Dict[str, Any], operation: str) -> Dict[str, Any]:
    """Expose the canonical machine broker without giving workers OS authority."""
    from .permissions import PermissionsService

    service = PermissionsService(context.workspace)
    if operation == "status":
        required = args.get("capabilities") if isinstance(args.get("capabilities"), list) else None
        external = args.get("external_roots") if isinstance(args.get("external_roots"), list) else None
    ...
    if operation == "require":
        required = args.get("capabilities") if isinstance(args.get("capabilities"), list) else []
        external = args.get("external_roots") if isinstance(args.get("external_roots"), list) else None
        return service.require(required, external_roots=external)
    if operation == "explain":
        return service.explain(args.get("capability"))
    raise ValueError(f"unknown permissions operation: {operation}")
...
_PRIOR_SOURCES = (
    ...
    ("hawking_ledger", "workspace/campaign/odyssey/HAWKING_LEDGER.json", "laws"),
    ("hawking_scars", "workspace/campaign/odyssey/HAWKING_LEDGER.json", "scars"),
)
...
def _odyssey_priors(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    HAWKING has had odyssey.record_law and odyssey.record_scar -- two WRITE doors --
    and no way to read either back. A memory you can only write to is not a
    memory, and its ledger (workspace/campaign/odyssey/HAWKING_LEDGER.json) had never
    ...
    for source, rel, key in _PRIOR_SOURCES:
        try:
            # These paths are fixed by the campaign, but a fixed repository
            # filename can still be replaced by a symlink. Keep a constrained
            # worker on the same resolved read boundary as fs.read.
            path = context.resolve_read_path(context.repo_root / rel)
        except PermissionError:
            missing.append(f"{rel} (outside read roots)")
            continue
        if not path.is_file():
            missing.append(rel)
            continue
        try:
            doc = json.loads(_read_bounded_bytes(
                path, maximum=_MAX_READ_BYTES, label=f"Odyssey prior source {rel}"
            ))
        except Exception as exc:
...
# ---------------------------------------------------------------------------
# HAWKING AS ADVERSARIAL AUDITOR. [S002]
#
...
def _odyssey_attack_law(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    that writes the laws. It does now: HAWKING records a law through
...
def _round_receipt_dir(context: ToolContext) -> Path:
    return context.repo_root / ".hawking" / "receipts"
...
def _physical_rounds(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # example, a failed/closed HAWKING turn). Limiting before filtering made the
...
def _physical_measure(context: ToolContext, args: Dict[str, Any]) -> Dict[str, Any]:
    """Refuse the retired source-specimen measurement bridge.

    ModelLake/MLX direct measurement was a historical research path.  Keeping
    the tool name readable lets old packets fail explicitly, while the live
    physical evidence path remains ``physical.emit`` over an already-running
    Hawking-native resident.
    """
    del context, args
    return {
        "refused": (
            "direct source-specimen measurement is retired; use physical.emit "
            "for a receipt produced by a Hawking-native resident"
        ),
...
CONSOLIDATION: Dict[str, Dict[str, Any]] = {
    ...
    "perception": {
        "summary": ("Hawking perception. `tools` says what is callable and what is refused, "
                    "and `query` calls one typed local evidence tool."),
        "ops": {"tools": "perception.tools", "capabilities": "perception.capabilities",
                "inspect": "perception.inspect", "query": "perception.query"},
    },
    "web": {
...
def default_tool_registry(
    ...
    *,
    ...
    permissions_handler = lambda operation: (
        lambda tool_context, args: _permissions(tool_context, args, operation)
    )
    registry.register(ToolSpec(
        "permissions.status",
        "Freshly observe Hawking machine permissions and logical workspace capabilities; cached state is diagnostic only.",
        {
            "type": "object",
            "additionalProperties": False,
            "properties": {
                "capabilities": {"type": "array", "items": {"type": "string"}},
                "external_roots": {"type": "array", "items": {"type": "string"}},
                "refresh": {"type": "boolean"},
            },
        },
        resources=("filesystem",), deterministic=False,
        handler=permissions_handler("status"),
    ))
    registry.register(ToolSpec(
        "permissions.require",
        "Preflight an exact Goal capability set before cognition; reports PERMISSION_BLOCKED without starting a provider loop.",
        {
            "type": "object",
        ...
        resources=("filesystem",), deterministic=False,
        handler=permissions_handler("require"),
    ))
    registry.register(ToolSpec(
        "permissions.explain",
        "Explain one missing machine or logical capability and give the operator's next action.",
        {
            "type": "object",
            "required": ["capability"],
            "additionalProperties": False,
        ...
        resources=("filesystem",), handler=permissions_handler("explain"),
    ))
    registry.register(ToolSpec(
        "tools.catalog",
    ...
    registry.register(ToolSpec(
        ...
        {"type": "object", "required": ["pattern"], "additionalProperties": False,
         "properties": {"pattern": {"type": "string"}, "root": {"type": "string"}, "path": {"type": "string"}, "glob": {"type": "string"}, "max_results": {"type": "integer"}, "max_per_file": {"type": "integer"}, "corpus": {"type": "string", "enum": list(SEARCH_CORPORA)}}},
    ...
    structural_scope_schema = {
        "type": "string",
        "description": "authorized source directory or one known source file; defaults to workspace",
    }
    registry.register(ToolSpec(
        "source.owner",
        "Rank the current source owner for a question using bounded structural and content evidence.",
        {"type": "object", "required": ["query"], "additionalProperties": False,
         "properties": {"query": {"type": "string"}, "scope": structural_scope_schema,
                        "max_results": {"type": "integer"}, "max_files": {"type": "integer"}}},
        resources=("filesystem",), handler=source_owner,
    ))
    registry.register(ToolSpec(
        "source.outline",
        "Return bounded definitions and calls for one known source file without returning its whole body.",
        {"type": "object", "required": ["path"], "additionalProperties": False,
         "properties": {"path": {"type": "string"}}},
        resources=("filesystem",), handler=source_outline,
    ))
    registry.register(ToolSpec(
        "source.symbol",
        "Find bounded structural definitions for a symbol inside an authorized source scope.",
        {"type": "object", "required": ["name"], "additionalProperties": False,
         "properties": {"name": {"type": "string"}, "scope": structural_scope_schema,
                        "max_results": {"type": "integer"}, "max_files": {"type": "integer"}}},
        resources=("filesystem",), handler=source_symbol,
    ))
    registry.register(ToolSpec(
        "source.references",
        "Find bounded definition/reference lines for one identifier; carries a content-scoped revision.",
        {"type": "object", "required": ["symbol"], "additionalProperties": False,
         "properties": {"symbol": {"type": "string"}, "scope": structural_scope_schema,
                        "max_results": {"type": "integer"}, "max_files": {"type": "integer"}}},
        resources=("filesystem",), handler=source_references,
    ))
    registry.register(ToolSpec(
        "source.ast_query",
        "Run a safe bounded structural query: symbol:name, function:name, class:name, or call:name.",
        {"type": "object", "required": ["pattern"], "additionalProperties": False,
         "properties": {"pattern": {"type": "string"}, "scope": structural_scope_schema,
                        "max_results": {"type": "integer"}, "max_files": {"type": "integer"}}},
        resources=("filesystem",), handler=source_ast_query,
    ))
    registry.register(ToolSpec(
        "source.affected_tests",
        "Find likely test files affected by named source paths using bounded dependency evidence.",
        {"type": "object", "required": ["paths"], "additionalProperties": False,
         "properties": {"paths": {"type": "array", "items": {"type": "string"}},
                        "scope": structural_scope_schema, "max_results": {"type": "integer"},
                        "max_files": {"type": "integer"}}},
        resources=("filesystem",), handler=source_affected_tests,
    ))
    registry.register(ToolSpec(
        "source.rewrite_preview",
        "Preview a literal anchored rewrite and its diff; this capability never writes.",
        {"type": "object", "required": ["pattern"], "additionalProperties": False,
         "properties": {"pattern": {"type": "string"}, "replacement": {"type": "string"},
                        "scope": structural_scope_schema, "max_results": {"type": "integer"},
                        "max_files": {"type": "integer"}}},
        resources=("filesystem",), handler=source_rewrite_preview,
    ))
    ...
    registry.register(ToolSpec(
        ...
        {"type": "object", "required": ["path", "content"], "additionalProperties": False,
         "properties": {"path": {"type": "string"}, "content": {"type": "string"}, "overwrite": {"type": "boolean"}, "encoding": {"type": "string"}, "expected_base_sha256": {"type": ["string", "null"]}, "mutation_request": {"type": "object"}, "request_id": {"type": "string"}, "goal_id": {"type": "string"}, "workunit_id": {"type": "string"}, "worker_attempt_id": {"type": "string"}, "worktree_id": {"type": "string"}, "lease_id": {"type": "string"}}},
        mutation=REVERSIBLE_REPO,
        resources=("filesystem", "ssd"),
    ...
    # by wrapping hawking/processes.py's existing read paths only -- nothing new
    ...
    registry.register(ToolSpec(
        ...
        "reap; reaping is a startup-only self-heal in hawking/runtime.py.",
        {"type": "object", "additionalProperties": False, "properties": {}},
        handler=_processes_orphaned,
    ))
    # Phase D browser world interface.  Playwright is a managed local sidecar;
    # these names are Hawking capabilities with normal registry permission
    # checks, bounded payloads, canonical session IDs and durable WorldState.
    # Browser input is intentionally EXTERNAL_WRITE: a web click/type/select is
    # not a filesystem mutation, but it is still an externally observable act
    # and must be explicitly granted by the enclosing Goal.
    browser_session = {"type": "string"}
    browser_page = {"type": "string"}
    browser_target = {
        "type": "object",
        "additionalProperties": False,
        "properties": {
            "role": {"type": "string"}, "name": {"type": "string"},
            "label": {"type": "string"}, "test_id": {"type": "string"},
            "text": {"type": "string"}, "css": {"type": "string"},
            "exact": {"type": "boolean"},
        },
    }
    browser_timeout = {"type": "number"}
    browser_handler = lambda operation: (
        lambda context, args: _browser_world(context, args, operation)
    )
    macos_handler = lambda operation: (
        lambda context, args: _macos_world(context, args, operation)
    )
    # Phase E desktop capability.  Observation is broadly discoverable; the
    # semantic action tools remain EXTERNAL_WRITE and only become reachable
    # after a durable Goal contract admits a target scope and the caller
    # acquires a short-lived Hawking lease.
    registry.register(ToolSpec(
        "macos.health",
        "Report the versioned Hawking macOS helper and current native-perception readiness.",
        {"type": "object", "additionalProperties": False, "properties": {}},
        resources=("desktop",), handler=macos_handler("health"),
    ))
    registry.register(ToolSpec(
        "macos.observe",
        "Observe Hawking's bounded native desktop WorldState: applications, windows, focus, and AX availability.",
        {"type": "object", "additionalProperties": False, "properties": {}},
        deterministic=False, timeout_s=60.0, resources=("desktop",),
        handler=macos_handler("observe"),
    ))
    registry.register(ToolSpec(
        "macos.changed",
        "Return bounded native desktop WorldState deltas since a known revision; never invents history after a gap.",
        {"type": "object", "required": ["since_revision"], "additionalProperties": False,
         "properties": {"since_revision": {"type": "integer"}}},
        resources=("desktop",), handler=macos_handler("world.changed"),
    ))
    macos_target = {
        "type": "object",
        "additionalProperties": False,
    ...
    registry.register(ToolSpec(
        "macos.find",
        "Resolve exactly one semantic AX target inside one observed application; no action is performed.",
        {"type": "object", "required": ["target", "application"], "additionalProperties": False,
         "properties": {"application": {"type": "string"}, "target": macos_element,
                        "max_results": {"type": "integer"}}},
        deterministic=False, timeout_s=60.0, resources=("desktop",), handler=macos_handler("find"),
    ))
    registry.register(ToolSpec(
        "macos.verify",
        "Observe and verify one semantic AX state after an action; a transport success is not proof.",
        {"type": "object", "required": ["target", "application"], "additionalProperties": False,
         "properties": {"application": {"type": "string"}, "target": macos_element,
                        "max_results": {"type": "integer"}}},
        deterministic=False, timeout_s=60.0, resources=("desktop",),
        verifier_expectations=("verified is true only for exactly one observed semantic target",),
        handler=macos_handler("verify"),
    ))
    registry.register(ToolSpec(
        "macos.lease.acquire",
        "Acquire one short-lived, preemptible Hawking desktop action lease for an exact application scope.",
        {"type": "object", "required": ["target"], "additionalProperties": False,
         "properties": {"target": macos_target, "ttl_s": {"type": "number"},
                        "reason": {"type": "string"}}},
        mutation=EXTERNAL_WRITE, deterministic=False, timeout_s=30.0, resources=("desktop",),
        handler=macos_handler("lease.acquire"),
    ))
    for lease_name, lease_operation, description in (
        ("macos.lease.release", "lease.release", "Release the caller-owned macOS action lease and record the release receipt."),
        ("macos.lease.preempt", "lease.preempt", "Preempt the caller-owned macOS action lease on user-input or operator interruption."),
    ):
        registry.register(ToolSpec(
            lease_name, description,
            {"type": "object", "required": ["lease_id"], "additionalProperties": False,
             "properties": {"lease_id": {"type": "string"}, "reason": {"type": "string"}}},
            mutation=EXTERNAL_WRITE, deterministic=True, resources=("desktop",),
            handler=macos_handler(lease_operation),
        ))
    registry.register(ToolSpec(
        "macos.press",
        "Perform exactly one semantic AXPress after lease admission and verify the requested post-action state.",
        {"type": "object", "required": ["lease_id", "application", "target", "verify"],
         "additionalProperties": False,
         "properties": {"lease_id": {"type": "string"}, "application": {"type": "string"},
                        "target": macos_element, "verify": macos_element}},
        mutation=EXTERNAL_WRITE, deterministic=False, timeout_s=60.0, resources=("desktop",),
        verifier_expectations=(
            "requires one active lease, checks foreground preemption, observes before and after revisions, and returns verified true",
        ),
        handler=macos_handler("press"),
    ))
    registry.register(ToolSpec(
        "browser.health",
        "Report Hawking's managed Playwright sidecar health and pinned runtime version.",
        {"type": "object", "additionalProperties": False, "properties": {}},
        resources=("browser",), handler=browser_handler("health"),
    ))
    registry.register(ToolSpec(
        "browser.open",
        "Open or reuse one Hawking-owned browser session and optional initial page.",
        {"type": "object", "additionalProperties": False,
         "properties": {"browser_session_id": browser_session, "url": {"type": "string"},
                        "wait_until": {"type": "string", "enum": ["commit", "domcontentloaded", "load", "networkidle"]},
                        "timeout_s": browser_timeout}},
        mutation=REVERSIBLE_RUNTIME, deterministic=False, timeout_s=120.0,
        resources=("browser", "network"),
        verifier_expectations=("returns a canonical browser_session_id and page identity",),
        handler=browser_handler("open"),
    ))
    registry.register(ToolSpec(
        "browser.close", "Close one Hawking-owned browser session and release its context.",
        {"type": "object", "required": ["browser_session_id"], "additionalProperties": False,
         "properties": {"browser_session_id": browser_session}},
        mutation=REVERSIBLE_RUNTIME, deterministic=False, resources=("browser",),
        handler=browser_handler("close"),
    ))
    registry.register(ToolSpec(
        "browser.pages", "List pages in one Hawking browser session with canonical identities.",
        {"type": "object", "required": ["browser_session_id"], "additionalProperties": False,
         "properties": {"browser_session_id": browser_session}},
        resources=("browser",), handler=browser_handler("pages"),
    ))
    registry.register(ToolSpec(
        "browser.goto", "Navigate one Hawking browser page to an explicitly supplied http(s) or file URL.",
        {"type": "object", "required": ["browser_session_id", "url"], "additionalProperties": False,
         "properties": {"browser_session_id": browser_session, "page_id": browser_page,
                        "url": {"type": "string"},
                        "wait_until": {"type": "string", "enum": ["commit", "domcontentloaded", "load", "networkidle"]},
                        "timeout_s": browser_timeout}},
        mutation=REVERSIBLE_RUNTIME, deterministic=False, timeout_s=120.0,
        resources=("browser", "network"), handler=browser_handler("goto"),
    ))
    registry.register(ToolSpec(
        "browser.observe",
        "Observe a bounded semantic browser snapshot: page identity, visible controls, text, focus, and recent errors.",
        {"type": "object", "required": ["browser_session_id"], "additionalProperties": False,
         "properties": {"browser_session_id": browser_session, "page_id": browser_page,
                        "max_nodes": {"type": "integer"}, "timeout_s": browser_timeout}},
        deterministic=False, timeout_s=60.0, resources=("browser",),
        handler=browser_handler("observe"),
    ))
    registry.register(ToolSpec(
        "browser.changed",
        "Return bounded semantic WorldState changes since a known browser revision; reports a gap instead of inventing history.",
        {"type": "object", "required": ["browser_session_id", "since_revision"], "additionalProperties": False,
         "properties": {"browser_session_id": browser_session, "since_revision": {"type": "integer"}}},
        resources=("browser",), handler=browser_handler("world.changed"),
    ))
    registry.register(ToolSpec(
        "browser.find", "Resolve a semantic target and report zero, one, or many matches without guessing an action target.",
        {"type": "object", "required": ["browser_session_id", "target"], "additionalProperties": False,
         "properties": {"browser_session_id": browser_session, "page_id": browser_page,
                        "target": browser_target, "max_results": {"type": "integer"},
                        "timeout_s": browser_timeout}},
        deterministic=False, timeout_s=60.0, resources=("browser",), handler=browser_handler("find"),
    ))
    for browser_name, browser_operation, required, properties, description in (
        ("browser.click", "click", ["browser_session_id", "target"],
         {"target": browser_target}, "Click exactly one semantic browser target after explicit authorization."),
        ("browser.type", "type", ["browser_session_id", "target", "text"],
         {"target": browser_target, "text": {"type": "string"}}, "Replace text in exactly one semantic browser target after explicit authorization."),
        ("browser.select", "select", ["browser_session_id", "target", "option"],
         {"target": browser_target, "option": {}}, "Select one option in exactly one semantic browser target after explicit authorization."),
        ("browser.key", "key", ["browser_session_id", "key"],
         {"target": browser_target, "key": {"type": "string"}}, "Send one named key to the page or an explicitly resolved semantic target."),
        ("browser.scroll", "scroll", ["browser_session_id"],
         {"delta_x": {"type": "number"}, "delta_y": {"type": "number"}}, "Scroll one Hawking browser page after explicit authorization."),
    ):
        registry.register(ToolSpec(
            browser_name, description,
            {"type": "object", "required": required, "additionalProperties": False,
             "properties": {"browser_session_id": browser_session, "page_id": browser_page,
                            "timeout_s": browser_timeout, **properties}},
            mutation=EXTERNAL_WRITE, deterministic=False, timeout_s=120.0,
            resources=("browser",),
            verifier_expectations=("records semantic target resolution and requires later browser.verify for outcome claims",),
            handler=browser_handler(browser_operation),
        ))
    registry.register(ToolSpec(
        "browser.wait", "Wait for a bounded load state, target visibility, or short explicit delay.",
        {"type": "object", "required": ["browser_session_id"], "additionalProperties": False,
         "properties": {"browser_session_id": browser_session, "page_id": browser_page,
                        "target": browser_target, "state": {"type": "string", "enum": ["attached", "detached", "visible", "hidden"]},
                        "load_state": {"type": "string", "enum": ["domcontentloaded", "load", "networkidle"]},
                        "delay_ms": {"type": "integer"}, "timeout_s": browser_timeout}},
        mutation=REVERSIBLE_RUNTIME, deterministic=False, timeout_s=120.0,
        resources=("browser",), handler=browser_handler("wait"),
    ))
    registry.register(ToolSpec(
        "browser.screenshot", "Capture a derived screenshot artifact from one Hawking browser page.",
        {"type": "object", "required": ["browser_session_id"], "additionalProperties": False,
         "properties": {"browser_session_id": browser_session, "page_id": browser_page,
                        "full_page": {"type": "boolean"}, "timeout_s": browser_timeout}},
        deterministic=False, timeout_s=120.0, resources=("browser", "ssd"),
        handler=browser_handler("screenshot"),
    ))
    for browser_name, browser_operation, description in (
        ("browser.console", "console", "Read bounded recent browser console events from one Hawking session."),
        ("browser.network", "network", "Read bounded recent browser network failures/errors from one Hawking session."),
        ("browser.downloads", "downloads", "Read bounded download metadata from one Hawking browser session."),
    ):
        registry.register(ToolSpec(
            browser_name, description,
            {"type": "object", "required": ["browser_session_id"], "additionalProperties": False,
             "properties": {"browser_session_id": browser_session, "limit": {"type": "integer"}}},
            deterministic=False, resources=("browser",), handler=browser_handler(browser_operation),
        ))
    registry.register(ToolSpec(
        "browser.verify", "Verify URL, visible text, or semantic target state after a browser action; never treats an action return as proof.",
        {"type": "object", "required": ["browser_session_id"], "additionalProperties": False,
         "properties": {"browser_session_id": browser_session, "page_id": browser_page,
                        "url_includes": {"type": "string"}, "text": {"type": "string"},
                        "target": browser_target, "enabled": {"type": "boolean"}, "timeout_s": browser_timeout}},
        deterministic=False, timeout_s=60.0, resources=("browser",),
        verifier_expectations=("verified remains false until the requested observable condition holds",),
        handler=browser_handler("verify"),
    ))
    registry.register(ToolSpec(
    ...
    registry.register(ToolSpec(
        "odyssey.ingest",
        "Read the live mid-flight Odyssey state so HAWKING can take it over without restarting it.",
        {"type": "object", "additionalProperties": False, "properties": {}},
    ...
    registry.register(ToolSpec(
        ...
        # cycle 75: the body tried to check neighbours across hawking/tests -- step
    ...
    registry.register(ToolSpec(
        "perception.inspect", "Inspect Hawking's native perception API and profile/tool surface without starting a worker.",
        {"type": "object", "additionalProperties": False, "properties": {"profile": {"type": "string"}}},
        mutation=READ_ONLY, deterministic=True, resources=("filesystem",), handler=_perception_inspect,
    ))
    registry.register(ToolSpec(
        "perception.query", "Call one allowlisted read/evidence tool on Hawking's native perception surface.",
        {"type": "object", "required": ["tool"], "additionalProperties": False, "properties": {
            "profile": {"type": "string"},
            "tool": {"type": "string"},
            "arguments": {"type": "object"},
        }},
        mutation=READ_ONLY, deterministic=False, resources=("filesystem",), handler=_perception_query,
    ))
    registry.register(ToolSpec(
    ...
    # External swarm tools were retired with the vendor worker boundary. The
    # bounded Hawking worker staging contract is owned by the native Rust CLI;
    # no typed tool may launch or poll an external agent.
    ...
    registry.register(ToolSpec(
        ...
        {"type": "object", "additionalProperties": False, "properties": {}},
        handler=_perception_capabilities,
    ))
    registry.register(ToolSpec(
        "perception.tools",
        "Which Hawking perception tools can actually run, and the named mechanism "
        "that refuses the rest. Read this before perception op=query.",
        {"type": "object", "additionalProperties": False, "properties": {}},
        handler=_perception_tools,
    ))
    registry.register(ToolSpec(
        "odyssey.priors",
    ...
    registry.register(ToolSpec(
        "physical.measure",
        "Retired source-specimen measurement bridge. Use physical.emit on a "
        "receipt produced by the Hawking-native resident.",
        {"type": "object", "required": ["snapshot", "specimen", "confirm"],
         ...
         "properties": {"snapshot": {"type": "string"}, "specimen": {"type": "string"},
                        ...
                        "backend": {"type": "string", "enum": ["hawking-native"],
                                    "description": "Only the Hawking-native resident "
                                                   "is an executable physical backend."},
                        "write_receipt": {"type": "boolean"}}},
        mutation=COSTLY, deterministic=False,
