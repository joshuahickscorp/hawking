...
def find_repo_root(start: Optional[Union[str, Path]] = None) -> Path:
    ...
    Works from an editable install (package lives at ``<repo>/hawking``) and
    ...
    worktrees use a file, and a stamped ``~/.local/share/hawking/current``
    ...
    for cand in (here, *here.parents):
        if (cand / ".git").exists() or (cand / "Cargo.toml").is_file():
            ...
            if (cand / "Cargo.toml").is_file():
                return cand
    # Stamped install: hawking/paths.py -> package dir -> container.
