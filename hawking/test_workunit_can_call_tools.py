...
Before this, HAWKING had 41 registered tools and NO call site: `ToolRegistry`
...
from hawking.executors import BACKEND_TOOL, WorkUnitExecutor, select_backend_name
...
def test_a_workunit_actually_invokes_a_real_tool(tmp_path):
    ...
    assert result["tool_result"]["schema"] == "hawking.agentos.tool.result.v1"
...
def test_the_resident_can_read_its_own_source_through_a_tool(tmp_path):
    ...
    result = executor.execute(
        _unit(tool="filesystem.read", tool_arguments={"path": "hawking/executors.py"})
    )
    assert result["validation"]["ok"] is True, result["validation"]
...
def test_a_huge_tool_return_is_bounded_in_the_record(tmp_path):
    ...
    from hawking.executors import _redact_tool_value
...
def test_non_engine_backends_now_emit_bus_events(tmp_path):
    ...
    reached EventSink/hawking resident watch) -- only the qwen/engine path did.
