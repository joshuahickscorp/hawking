"""The resident can see and drive the live Odyssey mission through HAWKING.

Odyssey (tools/odyssey_ctl.py) is real and mid-flight -- O003 sealed, O006 on
disk, O010-O013 queued -- but HAWKING has no odyssey verb at all. hawking/odyssey.py
...
2. every verb that can mutate Odyssey state, spawn a subprocess, or start a
   ...
   hawking/tool_registry.py already uses for benchmark.run/accelerator.benchmark
...
3. the "own" verbs (ingest/add_to_eligibility/park_specimen/record_law/
   ...
   *before* the ledger file is touched, refuse a specimen HAWKING invented, and
...
Runnable two ways:
    ...
    python3 -m pytest hawking/test_odyssey.py -q
    python3 hawking/test_odyssey.py
"""
...
def test_admit_check_is_read_only_despite_the_verb_name():
    ...
    result = odyssey.admit_check("hawking-odyssey-probe", 1.0)
    assert result["exit_code"] in (0, 1), result
    assert "slug=hawking-odyssey-probe" in result["stdout"]
...
def isolated_ledger(monkeypatch, tmp_path):
    ...
    path = tmp_path / "HAWKING_LEDGER.json"
...
def test_own_verbs_refuse_without_confirm_before_touching_the_ledger(isolated_ledger, monkeypatch):
    """Refusal must happen before HAWKING_LEDGER.json is ever written."""
...
def test_add_to_eligibility_refuses_a_specimen_hawking_invented(isolated_ledger):
