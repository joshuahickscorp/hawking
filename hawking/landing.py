"""The one remaining self-hosting gap: HAWKING can search, read, write and test its
...
built from three roles that do not trust each other.
    ...
    LandingService         the only code in this file (and, by construction,
                           the only code anywhere in HAWKING) that shells out to
...
_ALWAYS_REFUSED_PREFIXES: Tuple[str, ...] = (
    ...
    # scratch repo, proposed `allowed_paths=["hawking/tool_registry.py"]` with a
    ...
    "hawking/test_goal_verifier_synthesis.py",
    "hawking/test_hawking_overhead.py",
    "hawking/test_self_mutation_e2e.py",
    "hawking/test_context_compiler_runtime.py",
    "hawking/test_qwen38_prefill_pipeline.py",
    "hawking/test_long_context_runtime.py",
    "hawking/test_deltanet_state_checkpoint.py",
    "hawking/test_autonomous_frontier_metabolism.py",
    "hawking/test_capability_callsite_reachability.py",
    "hawking/test_resident_protected_performance.py",
    "hawking/test_resident_successor_handoff.py",
    "hawking/test_negative_science_runtime.py",
    "hawking/test_resident_watch_control_plane.py",
...
AUTHORSHIP_TRAILER = "Landed-By: hawking-landing-service"
...
class LandingService:
    """The sole writer. This is the only class in HAWKING whose methods are
    ...
    def land(self, proposal: LandingProposal) -> LandingResult:
        ...
        # declaring only feature.txt was landed with hawking/landing.py overwritten
...
def propose_landing(
    ...
    *,
    ...
    return {
        "schema": "hawking.landing.result.v1",
