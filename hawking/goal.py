"""Goal text compiler for HAWKING.
...
def infer_required_capabilities(
    ...
    *,
    ...
    explicit: Optional[Iterable[Any]] = None,
) -> List[str]:
    """Compile only capabilities the execution plan can actually need.

    This is deliberately lexical and conservative.  It never infers a TCC
    permission from a generic word such as ``permission`` or ``keyboard``;
    an explicit Goal capability list remains the authoritative way to request
    a specialist operation.  The resulting list is a lease ceiling, not an OS
    grant.
    """
    text = str(goal_text or "")
    lower = text.lower()
    values = list(explicit or [])
    # Every local Goal reads its own logical workspace.  Read-only research
    # does not inherit mutation/process requirements from the discrete Goal
    # completion contract.
    values.append("WORKSPACE_READ")
    if str(goal_mode or "discrete_goal").strip() != "worker_research":
        if re.search(r"\b(edit|modify|change|create|write|update|fix|implement|patch|mutat)", lower):
            values.append("WORKSPACE_WRITE")
        # Discrete Goals have a focused-test acceptance edge, so process
        # execution is known before dispatch rather than discovered after
        # cognition starts. A research worker remains read-only.
        # The discrete completion contract includes a focused test/build edge;
        # record that known need before dispatch rather than after cognition.
        values.append("PROCESS_EXECUTION")
    if re.search(r"\b(browser|web page|website|playwright|navigate|open a browser|internet|online|http[s]?://)", lower):
        values.extend(("BROWSER", "NETWORK"))
    if browser_authority:
        values.append("BROWSER")
        if browser_authority.get("actions"):
            values.append("NETWORK")
    if re.search(r"\b(accessibility|axpress|ax observe|semantic ui|desktop application|macos ui|mac app|finder window|operate .*app|click .*app)", lower):
        values.append("ACCESSIBILITY")
    if macos_authority:
        values.append("ACCESSIBILITY")
    if re.search(r"\b(screen capture|screen recording|screenshot|visual observation|observe the screen|ocr)", lower):
        values.append("SCREEN_CAPTURE")
    if re.search(r"\b(input monitoring|global keyboard|global mouse|monitor keyboard|monitor mouse events)", lower):
        values.append("INPUT_MONITORING")
    if re.search(r"\b(automation|apple events|osascript|automate finder|automate terminal|system events)", lower):
        target = None
        for candidate in ("Finder", "Terminal", "System Events"):
            if candidate.lower() in lower:
                target = candidate
                break
        values.append(f"AUTOMATION:{target}" if target else "AUTOMATION")
    if re.search(r"\b(microphone|mic|record audio|voice input)", lower):
        values.append("MICROPHONE")
    if re.search(r"\b(speech recognition|transcrib|dictat|apple speech)", lower):
        values.append("SPEECH_RECOGNITION")
    if "/volumes/" in lower or re.search(r"\b(external drive|mounted volume|corpdrive)", lower):
        values.append("EXTERNAL_DRIVE_READ")
        if re.search(r"\b(write|edit|modify|create|update|copy|move)", lower):
            values.append("EXTERNAL_DRIVE_WRITE")
    if re.search(r"\b(xcode|swiftc|xcrun|developer tools|cargo|rust compiler)", lower):
        values.append("DEVELOPER_TOOLS")
...
class WorkUnitDAG:
    ...
    def add_unit(
        ...
        *,
        ...
        capability_lease: Optional[Mapping[str, Any]] = None,
    ) -> WorkUnit:
        if not unit_id:
        ...
        wu = WorkUnit(
            ...
            capability_lease=(dict(capability_lease) if isinstance(capability_lease, Mapping) else None),
...
class GoalCompiler:
    ...
    def _synthesised_verify_command(self, blob: str, named: List[str]) -> str:
        ...
        # reads as absent. Fine here (HAWKING lands its work as commits); switch to
    ...
    def compile(
        ...
        *,
        ...
        required = infer_required_capabilities(
            text, goal_mode=goal_mode, explicit=required_capabilities,
        )
        dag = self._build_workunit_dag(obligations, primary, files, required)
...
def compiled_ir_to_jsonable(compiled: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for key in ("invariants", "acceptance_criteria", "referenced_files", "required_capabilities"):
...
def compiled_ir_from_jsonable(data: Any) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "required_capabilities": normalize_capabilities(data.get("required_capabilities") or []),
    }
...
def _mentioned_and_known_files(
    ...
    ``HAWKING_NO_TOOLS=1`` evidence is the only channel to a file's bytes, so the
    model was handed a filename, no bytes and no tool, and invented an anchor
    for a different file entirely. Receipt
    ``.hawking/receipts/16c237d7-d2e0-40b4-9b62-742e6edf2824.json``: four resident
    calls, zero operations, "your old_text for hawking/engine.py matches nothing
    ...
    if known:
        ...
        source_first = (
            ...
            and "hawking/engine.py" in known_set
            else None
        )
        ...
        # `tools/hawking_metric.py` and the compiler captured it in
