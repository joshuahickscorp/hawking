...
NOMENCLATURE_VERSION = "HAWKING_NOMENCLATURE_V3"

# New observations use descriptive, product-neutral filenames.  The former
# HCLI-prefixed files are sealed historical evidence, so readers preserve and
# resolve them rather than rewriting their bytes or fabricating new receipts.
FLASH_SCIENCE_RECEIPT_NAMES = (
    "FLASH_NEXT_PRE_RUNTIME_SCIENCE.json",
    "HCLI_FLASH_NEXT_PRE_RUNTIME_SCIENCE.json",
)
...
def preferred_headless_receipt(
    repo: str | PathLike[str] | Path,
    names: tuple[str, ...],
) -> Path:
    """Return present evidence first, otherwise the canonical future path."""
...
def preferred_flash_science_receipt(repo: str | PathLike[str] | Path) -> Path:
    return preferred_headless_receipt(repo, FLASH_SCIENCE_RECEIPT_NAMES)


def preferred_modellake_census_receipt(repo: str | PathLike[str] | Path) -> Path:
    return preferred_headless_receipt(repo, MODELLAKE_CENSUS_RECEIPT_NAMES)


def preferred_modellake_supervision_receipt(repo: str | PathLike[str] | Path) -> Path:
    return preferred_headless_receipt(repo, MODELLAKE_SUPERVISION_RECEIPT_NAMES)
