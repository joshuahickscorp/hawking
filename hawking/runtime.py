...
OWNERSHIP_SCHEMA = "hawking.runtime_pool.v1"
OWNERSHIP_NAME = "runtime_pool.json"
RESIDENT_OWNERSHIP_NAME = "resident_ownership.json"
OVERLAP_SCHEMA = "hawking.model_overlap.v1"
...
def _install_hooks() -> None:
    ...
    with _HOOKS_LOCK:
        ...
        if os.environ.get("HAWKING_DISABLE_SIGNAL_HOOKS") == "1":
            return
...
def _reap_orphans_once() -> None:
    ...
    try:
        ...
        if result.get("reaped"):
            ...
            print(
                f"[hawking] reclaimed {len(result['reaped'])} orphaned resident "
...
def load_observed_overlap(workspace: Union[str, Path]) -> Optional[int]:
    path = Path(workspace) / ".hawking" / OVERLAP_NAME
    if not path.is_file():
        return None
    try:
...
def store_observed_overlap(workspace: Union[str, Path], n: int) -> None:
    ...
    if value < 1:
        return
    root = Path(workspace)
    path = root / ".hawking" / OVERLAP_NAME
    with _OVERLAP_LOCK:
...
def resolve_workspace(workspace: Optional[Union[str, Path]] = None) -> Path:
    ...
    env = os.environ.get("HAWKING_WORKSPACE")
    if env:
        return Path(env).expanduser().resolve()
    cur = Path.cwd().resolve()
    for path in (cur, *cur.parents):
        if (path / ".hawking").exists():
            return path
    return cur
...
class Runtime:
    ...
    def stop(self) -> None:
        ...
        if (
            self.pid
            and pid_is_alive(self.pid)
            and process_identity_matches(self.pid, self.start_time)
        ):
...
class RuntimePool:
    ...
    available via HAWKING_DECODE_TOPOLOGY=process.
    """
    ...
    def _overlap_admit_cap(self) -> int:
        ...
        Constructor `observed_overlap` wins, then HAWKING_OBSERVED_MODEL_OVERLAP,
        ...
        if self.observed_overlap is not None:
            try:
                ...
                pass
        raw = os.environ.get("HAWKING_OBSERVED_MODEL_OVERLAP")
    ...
    def _admit(self, n: int) -> int:
        ...
        raw = os.environ.get("HAWKING_MAX_RUNTIMES")
        if raw:
            try:
        ...
        if cap < want:
            ...
            self.admission_narrowed = {
                ...
                "source": (
                    ...
                    else "HAWKING_OBSERVED_MODEL_OVERLAP"
                    if os.environ.get("HAWKING_OBSERVED_MODEL_OVERLAP")
    ...
    def _ownership_path(self) -> Path:
        return self.workspace / ".hawking" / OWNERSHIP_NAME

    def _resident_ownership_path(self) -> Path:
        return self.workspace / ".hawking" / RESIDENT_OWNERSHIP_NAME
    ...
    def _attach_resident_backend(
        ...
        if self.backend_factory is not None:
            ...
            backend = make_canonical_backend_for_model(
                self.model_path,
                port=port,
                n_slots=n_slots,
    ...
    def _make_backend(self, index: int, n_slots: int, port: int) -> Any:
        ...
        elif is_remote_endpoint(self.model_path):
            # A provider-neutral HTTP selection is already a canonical
            # Hawking runtime input. Keep it behind the same RuntimePool
            # ownership/admission boundary, but do not route it through the
            # native-artifact constructor intended for local runtimes.
    ...
    def start(self) -> None:
        ...
        # Remote OpenAI-compatible endpoints are already provider-neutral
        # Hawking runtime selections; they do not need to masquerade as local
        # native artifacts. Keep the local/vendor path refusal intact while
        # allowing the canonical remote backend to reach its own health gate.
        if (
            self.backend_factory is None
            and not is_remote_endpoint(self.model_path)
            and not is_hawking_native_path(self.model_path)
        ):
            raise RuntimeError(
                "Hawking RuntimePool accepts only an admitted Hawking-native/Noetic "
                f"artifact; refusing legacy or vendor model path {self.model_path!r}"
            )
    ...
    def stop(self) -> Dict[str, Any]:
        reaped: List[int] = []
        unreaped: List[int] = []
        unreaped_start_times: Dict[int, Optional[str]] = {}
        deferred: List[int] = []
        deferred_start_times: Dict[int, Optional[str]] = {}
        skipped: List[Dict[str, Any]] = []

        def retain_unreaped(pid: int, start_time: Optional[str]) -> None:
            if pid not in unreaped:
                unreaped.append(pid)
            unreaped_start_times[pid] = start_time

        def retain_deferred(pid: int, start_time: Optional[str]) -> None:
            if pid not in deferred:
                deferred.append(pid)
            deferred_start_times[pid] = start_time

        def skip_unverified(
            pid: int, start_time: Optional[str], identity_status: str
        ) -> None:
            skipped.append(
                {
                    "pid": pid,
            ...
            if identity_status == "live_start_time_unreadable" and start_time:
                retain_deferred(pid, start_time)
        ...
        for runtime in list(self.runtimes):
            ...
            if backend is not None and id(backend) not in seen_backends:
                ...
                if pid and gone and not pid_is_alive(pid):
                    ...
                    identity_status = process_identity_status(pid, runtime.start_time)
                    if identity_status == "matches":
                        killed = terminate_pid(int(pid))
                        if killed.get("gone"):
                            reaped.append(int(pid))
                        else:
                            retain_unreaped(int(pid), runtime.start_time)
                    else:
                        skip_unverified(int(pid), runtime.start_time, identity_status)
                for leftover_pid in leftover:
                    try:
                        leftover_pid = int(leftover_pid)
                    except (TypeError, ValueError):
                        continue
                    if not leftover_pid or leftover_pid == pid:
                        continue
                    if not pid_is_alive(leftover_pid):
                        reaped.append(leftover_pid)
                        continue
                    identity_status = process_identity_status(
                        leftover_pid, runtime.start_time
                    )
                    if identity_status == "matches":
                        retain_unreaped(leftover_pid, runtime.start_time)
                    else:
                        skip_unverified(
                            leftover_pid, runtime.start_time, identity_status
                        )
        ...
        if unreaped or deferred:
            ...
            for pid in unreaped:
                still.append(
                    {
                        "pid": pid,
                        "start_time": unreaped_start_times.get(pid),
                        "model": self.model_path,
                        "cleanup_status": "unreaped",
                    }
                )
            for pid in deferred:
                still.append(
                    {
                        "pid": pid,
                        "start_time": deferred_start_times.get(pid),
                        "model": self.model_path,
                        "cleanup_status": "identity_unreadable",
                    }
                )
            try:
                _atomic_write(
