...
Runnable two ways:
    ...
    python3 -m pytest hawking/test_machine_status.py -q
    python3 hawking/test_machine_status.py
"""
...
def write_resident(root: Path, **fields) -> Path:
    path = root / ".hawking" / "resident" / "state.json"
...
def test_status_roots_fall_back_to_the_repo():
    ...
    assert (roots[1] / "hawking" / "commands.py").is_file(), roots


def test_status_roots_also_try_resolve_workspace():
    """`resolve_workspace()` (HAWKING_WORKSPACE / .hawking ancestor walk-up) is an
    existing convention built for exactly this and was never wired in."""
    old = os.environ.pop("HAWKING_WORKSPACE", None)
    marked = fresh()
    (marked / ".hawking").mkdir()
    try:
        os.environ["HAWKING_WORKSPACE"] = str(marked)
        ...
        if old is None:
            os.environ.pop("HAWKING_WORKSPACE", None)
        else:
            os.environ["HAWKING_WORKSPACE"] = old
...
def test_a_real_workspace_with_no_state_yet_is_quiet_not_invisible():
    ...
    # `.hawking` alone was the old marker and it is the exact shape of a stray
    # leftover in the system tmp dir, so it can no longer stand for "real".
    (real_root / ".hawking").mkdir()
...
def test_a_neutral_cwd_with_nothing_real_says_so_not_absent():
    ...
    bare_workspace = fresh()  # no .hawking/receipts/civilization/.git in it
    bare_repo = fresh()  # stands in for a stamped install with no vcs metadata
    old = os.environ.pop("HAWKING_WORKSPACE", None)
    try:
        os.environ["HAWKING_WORKSPACE"] = str(bare_workspace)
...
def test_runtime_health_line_never_exceeds_the_frame():
    ...
    for n_ctx, prompt, tps in (
        ...
        text = format_status(snap)
        runtime_lines = [l for l in text.splitlines() if l.startswith("Hawking ")]
...
# --- the failure path the HAWKING_WORKSPACE-based test bypassed ---------------
# `_looks_like_hawking_root` accepted a bare `.git`, and `resolve_workspace()`
# walks up to the filesystem root looking for `.hawking`. A verifier reproduced
# both live: a stray `.hawking` in the system tmp dir and a freshly `git init`-ed
...
def test_a_bare_git_repo_is_not_a_hawking_workspace(tmp_path):
    from hawking.commands import _looks_like_hawking_root

    (tmp_path / ".git").mkdir()
    assert _looks_like_hawking_root(tmp_path) is False


def test_a_stray_hawking_directory_alone_is_not_a_hawking_workspace(tmp_path):
...
def test_the_live_repo_is_still_recognised():
    from hawking.commands import _looks_like_hawking_root
    from hawking.paths import find_repo_root
