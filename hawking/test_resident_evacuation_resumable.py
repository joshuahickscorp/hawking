...
def _state(tmp_path):
    return json.loads((tmp_path / ".hawking" / "mission" / "state.json").read_text())


def test_evacuation_leaves_the_mission_advanceable(tmp_path, monkeypatch):
    from hawking import mission as mission_mod
...
def test_the_resident_sigterm_handler_evacuates_and_does_not_cancel(tmp_path, monkeypatch):
    ...
    import hawking.resident as resident_mod
    from hawking.resident import ResidentConfig, ResidentDaemon
    ...
    monkeypatch.setattr(hawking, "Hawking", FakeHawking, raising=True)
    ...
    state_path = tmp_path / ".hawking" / "resident" / "state.json"
