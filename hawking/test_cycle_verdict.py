...
from hawking.cycle_verdict import FAIL, PASS, UNKNOWN, evaluate, render


def _workspace(tmp_path, *, mission=None, daemon=None, dag=None, log=None, events=None):
    hawking = tmp_path / ".hawking"
    (hawking / "mission").mkdir(parents=True)
    (hawking / "resident").mkdir(parents=True)
    if mission is not None:
        (hawking / "mission" / "state.json").write_text(json.dumps(mission))
    if daemon is not None:
        (hawking / "resident" / "state.json").write_text(json.dumps(daemon))
    if dag is not None:
        (hawking / "dag.json").write_text(json.dumps(dag))
    if log:
        (hawking / "mission" / "mission.log").write_text(
            "\n".join(json.dumps(r) for r in log)
        )
    if events:
        (hawking / "mission" / "events.jsonl").write_text(
...
def test_a_fully_good_run_has_no_failing_criterion(tmp_path):
    ...
    receipts = tmp_path / ".hawking" / "receipts"
    receipts.mkdir()
...
def test_old_receipts_do_not_hold_the_bar_down(tmp_path):
    ...
    started = json.loads((tmp_path / ".hawking" / "mission" / "state.json").read_text())[
        "started_at"
    ]
...
def test_budgets_are_named_and_overridable(tmp_path):
    ...
    from hawking.cycle_verdict import DEFAULT_BUDGETS
