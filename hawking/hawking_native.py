...
The HAWKING engine already owns prompt construction, schema retry, mutation
...
# A constrained HAWKING worker may observe an already-live native resident, but
# must not turn a failed request into a new child process. The fence is
# context-local so an ordinary daemon request can still use its explicitly
# owned recovery path while a concurrent constrained request fails closed.
_NATIVE_RUNTIME_SPAWN_ALLOWED: ContextVar[bool] = ContextVar(
    "hawking_native_runtime_spawn_allowed", default=True,
)
_NATIVE_BOUNDARY_TRACE_ALLOWED: ContextVar[bool] = ContextVar(
    "hawking_native_boundary_trace_allowed", default=True,
)


def native_runtime_spawning_allowed() -> bool:
    return bool(_NATIVE_RUNTIME_SPAWN_ALLOWED.get())


def native_boundary_trace_allowed() -> bool:
    """Report whether durable boundary telemetry is currently permitted.

    The constrained-worker fence suppresses both native child lifecycle
    changes and boundary telemetry.  Callers that need to decide whether a
    trace record will actually be emitted must consult this predicate rather
    than re-reading the environment, so the context-local fence stays the
    single source of truth.
    """
    return bool(_NATIVE_BOUNDARY_TRACE_ALLOWED.get())


@contextmanager
def suppress_native_runtime_spawning():
    """Forbid native child lifecycle changes and durable boundary telemetry."""
    token = _NATIVE_RUNTIME_SPAWN_ALLOWED.set(False)
    trace_token = _NATIVE_BOUNDARY_TRACE_ALLOWED.set(False)
    try:
        yield
    finally:
        _NATIVE_BOUNDARY_TRACE_ALLOWED.reset(trace_token)
        _NATIVE_RUNTIME_SPAWN_ALLOWED.reset(token)


def _boundary_trace(event: str, **fields: Any) -> None:
    """Append opt-in cross-process boundary telemetry without touching payloads."""
    if not _NATIVE_BOUNDARY_TRACE_ALLOWED.get():
        return
    destination = os.environ.get("HAWKING_BOUNDARY_TRACE")
    if not destination:
        return
    record = {
        "component": "hawking.python",
        "event": str(event),
        "pid": os.getpid(),
...
class HawkingNativeUnavailable(HawkingNativeError):
    """The caller intentionally forbids creating or reviving a runtime."""
...
def _artifact_inventory(root: Path) -> Dict[str, Any]:
    ...
    # fixture with HAWKING_SCAN_ARTIFACT_BYTES=1.
    scanned_bytes = None
    if declared_bytes is None and os.environ.get("HAWKING_SCAN_ARTIFACT_BYTES") == "1":
...
class HawkingNativeConfig:
    ...
    HAWKING, AgentOS, or the verifier know its architecture.
    """
    ...
    profile_schema: str = "hawking.provider.profile.v1"
    ...
    def __post_init__(self) -> None:
        ...
        self.profile_schema = str(self.profile_schema or "hawking.provider.profile.v1")
    ...
    def from_mapping(cls, data: Dict[str, Any]) -> "HawkingNativeConfig":
        ...
        return cls(
            ...
            profile_schema=data.get("profile_schema", "hawking.provider.profile.v1"),
    ...
    def defaults(cls, repo_root: Optional[os.PathLike[str] | str] = None) -> "HawkingNativeConfig":
        ...
        resident_binary = Path(
            os.environ.get(
                "HAWKING_RESIDENT_BINARY",
                str(repo / "workspace/ops/build/rust/release-fast/examples/ascension_qwen38_resident"),
            )
        ).expanduser()
        return cls(
            artifact_root=str(artifact),
            tokenizer=os.environ.get("HAWKING_TOKENIZER", str(artifact / "tokenizer.json")),
            binary=str(binary),
            resident_binary=str(resident_binary),
            max_seq_len=os.environ.get("HAWKING_MAX_SEQ_LEN", 8192),
            generation={
                "max_new_tokens": int(os.environ.get("HAWKING_MAX_NEW_TOKENS", "2048")),
                "temperature": 0.0,
                "top_k": 1,
                "top_p": 1.0,
                "do_sample": False,
            ...
            runtime_env={},
            resident_identity=os.environ.get("HAWKING_RESIDENT_IDENTITY", "sealed-3.14"),
            mode=os.environ.get("HAWKING_NATIVE_MODE", "auto"),
            family="qwen3.8",
    ...
    def with_mode_override(self) -> "HawkingNativeConfig":
        override = (os.environ.get("HAWKING_NATIVE_MODE") or "").strip().lower()
    ...
    if model_path:
        ...
        if candidate.suffix.lower() in {".gravity", ".nx", ".noetic", ".hawking"}:
            raise HawkingNativeConfigError(
                ...
                "set HAWKING_NATIVE_CONFIG to an explicit profile"
            )
        raise HawkingNativeConfigError(
            f"model path {candidate} is not a Hawking-native profile; "
            "use MLX/llama.cpp discovery or set HAWKING_NATIVE_CONFIG"
        )
...
class _TokenizerRenderer:
    ...
    def _fallback_render(
        ...
        *,
        ...
        case is profile data, not an HAWKING-wide model assumption.
        """
...
class ResidentProcess:
    ...
    def __init__(self, config: HawkingNativeConfig) -> None:
        ...
        # The reader runs on a startup-created thread, which does not inherit
        # a request ContextVar. Keep the constrained-request trace fence on
        # the resident itself so its EOF callback cannot append telemetry for
        # a read-only/reason-only request whose child just died.
        self._trace_suppression_lock = threading.Lock()
        self._trace_suppressions = 0
    ...
    def suppress_boundary_trace(self):
        with self._trace_suppression_lock:
            self._trace_suppressions += 1
        try:
            yield
        finally:
            with self._trace_suppression_lock:
    ...
    def _boundary_trace_allowed(self) -> bool:
        with self._trace_suppression_lock:
            return self._trace_suppressions == 0
    ...
    def start(self, timeout: float = 300.0) -> None:
        ...
        log = tempfile.NamedTemporaryFile(
            prefix="hawking-hawking-resident-", suffix=".log", mode="w", delete=False
        )
        ...
        process = subprocess.Popen(
            ...
            # not stable.  Use the HAWKING checkout as the portable working
        ...
        def read_stdout() -> None:
            ...
            try:
                ...
                if self._boundary_trace_allowed():
                    _boundary_trace("pipe_stdout_eof", resident_pid=process.pid)
                self._lines.put(None)

        self._reader = threading.Thread(
            target=read_stdout,
            name="hawking-hawking-resident-reader",
            daemon=True,
        )
        self._reader.start()
...
class HawkingNativeConnector:
    """Translate HAWKING's OpenAI-shaped payload to a profile's native runtime."""
    ...
    def start(self, timeout: float = 300.0) -> None:
        if not native_runtime_spawning_allowed():
            raise HawkingNativeUnavailable(
                "native runtime start is forbidden for this constrained request"
            )
        self.config.validate()
        if self.resident is not None:
    ...
    def _restart_resident(self, timeout: float) -> None:
        if not native_runtime_spawning_allowed():
            raise HawkingNativeUnavailable(
                "native resident recovery is forbidden for this constrained request"
            )
        if self.resident is None:
            return
        self.resident.stop()
    ...
    def _run_one_shot(
        ...
        *,
        ...
        if not native_runtime_spawning_allowed():
            raise HawkingNativeUnavailable(
                "one-shot native execution is forbidden for this constrained request"
            )
        self.config.validate()
        output_fd, output_name = tempfile.mkstemp(prefix="hawking-hawking-", suffix=".json")
        os.close(output_fd)
    ...
    def _native_body_to_openai(
        ...
        *,
        ...
        hawking = {
            ...
            # core HAWKING contract still treats text/usage as the portable fields.
    ...
    def complete_payload(
        ...
        *,
        ...
        started_ns = now_ns()
        trace_scope = (
            self.resident.suppress_boundary_trace()
            if not native_runtime_spawning_allowed() else nullcontext()
        )
        with trace_scope:
            try:
                ...
                # resident is safe for ordinary daemon traffic. The constrained
                # context rejects _restart_resident before it can stop/start a
                # child or let its reader append a boundary trace.
...
__all__ = [
    ...
    "native_runtime_spawning_allowed",
