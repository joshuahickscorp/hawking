...
from hawking.session import CONTEXT_MEMORY_SCHEMA, Session
...
def test_compaction_remembers_goal_steering_and_git_index_state(tmp_path):
    ...
    _git(tmp_path, "config", "user.email", "hawking@example.invalid")
    _git(tmp_path, "config", "user.name", "HAWKING Test")
