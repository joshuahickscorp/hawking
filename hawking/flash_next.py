...
SCHEMA = "hawking.flash-next.identity.v1"
PROMOTION_SCHEMA = "hawking.flash-next.promotion.v1"
...
def _local_observation(root: Optional[Path]) -> Dict[str, Any]:
    if root is None:
        return {"status": "NOT_STAGED", "reason": "set HAWKING_FLASH_NEXT_ROOT to inspect a local acquisition"}
...
def flash_next_profile(local_root: Optional[str | os.PathLike[str]] = None) -> Dict[str, Any]:
    root_value = local_root or os.environ.get("HAWKING_FLASH_NEXT_ROOT")
    ...
    profile = ResidentProfile(
        ...
        capabilities=CapabilityContract.from_mapping({
            "schema": "hawking.provider.capabilities.v1",
            "features": {
