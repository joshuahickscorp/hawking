...
The Claude-removal ladder (C1 >=50% HAWKING-generated, C2 >=80%, C3/C4 100% over a
...
The default is deliberately None, not "hawking". Defaulting to the value the goal
...
from hawking.dag_store import _apply_disk_extras, _unit_to_disk
from hawking.workunit import WorkUnit, authorship_report


class TestWorkUnitAuthorship(unittest.TestCase):
    def test_author_defaults_to_unrecorded_not_hawking(self) -> None:
    ...
    def test_author_survives_persistence(self) -> None:
        wu = WorkUnit(id="u1", role="worker", description="x", author="hawking")
        payload = _unit_to_disk(wu)
        self.assertEqual(payload.get("author"), "hawking", "authorship never reached disk")
        restored = WorkUnit(id="u1", role="worker", description="x")
        _apply_disk_extras(restored, payload)
        self.assertEqual(restored.author, "hawking", "authorship lost on reload")

    def test_ratio_is_over_recorded_units_and_names_the_blind_ones(self) -> None:
        units = [
            WorkUnit(id="a", role="worker", description="x", author="hawking"),
            WorkUnit(id="b", role="worker", description="x", author="hawking"),
        ...
        self.assertEqual(rep["counts"], {"hawking": 2, "claude": 1})
        self.assertEqual(rep["unrecorded"], 1)
        self.assertEqual(rep["total"], 4)
        self.assertAlmostEqual(rep["hawking_fraction"], 2 / 3, places=6)
        self.assertEqual(rep["hawking_fraction_basis"], "recorded")
    ...
    def test_an_all_blind_ledger_refuses_to_report_a_fraction(self) -> None:
        ...
        rep = authorship_report(units)
        self.assertIsNone(
            rep["hawking_fraction"],
            "a fraction computed from zero recorded units is fiction",
        )
        self.assertEqual(rep["hawking_fraction_basis"], "NOT_INSTRUMENTED")


    def test_the_resident_records_itself_as_the_author(self) -> None:
        """The one site where HAWKING itself creates work must say so."""
        from hawking.resident import _child_workunit
    ...
    def test_a_repair_inherits_authorship(self) -> None:
        """A repair of an HAWKING unit is HAWKING's work, not an unrecorded unit."""
        from hawking.workunit import emit_repair

        parent = WorkUnit(id="p1", role="worker", description="x", author="hawking")
