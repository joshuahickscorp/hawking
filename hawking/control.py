"""Hawking's durable, provider-neutral control plane."""
...
def _default_repo_root(workspace: str) -> Path:
    for base in (Path.cwd(), Path(__file__).resolve().parents[1]):
        for candidate in (base, *base.parents):
            if (candidate / "hawking").is_dir() and (candidate / "pyproject.toml").is_file():
...
def _add_paths(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--workspace", default=os.getcwd(), help="Hawking workspace root")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="hawking",
        description="Inspect and operate Hawking's durable provider-neutral control plane.",
    )
    ...
    recovery_status = sub.add_parser(
        "recovery", help="inspect or deterministically reconcile checkpointed Hawking state"
    )
    _add_paths(recovery_status)
    recovery_status.add_argument("--goal-id", default=None)
    recovery_status.add_argument("--reconcile", action="store_true",
                                 help="reconcile observable mutation/owner outcomes; never replay work")

    resident = sub.add_parser(
        "resident",
        help="run HAWKING as a durable, memory-aware resident daemon",
    )
    ...
    perception = sub.add_parser("perception-gate", help="run the Hawking perception evidence-boundary gate")
    perception.add_argument("--workspace", default=None)
    perception.add_argument("--repo-root", default=None)
    perception.add_argument("--search-query", default="Hawking native perception evidence surface")
    perception.add_argument("--emit", default=None)
    perception.add_argument("--timeout-s", type=float, default=12.0)

    native = sub.add_parser("native-gate", help="run the live native HAWKING A1-A6 reproduction ladder")
    ...
    autonomy = sub.add_parser("autonomy-gate", help="run live Hawking WorkUnit and crash-recovery qualification")
    ...
    flash_vocabulary_prefilter = sub.add_parser(
        "flash-vocabulary-prefilter",
        help="verify E13 vocabulary-row geometry and controls without reading tensor payloads",
    )
    flash_vocabulary_prefilter.add_argument("--root", default=None, help="final specimen root; defaults to the canonical ModelLake specimen")
    flash_vocabulary_prefilter.add_argument("--repo-root", default=None)
    flash_vocabulary_prefilter.add_argument("--plan", default=None, help="JSON vocabulary-control plan bound to the exact tokenizer and source")
    flash_vocabulary_prefilter.add_argument("--row-budget", type=int, default=4096)
    flash_vocabulary_prefilter.add_argument("--emit", default=None)
    flash_vocabulary_prefilter.add_argument(
        "--verify-receipt",
        default=None,
    ...
    architecture_queue = sub.add_parser(
        "architecture-queue",
        help="compile the architecture atlas into Accelerator specs and HAWKING WorkUnits",
    )
    ...
    physical_queue.add_argument("--receipt", default=None, help="protected HAWKING receipt to import as measurements")
    ...
    workunit = sub.add_parser(
        "workunit",
        help="Hawking-owned long-run workunit loop (lease-resolved worker)",
    )
    wsub = workunit.add_subparsers(dest="workunit_command")
    wu_start = wsub.add_parser("start", help="start a durable workunit")
    wu_start.add_argument("--workspace", default=os.getcwd())
    wu_start.add_argument("--contract", required=True)
    wu_start.add_argument("--workunit-id", default=None)
    wu_start.add_argument("--session-id", default=None)
    wu_start.add_argument("--model", default="KIMI_P0_OPERATIONAL")
    wu_start.add_argument("--wall-ceiling-s", type=float, default=12 * 3600.0)
    wu_start.add_argument("--foreground", action="store_true")
    wu_status = wsub.add_parser("status", help="inspect one workunit")
    wu_status.add_argument("--workspace", default=os.getcwd())
    wu_status.add_argument("--workunit-id", required=True)
    wu_pause = wsub.add_parser("pause", help="pause / yield GPU resources")
    wu_pause.add_argument("--workspace", default=os.getcwd())
    wu_pause.add_argument("--workunit-id", required=True)
    wu_pause.add_argument("--reason", default="user", choices=["user", "resource"])
    wu_resume = wsub.add_parser("resume", help="resume paused/blocked workunit")
    wu_resume.add_argument("--workspace", default=os.getcwd())
    wu_resume.add_argument("--workunit-id", required=True)
    wu_cancel = wsub.add_parser("cancel", help="cancel a workunit")
    wu_cancel.add_argument("--workspace", default=os.getcwd())
    wu_cancel.add_argument("--workunit-id", required=True)
    wu_resolve = wsub.add_parser("resolve-worker", help="show lease-resolved worker endpoint")
    wu_resolve.add_argument("--model", default="KIMI_P0_OPERATIONAL")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        if args.command == "recovery":
            ...
            value = (
                reconcile_recovery(args.workspace, goal_id=args.goal_id)
                if args.reconcile else recovery_status(args.workspace)
            )
            _emit(value)
            return 0
        ...
        if args.command == "research-gate":
            from hawking.research_gate import run_research_gate
        ...
        if args.command == "perception-gate":
            from hawking.perception_gate import run_perception_gate
        ...
        if args.command == "native-gate":
            from hawking.native_gate import run_native_gate
        ...
        if args.command == "resident-gate":
            from hawking.resident_gate import run_resident_gate
        ...
        if args.command == "native-mission-gate":
            from hawking.native_mission_gate import run_native_mission_gate
        ...
        if args.command == "unattended-window":
            from hawking.autonomy_gate import run_unattended_window
        ...
        if args.command == "accelerator-regression":
            from hawking.agentos.accelerator_regression import run_accelerator_regression
        ...
        if args.command == "qwen27-runtime-archaeology":
            from hawking.agentos.qwen27_runtime_identity import run_runtime_archaeology
        ...
        if args.command == "qwen27-mlp-ab":
            from hawking.agentos.qwen27_mlp_diagnostic import run_qwen27_mlp_diagnostic_ab
        ...
        if args.command == "qwen38-fusion-audit":
            from hawking.agentos.qwen38_fusion_audit import run_qwen38_fusion_source_audit
        ...
        if args.command == "modellake-census":
            from hawking.modellake_gate import run_modellake_census
        ...
        if args.command == "modellake-supervise":
            from hawking.agentos.modellake_supervisor import run_model_lake_supervision
        ...
        if args.command == "flash-tensor-probe":
            from hawking.agentos.flash_tensor_probe import DEFAULT_TENSOR, run_flash_tensor_probe
        ...
        if args.command == "flash-vocabulary-prefilter":
            ...
            if args.verify_receipt:
                try:
                    ...
                    _emit({"status": "INVALID_E13_RECEIPT", "reason": str(exc)})
                    return 1
                _emit({
        ...
        if args.command == "flash-transform-parity":
            from hawking.agentos.flash_transform_parity import DEFAULT_TENSOR, run_flash_transform_parity
        ...
        if args.command == "flash-loader-roundtrip":
            from hawking.agentos.flash_loader_roundtrip import DEFAULT_TENSOR, run_flash_loader_roundtrip
        ...
        if args.command == "flash-component-body":
            from hawking.agentos.flash_component_body import DEFAULT_TENSOR, run_flash_component_body
        ...
        if args.command == "flash-matrix-body":
            from hawking.agentos.flash_matrix_component_body import run_flash_matrix_component_body
        ...
        if args.command == "flash-vector-body":
            from hawking.agentos.flash_vector_component_body import run_flash_vector_body
        ...
        if args.command == "flash-router-graph":
            from hawking.agentos.flash_router_graph import run_flash_router_graph
        ...
        if args.command == "flash-router-selection":
            from hawking.agentos.flash_router_selection import run_flash_router_selection
        ...
        if args.command == "flash-router-representation-ab":
            from hawking.agentos.flash_router_representation_ab import run_flash_router_representation_ab
        ...
        if args.command == "flash-component-campaign":
            from hawking.agentos.flash_component_campaign import run_flash_component_campaign
        ...
        if args.command == "preboard":
            from hawking.agentos.preboard import run_preboard
        ...
        if args.command == "initial-charge":
            from hawking.agentos.charge import create_initial_charge
        ...
        if args.command == "science-maps":
            from hawking.agentos.science_maps import write_science_maps
        ...
        if args.command == "ab-scaffold":
            from hawking.agentos.representation_ab import run_ab_scaffold
        ...
        if args.command == "fpga-preboard":
            from hawking.agentos.fpga_preboard import run_fpga_preboard
        ...
        if args.command == "protected-bench-watch":
            from hawking.agentos.protected_benchmark_watcher import run_protected_benchmark_watcher
        ...
        if args.command == "protected-accelerator-bench":
            from hawking.agentos.protected_accelerator_benchmark import run_protected_accelerator_benchmark
        ...
        if args.command == "handoff":
            from hawking.agentos.handoff import build_handoff
        ...
        if args.command == "resident":
            from hawking.resident import ResidentDaemon, start_resident
            ...
            if args.resident_command == "child":
                ...
                return 0
            _emit({"schema": "hawking.control.resident_cli.v1", "status": "NO_COMMAND"})
            return 0

        if args.command == "workunit":
            from hawking.workunit_owner import WorkunitOwner, resolve_live_worker, run_loop
            from pathlib import Path as _Path
            import json as _json

            cmd = args.workunit_command
            if cmd == "resolve-worker":
                _emit(resolve_live_worker(args.model))
                return 0
            ...
            if cmd == "start":
                ...
                if args.foreground:
                    return run_loop(_Path(args.workspace), result["owner"]["workunit_id"])
                _emit(result)
                return 0
            if cmd == "status":
                _emit(owner.status(args.workunit_id))
                return 0
            if cmd == "pause":
                _emit(owner.pause(args.workunit_id, reason=args.reason))
                return 0
            if cmd == "resume":
                _emit(owner.resume(args.workunit_id))
                return 0
            if cmd == "cancel":
                _emit(owner.cancel(args.workunit_id))
                return 0
            build_parser().parse_args(["workunit", "--help"])
            return 0

        agent = _agent(args.workspace, args.repo_root)
        if args.command == "tools":
            _emit({"schema": "hawking.control.tool_catalog.v1", "tools": agent.tools.discover(role=args.role)})
            return 0
        ...
        if args.command == "background":
            ...
            if command == "list":
                _emit({"schema": "hawking.control.background_catalog.v1", "jobs": agent.background_jobs()})
                return 0
            if command == "status":
        ...
        _emit({
            "schema": "hawking.control.cli_error.v1",
            "status": "FAILED",
...
__all__ = ["CONTROL_VERBS", "build_parser", "main"]
