...
CONTEXT_MEMORY_SCHEMA = "hawking.context.memory.v1"
...
class Session:
    def __init__(self, session_id: Optional[str] = None, goal: str = "", runtime_count: int = 1, model: Optional[str] = None):
        ...
        # Small client preferences belong to the durable Hawking session, not
        # to browser memory.  This is intentionally limited to UI routing
        # state (selected model/scope and the active Goal handle); it is not a
        # second mission or evidence store.
    ...
    def append_message(
        ...
        *,
        ...
        if message_id:
            # A client-visible message handle is useful for artifact metadata;
            # the sequence number remains Hawking's canonical ordering key.
            message["message_id"] = str(message_id)[:160]
        if attachments:
            # Session state retains references and metadata only.  Artifact
            # bytes remain in Hawking's content-addressed owner.
            message["attachments"] = [
                dict(item) for item in attachments[:8]
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Session":
        ...
        ui = data.get("ui")
        s.ui = dict(ui) if isinstance(ui, dict) else {}
        return s


class SessionStore:
    def __init__(self, workspace: str):
        self.dir = os.path.join(workspace, ".hawking", "sessions")
