"""The Python process view is a compatibility skin over Rust HAWKING authority."""
from __future__ import annotations

from hawking import processes as processes_module
from hawking.process_identity import branded_python_entrypoint
from hawking.processes import Process, live_processes, render, summary
...
def test_native_binary_follows_linked_worktree_to_primary_build(tmp_path, monkeypatch):
    monkeypatch.delenv("HAWKING_NATIVE_HAWKING", raising=False)
    monkeypatch.delenv("HAWKING_RUST_BIN", raising=False)
    primary = tmp_path / "primary"
    worktree = primary / ".worktrees" / "candidate"
    gitdir = primary / ".git" / "worktrees" / "candidate"
    binary = primary / "workspace" / "ops" / "build" / "rust" / "debug" / "hawking"
...
def test_native_binary_prefers_snapshot_bundle(tmp_path, monkeypatch):
    ...
    package = snapshot / "hawking"
    package.mkdir(parents=True)
    binary = snapshot / "hawking-rust"
    binary.write_text("#!/bin/sh\n", encoding="utf-8")
    binary.chmod(0o755)
