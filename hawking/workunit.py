...
CLASSIFICATION_PERMISSION_BLOCKED = "PERMISSION_BLOCKED"
...
class WorkUnit:
    ...
    # Machine capabilities are part of the durable unit contract.  The list is
    # the task ceiling; it never widens the OS authority held by hawkingd or
    # its stable native helper.
    required_capabilities: List[str] = field(default_factory=list)
    capability_lease: Optional[Dict[str, Any]] = None
    ...
    # Who created this unit: "hawking", "claude", or "human". The Claude-removal
    # ladder is a RATIO of this field, so it defaults to None -- unrecorded --
    # rather than to "hawking". Defaulting to the value the goal is trying to move
    ...
    # Runtime identity is intentionally separate from content identity. Two
    # same-model workers may execute the same-shaped WorkUnit independently;
    # restart/reconciliation needs to distinguish their attempts without
    # making a replan look like different work.
    worker_id: Optional[str] = None
    worker_attempt_id: Optional[str] = None
    context_packet_digest: Optional[str] = None

    def __post_init__(self) -> None:
        self.resource_class = normalize_resource_class(self.resource_class)
        self.required_capabilities = list(dict.fromkeys(
            str(item).strip().upper() for item in (self.required_capabilities or [])
            if str(item).strip()
        ))
        if self.capability_lease is not None and not isinstance(self.capability_lease, dict):
            self.capability_lease = None
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "execution_identity": execution_identity(self),
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "WorkUnit":
        ...
        return cls(
            ...
            capability_lease=(
                dict(data.get("capability_lease"))
                if isinstance(data.get("capability_lease"), dict) else None
            ),
            provider=data.get("provider"),
            tool=data.get("tool"),
            tool_arguments=(
                dict(data.get("tool_arguments"))
                if isinstance(data.get("tool_arguments"), dict) else None
            ),
            author=data.get("author"),
            worker_id=data.get("worker_id"),
            worker_attempt_id=data.get("worker_attempt_id"),
            context_packet_digest=data.get("context_packet_digest"),
        )
...
def content_identity(wu: WorkUnit) -> str:
    ...
    Identity is (role, description, dependencies, verifier, required
    capabilities). The heading
    ...
    payload = {
        ...
        "required_capabilities": [
            str(item) for item in (getattr(wu, "required_capabilities", None) or [])
        ],
    }
...
def execution_identity(wu: WorkUnit) -> str:
    """Hash the runtime binding without changing WorkUnit content identity."""
    payload = {
        "worker_id": str(getattr(wu, "worker_id", None) or ""),
        "worker_attempt_id": str(getattr(wu, "worker_attempt_id", None) or ""),
        "provider": str(getattr(wu, "provider", None) or ""),
        "tool": str(getattr(wu, "tool", None) or ""),
        "tool_arguments": getattr(wu, "tool_arguments", None) or {},
        "context_packet_digest": str(getattr(wu, "context_packet_digest", None) or ""),
    }
...
def emit_repair(
    ...
    repair = WorkUnit(
        ...
        # A repair of an HAWKING-authored unit is HAWKING's work. Inheriting keeps the
        ...
        required_capabilities=list(getattr(wu, "required_capabilities", None) or []),
        capability_lease=(
            dict(getattr(wu, "capability_lease", None))
            if isinstance(getattr(wu, "capability_lease", None), dict) else None
        ),
        repair_root=root,
        repair_depth=depth,
    )
...
def mark_permission_blocked(
    wu: WorkUnit,
    missing: Optional[Sequence[Mapping[str, Any]]] = None,
    snapshot: Optional[Mapping[str, Any]] = None,
) -> bool:
    """Pause a unit for a missing capability without consuming a retry."""
    if wu.status in {"completed", "failed"}:
        return False
    if wu.status == "running":
        if not transition_status(wu, "blocked"):
            wu.status = "blocked"
    else:
        wu.status = "blocked"
    wu.classification = CLASSIFICATION_PERMISSION_BLOCKED
    wu.assigned_runtime = None
    context = dict(wu.failure_context or {})
    context.update({
        "classification": CLASSIFICATION_PERMISSION_BLOCKED,
        "missing": [dict(item) for item in (missing or [])],
        "snapshot": dict(snapshot or {}),
        "resume_policy": "recheck_then_resume_exact_workunit",
    })
    wu.failure_context = context
    return True
...
def is_ready(wu: WorkUnit, all_units: Dict[str, WorkUnit]) -> bool:
    if wu.status not in ("pending", "failed", "interrupted", "blocked"):
        return False
    if wu.status == "blocked" and wu.classification != CLASSIFICATION_PERMISSION_BLOCKED:
        return False
...
AUTHOR_HAWKING = "hawking"
AUTHOR_CLAUDE = "claude"
AUTHOR_HUMAN = "human"


def authorship_report(units: Iterable["WorkUnit"]) -> Dict[str, Any]:
    """Authorship counts, and the HAWKING fraction over RECORDED units only.
    ...
    if recorded <= 0:
        ...
        fraction = counts.get(AUTHOR_HAWKING, 0) / recorded
        basis = "recorded"
    return {
        "counts": counts,
        ...
        "recorded": recorded,
        "total": total,
        "hawking_fraction": fraction,
        "hawking_fraction_basis": basis,
    }
