...
machine-readable contract for Hawking's research and worker legs. It labels new output;
...
OPTIMIZATION_CLAIM_KEY = "hawking_executes_optimization"
_LEGACY_OPTIMIZATION_CLAIM_KEY = "hcli_executes_optimization"
...
def taxonomy_manifest() -> dict[str, Any]:
    ...
    if (
        ...
        # New manifests name Hawking directly.  Read the retired HCLI key only
        # for sealed/retained manifests; new taxonomy files must emit the
        # canonical spelling.
        or claim_limits.get(
            OPTIMIZATION_CLAIM_KEY,
            claim_limits.get(_LEGACY_OPTIMIZATION_CLAIM_KEY),
        ) is not False
