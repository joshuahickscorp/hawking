"""Resident HAWKING daemon and its low-risk qualification harness.
...
from hawking.workunit import WorkUnit, AUTHOR_HAWKING
from hawking.goal_bank import GoalBank
from hawking.steering import SteeringQueue
from hawking.event_sink import EventSink, read_events
from hawking.stream_render import render_event


SCHEMA = "hawking.resident_daemon.v1"
...
MAX_KNOWLEDGE = 256
# No fixed child-count ceiling. Admission still goes through the central
# scheduler, dependency checks, writer/resource guards, and per-item size
# validation; this value is retained as an inspectable compatibility symbol.
MAX_CHILD_WORKUNITS: Optional[int] = None
MAX_CHILD_DESCRIPTION = 4000
DEFAULT_INTERVAL_S = 30.0
DEFAULT_EVACUATION_GRACE_S = 10.0
DEFAULT_MAX_RESTARTS = 3
# Keep the model-free daemon's default aligned with ``hawking.machine.MemGate``.
...
BEHAVIOR_SCHEMA = "hawking.resident_behavior.v1"
...
DETACHED_ENV = "HAWKING_RESIDENT_DETACHED"
...
def _effective_swap_ceiling(value: Optional[int]) -> int:
    ...
    raw = os.environ.get("HAWKING_SWAP_CEILING_GIB")
    if raw:
        try:
...
def _admit_native_resident_model(model: str) -> str:
    """Resolve one executable Hawking-native profile before a daemon forks.

    The resident is Hawking's durable runtime boundary. Letting an MLX
    directory or remote endpoint through here would create a second runtime
    owner beneath the same daemon name, so reject it before state or a child
    process is created. Any Hawking model remains admissible through its own
    validated native profile or native artifact directory.
    """
    ...
    try:
        ...
        raise SystemExit(
            f"--model {model!r} does not resolve to a native Hawking profile or artifact: {exc}"
        ) from exc
    if not is_hawking_native_path(str(resolved)):
        raise SystemExit(
            f"--model {model!r} is not a native Hawking profile or artifact. "
            "The resident refuses legacy MLX and remote runtime shapes; provide a "
            "validated Hawking-native profile for the model you want to run."
        )
    try:
        config_for_model_path(str(resolved)).validate()
    except HawkingNativeConfigError as exc:
        raise SystemExit(
            f"--model {model!r} is not an executable Hawking-native profile: {exc}"
        ) from exc
    return str(resolved)
...
def resident_dir(workspace: str | os.PathLike[str]) -> Path:
    return Path(workspace).expanduser().resolve() / ".hawking" / STATE_DIRNAME
...
class ResidentStore:
    ...
    def update_body(self, **changes: Any) -> Dict[str, Any]:
        with self._locked():
            ...
            value.setdefault("schema", "hawking.resident_body.v1")
...
class ResidentAlreadyRunning(RuntimeError):
    ...
    def __init__(self, state: Mapping[str, Any]) -> None:
        ...
        super().__init__(
            ...
            "  Use: hawking resident status | hawking resident stop | "
            "hawking resident replace --goal ..."
        )
...
# Defined in hawking.mission (it is a property of Mission.phase) and re-exported
# here so this module stays the name every existing caller imports it from.
from hawking.mission import TERMINAL_MISSION_PHASES  # noqa: E402
...
def mission_blocked_reason(workspace: Path) -> Optional[str]:
    ...
    """
    from hawking.mission import MissionCorruptError, load_state

    try:
        value = load_state(workspace / ".hawking" / "mission" / "state.json")
    ...
    return (
        ...
        "archive .hawking/mission/state.json or start a new goal"
    )
...
def _mission_has_work(workspace: Path) -> bool:
    ...
    path = workspace / ".hawking" / "mission" / "state.json"
    try:
...
def _child_workunit(parent_id: str, value: Mapping[str, Any]) -> WorkUnit:
    ...
    return WorkUnit(
        ...
        # the only site where HAWKING itself creates work, which makes it the one
        # place the Claude-removal ratio can be recorded truthfully rather than
        # reconstructed afterwards.
        author=AUTHOR_HAWKING,
    )


def admit_evidence_children(mission: Any, evidence: Any) -> List[Dict[str, Any]]:
    """Admit child WorkUnits from verified parent evidence only.
    ...
    for event in events:
        ...
        candidate_rows = (
            candidates
            if MAX_CHILD_WORKUNITS is None
            else candidates[:MAX_CHILD_WORKUNITS]
        )
...
class ResidentDaemon:
    ...
    def configure(self, config: ResidentConfig) -> Dict[str, Any]:
        ...
        mission_path = Path(self.workspace) / ".hawking" / "mission" / "state.json"
        if (
...
class ResidentSupervisor:
    ...
    def _memory(self, config: ResidentConfig) -> Dict[str, Any]:
        try:
            from hawking.machine import host_snapshot
        ...
        if self._worker_alive(self.store.read()):
            return decision
        try:
            from hawking.backends import is_remote_endpoint
            from hawking.hawking_native import (
                HawkingNativeConfigError,
                config_for_model_path,
                is_hawking_native_path,
            )
            from hawking.machine import MemGate, resolve_decode_topology
            ...
            # A profile whose executable/artifact binding is absent or wrong
            # is an artifact refusal, not an unknown GPU measurement.
            ...
            decision["safe"] = False
            decision.setdefault("reasons", []).append(
                "runtime artifact admission refused: "
                + str(decision["runtime_gate"]["refusal_reason"])
            )
        except Exception as exc:
    ...
    def _spawn_worker(self) -> None:
        ...
        config = self._config()
        if not config.model:
            # An unbound Hawking daemon can still own status, goals, and its
            # one control port. It must not fall through to Controller's
            # historical auto-discovery, where an MLX/VLM runtime could become
            # a second body owner merely because no native profile was named.
            self.store.update(
                state="WAITING_FOR_NATIVE_MODEL",
                worker_pid=None,
                worker_start_token=None,
                worker_live=False,
                last_event="waiting_for_native_model",
                error=None,
            )
            return
        env = os.environ.copy()
        ...
        # hawking/controller.py selects on self.model_info.path or, failing that,
        # HAWKING_MODEL_PATH -- and neither was being set from the flag, so three
        ...
        # HAWKING_MODEL_PATH is making a deliberate choice this should not silently
        # override.
        if config.model and not env.get("HAWKING_MODEL_PATH"):
            env["HAWKING_MODEL_PATH"] = str(config.model)
    ...
    def run(self) -> int:
        ...
        try:
            while not self._stop:
                ...
                mission_path = self.workspace / ".hawking" / "mission" / "state.json"
                ...
                if worker_live and not config.model:
                    # A legacy state file may describe an already-running
                    # worker without a native binding. Reclaim only this
                    # resident-owned process rather than letting it continue
                    # under an unqualified fallback runtime.
                    self._evacuate(
                        "validated Hawking-native model binding is required",
                        config,
                        waiting_state="WAITING_FOR_NATIVE_MODEL",
                    )
                    state = self.store.read()
                    worker_live = False
                ...
                if decision["action"] == "WAIT_FOR_CLEAN_ROOM":
                    ...
                    if blocked and not worker_live:
                        ...
                        break
                    if not worker_live and not config.model:
                        # Keep supervision and read-only control alive, but
                        # never fall through to Controller's legacy model
                        # auto-discovery. An explicit validated native profile
                        # is the only wake condition for execution.
                        self.store.update(
                            state="WAITING_FOR_NATIVE_MODEL",
                            worker_pid=None,
                            worker_start_token=None,
...
def _session_ledger_evidence(agent: Any) -> Optional[Dict[str, Any]]:
    ...
    state -- the same numbers `hawking resident watch` already surfaces for
    everything else this worker did. This never lands, pushes, or merges;
    `hawking.landing` already owns that path and has a verifier in front of it
...
def _worker_main(state_path: str) -> int:
    ...
    heartbeat_thread = threading.Thread(
        target=heartbeat,
        name="hawking-resident-worker-heartbeat",
        daemon=True,
    )
    ...
    try:
        ...
        mission_path = Path(config.workspace) / ".hawking" / "mission" / "state.json"
...
def _watch_header_lines(state: Mapping[str, Any]) -> List[str]:
    ...
    return [
        f"HAWKING RESIDENT  {state.get('state', '?')}   heartbeat {age} ago"
...
def _watch_steer(root: Path, mission: Mapping[str, Any], text: str) -> List[str]:
    ...
    (`.hawking/steering/<session_id>.json`) -- the same file a fresh worker cycle
    loads on `recover_mission()`, and the same construction pattern already
    used cross-process elsewhere in this codebase (see hawking/delegate.py's
...
def watch_resident(workspace: str | os.PathLike[str], interval_s: float = 2.0) -> int:
    ...
    per-event stream a worker appends to `.hawking/mission/events.jsonl` -- so it
    ...
    mission_path = root / ".hawking" / "mission" / "state.json"
    events_path = root / ".hawking" / "mission" / "events.jsonl"
    ...
    try:
        ...
        print("\n[hawking] detached. The resident is still running.")
        return 0
    finally:
        if fd is not None:
...
def retire_incumbent(daemon: "ResidentDaemon", timeout_s: float = 30.0) -> Dict[str, Any]:
    ...
    resident. The mission state is MOVED to `.hawking/mission-retired/<stamp>/`,
    ...
    mission_dir = Path(daemon.workspace) / ".hawking" / "mission"
    if mission_dir.is_dir() and any(mission_dir.iterdir()):
        stamp = time.strftime("%Y%m%d-%H%M%S", time.gmtime())
        retired = Path(daemon.workspace) / ".hawking" / "mission-retired" / stamp
        ...
        # restart, forever. Observed with 11 units pre-spent in `.hawking/dag.json`
        # before mission 89e411d8 had run a single one of them.
        dag_path = Path(daemon.workspace) / ".hawking" / "dag.json"
        if dag_path.is_file():
...
def start_resident(
    ...
    *,
    ...
    daemon = ResidentDaemon(workspace)
    if model:
        model = _admit_native_resident_model(str(model))
    if replace:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="hawking resident",
        description="Run HAWKING as a durable, memory-aware resident daemon.",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.command in ("start", "replace"):
        ...
        from hawking.cycle_verdict import evaluate, render
...
def daemon_argv(role: str, state_path: str) -> List[str]:
    ...
    succession, and `python3 -m hawking.resident` says nothing about
    either. `hawkingd --supervise <state>` is model-neutral and reads like the
    daemon it is, with `hawking` remaining the client that talks to it.
    ...
    (`hawking.resources.process_start_token`), never by argv, so an incumbent
    ...
    return [sys.executable, "-m", "hawking.hawkingd", role, str(state_path)]
...
def daemon_main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    `hawking resident` uses, so both names accept the same verbs.
    """
