...
WHY THIS EXISTS. `hawking serve` was intellectually stateless: Open WebUI held the
conversation and resent it whole every turn, so HAWKING had no session, no memory
...
what HAWKING must still KNOW: the objective, the active plan and its step, the
...
SCHEMA = "hawking.chat.session.v1"
PLAN_SCHEMA = "hawking.chat.plan.v1"
...
@dataclass
class ChatSession:
    """What HAWKING must still know about this conversation."""
    ...
    # A Goal mutation may arrive as a valid source proposal followed by a
    # missing-test repair turn.  Persist only the machine-validated pending
    # bundle; never persist arbitrary model prose or an unvalidated operation.
    pending_mutation_bundle: Dict[str, Any] = field(default_factory=dict)
    # The provider loop is bounded by Hawking, not by model prose.  Persist
    # the last observed ceiling/use/remaining values with the same chat
    # session so a continuation can be admitted deliberately after a budget
    # checkpoint instead of silently starting a fresh allowance.
    ...
    def path_for(workspace: str, session_id: str) -> Path:
        return (Path(workspace) / ".hawking" / "chat" / f"{session_id}.json")
...
def working_set(session: ChatSession) -> str:
    ...
    budget = session.tool_budget or {}
    if budget:
        lines.append(
            "BUILD_TOOL_BUDGET "
            f"state={budget.get('state', 'UNKNOWN')} "
            f"ceiling={budget.get('ceiling', '?')} "
            f"used={budget.get('used', '?')} "
            f"remaining={budget.get('remaining', '?')}"
        )
    pending = session.pending_mutation_bundle or {}
    if pending:
        missing = ",".join(str(item) for item in (pending.get("missing_requirements") or []))
        preserved = pending.get("source_operations") or pending.get("operations") or []
        try:
            preserved_json = json.dumps(
                preserved, sort_keys=True, separators=(",", ":"), default=str)
        except Exception:
            preserved_json = str(preserved)
        if len(preserved_json) > 1800:
            preserved_json = preserved_json[:1800] + "…"
        test_paths = ",".join(str(item) for item in (pending.get(
            "test_mutation_paths") or []))
        lines.append(
            "PENDING_MUTATION_BUNDLE source operation preserved; emit only the "
            f"missing operation ({missing or 'repair'}) through repo.edit. "
            "Do not emit or alter this preserved source operation: "
            f"{preserved_json}. The missing operation must mutate the focused "
            f"test file ({test_paths or 'from the Goal focus test'}) and retain "
            "the existing focused test invocation.")
    if not lines:
        return ""
...
def archive_turns(session: Any, turns: List[Dict[str, str]]) -> str:
    ...
    root = Path(session.workspace) / ".hawking" / "chat" / "archive"
...
CHECKPOINT_SCHEMA = "hawking.chat.checkpoint.v1"
TRACE_SCHEMA = "hawking.chat.tool-trace.v1"
...
def record_tool_trace(session: Any, trace: List[Dict[str, Any]], *,
    ...
    """
    path = (Path(session.workspace) / ".hawking" / "chat"
...
def checkpoint(session: Any, *, next_action: str = "",
    ...
    record = {
        ...
        # HEAD alone is not enough for a dirty Build workspace.  The caller
        # supplies a bounded digest of the source/test/Git observations that
        # actually informed this checkpoint; a future turn must reconcile it
        # before continuing when the working tree has moved underneath it.
        "source_revision": str(source_revision or ""),
        "tool_budget": dict(tool_budget or getattr(session, "tool_budget", {}) or {}),
        "state": str(state or ""),
        "note": note,
        "written_at": time.time(),
    }
...
def resume_block(record: Optional[Dict[str, Any]]) -> str:
    ...
    if record.get("source_revision"):
        lines.append(f"  SOURCE_REVISION {record['source_revision']}")
