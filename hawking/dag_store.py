...
Writes go to ``<workspace>/.hawking/dag.json`` via a temp file and ``os.replace``.
...
``completed``, and nothing here resumes an inference mid-token.
    ...
    Legacy external-provider fields remain readable as historical data. They
    are never adopted, polled, or relaunched: every running unit is interrupted
    and returned to the normal Hawking-owned WorkUnit path.
"""
...
def _unit_to_disk(wu: WorkUnit) -> Dict[str, Any]:
    # WorkUnit is the single serialization owner.  Keeping a second field
    # overlay here allowed the two schemas to drift and made restart state
    # depend on which writer last touched dag.json.
    return wu.to_dict()


def _apply_disk_extras(wu: WorkUnit, payload: Dict[str, Any]) -> None:
    # Retained as a compatibility hook for callers importing this helper;
    # current WorkUnit.from_dict owns all known fields.
    del wu, payload
...
class DagStore:
    def __init__(self, workspace: Union[str, Path]) -> None:
        ...
        self.dir = self.workspace / ".hawking"
    ...
    def retire(self, *, reason: str = "superseded") -> Optional[Path]:
        ...
        does; the retired document is kept under ``.hawking/dag-retired/`` so an
    ...
    def load(
        ...
        *,
        ...
        # Kept in the signature for old persisted callers; external task
        # liveness is no longer an execution authority.
        del grok_liveness
