...
def resolve_active_decode_limit(
    ...
    that RuntimePool had already refused. One authority: ``hawking.machine``.
...
class ResourceLimits:
    ...
    # Hawking cognition is not a fixed resource slot class.  ``None`` keeps
    # admission open; provider health, process/OS resources, dependency state,
    # budget authority, and effect-scope leases remain the real gates.
    hawking: Optional[int] = None
    hawking_source: str = "unbounded by fixed Hawking slot count"
    ...
    def resolve(
        ...
        return cls(
            ...
            hawking=None,
            hawking_source="unbounded by fixed Hawking slot count",
        )

    def limit_for(self, resource_class: str) -> Optional[int]:
        ...
        mapping = {
            ...
            ResourceClass.HAWKING.value: self.hawking,
        }
...
def normalize_resource_class(value: Any) -> str:
    ...
    if text.upper() in {"GROK", "GROK_BOT", "GROK-BOT"}:
        # Persisted pre-consolidation units remain readable, but all new and
        # recovered work is admitted as Hawking cognition.
        return ResourceClass.HAWKING.value
    try:
...
def can_admit(resource_class: str, occupied: Counter, limits: ResourceLimits) -> bool:
    ...
    return limit is None or occupied[rc] < limit
...
def process_start_token(pid: int) -> Optional[str]:
    ...
    if not isinstance(pid, int) or pid <= 0:
        return None
    # A constrained native request can inspect an already-live child but may
    # not spawn a fallback ``ps`` process just to refresh metadata. Import
    # lazily to avoid making the resource primitives depend on the native
    # connector during ordinary module initialization.
    try:
        from .hawking_native import native_runtime_spawning_allowed
        allow_ps_fallback = native_runtime_spawning_allowed()
    except Exception:
        # Resource identity remains conservative if the optional native
        # connector is unavailable; preserve the historical ordinary path.
        allow_ps_fallback = True
    readers = (_start_token_libproc, _start_token_procfs)
    if allow_ps_fallback:
        readers = (*readers, _start_token_ps)
...
def process_identity_status(pid: int, recorded_start_time: object) -> str:
    """Classify whether ``pid`` still names the recorded process incarnation.

    A PID alone is not ownership: it can be recycled after a child exits. A
    missing or unreadable start token therefore fails closed for destructive
    process cleanup.
    """
    if not recorded_start_time:
        return "no_recorded_start_time"
    live_start_time = process_start_token(pid)
    if live_start_time is None:
        return "live_start_time_unreadable"
    if str(live_start_time) != str(recorded_start_time):
        return "pid_reused_start_time_mismatch"
    return "matches"


def process_identity_matches(pid: int, recorded_start_time: object) -> bool:
    """Whether ``pid`` still names the recorded process incarnation."""
    return process_identity_status(pid, recorded_start_time) == "matches"
...
class MutationLock:
    ...
    def __init__(self, workspace: Optional[Union[str, Path]] = None) -> None:
        ...
        self.path = (
            self.workspace / ".hawking" / MUTATION_LOCK_FILENAME
...
KNOWN_BACKENDS: Tuple[str, ...] = ("hawking", "cpu")
...
def normalize_backend(value: Any) -> str:
    ...
    if text in {"qwen", "grok", "grok_bot", "hawking-native", "native"}:
        text = "hawking"
...
class BackendHealth:
    ...
    Persists under ``<workspace>/.hawking/backend_health.json`` via the same
    ...
    "hawking is unavailable". Mixing them would make a GPU slot look full because
    a different backend's breaker is open.

    The mixed-backend dispatcher (mission / scheduler, reserved) is the
    other consult site: before handing a unit to the hawking or cpu
    ...
    def __init__(
        ...
        *,
        ...
        self.workspace = Path(workspace) if workspace is not None else None
        self.path = (
            self.workspace / ".hawking" / HEALTH_FILENAME
            if self.workspace is not None
            else None
        )
