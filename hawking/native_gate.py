"""Live HAWKING/native reproduction ladder.
...
from hawking.latency import now_ns, seconds_from_ns, since_ns


SCHEMA = "hawking.native_gate.v1"
...
def _profile_path(profile: Optional[str], repo_root: Optional[Path]) -> Path:
    value = profile or os.environ.get("HAWKING_NATIVE_CONFIG")
    if value:
        return Path(value).expanduser().resolve()
    root = repo_root or Path(__file__).resolve().parents[1]
    return (root / "hawking" / "hawking-native.sealed-3.14.json").resolve()
...
def _temporary_generation_env(model_tokens: int) -> Iterator[None]:
    names = {
        "HAWKING_MODEL_TOKENS": str(max(1, int(model_tokens))),
        "HAWKING_STRUCTURED_OUTPUT_ATTEMPTS": "1",
    }
...
def _write_receipt(report: Dict[str, Any], emit: Optional[str], repo_root: Path) -> None:
    destination = Path(emit).expanduser() if emit else repo_root / "receipts" / "headless" / "HAWKING_NATIVE_GATE.json"
...
def causality_payload(report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "probe_performed": (
            ...
            "A4 Controller.complete_text, A5 structured cognition, A6 hawking CLI task; "
...
def record_native_causality(report: Dict[str, Any], **kwargs: Any) -> dict[str, Any]:
    ...
    return sc.stamp_gate(
        report,
        source="hawking/native_gate.py::run_native_gate",
        **payload,
    )
...
def run_native_gate(
    ...
    *,
    ...
    from hawking.hawking_native import HawkingNativeConfig, HawkingNativeConnector, _TokenizerRenderer
    ...
    report: Dict[str, Any] = {
        "schema": SCHEMA,
        "status": "RUNNING",
        "qualification": "LIVE_NATIVE_HAWKING_LADDER",
        "timing_unit": "ns",
    ...
    try:
        with _temporary_generation_env(model_tokens):
            plain_record = record("A4_hawking_plain_cognition", lambda: controller.complete_text(prompt))
            ...
            structured_record = record("A5_hawking_structured_cognition", lambda: controller.execute(prompt))
            ...
            if isinstance(structured, dict):
                report["stages"].append(
                    _stage(
                        "A5_hawking_structured_cognition",
                        config=config,
    ...
    def run_cli() -> Dict[str, Any]:
        executable = shutil.which("hawking")
        if executable:
            command = [executable]
        else:
            command = [sys.executable, "-m", "hawking"]
        command.extend(["--model", str(profile_path), "--task", prompt])
        env = os.environ.copy()
        env["HAWKING_MODEL_TOKENS"] = str(max(1, int(model_tokens)))
        env["HAWKING_STRUCTURED_OUTPUT_ATTEMPTS"] = "1"
...
def _hawking_gate_admits(gate, capability):
    """Return True only when the protected gate admits the capability.

    Executable behavior: a capability is admitted iff the gate is not withheld
    and the capability is present in the gate's sealed capability set. This is
    the narrowest missing executable check; it does not enable hardware or
    device execution.
    """
    if getattr(gate, "withheld", False):
        return False
    sealed = getattr(gate, "sealed_capabilities", None)
    if sealed is None:
        return False
    return capability in sealed