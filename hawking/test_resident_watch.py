...
from hawking.resident import ResidentConfig, ResidentDaemon, watch_resident
...
def test_watch_opens_no_runtime_and_no_model(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("hawking.resident.time.sleep", stop_after_one)
...
def test_watch_renders_the_live_state(tmp_path, monkeypatch, capsys):
    ...
    assert "HAWKING RESIDENT" in out
