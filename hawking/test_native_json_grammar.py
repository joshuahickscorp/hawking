...
def test_the_contract_sends_json_when_the_backend_honours_a_grammar():
    ...
    it.
    """
    from hawking.backends import make_structured_output_contract
    ...
    honours = make_structured_output_contract(Backend(True), HAWKING_RESULT_SCHEMA)
    ...
    cannot = make_structured_output_contract(Backend(False), HAWKING_RESULT_SCHEMA)
...
def test_the_sealed_profile_declares_only_what_the_resident_implements():
    ...
    profile = json.loads(
        (Path(__file__).resolve().parents[1] / "hawking/hawking-native.sealed-3.14.json")
