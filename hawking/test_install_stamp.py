"""`hawking` execs a copy under ~/.local/share/hawking/current, not the repo. On
2026-09-01 that symlink pointed at a 2026-08-27 snapshot while the working
tree was 2026-08-31, so `hawking` and `PYTHONPATH=. python3 -m hawking` ran two
...
def _prep(tmp_path, monkeypatch):
    ...
    source we can then edit. It deploys ``Path(hawking.cli.__file__).parent``."""
    src = tmp_path / "repo" / "hawking"
    src.mkdir(parents=True)
    (src / "__init__.py").write_text("", encoding="utf-8")
    (src / "cli.py").write_text("VERSION = 1\n", encoding="utf-8")
    monkeypatch.setattr("hawking.cli.__file__", str(src / "cli.py"))
    return src, tmp_path / "home" / ".local" / "share" / "hawking"
...
def test_A_MATCHING_INSTALL_SAYS_NOTHING(tmp_path, monkeypatch, capsys):
    ...
    capsys.readouterr()
    monkeypatch.setattr("hawking.cli.__file__", str(share / "current" / "hawking" / "cli.py"))
    warn_if_stale()
...
def test_THE_STAMP_RECORDS_WHAT_WAS_DEPLOYED(tmp_path, monkeypatch):
    ...
    assert stamp["native_hawking"]["status"] == "unavailable"
    assert stamp["native_process_authority"]["status"] == "unavailable"
