...
closing ``hawking`` or starting a resident does not make the queue disappear.

The file is deliberately small and boring JSON. The lock serializes multiple
HAWKING front ends, while atomic_write_json makes a crash leave the previous
...
GOAL_BANK_SCHEMA = "hawking.goal_bank.v1"
...
class GoalBank:
    ...
    def __init__(self, workspace: str | os.PathLike[str]):
        ...
        self.directory = self.workspace / ".hawking"
    ...
    def _lock(self) -> Iterator[None]:
        """Serialize read-modify-write operations across local HAWKING clients."""
    ...
    def recover_inflight(self) -> int:
        """Return goals from dead HAWKING owners to the FIFO queue."""
        recovered = 0
        with self._lock():
            ...
            for item in data["goals"]:
                ...
                item["last_error"] = "previous HAWKING owner ended; goal returned to the bank"
                recovered += 1
            if recovered:
