...
SCHEMA = "hawking.resident_gate.v1"
DEFAULT_COUNT = 20
DEFAULT_PROMPTS = (
...
def record_resident_causality(report: Dict[str, Any], **kwargs: Any) -> dict[str, Any]:
    ...
    return sc.stamp_gate(
        report,
        source="hawking/resident_gate.py::run_resident_gate",
        **payload,
    )
...
def _write_receipt(report: Dict[str, Any], emit: Optional[str], repo_root: Path) -> None:
    destination = Path(emit).expanduser() if emit else repo_root / "receipts" / "headless" / "HAWKING_RESIDENT_GATE.json"
...
def run_resident_gate(
    ...
    *,
    ...
    old_tokens = os.environ.get("HAWKING_MODEL_TOKENS")
    ...
    try:
        ...
        os.environ["HAWKING_MODEL_TOKENS"] = str(model_tokens)
        ...
        if old_tokens is None:
            os.environ.pop("HAWKING_MODEL_TOKENS", None)
        else:
            os.environ["HAWKING_MODEL_TOKENS"] = old_tokens
