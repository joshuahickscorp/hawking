...
def evaluate(
    ...
    mission = _read_json(hawking / "mission" / "state.json")
    daemon = _read_json(hawking / "resident" / "state.json")
    dag = _read_json(hawking / "dag.json")
    events = _read_events(hawking / "mission" / "events.jsonl")
    log = _read_log(hawking / "mission" / "mission.log")
    ...
    if mission is None:
        ...
        from .resident import BLOCKED_MISSION_PHASES
    ...
    receipts = []
    if (hawking / "receipts").is_dir():
        for path in (hawking / "receipts").glob("*.json"):
            try:
    ...
    return {
        "schema": "hawking.cycle_verdict.v1",
        "verdict": verdict,
        "measured": measured,
        "budgets": budgets,
        "failed": failed,
