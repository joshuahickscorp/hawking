...
Hawking control plane: no queue, no retry, no deliver_promoted. LABORATORY
profile is refused. Hawking owns its five-axis scorer; Tabula consumes that
contract for science work but is not a runtime dependency.
...
from hawking.perception.file_eye import observe as file_observe
from hawking.perception.terminal import capture as pty_capture
from hawking.perception.receipts import content_digest, sha256_bytes, utc_now
from hawking.perception.doctor import profile as doctor_profile
...
# The behavior matrix is the runtime owner of its own five-axis evidence
# vector. Tabula consumes this exact contract for science work, but the normal
# Hawking import path must not load Tabula's optional numerical experiment
# stack.
...
NON_BEHAVIORAL_AXES: tuple[str, ...] = tuple(
    axis for axis in SCORE_AXES if axis != "behavioral"
)
...
class IncompleteScoreVector(ValueError):
    """A behavior score omitted an independently required axis."""
    ...
    def from_mapping(cls, mapping: Mapping[str, Any]) -> "ScoreVector":
        ...
        if missing:
            raise IncompleteScoreVector(
                "missing axes "
                + repr(missing)
                + "; independent behavior evaluation requires "
                + ",".join(SCORE_AXES)
            )
        non_numeric = [
            axis
            for axis in SCORE_AXES
            if not isinstance(mapping[axis], (int, float))
        ]
        if non_numeric:
            raise IncompleteScoreVector(f"non-numeric axes {non_numeric}")
...
def evaluate(
    ...
    *,
    ...
    """Require a behavioral result without regressions in independent axes."""
    vector = scores if isinstance(scores, ScoreVector) else ScoreVector.from_mapping(scores)
    body = vector.to_dict()
    target_hit = float(vector.behavioral) >= float(behavioral_target)
    regressions = tuple(
        axis
        for axis in NON_BEHAVIORAL_AXES
        if float(getattr(vector, axis)) < float(regression_limit)
    )
    if target_hit and regressions:
        return Verdict(
            outcome="FAILURE",
            target_hit=True,
            ...
            reason=(
                ...
                + ",".join(regressions)
                + "; independent capability evidence prevents a behavioral-only pass"
            ),
            scores=body,
        )
    if not target_hit:
...
def scores_from_behavior_lab(rows: Sequence[Mapping[str, Any]]) -> ScoreVector:
    """Convert actual fixture evidence into the shared behavior score vector."""
    if not rows:
        raise IncompleteScoreVector(
            "behavior lab produced no fixtures; independent evaluation requires "
            + ",".join(SCORE_AXES)
        )
    count = float(len(rows))

    def fraction(predicate: Callable[[Mapping[str, Any]], bool]) -> float:
        return sum(1.0 for row in rows if predicate(row)) / count

    behavioral = fraction(lambda row: bool(row.get("ran")) and not row.get("blocked"))
    capability = fraction(
        lambda row: bool(row.get("goal_met") or row.get("ok"))
        and not row.get("empty_success")
    )
    tool_use = fraction(
        lambda row: bool(row.get("tool_receipt_ok"))
        and not row.get("empty_success")
    )
    reasoning_rows = [
        row for row in rows if str(row.get("id")) in _BHV_REASONING_IDS
    ]
    reasoning = (
        sum(1.0 for row in reasoning_rows if row.get("reasoning_ok"))
        / float(len(reasoning_rows))
        if reasoning_rows
        else capability
    )
    instruction_rows = [
        row for row in rows if str(row.get("id")) in _BHV_INSTRUCTION_IDS
    ]
    instruction_following = (
        sum(1.0 for row in instruction_rows if row.get("instruction_ok"))
        / float(len(instruction_rows))
        if instruction_rows
        else capability
    )
    return ScoreVector(
    ...
    def git(self, *git_args: str) -> dict[str, Any]:
        rec = doctor_profile(
            [
                "git",
                "-c",
                "user.email=perception-lab@local",
                "-c",
                "user.name=perception-lab",
                "-c",
    ...
    *,
    ...
    body = {
        ...
        "tool_receipt_ok": bool(lab.tool_trace) and all(
            r.get("schema") == "hawking.perception.tool_receipt.v1" for r in lab.tool_trace
        ),
...
def _bhv07(lab: _Lab) -> dict[str, Any]:
    ...
    initial = lab.head()
    lab.wu("missing-tool")
    missing = doctor_profile(["perception-definitely-not-a-tool-xyz"])
...
def run_fixture(fid: str, *, root: Path | None = None) -> dict[str, Any]:
    ...
    if root is None:
        with tempfile.TemporaryDirectory(prefix=f"perception-{fid}-") as tmp:
            lab = _Lab(Path(tmp))
            return fn(lab)
    lab = _Lab(Path(root))
    return fn(lab)
...
def run_matrix(
    ...
    *,
    ...
    # Independent five-axis evaluation belongs to this behavior kernel. Tabula
    # consumes this exact contract rather than becoming a runtime dependency.
    ...
    return {
        ...
        "note": (
            ...
            "scores evaluated by the PERCEPTION behavior kernel"
        ),
    }
