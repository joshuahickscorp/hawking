"""HAWKING's decision surface for "what does ModelLake acquire next, and why."

``acquire-next`` (tools/odyssey_ctl.py, wrapped read-only by hawking/odyssey.py)
...
HAWKING_LEDGER.json, or partial/ -- it only reads three places an acquisition
...
``hawking.odyssey.acquire_next(confirm=True)`` -- exactly as costly a step as
it already was.
...
subprocess/text-parsing wrapper hawking/odyssey.py uses elsewhere in this
...
def propose(
    ...
    *,
    ...
    """Rank the ladder's queued acquisitions HAWKING could act on next.
