...
`retire_incumbent` moved `.hawking/mission/` and left `.hawking/dag.json` in place.
...
Measured on the live daemon. `.hawking/dag.json` held signatures for eleven units
...
from hawking.resident import ResidentConfig, ResidentDaemon, retire_incumbent
from hawking.workunit import rebuild_repair_budget


def _seed(tmp_path, *, dag: bool = True):
    mission = tmp_path / ".hawking" / "mission"
    mission.mkdir(parents=True)
    (mission / "state.json").write_text(json.dumps({"id": "old", "phase": "cancelled"}))
    if dag:
        (tmp_path / ".hawking" / "dag.json").write_text(json.dumps({
            "version": 1,
            "units": {},
...
def test_the_dag_is_archived_beside_the_mission(tmp_path):
    ...
    assert not (tmp_path / ".hawking" / "dag.json").exists(), (
