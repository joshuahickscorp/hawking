"""Bounded, resumable Gravity search owned by HAWKING.
...
SCHEMA = "hawking.odyssey.gravity_gauntlet.v1"
...
def _spec_bits(spec: str) -> int | None:
    ...
    if not m:
        return None
    bits = int(m.group(1))
    # A precision of zero bits is not a representation: it cannot store any
    # magnitude, so treating it as a measured candidate would let a degenerate
    # spec (e.g. "q0") enter the frontier as if it were runnable. Reject it the
    # same way an absent precision is rejected.
...
class GravityGauntlet:
    ...
    def _load_or_init(self) -> dict[str, Any]:
        ...
        state = {
            "schema": SCHEMA,
            "authority": "HAWKING single writer; verifier decides target status",
...
def run_patient_runner(state_path: str | Path, specimen: str, specs: Iterable[str], budget: int,
    ...
    return engine.run(evaluator)
def gravity_gauntlet_score(samples):
    """Return the mean residual magnitude across gravity samples.

    Each sample must provide a numeric ``residual`` field. The score is the
    arithmetic mean of ``abs(residual)`` over all samples. An empty input
    yields ``0.0`` so callers get a defined, observable result.
    """
    total = 0.0
    count = 0
    for sample in samples:
        total += abs(float(sample["residual"]))
        count += 1
    if count == 0:
        return 0.0
    return total / count