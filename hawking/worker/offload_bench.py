...
SCHEMA = "hawking.worker.offload_bench.v4"
...
def _t_grep_proposed(tmp: Path) -> tuple[bool, str, str | None]:
    result, skip, produced = _act_or_skip(
        tmp,
        "Search the Hawking worker tree at hawking/worker for the symbol proposed_complete.",
        known_tools=["grep"],
    )
    if result is None:
        return False, skip, produced
...
def _t_pytest_option_c(tmp: Path) -> tuple[bool, str, str | None]:
    result, skip, produced = _act_or_skip(
        tmp,
        "Run the Option-C unit tests at hawking/tests/test_worker_option_c.py.",
        known_tools=["pytest"],
    )
    if result is None:
        return False, skip, produced
...
def _t_machine_state(tmp: Path) -> tuple[bool, str]:
    script = REPO_ROOT / "hawking" / "machine_state.py"
...
def _t_seal_receipt(tmp: Path) -> tuple[bool, str]:
    from hawking.receipts import seal as _seal
...
def _t_plan_g015(tmp: Path) -> tuple[bool, str]:
    ...
    need = {"session_context", "grok_delegate", "grok_consume", "hawking_verify", "resource_pause"}
...
def _t_model_grep_verified(tmp: Path) -> tuple[bool, str, str | None]:
    result, skip, produced = _act_or_skip(
        tmp,
        "Search hawking/worker for the symbol verified_complete.",
        known_tools=["grep"],
    )
    if result is None:
        return False, skip, produced
...
def _t_grep_nativedecode_file_line(tmp: Path) -> tuple[bool, str, str | None]:
    result, skip, produced = _act_or_skip(
        tmp,
        "Search hawking/worker for the symbol NativeDecodeRefused and report the "
        "first file:line hit.",
        known_tools=["grep"],
    )
    if result is None:
...
TASKS: list[OffloadTask] = [
    ...
    OffloadTask(
        ...
        "Search the Hawking worker tree for proposed_complete",
        "hawking/worker/special_unit.py (this lane's first recon move, same shape as every Claude grep)",
        True,
        _t_grep_proposed,
        PRODUCER_MODEL,
    ),
    OffloadTask(
        "run_option_c_tests",
        "Run hawking/tests/test_worker_option_c.py",
        "hawking/tests/test_worker_option_c.py — standing Option-C suite Claude already maintains",
        True,
        _t_pytest_option_c,
        PRODUCER_MODEL,
    ),
    ...
    OffloadTask(
        ...
        "Model-driven search of hawking/worker for verified_complete",
        "hawking/worker/special_unit.py",
        True,
    ...
    OffloadTask(
        ...
        "hawking/worker/special_unit.py NativeDecodeRefused",
        True,
    ...
    OffloadTask(
        ...
        "Query hawking/machine_state.py",
        "hawking/machine_state.py + receipts/agentos/PROGRAM_PLAN.md W0.T10",
        True,
        _t_machine_state,
        PRODUCER_HARNESS,
    ),
    ...
    OffloadTask(
        ...
        "G015 Hawking worker interrupt + restart/resume",
        True,
        _t_interrupt_resume,
        PRODUCER_HARNESS,
    ),
