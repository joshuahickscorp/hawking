...
``hawking.genesis_resident`` client protocol on an explicit worker
...
``hawking.receipts.seal``, ``hawking.machine_state.py``. This is not a
...
from hawking.execution_sandbox import (
...
from hawking.receipts import seal
from hawking.verification_authority import (
    AuthorityPrincipal,
    CandidateKind,
    SelfPromotionError,
...
SCHEMA = "hawking.worker.special_unit.v1"
SESSION_SCHEMA = "hawking.worker.special_unit.session.v1"
PLAN_SCHEMA = "hawking.worker.special_unit.plan.v1"
CONSUMPTION_SCHEMA = "hawking.worker.special_unit.grok_consumption.v1"
...
# Continuity worker homes (hawking.lineage.continuity.WORKER_SESSION_ROLES).
...
def project_context(repo: Path, *, lock_path: Path = GPU_LOCK) -> dict[str, Any]:
    ...
    ctx = {
        "schema": "hawking.worker.special_unit.project_context.v1",
        "repo": str(repo),
        ...
        "model_identity": {
            ...
            "native_leg": "DONE",
            "harness": "hawking.worker.special_unit",
        },
    }
...
class ToolExecutor:
    ...
    def _pytest(self, args: Mapping[str, Any]) -> ToolResult:
        target = str(args.get("target") or "hawking/tests/test_worker_option_c.py")
        self.policy.require(
    ...
    def _prepare_candidate(self, args: Mapping[str, Any]) -> ToolResult:
        ...
        from hawking.lineage.lifecycle import LifecycleError, build_candidate_request
        ...
        try:
            request = build_candidate_request(
                ...
                world_state={"hawking_worktree": str(self.repo)},
            )
    ...
    def _submit_candidate(self, args: Mapping[str, Any]) -> ToolResult:
        ...
        try:
            from hawking.lineage.lifecycle import CandidateInbox, LifecycleError
...
# ---------------------------------------------------------------------------
# Hawking worker plan + proposed_complete vs verified_complete
# ---------------------------------------------------------------------------
...
def default_g015_steps() -> list[dict[str, Any]]:
    ...
    return [
        {
            "id": "session_context",
            "title": "Hawking worker conversation + project context + persistent session",
            "dependencies": [],
        ...
        },
        {
            ...
            "dependencies": ["session_context"],
            "oracle": {"kind": "file_exists", "path": "hawking/worker/special_unit.py"},
        },
        {
            "id": "build_test",
            "title": "build/test execution",
            "dependencies": ["tool_execution"],
            "oracle": {"kind": "pytest", "target": "hawking/tests/test_worker_option_c.py"},
        },
        {
            "id": "grok_delegate",
        ...
        {
            "id": "hawking_plan",
            "title": "Hawking worker planning",
            "dependencies": ["session_context"],
            "oracle": {"kind": "plan_present"},
        },
        {
            "id": "hawking_verify",
            "title": "Hawking worker verification (proposed_complete vs verified_complete)",
            "dependencies": ["hawking_plan"],
...
class ResidentClient(Protocol):
    """Subset of ``hawking.genesis_resident.propose`` this adapter calls."""
...
def _load_resident_propose() -> Any:
    ...
    propose = getattr(genesis_resident, "propose", None)
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m hawking.worker.special_unit")
