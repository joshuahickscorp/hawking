"""The campaign tools must be REACHABLE from HAWKING, not merely written.
...
non-test references anywhere in hawking/. HAWKING could select a specimen and report
...
def test_the_ledger_can_name_what_is_still_owed(registry, tmp_path):
    """HAWKING must be able to choose its next target from disk, unaided.
