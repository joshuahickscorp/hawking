...
- The durable owners are HAWKING, Hawking, Gravity, AgentOS, Nova, Noetic, NX, and
...
- `civilization/sovereign-goal.txt`, HAWKING goal/mission state, and the current
...
Use HAWKING for durable or bounded work that should survive a model session:

- `python3 -m hawking --help`
- Before a resident-backed delegation, verify the actual OpenAI endpoint. On
  this machine the live HAWKING resident surface was `http://127.0.0.1:8011/v1`
  while `:8080` was the Open WebUI surface and returned HTTP 405 to delegation
  POSTs. Set `HAWKING_ENDPOINT` explicitly when needed:
  `HAWKING_ENDPOINT=http://127.0.0.1:8011/v1/chat/completions python3 -m hawking run ...`
- `python3 -m hawking run --goal "..." --verify strict --root "$PWD"`
- `python3 -m hawking status <mission>` and `python3 -m hawking result <mission>`
- `python3 -m hawking steer <mission> "..." --kind knowledge|correction|constraint`
- HAWKING's `/ultragoal`, `/mission`, `/steer`, `/grok`, and `/receipts` commands
...
- Start a new Grok V2 mission with `/grok mission path/to/mission.md BALANCED
  ...
  invocation in `.hawking/grok/mission-*.json`; V2 stdout and `/grok verify` are
  provider evidence, not HAWKING acceptance. HAWKING must independently inspect the
...
HAWKING's protected-command guard may also refuse an interpreter command such as
...
- Use Codex's native parallel/task surfaces for independent reasoning when they
  ...
  sake.
- HAWKING's Grok bridge and the existing
