...
"""Teacher-forced, layer-major donor activation executor (architecture-config driven).

Generalized for PROTO_FRANKENSTEIN multi-stage inheritance:
  Stage 1 donor = GLM-5.2 (math) → Proto
  Stage 2 donor = Kimi-K3 (strategic/agentic) → Final

This is NOT a chat/decode server.  It freezes a batch of sequences, streams one
donor layer at a time over ALL sequences, captures bounded hidden-state evidence,
...
Architecture (repo, revision, hidden size, layer count, attention/MoE variant,
schemas, reference backend) is a ``DonorArchitecture`` config object — never
hardcoded as the only supported model.  GLM-5.2 remains the default binding and
must reproduce sealed GLM capture receipts byte-for-byte.

Honesty: never fabricates activations.  Official donor bodies require resident
source shards; when absent the executor fail-closes and reports the deepest
verified layer (synthetic full-stack proves mechanism end-to-end for backends
that ship a miniature twin).
"""
...
from frankenstein_trace_format import (
    MEMBERSHIP_SPLITS,
    MembershipManager,
    build_paired_trace,
    empty_side,
...
# ---------------------------------------------------------------------------
# Donor architecture config (model-specific constants live here, not inline)
# ---------------------------------------------------------------------------


@dataclass(frozen=True)
class DonorArchitecture:
    """Pinned donor geometry + IO contracts for teacher-forced capture.

    Numbers for official donors must come from admission / config.json — do not
    invent.  Synthetic profile may use miniature geometry loaded from fixture
    config.json; the fields below are the official-scale defaults used for
    planning and receipt identity when the body is absent.
    """

    family: str
    repo_id: str
    immutable_revision: str
    profile_official: str
    profile_synthetic: str
    hidden_size: int
    ...
    torch_dtype: str
    schema_receipt: str
    schema_layer_shard: str
    schema_corpus: str
    schema_eviction: str
    schema_carry: str
    receipt_filename: str
    paired_side_name: str
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "profile_official": self.profile_official,
            "profile_synthetic": self.profile_synthetic,
            ...
            "weight_shard_bytes": self.weight_shard_bytes,
...
# Official GLM-5.2 (Stage-1 mathematical donor). Geometry matches sealed
# zai-org/GLM-5.2 admission; synthetic fixtures override via config.json.
GLM52_ARCHITECTURE = DonorArchitecture(
    family="glm52",
    repo_id=REPO_ID,
    immutable_revision=IMMUTABLE_REVISION,
    profile_official=PROFILE_OFFICIAL,
    profile_synthetic=PROFILE_SYNTHETIC,
    hidden_size=6144,
    num_hidden_layers=78,
    n_routed_experts=256,
    num_experts_per_tok=8,
    n_shared_experts=1,
    attention_variant="mla_dsa_indexshare",
    moe_variant="noaux_tc_swiglu",
    model_type="glm_moe_dsa",
    architecture_class="GlmMoeDsaForCausalLM",
    ...
    schema_carry="hawking.frankenstein.glm_carry_state.v1",
    receipt_filename="GLM_TEACHER_FORCED_CAPTURE_RECEIPT.json",
    paired_side_name="glm",
    ...
    vocab_size=154_880,
    tokenizer_kind="hf_sentencepiece",
    notes=(
        "Stage-1 mathematical donor for PROTO_FRANKENSTEIN_V0.",
        "Reference path reuses glm52_reference (sealed parity vs Transformers).",
    ),
)

# Official Kimi-K3 (Stage-2 strategic donor). Numbers from HF config.json at the
# pinned admission revision (see KIMI_K3_SOURCE_ADMISSION.json). No weights
# streamed here — architecture identity only.
KIMI_K3_ARCHITECTURE = DonorArchitecture(
    family="kimi_k3",
    repo_id="moonshotai/Kimi-K3",
    immutable_revision="9f62e4e9fffbd0a83ddd60e1c209d828994b3569",
    profile_official="kimi_k3_official",
    profile_synthetic="kimi_k3_synthetic",
    hidden_size=7168,
    num_hidden_layers=93,
    n_routed_experts=896,  # text_config.num_experts in config.json
    num_experts_per_tok=16,  # text_config.num_experts_per_token
    n_shared_experts=2,  # text_config.num_shared_experts
    attention_variant="kimi_linear_hybrid_mla_kda",  # full_attn_layers + kda_layers
    moe_variant="noaux_tc_situ_latent_moe",
    model_type="kimi_k3",
    architecture_class="KimiK3ForConditionalGeneration",
    torch_dtype="bfloat16",
    schema_receipt="hawking.frankenstein.kimi_teacher_forced_capture.v1",
    schema_layer_shard="hawking.frankenstein.kimi_layer_capture_shard.v1",
    schema_corpus="hawking.frankenstein.kimi_frozen_corpus.v1",
    schema_eviction="hawking.frankenstein.kimi_layer_eviction.v1",
    schema_carry="hawking.frankenstein.kimi_carry_state.v1",
    receipt_filename="KIMI_TEACHER_FORCED_CAPTURE_RECEIPT.json",
    paired_side_name="kimi",
    reference_module="PENDING_KIMI_REFERENCE_BACKEND",
    exact_ops=(
        "rmsnorm",
        "kimi_linear_attn_kda",
        "mla_full_attn",
        "noaux_tc_router",
        "situ_moe",
    ),
    ...
    notes=(
        "Stage-2 strategic/agentic donor. Backend NOT implemented in this prep lane.",
        "MoE counts from HF config.json text_config (not top-level; admission "
        "n_routed_experts was null because extractor did not descend text_config).",
        "Do not stream/download weights from this architecture object alone.",
    ),
)

# GLM schema aliases (backward-compatible module-level names used by GLM path).
SCHEMA_RECEIPT = "hawking.frankenstein.glm_teacher_forced_capture.v1"
SCHEMA_LAYER_SHARD = "hawking.frankenstein.glm_layer_capture_shard.v1"
SCHEMA_CORPUS = "hawking.frankenstein.glm_frozen_corpus.v1"
SCHEMA_EVICTION = "hawking.frankenstein.glm_layer_eviction.v1"
...
FRANK_EVIDENCE = REPO_ROOT / "hawking-experiments" / "frankenstein" / "data"
...
def _freeze_official_merged_corpus(
    *,
    ...
    n = CORPUS_LEVELS[level]
    default_l0 = (
        REPO_ROOT / "hawking-experiments" / "frankenstein" / "data"
        / "corpus"
...
def run_teacher_forced(cfg: ExecutorConfig) -> dict[str, Any]:
    ...
    arch = cfg.architecture or GLM52_ARCHITECTURE
    if arch.backend != "glm52":
        raise TeacherForcedError(
            f"teacher-forced backend {arch.backend!r} is not implemented in this "
            f"lane (architecture family={arch.family}). Prep-only: use "
            f"GLM52_ARCHITECTURE until the Kimi reference backend lands after V0 seal."
        )
    profile = cfg.profile
    ...
    for layer in range(n_layers):
        ...
        if use_stream and streamer is not None:
            try:
                ...
                deepest_blocker = (
                    f"hawking-experiments/frankenstein/operators/frankenstein_teacher_forced_executor.py:"
    ...
    if len(layers_captured) == n_layers:
        try:
            ...
            if deepest_blocker is None:
                deepest_blocker = (
                    f"hawking-experiments/frankenstein/operators/frankenstein_teacher_forced_executor.py:final_norm "
    ...
    receipt = seal(
        {
            "schema": arch.schema_receipt,
            "status": status,
            "mode": cfg.mode,
            "profile": profile,
            "repo": (
                arch.repo_id
                if profile == arch.profile_official
                else (
                    "synthetic/GLM-5.2-twin"
                    if arch.family == "glm52"
                    else f"synthetic/{arch.family}-twin"
                )
            ),
            "revision": arch.immutable_revision
            if profile == arch.profile_official
            else "synthetic-revision",
            "architecture_binding": arch.as_dict(),
            "architecture": {
            ...
            "forward": {
                ...
                "reference_module": arch.reference_module,
                "exact_ops": list(arch.exact_ops),
                "verification": (
    ...
    atomic_json(out / arch.receipt_filename, receipt)
    return receipt
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Teacher-forced layer-major donor activation capture (architecture-config)"
    )
    parser.add_argument(
        "--mode",
