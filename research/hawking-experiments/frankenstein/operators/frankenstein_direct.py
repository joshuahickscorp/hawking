...
def default_paths(
    *,
    ...
    # Custom --out-dir keeps the historical campaign-relative output layout.
    # Default (no --out-dir) writes into the moved frankenstein data tree.
    evidence = (
        root / "evidence" / "models" / "frankenstein"
        if out_dir is not None
        else (REPO_ROOT / "hawking-experiments" / "frankenstein" / "data")
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "fusion-spec":
            ...
            from frankenstein_transfer import run_weight_only_transfer
