"""Recomposed science module quality_contract (C-SCI-R1)."""
...
CONTRACT = {'schema': 'hawking.foundry.quality_contract.v1', 'max_mean_symmetric_kl': 0.1, 'min_argmax_agreement': 0.95, 'min_capability_tokens': 1000}
_BOUND_KIND = {'max_mean_symmetric_kl': 'upper', 'min_argmax_agreement': 'lower', 'min_capability_tokens': 'lower'}
...
def assert_not_weakened(proposed: dict, sealed_sha256: str=SEALED_CONTRACT_SHA256) -> dict:
    ...
    if weakened:
        raise ContractViolation('quality thresholds may not be weakened: ' + '; '.join(weakened))
    return {'ok': True, 'change': 'tightened' if tightened else 'non_threshold_edit', 'tightened': tightened, 'sealed_sha256': contract_hash(proposed)}
EVIDENCE_CLASSES = ('PHYSICAL', 'FUNCTIONAL_PROXY', 'LAYER', 'SHORT_END_TO_END', 'CAPABILITY')
...
def classify(evidence: dict) -> str:
    ...
    if real_forward and has_gate_metrics and (ev.get('split') == 'holdout') and (tokens >= CONTRACT['min_capability_tokens']) and (set(ev.get('domains') or ()) >= set(PROTECTED_QUALITY_DOMAINS)):
        return 'CAPABILITY'
...
def assert_not_overclaimed(claim: str, evidence: dict) -> dict:
    ...
    if _RANK[claim] > _RANK[supported]:
        raise ContractViolation(f'overclaim: reported {claim} but the evidence only supports {supported}')
...
def evaluate(metrics: dict, evidence: dict=None) -> dict:
    ...
    if kl is None or agree is None:
        raise ContractViolation('gate needs both mean_symmetric_kl and argmax_agreement; partial metrics are not a gate')
    ...
    return {'schema': SCHEMA_EVALUATION, 'passed': kl_ok and agree_ok, 'mean_symmetric_kl': kl, 'argmax_agreement': agree, 'kl_ok': kl_ok, 'argmax_ok': agree_ok, 'recorded': {'logit_cosine': metrics.get('logit_cosine'), 'top5_overlap': metrics.get('top5_overlap'), 'candidate_perplexity': metrics.get('candidate_perplexity')}, 'evidence_class': supported, 'may_select_frontier': (kl_ok and agree_ok) and supported == 'CAPABILITY', 'contract_sha256': SEALED_CONTRACT_SHA256}
