"""Recomposed science module glm52_teacher_capture (C-SCI-R1)."""
from __future__ import annotations
from lab.operators import glm52_reference as reference
from lab.operators.glm52_adapter import IMMUTABLE_REVISION
from lab.operators.glm52_adapter import OFFICIAL_INDEXER_TYPES
from lab.operators.glm52_adapter import PROFILE_OFFICIAL
from lab.operators.glm52_adapter import load_json_strict
from lab.operators.glm52_adapter import validate_config
from lab.operators.glm52_common import Glm52Error
from lab.operators.glm52_common import atomic_bytes
...
def official_config() -> dict[str, Any]:
    ...
    if not path.exists():
        raise TeacherCaptureError(f'pinned config.json is absent: {path} (fetch it at revision {IMMUTABLE_REVISION} before capture)')
    ...
    resolved['mlp_layer_types'] = ['dense' if layer < geometry.first_k_dense_replace else 'sparse' for layer in range(geometry.num_hidden_layers)]
    return resolved
...
def _tensor_table(graph: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for tensor in graph['tensors']:
        if tensor.get('alias_of'):
            continue
        table[tensor['name']] = {'shard': tensor['shard'], 'absolute_start': int(tensor['absolute_start']), 'payload_bytes': int(tensor['payload_bytes']), 'dtype': tensor['dtype'], 'shape': tuple((int(value) for value in tensor['shape']))}
    return table
...
class ShardTensorSource:
    ...
    def __init__(self, root: Path, table: dict[str, dict[str, Any]], *, max_tensor_bytes: int=MAX_TENSOR_BYTES) -> None:
        self.root = Path(root)
        self.table = table
    ...
    def _pread(self, record: dict[str, Any], count: int, offset: int, name: str) -> bytes:
        ...
        try:
            ...
            while remaining:
                ...
                if not block:
                    raise TeacherCaptureError(f'short read for {name}: wanted {count} at {offset} in {path.name}')
                chunks.append(block)
                remaining -= len(block)
                position += len(block)
        finally:
    ...
    def tensor(self, name: str) -> np.ndarray:
        ...
        if record['payload_bytes'] > self.max_tensor_bytes:
            raise TeacherCaptureError(f'bounded read refused {name}: {record['payload_bytes']} bytes exceeds {self.max_tensor_bytes}')
    ...
    def rows(self, name: str, ids: Iterable[int]) -> np.ndarray:
        ...
        for slot, row in enumerate(wanted):
            payload = self._pread(record, stride, record['absolute_start'] + int(row) * stride, name)
...
def calibration_ids(split: str, *, vocab_size: int, tokens: int=CALIBRATION_TOKENS) -> np.ndarray:
    ...
    if not is_math and split not in program.SPLIT_PARTITIONS:
        raise TeacherCaptureError(f'unknown calibration split: {split!r}; expected one of {sorted(program.SPLIT_PARTITIONS)} or {MATH_CALIBRATION_SPLIT!r}')
...
def _capsule_note(vocab_size: int) -> str:
    ...
    if calibration_source(vocab_size) == 'NATURAL_CORPUS_PARTITION':
        return 'Real BF16 block evidence on a bounded batch of real corpus records, disjoint by split and covering every declared domain. Hundreds of tokens, not a benchmark: not an evaluation, not a capability result.'
    return 'Fixture capture on a synthetic probe against a miniature vocabulary. Machinery evidence only. Never evidence about GLM-5.2.'

def calibration_source(vocab_size: int) -> str:
    """Which calibration path a capture at this vocabulary will take."""
    return 'NATURAL_CORPUS_PARTITION' if vocab_size >= program.corpus.TOKENIZER_VOCAB_SIZE else 'SYNTHETIC_FIXTURE_PROBE_NOT_EVIDENCE'
...
def _fixture_probe(split: str, *, vocab_size: int, tokens: int) -> np.ndarray:
    ...
    stream = hashlib.sha256(canonical({'revision': IMMUTABLE_REVISION, 'split': split, 'tokens': tokens})).digest()
    ids: list[int] = []
    counter = 0
...
def membership_sha256(ids: np.ndarray, split: str) -> str:
    return hashlib.sha256(canonical({'split': split, 'token_ids': np.asarray(ids).tolist()})).hexdigest()
...
def _layer_metrics(arrays: dict[str, np.ndarray]) -> dict[str, float]:
    ...
    metrics = {'block_output_l2': float(np.sqrt(np.sum(output * output, dtype=np.float64))), 'block_output_absmax': float(np.max(np.abs(output))), 'block_output_mean': float(np.mean(output, dtype=np.float64)), 'attention_output_l2': float(np.sqrt(np.sum(_finite(arrays['attention_output']) ** 2, dtype=np.float64))), 'post_moe_l2': float(np.sqrt(np.sum(_finite(arrays['post_moe']) ** 2, dtype=np.float64)))}
    if 'router_logits' in arrays:
        logits = _finite(arrays['router_logits'])
        margin = _finite(arrays['topk_margin_8th_vs_9th'])
        metrics.update({'router_logit_absmax': float(np.max(np.abs(logits))), 'router_selected_expert_count': float(len(set(np.asarray(arrays['topk_indices']).ravel().tolist()))), 'topk_margin_mean': float(np.mean(margin, dtype=np.float64)), 'topk_margin_min': float(np.min(margin)), 'shared_output_l2': float(np.sqrt(np.sum(_finite(arrays['shared_expert_output']) ** 2, dtype=np.float64))), 'routed_output_l2': float(np.sqrt(np.sum(_finite(arrays['routed_expert_output']) ** 2, dtype=np.float64)))})
    if 'expert_contribution_l2' in arrays:
        ...
        hit_experts = hit > 0
        metrics.update({'experts_hit_this_batch': float(np.count_nonzero(hit_experts)), 'expert_contribution_l2_max': float(np.max(contribution)) if hit_experts.any() else 0.0, 'expert_contribution_l2_mean_over_hit': float(np.mean(contribution[hit_experts])) if hit_experts.any() else 0.0})
    return {key: value for key, value in sorted(metrics.items())}

def _router_margin(router_logits: np.ndarray, correction_bias: np.ndarray, top_k: int) -> np.ndarray:
...
def _expert_cartography_arrays(per_expert: dict[int, dict[str, np.ndarray]], n_routed_experts: int) -> dict[str, np.ndarray]:
    ...
    for expert, data in per_expert.items():
        ...
        contribution_l2[expert] = float(np.sqrt(np.sum(weighted.astype(np.float64) ** 2)))
    ...
    return {'expert_contribution_l2': contribution_l2, 'expert_hit_count': hit_count, 'expert_coselection_count': coselection_count}

def capture_layer(hidden: np.ndarray, source: Any, layer: int, config: dict[str, Any], previous_topk: np.ndarray | None, cache: reference.ReferenceCache, *, retain_per_expert: bool=False) -> tuple[np.ndarray, np.ndarray, dict[str, np.ndarray]]:
    ...
    """
    positions = np.broadcast_to(np.arange(hidden.shape[1], dtype=np.int64)[None, :], hidden.shape[:2]).copy()
    indexer_type = config['indexer_types'][layer]
    mlp_type = config['mlp_layer_types'][layer]
    output, topk, trace = reference.decoder_layer(hidden, source, layer, config, positions, cache, mlp_type=mlp_type, indexer_type=indexer_type, previous_topk=previous_topk, retain_per_expert=retain_per_expert)
    attention = trace['attention']
    arrays: dict[str, np.ndarray] = {'input_hidden': np.asarray(trace['input'], dtype=np.float32), 'attention_input': np.asarray(trace['attention_input'], dtype=np.float32), 'index_selection': np.asarray(attention['topk_indices'], dtype=np.int32), 'index_scores': np.asarray(attention['index_scores'], dtype=np.float32), 'attention_output': np.asarray(attention['attention_output'], dtype=np.float32), 'post_attention_hidden': np.asarray(trace['post_attention'], dtype=np.float32), 'pre_router_hidden': np.asarray(trace['mlp_input'], dtype=np.float32), 'block_output': np.asarray(output, dtype=np.float32)}
    ...
    if mlp['kind'] == 'sparse':
        ...
        arrays.update({'router_logits': np.asarray(mlp['router_logits'], dtype=np.float32), 'topk_indices': np.asarray(mlp['topk_indices'], dtype=np.int32), 'topk_weights': np.asarray(mlp['topk_weights'], dtype=np.float32), 'topk_margin_8th_vs_9th': _router_margin(mlp['router_logits'], bias, int(config['num_experts_per_tok'])), 'shared_expert_output': np.asarray(mlp['shared_output'], dtype=np.float32), 'routed_expert_output': np.asarray(mlp['routed_output'], dtype=np.float32)})
        if retain_per_expert and 'per_expert' in mlp:
            arrays.update(_expert_cartography_arrays(mlp['per_expert'], int(config['n_routed_experts'])))
...
def _logit_lens(source: Any, hidden: np.ndarray, config: dict[str, Any]) -> dict[str, np.ndarray]:
    ...
    if not source.resident('model.norm.weight') or not source.resident('lm_head.weight'):
        return {}
    normed = reference.rmsnorm(hidden, source.tensor('model.norm.weight'), float(config['rms_norm_eps']))
...
def capture_layers(layers: list[int], *, split: str=DEFAULT_SPLIT, source_root: Path | None=None, graph: dict[str, Any] | None=None, schedule: dict[str, Any] | None=None, config: dict[str, Any] | None=None, capsule_dir: Path | None=None, retain_per_expert: bool=False) -> dict[str, Any]:
    ...
    window_ids = sorted((window['window_id'] for window in schedule['windows'] if {layer_of_organ(organ) for organ in window['organ_ids']} & set(layers)))
    table = _tensor_table(graph)
    source = ShardTensorSource(root, table)
    shards = organ_shards(graph)
    missing = sorted((shard for layer in layers for shard in shards.get(f'text_layer_{layer:02d}', set()) if not (root / shard).exists()))
    if missing:
        raise TeacherCaptureError(f'{identity} is not fully resident; cannot capture: missing {missing[:3]}')
    ids = calibration_ids(split, vocab_size=int(config['vocab_size']), tokens=CALIBRATION_TOKENS)
    ...
    if previous is not None:
        ...
        if previous.get('calibration_membership_sha256') != membership:
            raise TeacherCaptureError('previous capsule was captured on a different calibration membership')
    else:
    ...
    for layer in layers:
        hidden, previous_topk, layer_arrays = capture_layer(hidden, source, layer, config, previous_topk, cache, retain_per_expert=retain_per_expert)
    ...
    arrays.update({f'logit_lens/{key}': value for key, value in _logit_lens(source, hidden, config).items()})
    ...
    receipt = seal({'schema': SCHEMA_CAPSULE, 'capsule_id': identity, 'window_ids': window_ids, 'layers': layers, 'first_layer': layers[0], 'last_layer': layers[-1], 'repo': graph['repo'], 'revision': graph['revision'], 'graph_seal_sha256': graph['seal_sha256'], 'schedule_seal_sha256': schedule['seal_sha256'], 'reference_commit': _git_head(), 'source_shards': sorted((shard for layer in layers for shard in shards.get(f'text_layer_{layer:02d}', set()))), 'source_shard_sha256': {shard: by_path[shard]['lfs_sha256'] for layer in layers for shard in sorted(shards.get(f'text_layer_{layer:02d}', set())) if shard in by_path}, 'calibration_split': split, 'calibration_tokens': tokens, 'calibration_records': batch, 'calibration_source': calibration_source(int(config['vocab_size'])), 'calibration_vocab_size': int(config['vocab_size']), 'calibration_membership_sha256': membership, 'input_provenance': provenance, 'chain_gap_layers': chain_gap, 'index_selection_trivially_complete': tokens <= int(config['index_topk']), 'capsule_path': str(capsule_path), 'capsule_bytes': len(payload), 'capsule_sha256': hashlib.sha256(payload).hexdigest(), 'array_sha256': {name: _array_sha256(value) for name, value in sorted(arrays.items())}, 'metrics': metrics, 'payload_bytes_read': source.payload_bytes_read, 'bounded_read_calls': source.read_calls, 'capture_seconds': round(time.time() - started, 2), 'captured_at': utc_now(), 'capability_claim_permitted': False, 'note': _capsule_note(int(config['vocab_size']))})
    atomic_json(out_dir / f'{identity}.json', receipt)
    _append_ledger({'event': 'TEACHER_CAPTURED', 'capsule_id': identity, 'window_ids': window_ids, 'layers': layers, 'revision': receipt['revision'], 'source_shards': receipt['source_shards'], 'calibration_split': split, 'calibration_membership_sha256': membership, 'capsule_path': str(capsule_path), 'capsule_bytes': receipt['capsule_bytes'], 'capsule_sha256': receipt['capsule_sha256'], 'seal_sha256': receipt['seal_sha256'], 'metrics': metrics, 'input_provenance': provenance, 'eviction_authorized_shards': receipt['source_shards'], 'at': receipt['captured_at']})
    return receipt
...
def capture_window(window_id: str, *, schedule: dict[str, Any] | None=None, graph: dict[str, Any] | None=None, source_root: Path | None=None, capsule_dir: Path | None=None, **kwargs: Any) -> list[dict[str, Any]]:
    """Capture every contiguous fully-resident layer run of one window."""
    schedule = schedule if schedule is not None else _schedule()
    graph = graph if graph is not None else _graph()
    root = Path(source_root) if source_root is not None else SOURCE_ROOT
    window = next((item for item in schedule['windows'] if item['window_id'] == window_id), None)
    if window is None:
        raise TeacherCaptureError(f'unknown window: {window_id!r}')
    shards_of = organ_shards(graph)
    resident = [layer for organ in window['organ_ids'] if (layer := layer_of_organ(organ)) is not None and all(((root / shard).exists() for shard in shards_of.get(organ, set())))]
    if not resident:
        raise TeacherCaptureError(f'{window_id} has no fully resident text layer')
    return [capture_layers(run, schedule=schedule, graph=graph, source_root=root, capsule_dir=capsule_dir, **kwargs) for run in contiguous_runs(resident)]
...
def verify_capsule(identity: str, *, capsule_dir: Path | None=None) -> dict[str, Any]:
    ...
    graph = _graph()
    if graph['revision'] != receipt['revision'] or graph['seal_sha256'] != receipt['graph_seal_sha256']:
        raise TeacherCaptureError(f'{identity} lineage mismatch against the sealed graph')
    ids_expected = calibration_ids(receipt['calibration_split'], vocab_size=int(receipt['calibration_vocab_size']), tokens=int(receipt['calibration_tokens']))
    if membership_sha256(ids_expected, receipt['calibration_split']) != receipt['calibration_membership_sha256']:
    ...
    if set(arrays) != set(declared):
        raise TeacherCaptureError(f'{identity} array set differs from the receipt: missing={sorted(set(declared) - set(arrays))[:3]} unexpected={sorted(set(arrays) - set(declared))[:3]}')
    ...
    for layer in receipt['layers']:
        ...
        subset = {name[len(prefix):]: value for name, value in arrays.items() if name.startswith(prefix)}
    ...
    return {'capsule_id': identity, 'window_ids': receipt['window_ids'], 'status': 'REPRODUCED', 'layers': receipt['layers'], 'capsule_sha256': receipt['capsule_sha256'], 'arrays': len(arrays), 'metrics': reproduced, 'verified_at': utc_now()}
...
def eviction_authority(shards: Iterable[str], *, source_root: Path | None=None, graph: dict[str, Any] | None=None, capsule_dir: Path | None=None, ever_verified: set[str] | None=None) -> dict[str, Any]:
    ...
    return {'authorized': authorized, 'refused_uncaptured_but_capturable': refused, 'refused_incomplete_organs': incomplete, 'authorized_with_unrecoverable_organs': unrecoverable}

def layers_capturable_now(*, source_root: Path | None=None, graph: dict[str, Any] | None=None, capsule_dir: Path | None=None) -> list[int]:
...
def ensure_captured(layers: Iterable[int], *, source_root: Path | None=None, capsule_dir: Path | None=None, **kwargs: Any) -> dict[str, str]:
    ...
    for run in contiguous_runs(layers):
        ...
        try:
            capture_layers(run, source_root=source_root, capsule_dir=capsule_dir, **kwargs)
            verify_capsule(identity, capsule_dir=capsule_dir)
            outcome[identity] = 'CAPTURED'
        except Exception as exc:
            outcome[identity] = f'{type(exc).__name__}: {exc}'
            _append_ledger({'event': 'TEACHER_CAPTURE_FAILED', 'capsule_id': identity, 'layers': run, 'error': outcome[identity], 'at': utc_now()})
    return outcome

def capture_for_eviction(shards: Iterable[str], *, source_root: Path | None=None, graph: dict[str, Any] | None=None, capsule_dir: Path | None=None, **kwargs: Any) -> dict[str, Any]:
    ...
    """
    graph = graph if graph is not None else _graph()
    authority = eviction_authority(shards, source_root=source_root, graph=graph, capsule_dir=capsule_dir)
    blocking = {organ for organs in authority['refused_uncaptured_but_capturable'].values() for organ in organs}
    if not blocking:
        return authority
    wanted = sorted((layer for organ in blocking if (layer := layer_of_organ(organ)) is not None))
    outcome = ensure_captured(wanted, source_root=source_root, capsule_dir=capsule_dir, **kwargs)
    authority = eviction_authority(shards, source_root=source_root, graph=graph, capsule_dir=capsule_dir)
    authority['capture_outcome'] = outcome
    return authority

def write_policy() -> dict[str, Any]:
    policy = seal({'schema': SCHEMA_POLICY, 'status': 'ARMED', 'repo': 'zai-org/GLM-5.2', 'revision': IMMUTABLE_REVISION, 'pipeline': ['VERIFY', 'TEACHER_CAPTURE', 'PROBE', 'PACK', 'SEAL', 'EVICT'], 'invariant': 'A shard may be evicted only when every text-layer organ it carries is either sealed in a teacher capsule or already unrecoverable because its sibling shards were destroyed under the pre-capture policy.', 'ledger_path': str(LEDGER), 'capsule_directory': str(CAPSULES), 'calibration': {'kind': 'sealed deterministic synthetic token-id probe', 'natural_text': False, 'tokens': CALIBRATION_TOKENS, 'splits': list(CALIBRATION_SPLITS), 'disjoint_by_construction': True, 'membership_hash': 'sha256 over canonical {split, token_ids}'}, 'captured_evidence': ['input hidden state', 'attention input', 'IndexShare selection', 'indexer scores', 'attention output', 'post-attention residual', 'pre-router hidden state', 'router logits', 'top-8 indices', 'top-8 weights', '8th-versus-9th margin', 'shared-expert output', 'weighted routed-expert output', 'post-MoE state', 'post-block residual', 'logit-lens short logits over a sealed vocabulary subset'], 'bounds': {'no_unrestricted_activation_corpus': True, 'max_single_tensor_bytes': MAX_TENSOR_BYTES, 'logit_lens_vocabulary_rows': LOGIT_LENS_ROWS, 'batch': 1}, 'limitations': ['The calibration probe is synthetic token ids, not natural text, so routing statistics are not the natural-text routing distribution.', 'A window captured before its upstream layers were resident is seeded from the embedding table; its chain gap is recorded per capsule.', 'Capsules are teacher evidence for representation fitting. They are not an evaluation and permit no capability claim.'], 'capability_claim_permitted': False})
...
def status() -> dict[str, Any]:
    receipts = [{'capsule_id': receipt['capsule_id'], 'window_ids': receipt['window_ids'], 'layers': receipt['layers'], 'capsule_bytes': receipt['capsule_bytes'], 'capsule_sha256': receipt['capsule_sha256'], 'input_provenance': receipt['input_provenance'], 'captured_at': receipt['captured_at']} for receipt in sealed_capsules()]
    rows = 0
    if LEDGER.exists():
        rows = sum((1 for line in LEDGER.read_text().splitlines() if line.strip()))
    return {'capsules': receipts, 'capsule_count': len(receipts), 'capsule_bytes_total': sum((row['capsule_bytes'] for row in receipts)), 'captured_layers': sorted(captured_layers()), 'latest_capsule_seal': max((row['captured_at'] for row in receipts), default=None), 'ledger_rows': rows, 'ledger_path': str(LEDGER), 'capturable_now': layers_capturable_now() if SOURCE_ROOT.exists() else [], 'policy_sealed': POLICY_PATH.exists(), 'at': utc_now()}

def selftest() -> dict[str, Any]:
    """No network, no source, no writes: the pure invariants only."""
    ids_fit = calibration_ids('teacher_fit', vocab_size=154880)
    ids_hold = calibration_ids('teacher_holdout', vocab_size=154880)
    if np.array_equal(ids_fit, ids_hold):
        raise AssertionError('calibration splits are not disjoint')
    if not np.array_equal(ids_fit, calibration_ids('teacher_fit', vocab_size=154880)):
    ...
    return {'status': 'PASS', 'splits_disjoint': True, 'deterministic_ids': True, 'margin_is_boundary_gap': True, 'calibration_tokens': CALIBRATION_TOKENS}
