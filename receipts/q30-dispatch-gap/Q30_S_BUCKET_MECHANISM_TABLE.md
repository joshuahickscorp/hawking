...
| **Status this worktree** | **Implemented, default ON** (`HAWKING_QWEN30_DEVICE_EXPERT_TABLE` / alias `HAWKING_QWEN30_DEVICE_ROUTE`). Per-layer device address tables (128×gate/up/down gpuAddresses) + `qwen30_device_expert_table.metal` kernels index device `route_ids` without host readback. Opt-out `=0` restores host-bind control for A/B. Diagnostic/parity/HQ30GR2 modes force host path. Structural target: `host_route_id_readbacks=0`, `command_buffers≈1` per warm token. |
